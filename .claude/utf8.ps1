...
if ([string]::IsNullOrWhiteSpace($commandText)) {
    ...
    Write-Output "Usage in PowerShell: & .claude/utf8.ps1 -Command 'uv run python -X utf8 ...'"
    Write-Output "Usage from Bash/cmd: powershell -NoProfile -ExecutionPolicy Bypass -File .claude/utf8.ps1 -Command `"uv run python -X utf8 ...`""
    exit 0
}
