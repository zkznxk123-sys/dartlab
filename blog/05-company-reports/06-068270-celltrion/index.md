...
1983년 건국대를 졸업하고 삼성전기에 입사했다. 첫 직장이었다. 7년 정도 있다가 한국생산성본부로 옮겼다. 컨설팅이 주 업무였고, 그가 컨설팅한 회사 중에 대우자동차가 있었다. 1991년, 대우자동차가 그를 스카웃했다. 그가 34세 때였다.
...
다음 해인 1999년, 그는 인천 연수구청 7층의 벤처센터 작은 사무실 한 칸을 빌렸다. 자본금 5천만원, 직원 6명. 회사 이름은 **넥솔**이었다. 무엇을 만들지도 정해지지 않은, 정확히 말하면 "IT 컨설팅 회사" 정도의 사업자 등록 한 줄짜리 회사였다. 그가 본 것은 단 하나 — IMF로 한국 회사들의 자본 가치가 절반 이하로 떨어진 시기에, 새로 만든 작은 회사에는 잃을 게 없다는 것.
...
이 시기에 대해 서정진은 나중에 한 매체 인터뷰에서 더 솔직한 한 줄을 남겼다 — *"자살 시도까지 한 적이 있을 정도로 힘들었습니다."* 1999년부터 2002년까지 약 3년이었다. 한 회사의 대표가, 41세에 시작한 첫 창업 3년 안에, 자살을 시도할 만큼 몰렸던 시기.
...
## 5막 — 2023년 12월 28일, 한 결정이 만든 13.78조
...
서정진의 25년이 dartlab의 손익계산서·재무상태표·현금흐름표에 정확히 어떻게 찍혀 있는지 한 화면에 펼친다. 본문의 모든 사건이 이 세 표 안에 숫자로 들어 있다.
...
| 판관비 | 1.30 | 1.19 | 0.40 | 0.40 | 0.35 | 0.31 | 0.25 | 0.22 | 0.20 | 0.17 |
| 영업이익 | **1.17** | 0.49 | 0.65 | 0.66 | 0.76 | 0.72 | 0.38 | 0.33 | 0.50 | 0.22 |
| 당기순이익 | **1.03** | 0.42 | 0.54 | 0.54 | 0.60 | 0.53 | 0.30 | 0.24 | 0.38 | 0.15 |

전년 대비 변화: 매출 2022 → 2023 **-5%**(합병 직후 내부거래 제거 효과) → 2023 → 2024 **+63%** → 2024 → 2025 **+17%**. 영업이익 2024 → 2025 **+138%**(0.49 → 1.17조). 합병 후 비용 통합이 끝난 2025년에 마진이 한 번에 벌어졌다.

핵심 관찰: 매출원가 9년 0.28→1.70조(6배), 같은 기간 매출 0.67→4.16조(6.2배). 거의 동일 비율 — **매출총이익률이 60% 안팎으로 9년 동안 거의 일정하다**. 항체 바이오시밀러 한 종류의 단가가 변하지 않는다는 뜻. 변한 것은 판매 볼륨이고, 그 볼륨이 합병 후 통합 매출에서 두 배가 됐다.

### 재무상태표 (BS)

```python
c.select("BS", ["자산총계","부채총계","자본총계","현금및현금성자산","무형자산"])
```
...
| 현금및현금성 | 1.12 | 1.00 | 0.56 | 0.55 | 1.19 | 0.68 | 0.55 | 0.41 | 0.42 | 0.27 |
| 무형자산 | **13.78** | 13.70 | **13.34** | 1.62 | 1.49 | 1.40 | 1.01 | 0.89 | 0.79 | 0.72 |

표시: **17.35**(2025) = 자본총계 (2022 4.27조의 4배) / **13.78**(2025) = 무형자산 / **13.34**(2023) = 합병 한 분기 만에 8.5배 폭증

핵심 관찰: 2022→2023 한 해의 변화가 회사 BS의 본질을 바꿨다. **자산총계 5.89→19.92조(3.4배), 자본총계 4.27→17.13조(4배), 무형자산 1.62→13.34조(8.2배).** 같은 기간 부채총계는 1.62→2.79조(1.7배)로 비교적 작게 늘었다. 즉 합병으로 자산이 4배가 됐는데 거의 전부가 자본 + 무형자산이고 부채 효과는 작다. **회사의 본질이 공장과 설비가 아니라 약품 특허와 글로벌 판권으로 이동한 한 분기다.**
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 영업CF | 6,461 | **9,019** | 5,372 | **9** | 9,112 | 3,507 | 3,420 | 3,648 | 4,860 | 2,301 |
| 유형자산취득 | 1,082 | 1,351 | -2,150 | -3,930 | -2,148 | -2,369 | -1,413 | 184 | -4,029 | 2,243 |
| 배당금지급 | 1,538 | 1,036 | -2,166 | -2,793 | -160 | 0 | 0 | 0 | 0 | 0 |

표시: **9**(2022) = 영업CF가 9억 — 합병 전 마지막 해의 운전자본 묶임 / **9,019**(2024) = 합병 후 첫 해 영업CF 회복 / **6,461**(2025) = 정상화

핵심 관찰: 2022년 영업CF가 9억밖에 안 된 게 충격적이다. 같은 해 영업이익이 6,600억이었는데 영업CF는 9억. 약 6,591억의 격차가 운전자본(매출채권/재고)에 묶였다. 이게 합병 직전의 회사 모습이고, 5년 공매도 전쟁의 가장 결정적 정량적 빌미였다 — *"영업이익은 나는데 현금이 안 들어온다 = 그룹 내부거래 매출일 뿐이다"*. 그 다음 해 합병 후 영업CF가 한 번에 5,372억 → 9,019억으로 회복된다. 합병이 운전자본 문제를 한 번에 해소했다는 뜻.

배당금 지급 행도 흥미롭다. 2021년부터 시작 (-160억), 2022~2023 -2,793/-2,166억(소액주주 대상), 2024~2025 +1,036/+1,538억으로 부호 바뀜. 합병 후 배당 정책이 통합 정상화됐다.
...
|---|---|---|
| 2025 매출 4조 1,625억 / 영업이익 1조 1,685억 / 순이익 1조 315억 | `c.select("IS", [...])` 분기 합산 | ✅ 실측 |
...
| 무형자산 1.62조(2022) → 13.34조(2023) → 13.78조(2025) — 8.5배 | `c.select("BS", ["무형자산"])` Q4 | ✅ 실측 |
| 2025 자산총계 22.33조 (무형자산 비중 약 62%) | `c.select("BS", ["자산총계"])` Q4 | ✅ 실측 |
| 매출 시계열: 0.67(2016) → 0.95 → 0.98 → 1.13 → 1.85 → 1.91 → 2.30 → 2.18 → 3.56 → 4.16조 | `c.select("IS", ["매출액"])` 분기 합산 | ✅ 실측 |
...
# 본문 핵심 4개 호출만
c.select("IS", ["매출액","영업이익","당기순이익"])     # 매출 10년 + 합병 전후
c.select("BS", ["무형자산","자산총계","자본총계"])     # 무형자산 8.5배가 핵심
