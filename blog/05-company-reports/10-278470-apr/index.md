...
이 세 가지가 하나의 구조로 연결돼 있다. 가전이 화장품의 마진을 올리고, DTC가 유통 마진을 가져오고, 해외가 판가를 높인다 — **세 가지 모두 같은 방향, "마진 24%"를 만드는 데 기여한다.** dartlab 스코어카드에 **A가 6개** — 성장, 수익, 현금흐름, 효율, 투자효율, 재무정합. Piotroski F-Score **8/9**. ROE **65%**. K-뷰티에서 이런 스코어카드를 본 적이 없다.
...
관세는 방어했다. 그러면 진짜 리스크는 뭔가. 메디큐브가 에이피알 매출의 **90%+**. 단일 브랜드 올인.
