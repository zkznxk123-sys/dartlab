...
date: 2026-06-13
description: "매출이 FY20 $166.8B에서 FY25 $275.2B로 65% 불었다. 그런데 영업이익률은 줄곧 3%대 — 단 한 번도 4%를 넘은 적이 없다. 규모를 키우면 마진이 좋아진다는 유통의 상식이 여기서 정면으로 깨진다. 코스트코는 마진을 안 키운다."
...
youtubeId: ""
ai:
  verdict: "간판은 매출 $275.2B의 거대 창고형 마트다. 그런데 dartlab 연결 재무가 증명하는 단 하나는 — 이 회사가 마진을 '키우지 않는다'는 것이다. 매출은 FY20 166.8B에서 FY25 275.2B로 65% 불었는데, 연결 영업이익률은 FY20 3.26%에서 FY25 3.77%로 0.5%p 올랐을 뿐, 6년간 단 한 번도 4%를 넘은 적이 없다. 규모가 마진을 끌어올린다는 유통의 상식과 정반대다. 상품 마진을 일부러 3%대에 묶어 두는 회사가 영업이익 $10.4B을 찍는다 — 이 모순이 핵심이다. 연결로 단정 가능한 건 '수평에 가까운 3%대 OPM'까지이고, 그 이익의 큰 줄기가 진열대(상품 마진)가 아니라 입구의 멤버십 연회비에서 나온다는 것은 10-K(세그먼트, 외부 인용)·가설이다. 영업현금흐름 $13.3B이 순이익 $8.1B을 5B 넘게 웃도는 것은 회비 선수령 구조와 정합적인 단서이되, 감가상각·운전자본도 함께 키우므로 회비 단독의 증거로 단정하지 않는다."
  direction: 유지
  confidence: 높음
  archetype: 성장
  strengths: ["매출 6년 +65%(166.8B→275.2B)에도 OPM 3%대 유지 — 마진을 의도적으로 평평하게 묶는 일관성", "영업이익 절대액 5.4B→10.4B(+91%)로 매출보다 빠르게 성장", "영업현금흐름 13.3B > 순이익 8.1B(1.65배) — 회비 선수령과 정합적", "회원 갱신율 90%대(외부) — 낮은 마진 자체가 이탈을 막는 해자"]
  weaknesses: ["진짜 이익원(멤버십 회비)이 연결 손익 한 줄에 안 나옴 — 10-K 외부·가설 의존", "상품 사업 자체는 ~2% 초박리 — 마진을 끌어올릴 여지를 스스로 봉인", "OPM 3%대는 '실수가 거의 용납 안 되는' 구조 — 운영 리스크가 곧 이익 리스크", "회비 비중·갱신율 정확치는 연결로 증명 불가(외부 확정 필요)"]
...
> **데이터 기준**: 2026-06-13 dartlab 실측 — Costco(COST) **미국 연결(USD)** 기준, 분기 데이터를 회계연도(8~9월 결산)로 합산. 멤버십 회비의 절대액·영업이익 대비 비중은 연결 손익에 별도 라인으로 안 나오므로 **10-K(외부 인용)·가설**로 표기. ※대차대조표 항목은 매핑이 불안정해 인용에 주의.
>
...
여기서 정직해야 할 한 가지 — '상품에 14~15% 이상 안 붙인다'는 식의 구체적 가격 상한 룰은 외부 경영 일화로 널리 알려져 있지만, dartlab 연결 숫자로 증명되는 건 *비율이 안 움직인다*는 사실까지다. '일부러 안 키운다'는 의도는 가설로만 단다. 그렇다면 마진을 0 근처로 깎은 회사가 어떻게 $10.4B을 버나?
...
그 다른 줄기가 멤버십 연회비다. 회비는 매입원가도, 물류비도, 재고 부담도 거의 없는 *거의 순수익에 가까운* 줄기다. 10-K 기준(외부 인용) 코스트코의 연회비 수입은 연 약 $5B 안팎으로, 영업이익 $10.4B의 *절반에 가깝다.* 즉 진열대에서 $270B어치 물건을 거의 원가에 팔아 회원을 모으고, 그렇게 모은 회원이 매년 내는 입장료가 이익의 큰 절반을 만든다.

산수를 한 발 더 밀어 보자. 영업이익 $10.4B에서 회비(약 $5B, 외부)를 빼면, $270B어치 상품을 팔아 남긴 영업이익은 대략 $5B 안팎이다 — 상품 사업의 영업마진이 약 **2%**라는 뜻이다(외부 회비 추정에 기댄 역산이므로 정밀치는 아니다). 일반 대형마트의 절반에도 못 미치는 이 초박리가 우연이 아니라 *설계*라는 게 이 글의 핵심 가설이다. 상품에서 거의 안 남기고, 그 '거의 안 남김'을 입장료로 정산받는 구조 — 그래서 코스트코의 손익을 '마트 손익'으로 읽으면 절반을 놓친다. 절반은 진열대(2% 박리), 절반은 입구(거의 순수익 회비)다.

![입구의 회원 카드 — 이익은 진열대가 아니라 여기서 시작된다](./assets/104-membership-card.webp)

★여기가 이 글의 핵심 가드다 — **dartlab 연결 손익계산서에는 '회비'라는 별도 라인이 없다.** '회비 = 영업이익 절반'은 10-K(세그먼트, 외부 인용)이자 가설이고, 연결만으로 단정할 수 있는 건 '상품 마진을 3%대에 묶은 회사가 그래도 $10.4B을 번다'는 모순까지다. 회비가 정확히 얼마인지, 영업이익의 몇 %인지는 최신 10-K로 확인할 사항이다. 그래도 이 모순 자체가 화살표를 입구 쪽으로 분명히 돌린다. '길목에서 통행료를 걷는' 같은 결의 회사로는 국세청 의무 장부의 [더존비즈온](/blog/012510-douzone)이 있다 — 둘 다 *본업의 마진이 아니라 길목의 입장료*가 이익을 떠받친다.
...
2막에서 본 '마진을 안 키운다'와 3막의 '회비가 이익'은 사실 같은 동전의 양면이다. 마진을 3%대로 묶어 *"여기서 사면 남는다"*를 매번 증명하니까, 회원은 입장료를 내고도 떠나지 않는다. 회원 갱신율은 90%대(외부 인용)로, 이 줄기를 매년 새로 굳힌다. 낮은 마진 자체가 이탈을 막는 *해자*가 되는 구조다.
...
(인물·연혁 일화 — 창업자, '14~15% 상한 룰' 같은 서사 — 는 연결 숫자로 뒷받침되지 않으므로 본문은 갱신율 한 숫자만 외부 라벨로 쓰고 즉시 3.8%로 돌아온다.)

---

## 6막 — 칼은 한 자루, 회비는 그 칼이 가리키는 각주

**그래서 정직하게 휘두를 수 있는 칼은 무엇인가.** '수평에 가까운 3%대 OPM' 한 자루다.
...
1. **멤버십 회비 절대액과 영업이익 대비 비중** — 진짜 엔진의 크기. 최신 10-K에서 연회비 수입이 영업이익의 몇 %인지가 이 회사 가치의 토대다(외부).
2. **회원 갱신율** — 90%대(외부)가 유지되는지. 갱신율이 깨지면 회비 줄기 자체가 흔들린다.
...
본문 인용 수치를 dartlab 호출과 결과로 검증한다. 외부 출처(10-K 회비·갱신율)는 분리 표기. 📅 dartlab 실측 2026-06-13 · Costco(COST) 미국 연결(USD)·회계연도 합산 기준.
...
| 멤버십 회비 연 약 $5B = 영업이익의 절반에 가까움 | [Costco 10-K (SEC)](https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=0000909832&type=10-K) | 외부 인용·가설 |
| 회원 갱신율 90%대 | [Costco IR](https://investor.costco.com/) | 외부 인용 |
| 연회비·멤버십 구조(Gold Star·Executive) | [Costco 멤버십](https://www.costco.com/membership.html) | 외부 인용 |
| 회비=영업이익 절반 보도·세그먼트 | [Reuters](https://www.reuters.com/) · [CNBC](https://www.cnbc.com/) | 외부 인용 |
| 회비는 연결 손익에 별도 라인 없음 — 외부 의존 | dartlab 연결 IS 한계 | 방법론 |
| BS(대차대조표) 매핑 불안정 — 인용 주의 | dartlab 데이터 한계 | 주의/제외 |

본문의 숫자 중 이 표에 없는 것은 발행 차단 대상이다. 멤버십 회비의 절대액·비중·갱신율은 dartlab 연결로 증명되지 않으며 10-K 외부 인용·가설임을 명시한다.
