...
**업데이트 후 독법 14 — 연간 환산은 유혹적이지만 위험하다.** 2026년 2분기 매출을 네 배 해서 연간 매출을 추정하고 싶을 수 있다. 하지만 스타벅스의 분기별 계절성, 프로모션, 구조조정 비용, 중국 전환, 라이선스 믹스가 모두 다르기 때문에 단순 연율화는 위험하다. 특히 마진은 분기별 비용 타이밍에 크게 흔들릴 수 있다. 그래서 이 글에서는 분기 OPM과 반기 OPM을 보여 주되, 2026년 연간 결론을 미리 쓰지 않는다. 연간 글은 연간 숫자로 써야 한다. 분기는 방향을 알려 주는 신호이고, 판결은 누적 데이터가 내려야 한다.
