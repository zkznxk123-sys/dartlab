...
**5A와 2D의 공존.** 일회성(상장 비용)이 빠지면 D가 올라갈 수 있다. 그런데 **러시아 지정학 비용이 구조적이라면?** 3막에서 봤듯 러시아가 해외 매출의 29%다. 금융 제재, 물류 애로, 환율 변동이 매 분기 영업외에서 이익을 깎아먹는다면 — 이 D는 일회성이 아니라 구조적일 수 있다. 러시아 비중이 줄어야 D가 올라간다. 그게 5막의 질문이다.

---

## 5막 — 북미가 러시아를 넘는 시점이 분기점이다

4막의 D를 해소하려면 러시아 비중이 내려가야 한다. 그러려면 **북미가 터져야 한다.**

숫자로 계산해보자. 에이피알이 울타뷰티 1,400개 매장에서 2025년 미국 매출 약 5,700억을 뽑았다. 달바는 울타 200개 매장에서 시작해 **1,300개로 확대** 예정. 에이피알 매출을 매장 수 비례로 환산하면:
...
그 시점이 분기점인 이유: 북미 판가가 러시아보다 높다 → 북미 비중 ↑ = GP 마진 ↑ + 러시아 지정학 리스크 ↓ = **수익성 D → B 전환의 조건.**
...
달바의 매총이익률 76%는 **이미 글로벌 프리미엄 수준**이다. 로레알 Luxe 부문 영업이익률 25%에 도달하려면 판관비율을 56%에서 51%로 5%p 낮춰야 한다 — 매출이 7,000억→1조로 가면 스케일 효과로 가능한 범위. 다만 **단일 브랜드 + 미스트 71% 집중**이 프리미엄 전략과 맞는가는 별개의 질문. 키엘은 스킨케어 라인이 수십 개, 이솝은 핸드케어/바디/프레그런스까지 확장. 달바가 미스트 하나에서 라인 확장 없이 프리미엄 브랜드가 될 수 있는가.
