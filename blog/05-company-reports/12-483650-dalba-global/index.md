...
종목코드 483650, **달바글로벌**. 자체 공장 하나 없이 미스트 하나로 **5,000만병**을 팔고, 러시아에서 럭셔리 3위에 올라간 회사. 숫자가 먼저 말을 걸었으니, 숫자부터 따라가 보자.
...
재무제표에 이 전략이 어떻게 찍히는가:
- CAPEX 0.1% → **BS에 유형자산이 거의 없다**
- 매출원가율 24.1% → **ODM 비용만 24%, 나머지 76%가 GP**
- 판관비율 56.4% → **마케팅+물류가 비용의 대부분**

공장이 없으니 고정비가 작다. 고정비가 작으니 손익분기가 낮다. 손익분기가 낮으니 매출이 조금만 늘어도 이익이 빠르게 뛴다 — 그래서 1년 만에 매출 3배, 이익 3.7배가 가능했다.
...
**해외 비중**: 2021년 7% → 2023년 22% → 2024년 46% → 2025년 **63%**. 해외 매출 연평균 **208%** 성장.

에이피알은 미국부터 갔다. 달바는 **러시아부터 갔다.** 2022년 러시아-우크라이나 전쟁 이후 서방 브랜드(에스티로더, 로레알 등)가 러시아에서 철수했다. 그 진공을 K-뷰티가 채웠다. 달바는 러시아 최대 뷰티 리테일러 **골드애플**에서 럭셔리 카테고리 **3위**에 올랐다.
...
이유: **재고 급증**. 재고자산 479억(2024) → 662억(2025). 해외 확장 속도에 맞춰 재고를 미리 쌓는 구조 — 에이피알, HD현대일렉트릭에서 봤던 것과 같은 패턴.

**그래서 5A와 2D의 공존 의미:** 사업 구조(성장/안정/투자효율)는 최상급인데, 상장 비용 + 해외 확장 재고 비축으로 수익성/현금이 일시적으로 눌려 있다. 이 D가 다음 해에 B 이상으로 올라가는가가 이 회사의 진짜 질문이다.

---

## 5막 — 울타, 코스트코, 아마존. 북미가 터진다

달바의 다음 성장 엔진은 **북미**다.

2025년 북미 매출 비중 18%. 그런데 움직임이 심상치 않다:
- **울타뷰티**: 200개 매장 → **1,300개** 확대 예정
- **코스트코**: 29억원 공급 계약
- **아마존**: 10개 권역 직접 브랜드 스토어 운영, 미스트 베스트셀러
- **TikTok Shop**: K-뷰티 카테고리 진입

에이피알이 울타 1,400개 매장 + TikTok 1위로 미국을 터뜨린 것과 같은 경로를 달바가 1~2년 뒤에 밟고 있다. 달바가 북미 비중 30%+에 도달하면 — 러시아 의존도가 내려가면서 동시에 마진이 올라가는 구조가 만들어진다. 북미 판가가 러시아보다 높기 때문.

2026년 목표: 매출 7,000억(+35%), OPM 21%. 해외 오프라인 5,000개 매장.
...
| 지표 | 달바 | 키엘 (로레알) | 이솝 (로레알) |
|------|------|-------------|-------------|
| 매총이익률 | **76%** | ~80% (추정) | ~80% (추정) |
| 가격대 | 중~프리미엄 | 프리미엄 | 초프리미엄 |
| 유통 | DTC + 오프라인 | 백화점 + 직영 | 직영 위주 |
| 브랜드 수 | 1개 | 1개 | 1개 |

매총이익률 76%는 글로벌 프리미엄과 비슷한 수준. 가격을 올리면 매총이익률이 80%로 갈 수 있고, 그러면 영업이익률이 25%+로 올라간다. 다만 **단일 브랜드 + 미스트 71% 집중**이 프리미엄 전략과 맞는가는 별개의 질문. 키엘은 스킨케어 라인이 수십 개, 이솝은 핸드케어/바디/프레그런스까지 확장. 달바가 미스트 하나에서 라인 확장 없이 프리미엄 브랜드가 될 수 있는가.
