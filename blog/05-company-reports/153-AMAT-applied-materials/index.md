...
date: 2026-06-14
description: "반도체 다운사이클에 마이크론은 적자로 출렁이는데 어플라이드머티리얼즈의 영업이익률은 29~30% 밴드에서 평탄하다. dartlab 실측(2019~2025)으로 무엇을 파느냐가 마진의 바닥을 어떻게 가르는지, 그리고 이 표가 증명하지 못하는 한계까지 정직하게 읽는다."
...
youtubeId: ""
ai:
  verdict: "dartlab 실측(2019~2025, 미국 연결 USD, 역년 합산)으로 검증 가능한 사실은 다음이다. 어플라이드머티리얼즈의 영업이익률은 2021~2025년 28.8~30.2%의 1.4%포인트 밴드 안에서 평탄했고, 매출은 14.61B에서 28.37B로 한 해도 감소 없이 약 1.9배 성장했으며, 영업이익은 3.35B에서 8.29B로 늘었다. 같은 표 안에서 미검증으로 남는 부분도 분명하다. 첫째, 이 7년에는 매출이 감소한 해가 없으므로 '진짜 다운사이클을 29%로 통과한다'는 직접 증명되지 않으며, 데이터가 보장하는 것은 '마이크론 같은 적자 변동성이 없다'까지다. 둘째, 마진 안정의 원인으로 거론되는 장비/서비스 믹스·AGS 비중은 10-K 외부 자료라 손익으로 단정 불가다. 일회성/균열로 식별되는 항목은 2025년 순이익률이 26.4%에서 24.7%로 약 1.7%포인트 하락하고 순이익이 7.18B에서 7.00B로 줄어든 것인데, 같은 해 매출·영업이익·영업이익률은 모두 상승했으므로 원인은 영업외·세금·일회성 중 어디인지 손익 요약으로 구분되지 않아 미결로 둔다. 2020년은 매출 3분기 집계로 OPM/NPM에서 제외했다. 마이크론의 OPM −37%는 별개 회사 외부 대조 수치다. 목표주가·매수의견은 제시하지 않는다."
  direction: "긍정"
  confidence: "중간"
  archetype: "안정마진"
  strengths: ["영업이익률이 2021~2025년 1.4%p 밴드(28.8~30.2%) 안에서 평탄 — 사이클 산업에서 드문 마진 안정성", "매출 14.61B→28.37B로 7년간 한 해도 감소 없이 약 1.9배 성장하며 영업이익도 3.35B→8.29B로 동반 확대", "2023~2024 메모리 다운사이클 구간에 매출이 26.52→27.18B로 오히려 증가하고 OPM 28.8→29.0% 유지 — 마이크론(같은 구간 영업적자, 외부)과 정반대", "commodity 칩이 아니라 장비+서비스를 파는 손익 구조로 적자 변동성 부재(7년 내내 영업흑자)"]
  weaknesses: ["검증 데이터 안에 매출 감소 연도가 없어 '진짜 다운사이클 통과'는 실측으로 증명되지 않음(이 표 밖)", "순이익률 2024 26.4%→2025 24.7% 약 1.7%p 하락, 순이익도 7.18→7.00B 감소 — 영업 단은 멀쩡한데 순이익 단만 빠짐(원인 미결)", "마진 안정의 가설인 장비/서비스 믹스·AGS 비중은 10-K 외부 자료라 손익만으로 단정 불가", "영업현금흐름이 2023~2024 8.70/8.68B에서 2025년 7.96B로 내려와 매끈한 우상향은 아님"]
  keyMetrics: {revenue: 28.37, opm: 29.2, npm: 24.7, ocf: 7.96}
  dataAsOf: "2026-06-14"
---

> **데이터 기준**: 2026-06-14 dartlab 실측 — Applied Materials(AMAT) 미국 연결(USD), 분기→역년 합산. (세그먼트 등 손익 밖은 10-K/IR/언론 외부 인용).
>
...
여기서 정직하게 선을 그어야 한다. '장비+서비스 믹스가 마진을 지탱했다'는 메커니즘은 손익 요약 한 줄로 증명되지 않는다. 어떤 세그먼트(장비·서비스·디스플레이)가 얼마를 벌었는지, 서비스 부문(AGS) 비중이 얼마인지는 손익계산서가 아니라 10-K 외부 자료에 있다. 그러니 이 글이 손익으로 말할 수 있는 것은 '영업이익률이 29%대에서 평탄했다'는 결과까지이고, '장비 믹스 때문이다'는 그 결과를 설명하는 *손익 밖 외부 서사*로 보인다까지만 쓴다.
...
그래서 이 글은 주장 수위를 정직하게 낮춘다. '어플라이드는 어떤 다운사이클도 29%로 통과한다'는 것은 미래에 대한 약속이고 증명 불가능한 명제다. 이 글은 그 명제를 하지 않는다. 대신 데이터로 말할 수 있는 것은 두 가지다.
...
그러니 이 글의 정직한 결론은 이렇다. 어플라이드의 손익 구조는 '마이크론 같은 적자 변동성이 없다'까지 단정할 수 있고, '어떤 미래 다운사이클도 29%로 통과한다'는 약속은 하지 않는다. 둘을 섞으면 데이터가 보장하지 않는 곳까지 미끄러진다.
...
다만 여기서도 한 줄을 정직하게 덧붙여야 한다. 메모리 업계가 추운데 어플라이드의 매출이 오히려 늘어난 이유는, 메모리 약세를 파운드리·로직 쪽 capex와 서비스 매출이 상쇄한 *믹스 효과*로 보인다. 즉 메모리 장비가 식어도, 인공지능·고성능 로직 칩을 만드는 파운드리의 설비투자와 이미 깔린 장비의 서비스 매출이 빈자리를 메웠을 가능성이 크다. 하지만 이 분해는 세그먼트 명세(10-K 외부)에 있는 이야기이지 손익 요약으로 증명되는 것이 아니다. 그러니 '믹스가 상쇄했다'까지는 *보이며*, 그 비중과 인과는 손익으로 단정하지 않는다.
...
그 '무언가'가 무엇인지는 정직하게 미결로 둔다. 영업이익과 순이익 사이에는 영업외손익, 세금, 일회성 항목 같은 여러 줄이 끼어 있다. 그중 무엇이 2025년 순이익을 눌렀는지는 손익 요약만으로는 구분되지 않는다. 그래서 이 글은 '영업이익률은 지켰지만 순이익률은 약간 양보했다'는 사실을 병기하는 데까지만 간다. 원인을 세금이라고도, 일회성이라고도 귀속하지 않는다. 그것은 이 표의 해상도 밖이다.
...
영업현금흐름도 같은 이야기를 한다. 2019년 3.25B에서 2021~2022년 5.44/5.40B, 2023~2024년 8.70/8.68B로 올라섰다가 2025년 7.96B로 내려왔다. 절대 규모는 크게 늘었지만 매년 매끈하게 우상향한 것은 아니다. 손익의 평탄함과 현금흐름의 출렁임을 함께 보는 것이, 이 회사를 한쪽으로만 미화하지 않는 정직한 읽기다.

---

*데이터 한계 명시: (1) 검증 데이터(2019~2025)에는 매출이 감소한 해가 없으므로, 이 표는 '진짜 다운사이클 통과'가 아니라 '성장 구간의 평탄한 마진'을 직접 증명한다. (2) 2020년은 매출이 3개 분기만 집계되어 OPM/NPM 산출에서 제외했고, 영업이익(4.37B)·순이익(3.62B)·영업현금흐름(3.80B)만 유효하다. (3) 마이크론의 'OPM −37%'는 별개 회사의 외부 대조 수치로, AMAT 실측표와 섞지 않았다. (4) 세그먼트·서비스(AGS) 비중은 10-K 외부 자료이며 손익으로 단정하지 않았다. (5) 회계연도가 10월말 결산이라 역년 합산으로 재조립한 수치이므로, 회사 보고 FY 기준과 분기 경계가 다를 수 있다.*

출처: [SEC 10-K(EDGAR)](https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=AMAT&type=10-K) · [어플라이드머티리얼즈 IR](https://ir.appliedmaterials.com) · [Macrotrends 영업이익률](https://www.macrotrends.net/stocks/charts/AMAT/applied-materials/operating-margin) · [Macrotrends 매출](https://www.macrotrends.net/stocks/charts/AMAT/applied-materials/revenue) · [Reuters AMAT](https://www.reuters.com/markets/companies/AMAT.O)
