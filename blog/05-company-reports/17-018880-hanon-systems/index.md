...
| 2024 | 10.0조 | **955억** | **0.9%** | 0 |

매출이 6.87조에서 10.0조로 45% 늘었는데, 영업이익은 3,158억에서 955억으로 70% 줄었다. 매출이 올라가는데 이익이 줄어드는 회사. 이건 원가 문제가 아니다. 이자비용 문제다. 그리고 그 이자비용을 만든 건 배당이다.
...
빚이 늘면 이자비용이 늘어난다. 2024년 한온시스템의 연간 이자비용은 **2,646억원**이다. 같은 해 영업이익은 **955억원**이다. 이자비용이 영업이익의 **2.8배**다.
...
2024년 4분기, 한온시스템은 영업적자 **-988억원**을 기록했다. 1997년 외환위기 이후 **27년 만의 첫 분기 적자**다. 연간으로도 영업이익이 955억원에 그쳤는데, 이건 4분기 적자를 나머지 3분기가 메운 결과다. 4분기만 떼어내면 **-988억**이다.
...
내연기관 자동차에서 공조 시스템의 역할은 "에어컨"이었다. 냉매를 압축-팽창시켜 차 안을 시원하게 하는 것. 부품 단가는 차량당 약 **30~50만원**이다.

그런데 여기서 질문이 생긴다. **이자비용만 연 2,646억인 회사가, EV 열관리에 투자할 돈이 있는가?** 영업이익 2,718억에서 이자 2,646억을 빼면 **72억밖에 안 남는다.** 이 회사는 번 돈으로 이자를 겨우 갚고 있다. 그런데도 EV는 가야 한다 — 가지 않으면 죽기 때문이다.

![EV 부품 단가 2.5배](./assets/17-ev-opportunity.svg)

전기차에서는 다르다. 배터리는 온도에 극도로 민감하다. 배터리 최적 작동 온도는 20~40도인데, 겨울에 -10도, 여름에 40도 이상이 되면 주행거리가 급감하고 수명이 줄어든다. **배터리 열관리**가 전기차 성능의 핵심이 된 것이다. 여기에 실내 냉난방까지 합치면 EV 열관리 시스템의 부품 단가는 차량당 **80~150만원**이다. 내연기관 대비 **2~3배**다.

공조업체에게 EV는 위기가 아니라 기회다. 차 한 대당 들어가는 부품값이 2~3배로 뛰기 때문이다. 한온시스템은 이 기회를 잡기 위해 **4세대 히트펌프**를 개발했다. 2024년 기아 EV3에 세계 최초로 탑재됐다. 히트펌프는 외부 공기의 열을 가져와 배터리와 실내를 데우는 장치로, 기존 PTC 히터 대비 에너지 효율이 3배 높다. 겨울철 주행거리 감소를 30~40%에서 10% 이내로 줄인다.
...
```python
c.select("IS", ["매출액", "매출원가", "판매비와관리비", "영업이익", "이자비용", "당기순이익"], freq="Y")
```

| 항목 (억원) | 2025 | 2024 | 2023 | 2022 | 2021 |
|---|---:|---:|---:|---:|---:|
| 매출 | 108,823 | 100,176 | 95,587 | 86,889 | 75,717 |
| 매출원가 | 97,662 | 91,705 | 86,052 | 78,367 | 66,750 |
| 판관비 | 8,443 | 7,516 | 6,762 | 5,942 | 5,612 |
| 영업이익 | 2,718 | 955 | 2,773 | 2,580 | 3,355 |
| 이자비용 | 2,350 | 2,646 | 2,120 | 1,450 | 890 |
| 당기순이익 | 580 | -1,876 | 267 | 267 | 1,986 |

### BS 5년 요약

```python
c.select("BS", ["자산총계", "부채총계", "자본총계", "차입금"], freq="Y")
```

| 항목 (억원) | 2025 | 2024 | 2023 | 2022 | 2021 |
|---|---:|---:|---:|---:|---:|
| 자산총계 | 112,350 | 106,200 | 101,800 | 95,400 | 82,700 |
| 부채총계 | 79,800 | 76,200 | 70,500 | 63,800 | 52,900 |
| 자본총계 | 32,550 | 30,000 | 31,300 | 31,600 | 29,800 |
| 부채비율 | 245% | 254% | 225% | 202% | 177% |
...
| 항목 (억원) | 2025 | 2024 | 2023 | 2022 | 2021 |
|---|---:|---:|---:|---:|---:|
| 영업CF | 5,120 | 3,850 | 4,200 | 3,600 | 4,100 |
| 투자CF | -3,200 | -2,900 | -3,100 | -2,800 | -2,500 |
| 재무CF | -1,500 | -800 | -500 | 1,200 | -1,200 |
...
c.review()                              # 6막 자동 보고서
c.analysis("financial", "수익성")        # OPM 추이
c.analysis("financial", "자금조달")      # 차입금 구조
c.analysis("financial", "자본배분")      # 배당 이력
c.analysis("financial", "안정성")        # 이자보상배율
c.analysis("financial", "수익구조")      # EV 매출 비중
c.analysis("financial", "비용구조")      # 원가율 추이
c.analysis("financial", "종합평가")      # 재무 건강도

# 주가
dartlab.gather("price", "018880")
```

### 외부 출처

- [위키백과 — 한온시스템](https://ko.wikipedia.org/wiki/한온시스템)
- [뉴스웨이브 — 10년간 쥐어짠 배당](https://www.newswave.kr/news/articleView.html?idxno=516412)
- [이지경제 — 2025 실적](https://www.ezyeconomy.com/news/articleView.html?idxno=231797)
- [파이낸셜뉴스 — 4세대 히트펌프](https://www.fnnews.com/news/202408081611561497)
- [헤럴드경제 — 9000억 유증](https://biz.heraldcorp.com/article/10582305)
- [인베스트조선 — 시총 2조에 3.3조 투입](https://www.investchosun.com/site/data/html_dir/2025/09/26/2025092680058.html)
- [더벨 — 정몽원 한라공조 되찾기](https://www.thebell.co.kr/free/content/ArticleView.asp?key=201802260100049710003105&lcode=00)
- [삼성증권 — 빅배스 분석](https://www.samsungpop.com)
- [IB토마토 — 생산거점 축소](https://www.ibtomato.com)
- [전자신문 — 글로벌 공조 시장 점유율](https://www.etnews.com)

### 검증표

| 본문 수치 | 출처 |
|---|---|
| 매출 10.88조, OPM 2.5% (2025) | dartlab 실측 + 이지경제 |
| 2022 순이익 267억, 배당 1,850억 (6.9배) | 뉴스웨이브 |
| 10년 누적 배당 1.6조 | 뉴스웨이브 |
| 인수 전 배당 755억 → 인수 후 1,555억 (2배) | 뉴스웨이브 |
| 총차입금 4,000억 → 4.5조 (11배) | dartlab 실측 |
| 이자비용 2,646억 (2024) | dartlab 실측 |
| 영업이익 955억 (2024) | dartlab 실측 |
| 2024Q4 영업적자 -988억 | 다음뉴스 |
| 27년 만의 첫 적자 | 한온시스템 IR |
| 구조조정 652+608=1,260억 | 삼성증권 |
| 생산거점 20% 축소 | IB토마토 |
| 9,000억 유상증자 | 헤럴드경제 |
| 시총 2조에 3.3조 투입 | 인베스트조선 |
| EV 매출 28%, 4세대 히트펌프 | 파이낸셜뉴스 |
| 부채비율 254% (2024) | dartlab 실측 |
| 공조 세계 2위 (덴소 20%, 한온 13%) | 전자신문 |
| 인수대금 3.9조 | 위키백과 |
| 매출원가율 91.5% → 89.8% | dartlab 실측 |
| 정몽원 한라공조 되찾기 선언 | 더벨 |
| ICR 0.36배 (2024) → 1.2배 (2025) | dartlab 실측 |
