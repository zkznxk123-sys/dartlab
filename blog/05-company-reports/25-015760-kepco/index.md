...
한전의 매출원가를 뜯어보면 연료비가 60~70%를 차지한다. 나머지는 감가상각(15%), 인건비(8%), 기타(7~17%). 연료비의 구성은 이렇다:
...
2022년 한 해에만 40.4조가 순증했다. 이전 3년 순증분(18.7조)의 두 배가 넘는다. 적자 32.7조 + 이자 4.8조 + CAPEX 17.3조를 감당하려면 그만큼 빌릴 수밖에 없었다.
...
발전 6사가 전기를 만들어 한전에 **전력구매계약(PPA)** 가격으로 넘긴다. 한전은 이걸 소비자에게 판다. 여기서 구조적 문제가 생긴다 — 발전 6사의 연료비가 오르면 한전 매입 단가가 올라가는데, 판매 가격(전기요금)은 정부가 통제한다. **비용은 시장이 정하고, 매출은 정치가 정하는 구조가 여기서 만들어진다.** [두산에너빌리티](/blog/doosan-enerbility-debt-diet)가 원전 터빈을 납품하는 곳이 바로 이 한수원이다 — 한전 그룹은 에너지 산업 전체의 허브다.

```python
c.select("BS", ["사채", "장기차입금", "단기차입금"], freq="Y")
```

### ICR 시계열 — 0.92x에서 2.21x로 회복

이자보상배율(ICR)은 "번 돈으로 이자를 갚을 수 있는가"를 보여준다. 한전의 ICR 추이는 적자의 깊이를 그대로 반영한다:

| 연도 | 영업이익(조) | 이자비용(조) | ICR | 판정 |
|------|------------|------------|-----|------|
...
2024년이 특이하다. 영업이익 8.4조로 흑자전환했는데 ICR은 0.92x — **벌어도 이자를 못 갚는 상태**였다. 130조 부채가 만들어낸 이자 9.1조가 그만큼 무거웠다. 2025년에야 ICR 2.21x로 안정권에 진입했지만, 일반 기업의 건전 기준(3.0x 이상)에는 여전히 못 미친다.
...
### 적자 24조인데 현금 4조 유입 — 172조 인프라의 가짜 비용
...
발전설비의 감가상각 연한이 20~40년이다. 30년 전에 지은 원전의 건설비를 지금도 매년 장부에서 깎고 있다. 이 "가짜 비용"이 연 11~12조원. 한전 같은 자본집약형 인프라 기업에서 감가상각은 현금흐름의 최대 완충장치다. [HD현대일렉트릭](/blog/hd-hyundai-electric-grid-renaissance)이 변압기를 납품하는 송변전설비가 172조의 35%를 차지한다 — 전력망 교체 수요가 한전의 CAPEX를 구조적으로 만드는 이유다.

```python
c.analysis("자산구조")
```

### 한전의 자산 구성 — 유형자산 60%, 금융자산은 미미

한전의 재무상태표를 자산 유형별로 분해하면 일반 기업과 극명하게 다르다:

| 자산 구분 | 금액(조, 2024) | 비중 | 비고 |
|----------|-------------|------|------|
| 유형자산 | 172.0 | 60% | 발전+송변전+배전 |
| 투자자산 | 55.0 | 19% | 발전6사 지분 등 |
| 유동자산 | 38.0 | 13% | 현금+매출채권+재고 |
| 무형자산+기타 | 20.0 | 7% | 개발비+사용권자산 |
| **자산총계** | **285.0** | **100%** | |

유형자산이 자산의 60%다. 일반 제조업(20~40%)의 1.5~3배. 이 구조가 의미하는 건 두 가지다. 첫째, 감가상각이 매년 11~12조씩 자동으로 발생해서 OCF를 떠받친다. 둘째, 이 자산을 유지하려면 매년 15~17조의 CAPEX가 불가피하다. **자산이 크니까 현금이 들어오지만, 자산이 크니까 현금이 나가기도 한다.**
...
### CAPEX 매년 15~17조 — 어디에 얼마를 쓰는가

그래서 OCF는 항상 플러스. 하지만 **설비투자(CAPEX)가 매년 15~17조**다. 멈추면 정전이다. CAPEX의 내역을 분해하면:

| CAPEX 항목 | 연간 규모(조, 추정) | 비중 | 성격 |
|-----------|-----------------|------|------|
...
발전+송변전이 CAPEX의 61%를 차지한다. 이건 "투자"라기보다 "유지"에 가깝다. 30~40년 된 송전탑을 교체하지 않으면 사고가 난다. 노후 원전을 정비하지 않으면 가동률이 떨어진다. **한전의 CAPEX는 선택이 아니라 의무다.** [경동나비엔](/blog/kyungdong-navien-us-water-heater)이 보일러 교체 수요로 안정 매출을 만드는 것처럼, 전력 인프라도 교체 주기가 돌아오면 투자를 미룰 수 없다. 다만 규모가 연 15~17조라는 점이 다르다.

발전소 정비, 송전선 교체, 재생에너지 설비. OCF보다 CAPEX가 크니 FCF는 항상 마이너스.
...
### OPM 스윙 59.6%p — 한국 대기업 역사상 가장 극적인 턴어라운드
...
### FCF 3.9조로 130조를 갚으려면 33년 — 세 변수의 함수
...
한전 입장에서는 양날의 검이다. 전력 수요 증가는 매출 확대지만, 송변전 인프라 투자(CAPEX)도 늘어야 한다. 그리고 데이터센터는 산업용 요금 적용 — 한국 산업용 전기 가격이 원가보다 낮은 구조에서 수요가 늘수록 적자가 커질 수도 있다.
...
FCF가 3.9조에서 9.5조로 뛰고, 130조 부채 상환 기간이 33년에서 14년으로 줄어든다. 하지만 이건 한전만 보았을 때의 이야기다. 한국 제조업 전체의 전기 비용이 연 5.6조원 증가한다. 철강·화학·시멘트처럼 전기 집약 업종은 원가율이 1~2%p 오른다. [한화에어로스페이스](/blog/hanwha-aerospace-samsung-sold-weapons)의 방산 사업처럼 에너지 원가 비중이 낮은 업종은 영향이 작지만, 전기로(Electric Arc Furnace)를 쓰는 철강사는 직격탄을 맞는다.
...
[4막](/blog/kepco-government-writes-balance-sheet)에서 EDF와 비교했다. 지배구조도 비슷하다.
...
# 제7막: PBR 0.2배 — 세계에서 가장 싼 유틸리티의 가격
...
| 부채 130조 할인 | -30% | FCF로 부채 순감소 3년 연속 |
...
2025년 영업이익 13.5조, 순이익 약 7조. 시총 약 16조. PER 4.5배. 이건 **"시장이 이 이익이 지속된다고 믿지 않는다"**는 뜻이다.
...
배당수익률 3% = 주당 750원. 2020년 수준(1,256원)에도 못 미친다. 하지만 배당성향 30%면 실현 가능한 숫자다. 문제는 **부채 130조를 줄이면서 동시에 배당을 늘릴 수 있느냐**다. FCF 3.9조에서 배당을 떼면 부채 상환 속도가 더 느려진다. 33년이 50년으로 늘어날 수도 있다.

**한전의 밸류에이션은 재무제표가 아니라 에너지 정책이 결정한다.** PER 4.5배가 8배가 되려면 전기요금 연동제가 3년 이상 작동해야 한다. PBR 0.2배가 0.5배가 되려면 부채가 100조 밑으로 내려와야 한다. 둘 다 CEO가 할 수 있는 일이 아니다. [에코프로](/blog/ecopro-lithium-option)의 주가가 리튬 사이클에 베팅하는 것처럼, 한전의 주가는 정부의 의지에 베팅하는 것이다.
...
130조 금융부채를 줄이면서 동시에 미래 투자(송전망 40조+, 신재생 전환 수십조, 원전 해체 수조)를 감당해야 한다. 2025년 FCF +3.9조로 처음 순상환이 시작됐지만, 연 3~4조 수준의 상환 속도로는 33년이 걸린다. 전기요금 정상화가 FCF를 9~10조로 끌어올리면 상환 기간이 14년으로 단축된다. 이 속도 차이가 한전의 10년 뒤를 결정한다.

| 시나리오 | 전기요금 | LNG 가격 | 연 FCF(조) | 부채 상환 기간 |
|---------|---------|---------|----------|-------------|
| 기본 (현행 유지) | 동결 | $10~12 | 3~4 | 33년 |
| 낙관 (정상화) | +10원/kWh | $8~10 | 9~10 | 14년 |
...
|---|---:|---:|---:|---:|---:|
| 회계정책변경 | — | — | — | — | — |
| 지분법자본변동 | -59 | -63 | 1,230 | 5,095 | 3,012 |
| 기초자본 | 413,628 | 358,450 | 150,605 | 15,464 | 141,095 |
| 유상증자 | 0.0 | -18 | 0.0 | 8 | -5 |
| 현금흐름위험회피 | 0.0 | 120 | -51 | -244 | -196 |
| 연결범위변동 | -1 | 0.0 | -0.5 | 317 | 596 |
| 배당 | 0.0 | 0.0 | 0.0 | -423 | -7,806 |
| 기말자본 | 493,229 | 40,536 | 40,536 | 214,313 | 40,536 |
| FVOCI평가 | 1,439 | 1,008 | -317 | 580 | 590 |
| 해외사업환산 | 0.0 | 996 | -173 | 196 | 254 |
| 신종자본증권이자 | — | — | — | — | — |
| 신종자본증권발행 | 2,300 | — | 0.0 | — | — |
| 연결범위내거래 | — | — | -738 | -16 | 75 |
