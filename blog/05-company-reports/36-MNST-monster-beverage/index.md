...
영업레버리지(DOL, Degree of Operating Leverage, 매출 1% 변화 시 영업이익 변화율)가 2022 -0.85(지옥)에서 2025 +2.37로 올라왔다. 매출 10.7% 증가 시 영업이익이 25.3% 증가하는 구조다. dartlab 비용구조 엔진의 contributionProxy(1 − 변동비율)는 1.67 → 1.91로 상승. Alani Nu 프리미엄 믹스 + Bang 시너지 + 원자재 헷지 복귀 — 세 요소가 변동비를 깎아내며 **고정비 체질로의 변태** 가 진행되고 있다.

반면이 의미하는 또 한 면이 있다. 매출이 역성장할 때 영업이익이 더 빨리 무너진다는 뜻이다. 2022년 매출 +13.9%에도 OP -11.8% (DOL -0.85) 가 그 증거. 인플레이션이 재점화되면 Whiplash(채찍 반동)가 두 배로 온다.
...
**공장이 없으니 현금을 묶을 수 있는 곳은 운전자본뿐이다.** CAPEX/매출 2% 관통선의 이면이 여기서 드러난다. 4막에서 볼 "설비에 현금을 거의 쓰지 않는다"는 원칙은, 뒤집으면 "재고·매출채권이 조금만 부풀어도 OCF가 통째로 흔들린다"는 뜻이다. 3막은 그 흔들림의 해부다.
...
![CCC 분해 — DSO·DIO·DPO 스택](./assets/36-act3-ccc-stack.svg)
...
### "DuPont 역설 — ROE 23%인데 OPM은 회복 중인 이유"

![DuPont 역설 — ROE/OPM/Leverage 분기](./assets/36-act3-dupont.svg)

2021 ROE 38% → 2025 ROE 23%. 마진은 오히려 회복 중인데 ROE는 떨어진다. 이유는 분모다. 자기자본이 5년간 2.3배 팽창했다 ($36.1억 → $82.5억). 순이익은 38% 늘었는데 자본은 129%. 재투자처 부재의 결과다. CAPEX 2%, 배당 0%, 자사주도 2024 한 해만 폭탄. 그 동안 돈이 쌓이고 쌓였다.

레버리지 배수도 1.72 → 1.21로 빠졌다. 빚을 줄이고 자본을 늘리니 ROE는 이중으로 깎인다. dartlab 수익성 엔진이 정량으로 보여주는 것은 — 'ROE 하락'이 수익성 악화가 아니라 **성공의 저주** 라는 사실.
...
### "ROIC 워터폴 — 마진 +3.75pp, 회전 -7.25pp"

![ROIC 워터폴 — Margin vs Turnover](./assets/36-act4-roic-waterfall.svg)

ROIC(투하자본수익률) 2023 40.0% → 2025 28.4%. 3년 연속 하락이다. dartlab 투자효율 엔진이 원인을 분해한다 — 마진 효과 +3.75pp 기여, 자산회전 효과 **-7.25pp** (63.8% 설명력). 마진은 회복하는데 자본이 2배 빠르게 부풀어 회전율이 1.85 → 1.27로 무너졌다.

WACC 9.46% 대비 ROIC Spread 19%p — 한국 대기업 평균(2~5%p)의 4배. 여전히 경이적이지만 절대액 초과수익은 $11.8억 → $12.4억으로 멈췄다. **정점은 지났다**는 첫 정량 신호다.
...
> "DSO 58일 → 71일 (+22%) — 매출 인식 공격적 의심"
> ─ Damodaran "Aggressive Revenue Recognition" 기준 매치

OPM 회복, FCF 사상 최대 $19.7억. 표면은 완벽하다. 그런데 같은 해 재고는 17.7% 감소 (DIO 103일 → 80일). 이 조합 = "재고를 채널로 밀어낸 매출" 시그널이다. 2026~27년 매출 역성장 가능성을 지금 숫자가 미리 알려주고 있다.
...
보조 지표 한 줄로: 부채비율 4.5% · 유동비율 2.8배 · 순현금 $2.02B ($2.77B − 총차입 $748M) · OCF/총차입 2.81배 · CHS 부도확률(Campbell-Hilscher-Szilagyi 2008) 0.01% 미만. 2024년 첫 차입 $374M으로 "부채 0" 체제는 깨졌지만, 신용 기준 최상위는 흔들리지 않았다. 신용 위험은 사실상 없다.
...
> "사이클 정규화: mid-cycle FCF 적용 (최근 대비 5.2배 괴리)"
> "추정재무제표 (Pro Forma) 기반 FCF 사용 (3년 원본 + 연장)"
...
| ROE 2021 38% → 2025 23%, leverage 1.72→1.21 | `c.analysis('financial','수익성')['returnTrend']['history']` | ✅ 자체 출력 인용 |
| ROIC 워터폴: 마진 +3.75pp, 회전 -7.25pp | `c.analysis('financial','투자효율').roicWaterfall` | ✅ 자체 출력 인용 |
...
| DOL 2022 -0.85 → 2025 +2.37, contributionProxy 1.67→1.91 | `c.analysis('financial','비용구조').operatingLeverage` | ✅ 자체 출력 인용 |
