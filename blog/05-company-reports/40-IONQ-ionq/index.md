...
3. **영업활동현금흐름 (OCF)**: 가장 정직. 2025 -$283M
...
**5막의 결론: IonQ의 분기 순이익은 회계 노이즈다. warrant $2B 규모가 주가와 반대 방향으로 손익을 만든다. 진짜 지표는 영업이익, Adjusted EBITDA, OCF. 셋 모두 2025년에 악화됐다. 하지만 CEO는 바뀌었고 M&A는 공격적이다 — 6막이다.**
...
### 체크포인트 2: 영업현금흐름(OCF) 악화 여부

2025 OCF -$283M이 2026에 어떻게 변하는지. 매출이 $235M으로 +81% 커지는 동안 OCF가 -$283M에서 얼마나 개선되느냐가 **"규모의 경제"가 시작되는 신호**.
...
기준: **2026 OCF가 -$200M 이내로 개선**되면 긍정, 악화되면 자본 태우기 가속.

### 체크포인트 3: Warrant liability 잔량 + 주가 민감도

2025 말 warrant liability $2B 규모. 주가 ±20%에 분기 손익 ±$500M 변동. 투자자가 봐야 할 것은 **warrant 공정가치 손익이 아닌 영업이익과 OCF**.

체크 방법: 분기 실적 발표 때 GAAP 순이익과 Adjusted EBITDA 차이 확인. 차이가 클수록 warrant 영향 큼.

### 체크포인트 4: 추가 자본 조달 시점

현금 $1,030M / OCF -$283M = 3.6년. 2028년경 추가 자본 조달 필요. **Heights Capital이 3차 투자를 할지, 아니면 다른 투자자가 나설지**가 희석 정도를 결정.
...
- `수익성`: OPM -487%, 매출총이익률 71% → 고정비 과다 확인 ✅
- `성장성`: 매출 CAGR +290%, "외형 위주" → 성장 질 낮음 확인 ✅
- `이익품질`: Beneish M -0.16 (임계 초과), 매출채권 +278%p 매출 성장 초과 → warrant + 인식 플래그 ✅
- `안정성`: 부채 +3,919%, Altman Z 1.20 "부실 위험" → 레버리지 경고 ✅
- `투자효율`: ROIC -22.8% 3년 연속 → 자본 태우기 검증 ✅
- `현금흐름`: OCF -$283M, 5년 연속 음수 → 태우기 속도 확인 ✅
...
**9막의 결론: IonQ의 2026년은 "양자 컴퓨팅이 기업이 되는 해"인지 "계속 IR인 해"인지 결정된다. Tempo 출하, OCF 개선, warrant 희석, 경쟁사 격차, 한국·정부 계약 진도 — 다섯 축을 분기마다 추적하면 이 전환이 실재하는지 판단 가능. 숫자가 없는 기술 회사는 아직 기업이 아니다.**
...
| 영업손실/매출 4.87배 | `c.analysis("financial","수익성")["marginWaterfall"]` | 실측 OPM -487.4% |
