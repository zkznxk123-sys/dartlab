...
영업 레버리지(DOL, 매출 변동 대비 영업이익 변동 배수)가 **20배**다. 매출이 1% 늘면 영업이익이 20% 늘어나는 구조다. 반대로 매출이 5% 줄면 영업이익이 100% 날아갈 수 있다. 이 구조는 **양날의 검**이다.
...
| OCF/NI | **126%** | 154% | 194% | 119% | 115% | 83% | 20% | - |

**표시: 2023년 이후 OCF가 NI의 1.2~1.9배. 이익이 진짜 현금으로 들어온다.**

보통 이익보다 현금이 적은 회사가 문제다 — 이익은 회계 숫자이고, 현금은 실제 돈이니까. 삼성바이오로직스는 반대다.

### 감가상각비의 역설 — 비용이지만 현금이 안 나가는 돈

2018년, 삼성바이오로직스가 2공장 배양기를 설치했다. 수백억원짜리 스테인리스 배양기 한 기의 내용연수는 20년 이상이다. 올해 장부에 찍히는 감가상각비 3,597억원은 과거에 이미 지불한 공장 건설비를 매년 나누어 비용으로 인식하는 것이다. **현금은 이미 나갔고, 올해 추가로 나가는 돈이 아니다.** 순이익을 계산할 때는 비용으로 빠지지만, 현금흐름을 계산할 때는 다시 더해진다. 공장이 클수록 감가상각비가 크고, 감가상각비가 클수록 OCF와 NI의 격차가 벌어진다. 이것이 OCF > NI의 핵심 원인이다.

![현금흐름 진화 — 2023년 전환점에서 성숙형으로](./assets/41-cashflow-timeline.svg)

### 잉여현금흐름 — 투자를 빼고 진짜 남는 돈

| 연도 | OCF(억원) | CAPEX(억원) | FCF(억원) | CAPEX/매출 |
|:---|---:|---:|---:|---:|
...
잉여현금흐름(FCF, 영업현금에서 투자비를 뺀 진짜 남는 돈)은 2018년 -3,169억원에서 2025년 **+7,727억원**으로 전환됐다. 2023년이 전환점이다 — 이때부터 OCF가 CAPEX를 안정적으로 초과하기 시작했다. 4공장 투자가 마무리되면서 "확장형"에서 "성숙형" 현금흐름으로 바뀐 것이다.
...
| 체크포인트 | 왜 중요한가 |
|:---|:---|
| 4공장 가동률 추이 | 가동률이 올라가는데 OPM이 안 올라가면 → 천장 신호 |
| 신규 수주 파이프라인 | ADC(항체약물복합체), mRNA 등 차세대 모달리티 수주 비중 |
| CAPEX/매출 비율 | 30% 이하로 안정되면 → 성숙기 진입 확인 |
...
| 2025 OCF 2.25조 | `c.select("CF", ["영업활동현금흐름"])` 분기 합산 | ✅ 실측 |
...
| 영업레버리지 20배 | `c.analysis("financial", "비용구조")` DOL | ✅ 실측 |
...
| FCF 7,727억(2025) | OCF - CAPEX 계산 | ✅ 실측 |
