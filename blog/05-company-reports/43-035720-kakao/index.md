...
![카카오 핵심 구조 — 3,000만 DAU를 자회사 수십 개로 수익화한 구조가 OPM 한 자릿수의 원인](./assets/43-hero-map.svg)
...
카카오의 매출은 2018년 2.42조원에서 2025년 8.10조원으로 8년간 3.3배 성장했다. 연평균 성장률(CAGR) 약 16%. 한국 인터넷 기업 중 네이버와 함께 가장 안정적인 성장 궤적이다.
...
### 플랫폼 부문 OPM 20%+ vs 연결 OPM 9% — 격차의 원인
...
![영업이익률 8년 시계열 — OPM 11.1% 정점에서 5.8% 바닥까지](./assets/43-opm-timeline.svg)

### 2020년 OPM 11.1% — 코로나가 만든 착시 정점
...
### 2021~2024 OPM 9.9% → 8.4% → 7.8% → 5.8% — 4년 연속 하락
...
### 네이버 OPM 18% vs 카카오 OPM 9% — 정확히 2배의 격차
...
|:---|---:|---:|---:|---:|---:|
| 영업활동현금흐름(OCF) | **1.40** | 1.15 | 0.89 | 1.05 | 1.22 |
| 투자활동현금흐름 | -0.57 | -0.13 | -2.50 | -1.95 | -2.83 |
| 재무활동현금흐름 | -0.63 | -0.85 | 1.65 | 0.82 | 1.42 |

**표시: 2023년 투자CF -2.50조 = SM 인수 집중. 2025년 OCF 1.40조로 정상 수준 회복.**
...
1. **연결 OPM 분기 추이** — 2025년 연간 9.0%, 분기별 10% 돌파 여부
2. **SM엔터 단독 흑자 전환** — 연결 마진의 최대 변수
3. **순차입금 감소 속도** — OCF 1.4조 대비 상환 진도
...
![네이버 vs 카카오 비교 매트릭스 — 같은 플랫폼인데 OPM이 2배 차이](./assets/43-naver-comparison.svg)

### ROE 3.4% — 자본 대비 수익률이 은행 예금 수준
...
자기자본수익률(ROE, 주주 돈 대비 벌어들인 이익 비율) 3.4%. 자본 15.2조를 굴려서 순이익 0.52조. 은행 예금 금리와 비슷한 수준이다. [크래프톤](/blog/krafton)(ROE 약 15%)과 비교하면 같은 인터넷/콘텐츠 기업이라도 자본 효율이 현저히 낮다.
...
![2026년 체크포인트 — OPM 10% 돌파 여부를 판단하는 5가지 지표](./assets/43-checkpoint.svg)
...
| OPM 2020 정점 11.1% | `c.select("ratios",["영업이익률 (%)"])` 연간 합산 | ✅ 실측 |
| OPM 2024 바닥 5.8% | `c.select("ratios",["영업이익률 (%)"])` 연간 합산 | ✅ 실측 |
| OPM 2025 9.0% | `c.select("ratios",["영업이익률 (%)"])` 연간 합산 | ✅ 실측 |
...
| OCF 1.40조 (2025) | `c.select("CF",["영업활동현금흐름"])` 분기 합산 | ✅ 실측 |
| 순차입금 4.2조 | `c.analysis("financial","자금조달")` capitalOverview | ✅ 실측 |
| dCR-AA, health 84 | `c.credit("등급")` | ✅ 실측 |
| ROE 3.4% | `c.analysis("financial","수익성")` penmanDecomposition | ✅ 실측 |
