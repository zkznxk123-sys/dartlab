---
title: Company.network capability view
skillId: capability:Company.network
category: capability
---

# Company.network capability view

공개 capability `Company.network`를 찾고 실행에 연결하기 위한 generated skill view.

## Metadata

- id: `capability:Company.network`
- category: `capability`
- kind: `generated`
- status: `observed`
- Pyodide: `unknown`
...
- "계열사 관계도" → c.network() 또는 c.network().show() "같은 그룹 계열사" → c.network("members") "출자/지분 구조" → c.network("edges") "순환출자 있어?" → c.network("cycles")
