...
>
> **기각 근거**:
> - `engines.search` § "왜 임베딩 없이 되는가" — "어휘가 고정되어 있으므로 ngram 정확 매칭만으로 충분. 임베딩의 장점인 의미 유사도 검색이 이 도메인에서는 오히려 노이즈(실험 105 확인)"
> - `engines.search` § "역할 분리" — search=공식 용어 (ngram+BM25F), AI=자연어 이해. 임베딩 scope 추가는 이 경계 침범.
...
- `engines.search`: scope "title" (ngram) · "content" (BM25) 존재, `/search` API 는 beta, AI 도구 우선 사용 비권장.
...
1. `engines.search` 는 "임베딩 없이 되는 이유" 를 명시 — DART 공시 어휘 고정성으로 ngram 정확 매칭이 의미 유사도보다 우월. 임베딩은 노이즈 유발.
2. `engines.search` § "역할 분리" — search 엔진은 공식 용어, 자연어 이해는 `dartlab.ask()` 담당. semantic scope 추가는 이 경계 침범.
