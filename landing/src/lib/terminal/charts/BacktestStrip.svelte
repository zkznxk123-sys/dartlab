<script lang="ts">
	...
	let { result, presetLabel, period, withCosts, lang, onClear }: Props = $props();
...
<div class="btStrip" class:btLag={!beats}>
	...
	<div class="btKpis mono">
		...
		<span>MDD <b class="tDn">{m.mddPct.toFixed(1)}%</b><i class="btSub">({T('보유', 'B&H')} {result.bh.mddPct.toFixed(1)}%)</i></span>
	...
	{#if showTrades && result.trades.length}
		<div class="btTrades">
			<table class="btTable mono">
				<thead><tr><th>{T('진입', 'entry')}</th><th class="r">{T('진입가', 'px')}</th><th>{T('청산', 'exit')}</th><th class="r">{T('청산가', 'px')}</th><th class="r">{T('수익률', 'ret')}</th><th class="r">{T('보유일', 'days')}</th></tr></thead>
				<tbody>
					{#each result.trades.slice().reverse() as t (t.entryT)}
						<tr>
	...
	<div class="btFoot">
		...
		· {T('배당 미반영 · 무수정주가', 'dividends excluded · unadjusted prices')} │ {GOV_ATTRIBUTION}
	</div>
</div>
