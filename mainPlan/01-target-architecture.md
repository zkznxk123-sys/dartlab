...
dartlab/
  ...
  landing/                # 공개 콘텐츠 앱 + 영구 public shell (GitHub Pages)
    src/
      routes/             # blog/docs/legal/static + 제품 route wrapper (01 §4.1 전수 지도의 전 제품 route)
...
- **`ui/web` 제거(단계-11) 전 svelte 메이저 업그레이드·신문법 도입 금지** — 파일경로 alias 소비는 surfaces 원소스를 ui/web의 구식 컴파일러(vite-plugin-svelte 5)에 결합시킨다. 메이저 업그레이드는 단계-11 이후의 별도 작업 단위.
...
ui/packages/
  ...
  surfaces/
    ...
    src/
      index.ts
      terminal/    # /terminal/[code] · TerminalSurface · runtime.{company,price,filing,finance}
      viewer/      # /viewer/company/[code] · /compare · ViewerSurface · runtime.{filing,viewer}
      company/     # terminal/viewer 연계 · CompanySurface · runtime.company
      scan/        # /scan · /screener · ScanSurface · runtime.scan (DataExplorer·SQL노트북·ScreenBuilder)
      map/         # /map · /industry/[code] · MapSurface · runtime.map
      search/      # /search · SearchSurface · runtime.search
      ask/         # 로컬 /chat + 뷰어/터미널 drawer · AskSurface · runtime.ai
      charting/    # ChartSpec → ChartRenderer 단일 경로 (surface 간 공용 부품)
      evidence/    # 근거·출처 패널 (surface 간 공용 부품)
...
5. alias 추가 작업 단위와 제거 작업 단위를 07 원장에 기록한다.

허용 예:

```ts
// 단계-4b 윈도우 동안만 허용, 윈도우 종료 작업 단위에서 제거 — 방향은 항상 packages를 본다
'@dartlab/migration-terminal-surface': '../../ui/packages/surfaces/src/terminal'
```
...
### 8.1 이름 대칭 규칙 — 헷갈림 차단의 본체

작업면 이름 한 단어가 모든 계층을 관통한다. 트리만 보고 모든 것의 위치를 알 수 있어야 한다.

```text
/scan (route) → surfaces/src/scan/ (폴더) → ScanSurface (컴포넌트) → runtime.scan (port) → scan.* (service command id) → 플랫폼(단계-N) scan (커밋·원장)
```

1. **한 작업면 = 한 폴더.** 공개 API는 그 폴더의 `index.ts` 하나다.
2. 두 작업면이 한 폴더를 공유하지 않고, 한 작업면이 두 폴더에 분산되지 않는다. 공용 부품(charting, evidence)만 예외로 별도 폴더를 가진다.
3. surface 폴더 내부 형태 표준 (전 작업면 동일): `index.ts`(공개 표면) / `XxxSurface.svelte`(루트 컴포넌트) / `components/`(서브 컴포넌트) / `lib/`(로직·셀렉터·정규화). 내부 깊이 ≤ 2.
4. **새 작업면 추가 레시피 (항상 동일한 5수 + 마감 3종)**: ① contracts 타입 → ② runtime port 1 + adapter 2(public/local) → ③ surfaces 폴더 1 → ④ landing route wrapper → ⑤ local route wrapper. 마감 3종 = conformance fixture + service command 등록(해당 시) + §4.1 전수 지도 행 추가. "이건 어디에 두지?"라는 질문이 생기면 설계 위반 신호다.
5. **route 없는 기능은 작업면이 아니다** — 기존 surface 내부 기능으로 들어간다(예: 백테스팅 리포트 = terminal 내부, `surfaces/terminal/lib/backtest*`). 새 surface 신설은 독립 route를 가질 때만.
6. **의도된 비대칭 2건만 허용 (추가 금지)**: ① 보조 route는 주 작업면 폴더에 속한다 — `/screener`는 scan의 빌더 진입점(ScanSurface), `/compare`·`?vs=`는 viewer의 비교 모드(ViewerSurface). ② ask 작업면의 port는 `runtime.ai`다 — 작업면 이름(ask)=대화 UI, port 이름(ai)=능력 공급원.
7. **공용 부품 신설 게이트** — charting·evidence 외 3번째 공용 부품은 ① 2개 이상 surface 실소비 실증 ② 도메인 명명(기능명 — shared/common 금지) ③ design primitive 선판정(시각 원자=design, 도메인 위젯=surfaces 공용 부품) ④ 07 원장 entry, 전부 충족 시에만. charting 내부는 차트 종류별 소유 surface를 주석으로 박는다.
