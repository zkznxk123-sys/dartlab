...
4. **새 작업면 추가 레시피 (항상 동일한 5수)**: ① contracts 타입 → ② runtime port 1 + adapter 2(public/local) → ③ surfaces 폴더 1 → ④ landing route wrapper → ⑤ local route wrapper. "이건 어디에 두지?"라는 질문이 생기면 설계 위반 신호다.
