...
★ 6-0a(FilingPort 공개 배선) **생략 확정** — 6-1 착수 직전 재grep(플랜 [22-b] 명시 허가) 결과: 포트 filing.panel*
  실소비자 0(ViewerStudio·lab/viewer-* 3라우트 전부 `loadPanelBundle` 직접 소비 = full bundle gridBySection Map
  랜덤접근, compare/search/ask 가 section 단위 포트로 분해 불가). 유일 소비처 = test fake runtime 자기검증. →
  publicFilingPort panel* throw 게이트 유지(트립와이어), 뷰어는 panelLoad/panelWide **동반 이동**해 @dartlab/
  ui-runtime/data/hfRange 로 **자급**(C8, 양 앱 다 브라우저). bundleToContract 매퍼 불요.

단계-6(6-1) = 공시뷰어를 `ui/packages/surfaces/src/viewer` 로 승격(이동 원자 윈도우, 단일 커밋, green 중간상태 불가).
  census 15,219 LOC(컴포넌트 14 + $lib/viewer/* 45) — [22]/[22-b] 가 실행 SSOT(재조사 불필요). 결합수술: C1 basePath=
  prop·C2 onNavigate·C3 links=prop·★duckdb provideDuckDb seam(financeQuery)·R1 terminalShell lazy import·R3 webllm Worker
  원자동반·R6 prerender route([stockCode])·lab/viewer-* 3 재배선·buildIntentModel.py:35. 게이트(싼것부터): runtime tsc →
  surfaces check → landing check(이동안전 oracle) → local build → ui/web build → checkDevIsolation.
  ⚠ landing 풀 prerender 로컬 환경한계(HF seed 미보유 404) — 로컬 게이트 = check/compile/build, 풀 prerender 는 CI 권위.
이후: 단계-6 후 → 단계-7 완성(viewer AskDrawer + terminal AI command 를 AiPort 뒤로 통합) → 단계-8(scan/map/search 추출,
...
### [23] 단계-7 챗모드 기본 완료 + 6-0a 생략 확정 (재우선순위 — /goal 명시 종점 우선)
일시: 2026-06-13
commit: c94eff4e7
**재우선순위 근거**: /goal 갱신이 "챗모드와 터미널모드 완성"을 명시 종점으로 지정. 터미널 모드는 완료, 챗모드는
  placeholder(코드입력→리다이렉트)였음 = 최대 갭. 챗모드(단계-7 로컬)는 이미 배선된 AiPort(5-2b)로 저위험·고가치·
  목표 직결인 반면 단계-6(viewer 15k LOC 이동)은 최고위험·명시 종점 밖 → **명시 가치를 먼저 락인 후 단계-6/8로
  "전부 완성"**. 우회 아님(단계-6 이후 실행). 플랜 순서 6→7 은 AskDrawer 공유(단계-7 완성분)에 한해 6 의존 — 로컬
  /chat 기본 대화는 viewer 무의존.
**★6-0a 생략 확정**: 6-1 착수 직전 재grep(플랜 [22-b] 명시 허가 "기타 신규 importer 재grep 확정") 결과 —
  `.panelToc/.panelInit/.panelGrid` 포트 실소비자 0. ViewerStudio.svelte:118·lab/viewer-{dev,search,analyze} 3라우트
  전부 `loadPanelBundle` **직접** 소비(full PanelBundle, gridBySection Map 랜덤접근으로 compare/search/AskDrawer 구동).
  포트 panel* 소비처는 createFakeRuntime 자기검증뿐. → 뷰어를 section 단위 포트로 쪼갤 수 없음 = bundleToContract
  매퍼 + shared 주입 불요. 정공 = panelLoad/panelWide 를 surface 와 **동반 이동**(C8), @dartlab/ui-runtime/data/hfRange
  로 자급(양 앱 다 브라우저). publicFilingPort panel* throw 게이트는 트립와이어로 유지. [22-b] 의 6-0a 가정(뷰어가
  포트 소비)은 실코드와 불일치 — 재grep 이 정정(append-only, 정정은 새 entry).
변경 파일: ui/apps/local/src/lib/chat/chatSession.svelte.ts(신설 — ChatSession runes 클래스: messages/capabilities/
  busy/code $state, send=streamAsk 이벤트 누적[TEXT delta·TOOL_CALL_START/RESULT refDetails·ACTIVITY·RUN_FINISHED
  suggested·RUN_ERROR], 인덱스기반 proxy 변형으로 reactive 보장)·routes/chat/+page.svelte(placeholder→실 대화 UI:
  헤더 tier 배지+종목컨텍스트, 메시지 스트림 자동스크롤, 사용자/어시 버블·activity칩·근거칩·추천질문칩·에러,
  composer textarea Enter전송[IME가드]·Shift+Enter)·routes/+page.svelte(인덱스 stale 안내 갱신).
검증 (로컬 무중단): ui/apps/local svelte-check 0에러 415파일(21 warning=기존 terminal a11y, 챗 무에러) ✓ /
  ui/apps/local build green(adapter-static SPA, 4.62s) ✓ / landing·ui/web·runtime 무접촉(챗은 로컬앱 전용 신규).
  ※ 라이브 AI 응답(streamAsk SSE)은 dartlab ai 서버(/api·8400) 구동 필요 — build/check 가 로컬 게이트(단계-5 동일
