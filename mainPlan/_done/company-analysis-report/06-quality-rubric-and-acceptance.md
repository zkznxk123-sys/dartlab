...
품질 점수 = **발행 게이트**(장식 아님). bake 가 굽고 게이트가 발행/스킵을 결정한다. 화면엔 **한계 라벨만**(점수 숫자 비노출).

## 2. Reject-Gate (하드 차단 — 통과 못하면 점수 계산 전 발행 거부)

| 게이트 | 차단 | 성격 |
|---|---|---|
| **G1 근거-고아** | 핵심 claim 이 evidenceIds + sourceEngine 라벨 **둘 다 0**(근거 토대 없는 단정). ★풀 rcept 회로 미구현이므로 1차 검사 대상 = sixAct evidenceIds + sourceEngine 누락. "evidence 토대 0인 핵심 claim 주입 → G1 reject" **red 테스트로 죽은게이트 방지** | 범위 정합·우회불가 |
| **G2 NEVER-CLAIM** | "세계급/유일/최고/최강/압도적/종합점수/레이더/총점/매수/매도/목표주가/강력추천" — 소스 토큰 grep + **생성 출력문자열 claimGuard** 이중게이트(둘 다 신규, tests/audit 에 0개 실측) | 범위 정합·우회불가 |
...
| **G6 날조 URL** | dartlab.io/.com 등 환각 도메인 | 범위 정합·우회불가 |
| **G7 판정어휘** | C-2 변환표에 없는 판정형용사("양호/주의" raw)가 conclusion·본문에 등장 | 범위 정합·우회불가 |
...
| ④ 결손명시 | 16 | 결손 = 한계 라벨 명시, publishablePerspectives 미충족 관점 dim, 은폐 0 |
...
★축②가 무게중심(22)인데 1차 천장을 부분 실재회로(evidenceIds + sourceEngine) 위에서 표면화율에 비례해 올린다 — 설계 사상("갇힌 계산의 표면화")과 정합. 풀 회로(rcept)는 후속 트랙으로 천장을 올린다.
...
| < 75 | **발행 거부** | "데이터 부족 — 보고서 미생성(사유)" (약한 발행 < 발행 거부) |

★**점수 자체는 화면에 노출하지 않는다.** 하단 quality 섹션 = 한계 라벨(생략 섹션·미도달 축)만. acceptance 의 "점수 공개" 항목은 삭제(behavior 자기평가 노출금지 준수).
