...
**남은 트리 부채**: ① loadJson dedup(위 잔여). ~~② 폴더 미완~~ → **✅ P4 완성**(아래).
...
## ✅ 잔여 종결 (2026-06-17, "정공법이면 밀자" — 설계 패널 2기)

운영자 "정공법이면 밀자" → 잔여를 설계 패널(설계자 A + 적대 B)로 검토 후 처리. **핵심 반전: "loadJson 을 코어로
접는 게 정공법"이라는 전제가 틀렸다.** 적대 B 가 깨짐 지점을 정확히 짚음:
- 코어 origin `resolve:(path)=>string` **단일-URL 불변**이 워크벤치 핵심 가치 — loadJson 은 본질이 다중-URL(local↔HF
  폴백·preferLocal·hasHfLandingJson 라우팅) → 접으면 origin 2중 인격, 4 origin 추상 훼손.
- `base`(setStaticBase)는 **프로세스 전역 가변** — 코어는 의도적으로 "전역 싱글턴 금지". 접으면 *죽인 전역 부활*.
- `scope:'persist'` 유일 고객(landing JSON)은 이미 cacheStore 를 **2-tier(fresh→stale)**로 직접 써서 더 풍부 → 코어
  persist 는 다운그레이드. per-path 신선도 예외(macro/ecosystem)는 per-origin 불가. scan worker 손복사 loadHfJson 미도달.

**→ 정공법 = 진짜 결손만 메우고 죽은 선언은 지운다 (커밋 798d28039):**
- ✅ **loadJson in-flight dedup**(RequestDedup 1개, resolveJson 사다리를 감쌈). 동시 동일 자원이 cacheStore 읽기·fetch
  를 1회만 공유 — 옛 무-dedup 구멍(소비자 Map 이 메우던 이유) 해소. 시그니처·15 소비자·폴백순서 불변.
- ✅ **죽은 선언 제거**: `CachePolicy.scope` `'persist'`(선언만·미구현) → `'memory'|'none'`. `OriginId` `landingJson`
  (타입·주석만·미배선) 제거. "선언만 하고 안 살리는 작업대"는 이 프로젝트가 죽이려던 안티패턴 — 안 살리면 지우는 게 맞다.
- ✅ **dual-SSOT 최종 확정**: DataCore=parquet/HF-binary arm / loadJson=영속 JSON arm. negative-cache Map 유지(코어 계약 밖, 정당).
- 검증: runtime tsc·surfaces 0·landing svelte-check 0 errors·vitest 5/5·가드 PASS.
- B "마음 바꿀 조건": landing JSON 이 단일 canonical URL(HF 전용·local 폴백 제거)이 되면 그때 fold 가 진짜 정공법. dev
  동작 변경이라 별건 — 지금은 dual-source 자산을 그대로 반영한 dual-SSOT.

### ~~잔여~~ (위에서 종결 · 히스토리 보존)

- ~~**loadJson in-flight dedup**~~ → ✅ 위.  ~~landingJson/persist 미래 wave~~ → ✅ 죽은 선언 제거(접지 않음 확정).
- **relations·industryPool**: dual-SSOT 결론대로 *현 상태가 정답*. 코어 강제 이관 안 함.
