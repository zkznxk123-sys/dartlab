...
- 코어 origin `resolve:(path)=>string` **단일-URL 불변**이 워크벤치 핵심 가치 — loadJson 은 본질이 다중-URL(local↔HF
  폴백·preferLocal·hasHfLandingJson 라우팅) → 접으면 origin 2중 인격, 정직한 4 origin 추상 훼손.
...
- ✅ **죽은 선언 제거**: `CachePolicy.scope` `'persist'`(선언만·미구현) → `'memory'|'none'`. `OriginId` `landingJson`
  (타입·주석만·미배선) 제거. "선언만 하고 안 살리는 작업대"는 이 프로젝트가 죽이려던 안티패턴 — 안 살리면 지우는 게 정직.
...
- B "마음 바꿀 조건": landing JSON 이 단일 canonical URL(HF 전용·local 폴백 제거)이 되면 그때 fold 가 진짜 정공법. dev
  동작 변경이라 별건 — 지금은 dual-source 자산의 정직한 거울이 dual-SSOT.
