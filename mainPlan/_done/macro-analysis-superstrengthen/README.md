...
1. [01-superstrengthen-prd.md](01-superstrengthen-prd.md) — **완전 PRD**(비전·묻어둔 엔진 진단·표면화 축 선별·데이터 파이프라인 강화[sync→HF→prebuild]·UI progressive disclosure·시각화 품질·한계 가드·영향 파일/함수·테스트/롤백·Phase·이중평가·성공/실패 기준). 자기충족적.
...
| 문서 | 역할 | 본 초강화와의 관계 |
|---|---|---|
| [`macro-lens-dialog/`](../macro-lens-dialog/) | 엔진(transmission)·데이터 계약·시각화 조사·설계 원칙 SSOT | **계승** — 엔진 산출 형태(읽기만)·analyzeTransmission·sectorTailwind 그대로 소비 |
...
| **`macro-analysis-superstrengthen/`** (본 폴더) | 묻어둔 분석 *깊이*를 직접 배선 | 재설계 위에 progressive disclosure로 깊이만 추가. 새 메커니즘·verdict·단일 macro score·13섹션 과부하 절대 금지 |

즉 세 문서는 한 줄로 이어진다: **macro-lens-dialog(엔진·데이터)= 무엇을 가졌나 → macro-lens-redesign(UI)= 어떻게 깨끗이 보이나 → macro-analysis-superstrengthen(깊이)= 묻어둔 것을 어디까지 배선해 끌어올리나.**
