...
4. **적대 평가 → 95점까지 반복** — 냉정한 분석가(사실 정확성·데이터 정합·소스 줄번호/값 정확) + 실사용 파워유저(7초 가독성·시각 직관·압도 거부·문서 청결)가 6축 100점 만점으로 엄격 채점. 95점 미만이면 차단 항목을 ground-truth로 정밀 교정 후 재평가.

채점 6축: 시각직관 /25 · 사실정확 /20 · anti-clutter /15(설계가 깎았나 + 문서 자체 청결) · 구현준비 /20 · 데이터정합 /10 · 사용자가치 /10.
...
- 분석가 96 (시각24·사실24·anti14·구현19·데이터10·가치9) — "데이터정합 10/10, 모든 줄번호·값 실측 일치. 사실정확 20/20."
- 사용자 96 (시각24·사실24·anti14·구현20·데이터10·가치8) — "남은 사실오류 0건. 95+ 가치 있는 탁월한 PRD."
