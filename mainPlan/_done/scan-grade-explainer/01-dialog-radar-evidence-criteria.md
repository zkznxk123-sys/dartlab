...
- **★축 정합 함정 — 한계 라벨 필수**: 3개 축 집합이 *불일치*한다 — radar=**6축**(audit 누락)·grades=**7축**(+감사위험)·verdict.cells=**또 다른 6축**(prof/growth/stab/cf/val). 결정: 스파이더는 `co.radar` 6축 그대로 **"6축"으로 명시 라벨**(audit는 3단이라 0~1 정규화 시 해상도 거짓 → 스파이더 제외, 우측 근거 칩에서 audit 등급 별도 표시). **"등급 구성 팩터 전부"라 주장 금지**(audit 빠짐).
...
- **매수/매도 신호·목표주가·"좋은 주식"·인과(선행지표가 주가 예측) 금지**(`00 kill-list` 정합). 등급 = fact 아닌 **판정** → 근거+기준 동반은 요구와 정합(설계대로).
