...
## 5. 작업 3 — 마퀴 클릭 → 오버레이 배선 + 명시 분기 (유일한 진짜 신설)
...
**배선**: id 보유 항목만 `<button>`+`onclick=ctl.toggleEcon(id)`, **파생 5종은 비클릭**(커서 차단·디밍). "마퀴 도는 것 전부 버튼"은 이 5종에 거짓이므로 **명시 분기 필수**(허위 오버레이 금지). `ECON_MAX=3`(`chartState:68`) 4번째 토글의 침묵 무시(`:189 toggleEcon`)는 설계 위반 → **명시 피드백**(토스트 "동시 3개까지"·비활성 표시)으로 교체.
