...
## 기반 — persistent dock + 3단 tiering (UX 토대, 현 엔진 무수정)
...
3. 한계 표기 = `HonestyFooter`(11px·3단 tiering: spec 중립/caution slate/active amber + `ⓘ 방법론` 원장). 9.5px 위반 전수 교정.
...
### 테스트 / 롤백 / 위험
svelte-check 0. Playwright: dock 열고 차트 클릭→빌더 잔존·spine 접힘·한계 표기 11px 측정·empty-state. 위험: 레이아웃 회귀(중) — 차트 폭/리사이즈. 가역(dock→드롭다운 복원). **엔진 0 변경.**
...
4. 시도 조합 카운터 상존·OOS 강제(2010~19/2020~26, 끌 수 없음)·라벨(근사·월말·size틸트·추천아님).
...
### 테스트 / 롤백 / 위험
엔진 단위(holdings 보존·턴오버·decisionT<fillT·이중실행 청산만 분기)·DuckDB NTILE 결정론·PIT 미래행 미접근·grep. 위험: F1/F2 미측정 착수=밴드 거짓(높) → 선결로 차단. 가역(폴더+버튼 삭제, 기존 무수정). 상세 = 05.
...
> TradingView `request.financial()`이 한국 재무를 엮으므로 차별은 "재무 쓴다"가 아니라 **DART 계정 정규화 + 학술팩터 9 사전구현 + PIT 근사 라벨**. 단일종목이라 생존편향 무관.
...
- **D2 P4 강건성 / U3 local**: 퍼블릭 민감도(argmax 금지·**색=분산**, 04 §6)·MC 거래 재배열·multi-split anchored OOS(floor 범위 내 가능). 로컬 walk-forward(refit)·CPCV·DSR/PBO 수치는 04 §1 G2 게이트(영구 DEFER). `_backtestAdvanced.py` 이미 보유.
