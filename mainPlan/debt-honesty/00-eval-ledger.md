...
- **P3-3 builders 분할 = ✅ 완료** (`1c61ef8ec`) — 6,111줄 god-file → **14 도메인 모듈 + _shared**. 처음 2회 naive split 은 모듈레벨 상태(`_storyCurrency` ContextVar·`import contextvars` L503·module 상수 4)를 놓쳐 깨졌으나(PRD "일괄분해 import 회귀" 실증), **3차에 *public 함수만 제외한 모든 top-level 코드*를 _shared 로 + 정확한 `__all__`(`_` 이름 포함) + __init__ 재export(외부 `_`헬퍼 계약 보존) + 도메인별 명시 import(ruff F405 제거)** 로 신뢰성 완수. 검증: 277 story 테스트 통과·ruff 클린·architecture(cycleScan/folderMirror/providerGate) 전부 PASS·registry/dartlab import 정상. 외부 호출계약(`from dartlab.story.builders import X`) byte 보존. **교훈: god 분할은 함수뿐 아니라 모듈레벨 상태/숨은 import 전수 처리가 관건.**
