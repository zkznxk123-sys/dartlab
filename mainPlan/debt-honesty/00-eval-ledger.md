...
- **P3-3 builders 분할 = 2회 시도 후 defer(검증된 복잡도)** — 구조 분석은 양호(169 public 함수 *상호호출 0*·공유헬퍼 12). 그러나 자동 split 2회가 모두 실 import 버그: (1) keyword 그룹 불균형 (2) ★**모듈레벨 상태/숨은 import** — `import contextvars`(L503)+`_storyCurrency` ContextVar(L505)+함수내 `from ...blocks import EnrichedFlag`(L1052)가 header 밖이라 naive 함수-split 이 `__all__` 불일치로 깨짐. **이것이 PRD 경고 "god 일괄분해 → import 회귀"의 실증.** builders.py clean 복원(무손상)·import 정상. 신뢰성 split = 모듈레벨 코드 전수 처리 + 14 도메인 그룹핑 + 검증 = 별도 집중 세션(PRD "위임·점진" 정합). P3-1 트랙박제는 완료, 강행은 안티-정공법.
