# PRD — DartLab 부채 감소 (Debt-Honesty)
...
> 불변 가드: **공개 호출표면(Company facade·MCP advertised tools·HF artifact 경로·UI consume seam) 절대 불변.** 모든 제거는 운영자 제품결정 동반. 자동 docstring sweep 금지·scripts/ 신설 금지·master only·자동생성 도구 금지 전부 준수. churn 최소.
> 근거: 7차원 심층 전수조사(아키텍처·데이터·덕지덕지·테스트·문서·UI·AI, 245 tool 호출·683K 토큰) + **2차 census 보강**(미감사 엔진 analysis/quant/credit/industry/synth/reference/frame/simulate/channel/cli/server/skills/company+톱레벨 + landing/src + ui/packages 하위, 5 finder·222 tool·565K 토큰 — web/blog 제외) + 운영자 직접 ground-truth 검증(1차 7/7 + 2차 핵심 5/5: importlib L2/L1.5 우회·charts.ts 죽음·server 죽은라우터·analysis/graph 유령). **커버리지 = 전 엔진 + landing + ui-packages(ui/web·blog만 제외).**

---

## 1. 한 줄 결론 + 비전

**DartLab 전수조사가 드러낸 진짜 부채는 "코드가 더럽다"가 아니라 "보호받고 있다는 믿음과 현실의 간극"이다 — 가드가 죽은 경로(`scripts/`·`ops/`)를 가리키고, 강행 가드 다수가 CI에 배선조차 안 됐고, 노이즈로 못 켜고, 광고(MCP "6 종"·"canonical 7")와 실제(advertised 23 도구)가 다르고, 유령 자산(edinet 3,868줄·ui/web 155파일·ui/shared 20파일)이 parity 게이트나 stale 메모리로 박제되어 있다. 본 PRD는 큰 리팩토링이 아니라 *방어망 자체를 실제와 일치시키는 것*을 1순위로 삼는다 — 가드망이 실제와 맞아지면, 다른 PRD들이 이미 계획한 분해 작업이 비로소 안전해진다.**
...
### 왜 지금, 왜 이 순서인가
CLAUDE.md의 최상위 회귀 방어 철학은 **"⛔ 회귀 가드 — Guard Index 우선 (AST census > pytest 전수)"**다. 그런데 전수조사 결과 그 Guard Index의 구멍이 7개 차원에서 *독립적으로* 발견됐다. 즉 프로젝트가 의존하는 안전망 자체가 부분적으로 거짓이다. 다른 부채(god 분해·dead code 제거)를 먼저 건드리면 그 작업의 회귀를 잡아줄 가드가 거짓이라 위험하다. **따라서 P0=가드 배선 정정이 모든 후속 부채감소의 전제 조건이다.**
...
| (2차) analysis 42K·quant 21K LOC | 실측 **54K·27K** | seed LOC 도 과소 — `find\|wc` 직접 |
| (2차) "L2↔L2/L1.5 cross 0건" 가드 보장 | importlib 문자열 import는 **AST 미탐** | 레이어 가드 신뢰성 = 정적 import 한정. 동적 import 별도 탐지 필요 |
| (2차) "god=큰 파일" | `forecastRevenue` 588줄 파일에 548줄/86분기 *god 함수* | 함수-분기 밀도도 god 차원(파일 LOC만 보면 놓침) |
| (2차) charts.ts = live dispatcher(내 P2-3) | charts.ts importer **0=죽음**, live=surfaces MiniFinChart 단독 | dispatcher 후보도 importer grep 으로 live 검증 |
...
## 2.8 2차 census 보강 — 미감사 영역 전수 (엔진·landing·ui-packages, web/blog 제외)

1차 census는 *부채 차원*으로 조직돼 가장 큰 god/유령(providers·story/viz·ai/mcp·ui/web)에 집중했고, 여러 엔진은 LOC/테스트비율로만 스쳤다. 운영자 요청("모든 엔진·랜딩·프론트 다 봤나, web·blog 빼고")에 따라 2차 census(5 finder)로 **analysis·quant·credit·industry·synth·reference·frame·simulate·channel·cli·server·skills·company+톱레벨 + landing/src + ui/packages 하위(contracts·design·format·runtime)**를 직접 전수했다. 결과 = 38 신규 finding, 대부분 isGap=true(어떤 PRD도 미커버). **같은 5 메타테마가 미감사 영역에서 그대로 재현**되며, 두 새 메커니즘이 드러났다.

### 2.8.1 가드 환상 — 새 메커니즘: importlib 문자열 import가 AST 레이어 가드를 우회 (운영자 직접 검증)

| ID | 위반 | 증거(검증) | 가드 사각 |
|---|---|---|---|
| W-G0(분류) | importlib peer-cross 전수 | L2→L2 cross **5건**(analysis→macro 1·credit→macro 3·credit→analysis 1) + synth→frame/scan **4건**. (L2→synth/scan 다수는 L2→L1.5 *하향=합법*, intra-engine lazy 제외) | 전부 AST 미탐이나 *대부분 sanctioned* — 아래 구분이 정공법의 핵심 |
| W-G1(정정) | analysis→macro·credit cycle-break = **sanctioned** | analysis→macro(`macroExposure.py:618`)는 `architecture.md:172` *허용 정적 방향*. credit→macro 3(`excessBondPremium.py:16`·`creditCycle.py:16`·`crisisDetector.py:20`)+credit→analysis 1(`metrics.py:24`)은 `architecture.md:181` **pattern-4(importlib 동적 호출=cycleScan AST 우회) 명시 허용 cycle-break** | 위반 아님 — *의도된 AST 우회*. 단 가드가 sanctioned/unsanctioned 구분 못 해 둘 다 invisible = 진짜 문제 |
| W-G2(진짜 위반) | synth→frame/scan **L1.5 cross** (미허가·AST 은닉) | `synth/bottomUpBeta.py:104,277`(→`frame.sector`)·`:197`·`impliedERP.py:225`(→`scan.io.parquet`) — synth는 L1.5, frame/scan은 형제. pattern-4는 L2 전용이라 L1.5 cross엔 sanction 없음 | `test_l15_no_cross_import.py`(AST 전용) 미탐 = **유일한 미허가 AST-은닉 위반 4건** |
| W-G3 | landing/src 전체 무가드 | `checkUiDataWiring`(`uiDataWiring.baseline.json`) 스캔범위 밖 → landing raw fetch **25건** 무가드 | UI 데이터배선 SSOT가 landing 사각(공개 surface인데) |
| W-G4 | testCoverageGate carve-out | cli+server/api+server/web+channel **≈10,700 LOC** 통째 면제 | 가장 큰 LOC 덩어리가 회귀 가드 0 |

→ **핵심은 "importlib를 금지"가 아니다**(pattern-4가 cycle-break로 *명시 허용*). 진짜 문제 = 가드가 동적 import를 **아예 못 봐서 sanctioned(§172 방향·§181 pattern-4)와 unsanctioned(synth→frame/scan L1.5 cross)를 구분조차 못 함**. 1차 가드환상(죽은경로/미배선/노이즈)에 더해 **"AST 가드의 동적-import 사각 + sanctioned 예외 미원장화"**가 4번째 형태. (P1-7로 봉합 — `ast.Call(import_module)` 탐지 + §172 allowlist + pattern-4 baseline → 그래야 *미허가* synth L1.5 4건만 발화)

### 2.8.2 유령자산 — 새 카테고리: "빌드됐으나 미배선" 엔진 능력 ~6-8K LOC (전부 importer/caller 0)

1차 유령(edinet·ui/web·ui/shared)은 *provider/UI* 차원이었다. 2차는 **엔진 능력 자체가 빌드 후 배선 0**인 대량 데드코드를 드러냈다 — 학술 구현·9섹션 docstring·SeeAlso로 "완성 자산" 인상을 주나 어떤 verb/route로도 도달 불가:

| ID | 유령 | LOC·증거 | 처리 |
|---|---|---|---|
| W-PH1 | analysis/graph 인과그래프 | 1,147 LOC·`grep dartlab.analysis.graph` 전역 importer 0(audit 스크립트만 문자열 스캔) | 배선 or _attempts 회수 |
| W-PH2 | quant transforms+labels(AFML ML infra) | 719 LOC·`_registry.py` 미등록·caller 0 | _registry 등록 or 회수 |
| W-PH3 | quant/portfolio 4모듈(blackLitterman·meanCVaR·transactionCost·attribution) | caller 0(docstring SeeAlso만) | 배선 or 회수 |
| W-PH4 | analysis/forecast/calibrator·reference/capability/analysisGraph.py | calibrator test-only + analysisGraph 621 LOC(0.10 제거 MCP 도구 전용·live consumer 0) | 회수 |
| W-PH5 | **server/api 죽은 라우터** (viz·analysis·macro·room·dl) | ~1,840 LOC·라이브 UI consumer 0(ui/web[죽음] 또는 자기-테스트만). **ui/web 유령(FE-1)이 이 백엔드를 살려두는 연쇄** — 1차가 프론트만 보고 못 봄 | FE-1(ui/web)과 동일 변경단위 |
| W-PH6 | core/plugins.py + top-level plugins.py PluginContext | 312 LOC 죽은 평행 로더 + 채택 0 speculative framework(property test만 연명) | 삭제 |
| W-PH7 | frame/sector 5 shim·credit overrides·channel create_adapter | shim importer 0 / credit overrides=`evaluateCompany` 인자 미수용→try/except 폐기(40줄 docstring이 동작0 약속) / channel `__all__`에 부재 심볼 | 정리 |
| W-PH8 | ui-packages 계약↔런타임 drift | 18 mandatory ports 중 **7개가 throw-stub/no-op**(39% 미실) + RuntimeDataEnvelope anti-stale 계약 0 adapter 사용 + 38/158 타입 외부소비 0 + createFakeRuntime 601 LOC dead + v2-tokens.css 유령(0 소비) | 계약 채택 or 제거 |
| W-PH9 | landing charts.ts | 0 import 유령 dispatcher + buildCompanyCharts.py 좀비빌드 (→ **P2-3 정정의 근거**) | 삭제 |

→ 합산 **약 6-8K LOC의 빌드-후-미배선 데드코드.** 처리 원칙 = 각 묶음 **배선(실제 verb/route 연결) or 회수(_attempts/삭제)** 결정. importer 0이라 삭제 회귀위험 낮음(배선은 설계 필요). (P2-10 신규 트랙)

### 2.8.3 표면 드리프트·god·테스트 (기존 테마 보강)

- **표면드리프트**: Skill OS recipe(`quant/portfolio/blackLittermanViewBuild.md:69` 등)가 `from dartlab.quant.blackLitterman`로 *틀린 import 경로*의 유령 모듈을 catalog 공개 광고 → 복붙 시 ModuleNotFoundError(W-SD1, P0로). **내 P2-3가 죽은 charts.ts를 SSOT로 지목**한 자기-드리프트도 정정(LAND-2).
- **god**: synth/damodaranL15.py 1,656줄(45함수·`_deepDiveTable` 214줄)·skills/registry.py 1,130줄·landing build.ts 1,517줄·ui-packages financeSource.ts 822/reportSource.ts 723·`forecastRevenue` 548줄/86분기 함수(파일은 588줄로 평범 → LOC-기반 census가 놓친 *god 함수* 차원) (P3-7 백로그).
- **테스트비율**: ui-packages runtime 3.2%·contracts/format/design 0 테스트, financeSource/reportSource god 0 테스트, `/lab/*` noindex 실험라우트 6,396줄(landing 17%)이 프로덕션 번들 적재(P3-2 확장).

### 2.8.4 커버리지 맵 (1차+2차 종합)

| 영역 | 깊이 감사 |
|---|---|
| providers·gather·scan·pipeline·ai·mcp·macro·story·viz·core | ✅ 1차 |
| **analysis·quant·credit·industry·synth·reference·frame·simulate·channel·cli·server·skills·company+톱레벨** | ✅ **2차** |
| **landing/src·ui/packages(contracts·design·format·runtime·surfaces)** | ✅ 1차(surfaces)+2차 |
| ui/web · blog | ⛔ 운영자 제외(DEPRECATED·콘텐츠) |

**전 엔진 + landing + ui-packages 전수 완료 (ui/web·blog만 제외).**

---

## 3. 설계 원칙

1. **가드 선행 (Honesty-first).** 가드가 거짓이면 부채를 못 줄인다. P0/P1(가드 배선 정정)이 P2/P3(제거·분해)의 전제. 순서 역전 금지.
...
| P0-9 | W-SD1 | Skill OS recipe(`specs/recipes/quant/portfolio/blackLittermanViewBuild.md:69`·`meanCvarConstruction.md`)의 틀린 import 경로 `dartlab.quant.X`→실제 `dartlab.quant.portfolio.X` 정정(복붙 ModuleNotFoundError 차단). P2-10 배선 결정과 연동(미배선이면 catalog 노출 제거) | recipe 경로 grep 일치 |

### Phase 1 — 가드 CI 배선 정정 (배선 or 명시 폐기)
...
| P1-7 | W-G0/G1/G2 | **importlib AST 사각 봉합 (sanctioned 보존)**: `test_l2_no_cross_import.py`·`test_l15_no_cross_import.py`에 `ast.Call(func=import_module, 문자열 리터럴 인자)` 탐지 추가 + **(a) `architecture.md:172` 허용 정적방향(analysis→macro 등) allowlist (b) `architecture.md:181` pattern-4 cycle-break(credit→macro 3·credit→analysis 1) baseline 동결원장** → 그래야 sanctioned 5건은 통과하고 **미허가 synth→frame/scan L1.5 cross 4건만 발화**. 해소: synth 4곳을 합법 경로(frame/scan 함수를 호출자 inversion 또는 core/calcs 강등=pattern-1/2)로. *analysis→macro·credit cycle-break는 위반 아니므로 건드리지 않음* | 가드 보강 S(allowlist/baseline 동반 필수) + synth 4건 해소 M. **sanctioned 깨면 import cycle 부활** — allowlist 선행 |
| P1-8 | W-G3/G4 | landing/src를 `checkUiDataWiring` 스캔범위에 편입(raw fetch 25건 원장화 or 정공법 배선) + `testCoverageGate` carve-out(cli·server/api·server/web·channel ≈10,700 LOC)을 명시 원장+점진 해제 계획으로 전환(통째 면제=가드 0 해소) | landing은 공개 surface라 무가드 raw fetch가 진짜 위험. carve-out 해제는 점진 |
...
| P2-3 | PH-3, LAND-1/2 | ui/shared 삭제(import 0) + **landing `lib/browser/charts.ts`도 죽음**(importer 0, 2차 census 정정 — 1차 P2-3가 이를 'live dispatcher'로 잘못 지목) + `buildCompanyCharts.py` 좀비 빌드·`static/charts/` 산출 정리 + landing `$chart`·sharedChartDir 별칭 제거 + **`reference_financial_graph_ssot` 메모리·`financial-statement-lab` PRD를 *유일한 실제 dispatcher* `surfaces/.../terminal/charts/MiniFinChart.svelte`로 정정**(charts.ts 포함 금지). SSOT 정정이 삭제보다 중요 | prerender/notebook 동적사용 1회 확인. UI라 push 승인 |
...
| P2-10 | W-PH1~9 | **빌드-후-미배선 유령 ~6-8K LOC 인벤토리 처리** — 각 묶음 *배선(verb/route 연결) or 회수(_attempts/삭제)* 결정: analysis/graph(1,147)·quant transforms+labels+portfolio(~1,400)·calibrator·reference/capability/analysisGraph(621)·**server/api 죽은라우터(1,840, FE-1 ui/web과 동일 변경단위)**·core+top plugins(312+)·frame/sector shim·credit overrides·ui-pkg throw-stub 7포트+RuntimeDataEnvelope+createFakeRuntime(601)+v2-tokens.css·landing charts.ts. importer 0이라 삭제 회귀위험 낮음 | 묶음별 운영자 배선 vs 회수 결정. 학술구현·docstring이 '완성' 인상 줌 |
...
| P3-7 | W-god | 2차 신규 god 백로그(위임): synth/damodaranL15.py 1,656줄·skills/registry.py 1,130줄·landing build.ts 1,517줄·ui-pkg financeSource 822/reportSource 723·`forecastRevenue` 548줄/86분기 *god 함수*(파일은 평범 → LOC 아닌 함수-분기 밀도 측정 추가). builders.py(P3-3) 패턴으로 점진. `/lab/*` 6,396줄 noindex 라우트는 프로덕션 번들 분리 검토 |
...
**PM 관점**:
- ✅ 강점: "가드망 실태 정정 우선"이 비-자명하고 측정가능한 thesis. 부채 28건을 4 phase로 우선순위화. 기존 23 PRD와 중복 0(갭만). 착수=운영자 go로 다른 PRD와 동일 운영.
- ⚠ 위험: 범위가 repo 전체라 "한 번에 하나" 철학과 긴장. P2/P3가 제품결정·다른 루프 위임 의존이라 PRD 단독 완결 불가.
- 완화: P0/P1을 *독립 완결 가능 단위*로 설계(가드 배선 정정만으로도 출시가치). P2/P3는 "결정게이트+위임"으로 명시해 PRD 경계를 명확히 그음. MUST(P0+P1) / SHOULD(P2) / 위임(P3) 웨이브.
...
6. **(2차) importlib AST 사각 봉합**(P1-7): 레이어 가드가 `import_module` 문자열 import 탐지 + §172 allowlist + pattern-4 baseline → **미허가 cross = synth→frame/scan L1.5 4건만 해소**(analysis→macro·credit cycle-break는 sanctioned이라 보존). "동적 import도 가드가 본다 + sanctioned 예외 원장화"로 정의를 정밀화. *실위반 정의 = §172 미등록 + pattern-4 baseline 미등록 cross.*

**성공 (SHOULD — P2, 2차 보강)**:
- 빌드-후-미배선 유령 ~6-8K LOC(P2-10) 묶음별 배선 or 회수 결정 완료(analysis/graph·quant transforms/portfolio·server 죽은라우터·plugins·ui-pkg throw-stub 등). server 죽은라우터는 ui/web(FE-1)과 동일 변경단위.
...
**총 28(1차) + 38(2차 §2.8) = 66 census finding → 4 phase. P0(9)·P1(8)·P2(10)·P3(7+옵션). MUST=P0+P1(가드 배선 정정+importlib AST 봉합, 독립 완결). SHOULD=P2(유령 제거 + 빌드-후-미배선 ~6-8K LOC). 위임=P3(god 분해 트랙).**

> **커버리지**: 1차(7 부채차원) + 2차(미감사 엔진 13 + landing + ui-packages 4) = **전 엔진 + landing + ui-packages 전수**(ui/web·blog만 운영자 제외). 2차 신규 메커니즘 2종 = importlib 동적-import AST 사각(W-G1~2) + "빌드-후-미배선" 유령 카테고리(W-PH1~9). 2차 핵심 5건 운영자 직접 검증(importlib 우회·charts.ts 죽음·server 죽은라우터·analysis/graph 유령·synth L1.5 우회).
