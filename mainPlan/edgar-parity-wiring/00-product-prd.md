...
### 1.1 ★정직 경계 (v0.4: 결측은 *대부분 채운다* — 운영자 챌린지 대응)

[정정 v0.4] v0.3는 결측을 "정직하게 빈다"로 *회피*만 했으나, 직접 코드검증 결과 **대부분 채울 수 있다**(상세 `08-missing-data-resolution.md`): EDGAR가 KR report 17 중 **14 apiType 이미 추출**(`reportAccessor._SUPPORTED`), SIC·자기주식·executivePay 추출기 보유. 진짜 못 채우는 소수(가치사슬 *엣지*·전임원 보수·GICS·US 가격 라이선스)에만 "정직하게 빈다" 적용. **"채울 수 있는데 안 채우는 게으름"도 실패**(04 §5).

아래 표는 *구조적으로 다른* 잔여 + 채운 데이터의 정직 라벨 원칙. "100% parity" 주장 금지는 유지:
...
| **진짜 잔여 (D/구조차)** — 정직하게 빔 | `industry` 가치사슬 *엣지*(공급망)·`executivePay` 전임원(US=NEO5)·GICS sector·US 가격(라이선스) | 채울 수 있는 부분만(분류=SIC, 보수=NEO5) + 커버리지/한계 라벨. 합성·사칭 금지(04 §5). |
