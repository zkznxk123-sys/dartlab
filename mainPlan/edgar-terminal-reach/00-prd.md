...
## 9. 가격·지수 데이터 구조·파이프라인 (확정 — gov 대칭)

**구조 = `edgar/` = US 시장 데이터 네임스페이스** (edgar/finance·tickers·panel·prices·indices). KR gov 미러:

| KR gov | US edgar (구현) | 소스 |
|---|---|---|
| `gov/prices/company/{code}` | `edgar/prices/company/{ticker}` | Yahoo 백필(~11년 base) |
| `gov/prices/recent.parquet` | `edgar/prices/recent.parquet` | Polygon 일증분 tail (둘 다 직업로드) |
| `landing/map/prices-snapshot.json` | `landing/map/prices-snapshot-us.json` | daily 가 빌드(게이트) |
| `gov/indices/*` | **`edgar/indices/*`** (예약 — 미생성) | FRED(현재 직독) / 유료 Polygon Indices(미래 OHLCV) |

- **백필**(`edgarPricesBackfill.yml`, Yahoo per-ticker): company parquet 만(`writeSnapshotFlag=False`). 행수 가드(110행
  부분응답 차단). offset/limit 청크 이어받기(단일 잡 — 병렬 matrix 는 HF 동시 commit 충돌이라 직렬 dispatch).
- **일증분**(`edgarPricesDaily.yml`, Polygon by-day): `domains.polygon.fetchGroupedDaily`(gather 위임, gov.fetchGovBydd 대칭)
  → recent.parquet + 스냅샷 빌드(전 recent ticker 게이트). 1콜/날짜(6437 per-ticker 회피).
- **런타임**: `edgarPriceSource` company(base)+recent(tail) mergeDedup → PriceChart(govPriceSource 동형).
- **지수 결정**: US 지수는 `indexSource.ts` 가 이미 FRED 직독(큐레이트 4종, close=OHLC). 별도 수집 폴더 불요. OHLCV 지수가
  꼭 필요하면 유료 Polygon Indices(무료는 I:SPX/I:DJI=403)로 `edgar/indices/`(예약명)에 수집 — prices 와 동일 패턴.
- **marketCap**: shares(EDGAR dei `EntityCommonStockSharesOutstanding`) × 종가 — 게이트 후속(현재 null).
