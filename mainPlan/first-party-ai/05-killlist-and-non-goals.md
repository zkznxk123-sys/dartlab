...
### K4. "무제한 무료 AI" 과장
Cloudflare 무료티어 = 10k neurons/day. 예산 서킷브레이커로 보호하고, 마케팅·UI·문서 어디서도 "무제한/영구 무료" 로 쓰지 않는다(`ai-workbench-connector` 한계 표기 원칙 §7 계승). "퍼블릭 무료 가동, 한도 도달 시 온디바이스로" 가 있는 그대로의 표현.
...
- baked 리드/카피: 데이터 버전·asOf 기준 캐시 가능(작업대 TTL 계승).
...
```text
이 PRD = DartLab 1인칭, 근거에 묶인, 끊기지 않는, 명시된 한계의 티어 사다리.
