...
HF company artifacts
  └─ dashboards/finance.json#companies[*].macroExposure
        ↓
...
회사별 연결은 analysis macro exposure 계약이 맡는다. public 터미널에서는 새 Python 호출이나 새 per-company artifact 없이 `buildFinanceJson.py`가 기존 all-company `finance.json`에 `macroExposure.exposureQuality`를 포함한다.
...
- UI는 `co.macroExposure.exposureQuality`를 우선 소비하고, 값이 없을 때만 fallback 잠금 상태를 만든다.
