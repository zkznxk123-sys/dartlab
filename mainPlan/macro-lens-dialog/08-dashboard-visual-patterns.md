...
남은 강화는 엔진 산출물 승격이다. `OBS/PRIOR/TPL/LOCK` 자체는 UI가 만드는 표시지만, 장기적으로는 `macro.transmission`과 `analysis.macroExposure`가 동일 라벨을 source/date/value lineage와 함께 제공해야 한다.
