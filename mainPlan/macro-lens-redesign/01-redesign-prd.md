...
**한 줄 결론:** Macro Lens 다이얼로그의 무게중심을 *"판정 엔진(verdict·score·100)"*에서 *"정직한 매크로 계기판(dashboard)"*으로 역전한다. 13개 섹션이 쌓인 한 탭을 폐기하고, 4블록 IA — **무엇이 움직였나 → 어느 채널에 닿나 → 증거가 무엇인가 → 언제 다시 보나** — 로 되돌린다. 새 데이터 fetch 0, 새 패널 0, 새 필드 0. 코드는 ~600줄 순삭감.
...
- verdict 레이어 전부 (UI 13섹션 + view-model `buildMacroVerdict` + `MacroVerdict*` 타입군). 정직한 OBS/PRIOR/TPL/LOCK 상태 계약을 다시 점수·방향·승패로 재포장해 "판정"으로 읽히게 만든 레이어.
...
| 8 | Mechanism Rail | `mlMechanismPanel` | 정직 상태를 점수 막대로. |
...
| 12 | Phase Strip | `mlPhaseStrip` | **(계승 — 정직 자산)** |
| 13 | Pulse / Matrix / Gate / Release | `mlPulseStrip` `mlMatrix` `mlGateStrip` `mlReleaseRail` | **(계승 — 단, 2중 `<details>`로 접혀 묻힘)** |

**진단 (위계 역전):** 진짜 정직한 자산(11~13)은 2중 `<details>`로 접혀 화면 맨 아래에 묻혀 있고, 가드 위반 소지가 큰 verdict 레이어(1~10)가 화면을 점령한다. 위계가 뒤집혀 있다. 재설계는 이 위계를 정확히 뒤집는다 — verdict 1~10 삭제, 11~13을 첫 화면 주역으로 끌어올린다. 동시에 13섹션이 전부 같은 무게의 작은 mono 박스(8~11px)라 시각 위계 자체가 없는 문제를, §5 px 위계표·시각 토큰 SSOT·면적 강조로 닫는다.
...
- **D블록 Gate Strip = `evidenceGates` 중 quant 제외 4개**(데이터·전파·동행·회사). 정량 게이트는 `근거` 탭 Model Card 상단으로 강등하고 거기서 2케이스 문구로 정직 표시(§6.1).
...
지배 상태가 `blocked`이므로 사유문은 `notWiredYet`(미배선)이 아니라 실제 `blockedReason`(회귀 부재)을 그대로 쓴다 — 둘은 다른 정직 상태다(미배선=시도 안 함, blocked=시도했으나 표본 부족). "잠김=죽은 칸" 인상은 대체가치 문구가 차단한다. 진척 표기는 텍스트이지 gauge 아님. 구현: `status`로 직접 분기(`quantEvidenceBlocks`가 사유를 이미 생산 → status별 한글 라벨+대체가치 매핑만 추가, 새 계산 0).

### 6.2 초점 사슬 select + tie-break + 전파 게이트 정직 + fallback
...
- **신규:**
  ...
  - 정직 가드: snapshot에 `verdict` 부재, JSON에 `score`/`directionScore`/`/100` 부재.
...
### 12.2 PM
- 강등 2건(scenario·상세 driver 표)은 경로 탭으로 이동(정보 손실 0, 접근 +1클릭). pressureGrid·mobileDrillRail·legend는 실삭제(부분 승리). README 정직 표기.
- 정량 지배 케이스 `blocked`(2645·94%·회사 회귀 부재) — 사용자가 "거의 다 잠겨있네"로 느낄 수 있으나 첫 화면에서 빠지고 근거 탭에 대체가치 문구(co-move·sectorPrior)와 함께 표시 → 가짜 OPEN보다 정직한 LOCK.
...
| OBS 칩 의미 | 'edge 관측(회사 회귀 아님)' 정직 표기 |
