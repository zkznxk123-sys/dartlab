...
해법: 새 수집 0. **이미 빌드되는 parquet에서 rcept_dt 정렬 슬림 파일을 CI bake**(빌드 편승)하고, 좌측에 시간순 피드 + 주가영향 6탭 + 기관 보조칩(정직 라벨)을 신설한다.
...
- **연금/기관 = 독립탭 kill**: report_nm '연금' 0건·flr_nm 식별 **9.5%**·개인 오너 혼재. '지분·내부자' 탭 내 보조 [기관] 칩 + '제출자=기관(부분식별·약10%)' 정직 라벨로만.
...
- **정직 컷**: 호재/악재·종합점수·buy/sell 금지 · top-N 침묵 절단 금지(200cap+카운트 라벨) · content_raw 없으니 본문영향 추정 금지 · category 빌드 bake 금지(클라 분류, SSOT drift 회피).
