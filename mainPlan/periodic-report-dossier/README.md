...
> **무게중심**: 비(非)재무 사업보고서 실체 — dartlab 이 *이미 네 번이나* fetch 하면서도 얕게·출처 없이 흘려보내는 사람·자본배분·소유 팩트 — 를 **하나의 날짜 박힌, 출처추적되는, 자기이력 프레임의 도시에(dossier)** 로 묶는다. 그 결과 **패널 수는 오히려 줄고**(−1) 가독성·정보 범위는 올라간다.
>
...
진단(실측): 정기보고서 28개 구조화 API 중 14개만 차트화. 노출된 것조차 **얕다** — 직원=급여 상위3명, 배당=수익률만, 자사주=취득/처분 raw(소각 `buybackCancel` 은 fetch 후 버림). 같은 parquet 가 `DART 정기보고서 팩트` 평면 패널 + `주주환원`/`인력`/`타법인출자` 패널 + FinFullscreen 탭 + HoldingsDialog 로 **중복 표면화**되면서도 **출처(`rcept_no`)·자기이력·범위 라벨** 이 없다.
...
| [08-hardening-and-uiux.md](08-hardening-and-uiux.md) | **경화 delta** — 적대 재검토 평결("조건부 강함: 강한 제품, 약한 기반")·실버그 수정(DuckDB→hyparquet)·검증된 6갭(F2 도달불가·F5 경로·grep가드 신규·lossPct lift·공개로컬패리티·−1 드롭)·UI/UX 상태기계 §8·Phase-0 probe (4에이전트 `wf_c451d741-93f`) |
...
- **제외 컷**: 세그먼트 카드(4.6%만 clean)·가동률 숫자(데이터 0)·시총분모 총주주환원율(거짓정밀)·모든 종합점수/레이더 = **금지**.
- **순 패널 변화 = −1**. 강함은 더하기가 아니라 *갇힌 계산을 있는 그대로 표면화*.
