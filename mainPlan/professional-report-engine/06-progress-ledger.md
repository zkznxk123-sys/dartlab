...
- ◐ P1a 밸류에이션 de-gate + WACC/성장/fade/reverse — 게이트 G1·G2(백테스트)·G3·G5
  ...
  - ⛔ **G2/범위가드 실행함 → 데이터 오염 하드 증거로 valid 결과 불가 확정**(`run_gates.py` 실측): 가격 시리즈 *내부* 불일치 — 삼성 실현 +467.7%·하이닉스 +815.4% (12M 물리적 불가, 컷오프 무관). relative valuation 은 marketCap 주입에도 None → production dFV 산출 불가(liquidation 폴백). G2-lite 1/5 = 오염가격 노이즈(무효). **결론: ① G2 백테스트 ② damodaranPhase4 범위가드 = *올바른 가격데이터* 필수(CI 데이터잡 신선 HF / 정상 네트워크 / production). 오염데이터로 통과 적기 = 날조라 거부.** G4 calcValuationSynthesis = 내 변경 무영향(독자 WACC 경로).
