...
- ◐ P1a 밸류에이션 de-gate + WACC/성장/fade/reverse — 게이트 G1·G2(백테스트)·G3·G5
  ...
  - ☐ **damodaranPhase4 범위가드(140k–230k)**: relative valuation(peer scan)이 샌드박스서 None → full dFV(삼각검증) 산출 불가 + 가드가 구(舊)가격 baseline(삼성 현 ~340k). de-gate 가 through-cycle 로 IV 보수화 → 가드 재baseline 후보. full dFV 는 CI 데이터잡(peer scan 정상). G4 calcValuationSynthesis = 내 변경 무영향.
