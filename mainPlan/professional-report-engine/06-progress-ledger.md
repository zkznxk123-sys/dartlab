...
- ◐ P1a 밸류에이션 de-gate + WACC/성장/fade/reverse — 게이트 G1·G2(백테스트)·G3·G5
  ...
  - ☐ **남은 = 시장데이터 환경 후속(데이터 벽)**: 샌드박스는 라이브 price/marketCap 부재로 dFV 가 production 값(삼성 ~185k) 아닌 liquidation 폴백(36,912) → ① **G2 백테스트(20사 t+12M 실현주가, 02a §5)** ② **`test_damodaranPhase4.py` requires_data 범위가드(140k–230k) 데이터잡 무회귀 확인/필요시 재baseline**(de-gate 가 의도적 값 이동 — requires_data 라 CI-fast 미포함, 별도 데이터잡). G4 calcValuationSynthesis sanity = 내 변경 무영향(독자 computeCompanyWacc 경로). → P1a 본질 완료, 잔여는 시장데이터 환경.
