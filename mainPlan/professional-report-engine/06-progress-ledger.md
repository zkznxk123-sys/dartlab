...
- ◐ P1a 밸류에이션 de-gate + WACC/성장/fade/reverse — 게이트 G1·G2(백테스트)·G3·G5
  ...
  - ☐ **남은 = 시장데이터 환경 필요(데이터 벽)**: 샌드박스는 라이브 price/marketCap 부재로 dFV 가 production 값(삼성 ~185k) 아닌 liquidation 폴백(36,912) → ① **G2 백테스트(20사 t+12M 실현주가)** ② **`test_damodaranPhase4.py` 범위가드(140k–230k) 무회귀 확인/정정**(시장가 필요). G4 calcValuationSynthesis sanity = 내 변경 무영향(독자 computeCompanyWacc 경로). **push 보류** — CI-fast(offline)는 green 예상이나 범위가드(requires_data, 시장가) 검증 전 push 게이트 "신규 failure 0" 미확정.
