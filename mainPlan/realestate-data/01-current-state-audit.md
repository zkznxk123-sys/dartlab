...
- **★정직 경계:** 이 회로는 *제품(product)* 매핑이지 *지역(region)* 매핑이 **아니다.** 집계 섹터(건설·건자재) 연결은 가능하나 "현대건설 → 강남 아파트값" 같은 **기업-지역 특정은 LAWD_CD→회사 매핑 자산 0건이라 영구 honest-skip**(§6).
...
## 6. 정직한 한계 사전 등록 (확신오정렬 가드 — 코드 검증 정정 반영)
