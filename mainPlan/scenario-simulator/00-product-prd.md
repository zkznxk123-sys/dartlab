...
`Scenario Workbench`의 비전은 이 질문을 한 화면에서 다루는 것이다. 핵심 경험은 "뉴스를 읽고 감으로 판단"하는 것이 아니라, 이벤트를 사업 driver에 연결하고, driver를 손익과 가치평가로 전파하며, 마지막에 가격 민감도와 반증 조건을 동시에 확인하는 것이다.
