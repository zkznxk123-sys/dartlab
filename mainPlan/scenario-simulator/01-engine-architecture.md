...
4. **`getPresetScenarios` docstring은 이미 올바르게 기술됨.** (이전 v0.1·일부 논의가 "거짓 정정 부채"로 올렸으나) 실 docstring은 DFAST/BOK + 자체 시나리오·정적 상수를 있는 그대로 명시. **진짜 부채는 `SECTOR_ELASTICITY`** — 실측 **35키(KR 23 + US 12, `synth/scenario.py:229` inline 하드코딩)**, "36업종"·"11업종"은 전부 오기. seed/CI 0 무출처 prior(folk-stat 위험)이고, `getElasticity` docstring이 "2010~2024 회귀 패널"·"`data/synth/sectorElasticity.json` 로드"를 단정한 것은 **provenance 날조**(실제 inline) — 정정 완료(`synth/scenario.py` docstring 라벨). DriverRegistry pooled-panel β로 대체(02 §2B.4-B).
...
> **★(rank30) terminal fade 곡선 = 결정론 수렴(경쟁우위·moat fade, 새 노드/패널 0 — terminal 처리에 fade schedule 파라미터 1개):** terminal `ROIC − WACC > 0` 인 경우 Gordon TV 의 영구 spread 를 fade 곡선으로 명시화한다 — 초과수익은 경쟁으로 fade 하는 게 정상이라 영구 지속엔 명시 근거가 필요하다(§6.3 (d) moat 라벨의 *수렴 쪽 기계화*; 현재 (d)는 moat 라벨 쪽만 기계화). 규칙(결정론):
> - (a) **기본 = fade-adjusted TV:** `fadeYears`(CAP, 경쟁우위 지속기간) 명시 + ROIC 를 WACC 로 **선형수렴**시킨 fade-adjusted terminal value 를 기본으로 쓴다(fade = proforma vector 산수, 추정기 0).
> - (b) **fadeYears=∞ 는 명시 moat 정당화 한정:** 영구 spread(`fadeYears=∞`)는 명시 moat 라벨이 `capYears=∞`(rank5 구조 진술)를 정당화할 때만. 아니면 provenance `terminal_excess_no_fade` + `warnings 'perpetual_spread_no_fade — CAP unspecified(honest-gap)'`(§6.3 (d)·rank5 와 동일 채널).
> - 08 §6 발간 체크리스트 #6 'ROC→WACC 수렴 or 명시 moat' 의 **수렴 쪽 기계화**(현재 (d) 가 moat 라벨 쪽, 본 항이 fade 수렴 쪽 — 한 쌍). terminal 처리에 fade schedule 파라미터 1개 외 새 노드/leaf 0(선형수렴 = proforma vector 산수). _fnDcf 신설 코드 동반 → _attempts 졸업 게이트 경유(설계 명세만).

> **★rev.path elasticity provenance 명시(졸업 AC):** `rev.path` 노드의 elasticity 계수는 curated `SECTOR_ELASTICITY` priors(`synth/scenario.py:229`, 35키, OOS provenance 없음·seed/CI 0)다. DriverRegistry pooled-β(02 §2B.4-B)가 pooled-OOS partial-R² 를 공급하기 *전까지* `SimulationResult.warnings` 가 `"elasticity_prior_unvalidated"` 를 싣는다(`run.py` warnings 튜플의 base-revenue/shares honest-gap 동일 패턴) — 결과 표면에서 magic-constant fan-out 이 *침묵*하지 않게.
>
> **★(R1) elasticity_prior_unvalidated 즉시 실배선 — 졸업 AC를 P0 승격(코어 졸업 전이라도 가능한 수 줄 배선):** 현 `run.py` warnings 튜플은 **base-revenue/shares 2건만**(`run.py:226-229` 실측)이라 *무출처 35키 마법상수 fan 이 경고 없이 나간다*. 이 한 줄은 DriverRegistry 졸업(02 §2B)을 기다리지 않고 지금 박는다:
> - `_fnRevPath` 가 `transfer:` provenance 를 낼 때 사용한 elasticity 가 curated prior(=pooled-β 미유래)면 provenance 문자열을 `elasticity:반도체` → `elasticity:반도체(prior,unvalidated,seed0)` 로 라벨(노드 provenance 재사용, 새 필드 0).
> - `run.py` 가 그 노드 provenance 에 `(prior,unvalidated` substring 이 있으면 `run.warnings` 에 `"elasticity_prior_unvalidated"` 추가(base-revenue/shares 와 **동일 채널**). 05 §3.1 시각 규율(점선·회색 fan)이 이 warning 을 소비.
>
> **★(rank3) cpi silent-drop 명시 — preset 이 들고다니나 0영향 표면화(코드 정본 확인):** `MacroScenario.cpi`(`synth/scenario.py:26`, 3년 list 실재)는 preset 마다 채워지나(`:51,60,…` baseline=`[2.2,2.1,2.0]` 등) `transferMacroToFundamentals`(`transfer.py:110-127` 실측)는 **gdp/rate/fx 만 읽어** revenue/margin/wacc 로 보낸다 — cpi 는 transfer 채널이 *부재*(silent drop). 명시 출구(새 채널/leaf 0):
> - `transfer.py` docstring AntiPatterns 에 'cpi/inflation 현재 전달채널 부재 — preset 이 들고다니나 0영향' 명시(코드측 박제 완료).
> - `rev.path` 노드 provenance 에 `cpi:no-channel(nominal-real undecomposed)` 라벨을 부가(elasticity provenance 와 같은 provenance 문자열, 새 필드 0).
> - 선택 warnings `"inflation_input_not_transmitted"` 1줄(base-revenue/shares·elasticity_prior_unvalidated 와 **동일 채널** — preset 이 inflation 을 들고 왔으나 결과에 닿지 않음을 표면이 침묵하지 않게). 02 §2C census 표 'inflation(cpi)' 행(경로부재 0%·blocked)과 한 쌍. 인플레→명목/실질 분해 transfer 는 별도 능력·별도 `_attempts` 라운드(거시경로 예측 §9 와 동결).
> - **driverCoverage 경고 채널 결선(R1 핵심):** `rev.path` 결과의 `driverCoverage`(= 실제 transfer 로 분해한 driver 수 / 시도한 driver 수, 산출 SSOT = 02 §2B 'driver 수렴/census' — pooled-β admission census 가 공급, 그 전엔 단일 1-beta 근사라 coverage=1/시도수)가 `coverage < 임계`(잠정값 — 졸업 데모서 재보정; design target)면 `elasticity_prior_unvalidated` 와 **동일 `SimulationResult.warnings` 채널**로 `"sector_1beta_approximation — driverCoverage below floor(honest-gap)"` 를 표면화. coverage 와 elasticity-prior 는 *직교 경고 신호*(전자=분해 입자도 부족, 후자=계수 미검증)지만 둘 다 같은 warnings 튜플로 모아 §3 silent 값-대체 계약(공개 계약 honest-gap 확장 bullet)·05 fan 시각 규율이 한 채널만 읽으면 되게 한다. ⚠ `driverCoverage` 필드 자체는 DriverRegistry census(02 §2B) 산출분이라 *그 전엔 미배선*(현 4노드 코어는 단일 sector-1beta) — 본 항의 P0 부분은 **elasticity provenance 라벨 + warnings 1줄**이고, coverage 채널은 census 졸업 동반(졸업 AC).
...
| **`quality.baseline`** (R3) | `_fnBaselineQuality` → `scanQuality`(단일사 추출, `scan/financial/quality.py:250`) + `buildEvidenceForensicsMemo`(`synth/evidenceForensics.py:51`, revenue-cash bridge `_revenueCashBridge:350`) | **미구현(후속 — _attempts 졸업 게이트 경유)** | ★회계품질 producer 노드(현 4노드 DAG에 quality producer **부재**). 산출 = 단일사 `accrualRatio`·`cfToNi`(⚠ 코드 필드명 = `cfToNi`, `quality.py:147`; backlog 'cfoToNi' 는 오기) + forensics memo 의 revenue-cash bridge. vector=과거 N분기(scanQuality 가 분기 패널 반환), deps=(`snapshot` 또는 `proforma`). 신규 leaf 코드 0(두 leaf 다 실재·호출만) — 단 노드 fn `_fnBaselineQuality` 신설은 _attempts 졸업 게이트 경유, 본 문서엔 *설계 명세*만 |

**⚠ 졸업 데모(게이트 ②④):** §5b 노드는 가설. 삼성/카카오/현대 실측으로 (a) macro.gdp.beta 단일/분기별 (b) mc.distribution 위상(OQ8) — byte-패리티로 확정(04 §5).

> **★A6 흡수(Causal/LucaNet MC forward fan) = already(deferred 노드의 졸업 다듬기 1문장):** 드라이버-불확실성 MC fan은 이미 위 `mc.distribution`(OQ8)으로 명시 deferred라 **새 흡수 0**. cosmetic 가우시안 fan=통계적 연극 리스크는 σ 무출처면 `SimulationResult.warnings` 표면화 + S_T≈0 driver 토글 제거(02 §2B.4-C)로 이미 가드됨. 유일 다듬기 = **fan band σ provenance 시각 규율**: σ가 pooled-OOS residual(검증)에서 오면 **실선 fan**, elasticity prior(`elasticity_prior_unvalidated`, 미검증)에서 오면 **점선·회색 근사 fan**으로 그려 LucaNet/Causal의 매끈한 단일 fan과 명확히 가른다(honest-gap warning의 시각화 확장, 05 §3.1 동기). 가격 밴드는 결정론 매핑(가격 MC 금지)·`NodeValue.vector` 원소별 None 끊김(missing≠0) 규율 불변.

> **★(R12) mc.distribution = '측정된 비모수 예측분포'(졸업 AC, 미구현 후속 — 코드 분기 0·분위 함수만):** OQ8 의 가우시안 σ 곱은 *모수 연극* 위험이 있다(folk-stat 교훈: σ 가 무출처면 매끈한 fan 이 정밀로 오인). 졸업 데모서 다음 4건을 *측정으로* 닫는다 — 새 노드/분기 아닌 분위 함수·진단 통계 추가뿐.
> - **(1) regime-conditional σ:** σ 를 단일 상수가 아닌 **regime 조건변수**로 층화한다. ⚠ **코드 정본 정정**: backlog 의 "snapshot.macroRegime(EnvironmentSnapshot 실재)" 는 *틀림* — `EnvironmentSnapshot`(`core/credentials.py:27`)은 **credential 환경**(HF_TOKEN 등)이고 `macroRegime` 필드는 simulate snapshot 에 **부재**(`registry.py` snapshot 11키에 없음). 실재하는 regime 은 macro DataFrame 스키마(`core/schemas.py:172` `regime ∈ {expansion,peak,contraction,trough}`)다. ⟹ 설계 = regime 을 *snapshot 확장 필드*로 승격(버려진 `core/schemas.py` regime 컬럼을 `buildSnapshot` 가 읽어 `snapshot["macroRegime"]` 로 표면화 — 신규 *데이터* 아닌 기존 컬럼 배선)한 뒤, pooled-panel 잔차를 그 regime indicator 로 **층화**해 regime 별 σ 추정. regime 별 pooled-N<floor(02 §2B.11 잠정값 60) 이면 empirical-Bayes shrink(전역 σ 로 수축)+`warnings=["regime_sigma_shrunk"]`. **이 항은 '실재' 가 아니라 snapshot 확장이 선결인 design 가설**(졸업 AC).
> - **(2) 가우시안 모수화 → MC 경험분위 직접:** σ 곱-정규 대신 held-out 잔차의 **empirical quantile** 을 직접 분포로 쓴다 — fat-tail·비대칭(영업레버리지 하방꼬리)이 *자동* 표현(코드 분기 0, 분위 함수만). 모수 가정 제거 = 측정 기반 강화(측정된 분포 > 가정된 분포).
> - **(3) horizon cone 기울기 검증:** held-out `σ_emp(h)` vs `σ_model(h)` **variance-ratio test** 가 `[0.5, 2.0]`(잠정값 — 졸업서 재보정) 밖이면 `warnings=["cone_slope_misspecified"]`. + **Ljung-Box** 자기상관 진단 → 유의 시 `warnings=["horizon_iid_assumed"]`(현 cone 이 iid 가정 위에 섰음을 명시). 01 §13 'cone 확대' kill-test(`test_horizon_widens_cone`)의 *반대 방향 가드* — 확대 자체뿐 아니라 확대 *기울기*가 데이터와 맞는지.
>   - **★(rank25) cone σ(h) = 자기상관 의존(시계열 계량, 새 노드 0·R12 분위함수 확장):** σ(h) 구성을 iid 가정에서 자기상관 의존으로 전환한다 — iid 면 `σ(h)=σ·√h`, 추정 ρ 가 유의하면 **AR(1) 누적분산** `σ(h)² = σ²·Σ_{k=0}^{h-1} ρ^(2k)`(양의 자기상관 = √h 보다 빠른 확대, 음의 자기상관 = 느린 확대). (2) empirical-quantile 경로면 **h-step 잔차를 1-step σ·√h 로 외삽하지 말고 h별 직접측정**(R12-(2)가 이미 h별 분포 보유). variance-ratio test 는 σ(h) 의 *생성식*에서 그 *측정 σ(h)의 검증 게이트*로 재배치(측정값이 모델 누적식과 맞는지). **Ljung-Box 유의인데 σ(h) 를 iid 로 둔 경우 = `warnings=["cone_autocorr_uncorrected"]` 강제 + fan 점선 + status 강등**(경고에 그치지 않고 시각·품질까지) — §2B.2 'silent 확대 금지'(자의적 σ 확대 차단)와 충돌 없이 *자기상관을 σ(h)에 정식 반영*하는 출구(임의 확대가 아니라 추정 ρ 기반 누적분산).
> - **(4) OQ8 ε 분리 명시:** 현 OQ8 'byte-parity 제외, 분포통계 ±ε' 는 두 ε 를 conflate. 분리 = **(ε_a) TS-포팅 재현 ε**(V8↔Python `random.Random` 시퀀스 부재 → 분포통계 ±ε, 04 §5)와 **(ε_b) cone 기울기 ε**(σ_emp(h) vs σ_model(h) variance-ratio 허용폭). 둘은 다른 게이트(전자=패리티, 후자=misspecification).
> - **(5) tail measure = ES(coherent), not VaR-only(리스크관리 ES, 추정기 0):** 꼬리 측도를 분위(VaR류) *단독*이 아닌 **Expected Shortfall**(분위 *위* 꼬리 평균)로 보강한다 — VaR 는 비정합(non-subadditive)이라 꼬리 두께를 못 보고, ES 는 분위 위 *결정론 산수*(empirical-quantile 보유분 위 평균)라 점확률을 새로 만들지 않는다. `ScenarioFan` 에 `es5`/`es95` 필드 추가(=P5 이하 / P95 이상 시나리오값의 평균, 새 추정기 0·기존 (2) empirical-quantile 재사용). 05 §2.1 fan 범례에 'P5 = 밴드경계(VaR류 분위) vs P5-ES = 꼬리 평균' 2어휘로 가른다(매끈한 밴드가 꼬리 깊이를 가리지 않게). 03 §4.4 calibration 에 **(e) tail-coverage 채점** = P5 위반율이 명목 5% 로 수렴 + **Acerbi-Szekely ES backtest** → 비정합 시 `calibState='tail-miscalibrated'` + 점선·회색(03 §4.4 *동일 채널*). ES 는 분위 위 결정론 산수라 점추정·점확률 아님(검증 척추 유지).
> - **σ provenance 시각 결선(A6 박스 확장):** σ 가 OOS residual 유래면 provenance `sigma:oos-residual`(**실선 fan**), 미유래(prior)면 `sigma:prior-unvalidated`(**점선·회색**, 05 §3.1 결선). σ 가 OOS residual 미유래 시 `warnings=["distribution_width_unvalidated"]` **강제** — 분포 폭이 검증 안 됐음을 결과 표면이 침묵하지 않게(R1 elasticity_prior_unvalidated 와 동일 채널·동일 패턴).

> **★(R13) 라이프사이클 단계별 valuation 분기 = 결정론 게이트(미구현 후속 설계):** 08 §1(동형 확정, 라이프사이클별 leaf 스위치 단락)이 단계별 leaf 스위치(금융사=residualIncome/bankDFV·cyclical=정규화 마진)를 명시하나 *"AI lens 가 단계 분류"* 로 둔다 — 본 항이 **분류를 결정론 게이트로 정정**(AI 는 제안만, 채택은 결정론; 01 §6 det/ai 평행 = "확신오정렬>정렬실패" 일관). dcf 노드 `_fnDcf` 의 method-dispatch 를 *결정론 입력*으로 가른다:
> - **(a) 단계 분류 = 결정론(AI 아님):** ⭐ **금융사 판정 leaf 가 이미 실재** — `isFinancialCompany(company)`(`analysis/valuation/bankDFV.py:25`, bool) → 재발명 0, `buildSnapshot` 가 호출해 `snapshot["isFinancial"]` 로 flag(현 snapshot 11키에 추가). 백로그의 "업종코드(은행/보험/증권 WICS)+이자수익/매출>임계" 는 이 leaf 의 내부 판정과 *합치*(있으면 leaf 임계 재사용, 부족하면 leaf 보강). lifecycle(startup→…→decline) 분류 = revenue CAGR·ROIC·배당성향 **결정론 rule**(Damodaran 6단계 proxy, AI 아님) → `snapshot["lifecycle"]`. 둘 다 `buildSnapshot` 서 flag(신규 *데이터* 0 — 기존 series·valuation 입력 산수).
> - **(b) dcf 노드 method-dispatch:** `snapshot["isFinancial"]==True` → `_fnDcf` 가 proforma-FCFF 대신 **residualIncome leaf**(`analysis/valuation/residualIncome.py::calcRIM` / `bankDFV.py::calcBankDFV` 감사) 로 디스패치, 둘 다 결손이면 `'금융사=blocked, FCFF 부적합' honest-skip`(status='partial'·**가짜 FCFF 금지** — 0 대체 아닌 method 부적합 표면화, `warnings=["fcff_inapplicable_financial"]`). `lifecycle=="cyclical"` → proforma 입력 마진을 **정규화 마진**(trailing mid-cycle 평균)으로 치환해 peak/trough 단년 마진의 DCF 왜곡 차단.
> - **(c) AI lens = 제안만:** lens(§6.4)는 단계 분류를 *제안*할 수 있으나 채택 권한은 결정론 게이트(a). lens 제안과 결정론 분류가 갈리면 DisagreementLedger 에 fork 기록(det 우선). 졸업 AC: **금융사(KB금융 등)에 `dartlab.simulate` 시 FCFF 무단 적용 0건**(isFinancial→FCFF 디스패치된 케이스 = 골든에서 전수 차단). ⚠ method-dispatch 분기는 `_fnDcf` 신설 코드 — _attempts 졸업 게이트 경유(본 문서 = 설계 명세만).

**★NodeValue 계약 갭 + frozenInputs 형상 핀(구현 명세):** 현 `NodeValue`(sheet.py:50-56, 7필드)는 frozenInputs를 노출하지 않고, `evaluateSheet`(sheet.py:361-363)는 fn 반환 7-튜플의 frozenInputs를 `computeInputsHash` 에만 쓰고 *저장 안 한다*. 그 결과 *두 개의 recompute 우회*가 실재 — `registry._macroFrozen`(rev 노드가 rate/FX 경로 프리셋 재유도)·`run._marginPathFromSnapshot`(marginPath 같은 transfer 재계산). 둘 다 "감사 노드는 권위 provenance/refs/hash 그대로, 표시 숫자만 byte-동일 재산출"하는 패턴이나 우회다. **★형상 핀(09 §10.3 T-A1 미결 해소): NodeValue에 frozenInputs 추가 시 = RAW pre-normalize 수치값 저장(소비 전용)** + 해시는 `evaluateSheet`가 `_normalize(frozenInputs)`로 *별도* 산출(해시 전용) — 한 필드 두 파생, 경쟁 두 타입 아님. `_freezeInputs`(09 T-A1의 `tuple((k,_normalize(v))...)`)는 해시 경로 한정. **이 추가의 목적 = *우회 삭제*(엔진트랙)지 mc.distribution 아니다**(mc 는 proforma vector 소비라 frozenInputs 불필요 — 위 §5b 표·OQ8). 두 우회는 종전 PRD 미기재 — 본 절에 기록.
...
> **★(rank35) confidence-misalignment 재현 kill-test 졸업 AC 핀(적대 레드팀 — 새 명세 아닌 kill-test 핀):** 위 두 가드(`_isStrongDet` folk-stat 가드·groundingCheck (b) circular 가드)가 *원래 함정을 진짜 막는지* 적대 반증으로 핀한다 — 자기 수정이 함정을 막았다는 *주장*만으로 부족하고, 함정을 *심은 골든*에서 막힘을 증명해야 한다(01 §12 'test_horizon_widens_cone 이 옛 버그를 먼저 증명 후 전환' 동형).
> - **(1) `test_strongdet_rejects_spurious_inscope`:** gdp/rate/fx × lag0/1/2 에서 in-sample best-R² ≥ 0.3 이지만 OOS·자유도 미달인 **스퓨리어스 driver 를 심고**, `_isStrongDet` 이 `'weak'` 반환(=fork 허용, AI 못 이김 잠금 해제)을 단언. 옛 R²≥0.3 단독 화이트리스트 버그를 *먼저 증명*(스퓨리어스가 'strong' 으로 잠김)한 뒤 전환(`test_horizon_widens_cone` 동형 — 01 §13).
> - **(2) `test_grounding_rejects_in-tol_hypothesis`:** base metrics ±tol 안에 *드는* AI hypothesis(=groundingCheck (b) 통과처럼 보이나 사후적합)가 약한 det 를 교체하되 **사후적합임을 잡는** 케이스를 골든에 심음 — groundingCheck (b) circular 가드(OQ10 abstention-over-circular, ledger 없는 노드는 (b) 기권→fork)가 약한 det 자체분포 순환을 진짜 막는지 적대 반증. 자기 수정이 원래 함정을 *진짜* 막는지 증명하는 핀(account-struct '확신오정렬 > 정렬실패' 재현 차단). gate.py 미구현 동반 — 골든은 gate 신설과 함께(설계 명세만).
...
**`checkValuationCoherence(node, snapshot)` — ★(R6) 가치평가 내부정합 순수함수 분기(미구현 후속, gate.py 신설 시):** `dcf` 노드에 한해 proforma vector 에서 *함축* 양을 역산해 **DCF 입력의 자기모순**(Damodaran 1순위 valuation 죄악)을 차단한다. 현 발간 체크리스트(08 §6 항목6 "g≤Rf, reinvest=g/ROC, ROC→WACC 수렴")가 *산문 체크*로만 존재 → 본 함수가 그 항목6을 **gate.py 순수함수로 기계강제**(08 발간 게이팅이 소비). 규칙(순수함수, deps=dcf 노드 + snapshot):
> - (a) **terminal g 정합:** proforma vector 서 `implied ROIC`·`implied reinvestmentRate` 도출 → `g_supported = reinvestmentRate × ROIC`. `|terminalGrowth − g_supported| > 1pp`(잠정값 — 졸업서 재보정) 이면 `NodeAudit.status='incoherent'`(현 status enum = ok/partial, `run.py:53` → **'incoherent' 추가**=additive) + `warnings 'terminal g(X%) not supported by reinvestment×ROIC(Y%)'`.
> - (b) **value-destructive growth:** `ROIC_terminal < WACC` 인데 `terminalGrowth > 0` 이면 `'value-destructive growth' flag` — 성장이 가치를 *파괴*하는데 양의 g 를 박은 모순(Damodaran 전형 죄악).
> - (c) **salesToCapital 역검산:** `salesToCapital = ΔRevenue / Reinvestment` 로 proforma 의 reinvestment 가 매출증분을 자본효율로 지지하는지 역산 → proforma capex 와 `±tol`(잠정값) 벗어나면 flag.
> - (d) **영구 초과수익 moat 라벨 강제 (한국 성장주 과대평가 최빈 경로):** terminal `ROIC − WACC > spread_tol`(잠정값) 인데 fade/convergence 가정도, 명시 moat 라벨도 없으면 `status='incoherent'`(또는 `warnings 'perpetual excess return without moat/fade'`). (b)가 `ROIC<WACC` *가치파괴*만 잡는 **대칭** 으로, (d)는 영구 초과수익(`ROIC≫WACC` 무한 지속을 *암묵* 가정)이 만드는 과대평가를 잡는다 — 초과수익은 경쟁으로 fade 하는 게 정상이므로 영구 지속엔 *명시 moat* 가 필요하다. 입력 결손=partial. 신규 leaf 0(terminal ROIC·WACC = proforma vector 산수) — **08 §6 발간 체크리스트 #6 'ROC→WACC 수렴 or 명시 moat' 의 기계 강제**.
>   - **★(rank5) binary moat flag → 구조화 결정론 진술(경쟁우위·산업구조, 새 leaf/패널 0):** moat 를 bool 깃발이 아닌 구조 진술 `moat = {moatType ∈ (network|switching|intangible|cost|scale|none), capYears: int, fadeAssumed: bool}` 로 정밀화한다 — moat 의 *존재*만큼 *유형·지속기간·fade 동반 여부*가 영구 spread 정당화의 본질이기 때문. 결정론 분기(전부 동일 warnings 채널):
>     - `moatType ≠ none` 인데 `capYears`(지속기간)·`fadeAssumed` 미동반 → `warnings 'moat_type_without_duration — excess return persistence unverified'`(`elasticity_prior_unvalidated` 와 **동일 채널**: 라벨은 있으나 검증 차원 부재).
>     - `moatType`·`capYears` 가 무출처(사람/AI 라벨이고 근거 ref 부재) → `warnings 'moat_label_unsourced'`(provenance 에 출처 미동반).
>   - 졸업 AC 강화: **'라벨 누락 0'** → **'유형·지속기간 미동반 영구 spread 0건'**(moatType 명시돼도 capYears·fade 없이 영구 초과수익을 정당화한 케이스 = 골든에서 전수 차단). gate.py 미구현 동반(설계 명세만).
> - **데이터 부재 처리:** ROIC/reinvestment 산출 입력(NOPAT·investedCapital·capex)이 결손이면 `status='partial'`(0 대체 **금지** — incoherent 가 아닌 partial, 모순 *판정 불가*와 모순 *발견*을 구분). 졸업 AC: **'g not supported → flag 누락 0'**(coherence 위반 케이스를 심은 골든에서 flag 전수 포착). ⚠ 신규 leaf 코드 0(implied ROIC/reinvest/salesToCapital 은 proforma vector 산수) — gate.py 자체가 미구현(§6.3 상단 박스)이라 본 함수도 그 신설 동반(설계 명세만).

**`checkMacroCoherence(macroPath, snapshot) -> warnings` — ★(rank16) 거시 입력 내부정합 순수함수(거시 regime·정책 정합, `checkValuationCoherence` 형제·신규 파일 0):** valuation 입력의 자기모순을 지키는 `checkValuationCoherence` 와 **대칭** 으로, 거시 입력(macro.path 노드)의 자기모순을 지킨다 — macro.path preset 이 GDP/rate/FX/CPI 를 *독립* list 로 들고와 항등식 위반(예: 침체인데 정책금리 인상)을 침묵할 수 있기 때문. 4 항등식 **약한 체크**(전부 잠정·졸업 데모 재보정·차단 아닌 라벨, 전부 `SimulationResult.warnings` *동일 채널* + 05 fan 점선·회색 소비):
> - (a) **Taylor rule:** 인플레(cpi)·산출갭 대비 정책금리 경로가 Taylor 근사에서 크게 벗어나면 `policy_rate_path_inconsistent_with_inflation`.
> - (b) **환율→인플레 passthrough(KR 수입의존):** 큰 원화 약세(fx)인데 cpi 경로가 무반응이면 `fx_passthrough_to_cpi_missing`(한국 수입물가 전가 부재).
> - (c) **오쿤 법칙:** GDP 가 침체 구간인데 정책 대응(금리 인하) 부재면 `no_policy_response_to_recession`.
> - (d) **UIP(무위험 금리평형):** 금리차–환율 변화 방향이 UIP 와 모순이면 `rate_fx_uip_violation`.
> - 전부 잠정값(졸업 데모 재보정)·차단 아닌 라벨(거시 시나리오는 *가정*이라 incoherent 가 아닌 경고 라벨). gate.py 미구현 동반(§6.3 상단 박스 — 설계 명세만). 본문 §9(거시경로 예측 부재)와 한 쌍: §9 가 거시경로를 *예측하지 않음*을 있는 그대로 처리하고, 본 함수는 사람이 박은 preset 경로의 *내부 정합*을 약하게 지킨다(예측 부재 ≠ 정합 미검).
...
> **★(rank1) warnings 완전성 불변식 — provenance→required-warning 결정론 매핑(졸업 AC·적대 레드팀):** warnings 채널(`run.warnings`, 01:123 R1·02 §2C.2·02 §3.7·03 §4.4 *동일 채널*)이 의존하는 **모든** 경고 신호가 *빠짐없이* 실린다는 불변식을 단언함수로 박는다 — 경고를 *추가*하는 룰만큼 *누락*을 잡는 가드가 중요하다(레드팀: 무출처 신호가 경고 없이 새는 게 진짜 함정). 새 채널/필드 0, 불변식 단언함수 1개만.
> - **결정론 매핑(provenance substring → 필수 warning):** provenance 가 `(prior,unvalidated` 를 들면 `elasticity_prior_unvalidated` 필수, σ provenance 가 `sigma:oos-residual` 미유래(=`sigma:prior-unvalidated`, §5b R12)면 `distribution_width_unvalidated` 필수, `cpi:no-channel`(rank3)이면 `inflation_input_not_transmitted` 필수, `default:no-sector`/`default:no-wacc`(§3 공개 계약 honest-gap)면 `sector elasticity defaulted` 필수. 매핑은 *결정론*(provenance 문자열이 곧 필수 warning 을 함의).
> - **졸업 AC `assertWarningsComplete(result) -> None` 순수함수:** `SimulationResult` 의 전 노드 provenance 를 훑어 위 매핑이 함의한 warning 이 `result.warnings` 에 모두 있는지 검사, **하나라도 누락이면 `ValueError`**(silent swallow 금지의 기계 강제). 신규 leaf/필드 0 — provenance 문자열·기존 warnings 튜플만 소비하는 단언함수. gate.py 형제로 명세(§6.3 상단 박스 미구현 동반).
> - **시각 규율 fail-open→fail-closed 역전(1줄):** *검증 provenance 를 명시적으로 못 단 노드는 점선(미검증)이 기본, 실선은 검증 provenance(`ols:OOS`·`pooled:fe`·`sigma:oos-residual`)가 있어야 켜짐.* 05 §3.1 fan 시각 규율을 기본값 역전으로 정정 — "라벨 없으면 실선(안심)" 의 거짓 안심을 차단(R12 σ 시각 결선·A6 박스와 한 방향).
...
> **★(rank16) 예측 부재 ≠ 정합 미검 — `checkMacroCoherence`(§6.3 형제):** preset 경로를 *예측하지 않는다*고 그 경로의 *내부 정합*까지 안 지키는 건 아니다. 사람이 박은 preset(GDP/rate/FX/CPI 독립 list)이 Taylor/passthrough/오쿤/UIP 4 항등식과 모순이면 §6.3 의 `checkMacroCoherence` 순수함수가 `SimulationResult.warnings` 로 약하게 표면화한다(차단 아닌 라벨, valuation 입력 지키는 `checkValuationCoherence` 와 대칭). 거시경로 *예측* 능력은 여전히 별도 `_attempts` 라운드 — 본 정합 체크는 그 예측 부재 위에서 가능한 가드다.
...
2. **단, 숨은 표면 2건:**
   ...
   - **MCP "0건"은 거짓.** `ai/tools/engineCall.py`는 `loadCapabilities()` 카탈로그 기반 **동적 dispatch**(`getattr(dartlab, method)`). 시뮬 함수가 capability 카탈로그(skills/*.json)에 등재되면 MCP 표면에 노출. "카탈로그 경유 1표면"으로 기록, 실측 후 회귀 표면 포함.
...
4. **능력 한계:** 경량 횡단면은 elasticity 기반 *근사*(full 3-statement 아님) — "정밀 손익 투영"이 아니라 "시나리오 노출 스크리닝"으로 라벨. 회사 클릭 시에만 정밀.
