...
> **★(rank30) terminal fade 곡선 = 결정론 수렴(경쟁우위·moat fade, 새 노드/패널 0 — terminal 처리에 fade schedule 파라미터 1개):** terminal `ROIC − WACC > 0` 인 경우 Gordon TV 의 영구 spread 를 fade 곡선으로 정직화한다 — 초과수익은 경쟁으로 fade 하는 게 정상이라 영구 지속엔 명시 근거가 필요하다(§6.3 (d) moat 라벨의 *수렴 쪽 기계화*; 현재 (d)는 moat 라벨 쪽만 기계화). 규칙(결정론):
...
>
> **★(R1) elasticity_prior_unvalidated 즉시 실배선 — 졸업 AC를 P0 승격(코어 졸업 전이라도 가능한 수 줄 정직):** 현 `run.py` warnings 튜플은 **base-revenue/shares 2건만**(`run.py:226-229` 실측)이라 *무출처 35키 마법상수 fan 이 경고 없이 나간다*. 이 한 줄은 DriverRegistry 졸업(02 §2B)을 기다리지 않고 지금 박는다:
...
>
> **★(rank3) cpi silent-drop 정직화 — preset 이 들고다니나 0영향 표면화(코드 정본 확인):** `MacroScenario.cpi`(`synth/scenario.py:26`, 3년 list 실재)는 preset 마다 채워지나(`:51,60,…` baseline=`[2.2,2.1,2.0]` 등) `transferMacroToFundamentals`(`transfer.py:110-127` 실측)는 **gdp/rate/fx 만 읽어** revenue/margin/wacc 로 보낸다 — cpi 는 transfer 채널이 *부재*(silent drop). 정직 출구(새 채널/leaf 0):
...
> - **(1) regime-conditional σ:** σ 를 단일 상수가 아닌 **regime 조건변수**로 층화한다. ⚠ **코드 정본 정정**: backlog 의 "snapshot.macroRegime(EnvironmentSnapshot 실재)" 는 *틀림* — `EnvironmentSnapshot`(`core/credentials.py:27`)은 **credential 환경**(HF_TOKEN 등)이고 `macroRegime` 필드는 simulate snapshot 에 **부재**(`registry.py` snapshot 11키에 없음). 실재하는 regime 은 macro DataFrame 스키마(`core/schemas.py:172` `regime ∈ {expansion,peak,contraction,trough}`)다. ⟹ 정직 설계 = regime 을 *snapshot 확장 필드*로 승격(버려진 `core/schemas.py` regime 컬럼을 `buildSnapshot` 가 읽어 `snapshot["macroRegime"]` 로 표면화 — 신규 *데이터* 아닌 기존 컬럼 배선)한 뒤, pooled-panel 잔차를 그 regime indicator 로 **층화**해 regime 별 σ 추정. regime 별 pooled-N<floor(02 §2B.11 잠정값 60) 이면 empirical-Bayes shrink(전역 σ 로 수축)+`warnings=["regime_sigma_shrunk"]`. **이 항은 '실재' 가 아니라 snapshot 확장이 선결인 design 가설**(졸업 AC).
> - **(2) 가우시안 모수화 → MC 경험분위 직접:** σ 곱-정규 대신 held-out 잔차의 **empirical quantile** 을 직접 분포로 쓴다 — fat-tail·비대칭(영업레버리지 하방꼬리)이 *자동* 표현(코드 분기 0, 분위 함수만). 모수 가정 제거 = 정직 강화(측정된 분포 > 가정된 분포).
> - **(3) horizon cone 기울기 검증:** held-out `σ_emp(h)` vs `σ_model(h)` **variance-ratio test** 가 `[0.5, 2.0]`(잠정값 — 졸업서 재보정) 밖이면 `warnings=["cone_slope_misspecified"]`. + **Ljung-Box** 자기상관 진단 → 유의 시 `warnings=["horizon_iid_assumed"]`(현 cone 이 iid 가정 위에 섰음을 정직 표면화). 01 §13 'cone 확대' kill-test(`test_horizon_widens_cone`)의 *반대 방향 가드* — 확대 자체뿐 아니라 확대 *기울기*가 데이터와 맞는지.
>   - **★(rank25) cone σ(h) = 자기상관 의존(시계열 계량, 새 노드 0·R12 분위함수 확장):** σ(h) 구성을 iid 가정에서 자기상관 의존으로 정직화한다 — iid 면 `σ(h)=σ·√h`, 추정 ρ 가 유의하면 **AR(1) 누적분산** `σ(h)² = σ²·Σ_{k=0}^{h-1} ρ^(2k)`(양의 자기상관 = √h 보다 빠른 확대, 음의 자기상관 = 느린 확대). (2) empirical-quantile 경로면 **h-step 잔차를 1-step σ·√h 로 외삽하지 말고 h별 직접측정**(R12-(2)가 이미 h별 분포 보유). variance-ratio test 는 σ(h) 의 *생성식*에서 그 *측정 σ(h)의 검증 게이트*로 재배치(측정값이 모델 누적식과 맞는지). **Ljung-Box 유의인데 σ(h) 를 iid 로 둔 경우 = `warnings=["cone_autocorr_uncorrected"]` 강제 + fan 점선 + status 강등**(경고에 그치지 않고 시각·품질까지) — §2B.2 'silent 확대 금지'(자의적 σ 확대 차단)와 충돌 없이 *자기상관을 σ(h)에 정식 반영*하는 정직 출구(임의 확대가 아니라 추정 ρ 기반 누적분산).
...
> - **(5) tail measure = ES(coherent), not VaR-only(리스크관리 ES, 추정기 0):** 꼬리 측도를 분위(VaR류) *단독*이 아닌 **Expected Shortfall**(분위 *위* 꼬리 평균)로 보강한다 — VaR 는 비정합(non-subadditive)이라 꼬리 두께를 못 보고, ES 는 분위 위 *결정론 산수*(empirical-quantile 보유분 위 평균)라 점확률을 새로 만들지 않는다. `ScenarioFan` 에 `es5`/`es95` 필드 추가(=P5 이하 / P95 이상 시나리오값의 평균, 새 추정기 0·기존 (2) empirical-quantile 재사용). 05 §2.1 fan 범례에 'P5 = 밴드경계(VaR류 분위) vs P5-ES = 꼬리 평균' 2어휘로 가른다(매끈한 밴드가 꼬리 깊이를 가리지 않게). 03 §4.4 calibration 에 **(e) tail-coverage 채점** = P5 위반율이 명목 5% 로 수렴 + **Acerbi-Szekely ES backtest** → 비정합 시 `calibState='tail-miscalibrated'` + 점선·회색(03 §4.4 *동일 채널*). ES 는 분위 위 결정론 산수라 점추정·점확률 아님(정직 척추 유지).
> - **σ provenance 시각 결선(A6 박스 확장):** σ 가 OOS residual 유래면 provenance `sigma:oos-residual`(**실선 fan**), 미유래(prior)면 `sigma:prior-unvalidated`(**점선·회색**, 05 §3.1 결선). σ 가 OOS residual 미유래 시 `warnings=["distribution_width_unvalidated"]` **강제** — 분포 폭이 검증 안 됐음을 결과 표면이 침묵하지 않게(R1 elasticity_prior_unvalidated 와 동일 채널·동일 정직 패턴).
...
> **★(rank35) confidence-misalignment 재현 kill-test 졸업 AC 핀(적대 레드팀 — 새 명세 아닌 kill-test 핀):** 위 두 정직 가드(`_isStrongDet` folk-stat 가드·groundingCheck (b) circular 가드)가 *원래 함정을 진짜 막는지* 적대 반증으로 핀한다 — 자기 수정이 함정을 막았다는 *주장*만으로 부족하고, 함정을 *심은 골든*에서 막힘을 증명해야 한다(01 §12 'test_horizon_widens_cone 이 옛 버그를 먼저 증명 후 전환' 동형).
...
> - 전부 잠정값(졸업 데모 재보정)·차단 아닌 라벨(거시 시나리오는 *가정*이라 incoherent 가 아닌 honest warning). gate.py 미구현 동반(§6.3 상단 박스 — 설계 명세만). 본문 §9(거시경로 예측 부재)와 한 쌍: §9 가 거시경로를 *예측하지 않음*을 정직 처리하고, 본 함수는 사람이 박은 preset 경로의 *내부 정합*을 약하게 지킨다(예측 부재 ≠ 정합 미검).
...
> **★(rank1) warnings 완전성 불변식 — provenance→required-warning 결정론 매핑(졸업 AC·적대 레드팀):** warnings 채널(`run.warnings`, 01:123 R1·02 §2C.2·02 §3.7·03 §4.4 *동일 채널*)이 의존하는 **모든** 정직 신호가 *빠짐없이* 실린다는 불변식을 단언함수로 박는다 — 경고를 *추가*하는 룰만큼 *누락*을 잡는 가드가 중요하다(레드팀: 무출처 신호가 경고 없이 새는 게 진짜 함정). 새 채널/필드 0, 불변식 단언함수 1개만.
