...
- **min pooled-N (횡단면 풀 하한)**: pooled-panel 적합은 `섹터 내 기업수 × 유효 분기 ≥ 60`(예: 기업 10 × 분기 6) 미만이면 카드 `admission.state=dormant` + `warnings=["pooled_N_below_floor"]`. 풀 경계는 OQ2(WICS 11 vs sub-sector)에서 확정.
...
- **look-ahead 코드 강제(현재 텍스트 only)**: 규칙은 03 §3에 있으나 *코드 단언이 없다*. 강제 위치 = **`buildSnapshot`의 asOf 라벨이 모든 NodeValue.asOf로 전파**되고(이미 구현), replay 모드 진입 시 `admission`/`buildSnapshot`이 `availableAt <= decisionAt` *assert*를 거는 가드를 신설(현 `entry.py`/`registry.py`는 asOf 라벨만 들고 look-ahead assert 부재). 신호→체결 = `t종가 → t+1시가` 매핑은 backtest 경로(10)에서 강제.
