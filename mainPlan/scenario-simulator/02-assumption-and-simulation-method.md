...
- `sourceRefs`. — 명명 프리셋의 **출처 라벨 SSOT**. preset description이 규제 stress인 척하면(예: `synth/scenario.py`의 "CCAR 스타일"/"CCAR-style severe recession" 문구) 여기에 `curated-prior, not DFAST/CCAR-blessed`를 **강제 노출**해 검증된 stress 오인을 차단한다(00 §7.2-6 컨센서스 금지·kill-list 정합).
  > **★규제권위어 가드 — 기계강제 lint (R28, 새 lint 파일 0):** 위 산문 명세를 *기계*로 강제한다. `synth/scenario.py` preset 의 `description`(또는 임의 preset 객체)에 `CCAR`·`DFAST`·`SCAP`·`CCAR-style` substring 이 있으면 **반드시** 같은 객체 또는 `sourceRefs` 에 `curated-prior, not {regulator}-blessed` 라벨이 동반돼야 하고, 미동반 시 lint red(09 §0 P12 금지어 lint 에 *행 추가* — 새 lint 파일 신설 0, 기존 금지어 lint 스캔 범위 확장). 코드 실측 동기: `synth/scenario.py` 에 `CCAR 스타일 심각한 경기침체`·`CCAR-style severe recession` literal 이 실재(grep 확인) — 라벨 미동반이면 검증된 규제 stress 로 오인된다.
  > **★KR preset 정정 권고(R28)**: BOK(한국은행)는 CCAR 를 *운영하지 않는다* — CCAR/DFAST/SCAP 은 Fed 프로그램명이라 KR preset 에 이 이름을 차용하면 *권위 환각*(출처 라벨 위반, `feedback_no_fabricated_urls` 정신의 권위 버전)이다. KR preset 의 `CCAR 스타일` → `심각한 경기침체(curated)` 로 정정 권고(US preset 은 실제 Fed 시나리오 인용 시 라벨 동반으로 허용). 이 정정은 §2B.3 A2 `curated-prior` 라벨·00 §7.2-6 kill-list 와 일관.
  > **★preset tail-anchor 라벨(R37 — 라벨, 코드 0):** `severe`/`adverse` 분기는 *최악 프리셋 한 점*이지 그것이 분포 *꼬리(tail)* 임을 검증하는 면이 없다 — curated 프리셋은 CCAR 식 percentile 앵커(P5/P1 등)가 없어 "최악 프리셋 = 꼬리"가 *오인*된다. 분위 라벨 = 각 명명 프리셋 결과 손실을 `mc.distribution`(01 §5b) 경험분포 분위로 매핑해 `adverse≈P12(꼬리 아님)`·`severe-equivalent≈P5` 처럼 *분위*를 노출한다(프리셋 손실이 분포의 어디쯤인지 표면화). `mc.distribution` 미구현 동안은 `tail-anchor: unverified(분포노드 미구현)` 라벨(§4.4 미검증 동형·점추정 금지). 05 §5 분기 범례에 이 분위 라벨을 동반(05 소유자 cross-ref) — 색은 기존 분기 팔레트 그대로(새 색 0), 라벨 텍스트만 추가.
- `createdAt`.
...
- **★β 이질성 한계(pooled-panel이 식별하는 것의 한계, R19).** within-estimator pooled-panel은 **섹터 공통 기울기 β_s + 회사 고유 절편 α_i**만 식별한다 — 회사 *고유 기울기* β_i(회사별 macro 민감도)는 작은 표본(T=6~36)에서 비식별이다(고정효과는 절편 이질성만 흡수, 기울기 이질성은 firm-level 자유도 부족으로 못 가른다). 따라서 충격 전파 *크기*는 **섹터 평균**이다 — 어떤 회사가 섹터 평균보다 cyclical하거나 defensive하면 그 편차를 pooled-β로 못 잡는다. 강제 라벨 = `beta=sector-average, firm slope unidentified`(회사별 고유 민감도를 안다고 위장 금지, "확신오정렬 > 정렬실패" 차단).
...
- **회사별 베타는 *정제*로만.** `calcMacroRegression`은 `nObs ≥ 임계`이고 OOS holdout 통과 시에만 pooled 값 위에 회사 편차를 얹는다(라벨). 미달이면 pooled 섹터 transfer 사용(provenance hierarchy: pooled+회사확증 > pooled > preset 무출처). **★firm-slope 도 binary→shrunk 정정(R9, §2B.4-B prior-as-shrinkage 동형):** "nObs≥임계면 회사 편차 통째 / 미달이면 pooled 통째"의 binary 가 아니라, 회사 편차도 pooled 를 *수축 목표*로 한 가중평균 `β_i = (1−λ)·β_pooled + λ·β̂_i`, `λ = K/(K+T_i)`(T_i = 회사 유효 분기, K = 수축강도 잠정상수 §2B.11) — T_i 가 작을수록 λ→0(pooled 우세), 클수록 λ→1(회사 편차 우세). 강제 라벨 = `beta=shrunk(λ=K/(K+T_i))`(편차를 통째 믿지도 통째 버리지도 않음을 표면화, §2B.1 β 이질성 한계 정합). nObs<임계는 λ≈0 의 특수해라 위 hierarchy 와 SSOT 분열 없음.
...
- **G0 prior + family 등록:** 경제적 사전 있는 후보만 검정 패밀리에 넣고 시도 수 N을 빠짐없이 등록(사전등록 ledger — forking-paths 차단). **N = driver family 의 누적 DISTINCT 사전등록 trial 수** — vintages/sectors 횡단으로 시도-후-기각된 모든 distinct (series × transform × lag) 가설, `DriverCard.admission.nTrials`에 persist(G0 ledger 가 SSOT). 전 시장 카탈로그(130)는 over-counting 이라 기각하되, *per-prefit active pool*(under-counting, ~13)도 아닌 누적 distinct-trial. nTrials 는 NEW 가설이 family 에 들어올 때만 monotone 비감소(이미 등록된 driver 를 새 vintage 로 재적합하는 것은 N 증가 아님 — 아니면 새 탐색 0인데 G1' t-bar 가 시간 따라 무한 인플레, §2B.9 self-strangle). lag1/2가 상관-only면 N에 안 곱함(코드 정합).
  > **★nTrials 자동도출 — ledger 자신의 forking 닫기 (R47, 새 필드 0):** G1' t-bar `√(2·ln N)` 는 N=nTrials 에 *직접* 의존한다 → nTrials 를 사람이 과소 계상하면 t-bar 가 낮아져 false discovery 가 샌다(ledger 자체가 forking-paths 의 구멍이 됨). 따라서 nTrials 는 **`admission.evaluateAll` 호출 시점에 실제 시도된 (series, transform, lag) 조합 수에서 자동 도출**하고 개발자 수동 증가를 금지한다(자동 도출이라 과소·과대 계상 불가). 졸업 AC = `동일 family 재실행 시 nTrials 가 실제 검정 조합 수와 일치(과소계상 0)`(03 §4.2 #7 parameterSelectionLog 와 동형 — 선택 시도를 빠짐없이 ledger 에 기록). 코드가 ledger 의 forking 을 닫는다.
- **G0.5 정상성 강제 가드 (단위근 pre-test — 추정 아닌 *오용 차단*, R11):** transfer 적합 *전에* 각 시리즈의 단위근/정상성을 검정해 `transferSpec.transform`(level|yoy|diff)을 **데이터가 선택**하게 한다(사람 손튜닝 금지). 무거운 의존 0 — ADF/KPSS 또는 더 가벼운 variance-ratio·AR(1) ρ̂ 근사(numpy 일반선형대수만)로 충분. 비정상(단위근 보유)이면 **level 회귀 금지** → diff/yoy 로 강제 변환, 그래도 두 비정상 시리즈를 level 로 회귀하려는데 *공분적(cointegration) 미검증*이면 `nonstationary_level_regression_blocked` 로 카드를 dormant(spurious regression 차단 — 비정상 level 회귀는 R² 가 거짓 상방). AR(1) baseline 자체도 `ρ̂ > 0.95`면 `ar1_near_unit_root` 라벨(단위근 근방 = 평균회귀 가정 붕괴 경고). 이 가드는 새 추정기가 아니라 *적합 자격의 사전조건*(G0 family 등록과 G1 OOS 사이) — transform 을 데이터가 고르게 해 transferSpec 의 자의성을 닫는다.
- **G1' 다중검정 t-bar:** 보정 후 `|t| > √(2·ln N)`(N = G0 누적 distinct-trial `nTrials`, §2B.0)(또는 core driver는 Harvey-Liu-Zhu `t>3.0`). core(척추 6축)=step-down Holm(FWER), exploratory(뉴스·customs)=BHY-FDR(driver 상관 허용). **신설 계상 정정**(치명상 2): `multipleTesting.haircutSharpe`는 Sharpe·일별수익 시그니처라 직접 호출 불가 + Holm이 degenerate(rank-1=Bonferroni) → critical-t 공식 + 진짜 step-down Holm을 `admission.py`에 **독립 구현**(호출 아닌 신설).
...
- **G5 쇠퇴 강등 (2채널, R22):** driver 자격은 매 vintage 갱신하되 *두 종류*의 쇠퇴를 본다 — 점오차 drift와 coverage drift는 다른 실패다(점추정은 맞아도 밴드 폭이 틀릴 수 있고, 그 역도 성립).
  ...
  - **★silent 확대 금지(핵심 가드)**: coverage가 낮다고 *조용히 σ를 키워* 명목 coverage에 맞추면 사후적합(post-hoc overfit, horizonMeaning "확신오정렬" 재현)이다. 재추정한 σ는 **held-out 검증을 통과한 σ만 승격**(§2B.11 acceptance threshold·§3.7 fan σ 확대도 검증 σ만). 검증 미통과 σ는 강등 라벨로만 표면화하고 active로 안 올린다.
  - **채널C (신규) vintage-revision drift 라이브 재검 (R45, 2채널→3채널)**: §2B.5 #2 `revisedDataBias` 는 admission(적합 시점)에서만 본다 — 배포 *후* 라이브에 입력 vintage 가 다시 개정되는지 보는 채널이 없다. macro 시리즈가 active 배포 후 다시 revise 되면(GDP 는 분기마다 개정) 라이브 점오차가 *데이터 변경* 탓인지 *모델* 탓인지 분리 불가다 → 데이터 변경을 모델 실패로 오귀속하는 위험. 가드(새 게이트 메커니즘 0 — 라벨 비교만): `LiveScore` 에 `inputVintageId` field 를 append 하고, 라이브 평가 시 forecast 의 `fitProvenance.macroVintageId`(rank14) ≠ actual 평가 시점 `macroVintageId` 면 `SimulationResult.warnings` 에 `input_vintage_revised_since_forecast` append + 그 점오차를 **model-error 로 귀속하지 않는다**(decay 채널A 의 driftStat 에서 제외 — 데이터 변경분을 모델 쇠퇴로 잘못 retire 시키지 않음). 채널A(점오차)·채널B(coverage)가 *모델/밴드* 실패라면 채널C 는 *입력 데이터 변경* 식별이라 셋이 직교.
...
class TransferSpec:
    ...
    lag: int                 # 0|1|2
    transform: str           # "level"|"yoy"|"diff" — ★G0.5 정상성 pre-test 가 *데이터로* 선택
                             #   (§2B.2 R11, 사람 손튜닝 아님; 비정상 시리즈는 level 금지→diff/yoy)
...
class LiveScore:
    ...
    inputVintageId: str | None      # R45 — 채널C vintage-revision drift: 라이브 평가 시점 입력 vintage.
                                    #   forecast fitProvenance.macroVintageId ≠ 이 값이면
                                    #   input_vintage_revised_since_forecast (점오차를 model-error 로 귀속 안 함)
...
- **B 차원 붕괴:** N 시리즈 → 6~8 축(Stock-Watson diffusion index). 새 데이터는 raw if가 아니라 축 loading. transfer 계수는 pooled 베타에서 **학습**(predictionSpace `3.0/2.0/1.5/0.5` 손튜닝 마법상수 교체), 축 *level 정의*는 curated 고정(folk-stat 회피). seed=`predictionSpace`(6축)+`exogenousAxes`(라우팅)+`shrinkage`(축 공분산 안정화).
  > **★prior-as-shrinkage-target 정정(R9, 베이지안 conjugate 닫힌형 — 추정기/파일 0):** pooled-β가 curated prior 를 *교체(replace)* 하는 것이 아니라 *수축 목표(shrinkage target)* 다. 정본 = 가중평균 `β_post = w·β_pooled + (1−w)·β_prior`, `w = N_eff / (N_eff + τ)`(N_eff = 유효 time-DoF ≈T, §2B.1; τ = prior 강도 잠정상수, §2B.11 잠정값 박스에 등록·졸업 재보정). binary(pooled 충족이면 통째 학습 / 미달이면 통째 prior)를 **연속화** — pooled-N<60 으로 dormant 인 카드도 *데이터가 일부라도 있으면* w 만큼 prior 를 갱신한다(0→1 단절이 아니라 w∈[0,1] 연속). 수축량은 provenance 로 표면화: `elasticity:반도체(post, w=0.6, prior+pooled)`(w 가 클수록 데이터 우세, 작을수록 prior 우세 — 어느 쪽에 얼마나 기댔는지 숨기지 않음). 닻 = w=0(데이터 0)이면 순수 prior `elasticity_prior_unvalidated`(§2B.3 A2)·w=1(prior 강도 0)이면 순수 pooled, 두 극단이 기존 binary 의 특수해라 SSOT 분열 없음.
...
> **★sentiment/liquidity preset 비대칭 해소 (R32 — 새 게이트 0):** elasticity prior 는 `elasticity_prior_unvalidated` provenance + 점선으로 *라벨과 함께 자기-적발*하나, 위 `sentiment/liquidity는 exogenous 미존재→preset 유지`의 두 preset 가정은 *동급 무검증 가정인데 라벨이 없다*(한쪽만 적발하는 비대칭). 두 축은 외생 transfer 가 없어 preset 값을 그대로 쓰므로(elasticity prior 와 동일한 무검증 가정), 그 결과에 `sentiment_preset_unvalidated`·`liquidity_preset_unvalidated` provenance + `SimulationResult.warnings` 에 `flow/sentiment axis has no exogenous transfer — preset assumption(honest-gap)` 를 append 한다(§2.3 CRP·elasticity prior 전파와 동형 계약). 08 §3.4 점선-근사 provenance 목록에 두 접두(`sentiment_preset_`·`liquidity_preset_`)를 추가해 *시각도 점선 일관*(검증된 transfer 만 실선, 08 소유자 cross-ref). 자기 플랜이 한 축은 적발하고 다른 축은 침묵하는 비일관을 닫는다.

### 2B.8 수렴 증명 — if 상한이 시리즈 수와 독립 (Stock-Watson diffusion index)

**active edge 상한 = 검증된 축 수(6~8), 시리즈 수 N과 독립.** N개 외생 시리즈가 추가돼도 G3 routeAxis가 대부분을 6축 중 하나에 흡수(`absorb`) — 새 시리즈는 raw if가 아니라 *축 추가 loading*(diffusion index에 더 많은 관측 = 추정 정밀도↑, 차원↑ 아님). block 응축이 두 번째 안전판(customs HS 17→교역 block 1). 신축만 +1 차원인데 4게이트+수동 승인이라 통과율 극저 → **dN/d(시리즈)→0, active edge는 6~8에 점근.** 설계 한계: '축 정의 curated 고정' 전제 위에서만 성립(2B.4-B). 축을 데이터 주도로 자동 신설하면 folk-stat 재현(horizonMeaning 함정) → 신축만 4게이트+수동. ★단 k≤6 / 6~8축 수렴은 S_T 컷오프(§2B.11)가 held-out 데모서 보정된 *후에만* 닫힌 bound — 그 전까지 본 절은 proven bound 아닌 design target.
...
> **★잠정값 경고:** 아래 여섯 수치(min-T 12·pooled-N 60·holdout-k 4·Sobol S_T 0.05·embargo window 1분기·purge gap = label horizon 2분기) + **R9 수축 2상수(prior 강도 τ·firm 수축강도 K)**는 *잠정값* — un-calibrated, seed/CI=0, 아직 discipline 보장이 아니다(졸업 데모서 held-out 재보정). 현 단계 = design target. **이 12+종 잠정임계의 inline 매직넘버 금지·단일 명명상수 모듈 집결은 §2C.1 직후 잠정임계 SSOT 규약(rank18) 참조.**

> **★반-하드코딩 규약 (잠정임계 단일상수 SSOT — rank18, 새 수치 0):** 본 플랜의 잠정임계는 12+종(min-T 12·pooled-N 60·holdout-k 4·Sobol S_T 0.05·embargo 1Q·purge=horizon 2Q·R9 τ/K·receivable-led 10pp(§3.7)·one-off N=8·±2σ(§3.8)·driverCoverage 임계(§2C.2)·margin median window 등)이 본문 여기저기 흩어진 literal 이다. 규약 2줄:
> 1. **단일 명명상수 모듈 집결 + 코드레벨 provenance 태그**: 모든 잠정임계를 `simulate/` 의 단일 명명상수 모듈로 모으고 각각 `provenance='provisional-uncalibrated'` 태그를 단다. admission.py·evaluateSheet·게이트 함수 본문에 **inline 매직넘버 금지**(졸업 AC = `게이트임계 inline 매직넘버 0건` 정적검사 — 09 §6 `test_simulate_leaf_binding` leaf-binding 정적검사와 동형). 플랜 자신이 §2B 에서 비판한 factor-zoo 마법상수(`SECTOR_ELASTICITY` inline·predictionSpace `3.0/2.0/1.5/0.5` 손튜닝)와 같은 죄를 *자신의 게이트 임계*가 짓지 않도록 일관 적용.
> 2. **un-calibrated 임계 → `threshold_uncalibrated` 완전성 채널**: 잠정(미보정) 임계를 쓰는 게이트가 내는 결과는 자동으로 `SimulationResult.warnings`(§2C.2 동일 채널)에 `threshold_uncalibrated` 를 append 한다 — 재보정(졸업 데모) 전까지 그 게이트가 PASS 를 내도 **`미검증 PASS`** 라벨로만 표면화한다(검증되지 않은 임계로 통과시킨 것을 검증된 PASS 로 위장 금지, §2B.2 G5 silent 확대 금지 정신). 졸업 데모서 임계가 held-out 재보정되면 그 임계의 태그를 `calibrated` 로 승격하고 warning 을 끈다.

> **★shrinkage SSOT 통일(R9 — 3 site 동일 메커니즘):** 본 플랜은 shrinkage 를 *세 곳*에서 쓴다 — ⓐ **prior→pooled 수축**(§2B.4-B `β_post=w·β_pooled+(1−w)·β_prior`)·**firm→pooled 수축**(§2B.1 `β_i=(1−λ)·β_pooled+λ·β̂_i`) = *계수* 수축(empirical-Bayes), ⓑ **G2 effective-N / 후보 상관행렬 안정화**(§2B.2·`shrinkage`) = *선택통계* 수축, ⓒ **Σ 대각 shrink**(§3.11 R21b Ledoit-Wolf류 ill-conditioned 보호) = *공분산* 수축. 세 site 는 모두 "작은 표본의 분산을 더 강한 구조(prior·대각·축)로 끌어당긴다"는 **동일 shrinkage 원칙**이며, 거처는 `shrinkage`(quant/factor/shrinkage.py) 단일 모듈로 통일한다(새 추정기 0 — 계수 수축은 닫힌형 가중평균, 공분산은 기존 Ledoit-Wolf 경로 재사용). 수축강도(τ·K·Σ 대각 가산)는 전부 §위 잠정값 박스에 등록·졸업 재보정. 세 site 가 *서로 다른 임시 수축식*으로 흩어지면 SSOT 분열(`feedback_always_check_clutter`).
...
- **pooled-panel β가 leave-last-4Q OOS에서 GDP-beta 단순모델 대비 partial-R² 개선**(개선 없으면 pooled-panel transfer가 단순 1-beta보다 나을 근거 0 → §2C volume/price fallback과 동급으로 강등). **★seasonal-naive floor 동반(R11, skill 낙관편향 차단)**: GDP-beta 외에 *순수 산수* seasonal-naive(`x̂_t = x_{t−4}`, lag-4 전년동기) 도 floor baseline 으로 둔다 — pooled-β 가 이 무학습 lag-4 floor 마저 못 이기면 transfer 의 정보량이 계절성 이하다(03 §4.4-baseline 에 seasonal-naive floor 행을 동반, 03 소유자 cross-ref). 두 floor(GDP-1beta·seasonal-naive) 동시 상회가 졸업 partial-R² 합격선.
...
게이트 0이 닫히기 전엔 (a)~(b)가 *강제 불가능한 주장*(09 §10.2 "Brier 규율은 강제 불가")이라, write-end를 0번으로 못 박는 것이 100점 차단 항목의 명시적 인정이다.

---

## 2C. Driver Coverage Census — 능력 천장 (비전↔가용 거리, 새 추출능력 약속 0)

00 §5.3 Business Driver Bridge는 11 driver(volume/price/mix/fx/backlog/capacity/rawMaterial/labor/logistics/rnd/depreciation, §2.6 `BusinessDriverEstimate.driver` enum)가 *모두* 이벤트→손익으로 전파되는 비전을 그린다. 그러나 driver별 *결정론 추출 경로*의 실제 coverage는 driver마다 천차만별이고, 어떤 driver는 경로 자체가 부재다. 이 거리를 숨기면 8b의 "텍스트→driver 매핑 커버리지 미실증"(가장 큰 위협 ①)이 침묵 낙관으로 재현된다. 본 절은 그 거리를 *행 단위로 박제*하고, coverage<임계면 시뮬레이션이 스스로 'honest-gap'을 외치게 계약한다. **새 driver 추출 능력 약속 0 — 가진 능력의 천장을 명시적으로 표면화할 뿐**(메모리 segmentRnd 2/10·incomeExpense 66.9% 천장과 동형).

### 2C.1 Census 표 (★실측치 = 잠정값, 본진 졸업 데모서 KOSPI200 전수 재측정)

> **★coverage% 잠정 경고:** 아래 coverage%는 `tests/_attempts`(segmentRnd·incomeExpense) 소표본 실측에서 *유추*한 잠정값이다 — KOSPI200 전수가 아니고 held-out 검증도 0. 졸업 데모서 KOSPI200 실측으로 재보정한 *후에만* proven. 현 단계 = design target + 능력 천장 라벨.
> **★scope 의존 명시화(R55):** 아래 coverage% 는 *암묵 단일 scope* 에서 측정된 값이다 — IFRS 8 세그먼트 주석·R&D 주석 등 *주석 의존* driver(mix·rnd)는 주석이 **연결레벨**에서 작성되므로 별도-only 회사에선 주석이 부재할 수 있는데, `mix 2/10≈20%` 같은 수치가 이 scope 의존을 숨긴다. 졸업 데모 KOSPI200 재측정 시 **주석 의존 driver 의 coverage% 를 연결/별도 scope 별로 분리 보고**한다(단일 % 가 어느 scope 의 것인지 표면화).
...
| mix | `_noteCellsFromPanel("NT_D871100")`(세그먼트 주석, `_revenueSelect.py`) | 영업부문 주석(IFRS 8), XBRL `NT_D871100` | **2/10 ≈ 20%**(segmentRnd 실측 — 축-태깅 vs 행-라벨 인코딩이 가름) | 회사 전체 proxy(세그먼트 없음 라벨) | `partial` — XBRL 인코딩 + **IFRS 8 연결레벨 의존(scope 의존 — 별도-only 시 주석 부재 가능, R55)** |
...
| rnd | 2-tier: IS `select`(연구개발비) → SG&A 주석 `NT_D834310` | IS 본문 + R&D 주석(XBRL `NT_D834310`) | **6/10 ≈ 60%**(segmentRnd 실측) | IS 본문만(주석 결손 라벨) | `partial` — 2-tier + **주석 Tier 는 작성 scope 의존(R55)** |
| depreciation | CF 비현금 가산 + 비용성격 주석 | CF 본문(감가상각비 가산) + 주석 | 비교적 높음(CF 본문 표준) | 주석 결손 시 CF만 | `partial` |
| operating-income-definition (R27 라벨 행) | (분리 leaf 약속 0 — 공시 라인 존재 시 분리만) | IS 영업이익 라인 + 지분법손익 표시(연결=영업외 우세 / 별도=포함표시 우세, K-IFRS) | 표시정책 의존(판별 불가 多) | 영업이익 통째 매출연동(지분법기여 미분리 라벨) | `partial` — 지분법 포함 여부 unverified 시 operating leverage 왜곡(§3.8 R27) |
...
- volume/price처럼 coverage 0%(경로 부재) driver는 분해를 *시도조차 안 하고* 매출성장을 통째로 섹터 1-beta 근사로 폴백 — provenance `default:no-volume-price-split`. 이는 "volume과 price를 합쳐서 하나의 성장률로만 두지 않는다"(§3.7 주의)의 *능력 한계 표기*이지 규칙 폐기가 아니다(분해 가능한 회사는 여전히 분리).
...
- **새 추출 능력 약속 0.** 본 절은 volume/price 분해 leaf를 *신설하겠다는 약속이 아니다* — 그 경로는 데이터(비정형 생산·판매 실적 표)가 막혀 있고(segmentRnd 2/10·incomeExpense 66.9% 천장이 같은 데이터-부재 벽), 약속하면 8b honest caution("새로움≠검증")의 재현이다. coverage%를 *명시적으로 표면화*하는 것이 산물이지 coverage%를 *올리는* 것이 본 절 약속이 아니다.
...
1. macro regime을 **결정론 규칙으로 판정한다 — 판정 불가 시 `unclassified` 라벨**(R31, 빈 동사→정의된 동사·미판정 명시).
...
**★macroRegime 분류규칙 결정론 명세 (빈 동사 채움, R31 — 새 leaf 0):**

step 1 "macro regime을 판정한다"·01 §5b R12(1) `macroRegime` 박스의 *동사가 비어 있었다*(무엇으로 어떻게 판정하는지 미정의). 결정론 명세:

- **판정 입력 = 기존 leaf 재사용**: `macro/forecast/nowcast.py::gdpNowcast`(Kalman 현재국면 추정, 01 §16 실재 leaf — 코드 grep 확인)의 산출(현재 GDP 모멘텀/국면)을 `core/schemas.py` 의 regime enum 으로 **결정론 매핑**한다(예 nowcast 모멘텀 부호·임계대 → expansion/slowdown/contraction/recovery enum, 임계는 §2B.11 잠정상수 모듈 등록·rank18 태그). **새 leaf 0** — gdpNowcast 재사용, regime 추정기 신설 아님.
- **판정 불가 = `unclassified` 미판정**: gdpNowcast 가 입력 결손·미도달이면 `snapshot['macroRegime']='unclassified'` 라벨(0 대체·임의 추정 금지, §7 "결손값 0 처리 금지" 정신). 빈 동사를 *추측으로* 채우지 않고 *`unclassified`로 명시*한다.
- **소비처 정합**: 이 regime 필드는 §3.11 R34 regime-conditional Σ·01 §5b R12-(1) regime-conditional σ 가 *동일 필드*로 재사용(SSOT 단일, regime 분류를 여러 곳에서 따로 정의 금지).
...
**★forensic → PD/등급 전파 (회계품질 적신호의 solvency 채널, R26 — 새 채널 0):**

위 FQ2 의 동일 `quality.baseline` flag(accrualRatio·cfToNi 등)는 fan σ 하방뿐 아니라 **solvency 뷰 PD/등급에도 하방 전파**한다 — Sloan accrual·Beneish M-score 는 *valuation* 보다 *부도예측* 신호가 더 강하다는 것이 회계 forensic 의 실증이다(저품질 이익 → hazard 상향·신용등급 notch-down 후보). elasticity 미검증엔 경고를 solvency 로 전파하면서 회계 적신호엔 PD 채널을 안 준 비대칭의 해소다. 단:

- **전파 방향 = 하방·단방향**: 저품질 이익 → PD ↑(hazard 상향)·등급 notch-down 후보. 고품질 이익이 PD 를 낮추는 역방향은 *주지 않는다*(과대계상은 아래로 정정되지 위로 안 간다는 Sloan 비대칭, FQ2 σ 하방가중과 동형).
- **baseline actual 에서만 가중(R17 동형 가드)**: 09 §4 solvency leaf 가 actual 단면 기반이므로, *projected* 단면 forensic 은 R17(§3.9)처럼 `proforma_cash_plug — identity-closed, not forensic` 라 신호가 죽는다 → **PD 전파는 baseline actual 단면에서만** 켠다(projected cashConversionGap 으로 PD 를 흔들지 않음, 동일 R17 라벨 재사용). 09 §4.4 에 `forensic PD 가중 = baseline actual 한정 (projected cash-plug 신호 죽음)` 행 동반(09 소유자 cross-ref).
- **결손 처리**: accrualRatio·cfToNi 입력 결손이면 PD 가중 미적용 + `status='partial'`(0 대체 금지, 무근거 notch-down 금지 — FQ2 결손 처리·§7 "결손값 0 처리 금지" 재사용).
...
3. **불가 시 한계 표기**: 시계열이 짧아(분기 < N) median/σ 산출 불가면 정규화 못 함 → `provenance='margin_baseline_unnormalized(partial)'` + `SimulationResult.warnings`에 `margin baseline not normalized — one-off risk(partial)` append(§2C.2 driverCoverage와 동일 채널). 침묵 진행 금지.
...
**★영업이익 정의경계 (K-IFRS — operating_profit 정의중립 가정의 정정, R27):**

위 `operatingIncome = grossProfit - sga` 와 `simulate/registry.py::_baseMetrics`·proforma 가 읽는 `operatingIncome` 을 **정의중립 보편값**으로 취급하는 것은 K-IFRS 에서 정확하지 않다 — **지분법손익(equity-method income)의 영업이익 포함 여부**가 회사·표시정책마다 다르다(연결은 보통 영업외, 별도 재무제표는 영업이익에 포함표시하는 경우가 우세). 지분법손익이 영업이익에 섞여 있으면 *매출연동 operating leverage* 가정(§3.8 step 5 "매출 변화에 따른 operating leverage")이 왜곡된다 — 지분법기여는 회사 매출과 연동되지 않는데 매출에 곱해 전파하면 거짓 레버리지가 된다.

- **분리 시도**: baseline 영업이익에서 지분법손익(공시 라인 존재 시)을 분리해 **영업핵심마진**(매출연동 전파 대상)과 **지분법기여**(매출 무관, 별도 라벨)로 가른다. 매출연동 operating leverage 는 영업핵심마진에만 적용.
- **분리 불가 시 한계 표기**: 지분법손익 라인이 공시에 없거나 영업이익 포함/제외가 판별 불가면 `provenance='operating_income_definition_unverified(partial)'` + `SimulationResult.warnings`에 `operating income definition unverified — equity-method inclusion unknown(partial)` append(§2C.2 driverCoverage 동일 채널). 침묵 진행 금지.
- **새 추출 능력 약속 0**: 본 소절은 지분법손익 추출 leaf 를 *신설하겠다는 약속이 아니다* — 공시 라인이 있으면 분리, 없으면 라벨이다(§2C.3 "coverage% 를 *올리는* 것이 약속 아님" 정신). §2C.1 census 에 `operating-income-definition` 행을 *라벨 행으로만* 추가(새 추출 능력 약속 0, §2C.3 정신).
...
**★netDebt 정적상수 한계 명시 (`equityValue = EV − netDebt` 의 한계, R10 — 새 노드/패널 0):**

위 식의 `netDebt`는 **정적 단면(actual)**이다 — 시나리오가 매출·마진·FCF 를 projected 로 전파하면서도 차입경로·이자비용 동학은 *투영하지 않는다*(부채는 base 시점 값으로 고정). 따라서 강제 라벨 = `provenance='equity_via_static_netdebt — structure dynamics not projected'`(R17 baseline/projected 분리 라벨과 동형 — projected 마진은 동학인데 netDebt 만 정적인 비대칭을 숨기지 않음).

- **일관 단언(R10 핵심)**: 동일 금리경로가 valuation 쪽 WACC(할인율 상승 → EV 하락)와 solvency 쪽 coverage(이자비용 상승 → DSCR 하락)에 *반대 방향으로 침묵 적용*되지 않게 한다 — 금리 +200bp 시나리오가 WACC 는 올리면서 netDebt 이자부담은 base 값 그대로 두면 valuation 과 solvency 가 같은 충격에 비일관(09 §4.4 solvency 표가 actual 기반인 것과 짝). 09 §4.4 에 `netDebt=정적단면(actual) — projected 차입경로·이자비용 동학 미투영` 한계 표기 행을 동반(본 §3.10 식 라벨 ↔ 09 §4.4 표 행의 SSOT 짝, 09 소유자 동행 적용 cross-ref).
- **부채동학 투영은 Phase B defer**: 차입경로·롤오버·이자비용 동학의 projected 전파는 새 노드(부채 스케줄러)라 현 4노드 코어 밖 — Phase B 정공법으로 미룬다. *지금* 닫는 것은 silent 비일관의 **라벨화**(투영 없는데 있는 척 안 함)뿐이다.
...
- **★`rEvent`는 유동성 조건부다 (R57 — 새 추정기 0):** price bridge 가 `rFund/rFactor/rEvent` 를 분해하나 `rEvent` 가 thin-liquidity·저거래대금 종목에서 *전이적(mean-reverting)* 임을 안 보면, *영구/전이* 구분 없이 event 항을 영구 가치변화로 오독한다. 저유동성(거래대금 하위) 종목의 event 항은 flow-driven 전이 괴리일 수 있어 *영구 가치변화로 단정 금지* → `event term may be transient under thin liquidity(partial)` 라벨. 유동성은 price 단계에서 이미 읽는다(§3.3 step 5 liquidity — 새 입력 0). 08 §3.4 bridge waterfall 의 event 부유바는 저유동성 시 점선-근사 배지를 *상속*(08 소유자 cross-ref, 검증된 영구 항만 실선).
...
- **★동시점 Σ 한계 라벨 (R53 — 새 코드 0, 한계 표기 1줄):** 위 Σ 는 **동시점(contemporaneous) cross-correlation 만** 잡는다 — driver 충격의 *시점 간 전파*(한 분기 fx 충격이 다음 분기 cogs 로 lag 전달 = VAR/impulse-response)는 Σ 한 장으로 못 잡아 *다기간 누적 충격*의 분산이 과소 추정된다. 강제 라벨 = Σ 에 `covariance:contemporaneous-only`, 다기간 cone 에는 `dynamic_propagation_unmodeled`(추정 약속 0 — lag 전파 추정기 신설 아님, 동적 VAR/impulse-response 는 01 §9 동급 *별도 천장*). "동시점 Σ 로 다기간 분산을 다 잡았다"는 과신을 차단(점추정 금지·미검증 라벨 정신).
...
- **σ provenance 시각 규율 정합**: Σ 기반 fan은 §2B.5(능력 한계) #2·05 §시각 인코딩 SSOT(검증 σ=실선 / 미검증 elasticity prior=점선·회색)를 따른다 — Σ가 pooled-OOS residual(검증)이면 실선, prior 합성이면 점선.

**★Σ 도 regime-conditional (σ 층화와 대칭, R34 — 새 데이터 0):**

σ 는 regime 층화하는데(01 §5b R12-(1) regime-conditional σ) Σ 는 단일 *무조건* pooled = **correlation breakdown 미반영**(위기 시 상관이 1로 수렴하는 현상). 평시 상관으로 적합된 Σ 를 severe 분기 Cholesky 가 그대로 쓰면 joint 꼬리를 *과소* 추정한다(분산이 거짓으로 좁아져 severe 가 덜 severe 해 보임).

- **Σ_regime 층화**: Σ 를 regime indicator(§3.4 R31·01 §5b R12-(1)과 *동일 필드* 재사용 — regime 분류 SSOT 단일)로 층화해 `Σ_regime`(평시/수축 등)을 추정한다. σ 가 이미 쓰는 그 regime 필드를 Σ 가 같이 쓸 뿐이라 **새 데이터·새 추정기 0**.
- **작은 표본 보호**: regime 층화는 표본을 쪼개므로 pooled-N<60 이면 `Σ_regime` 를 empirical-Bayes 로 무조건 pooled Σ 쪽으로 shrink(§2B.11 shrinkage SSOT 재사용) + `SimulationResult.warnings` 에 `regime_corr_shrunk` append.
- **severe 분기 = Σ_contraction 우선**: severe/adverse 분기 fan 은 평시 Σ 가 아니라 `Σ_contraction`(수축 regime 상관)을 우선 적용해 joint 꼬리를 과소추정하지 않는다.
- **자연헤지 부호는 regime 무관**: §3.11 위 'curated prior 부호 제약'(fx↑↔cogs↑ 자연헤지 음의 상관)의 *부호*는 regime 과 무관하게 유지(수축 regime 이라고 자연헤지가 양으로 뒤집히지 않음) — regime 이 바꾸는 것은 *상관 강도*(위기 시 동방향 상관 강화)지 부호가 아니다.
