...
- **min pooled-N (횡단면 풀 하한)**: pooled-panel 적합은 `섹터 내 기업수 × 유효 분기 ≥ 60`(예: 기업 10 × 분기 6) 미만이면 카드 `admission.state=dormant` + `warnings=["pooled_N_below_floor"]`. 풀 경계는 OQ2(WICS 11 vs sub-sector)에서 확정. 단 pooled-N≥60 은 *기업-수준 응답 이질성*의 하한일 뿐 — macro-β t-stat 의 자유도는 pooled-N 이 아니라 유효 time-DoF(≈T)를 쓴다(common-shock 독립관측 ≈T, §2B.1).
