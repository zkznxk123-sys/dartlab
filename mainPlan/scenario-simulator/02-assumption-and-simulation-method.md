...
- `sourceRefs`. — 명명 프리셋의 **출처-정직 라벨 SSOT**. preset description이 규제 stress인 척하면(예: `synth/scenario.py`의 "CCAR 스타일"/"CCAR-style severe recession" 문구) 여기에 `curated-prior, not DFAST/CCAR-blessed`를 **강제 노출**해 검증된 stress 오인을 차단한다(00 §7.2-6 컨센서스 금지·kill-list 정합).
  ...
  > **★KR preset 정정 권고(R28)**: BOK(한국은행)는 CCAR 를 *운영하지 않는다* — CCAR/DFAST/SCAP 은 Fed 프로그램명이라 KR preset 에 이 이름을 차용하면 *권위 환각*(출처-정직 위반, `feedback_no_fabricated_urls` 정신의 권위 버전)이다. KR preset 의 `CCAR 스타일` → `심각한 경기침체(curated)` 로 정정 권고(US preset 은 실제 Fed 시나리오 인용 시 라벨 동반으로 허용). 이 정정은 §2B.3 A2 `curated-prior` 라벨·00 §7.2-6 kill-list 와 일관.
  > **★preset tail-anchor 라벨(R37 — 라벨, 코드 0):** `severe`/`adverse` 분기는 *최악 프리셋 한 점*이지 그것이 분포 *꼬리(tail)* 임을 검증하는 면이 없다 — curated 프리셋은 CCAR 식 percentile 앵커(P5/P1 등)가 없어 "최악 프리셋 = 꼬리"가 *오인*된다. 정직 분위 라벨 = 각 명명 프리셋 결과 손실을 `mc.distribution`(01 §5b) 경험분포 분위로 매핑해 `adverse≈P12(꼬리 아님)`·`severe-equivalent≈P5` 처럼 *분위*를 노출한다(프리셋 손실이 분포의 어디쯤인지 표면화). `mc.distribution` 미구현 동안은 `tail-anchor: unverified(분포노드 미구현)` 라벨(§4.4 미검증 동형·점추정 금지). 05 §5 분기 범례에 이 분위 라벨을 동반(05 소유자 cross-ref) — 색은 기존 분기 팔레트 그대로(새 색 0), 라벨 텍스트만 추가.
- `createdAt`.
...
- **B 차원 붕괴:** N 시리즈 → 6~8 축(Stock-Watson diffusion index). 새 데이터는 raw if가 아니라 축 loading. transfer 계수는 pooled 베타에서 **학습**(predictionSpace `3.0/2.0/1.5/0.5` 손튜닝 마법상수 교체), 축 *level 정의*는 curated 고정(folk-stat 회피). seed=`predictionSpace`(6축)+`exogenousAxes`(라우팅)+`shrinkage`(축 공분산 안정화).
  > **★prior-as-shrinkage-target 정정(R9, 베이지안 conjugate 닫힌형 — 추정기/파일 0):** pooled-β가 curated prior 를 *교체(replace)* 하는 것이 아니라 *수축 목표(shrinkage target)* 다. 정본 = 가중평균 `β_post = w·β_pooled + (1−w)·β_prior`, `w = N_eff / (N_eff + τ)`(N_eff = 유효 time-DoF ≈T, §2B.1; τ = prior 강도 잠정상수, §2B.11 잠정값 박스에 등록·졸업 재보정). binary(pooled 충족이면 통째 학습 / 미달이면 통째 prior)를 **연속화** — pooled-N<60 으로 dormant 인 카드도 *데이터가 일부라도 있으면* w 만큼 prior 를 갱신한다(0→1 단절이 아니라 w∈[0,1] 연속). 수축량은 provenance 로 표면화: `elasticity:반도체(post, w=0.6, prior+pooled)`(w 가 클수록 데이터 우세, 작을수록 prior 우세 — 어느 쪽에 얼마나 기댔는지 숨기지 않음). 정직 닻 = w=0(데이터 0)이면 순수 prior `elasticity_prior_unvalidated`(§2B.3 A2)·w=1(prior 강도 0)이면 순수 pooled, 두 극단이 기존 binary 의 특수해라 SSOT 분열 없음.
...
> **★sentiment/liquidity preset 자기-적발 비대칭 해소 (R32 — 새 게이트 0):** elasticity prior 는 `elasticity_prior_unvalidated` provenance + 점선으로 *정직하게 자기-적발*하나, 위 `sentiment/liquidity는 exogenous 미존재→preset 유지`의 두 preset 가정은 *동급 무검증 가정인데 라벨이 없다*(한쪽만 적발하는 비대칭). 두 축은 외생 transfer 가 없어 preset 값을 그대로 쓰므로(elasticity prior 와 동일한 무검증 가정), 그 결과에 `sentiment_preset_unvalidated`·`liquidity_preset_unvalidated` provenance + `SimulationResult.warnings` 에 `flow/sentiment axis has no exogenous transfer — preset assumption(honest-gap)` 를 append 한다(§2.3 CRP·elasticity prior 전파와 동형 계약). 08 §3.4 점선-근사 provenance 목록에 두 접두(`sentiment_preset_`·`liquidity_preset_`)를 추가해 *시각도 점선 일관*(검증된 transfer 만 실선, 08 소유자 cross-ref). 자기 플랜이 한 축은 적발하고 다른 축은 침묵하는 비일관을 닫는다.
...
> **★scope 의존 정직화(R55):** 아래 coverage% 는 *암묵 단일 scope* 에서 측정된 값이다 — IFRS 8 세그먼트 주석·R&D 주석 등 *주석 의존* driver(mix·rnd)는 주석이 **연결레벨**에서 작성되므로 별도-only 회사에선 주석이 부재할 수 있는데, `mix 2/10≈20%` 같은 수치가 이 scope 의존을 숨긴다. 졸업 데모 KOSPI200 재측정 시 **주석 의존 driver 의 coverage% 를 연결/별도 scope 별로 분리 보고**한다(단일 % 가 어느 scope 의 것인지 표면화).
...
1. macro regime을 **결정론 규칙으로 판정한다 — 판정 불가 시 `unclassified` 라벨**(R31, 빈 동사→정의된 동사·정직 abstention).
...
- **판정 불가 = `unclassified` 정직 abstention**: gdpNowcast 가 입력 결손·미도달이면 `snapshot['macroRegime']='unclassified'` 라벨(0 대체·임의 추정 금지, §7 "결손값 0 처리 금지" 정신). 빈 동사를 *추측으로* 채우지 않고 *정직한 abstention* 으로 채운다.
...
- **분리 불가 시 정직 라벨**: 지분법손익 라인이 공시에 없거나 영업이익 포함/제외가 판별 불가면 `provenance='operating_income_definition_unverified(partial)'` + `SimulationResult.warnings`에 `operating income definition unverified — equity-method inclusion unknown(partial)` append(§2C.2 driverCoverage 동일 채널). 침묵 진행 금지.
...
**★netDebt 정적상수 정직화 (`equityValue = EV − netDebt` 의 한계, R10 — 새 노드/패널 0):**
