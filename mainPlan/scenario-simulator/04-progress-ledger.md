...
✅ **개념검증 + foundation 졸업 완료(2026-06-14)** — ★layer subtlety 는 **§10 born-clean 으로 해소**(아래 "1." 의 a/b/c 고민 불필요): simulate/ 는 *소비처 0 신생, 기존 simulation.py 무접촉* → transfer 는 L2.5 에 새로 태어나고(synth L1.5 상수만 forward import) legacy `_applyMacroShock` 잔존(§4 move 는 L2→L2.5 역방향이라 *회피*, BC 위임은 성숙 후 별도). 
  ...
  - **진행 중**: `registry.py`(driver 노드 정의: macro.path→rev.path[transfer]→proforma[buildProforma]→dcf[calcDFV]) + `run.py`(runScenario 내부 실행→SimulationResult).
  - **다음 phase**: 공개 verb `dartlab.simulate`/`Company.simulate`(apiContract+EngineCall) · DriverRegistry 수렴(pooled-panel transfer) · lens 보조(`ai/tools/lens.py`, det 평행) · Play 미래리플레이 · (선택)import-linter pyproject contract entry.
