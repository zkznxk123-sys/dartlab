...
- [x] MC seed kill-test 선결 명시 (P1)
  - **착수 준비 실측(2026-06-14)**: ① **seed→PCG64**(분포 불변·재현성만, 安) = `_simMonteCarlo.py:12,145,202-203`(`import random`/`random.seed`/`random.gauss`) + `pricetarget.py:11,278,318` 둘 다 `np.random.default_rng(seed)`+`rng.normal`. ② **:205 cumprod**(제품 cone 확대·회귀 위험, 운영자 가시 데모 필요) = `_simMonteCarlo.py:197-211` 내부 `for yr` 루프가 `simRev`/`simMargin`을 **매년 덮어써** 마지막 해 노이즈만 반영(중간 해 폐기). fix=연도별 성장계수 cumprod(`cumRevFactor*=(1+revNoise)`·margin은 `cumMarginNoise+=`, mean path 보존+cone 호라이즌 확대). **회귀 표면 낮음**: 단언 = `test_simulation.py:353` expectedValue>0 · `:385` adverse<base(정성적, 정확값 단언 0) → cumprod fix 후에도 통과 예상, kill-test가 '기존=마지막해만'을 먼저 증명. ①②분리 실행(①먼저 安).
