...
| 06-index-chart | ✅ v0.3 | subject 소유권 seam(CenterStack-local $state, ctl 미상향)·IndexPort(catalog/search/series)·US 지수 부재 정직 가드·코드 그라운드 전수 |
...
12. **★US 지수(SP500/NASDAQ) — ui 데이터 전무(grep 0건). 운영자 결정 필요:** (a) 별 트랙으로 US 소스 수집 후 close-only 라인차트(고저 부재 → MA/RSI만) subject 추가, (b) 06은 KR gov 지수로 확정·US는 데이터 확보 전까지 범위 밖. 어느 쪽이든 06 핵심(KR subject) 불변. 06 §6.
