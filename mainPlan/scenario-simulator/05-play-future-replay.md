...
상태: PRD v0.3 (2026-06-20 9인 패널 백로그 — 시각 인코딩 SSOT·레이아웃 와이어프레임·ReportDock 거처·초보 학습성·상태 카피 매트릭스 추가)
...
- **모드 색 구분(정직 가드)**: `simulate` 일 때 transport·세그 active 를 **amber(#fb923c)** — 과거 replay 는 중립 톤. amber = HonestyFooter Tier3 active 경고색(`HonestyFooter.svelte:110,121`)·"미검증·가정 구간" 의미색 재사용(§10 status 표 hypothesis/active 행과 정합). 무지개 금지.
...
- **fan 범례(점추정 아님 캡션 상존)**: 'P50 중심 · P25–P75 / P5–P95 백분위 밴드' 범례 + 하단 캡션 **"백분위 밴드 (점추정 아님)"** 상시. 단일선 강조 금지(§2 fan chart 단일선 거부 재확인). σ provenance 가 미검증(`elasticity_prior_unvalidated`)이면 점선·회색 근사 fan(§3.1 σ provenance 규율·§10 estimate/hypothesis 채널).
...
- **다중 분기**(`sim.branches[]`, ≤3 — 09 §13 OOM 상한): baseline/adverse/severe 동시 전진(색상 구분 P50 3선 + 반투명 밴드 중첩). 분기 갈라지는 시점(예: 금리 결정 분기)에서 부채처럼 벌어짐. 트리 노드=RunSpec, 부모 분기말 BS 상속.
  ...
  - **★occlusion 회피(VIZ — 3밴드 중첩 가독성)**: 3 분기 fan 밴드 fill 중첩은 색 진흙탕(muddy alpha)이 된다 → **(중첩 모드 기본)** hover/focus 한 분기만 밴드 **fill**, 나머지는 **외곽선(P50 선 + 밴드 경계선)만** 표시(focus+context). **(small-multiples 토글 1개)** = 분기별 미니 패널 3개 **공유 y축**(축 정렬로 폭 비교 정직). 두 모드는 §0.5 if 인셋 옆 토글 1개(새 패널 더미 0). focus 분기 = §1.2 highlightId bus 와 연동(시안 외곽).
...
- **base→scenario dumbbell**: base value ●——● scenario value 한 칸 dumbbell(이동 방향·폭 = 가정 강도). 부호는 §10 부호색 규율(증가=시안 계열·감소=중립 회색, 호재/악재 단정 금지).
...
- **헤더 status 분포 stacked bar**: 표 헤더에 전 행 status 분포 1줄 stacked bar(fact/estimate/hypothesis/missing/blocked 비율) — "이 시나리오가 얼마나 fact 기반인가" 한눈. 08 §6 체크리스트 #7(결손=결손) 시각 동기화.
...
## 10. 시각 인코딩 SSOT (정직-상태 표 — VIZ-1, 05·08·00 §6 단일 참조원)

> **이 §10 이 시각 인코딩 단일 SSOT다.** Play fan band·Assumption Ledger(§6.1)·ReportDock 12블록(08 §3.2)·DriverGraph 점등(§1.2) 의 *모든* status 인코딩이 이 표를 참조한다(컴포넌트마다 색 재발명 금지). 기존 정직 컴포넌트(HonestyFooter 3단·AuditStrip bad=적색)의 **확장**이지 새 체계가 아니다 — 1:1 정합을 아래 명시.

### 10.1 status × redundant 2채널 매핑 (색 + 선종/형태, 단일 채널 의존 금지)

각 status 는 **색 + 선종/형태**(또는 + 배지/캡션) **최소 2채널**로 표현(색맹·인쇄·저대비 환경 robust, 색 단독 금지). 색은 *의미색만*(시안=focus 1곳·적색=blocked·amber=미검증 — 무지개 금지):

| status | 색 (CSS var) | 선종/형태 | 불투명 | 배지/캡션 | 0 보간 |
|---|---|---|---|---|---|
| **fact / usable** | `--dl-ink #c8cfdb`(중립 잉크) | 실선 | 100% | 없음(기본=신뢰) | — |
| **estimate** | `--dl-ink` dim | 실선 | 85% | (옵션) ⟨est⟩ | — |
| **hypothesis (AI)** | 회색 `#9aa`(=#8b94a3 계열) | **점선** | — | 우상단 **⟨hyp⟩ 배지** | — |
| **missing** | — (렌더 안 함) | **구간 끊김 + 45° 해치 플레이스홀더** | — | **'결손' 캡션 + 'missing(0아님)' 칩** | **절대 금지** |
| **blocked** | 적색 `--dn #f0616f` 외곽 | **빗금 + 잠금 아이콘** | — | 'blocked' | — |
| **partial** | 중립 | **점선 외곽** | 60% | 'partial' | — |
| **usableWithGaps** | 약한 부분만 amber `#fb923c` | 실선 + 약부분 점선 | — | **'신뢰도 낮음' 라벨 상시** | — |
| **rejected** | dim | **취소선 / 미표시** | — | 'rejected' | — |
...
### 10.2 기존 정직 컴포넌트 1:1 정합 (재발명 아닌 확장)
...
| **전이 펄스** | 1회 200ms 펄스(반복 금지 — 주의 분산 방지) |
...
- **형태/선종 = 정직 상태**: 실선/점선/해치/취소선 = §10.1 status. 색과 redundant(2채널).
...
| status | 아이콘 | 한 줄 제목(초보 평어) | 사유 | 복구 안내 | 톤(고장 아님) |
|---|---|---|---|---|---|
| **blocked** | 🔒 | "이 종목은 아직 시뮬레이션할 수 없어요" | "상장 후 데이터가 회귀 최소 기간보다 짧습니다"(또는 BS 항등식 과거 미닫힘) | "더 긴 이력이 쌓이면 자동으로 가능해집니다" | "제품 결함이 아니라 *근거 보존* 규칙입니다 — 없는 데이터를 지어내지 않습니다" |
| **rejected** | ⛔ | "이 가정으로는 결과를 낼 수 없어요" | "필수 입력(종목/기간/통화) 또는 결손 0 대체 금지 규칙에 걸렸습니다" | "가정 X 를 채우거나 horizon 을 조정하면 풀립니다" | "차단은 *정직 가드*가 작동한 정상 동작입니다" |
| **usableWithGaps** | ⚠ | "결과는 나왔지만 일부 근거가 빠졌어요" | "일부 driver 의 ref/검증이 비어 신뢰도가 낮습니다" | "약한 부분(배지 표시)에 가정/근거를 더하면 올라갑니다" | "부분 결과를 *숨기지 않고* 약한 곳을 드러냅니다" |
| **partial** | ◐ | "절반만 시뮬레이션됐어요" | "일부 분기/노드만 계산되고 나머지는 데이터 부재입니다" | "결손 분기 데이터가 채워지면 완전해집니다" | "빈 곳을 0 으로 메우는 대신 *부분* 으로 표기합니다" |
| **missing** | ▢ | "이 구간은 데이터가 없어요" | "해당 기간/항목 공시가 존재하지 않습니다" | "공시가 올라오면 자동 반영됩니다" | "**'missing(0 아님)'** 칩 — 0 으로 위장하지 않습니다(§10.1)" |
...
### 12.2 TermGloss 데이터 계약 (용어 tooltip — 첫 등장 1회 inline 펼침)
...
```typescript
interface TermGloss { term: string; oneLineDef: string; example: string; /* 이 회사 실측 */ whyMatters: string; }
```
