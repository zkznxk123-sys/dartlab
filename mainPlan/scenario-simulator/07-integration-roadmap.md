...
★**fatal④(US 해금) = Phase A 와 병행 선행 가능**(지수와 동급, 의존 0): `simulate/{entry,registry,run,transfer}.py`+`synth/scenario.py`+`test_verb`/`test_run` — fatal①②③ 무관, 천장 상승 즉시 1건. registry/run 은 엔진트랙 Phase4 최신 위 rebase(09 §10.4 동시세션). ★**fatal①T1(lint 스켈레톤)도 Phase A 와 동급 선행 가능**(발간 표면 0파일=green no-op, src 미스캔=CI red 0) — Phase 6 의 T2(발간 표면)와 독립.
