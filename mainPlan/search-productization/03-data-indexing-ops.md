...
catalog mode 가 default 다. source artifacts 가 없으면 scheduled/default run 은 ops gate 에서 막혀야 하며, `legacy` 는 운영자가 원인 파악이나 긴급 복구 목적으로 명시 dispatch 할 때만 쓰는 예외 경로다.
