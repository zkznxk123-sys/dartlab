...
7. 제품 졸업은 실제 query-log gold 와 reviewer-approved hard-negative 300행 통과 후에만 한다. 2026-06-16 direct-review 106 row userLog/reviewed 품질팩은 historical 첫 gate 를 통과했지만, 2026-06-18 이후 베타 제거 증거는 아니다.
8. 전문 검토 결론: 본진 이식 착수 단계를 넘겨 현재 checkout 은 defaultReplacement 배선을 한 번 증명했다. 최신 상태는 `opsReady=true`, `releaseReady=false` 이며, 실제 Actions 재실행 증거와 품질 flywheel 은 운영 루프로 계속 유지한다.
...
70. 2026-06-16 local catalog-mode proof bundle 초기값은 `designReady=true`, `opsReady=false`, `releaseReady=false` 였다. HF full/lite manifest-pointer publish/round-trip/promote 뒤 quality 미포함 full_lite proof bundle 은 `opsReady=true`, `releaseReady=false` 였고, direct-review 품질팩 이후 historical `releaseReady=true` 로 supersede 됐다. 2026-06-18 hard-negative 300행 gate 추가 뒤 최신 판정은 다시 `releaseReady=false` 다.
...
77. 2026-06-16 live proof bundle 초기값은 local indexInfo `nDocs=361136` 기준으로 result contract 30 rows invalidRows 0, canary 5/5 pass 였지만 원격 source/content/round-trip blocker 가 남아 있었다. HF bootstrap 이후 full_lite proof bundle 은 remote evidence valid, full/lite round-trip valid, result contract valid, canary pass 로 `opsReady=true` 를 반환했고, direct-review quality report 이후 historical `releaseReady=true` 로 갱신됐다. 최신 기준은 hard-negative 300행 + noAnswer 60행 reviewer-approved 승격을 다시 요구한다.
78. 제품화 판단을 얇은 개선 목록이 아니라 cutover 상태 기계로 고정했다. `13-cutover-contract.md` 는 S1 designReady, S2 opsReady, S3 releaseReady, S4 defaultReplacement 를 분리하고, 본진 기본값 교체는 S2 proof bundle, 제품 졸업은 S3 real query-log gold + reviewer-approved hard-negative gold 로만 승인한다.
...
104. direct-review 품질 지표는 `overallReadyRate=1.0`, `docHit10=1.0`, `memoryCitationTop3Exact=1.0`, `newsSourcePrecision10=1.0`, `noAnswerFalseAcceptRate=0.0` 이다. CAPEX/AI 데이터센터 사업보고서 2개 후보는 sourceHint mismatch 로 reviewable set 에서 제외했다.
...
107. `data/dart/searchCatalog/searchProofBundle.directReview/searchCutover.json` 은 blockers=[], historical `releaseReady=true`, `defaultReplacement=true`, state=`S4_DEFAULT_REPLACEMENT` 다. 현재 checkout 의 본진 기본값 교체 배선은 S4 까지 한 번 닫혔다. 단 최신 제품 졸업은 hard-negative 300행 review gate 로 다시 막혀 있고, 새 workflow 가 원격 Actions 에서 다시 실행된 artifact 는 별도 운영 확인 대상이다.
...
114. current-data hard-negative gold builder 를 추가했다. `.github/scripts/search/buildSearchHardNegativeGold.py` 는 source catalog parquet 에서 same-company-different-year, same-company-sibling-filing, similar-event-other-company, report-type-mismatch, news-filing-confusion, filing-news-confusion, edgar-dart-confusion, panel-filing-confusion 을 자동 생성한다. 기본 산출물은 `goldOrigin=currentDataHardNegative`, `reviewStatus=candidate` 이며 `--mark-reviewed` 는 운영자 review 후에만 쓴다.
115. productization status 는 hard-negative 300행 부재를 release blocker 로 막는다. `evaluateSearchProductizationStatus.py` 는 quality report metrics 의 `hardNegativeRows>=300` 과 `hardNegativeExactDocHit10`, `hardNegativeWinRate`, `forbiddenTop3Rate`, `forbiddenTop10Rate`, `sourceIntentLeakRate`, `constraintViolationRate` 존재를 다시 검사한다. `releaseEligible=true` 만으로 S3 releaseReady 가 될 수 없다.
116. 실제 `.tmp/hf-contentIndex/catalog_snapshot.parquet` 475,549 rows 에서 300행 candidate hard-negative 를 생성하고 `.tmp` active index 475,611 docs 에 대해 본진 `dartlab.search` 를 평가했다. 초기 baseline 은 `overallReadyRate=0.4367`, `docHit10=0.4367`, `hardNegativeExactDocHit10=0.2400`, `hardNegativeWinRate=0.2367` 로 미달이었다. 이후 data-compiled semantic mapper, source/entity/year mask, metadata constraint lane, constraint-filtered rerank, answerability sink, 최신성 우선 phase tie-break 를 본진 search 에 넣었다. 최신 `.tmp/search-hard-negative/qualityReport.hardNegative.withNoAnswer.eventOnly.candidate.json` 결과는 hard-negative 300행 + noAnswer 60행 기준 `overallReadyRate=0.9806`, `docHit10=0.9767`, `exactDocHit10=0.9667`, `hardNegativeExactDocHit10=0.9667`, `hardNegativeWinRate=0.9667`, `noAnswerFalseAcceptRate=0.0`, `forbiddenTop3Rate=0.0`, `forbiddenTop10Rate=0.0`, `sourceIntentLeakRate=0.0`, `constraintViolationRate=0.0` 이다. metric blocker 와 noAnswer coverage blocker 는 해소됐지만 `realReviewedRows=0/300`, `proxyGoldRows=360`, `unreviewedGoldRows=360` 이 남아 release 졸업은 아직 불가다.
...
| direct-review proof bundle | historical opsReady=true, releaseReady=true, defaultReplacement=true, state=`S4_DEFAULT_REPLACEMENT`; latest hard-negative gate status is opsReady=true, releaseReady=false |
...
1. 실제 query-log gold 는 direct-review 106 rows 로 historical 첫 release gate 를 통과했다. 최신 약점은 noAnswer 포함 reviewer-approved hard-negative/query-log gold 를 300 rows 방향으로 늘리는 것이다.
...
- actual productization status CLI 초기값은 `localIndexMissingSourceDataAsOf:edgar-panel` blocker 를 포함했지만, 이후 proof bundle 재실행에서 `sourceDataAsOf.edgar-panel` 이 해소됐다. direct-review 106행 proof bundle 은 historical 기준 `opsReady=true`, `releaseReady=true`, blockers=[] 였다. 2026-06-18 hard-negative 300행 + noAnswer 60행 gate 추가 뒤 최신 판정은 `opsReady=true`, `releaseReady=false` 이며 reviewer-approved 승격 blocker 가 남아 있다.
...
- 초기 live proof bundle 재실행(2026-06-16): `buildSearchProofBundle.py --query "삼성전자 대표이사 변경" --skip-quality` 는 원격 산출물/round-trip 증거 부재로 `opsReady=false` 를 반환했다. 이 상태는 full_lite HF publish/round-trip/promote 뒤 `opsReady=true` 로, direct-review 품질팩 뒤 historical `releaseReady=true` 로 supersede 됐다.
...
- 초기 cutover audit 재실행(2026-06-16): 당시 proof bundle 은 `state=S1_DESIGN_READY`, `opsReady=false`, `releaseReady=false`, `defaultReplacement=false` 로 판정됐다. `13-cutover-contract.md` v0.8 기준 direct-review proof 는 historical `S4_DEFAULT_REPLACEMENT` 였다. 최신 제품 졸업 판정은 hard-negative 300행 review gate 를 추가해 별도 판단한다.
...
- live full/lite proof bundle(2026-06-16): `buildSearchProofBundle.py --content-tiers full,lite --skip-quality` 는 quality 미포함이라 `opsReady=true`, `releaseReady=false` 였고, direct-review quality report 포함 proof 는 historical `releaseReady=true` 로 supersede 됐다.
...
- direct-review cutover 재실행(2026-06-16): `evaluateSearchCutover.py --fail-on-default-not-ready` 는 historical `state=S4_DEFAULT_REPLACEMENT`, `defaultReplacement=true`, `releaseReady=true`, blockers=[] 를 반환했다.
...
- hard-negative+noAnswer 360행 최신 실측(2026-06-18): `.tmp/search-hard-negative/qualityReport.hardNegative.withNoAnswer.eventOnly.candidate.json` 은 `overallReadyRate=0.9806`, `exactDocHit10=0.9667`, `hardNegativeWinRate=0.9667`, `noAnswerFalseAcceptRate=0.0`, `forbiddenTop3Rate=0.0`, `forbiddenTop10Rate=0.0`, `sourceIntentLeakRate=0.0`, `constraintViolationRate=0.0` 을 기록했다. `.tmp/search-hard-negative/productizationStatus.hardNegative.withNoAnswer.eventOnly.candidate.json` 은 `designReady=true`, `opsReady=true`, `releaseReady=false` 이며 blockers 는 `qualityNotReleaseEligible`, `qualityRealReviewedRows:0/100`, `qualityProxyGoldRows:360`, `qualityUnreviewedGoldRows:360`, `qualityReportBlocker:realReviewedRows:0/300`, `qualityReportBlocker:proxyGoldRows:360`, `qualityReportBlocker:unreviewedGoldRows:360` 다.
- runtime slim metadata load(2026-06-18): `fieldIndex.loadSegment()` 기본 경로는 `evidenceText` 를 제외한 runtime 필수 column 만 읽고, 긴 evidence text 는 `includeEvidenceText=True` 로 opt-in 한다. warm benchmark `.tmp/search-hard-negative/perfReport.afterSlimMeta.warm.json` 은 coldWarmup 9273.9ms, warm p50 419.0ms, p95 2015.8ms, RSS afterQueries 1.81GB, warm query delta 43MB 를 기록했다. full meta 로드 경로의 afterQueries 약 4.36GB 대비 운영 메모리 병목을 낮췄다.
- focused validation(2026-06-18): `tests\\providers\\dart\\search tests\\search` 는 404 passed, `ruff check src\\dartlab\\providers\\dart\\search tests\\providers\\dart\\search tests\\search` 는 pass. 최신 변경은 search 범위에 한정해 검증했다.

아직 완료 아님:

- `searchIndexDelta.yml` 은 catalog default 이고 legacy 는 운영자 명시 dispatch 예외다. 다음 단계는 실제 scheduled run 에서 catalog artifacts 생성과 catalog-mode publish 를 최신 hard-negative gate 와 함께 확인하는 것이다.
...
- query-log gold 준비/평가 게이트, canonical 저장 위치, raw capture hook 은 들어갔다. direct-review 106 reviewer-labeled rows 는 historical gate 를 통과했지만, 최신 release gate 는 noAnswer 포함 reviewer-approved hard-negative 300행을 요구한다.
...
- 제품화 상태 감사 CLI 는 direct-review 106행 기준으로는 historical `opsReady=true`, `releaseReady=true` 를 만들었지만, 최신 hard-negative 300행 + noAnswer 60행 기준으로는 `opsReady=true`, `releaseReady=false` 를 반환한다. 원격 sourceCatalog/contentIndex manifest, expected source별 freshness, full/lite HF round-trip, result contract, canary source coverage 는 운영 배선 증거이고, release 졸업은 reviewer-approved hard-negative/noAnswer gold 승격 후에만 가능하다.
