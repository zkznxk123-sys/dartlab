...
3. 실제 query-log gold 와 hard-negative review gold 가 아직 부족하다. release graduation 은 generated/proxy/candidate gold 로 하지 않는다. `qualityGate.py` 와 `evaluateSearchGold.py` 로 real/proxy/review status 를 자동 판정하는 게이트는 1차 구현됐다. `DARTLAB_SEARCH_QUERY_LOG` raw capture hook 은 실제 검색 호출 후보를 남기는 입력 루프일 뿐이며, reviewer label 이 붙기 전에는 release gold 가 아니다. 2026-06-18 기준 current-data hard-negative 300행은 metric gate 를 통과했지만 `reviewStatus=candidate` 라 release 증거가 아니다.
...
현재 상태는 운영 배선과 default replacement evidence 를 한 번 닫았지만, 최신 제품 졸업은 아직 아니다. 2026-06-16 local HF bootstrap 은 EDGAR raw mirror 를 3,398 files 로 맞춘 뒤 allFilings, DART panel, EDGAR panel, newsPublic source catalog 를 만들고, 462,947-doc current catalog, catalog-mode full rebuild, 280,747-doc catalog-mode lite rebuild, HF stage-only candidate, full/lite round-trip, current promote, remote evidence audit, canary, result contract, proof bundle 을 통과했다. direct-review 품질팩은 106 real reviewed userLog rows 로 historical `releaseReady=true` 를 만들었고, replacement evidence 는 `state=S4_DEFAULT_REPLACEMENT`, `defaultReplacement=true` 를 만들었다. 그러나 2026-06-18 hard-negative 300행 gate 추가 뒤 최신 status 는 `designReady=true`, `opsReady=true`, `releaseReady=false` 이다. 실제 source/search Actions run 증거와 reviewer-approved hard-negative/noAnswer gold 승격을 새 workflow artifact 로 재확인한다.
...
| release graduation 가능 | 제품 검색이라고 말할 수 있음 | S3 `releaseReady=true`, real query-log gold 100~300 rows, reviewer-approved hard-negative 300 rows, filing/news/noAnswer/EDGAR coverage, 품질 기준 통과 |

현재 상태는 "본진 기본값 전환 가능"까지 넘겼고 direct-review 기준 defaultReplacement 배선을 한 번 증명했다. runtime 구현, catalog/local updater/result contract audit, full-source evidence, real reviewed quality report, replacement evidence 가 모두 통과했다. 다만 최신 hard-negative 300행 기준 productization status 는 `opsReady=true`, `releaseReady=false` 이므로 release graduation 가능 단계는 아직 아니다. 남은 것은 새 Search Main/Delta Actions artifact 로 같은 순서를 재현하고, noAnswer 포함 reviewer-approved hard-negative/query-log gold 를 300 rows 방향으로 늘리는 운영 루프다.
...
- productization status audit 가 proof bundle 을 판정함. direct-review 106행 상태는 historical `opsReady=true`, `releaseReady=true`, blockers=[] 였지만, 최신 hard-negative 300행 상태는 `opsReady=true`, `releaseReady=false` 다. workflow 기본 ops gate/release gate 는 blocker 를 hard fail 한다.
- cutover state audit 가 S2/S3/S4 를 분리하고, S4 는 catalog 단일 기본값과 fail-closed publish replacement evidence 가 없으면 차단함. direct-review cutover 는 historical `S4_DEFAULT_REPLACEMENT` 다. workflow release gate 는 `--fail-on-default-not-ready` 로 같은 조건을 강제하되, 품질 졸업은 hard-negative review gate 를 별도로 만족해야 한다.
- real query-log gold gate 는 106 reviewed rows 로 historical 통과했다. 최신 게이트 구현은 hard-negative 300행과 noAnswer coverage 를 blocker 로 보며, current-data candidate 360행은 metric/noAnswer gate 를 통과했지만 reviewer-approved 상태가 아니라 releaseReady 가 아니다.
