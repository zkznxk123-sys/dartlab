...
1. daily search delta 는 `auto/catalog/legacy` 모드를 갖고, catalog artifacts 가 있으면 allFilings/panel/news/EDGAR changed set 을 쓴다. 2026-06-16 local HF bootstrap 으로 source catalog/full/lite contentIndex 원격 evidence 는 확보됐지만, scheduled Actions run 으로 catalog-mode delta publish 를 확인해야 한다.
...
15. S4 기본 교체는 S2/S3 와 별도다. `evaluateSearchCutover.py` 의 replacement evidence 는 `defaultBuildMode=catalog`, `scheduledBuildMode=catalog`, `legacyFallbackOperatorOnly=true`, `failClosedPublish=true`, active/previous manifest id, rollback command, rollback 검증, run evidence 기록, surface naming review 를 요구한다. 이 증거가 없으면 releaseReady 여도 S3 에서 멈춘다.
...
현재 상태는 design-ready 를 넘겨 ops-ready 까지 한 번 닫았다. 2026-06-16 local HF bootstrap 은 EDGAR raw mirror 를 3,398 files 로 맞춘 뒤 allFilings, DART panel, EDGAR panel, newsPublic source catalog 를 만들고, 462,947-doc current catalog, catalog-mode full rebuild, 280,747-doc catalog-mode lite rebuild, HF stage-only candidate, full/lite round-trip, current promote, remote evidence audit, canary, result contract, proof bundle 을 통과했다. 최신 proof bundle 은 `opsReady=true`, `releaseReady=false`, blocker=`missingQualityReport` 다. 실제 source/search Actions run 증거와 real query-log gold 전에는 release-ready/defaultReplacement 라고 부르지 않는다.
...
현재 구현된 selfcheck 는 이 계약의 대부분을 포함한다. manifest required files/hash/load smoke, schema compatibility, local canary query, manifest 내 artifact 기반 source/no-answer canary pack, source catalog diff smoke, source catalog batch publish helper, bootstrap plan CLI, publish 전 local selfcheck, staging upload/current manifest pointer publish helper, manifest 기반 source/no-answer canary pack evaluator, result contract evaluator, result contract workflow evidence upload, query 본문 회사명 stockCode facet regression, local end-to-end pipeline drill, HF current manifest round-trip CLI, productization status evaluator 는 들어갔다. 2026-06-16 local HF bootstrap 에서 sourceCatalog/currentCatalog/full rebuild/lite rebuild/canary/resultContract/full-lite round-trip/promote 는 통과했다. 실제 운영자가 큐레이션한 최신 canary rows, random pressure, 동일 순서의 Actions 결과 확인은 아직 필요하다. real query-log quality report 전까지는 release blocker 다.
...
현재 상태는 "본진 코드 이식 가능"은 넘겼고 "본진 기본값 전환 가능" 직전이다. runtime 1차 구현과 catalog/local updater/result contract audit 배선은 들어갔지만, 실제 Actions artifact run, full-source evidence/result contract report 확인, real query-log gold 가 남아 있어 release graduation 은 금지다.
...
- `searchIndexDelta.yml` 기본 경로가 all-source catalog delta 를 사용하고 legacy fallback 이 운영상 예외로만 남음.
- publish workflow 가 current pointer 를 gate 전에 바꾸지 않고, staged candidate manifest 를 검증한 뒤 promote 함. (helper/round-trip/워크플로 배선 완료, 실제 Actions run 증거 필요)
...
- productization status audit 가 proof bundle 을 판정함. 현재 상태는 blocker 가 남으면 `opsReady=false` / `releaseReady=false` 로 자동 차단하고, workflow 기본 ops gate/release gate 에서 hard fail 한다.
- cutover state audit 가 S2/S3/S4 를 분리하고, S4 는 catalog 단일 기본값과 fail-closed publish replacement evidence 가 없으면 차단함.
- real query-log gold gate 를 통과함. 게이트 구현은 1차 완료됐고, 실제 real gold rows 확보와 실행 증거가 남아 있다.
