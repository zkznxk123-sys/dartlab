...
| Quality gate | `tests/_attempts/searchProductCore/**`, `src/dartlab/providers/dart/search/artifactCanary.py`, `src/dartlab/providers/dart/search/canaryPack.py`, `src/dartlab/providers/dart/search/facetPlanner.py`, `src/dartlab/providers/dart/search/semanticConstraints.py`, `src/dartlab/providers/dart/search/goldLog.py`, `src/dartlab/providers/dart/search/qualityGate.py`, `src/dartlab/providers/dart/search/resultContract.py`, `.github/scripts/search/evaluateSearchCanary.py`, `.github/scripts/search/prepareSearchGold.py`, `.github/scripts/search/evaluateSearchGold.py`, `.github/scripts/search/evaluateSearchResultContract.py`, `.github/scripts/search/evaluateSearchProductizationStatus.py`, `.github/scripts/search/runSearchQualityDrill.py`, `.github/scripts/search/buildSearchHardNegativeGold.py` | replacement/random pressure report, artifact source/no-answer canary pack/report, query company facet regression, data-compiled semantic mapper regression, hard-negative/noAnswer candidate gold, opt-in raw query-log candidate, canonical query-log gold JSONL, quality report, miss ledger, result contract report | quick/full/replacement/hfPipeline/localUpdater profiles. artifact positive canary 는 answerable result 를 요구한다. quality drill 은 `drillSynthetic`/`drillReviewed` 로 표식되고 release evidence 가 아니다. 본문 의미 질문은 title lane mapper 가 아니라 `searchUnified` body semantic rerank 와 data-compiled constraint plan 을 통과해야 한다. 전체 후보군 score 가 confidence floor 아래면 `answerable=false(lowConfidence)` 로 no-answer false accept 를 막는다. current-data 360행 candidate hard-negative/noAnswer metric gate 는 통과했지만, productization status 는 quality report 의 origin/review counts 와 realReviewedRows 를 다시 검사하므로 spoofed releaseEligible 로 졸업할 수 없다. reviewer-labeled real query-log gold, reviewed hard-negative/noAnswer gold, result contract report 전에는 release graduation 금지. |
...
- productization status audit: `evaluateSearchProductizationStatus.py` 1차 구현. 2026-06-16 초기 로컬 catalog-mode proof 는 `opsReady=false` 였지만, local HF bootstrap 이후 full_lite proof bundle 은 sourceCatalog/contentIndex/fileSources/full-lite round-trip/canary/resultContract 를 통과했고, direct-review 106행 품질팩 이후 historical `opsReady=true`, `releaseReady=true` 로 갱신됐다. 2026-06-18 hard-negative 300행 gate 추가 뒤 최신 판정은 `opsReady=true`, `releaseReady=false` 이며 live HF proof bundle 의 최신 blocker 는 증거 원장을 기준으로 본다.
...
| semantic constraints | `semanticConstraints.py`, `unified.py`, `answerability.py` | 동일 | content index metadata 에서 mapper 를 컴파일하고 source/entity/year/topic/report phase 제약을 ranking/answerability 에 적용. 360행 current-data candidate hard-negative/noAnswer metric gate 통과. reviewer-approved gold 승격 필요 |
...
| 2026-06-16 | direct-review quality gate | HF current repo + reviewed userLog pack | userLog reviewed | historical `releaseEligible=true`, blockers=[] | `data/dart/searchCatalog/searchQualityReviewPack.directReview/qualityCycleDirect/qualityReport.json`: 106 real reviewed rows, coverage filing 54 / news 20 / EDGAR 20 / noAnswer 12, ready/docHit/memory/news precision all 1.0, noAnswer false accept 0.0. 2026-06-18 hard-negative 300행 gate 뒤에는 단독 release proof 아님 |
| 2026-06-16 | direct-review productization status | HF current repo + full/lite round-trip + quality report | proof bundle | historical `opsReady=true`, `releaseReady=true` | `data/dart/searchCatalog/searchProofBundle.directReview/searchProductizationStatus.json`: remote evidence valid, result contract valid, canary valid, full/lite activation+rollback valid, blockers=[]. 최신 release 판정은 hard-negative gate rows 를 기준으로 재계산 |
...
| 2026-06-16 | direct-review cutover state | proof bundle + replacement evidence | cutover | historical `S4_DEFAULT_REPLACEMENT` | `data/dart/searchCatalog/searchProofBundle.directReview/searchCutover.json`: `releaseReady=true`, `defaultReplacement=true`, blockers=[]. 최신 제품 졸업은 hard-negative/noAnswer gold reviewer-approved 승격 전까지 미완 |
...
| 2026-06-18 KST | hard-negative+noAnswer 360 current-data gate | HF/current allFilings + DART panel + EDGAR panel + news | currentDataHardNegative candidate | metric pass, releaseReady=false | `.tmp/search-hard-negative/qualityReport.hardNegative.withNoAnswer.eventOnly.candidate.json`: rows 360, coverage filing 116 / news 47 / edgar 137 / noAnswer 60, overallReadyRate 0.9806, docHit10 0.9767, exactDocHit10 0.9667, hardNegativeWinRate 0.9667, noAnswerFalseAcceptRate 0.0, forbiddenTop3Rate 0.0, forbiddenTop10Rate 0.0, sourceIntentLeakRate 0.0, constraintViolationRate 0.0. `productizationStatus.hardNegative.withNoAnswer.eventOnly.candidate.json`: `designReady=true`, `opsReady=true`, `releaseReady=false`, blockers include realReviewedRows 0/300, proxyGoldRows 360, unreviewedGoldRows 360 |
| 2026-06-18 KST | runtime slim metadata perf | local full active index `.tmp` | `dartlab.search(...)` warm benchmark | RSS afterQueries 1.81GB, warm p95 2015.8ms | `.tmp/search-hard-negative/perfReport.afterSlimMeta.warm.json`: coldWarmup 9273.9ms, p50 419.0ms, p90 1922.7ms, p95 2015.8ms, max 2099.5ms, warm query delta 43MB. `fieldIndex.loadSegment()` default excludes `evidenceText`; opt-in evidenceText load remains available |
