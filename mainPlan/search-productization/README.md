...
- 그래도 release-ready 는 아니다. 실제 HF source catalog/contentIndex manifest, full-source result contract report, full/lite round-trip, canary 는 통과했지만 real query-log gold 100~300 rows 가 채워져야 제품 졸업이다.
- 현재 full_lite proof bundle 실측은 `opsReady=true`, `releaseReady=false` 다. full current 는 462,947 docs, lite current 는 280,747 docs 이고, remote evidence valid, full/lite round-trip valid, canary passRate 1.0, result contract valid 다. blocker 는 `missingQualityReport` 하나다.
...
- 남은 핵심은 실제 GitHub Actions run 으로 같은 source/search artifact publish 순서를 확인하는 것, real query-log gold 를 확보하는 것, 326.3MB lite tier 를 더 가볍게 만드는 것이다. stale source 판정과 bounded chunk evidence 는 1차 runtime 정책으로 들어갔다.
