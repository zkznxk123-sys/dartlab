...
실험과 운영 증거는 충분히 강하다. 57337 docs large gate 에서 combined product readiness 는 통과했고, seeded random curatedDraft 300 rows 는 readyRate 0.99 까지 나왔다. 2026-06-16 local HF bootstrap 이후에는 allFilings, DART panel, EDGAR panel, newsPublic source catalog 와 full/lite contentIndex current manifest 가 HF 에 올라갔고, full/lite round-trip 과 remote evidence audit 까지 통과했다. 이후 direct-review 106 row userLog/reviewed 품질팩이 release gate 를 통과했고, replacement evidence 가 `S4_DEFAULT_REPLACEMENT` 를 만들었다. 2026-06-18 최신 `Search Index Delta` release dispatch `27734759585`, `Data Prebuild (DART)` dispatch `27734759598`, CI Fast `27734740493`, CI Full `27734740472`, CodeQL `27734740475` 가 모두 성공했다. 현재 checkout 의 본진 교체 기준은 닫혔고, 운영 루프는 같은 proof bundle/replacement/cutover artifact 를 계속 남기는지 점검한다.
...
- 제품 졸업은 실제 query-log gold 100~300 rows 통과 후에만. curatedDraft, stratifiedSynthetic 은 압박 실험일 뿐이다. 현재 direct-review 106 row userLog/reviewed 품질팩은 첫 release gate 를 통과했다.
- 본진 이식 단계를 넘어 현재 checkout 은 S4 default replacement 증거까지 확보했다. 운영 루프는 Actions proof 재확인과 query-log gold 확대를 계속한다.
...
- 본진 교체 기준은 `13-cutover-contract.md` 의 상태 기계다. `designReady=true` 는 코드/설계가 본진 이식 가능하다는 뜻이고, `opsReady=true` 는 기본값 교체 가능, `releaseReady=true` 는 실제 query-log gold 기준 제품 졸업 가능이라는 뜻이다.
...
- direct-review 기준 release-ready 다. 실제 HF source catalog/contentIndex manifest, full-source result contract report, full/lite round-trip, canary, 106 row real reviewed userLog quality gate 가 모두 통과했다.
- 현재 direct-review proof bundle 실측은 `opsReady=true`, `releaseReady=true`, `defaultReplacement=true`, state=`S4_DEFAULT_REPLACEMENT` 다. full current 는 462,947 docs, lite current 는 280,747 docs 이고, remote evidence valid, full/lite round-trip valid, canary passRate 1.0, result contract valid, quality blockers=[] 다.
- 2026-06-16 HF full-source catalog-mode cycle 은 닫혔다. sourceCatalog 4종(allFilings/panel/EDGAR/news), 462,947-doc current catalog, catalog-mode full rebuild, catalog-mode lite rebuild, HF stage/round-trip/promote, canary, result contract, status audit 가 통과했다.
- 남은 핵심은 실제 GitHub Actions run 으로 같은 source/search artifact publish 순서를 확인하는 것, query-log gold 를 300 rows 방향으로 늘리는 것, 326.3MB lite tier 를 더 가볍게 만드는 것이다. stale source 판정과 bounded chunk evidence 는 1차 runtime 정책으로 들어갔다.
