...
> 정직 표기: `.xlsx`=ZIP+XML, STORE 메서드, `<mergeCell>`, CRC32는 모두 공개 표준이라 구현 위험이 낮다. 단 ZIP 로컬 헤더·중앙 디렉토리·EOCD·CRC32·크기 필드의 정확한 바이트 레이아웃은 한 번에 맞춰야 하므로 골든 픽스처 + 실제 Excel/Sheets 라운드트립 검증을 게이트로 둔다(05). LOC ~250·STORE 크기는 추정.
