...
ui/packages/surfaces/src/_shared/tour/
  ...
  tourStore.svelte.ts    // module-level $state — 활성 surface id + open (viewerEntry.svelte.ts 패턴)
...
```ts
// terminal/content/terminalTourAnchors.ts — anchor id SSOT (닫힌 집합)
...
2. `map/content/map.tour.ts` = 현 STEPS 13개를 그대로 옮기되 `selector→anchor`, `demo.run→demo.actionId` 치환. **map은 className 셀렉터(`.brand-bar`·`.color-switch`)를 이미 안정적으로 쓰므로** anchor가 `.`로 시작하면 셀렉터 폴백(§1.2) → map은 data-tour 부착 0으로 추출 가능 = 회귀면 최소.
...
| 투어 store | `ui/packages/surfaces/src/_shared/tour/tourStore.svelte.ts` |
| 타입·가드·신규기능 | `ui/packages/surfaces/src/_shared/tour/{types.ts,guard.ts,whatsNew.ts}` |
| map 콘텐츠 | `ui/packages/surfaces/src/map/content/map.tour.ts` |
| map 어댑터(축소) | `ui/packages/surfaces/src/map/components/TutorialTour.svelte` (기존, export 유지) |
| terminal 콘텐츠·앵커 | `ui/packages/surfaces/src/terminal/content/{terminal.tour.ts,terminalTourAnchors.ts}` |
