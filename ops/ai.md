...
**왜 서버 경유인가**: 실사용자는 landing/UI → `POST /api/ask`로 들어온다. 직접 `dartlab.ask()`는 `server/api/ask.py` · `services/ai_analysis.run_plain_chat` · `streaming.stream_ask` · SSE · 요청 직렬화 · 미들웨어를 전부 건너뛴다. 이 레이어에서 터지는 에러(직렬화 실패, stream 중단, tool_choice 매핑, 상태 오염)는 서버를 띄워야만 드러난다. 인라인 호출은 "내 프로세스에서는 되던데"의 근원.
