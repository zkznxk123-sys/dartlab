...
### 품질 계약 — 런타임에서 최소 형식을 검사한다

응답 품질 계약의 코드 SSOT 는 `ai/runtime/quality.py` 다. 프롬프트에 예외 문구를 계속 추가하지 않고, FINANCE 최종 답변 직전 런타임이 아래 최소 계약만 검사한다.
...
첫 위반은 1회 재작성 지시로 보정한다. 재작성도 실패하면 숨기지 않고 `quality_check` 이벤트와 audit 로그에 violation 을 남긴다.
