...
ai.configure(provider="<profile-id>", model=None, temperature=0.3)  # 글로벌 AI profile 갱신
...
dartlab.ask(question, *, stockCode?)
  ...
  │     → 질문별 subset 라우팅 (category · intent · CAPABILITIES 검색)
  │     → 최종 답변 품질 계약 검사 (runtime/quality.py)
  │     병렬 tool 허용: 한 라운드 최대 4개 (resp.tool_calls[:4]).
...
| 원천 | 접근 경로 | 특징 |
|---|---|---|
| **CAPABILITIES** (엔진 tool) | `show` · `analysis` · `scan` · `macro` · `credit` · `quant` · `gather` · `story` · `search` · `searchCompany` · `pythonExec` · `causalWeights` · `valuationImpact` · `storyTree` · `narrativeDiff` · `industry` | 엔진은 AI 근육. docstring→generateSpec.py→_generated.py 자동 등록. axis enum 자동 주입 (추측 금지). |
...
| 축 | 재정의 | 코드 |
|---|---|---|
| **CAPABILITIES 기반** | 도구 = AI 근육. `dartlab.__all__` + Company 공개 method **자동 순회 + 블랙리스트** 후 질문별 subset 만 노출. 수동 dict 금지. | `ai/tools/__init__.py::_autoDiscover` · `selectToolsForQuestion` |
...
### P3. tool 자동 등록
tool 목록은 `dartlab.__all__` + Company 공개 method 를 **자동 순회** 해 블랙리스트 필터 후 등록한다. 이후 `selectToolsForQuestion` 이 category · intent · CAPABILITIES 검색 결과로 LLM 에 실제 노출할 subset 을 고른다. 블랙리스트: setup · collect · collectAll · downloadAll · listing · Company · config · ask · canHandle · priority · status · update · view · 기타 비분석 API. 수동 dict 유지는 중복 SSoT 를 만들기 때문에 자동 경로만 유지.

### P4. tool 호출 수는 질문 복잡도에 따라 자율
AI 는 간단 질문 1회, 복잡 질문 10회까지 tool 호출을 **자율 결정**. 시스템 프롬프트에 "분석당 tool N회" 같은 숫자 하한은 두지 않는다. 런타임은 전체 tool 카탈로그를 한 번에 주입하지 않고 질문별 subset 을 제공해 선택 오류와 token 비용을 줄인다. "질문 유형별 축 조합" 표는 **참고 가이드** 로만 활용.
...
`c.analysis` / `c.credit` / `c.story` / `c.gather` / `c.quant` / `c.show` — 사용자 프로그래밍 경로. 같은 `_Impl` 을 공유하므로 AI tool 과 결과 동일. 노트북 · 스크립트 · publisher 에서 활용.
...
- **FINANCE** (기업·시장·매크로·공시·가치·신용 + 애매 주변): **tool 최소 1회 경유**. tool 0회 응답은 dartlab 정체성 훼손 (일반 답변기와 동일).
...
| 질문 유형 | 축 조합 예 |
|---|---|
| 수익성 | `analysis("수익성")` + `analysis("비용구조")` + story credit 블록 참고 |
...
- **병렬 tool 허용** — 한 라운드에 최대 4개 tool 동시 호출 가능. `toolLoop` 에서 `[:4]` 상한. 실행은 같은 라운드 내 순차 (threading 병렬 아님) → 메모리 안전. 복합 분석 (3+ 엔진 조합) 시 `pythonExec` 으로 한 번에 모아 실행도 가능. `maxRounds=10` 유지하되 실효 처리량 10→30~40 tool.
...
### Contracted Intelligence Loop — 계약으로 이해하고 trace 로 개선한다

AI 품질 운영체계는 `docstring/capabilities → Analysis Graph → Process Map → Understanding Packet → Analysis Workspace → quality gate → audit/MCP` 한 흐름이다. 새 planner 나 새 프롬프트 레이어가 아니라, 이미 있는 원천 계약을 실행 전·실행 중·실행 후에 같은 id 로 추적하는 구조다.

핵심 원칙:

- SSOT 는 계속 docstring/capabilities/AIContract 다. Graph 와 Process Map 은 generated 산출물이다.
- Process Map 은 첫 LLM 호출 전 “해야 할 말”이 아니라 `requiredTools`, `requiredEvidence`, `requiredArtifacts`, `requiredVisuals`, `freshness`, `failurePolicy`, `acceptanceCriteria` 계약만 전달한다.
- Workspace 는 실제 tool 결과와 최종 답변을 evidence/claim/visual/limit ledger 로 묶고, 계약 충족률을 계산한다.
- quality gate 는 Workspace ledger 를 우선 보고, text regex 는 보조로만 쓴다.
- audit JSONL 은 결과 로그가 아니라 trace ledger 다. tool 선택, 인자 보정, tool duration/result size, evidence/claim/visual id, rewrite, quality issue 를 남긴다.
- 반복된 수동 P/T/C/V 이슈만 docstring/capabilities 계약 승격 후보가 된다. 자동 violation 하나만으로 SSOT 를 바꾸지 않는다.

### Analysis Graph — 기계가 읽는 계약 인덱스로 간다

AI 품질 계약의 원천은 계속 docstring/capabilities/axis registry/CapabilitySpec 이다. `Analysis Graph` 는 이 원천을 generated JSON 으로 컴파일한 조회 인덱스이며, SSOT 를 새로 늘리는 레이어가 아니다.

Graph 는 수동 지식 그래프가 아니다. 사람이 관리하는 곳은 docstring/capabilities 의 계약 문장과 metadata 뿐이다. `scripts/build/generateSpec.py` 가 이를 `_generated_analysis_graph.py` 로 자동 컴파일하고, runtime/toolLoop/quality/audit/MCP 는 generated graph 를 조회한다. 질문별 예외를 graph 파일에 직접 손으로 넣지 않는다.

Analysis Graph 는 tool 선택, preflight, tool budget, evidence schema, freshness, visual policy, quality gate, audit, MCP 가 같은 `contractId` 를 보게 만든다. 예: `gather.krx.close`, `comparison.same_axis`, `disclosure.importance`, `cashflow.primary`, `macro.recent`.

공식 용어는 DartLab 내부 개념으로 고정한다. 외부 프로젝트명이나 외부 클라이언트 브랜드로 사상을 설명하지 않는다. 문서·audit·MCP 에서 쓰는 핵심 용어는 `Analysis Graph`, `Process Map`, `Understanding Packet`, `Analysis Workspace`, `Result Bundle`, `Visual Explanation` 이다.

`Process Map` 은 질문 유형별 실행 계약을 graph 에서 자동 생성한 산출물이다. 예: 최근 상승 종목은 `recent_price_mover` process 가 `gather.krx.close` evidence, primary CSV artifact, ranking visual, freshness disclosure 를 요구한다. 이 절차를 별도 planner 로 두지 않고, docstring/capabilities 계약에서 컴파일한다.

Process Map 이 담는 공식 필드는 `requiredTools`, `requiredEvidence`, `requiredArtifacts`, `requiredVisuals`, `freshness`, `failurePolicy`, `acceptanceCriteria` 다. MCP 의 `planDartlabQuestion` 과 `validateDartlabPlan`, runtime 의 Understanding Packet, audit summary 는 모두 같은 Process Map 을 조회한다.

프롬프트에 예외 문구를 계속 추가하지 않고, FINANCE 최종 답변 직전 런타임이 graph 와 Workspace 를 기준으로 아래 최소 계약만 검사한다.

- `missing_tool_evidence` — 금융 질문인데 엔진 tool 근거가 없음.
- `missing_numeric_table` — 분석형 질문인데 수치 표가 없음.
- `missing_reading_notes` — 표 뒤 `이 표에서 읽을 포인트` 가 없음.
- `missing_judgment` — 첫 문단에 자연스러운 판단문이 없음.
- `company_mismatch_risk` — 사용자/UI 종목 힌트와 Company-bound tool 의 종목코드가 다름.
- `stale_date_risk` — contract freshness 기준과 답변 기준일 고지가 충돌.
- `partial_comparison` — 비교 contract 의 targets × metrics 동일 축 evidence 부족.
- `unsupported_claim` — Claim Ledger 의 강한 판단이 evidenceIds 와 연결되지 않음.
- `missing_visual_explanation` / `unsupported_visual` — visualPolicy 위반 또는 evidenceIds 없는 visual.

첫 위반은 1회 재작성 지시로 보정한다. 재작성도 실패하면 숨기지 않고 `quality_check` 이벤트와 audit 로그에 `quality_issues`, `contract_ids`, `contract_violations` 를 남긴다.

### Trace Ledger — 실행 흔적을 평가 단위로 본다

audit 은 “답이 그럴듯했는가”만 저장하지 않는다. 한 요청의 model/tool/guardrail 흐름을 trace 로 남긴다.

필수 trace 필드:

- `selectedTools`, `skippedCandidateTools`
- `toolArgs`, `sanitizedArgs`
- `toolResultSize`, `toolDurationMs`, `round`
- `processMapIds`, `evidenceIds`, `claimIds`, `visualIds`
- `qualityIssues`, `rewriteCount`
- `processMapSatisfied`, `claimSupportRate`, `toolArgValidRate`, `freshnessSatisfied`, `visualSatisfied`

trace 는 SSOT 가 아니다. SSOT 를 바꾸기 위한 관측 자료다. 같은 `issue_code` 와 `root_cause` 가 반복될 때만 docstring/capabilities 계약 수정 후보가 된다.
...
| 11 | 랭킹 | "최근 주가가 많이 오른 종목을 찾아줘" | KRX close 기반 ranking evidence + primary CSV + visual |
| 12 | 랭킹 stream | q11 과 동일, `stream=true` | SSE `tool_result.artifacts` + `chart` + `done.responseMeta` |
...
```powershell
uv sync
$env:PYTHONPATH="src"; .\.venv\Scripts\python.exe -m uvicorn dartlab.server:app --host 127.0.0.1 --port 8400
...
Windows 에서는 audit 서버를 `uv run uvicorn ...` 으로 띄우지 않는다. uv console-script wrapper 로 서버가 뜨면 `sys.executable` 이 `.venv` Python 이 아니라 uv base Python 을 가리킬 수 있고, `pythonExec` subprocess 에서 `polars`/`httpcore` 같은 런타임 모듈 누락이 발생한다. 서버와 audit runner 는 같은 `.venv/Scripts/python.exe` 를 사용한다.

표준 runner:

```powershell
$env:PYTHONPATH="src"; .\.venv\Scripts\python.exe -X utf8 scripts\audit\serverAskAudit.py --run {run-id}
```
...
q11/q12 KRX 급등주 질문은 성공 기준이 더 엄격하다. `qualityIssues=[]`, `toolFailed=0`, `primaryCsvArtifactCount>=1`, `evidenceCount>=20`, `visualCount>=1`, `artifactViolation=false` 이어야 한다. 답변은 `asOf/기간/universe/metric` 을 명시하고, 요청일보다 최신 가용 종가가 하루 이상 낮으면 "가용 데이터 기준" 한계를 밝혀야 한다.

**반복 실패** — 직접 `dartlab.ask()` 호출로 audit 결론. 서버 레이어 결함 미탐지. `uv run uvicorn` 으로 서버를 띄워 pythonExec 모듈 경로를 흔드는 것.

---

## 12. Provider — audit 표준 profile 로 간다

provider catalog 의 SSOT 는 코드(`src/dartlab/guide/providers.py`)다. ops 문서는 외부 모델·브랜드 이름을 나열하지 않는다. AI 사상과 품질 개념은 DartLab audit 의 P/T/C/V, tool 성공률, latency 로만 설명한다.

**벤치마크 숫자로 기본 모델을 정하지 않는다.** 각 profile 로 dartlab AI audit 실행 → P/T/C/V 등급 비교.
...
9. audit 는 표준 12 질문 + 서버 경유 (`POST /api/ask`) 로 P/T/C/V 판정. 직접 `ask()` 호출 금지.
10. provider 는 audit 표준 profile 로 검증한다. 기본 모델 선택은 audit 결과로.

## Appendix. Analysis Workspace

AI 응답 품질은 프롬프트를 덧대서 올리지 않는다. docstring/capabilities 가 규칙의 원천이고, `Analysis Graph` 가 이를 런타임에서 기계가 읽는 generated index 로 제공한다. `AnalysisWorkspace` 는 한 요청 안에서 실제로 생산된 산출물을 묶는 작업공간이다.

Workspace 는 질문 해석 planner 가 아니다. 이미 호출된 tool 결과를 contract evidence schema 로 구조화하고, 최종 답변의 주장·근거·시각 설명·한계·지연 시간을 연결해 검증 가능하게 만드는 요청 단위 compiler 다.

| Ledger | 역할 |
|---|---|
| Evidence Ledger | tool 결과, CSV artifact, 기준일, 대상, metric 을 구조화한 근거 |
| Claim Ledger | 최종 답변의 판단문과 근거 연결 상태 |
| Visual Ledger | chart/diagram spec 과 근거 연결 상태 |
| Limits | 데이터 없음, truncation, 기준일 한계 같은 고지 |
| Coverage | 비교 질문에서 targets × metrics evidence matrix 와 `contractIds` |
| Freshness | metric 별 asOf, cadence, staleDaily 여부 |
| Latency | LLM round, tool duration, rewrite count, slow reason |

`/api/ask` 는 기존 `answer`/`artifacts` 를 유지하고, 선택 필드로 `evidence`, `claims`, `visuals`, `limits`, `responseMeta` 를 추가한다. 기존 소비자는 `answer` 만 읽어도 깨지지 않는다.

비교·랭킹·시계열·인과 질문은 text-only 해석으로 끝내지 않는다. tool 별 visual 이 없더라도 Workspace 가 전체 evidence 를 모아 chart 또는 diagram visualPlan 을 생성한다. daily evidence 가 낡으면 답변은 "현재" 단정이 아니라 가용 데이터 기준 판단으로 제한한다.

비교 질문은 Workspace 가 `targets × metrics` 같은 축 evidence 를 압축한 comparison brief 를 최종 LLM 호출 직전에 제공한다. LLM 은 이를 "사용자 제공 근거" 라고 부르지 않고 "tool 근거" 또는 "Workspace 근거" 라고 부른다. 강한 비교 결론은 양쪽 target 에 같은 metric evidence 가 있을 때만 낸다.

`pythonExec` 는 서버 프로세스의 wrapper Python 이 아니라 project `.venv` 의 실제 Python 을 사용해야 한다. `DartlabCodeExecutor` 는 `VIRTUAL_ENV` 의 `Scripts/python.exe` 또는 `bin/python` 을 우선 사용한다. 이는 KRX ranking, CSV artifact, visual extraction 이 `polars` 누락으로 실패하지 않게 하는 런타임 계약이다.

**반복 실패** → Workspace 를 새 지식 레이어로 쓰거나 planner 로 키우는 것. 규칙 원천은 계속 docstring/capabilities 이고, runtime 은 Analysis Graph 를 실행·검증한다.
