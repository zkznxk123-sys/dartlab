...
### Capability Contract Graph — 기계가 읽는 계약 SSOT 로 간다

AI 품질 계약의 코드 SSOT 는 `ai/runtime/contract_graph.py` 다. 원천은 계속 docstring/capabilities 이고, generator 가 `AIContract` metadata 를 `_generated.py::CAPABILITIES` 에 구조화한다. runtime 은 이 graph 를 조회할 뿐, 별도 planner 나 질문 해석 지식 레이어를 만들지 않는다.

Contract Graph 는 tool 선택, preflight, tool budget, evidence schema, freshness, visual policy, quality gate, audit 가 같은 `contractId` 를 보게 만든다. 예: `gather.krx.close`, `comparison.same_axis`, `disclosure.importance`, `cashflow.primary`, `macro.recent`.
...
AI 응답 품질은 프롬프트를 덧대서 올리지 않는다. docstring/capabilities 가 규칙의 원천이고, `Capability Contract Graph` 가 이를 런타임에서 기계가 읽는 SSOT 로 제공한다. `AnalysisWorkspace` 는 한 요청 안에서 실제로 생산된 산출물을 묶는 작업공간이다.
...
**반복 실패** → Workspace 를 새 지식 레이어로 쓰거나 planner 로 키우는 것. 규칙 원천은 계속 docstring/capabilities 이고, runtime 은 Contract Graph 를 실행·검증한다.
