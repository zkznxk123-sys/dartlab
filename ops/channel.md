...
**현재**: Microsoft DevTunnels 자동화 파이프라인 (Phase A~G) · GitHub OAuth 1 회 · anonymous access entry · 영구 URL 재사용.
**방향**: `--auth-github` access control · VSCode 확장 통합 · HF Space 이관 옵션.

PC dartlab 을 폰·외부 어디서나 그대로 쓰게 하는 공유 엔진. 각 섹션은 **"이렇게 한다"** 명제로 열고, 반복된 실수는 섹션 하단 **"반복 실패"** 에 정리한다.

| 항목 | 내용 |
|---|---|
| 레이어 | L4 (표현·접근) |
| 진입점 | `dartlab channel` |
| 기술 백엔드 | Microsoft DevTunnels (VS Code Remote Tunnels 와 동일 인프라) |
| 폰 호환 | Android Chrome · iOS Safari 검증 (2026-04-07) |
| 영구 URL | `https://<id>-<port>.<region>.devtunnels.ms` |
| 요구 조건 | GitHub 계정 (1 회 OAuth) |
...
**흐름**:
1. winget 으로 devtunnel CLI 자동 설치 (1~2 분, 최초 1 회).
2. GitHub OAuth (브라우저 자동 오픈, 최초 1 회).
3. tunnel 자동 생성 + anonymous access + host 시작.
4. URL + QR 출력.
5. 폰 Chrome 에 URL 입력 (또는 QR 스캔).
...
| 컴포넌트 | 선택 이유 |
|---|---|
| **Microsoft DevTunnels** | VS Code Remote Tunnels 의 기반 인프라. MS 공식, 모바일 호환성 검증됨. winget 자동 설치 가능 |
| **anonymous access entry** | 토큰 시스템 없이 URL 만으로 접근 가능. dartlab CLI 자체로 인증 모델 단순화 |
...
| 백엔드 | 폐기 이유 |
|---|---|
| Cloudflare Quick Tunnel (`*.trycloudflare.com`) | 모바일에서 fetch 응답 hang 버그. 데스크탑은 OK 인데 폰 LTE 에서 응답 본문 미수신. [cloudflared#1385](https://github.com/cloudflare/cloudflared/issues/1385) |
...
**반복 실패** — `--allow-anonymous` 만 설정하고 Phase F 의 access entry 등록을 빼면 폰에서 접근 실패. 두 단계 모두 필요.

---

## 4. 보안 모델 — anonymous reader 기본, URL 노출에 주의한다

dartlab channel 은 **anonymous reader 기본**. URL 을 알면 누구나 접근 가능.
...
**반복 실패** — URL 이 새면 누구나 접근 가능 → 회수는 `devtunnel tunnel delete <id>` 후 재실행 (새 URL). dartlab 미들웨어 (토큰·화이트리스트·레이트리밋) 는 비활성 상태로, 운영용 공개 SaaS 에는 부적합. 개인 PC 를 자기 폰에서 쓰는 게 주 시나리오.
...
사람이 손대는 건 **최초 1 회 GitHub OAuth 클릭**. 이후는 자동.
...
`src/dartlab/channel/adapters/` 의 어댑터 백그라운드 실행. 메시징 앱에서 dartlab AI 에 직접 질문 가능.
...
| 폰에서 anti-phishing 페이지 | 첫 접속 confirmation | "Continue" 1 번 클릭 (이후 쿠키) |
| 폰에서 화면 안 뜸 | dartlab UI 모바일 깨짐 | dartlab 0.x 빌드 확인. legacy plugin 적용된 빌드인지 |

---

## 8. 모바일 호환 진단 — 진단 페이지·`document.title`·블로그 비교 3 단계로 잡는다
...
1. **폰 콘솔 접근이 어렵다 → 진단 페이지를 별도 제공**. 단순 HTML + `window.addEventListener('error', ...)` 핸들러로 화면에 에러 메시지 직접 표시.
...
추가로 `lucide-svelte` 등 외부 패키지의 deprecated export 의심 — 메이저 업데이트 후 이전에 잘 됐던 게 깨지면 import 일괄 grep.

### 기본 설정

안정적 모바일 호환을 위해 다음 기본 조합을 유지:
...
**반복 실패** — 데스크탑에서만 테스트하고 배포 → 모바일에서 외부 패키지 deprecated export 로 hydration 전체 실패. 빌드 전 모바일 Chrome 직접 확인.

---

## 9. 설계 원칙

- **진입점 1 개** — `dartlab channel`.
- **백엔드 1 개** — Microsoft DevTunnels.
- **자동화 파이프라인** — 사람 손은 GitHub OAuth 1 번.
...
| `ui/web/package.json` | `svelte ^5.55.1`, `@sveltejs/vite-plugin-svelte ^6.2.4` |
...
- GitHub OAuth 1 회 인증 후 tunnel ID 재사용 (재실행 시 같은 URL).
...
1. `dartlab channel` 한 줄로 PC dartlab 을 폰·외부에 공유, Microsoft DevTunnels 단일 백엔드.
2. Phase A~G 자동화 — binary 탐색·설치·GitHub OAuth·tunnel 생성·port 매핑·anonymous access·host 시작.
3. 보안 모델은 anonymous reader 기본, URL 노출에 주의하고 회수는 `devtunnel tunnel delete`.
4. 최초 1 회 GitHub OAuth 후 영구 URL 재사용, 2 회차부터 자동.
5. 메시징 봇 (telegram · slack · discord) 은 `--telegram/--slack/--discord` 토큰 플래그로 옵션 기동.
6. 모바일 호환 진단 3 단계 — 진단 페이지 · `document.title` 단계 표기 · 블로그 비교.
7. 기본 설정 — Svelte 5.55 + vite-plugin-svelte 6 + `@vitejs/plugin-legacy` es2020 + `_core` 싱글톤 + `matchMedia` listener.
