...
## 1. 자동 파이프라인 — 5 워크플로우로 간다

5 개 워크플로우 · 단일 책임 · concurrency group 직렬화.
...
## 5. Flow 4 — 일일 감사 (dataAudit.yml) 는 4 워크플로우 status 를 본다
...
- **흐름**: `monitorPipeline.py` 실행 → 4 개 워크플로우 상태 확인 → 실패 시 `pipeline-failure` 라벨 Issue 생성/갱신.
...
| `.github/workflows/` | 워크플로우 5 개 (dataSync · dataPrebuild · edgarSync · dataAudit · kindlist) |

---

## 요약 — 명제 10 줄

1. 5 워크플로우 — kindlist(법인) / dataSync(DART recent 12h) / dataPrebuild(scan) / edgarSync(벌크 daily+weekly) / dataAudit(5 am).
