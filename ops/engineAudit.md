...
**방향**: 샘플 종목 자동 순회 · 결과 snapshot diff · engine audit → story audit 연계.

AI audit (AI 가 엔진을 제대로 쓰는지) 과 달리, engine audit 은 **엔진 자체가 제대로 작동하는지** 점검한다. 단위 테스트는 로직을 보고, engine audit 은 **실제 데이터로 end-to-end 결과** 를 본다. 각 섹션은 **"이렇게 한다"** 명제로 열고, 반복된 실수는 섹션 하단 **"반복 실패"** 에 정리한다.
...
- [ ] `c.show("inventory")` — 주석 상세 (notes 12 항목 중 대표)
...
- [ ] `dartlab.scan("governance")` — 지배구조 5 축
...
- [ ] `c.gather("news")` — 뉴스
- [ ] EDGAR 에서 `c.gather("flow")` 는 `None` 반환 확인
...
- [ ] `c.story("수익성")` — 단일 섹션 ~5 초
- [ ] Story 객체 반환, `.toMarkdown()` · `.toHtml()` · `.toJson()`
...
## 4. 등급 판정 — Pass · Warning · Fail 3 단계

| 등급 | 기준 |
|---|---|
| **Pass** | 모든 체크 통과. 예외 없음. |
| **Warning** | 1~3 개 실패. 재실행 시 재현되는지 확인. |
| **Fail** | 4 개 이상 실패 또는 Company 생성 자체 실패. |

---

## 5. 자동화 출력 — 이 JSON 형식으로 기록한다
...
## 7. 발견 시 — 즉시 중단하고 원인 분석한다

- **Fail 발견 시 즉시 중단** + 원인 분석.
- 엔진 버그 → 해당 파일 근본 수정 (`feedback_code_quality.md`).
- 데이터 이슈 → `_reference/LEARNING_WORKFLOW.md` 6 단계.
- Warning 은 별도 등록. Fail 잡은 후 해결.

**반복 실패** — engineAudit Fail 을 무시하고 aiAudit 실행 → 엔진이 깨진 상태라 AI 검증 무의미. 반드시 engineAudit Pass 확인 후 aiAudit.
...
engineAudit 이 엔진 품질을, aiAudit 이 AI 활용 품질을 본다. 둘 다 필수.
...
1. engine audit 은 엔진 자체 end-to-end 동작 검증, AI audit 의 선행 게이트.
2. Company · show · select · analysis · credit · scan · macro · gather · quant · story · search · SuperMaster 12 영역 체크리스트로 점검.
3. 종목 커버리지 5 종 (삼성전자 · 대우건설 · 삼양식품 · Apple · Microsoft) KR+US 믹스.
4. 판정 3 단계 — Pass (전체) · Warning (1~3 실패) · Fail (4+ 또는 Company 생성 실패).
5. 결과는 JSON + `report.md` 로 `data/audit/engine/{date}/{stockCode}.json`.
6. Fail 즉시 중단, 원인 분석 → 엔진 버그는 근본 수정, 데이터 이슈는 LEARNING_WORKFLOW 6 단계.
