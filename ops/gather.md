...
| 경로 | 트리거 | 키 | 소스 | 출력 |
|---|---|---|---|---|
| **A. 운영자 cron (벌크)** | `.github/workflows/buildKrxData.yml` cron (KST 20:00 평일) | 환경변수 `KRX_API_KEY` (GitHub secret, 운영자 빌드 스크립트만 read) | KRX OpenAPI 직접 | HF dataset `eddmpython/dartlab-data` 의 `krx/prices/` 빌드 (long parquet) |
...
- KRX OpenAPI `sto/*_bydd_trd` 는 당일(T-0) 장마감 이후에도 빈 응답이 나오는 사례가 있으므로 자동 cron 은 직전 거래 가능 평일(T-1)까지만 기대한다.
- 운영자 cron: KST 20:00 평일 — **T-1 포함**. 당일 긴급 반영은 `workflow_dispatch mode=backfill` 로 명시 실행한다.
- `buildIncremental` 은 1일 fixed 가 아니라 **"마지막 저장일 + 1 ~ T-1" 가변 윈도** fetch — cron 누락 / 캐시 miss / 정산 지연 등으로 갭이 생겨도 다음 cron 이 자동 메움.
- 마지막 저장일 조회: 로컬 parquet 우선 → HF 현재 연도 fallback (캐시 miss 대비).
- 사용자 직접 `apiKey` 호출도 오늘(T-0) 일자는 확정 전으로 보고 빈 DataFrame 을 반환한다. 확정 일자는 T+1 이후 조회한다.
