...
2. **노드·엣지 = JSON** (~3,000 건). industry 엔진 안에서 자급자족.
3. **파이프라인 = 코드** (4 단계 빌드). 어떻게 추출/분류하는가만 고정.
4. **AI 개입** = AI 가 도구로 사용 + 오분류 보정 + 신규 산업 초안 생성.

**반복 실패** — 분류 rules 를 코드에 하드코딩 → taxonomy 진화 차단. JSON 데이터로만 유지.
...
- AI 가 `addOverride()` 로 즉시 수정.
- 사람이 JSON 직접 편집.
- 다음 빌드 시 4 단계에서 자동 반영.
...
### 1 단계: KindList 업종 → 대분류 (`stage1_ksic.py`)

KindList 의 업종(KSIC) 컬럼을 taxonomy 의 ksicCodes 와 매칭.
"반도체 제조업" → semiconductor. confidence=0.7.

### 2 단계: 주요제품 → 중분류 (`stage2_product.py`)

KindList 의 주요제품 텍스트를 쉼표 토큰화 → taxonomy keywords 매칭.
"DRAM,NAND" → semiconductor.fab. confidence 0.6~0.9.

### 3 단계: docs → 소분류 (`stage3_docs.py`)

docs parquet 의 businessOverview/productService/rawMaterial 텍스트 스캔.
taxonomy keywords 매칭으로 stage 보강/신뢰도 갱신.
Company 객체 미로드 — Polars lazy frame 직접 스캔 (메모리 안전).

### 4 단계: override 적용 (`stage4_review.py`)

`overrides.json` 의 확정 매핑을 반영. confidence=1.0, source="manual".
...
**반복 실패** — stage3 에서 Company 객체로 docs 로드 → 전종목 메모리 폭발. Polars lazy frame 직접 스캔.

---

## 5. AI 개입 — 도구로 쓰고, 보정하고, 제안한다

### 5-1. AI 가 산업지도를 사용

```python
dartlab.ask("삼성전자는 반도체 밸류체인 어디에 있어?")
# → AI 가 c.industry() 호출 → nodes.json 조회 → 분석
```
...
- industry(L2) 에서 AI(L4) 를 import 하지 않음 (레이어 규칙).
- AI(L4) 가 코드 실행 루프에서 industry 함수를 호출.

### 5-3. AI 가 새 산업을 제안

1. AI 가 KindList 필터 + docs 스캔으로 초안 생성.
2. `taxonomy.json` 에는 **사람만** 추가 (AI 직접 수정 금지).
3. `overrides.json` 은 AI 가 직접 수정 가능.

### 5-4. AI context 주입 (Phase 2)

ContextBuilder 에 industry selector 추가:
- ACT1(사업이해) intent 에서 chainPosition 자동 주입.
- 같은 공정 peer 목록 포함.

**반복 실패** — AI 가 `taxonomy.json` 을 직접 수정. taxonomy 는 사람 승인 라인, override 는 AI 자유.

---

## 6. 지속 학습 — 선순환 루프로 정밀도를 올린다
...
| 회사 분할/합병 | AI 가 override 이전 → 사람 확인 |
...
시간이 지날수록 override 가 쌓이고, taxonomy 키워드가 보강되어 자동 분류 정밀도가 올라간다.

### 블로그 연동

`blog/07-industry-map/` 카테고리에 산업지도 포스트를 발간한다. frontmatter 에 구조화된 산업 데이터를 포함하여 코드가 읽을 수 있다.
...
## 10. 산업지도 빌드 — `buildIndustryMap.py` 가 landing 정적 JSON 으로 변환한다

industry 엔진의 노드·엣지·분류 데이터를 **landing/static/map/** 의 정적 JSON 으로 변환하는 빌드 스크립트. GitHub Pages 배포 시 포함.
...
## 11. 자동화 파이프라인 — cron 체인으로 매일 재생성한다
...
- **TTL = 12 시간**: 12 시간마다 HF 와 ETag 비교 → 변경 시 자동 다운로드.
...
| `scripts/build/buildIndustryMap.py` | 산업지도 JSON 전체 빌드 |
...
1. taxonomy.json = 분류체계 데이터 SSOT. 코드 하드코딩 금지, JSON 직접 편집으로 진화.
2. 4 JSON (taxonomy / nodes / edges / overrides) + 4 단계 빌드 (KSIC → 주요제품 → docs → override) 로 지도 생성.
3. 호출은 `dartlab.industry(...)` + `c.industry()` 양쪽, Company-bound 는 `calcChainPosition` 반환.
