...
Company-bound `c.macro()` 는 내부적으로 `dartlab.macro()` 를 호출한다.
Company 가 필요한 이유: review 연동 시 company.market 참조.
**macro 자체는 종목코드 불필요** — `dartlab.macro()` 로 독립 호출 가능.
...
dartlab.macro("시나리오", "2008 금융위기")  # 역사적 충격 재현 (~146개 프리셋)
dartlab.macro("시나리오", "신용 충격", severity="severe")
