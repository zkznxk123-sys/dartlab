...
**현재**: 6 매퍼 단일 인터페이스 · JSON SSOT (코드 매핑 데이터 0 줄) · AI 소비·학습 전제 설계.
**방향**: 매핑률 ≥95% 유지 자동 검증 · 매퍼 리포트 블록화 · 학습 워크플로우 자동 측정.

dartlab 전체 매핑 데이터의 통합 엔진. 6 매퍼를 단일 인터페이스로 관리한다. 각 섹션은 **"이렇게 한다"** 명제로 열고, 반복된 실수는 섹션 하단 **"반복 실패"** 에 정리한다.
...
| 학습 | Scanner 2,877 종목 학습 완료 → 분기별 갱신 |
...
1. **코드에 매핑 데이터 0 줄** — 모든 매핑은 JSON 파일에. 코드는 JSON 로드만.
2. **단일 정의** — 같은 함수·데이터의 중복 정의 없음. `common.py` 가 공통 유틸.
3. **파서 로직은 매퍼 패키지에 두지 않는다** — 매퍼는 데이터만, 파서는 `providers/`.
4. **private 함수 외부 import 없음** — 공개 API 만 노출 (`loadAffiliate` · `loadCostByNature` 등).
5. **매퍼 추가 시 테스트** — `test_mappers.py` 에 `lookup` · `stats` · `allKeys` 3 개 이상.
6. **JSON 수정 시 기존 키 삭제 없음** — 신규 추가만. 삭제는 scanner 검증 후.
7. **AI 학습 전제** — 모든 매핑 데이터는 구조화되어 AI 가 읽을 수 있어야 한다.

**반복 실패** — 코드 안에 dict 하드코딩으로 매핑 추가 → JSON SSOT 붕괴. 파서 로직을 매퍼에 섞어 넣음 → 레이어 구조 훼손.
...
| notes | `notesStructure.json` | 2,640 | notes 항목 유형·외화·카테고리 (2,877 종목 학습) |
...
## 6. AI 학습 설계 — 매퍼 데이터가 AI 의 학습 재료다

### 데이터 구조

모든 매핑 JSON 은 AI 가 읽고 학습할 수 있는 구조:
...
| 경로 | 역할 |
|---|---|
| `src/dartlab/core/mappers/` | 매퍼 엔진 (11 파일) |
...
| `tests/test_mappers.py` | 45 unit 테스트 |
...
1. 매퍼 데이터는 `core/data/` JSON 한 곳에 있고 코드에 매핑 0 줄 유지.
2. 6 매퍼 (account · topic · alias · flow · notes · parser) 를 `getEngine()` 싱글턴으로 통합.
3. 공통 유틸은 `common.py` 단일 정의, 파서 로직은 매퍼 밖 `providers/`.
4. 매퍼 JSON 은 `_metadata.description` + 한국어→영문 canonical 구조로 AI 학습 친화.
5. Scanner 2,877 종목 학습 → `notesStructure.json` 분기별 갱신 → AI 자동 분류 → Curator 수동 검증.
6. 매퍼 추가는 JSON → 매퍼 클래스 → `__init__` 등록 → 인라인 삭제 → 테스트 → ops 갱신 6 단계.
7. JSON 수정은 신규 키 추가만, 삭제는 Scanner 검증 후.
