...
**방향**: AI tool 자동 등록과 MCP 도구 목록의 공통 원천 (docstring/Analysis Graph) 공유 폭 확장 · resource URI 체계화 · DartLab 주도 설치 계약.
...
MCP 경로에서 별도 지식 그래프나 수동 planner 를 만들지 않는다. 외부 실행 환경이 dartlab 을 이해해야 할 때도 원천은 Python API docstring/capabilities 이며, `generateSpec.py` 산출물만 공식 표면이다.
...
- `pythonExec` (AI tool 엔진 전용 escape hatch — MCP 경로에 임의 코드 실행 노출은 보안 리스크).
...
4. 엔진 도구 + API discovery + Analysis Graph/Process Map 도구, SSOT 는 Python API · docstring · capabilities → `generateSpec.py` 자동 생성.
