...
**현재**: `python -m dartlab.mcp` stdio 서버 + SSE ASGI 앱 + `.mcp.json` 자동 설치 헬퍼 + 26 도구 자동 생성 (`_generated_tools.py`) + 4 리소스 + 3 API discovery 도구. VSCode 확장이 활성화 시 `.mcp.json` 에 자동 등록.
**방향**: AI tool 자동 등록과 MCP 도구 목록의 공통 원천(docstring) 공유 폭 확장 · resource URI 체계화 · 외부 클라이언트별 (Claude Desktop, Codex, Cursor) 설치 가이드.
...
| 도구 | 26 개 (자동 생성) + 3 API discovery |
| 리소스 | 4 개 (`dartlab://info` · `dartlab://scan/{axis}` · `dartlab://macro/{axis}` · `dartlab://company/{code}/topics`) |
...
**수동 설정** (`.mcp.json` 또는 Claude Desktop 설정):
...
create_server()
    ├─ list_tools  → _TOOLS (_generated_tools.py + _DISCOVERY_TOOLS)
...
## 도구 체계 — SSOT 는 dartlab Python API

MCP 도구 목록은 **수동 나열 금지**. `scripts/build/generateSpec.py` 가 dartlab 공개 API 의 시그니처 + 독스트링을 읽어 `src/dartlab/mcp/_generated_tools.py` 를 재생성한다. 수정이 필요하면 원본 함수의 시그니처/독스트링을 고치고 generateSpec 을 재실행한다.

### 도구 카테고리

| 카테고리 | 도구 | 설명 |
|---|---|---|
| 빠른 판단 | `companyInsights` | 7영역 등급 + 투자 프로파일. 분석 시작 시 먼저 쓰는 도구 |
| 종목 조회 | `searchCompany` | 종목코드 (005930) · 회사명 (삼성전자) · 부분검색 |
| 원본 데이터 | `companyFinancials` / `companyRatios` / `companyShow` / `companyTopics` / `companyProfile` / `companySections` / `companyFilings` / `companyDiff` | IS/BS/CF 원본, 비율, 토픽 |
...
| 보고서 | `companyReview` | 정리된 보고서 (사용자 명시 요청 시) |
...
캐시에 로드된 Company 는 런타임에 `company/{code}/topics` 리소스로 자동 노출된다.

---

## AI tool 과의 관계 — 독립 registry, 공유 원천
...
| 결과 포맷 | `ToolResponse` (llmText / uiData / dataAsOf) | MCP `TextContent` (문자열만, 12000 자 상한) |
...
**공통**: Python 함수 시그니처와 독스트링이 단일 진실의 원천. 한쪽만 갱신하지 말고 `generateSpec.py` 로 양쪽 재생성.
...
외부 LLM 컨텍스트 절약을 위해 도구 결과 문자열은 `_MCP_MAX_RESULT_CHARS = 12000` 로 잘라낸다 (초과 시 "결과 잘림, 전체 N자. 범위를 좁혀 재조회하세요" 안내 포함). AI tool 의 `ToolResponse` 와 다른 이유: MCP 클라이언트는 스트리밍 지원이 일관되지 않고 단일 응답이 너무 크면 에이전트 루프가 깨진다.
...
| `src/dartlab/mcp/_generated_tools.py` | `_TOOLS` · `TOOL_FEATURE_MAP` (generateSpec.py 로 자동 생성, 수동 수정 금지) |
