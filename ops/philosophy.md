...
**현재**: 사상이 `ops/ai.md` · `ops/company.md` · `ops/analysis.md` · `ops/skills.md` 에 분산되어 있던 것을 본 문서로 통합.
**방향**: 본 문서를 정점으로 `ops/coreloop.md` · `ops/ai.md` · `ops/skills.md` · `ops/story.md` 등 하위 문서는 구현 상세만 담는다.
...
12. **인터리빙 서사 강제** — `src/dartlab/ai/runtime/toolLoop.py` 와 `providers/oauth_codex.py` 는 `tool_calls[:1]` 로 parallel 호출을 잘라 한 라운드당 tool 1 개만 실행. 사용자가 분석 과정을 따라갈 수 있게.
...
- [`ops/ai.md`](ai.md) — AI 엔진 구현 (4 축 사상 · P1~P8 · override 매커니즘).
- [`ops/skills.md`](skills.md) — docstring = skill 규약 · 9 섹션 상세.
- [`ops/story.md`](story.md) — story 만능 진입점 · 보고서 빌더.
...
| tool 자동 등록 | ai.md | 다른 곳은 한 줄 언급만 |
| 3 정보층 경계 | philosophy.md §7 | 다른 곳은 요약 + 링크 |

**반복 실패** — 사상 섹션 ↔ ops/ai.md ↔ ops/skills.md 에 같은 내용 중복 기술. 수정 시 여러 곳 동시 갱신 강요. 본 문서를 정점으로 통합.
