...
**현재**: 사상이 `ASK_WORKBENCH_KERNEL.md` · `ops/company.md` · `ops/analysis.md` · `ops/skills.md` 에 분산되어 있던 것을 본 문서로 통합.
**방향**: 본 문서를 정점으로 `ops/coreloop.md` · `ASK_WORKBENCH_KERNEL.md` · `ops/skills.md` · `ops/story.md` 등 하위 문서는 구현 상세만 담는다.
...
| AI 설계 | `ASK_WORKBENCH_KERNEL.md` | 다른 곳은 한 줄 언급만 |
| 3 정보층 경계 | philosophy.md §7 | 다른 곳은 요약 + 링크 |

**반복 실패** — 사상 섹션 ↔ ASK_WORKBENCH_KERNEL.md ↔ ops/skills.md 에 같은 내용 중복 기술. 수정 시 여러 곳 동시 갱신 강요. 본 문서를 정점으로 통합.
