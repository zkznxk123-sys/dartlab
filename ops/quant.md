...
- `sector` 는 `industry/nodes.json` primary 산업 → `quant/benchmarkMap.py` 지수 매핑 → KRX index 존재성 검증 순서로 섹터 지수를 고른다. 없으면 시장 지수로 fallback하고 `fallbackReason` 을 남긴다.
- `style` 은 최신 KRX 시총 분위로 코스피/코스닥 대형·중형·소형 지수를 고른다. value/growth 스타일은 v1 범위가 아니다.
- `benchmark="코스피 200"` 처럼 명시한 값은 항상 최우선이다.
