...
> 2. **review** 가 재료를 블록 단위로 조합하여 **보고서**를 만든다 — 11 가지 보고서 타입 × 7 가지 기업유형 템플릿.
> 3. **AI/사람** 이 보고서를 읽고 **해석하고 판단**한다.
>
> review 는 해석을 제공하지 않는다. **다양한 관점의 근거를 체계적으로 배치**한다. 해석은 AI 또는 사람의 몫이다. 어떤 관점이든 대응할 수 있는 보고서 구조 — 신용, 성장, 위기, 배당, 지배구조, 매크로, 가치평가.
>
> **AI 는 `review()` 를 직접 호출하지 않는다** (83 초 타임아웃). 대신 review 의 보고서 타입을 참고하여 분석 관점을 잡되, 엔진을 직접 호출하여 자기 주관으로 판단한다.

| 항목 | 내용 |
|------|------|
| 레이어 | **L3** (1.0.0 리팩토링 이후) — L2 4 엔진 + L1.5(scan) 소비 |
| 사상 | **이야기꾼** — 엔진이 제공한 숫자를 서사로 조립. 해석 문장 생성 포함 |
| 진입점 | `c.review()`, `c.review(type=...)`, `c.reviewer()` |
...
**반복 실패** — review 가 해석 문장을 주장 ("삼성전자는 강력하다") 으로 전개. review 는 근거 배치, 해석은 AI/사람. narrate 함수는 조건분기 기술로만.

### 방향성 메모 — docstring SSOT 와의 교류 (2026-04-24)

SSOT = [`ops/skills.md`](skills.md) — **skill 은 별도 파일 없이 엔진 docstring 이 담는다**.

review 는 **사람 분석 루트 (L1 → L2 → review → 사람)** 의 종착. AI 는 review 를 우회하고 공개 함수 docstring 을 경험축으로 쓴다 — 루트가 다르다. 교류:

- **docstring → review**: 엔진 docstring Guide 섹션이 audit 로 충분히 검증되면 review 블록 템플릿에 반영 (같은 해석 규칙 · 같은 임계값).
- **review → docstring**: 기존 review 블록 중 재현 가능 · 해석 규칙 명확한 것은 공개 함수로 추출해 엔진 docstring 에 Guide 로 명시. AI · review 공용 호출.

현재 review 블록은 유지, 점진 정합. 상세 사상은 `ops/skills.md`.
...
`c.review(type="full", template="사이클")` 동시 사용 가능 — 6 막 순서 + 사이클 강조.
...
| 6 막-3 시장분석 | 시장분석 | **quant** | calcTrendData, calcRiskData, calcSignalData, calcStrategyData, calcCrosscheckData, calcQuantConclusionData (엔진=숫자만, review 가 narrate 호출) |
...
기존 perspective/preset 을 **reportType 단일축**으로 통합. `c.review(type="credit")`.
...
**4 채널 렌더링**: 같은 `c.review(type='dashboard')` 결과를 4 채널로 출력.
...
review 는 Company-bound — DART/EDGAR 자동 분기.
...
review 5-7 신용평가 섹션이 credit 의 모든 정보를 흡수한다:
...
**credit 자체 publisher 는 deprecated.** review.publisher 가 단일 진입점. 기존 16 개 credit 보고서는 `blog/04-credit-reports/` 에 아카이브로 보존.
...
## 13. Review Audit — review 보면서 근본 1 곳을 고친다

> review 를 실제로 돌려보면서 사람(또는 AI 대리)이 읽고 → 데이터 이상 / 엔진 부족 / 템플릿 부족 발견 → 코드 수정 → 다시 돌려보기. 이게 유일한 진짜 audit. 개별 엔진 단위 audit 는 파편화 — review audit 1 개로 통합.
...
```
1단계: review 실행
   c.review(type="full", template="auto")
...
review audit 에서 발견한 오류는 **3 가지 학습 매커니즘** 중 하나로 이어질 수 있다:
...
| 주기 | 대상 | 목적 |
|---|---|---|
| 신규 엔진/축 추가 시 | 1 종목 | 새 블록 review 정상 등장 |
...
- `grep -n 'HeadingBlock("' src/dartlab/review/builders.py` → 0 건이어야 함.
...
## 15. 개선 파이프라인 — review 변경 시 반드시 거친다
...
| 파일 | 역할 |
|------|------|
| `src/dartlab/review/catalog.py` | 블록/섹션 메타, 순서 (단일 진실의 원천, 164 블록/25 섹션) |
| `src/dartlab/review/templates.py` | 섹션 설정 + 7 개 스토리 템플릿 + 6 개 생애주기 + (deprecated) PERSPECTIVE_TEMPLATES |
...
| `src/dartlab/review/formats.py` | 6 막 헤더 + 막 결론 + 마크다운/HTML/JSON |
| `src/dartlab/review/narrative.py` | 7 개 인과 패턴 감지 + 6 막 전환 문장 |
...
1. review 는 보고서 빌더. 5 엔진 calc 를 재료로 블록 조립. 해석은 AI/사람.
...
9. review audit 는 review 보면서 근본 1 곳 수정. 학습은 accountMappings/topicGraph/labels 경로.
10. AI 는 review 직접 호출 안 함 (83 초), 엔진을 직접 호출. review 는 사람용 + 블로그 파이프.
