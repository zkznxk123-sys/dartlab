...
docstring 은 `inspect.getdoc` 경로로 `ai/tools/__init__.py::_toolDescription` 에 흘러 AI tool schema 의 description 으로 직접 노출된다. 즉 `ops/scan.md` 는 참조 인덱스이고, 레시피 본문·새 관점·새 질문 유형 추가는 **docstring 에 박는다** (CLAUDE.md "⛔ SSOT" · "⛔ Simple > Complex").
...
## 11. scan → review 모듈 매핑

scan 은 review 6-4 "비교분석" 섹션에 독립 calc 모듈로 **교차 조합 관점** 을 제공. 전종목 횡단 데이터를 2~3 축 교차하면 단일 종목에서 안 보이는 뷰가 나온다:
...
4. 7 관점 스크리닝 (Value · Growth · Quality · Momentum · Income · Recovery · Defensive) 으로 primitive 조합.
...
10. scanner 는 Company import 없음 (역의존 방지), scan → review 는 `scan/extended.py` wrapper 경유.
