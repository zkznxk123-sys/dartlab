...
`src/dartlab/skills` 가 skill/procedure 문서의 루트 SSOT 이다. `docs/skills/**` 같은 Markdown 문서 복제본은 만들지 않는다.
`landing/static/skills/*.json` 은 Web/UI 검색과 Pyodide compatibility 를 위한 generated index 일 뿐이며 직접 수정하지 않는다.
...
사람, AI, MCP, Web UI가 보는 원천은 `src/dartlab/skills` 와 docstring/generated capability 이다. 공개 UI는 이 원천 또는 generated JSON index를 읽어 렌더링한다.
