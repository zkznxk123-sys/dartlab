...
> 상위 사상: [philosophy.md](philosophy.md) · AI 설계: [../ASK_WORKBENCH_KERNEL.md](../ASK_WORKBENCH_KERNEL.md)

**주체**: DartLab 공용 분석 절차 계층 (`src/dartlab/skills`).
**현재**: API 사용법은 docstring/generated capabilities 가 SSOT 이고, 분석 절차는 SkillSpec 으로 공유한다.
**방향**: AI, MCP, story, UI, audit, 외부 LLM 이 같은 skill resolver 를 사용한다.
...
- SSOT: `src/dartlab/skills` 의 SkillSpec.
- 포함: 목적, 언제 쓰는지, 필요한 capability/tool/knowledge, 절차, required evidence, forbidden, audit 상태.
...
- 사용처: AI workbench, MCP, story, UI, audit, notebook.
...
- `category` (`basic`, `domain`, `user`, `capability`)
- `purpose`
- `whenToUse`
- `requiredInputs`
- `capabilityRefs`
...
수기 builtin spec 파일은 금융 분석 절차만 둔다.

```text
src/dartlab/skills/specs/domain/  # 금융 분석 절차
```
...
user skill 은 `scope=user`, `status=unverified` 로 시작한다. capability ref 유효성 검사를 통과해야 한다.
...
2. 분석 skill SSOT 는 `src/dartlab/skills` SkillSpec 이다.
