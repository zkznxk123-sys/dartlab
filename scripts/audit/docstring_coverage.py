...
전 엔진 커버리지
    uv run python scripts/audit/docstring_coverage.py --all

특정 엔진
    uv run python scripts/audit/docstring_coverage.py --engine scan --min 80

결과 JSON 저장
    uv run python scripts/audit/docstring_coverage.py --all --out data/audit/docstring_coverage.json
