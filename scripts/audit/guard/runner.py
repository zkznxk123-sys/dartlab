...
def l0L15Gates(repoRoot: Path, providers: str) -> list[ExternalGate]:
    ...
    return [
        runCommand(repoRoot, "cycleScan", [py, "-X", "utf8", "scripts/audit/cycleScan.py", "--strict-toplevel"]),
        runCommand(repoRoot, "architecturePytest", architectureCommand(py)),
        runCommand(
            repoRoot,
            "folderMirror",
            [py, "-X", "utf8", "scripts/audit/folderMirror.py", "--providers", providers, "--strict"],
            env=providerEnv,
        ),
        runCommand(repoRoot, "gatherGate", [py, "-X", "utf8", "scripts/audit/gatherGate.py"]),
        runCommand(
            repoRoot,
            "providerGate",
            [py, "-X", "utf8", "scripts/audit/providerGate.py", "--providers", providers],
            env=providerEnv,
        ),
        runCommand(
            repoRoot,
...
def quickGates(repoRoot: Path, changedFiles: list[str], providers: str) -> list[ExternalGate]:
    ...
    if needsGather:
        gates.append(runCommand(repoRoot, "gatherGate", [py, "-X", "utf8", "scripts/audit/gatherGate.py"]))
    if needsProvider:
        gates.append(
            runCommand(
                repoRoot,
