...
Sig::
    ...
    uv run python -X utf8 scripts/audit/measureProgress.py
    uv run python -X utf8 scripts/audit/measureProgress.py --record
    uv run python -X utf8 scripts/audit/measureProgress.py --record --json

핵심 측정 (read-only — 코드/baseline 변경 0):

1. baseline 부채 — ``scripts/audit/_baselines/*.json`` 의 ``violations`` list 또는
