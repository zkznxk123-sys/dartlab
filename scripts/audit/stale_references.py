...
설정 SSOT: ``scripts/audit/stalePatterns.yaml``
    - patterns: 카테고리별 정규식 + 심각도 (error/warn) + 메시지
    - whitelist: 경로 패턴 (의도된 잔존 — deprecation note, history 등)

사용법::

    uv run python -X utf8 scripts/audit/stale_references.py
    uv run python -X utf8 scripts/audit/stale_references.py --warn-only
    uv run python -X utf8 scripts/audit/stale_references.py --pattern review_to_story
