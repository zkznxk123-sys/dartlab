...
사용:
    # 변경된 파일만 검사 (PR 모드)
    uv run python -X utf8 scripts/audit/testCoverageGate.py --diff origin/master

    # 전체 검사 (nightly 모드)
    uv run python -X utf8 scripts/audit/testCoverageGate.py --all

    # warning 만 (default) — fail 차단 트리거 옵션
    uv run python -X utf8 scripts/audit/testCoverageGate.py --diff origin/master --fail-on-missing
