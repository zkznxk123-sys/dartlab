...
status: observed
category: engines
purpose: "analysis engine application skill for the 자산구조 axis: 조달한 돈으로 뭘 준비했는가."
whenToUse:
  - "analysis"
  - "자산구조"
  - "조달한 돈으로 뭘 준비했는가"
...
result = c.analysis("자산구조", "자산구조")
```

## 호출 동작

Reads the Company financial/disclosure/market snapshot required for the 자산구조 axis. The guide reports 4 calculation items. Missing values are represented through flags, assumptions, dataAsOf, nulls, or empty history; they are not zero-filled.
