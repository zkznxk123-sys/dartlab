...
status: observed
category: engines
purpose: "analysis engine application skill for the 재무정합성 axis: 재무제표가 서로 맞는가."
whenToUse:
  - "analysis"
  - "재무정합성"
  - "재무제표가 서로 맞는가"
...
result = c.analysis("재무정합성", "재무정합성")
```

## 호출 동작

Reads the Company financial/disclosure/market snapshot required for the 재무정합성 axis. The guide reports 6 calculation items. Missing values are represented through flags, assumptions, dataAsOf, nulls, or empty history; they are not zero-filled.
