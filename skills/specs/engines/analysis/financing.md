...
status: observed
category: engines
purpose: "analysis engine application skill for the 자금조달 axis: 돈을 어디서 조달하는가."
whenToUse:
  - "analysis"
  - "자금조달"
  - "돈을 어디서 조달하는가"
...
result = c.analysis("자금조달", "자금조달")
```

## 호출 동작

Reads the Company financial/disclosure/market snapshot required for the 자금조달 axis. The guide reports 9 calculation items. Missing values are represented through flags, assumptions, dataAsOf, nulls, or empty history; they are not zero-filled.
