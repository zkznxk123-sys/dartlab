...
status: observed
category: engines
purpose: "analysis engine application skill for the 매크로민감도 axis: 이 회사의 매출은 어떤 매크로 변수에 민감한가."
whenToUse:
  - "analysis"
  - "매크로민감도"
...
result = c.analysis("매크로민감도", "매크로민감도")
```

## 호출 동작

Reads the Company financial/disclosure/market snapshot required for the 매크로민감도 axis. The guide reports 1 calculation items. Missing values are represented through flags, assumptions, dataAsOf, nulls, or empty history; they are not zero-filled.
