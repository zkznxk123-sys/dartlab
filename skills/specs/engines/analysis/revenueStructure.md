...
status: observed
category: engines
purpose: "analysis engine application skill for the 수익구조 axis: 이 회사는 무엇으로 돈을 버는가."
whenToUse:
  - "analysis"
  - "수익구조"
  - "이 회사는 무엇으로 돈을 버는가"
...
result = c.analysis("수익구조", "수익구조")
```

## 호출 동작

Reads the Company financial/disclosure/market snapshot required for the 수익구조 axis. The guide reports 8 calculation items. Missing values are represented through flags, assumptions, dataAsOf, nulls, or empty history; they are not zero-filled.
