...
examples:
  ...
  - "`/api/agent/runs`를 AG-UI public event stream으로 검증"
...
- 공개 AI 답변 진입점은 `dartlab.ask(...)`와 `/api/ask`다. `/api/agent/runs`는 UI transport가 필요할 때만 public event adapter로 둔다.
...
- 최종 실행 계획은 반드시 root `skills/` 검색 결과와 generated capability/docstring 검색 결과로 만든다.
...
4. Agent Gateway: `/api/agent/runs`는 AG-UI allowlist만 방출한다.
