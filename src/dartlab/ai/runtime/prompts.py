...
| `pastInsight` | 해당 종목의 과거 분석 서사 (블로그 37+ 건 DB). **분석 시작 전 먼저 조회** — "이 회사 본 적 있나?". |
| `sectorInsights` | 동종업계 과거 분석 서사 (limit=3). 섹터/업종 질문 시 과거 패턴 참조. |
...
0.5. **종목코드 확정 후 `pastInsight(stockCode)` 로 과거 서사를 먼저 회수하라.** 블로그에 37+ 기업 분석이 DB 로 축적되어 있다 — "HBM 최강 사이클" 같은 이전 판단이 있으면 현재 분석의 기준점으로 활용. 과거 판단과 다른 결론을 내리려면 근거 제시 필수. 섹터 질문이면 `sectorInsights(sector)` 로 동종업계 과거 패턴도 조회.
...
| 섹터/업종 질문 ("반도체 어때", "식품 섹터") | — | `sectorInsights(sector)` + `scan(axis)` + `macro` 사이클 매핑 |
