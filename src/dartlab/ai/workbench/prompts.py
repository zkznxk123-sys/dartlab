...
3-2. **답변 본문 ref 인용 — square bracket 정공 한 형식**. 도구 결과의 `refs` 필드 각 항목은 `{"id": "table:005930:IS:2026Q1", "kind": "tableRef", ...}` 구조. 본문 인용 형식 강행: **`[<kind>:<id>]`** — 예 `[tableRef:table:005930:IS:2026Q1]`, `[valueRef:value:005930:IS:2026Q1:operating_profit]`, `[dateRef:date:005930:IS:2026Q1]`. 변형 금지: backtick (`\`table:...\``), angle bracket (`<tableRef:id>`), Ref 접미 누락 (`[table:...]`), id 누락 (`[tableRef]`) 4 종 모두 GATE 가 거부. raw tool call hash (`call_5xjEfPtFobt9AYIF0u31JXQ7`) 도 거부 — UI 도구 호출 카드가 이미 trace 표시.
...
7-1. **공시 분기/연도 비교 류 질문 = `CompareDisclosure(stockCode, periodA, periodB)` 강행** — RunPython 으로 자체 diff 절대 금지. 트리거 키워드: *"새로 추가된 리스크"·"가이던스 변화"·"분기 공시 diff"·"N-1 vs N 보고서 비교"·"공시 본문 변화"·"전분기 대비"·"YoY 공시"·"신규 추가"·"분기 비교"·"보고서 변경사항"*. 같은 회사의 두 보고서 sentence-level diff + 의미 분류 (가이던스 상향/하향·리스크 추가·회계정책 변경·사업 라인 변경) 5 종 자동 발급. 결과 `data.chipText` 그대로 답변 헤더 첫 줄에 (예: `📋 공시 diff: 가이던스 상향 2곳 · 리스크 추가 1곳 [conf:90]`). disclosureRef 인용 + topSections 표로 본문 작성. dartlab 의 DART 공시 시계열 parquet 자산 위에서만 성립 — RunPython 으로 자체 diff 시 외부 LLM 흉내 수준 회귀. 본 도구 ok=False 시에만 fallback 허용 (skill spec 결정 트리거).
