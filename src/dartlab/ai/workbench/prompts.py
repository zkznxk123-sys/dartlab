...
- `Company.executivePay` · `Company.relatedPartyTx` · `Company.notesDetail` · `Company.governance` · `Company.audit` · `Company.disclosure` · `Company.sections` · `Company.readFiling`
...
**별도 vs 연결 명시 강행**: K-IFRS 한국 기업의 NI · EBIT · 자본총계 등은 `basis` 인자 (separate · consolidated) 없이 답변 금지. Company.show(IS, basis=separate) 호출 결과는 답변 본문에 "(별도)" · Company.show(IS, basis=consolidated) 는 "(연결)" 한 글자 명시. 두 basis 결과를 동시에 비교할 때 별도 vs 연결 차이 narrative 동반 필수.
