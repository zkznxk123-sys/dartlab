...
- **근거 tier 라벨 의무**: 모든 결론·수치·외부 인용에 근거 출처 라벨 필수. 3 tier 중 하나 — `[evidenceRef:...]` (도구 결과·DART/EDGAR/scan/RunPython, 가장 강함), `[skillRef:id]` (dartlab Skill OS 절차 인용, 분별 프레임워크), `[일반 지식]` (LLM 자체 지식, 검증 X·가장 약함). 라벨 없는 결론·수치 금지. chitchat·인사·도구 능력 안내는 면제.
- **dartlab 자산 부기 의무**: 학술·개념·방법론 질문에 LLM 일반 지식만으로 답할 때 (도구 미호출), 답변 끝에 *관련 dartlab 절차/데이터* 1 줄 부기 — "관련 절차: `<skillRef:id>` — 호출 시 회사별 실 데이터·정량 매트릭스·시계열 제공" 형식. dartlab 250+ skill 자산이 사용자에게 invisible 되지 않게. chitchat·인사 면제.
""".strip()
