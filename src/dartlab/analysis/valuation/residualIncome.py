...
class RIMResult:
    ...
    intrinsicValue: float | None  # 주당 내재가치
    upside: float | None  # 현재가 대비 (%)
    terminalValue: float | None
...
def calcResidualIncome(
    ...
    currentPrice : float, optional
        현재 주가 (원).
    coe : float, optional
        자기자본비용 (%). None이면 CAPM 기반 자동 추정.
    currency : str
        통화 (기본 "KRW").
    beta : float, optional
        베타 계수. None이면 1.0 기본 + ROE 가감.
    ...
    RIMResult | None
        bps : float — 주당 순자산 (원)
        coe : float — 자기자본비용 (%)
        riHistory : list[dict] — 기간별 잔여이익 (index, ni, equity, ri)
        intrinsicValue : float | None — 주당 내재가치 (원)
...
def _estimateOmega(
    ...
    ROE-CoE spread의 AR(1) 자기상관으로 persistence 측정.
    안정적이면 omega 높음 (경쟁우위 지속), 급감 추세면 낮음 (평균 회귀).
    ...
    coeDecimal : float
        자기자본비용 (소수, 예: 0.10 = 10%).
    warnings : list[str]
        경고 메시지 누적 리스트 (in-place 추가).

    Returns
    -------
    float
        omega 값 (0.2~0.8). 데이터 부족 시 0.5.
    """
    n = min(len(niVals), len(eqVals))
    if n < 3:
...
def _estimateCoe(
    ...
    """자기자본비용(CoE) 추정 -- CAPM (MarketParams 기반).
