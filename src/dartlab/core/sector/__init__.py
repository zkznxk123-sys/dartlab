...
# 데이터 JSON은 dartlab/industry/ 에 위치 (역사적 경위).
# core/sector/__init__.py → parent(core/sector/) → parent(core/) → parent(dartlab/) / industry/
_DATA_DIR = Path(__file__).resolve().parent.parent.parent / "industry"
...
def get_sector_params_by_name(name: str) -> SectorParams | None:
    ...
    이름이 industryGroup 또는 sector 값과 일치하면 해당 params 반환, 없으면 None.
    """
    if not name:
        return None
    data = _loadSectorData()
    ...
    def _toParams(p: dict) -> SectorParams:
        return SectorParams(
            discountRate=p["discountRate"],
            growthRate=p["growthRate"],
            perMultiple=p["perMultiple"],
    ...
    if name in igp:
        return _toParams(igp[name])
    if name in sp:
        return _toParams(sp[name])
    return None
