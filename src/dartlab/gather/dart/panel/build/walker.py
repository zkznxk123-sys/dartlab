"""panel row walker — 손실 0 + 중복 0 정공법.
...
from .refScan.aclassExtractor import (
    _hasATOCID,
    _stripXbrlPrefix,
)
from .refScan.refMatcher import matchToRef
from .refScan.titleNormalizer import (
    normalizeTitle,
)
...
def detectSchemaEra(root) -> str:
    """DART 본문 XML 양식(era) 자동 감지.

    Args:
        root: lxml etree root (BODY 또는 DOCUMENT).

    Returns:
        ``"v2"`` = 2023Q4+ 신 양식 (ATOCID 박힘) / ``"v1.5"`` = ACLASS 있는 옛 양식 /
        ``"v1"`` = AASSOCNOTE only (~2015~2023Q3).

    Raises:
        없음 — lxml root element 를 가정, find/iter 만 사용.
    ...
    SeeAlso:
        - ``walkSections`` — era 를 받아 양식별 ACLASS 추출 분기.
        - ``refScan.aclassExtractor._hasATOCID`` — ATOCID 존재 판별 helper.

    Requires:
        - lxml.

    Capabilities:
        - 양식 era 판별 → walker 가 신 양식(ACLASS 직접) vs 옛 양식(fuzzy match) 분기.

    Guide:
        - builder 가 zip XML 마다 호출 — 직접 호출 X.
    ...
    LLM Specifications:
        ...
        Prerequisites:
            - lxml etree root.
        Freshness:
            - DART 양식 변경 시 ATOCID/ACLASS 감지 룰 재검증.
        Dataflow:
            - root → ATOCID 존재 → v2 / TABLE-GROUP[@ACLASS] 존재 → v1.5 / else v1.
        TargetMarkets:
            - KR (DART). EDGAR 는 별도 walker.
    """
    if _hasATOCID(root):
...
def walkSections(
    ...
    *,
    ...
    SeeAlso:
        - ``detectSchemaEra`` — era 인자 산출.
        - ``horizontalize`` — 본 element 행을 canonical 단위로 무손실 concat.
        - ``refScan.refMatcher.matchToRef`` — 옛 양식 fuzzy 매칭.

    Requires:
        - lxml. refDf (옛 양식 v1/v1.5 fuzzy 매칭 시).

    Capabilities:
        - DART 본문 XML 을 손실0/dup0 으로 element 행화 — contentRaw 태그 100% 보존(R4).
    ...
    LLM Specifications:
        AntiPatterns:
            - TABLE-GROUP 만 emit 금지 — narrative P/TABLE/IMAGE 동일 권리.
            - 같은 element 이중 emit 금지 — _markContainers 후 parent==container 조건.
            - contentRaw 에 태그 strip 금지 — etree.tostring 원본(R4).
        OutputSchema:
            - ``Iterator[dict]`` (10 key: chapter/sectionLeaf/blockLeaf/xbrlClass/
              xbrlMatched/xbrlMatchScore/atocId/aassocnote/blockOrder/contentRaw).
        Prerequisites:
            - lxml etree root + era. refDf(fuzzy 시).
        Freshness:
            - DART 양식 변경 시 wrapper TABLE-GROUP 감지 룰 재검증.
        Dataflow:
            - root → _markContainers → iterwalk → emit 조건 만족 시 row yield.
        TargetMarkets:
            - KR (DART). EDGAR 는 별도 walker.
    """
