...
## SSOT P-revised — canonical 7 데이터 도구
...
- EngineCall: 단일 capability 1 회 호출 (Company.show, scan, macro 등). JSON `{apiRef, args}` 양식.
...
마이그레이션 예 — 단일 capability 호출은 EngineCall 우선:
    ...
    RunPython(code='''
    a = dartlab.Company("005930").show("IS")
    b = dartlab.Company("000660").show("IS")
