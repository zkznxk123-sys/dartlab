...
분석 코드 (quant/industry/frame/scan) 는 본 10 컬럼만 사용해 DART/EDGAR 어느
쪽이든 동일 코드로 동작.
...
# DART 와 EDGAR 가 동시 보존해야 하는 공통 본체 컬럼 (10 개).
PROVIDER_AGNOSTIC_COLS: tuple[tuple[str, pl.DataType], ...] = (
    ...
    ("textSemanticPathKey", pl.Utf8),  # canonical cross-period 매칭 key
