...
마스터 플랜: v5 §2.2 + §5.1 양식 자동 감지 알고리즘.
"""
...
from dartlab.providers.dart.docs.sectionsNew.refScan.titleNormalizer import normalizeTitle
...
def _hasATOCID(root) -> bool:
    """본문에 ``<TITLE ATOCID>`` 존재 여부 = 양식 판별 (v5 §5.1).
...
def iterTableGroups(root) -> Iterator[tuple[etree._Element, etree._Element | None]]:
    ...
    Yields:
        ``(tg, parent_tg | None)`` — parent_tg 는 가장 가까운 ancestor
        TABLE-GROUP. 없으면 None (top-level).
    """
