"""sections row walker — 손실 0 + 중복 0 정공법 (v5 §5.4 재정의).
...
원칙 (사용자 9 요구 #5/#6 + memory/feedback_no_content_plain_precompute):
...
LLM Specifications:
    AntiPatterns:
        ...
        - 같은 element 를 walker + walkerV1 이중 호출로 emit 중복 금지.
        - SECTION-N stack 추적 시 ancestor 만 사용 — descendant 검사 금지.
    OutputSchema:
        - ``walkSections(root, era, refDf) -> iter[dict]`` 14 col row.
...
마스터 플랜: v5 §5.4 walkSections (재정의).
"""
...
from dartlab.providers.dart.docs.sectionsNew.refScan.refMatcher import matchToRef
...
def walkSections(
    ...
    *,
    ...
    blockOrder = 0
    rowsBuffer: list[dict] = []  # mark 청소 전 buffer (contentRaw 에 _dlContainer 잔재 방지)
