"""DART panel ref table scanner.

목적:
    DART 본문 XML 의 ``<TABLE-GROUP ACLASS="{XBRL}...">`` 를 회사 무관 canonical
    key 로 활용. 전체 102k zip 스캔 → ``panelXbrlRef.parquet`` 자동 추출.
...
LLM Specifications:
    ...
    OutputSchema:
        - ``panelXbrlRef.parquet`` 11 col: rawId / rawTitleCanonical /
