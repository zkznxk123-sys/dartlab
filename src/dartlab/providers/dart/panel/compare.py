...
def compare(
    ...
    *,
    ...
    """N 회사 공시 panel 을 회사 간 시점 비교 wide 로 정렬한다.

    한 회사 ``Panel`` 이 항목×기간이라면, ``compare`` 는 같은 공시 항목을 여러 회사에 걸쳐 가로로
    세운 wide 다. 정렬키(disclosureKey, scope, leafType)가 회사마다 다른 라벨·절 번호 drift 를
    자동 해소해 "같은 항목"을 한 행에 모은다 (예: 삼성 "7. 유형자산" ↔ SK "11. 유형자산" 한 행).
