...
def buildSourceManifest(
    ...
    *,
    ...
    fileRows = [_fileManifest(path) for path in paths]
...
def _fileManifest(path: Path) -> dict[str, Any]:
    rowCount, minDate, maxDate = _fileStats(path)
    return {
        "path": path.as_posix(),
...
def _fileStats(path: Path) -> tuple[int, str, str]:
    if not path.exists():
        return 0, "", ""
