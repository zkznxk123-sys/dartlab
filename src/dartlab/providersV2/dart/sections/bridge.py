...
tier1 ~60 seed:
    BS/IS/CF/EF 4 재무제표 + 핵심 주석 disclosure + 사업 일반 정보.
...
LLM Specifications:
    ...
    OutputSchema:
        ...
        - ``seedBridgeTier1() -> pl.DataFrame`` ~60 entry hardcoded.
    Prerequisites:
...
def _tier1Seed() -> list[dict]:
    """tier1 ~60 entry seed — DART ACLASS rawId + EDGAR us-gaap TextBlock 매핑.

    DART side 는 5 baseline scan 에서 확인된 NT_C_D###### / NT_S_D######
    ...
    rows: list[tuple[str, str, str, float]] = [
        ...
        # ── 주석 확장 (top corpCount entry from ref) ──
...
def seedBridgeTier1(*, overwrite: bool = False) -> pl.DataFrame:
    """tier1 seed parquet 생성.

    Args:
        overwrite: 기존 parquet 덮어쓰기 여부.

    Returns:
        seed DataFrame (~60 entry).
    """
