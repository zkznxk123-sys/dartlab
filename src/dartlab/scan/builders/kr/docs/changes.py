...
def _buildRawChanges(stockCode: str, sinceYear: int = 2021) -> pl.DataFrame | None:
    """panel 섹션 본문 → section 단위 변화 감지 (providers.dart.panel.text SSOT).
    ...
    Requires:
        panel artifact (providers.dart.panel.text.panelTextRows: sectionLeaf · contentRaw · period).

    SeeAlso:
        ``buildChanges`` · ``dartlab.providers.dart.panel.text.panelTextRows``.
    """
    from dartlab.providers.dart.panel.text import panelTextRows

    df = panelTextRows(stockCode)
...
def buildChanges(*, sinceYear: int = 2021, verbose: bool = True) -> Path | None:
    ...
    How:
        panel parquet 순회 → ``_buildRawChanges``(providers.dart.panel.text) → 200 단위 batch → final merge.
