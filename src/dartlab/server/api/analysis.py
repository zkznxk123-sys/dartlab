...
def _panelTextWide(company):
    """Return DART panel text wide view used by legacy text-analysis endpoints."""
...
def apiCompanyDiffMatrix(
    ...
    try:
        textWide = _panelTextWide(c)
        if textWide is None:
            raise HTTPException(status_code=404, detail="panel text 없음")
        matrixData = buildDiffMatrix(textWide, textOnly=textOnly)
...
def apiCompanyDiffTopic(
    ...
    try:
        ...
        if result is not None:
            from dartlab.providers._common.diff import charDiff
...
def apiCompanyBridge(
    ...
    try:
        ...
        if textWide is None:
            raise HTTPException(status_code=404, detail="panel text 없음")
        periods = sorted(
            [col for col in textWide.columns if _re.fullmatch(r"\d{4}(Q[1-4])?", col)],
            reverse=True,
        )
        if not periods:
        ...
        if target_period not in textWide.columns:
            raise HTTPException(status_code=400, detail=f"기간 {target_period} 없음")

        topic_rows = textWide.filter(pl.col("topic") == topic)
...
def apiCompanyTopicsGraph(
    ...
    try:
        ...
        if textWide is None:
            raise HTTPException(status_code=404, detail="panel text 없음")
        matrix = buildMentionMatrix(textWide)
...
def apiCompanySearchSections(code: str, q: str = Query("", description="검색어")):
    """현재 회사의 panel text 전체에서 substring 검색."""
    try:
        c = getCompany(code)
        sec = _panelTextWide(c)
        ...
        for row in sec.iter_rows(named=True):
            ...
            if not topic or topic in seen_topics:
                continue
            if "blockType" in sec.columns and str(row.get("blockType", "")) != "text":
                continue
...
def apiCompanySearchIndex(code: str, request: Request, response: Response):
    ...
    회사의 panel text 전체를 topic × period × blockType 단위 문서로 평탄화하여 반환.
    ...
    try:
        ...
        for row in sec.iter_rows(named=True):
            if doc_id >= _MAX_DOCS:
                break
            topic = row.get("topic", "")
            block_type = str(row.get("blockType") or "text")
