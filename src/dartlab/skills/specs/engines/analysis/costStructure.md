...
status: observed
category: engines
purpose: "analysis engine application skill for the 비용구조 axis: 비용이 어떻게 움직이는가."
whenToUse:
  - "analysis"
  - "비용구조"
  - "비용이 어떻게 움직이는가"
...
result = c.analysis("비용구조", "비용구조")
```

## 호출 동작

Reads the Company financial/disclosure/market snapshot required for the 비용구조 axis. The guide reports 5 calculation items. Missing values are represented through flags, assumptions, dataAsOf, nulls, or empty history; they are not zero-filled.
