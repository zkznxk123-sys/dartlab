...
status: observed
category: engines
purpose: "analysis engine application skill for the 이익품질 axis: 이익이 진짜인가."
whenToUse:
  - "analysis"
  - "이익품질"
  - "이익이 진짜인가"
...
result = c.analysis("이익품질", "이익품질")
```

## 호출 동작

Reads the Company financial/disclosure/market snapshot required for the 이익품질 axis. The guide reports 7 calculation items. Missing values are represented through flags, assumptions, dataAsOf, nulls, or empty history; they are not zero-filled.
