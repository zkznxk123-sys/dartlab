...
status: observed
category: engines
purpose: "analysis engine application skill for the 효율성 axis: 이 회사는 자산을 잘 굴리는가."
whenToUse:
  - "analysis"
  - "효율성"
  - "이 회사는 자산을 잘 굴리는가"
...
result = c.analysis("효율성", "효율성")
```

## 호출 동작

Reads the Company financial/disclosure/market snapshot required for the 효율성 axis. The guide reports 2 calculation items. Missing values are represented through flags, assumptions, dataAsOf, nulls, or empty history; they are not zero-filled.
