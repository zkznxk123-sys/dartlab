...
status: observed
category: engines
purpose: "analysis engine application skill for the 투자효율 axis: 투자가 가치를 만드는가."
whenToUse:
  - "analysis"
  - "투자효율"
  - "투자가 가치를 만드는가"
...
result = c.analysis("투자효율", "투자효율")
```

## 호출 동작

Reads the Company financial/disclosure/market snapshot required for the 투자효율 axis. The guide reports 5 calculation items. Missing values are represented through flags, assumptions, dataAsOf, nulls, or empty history; they are not zero-filled.
