...
status: observed
category: engines
purpose: "analysis engine application skill for the 종합평가 axis: 재무 상태를 한마디로."
whenToUse:
  - "analysis"
  - "종합평가"
  - "재무 상태를 한마디로"
...
result = c.analysis("종합평가", "종합평가")
```

## 호출 동작

Reads the Company financial/disclosure/market snapshot required for the 종합평가 axis. The guide reports 3 calculation items. Missing values are represented through flags, assumptions, dataAsOf, nulls, or empty history; they are not zero-filled.
