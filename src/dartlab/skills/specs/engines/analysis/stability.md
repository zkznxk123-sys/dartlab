...
status: observed
category: engines
purpose: "analysis engine application skill for the 안정성 axis: 이 회사는 망하지 않는가."
whenToUse:
  - "analysis"
  - "안정성"
  - "이 회사는 망하지 않는가"
...
result = c.analysis("안정성", "안정성")
```

## 호출 동작

Reads the Company financial/disclosure/market snapshot required for the 안정성 axis. The guide reports 6 calculation items. Missing values are represented through flags, assumptions, dataAsOf, nulls, or empty history; they are not zero-filled.
