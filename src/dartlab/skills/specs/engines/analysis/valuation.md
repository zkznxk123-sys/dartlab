...
status: observed
category: engines
purpose: "analysis engine application skill for the 가치평가 axis: 이 회사의 적정 가치는 얼마인가."
whenToUse:
  - "analysis"
  - "가치평가"
  - "이 회사의 적정 가치는 얼마인가"
...
result = c.analysis("가치평가", "가치평가")
```

## 호출 동작

Reads the Company financial/disclosure/market snapshot required for the 가치평가 axis. The guide reports 14 calculation items. Missing values are represented through flags, assumptions, dataAsOf, nulls, or empty history; they are not zero-filled.
