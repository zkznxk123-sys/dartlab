...
status: observed
category: engines
purpose: "analysis engine application skill for the 밸류에이션밴드 axis: PER/PBR이 과거 대비 어디에 있는가."
whenToUse:
  - "analysis"
  - "밸류에이션밴드"
...
result = c.analysis("밸류에이션밴드", "밸류에이션밴드")
```

## 호출 동작

Reads the Company financial/disclosure/market snapshot required for the 밸류에이션밴드 axis. The guide reports 1 calculation items. Missing values are represented through flags, assumptions, dataAsOf, nulls, or empty history; they are not zero-filled.
