---
id: "engines.gather.collect"
title: "Gather - 전체 snapshot"
kind: curated
scope: builtin
status: observed
category: engines
...
sourceRefs:
  - "dartlab://skills/engines.gather.collect"
requiredEvidence:
  - "target"
  - "provider"
  - "latestAsOf"
...
# 1. KR 기본
result = dartlab.gather("collect", "005930")

# 2. US 시장
result = dartlab.gather("collect", "AAPL", market="US")
```

## 호출 동작

provider / cache / snapshot 데이터를 읽어 전체 snapshot 결과를 반환한다. 캐시 hit 시 즉시 반환, miss 시 외부 API 호출 (실패 시 fallback chain). API 키가 필요한 provider 는 누락 시 `None` / 빈 결과 + 안내 — 결과 없음으로 오해하지 않는다. 자세한 동작은 base SKILL `engines.gather` + `Gather.collect` 메서드 docstring 참조.
...
전체 키는 `Gather.collect` docstring 으로 검산.
...
이 skill 은 공개 실행 문서다. `Gather.collect` 메서드의 호출 시그니처, 반환 키, 오류 / 제한 동작이 변경되면 같은 변경에서 본 파일을 갱신한다. SSOT 는 `Gather` class docstring + base SKILL `engines.gather`.
