...
status: unverified
category: engines
purpose: 기업 지표를 산업 공정, 밸류체인, peer 맥락과 연결하는 절차를 설명한다.
whenToUse:
...
runtimeCompatibility:
  ...
  webAi:
    status: limited
    notes:
      - taxonomy 조회는 가능하나 summary/timeline은 finance coverage에 좌우된다.
  pyodide:
    status: limited
    dataSources:
      - packaged taxonomy snapshot
    limitations:
      - 공정별 재무 summary는 finance snapshot coverage를 따른다.
failureModes:
  - 산업 분류를 투자 결론으로 바로 연결
  - peer universe 없이 peer 비교
  - 산업 driver와 기업 지표의 인과를 검증 없이 단정
forbidden:
  - API parameters/returns를 SkillSpec에 중복하지 않는다.
  - 산업명만으로 기업 실적 원인을 단정하지 않는다.
examples:
  - 반도체 기업은 industry로 공정 위치를 확인하고 scan/analysis로 peer와 수익성을 비교한다.
  - macro shock은 industry driver를 거쳐 기업 현금흐름 영향으로 연결한다.
source:
