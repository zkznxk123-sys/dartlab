---
id: "engines.quant.accruals"
title: "Quant - Sloan Accrual"
kind: curated
scope: builtin
status: observed
category: engines
purpose: "quant 엔진의 Sloan Accrual 축 응용 — Sloan 1996 — (NI−CFO)/TA, high/neutral/low 3 그룹 분포."
whenToUse:
  - "quant"
  - "accruals"
  - "Sloan Accrual"
  - "Sloan 1996 — (NI−CFO)/TA, high/neutral/low 3 그룹 분포"
inputs:
  - "종목코드 또는 종목 리스트"
  - "기준 기간"
  - "benchmark / 가정 (해당 시)"
...
knowledgeRefs:
  ...
  - "engines.analysis"
sourceRefs:
  - "dartlab://skills/engines.quant.accruals"
requiredEvidence:
  - "target"
  - "period"
  - "metric"
...
quant 엔진의 Sloan Accrual 축 응용 skill — Sloan 1996 — (NI−CFO)/TA, high/neutral/low 3 그룹 분포. fundamental 그룹. SSOT 는 `_AXIS_REGISTRY` (`src/dartlab/quant/__init__.py`).
...
전종목 universe 의 가격 · 재무 · 시계열 snapshot 을 읽어 Sloan Accrual 축 계산을 수행한다. Sloan 1996 — (NI−CFO)/TA, high/neutral/low 3 그룹 분포. 결손 / 비교 불가 케이스는 결과 dict 또는 DataFrame 의 `flags` / null 로 표현하며 0 으로 채우지 않는다. 자세한 동작은 base SKILL `engines.quant` + `_AXIS_REGISTRY['accruals'].fn` 함수 docstring 참조.
...
- 축 고유 metric / score / verdict / rank column (정확한 spec 은 `_AXIS_REGISTRY['accruals'].fn` 함수 docstring 검산)
