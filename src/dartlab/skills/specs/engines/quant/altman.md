...
status: observed
category: engines
purpose: "quant engine application skill for Altman Z: Altman 1968/1995 — 전종목 부실확률 (safe/grey/distress 3 zone) + topSafe/topDistress (종목 불필요)."
whenToUse:
  - "quant"
  - "altman"
  - "Altman Z"
  - "펀더멘털"
  - "Altman 1968/1995 — 전종목 부실확률 (safe/grey/distress 3 zone) + topSafe/topDistress (종목 불필요)"
inputs:
  - "target"
  - "period"
  - "axis or method"
...
```python
import dartlab
result = dartlab.quant("altman", "005930")
```

## 호출 동작

Runs a 펀더멘털 quantitative calculation. Confirm period, benchmark, target requirements, and for strategy/backtest axes separate rule and cost assumptions.
