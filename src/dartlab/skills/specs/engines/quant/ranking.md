...
status: observed
category: engines
purpose: "quant engine application skill for 순위: 멀티팩터 복합 순위 (모멘텀+가치+퀄리티+리스크) (종목 불필요)."
whenToUse:
  - "quant"
  - "ranking"
  - "순위"
  - "횡단면"
  - "멀티팩터 복합 순위 (모멘텀+가치+퀄리티+리스크) (종목 불필요)"
inputs:
  - "target"
  - "period"
  - "axis or method"
...
```python
import dartlab
result = dartlab.quant("ranking", "005930")
```

## 호출 동작

Runs a 횡단면 quantitative calculation. Confirm period, benchmark, target requirements, and for strategy/backtest axes separate rule and cost assumptions.
