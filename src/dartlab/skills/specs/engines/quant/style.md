...
status: observed
category: engines
purpose: "quant engine application skill for 스타일: 8 검증된 스타일 프리셋 일괄/단일 백테스트 (시총 의존 0) (종목 불필요)."
whenToUse:
  - "quant"
  - "style"
  - "스타일"
  - "전략 DSL"
  - "8 검증된 스타일 프리셋 일괄/단일 백테스트 (시총 의존 0) (종목 불필요)"
inputs:
  - "target"
  - "period"
  - "axis or method"
...
```python
import dartlab
result = dartlab.quant("style", "005930")
```
