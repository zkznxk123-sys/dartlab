---
id: engines.recipe.creditCovenantStressTest
...
2. breach prob > 0.3 → `engines.recipe.creditMacroStress` 와 결합 — 매크로 충격이 신용등급에 미치는 영향 동시 점검.
3. universe 적용 — `engines.recipe.distressCandidateScreen` 의 candidate set 에서 covenant 임박 종목 우선 평가.
