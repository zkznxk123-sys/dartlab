---
id: engines.recipe.dollarFundingStress
...
1. 달러 압력이 유동성 문제라면 `engines.recipe.globalLiquidityPulse` 로 원인을 확인한다.
2. 한국 시장 영향은 `engines.recipe.koreaMacroStressMap` 또는 `engines.recipe.koreaExportCycleNowcast` 로 연결한다.
3. 신용위험이 같이 커지면 `engines.recipe.creditCycleStressMap` 으로 확장한다.
