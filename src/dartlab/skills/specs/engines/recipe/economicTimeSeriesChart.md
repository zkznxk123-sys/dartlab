---
id: engines.recipe.economicTimeSeriesChart
title: 경제 원자료 시계열 차트
category: engines
kind: recipe
scope: builtin
...
linkedSkills:
  ...
  - engines.viz
  - engines.recipe.yieldCurveStress
  - engines.recipe.inflationBreadthWatch
toolRefs:
  - RunPython
  - CompileVisual
requiredEvidence:
...
1. 금리곡선이면 `engines.recipe.yieldCurveStress` 로 해석한다.
2. 물가 계열이면 `engines.recipe.inflationBreadthWatch` 로 확산성을 확인한다.
