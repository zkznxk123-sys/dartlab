---
id: engines.recipe.globalLiquidityPulse
...
1. 유동성 축소가 확인되면 `engines.recipe.yieldCurveStress` 로 금리곡선 압력을 확인한다.
2. 달러 유동성 압력이 크면 `engines.recipe.dollarFundingStress` 로 환율/위험회피 신호를 확인한다.
