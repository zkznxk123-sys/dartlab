---
id: engines.recipe.historicalPositioning
...
1. 위기 유사성이 높으면 `engines.recipe.creditCycleStressMap`.
2. scenario 손실 분포가 필요하면 `engines.recipe.tailRiskScenarioScan`.
3. 한국 시장 적용이 핵심이면 `engines.recipe.koreaMacroStressMap`.
