---
id: engines.recipe.koreaExportCycleNowcast
...
1. 환율 압력이 핵심이면 `engines.recipe.dollarFundingStress` 로 연결한다.
2. 한국 전체 스트레스 지도는 `engines.recipe.koreaMacroStressMap` 으로 확장한다.
