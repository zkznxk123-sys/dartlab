---
id: engines.recipe.macroQuantScenarioBacktest
...
2. quality factor 위기 outperform 확인 → `engines.recipe.qualityMacroBeta` 의 단일 회사 결과와 정합성 검증.
3. value 후행기 부진 → `engines.recipe.industryStageScreen` 의 stage filter 가 효과 있는지 보강.
