...
`scripts/build/generateSkills.py` **자동 실행 금지** — CI workflow · CLI 자동 호출 · hook · agent 트리거 어디서도. 산출물 동기화 필요 시 **운영자 명시 수동 실행만** (`uv run python -X utf8 scripts/build/generateSkills.py`). 스크립트 자체는 보존 (수동 도구), 자동 호출만 차단. 별도 commit "정리: skill 인덱스 동기화" ([commit-self-change](file://./.claude/skills/commit-self-change/SKILL.md)).
