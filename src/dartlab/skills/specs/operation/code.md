...
12. **호출계약 = 사용 · 일관·통일 · 추정가능** — 공개 표면 (facade 메서드 · `__all__` · Protocol surface) 은 세 조건 동시 만족. 위반은 "추가" 가 아니라 "정리" 대상.
    ...
    2. **일관·통일** — 동일 도메인 내 명명 · 시그니처 · 반환형 통일. 같은 일을 하는 변종을 다중 메서드로 쪼개기 금지 (예: `sectionsRaw`/`sectionsTables`/`sectionsLazy`/`sectionsLong` → `sections(*, format=, lazy=)` 단일화). cross-provider (dart/edgar) 표면 대칭 (rule 1 Protocol 과 동행).
