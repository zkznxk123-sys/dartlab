...
inputs:
  ...
  - 검증 범위 (dart-panel-only / Guard Index / panel unit)
outputs:
  - data/dart/panel/{code}.parquet (공개 SSOT)
  - data/original/dart/docs/{code}/{rcept_no}.zip (로컬 임시, HF 미공개)
  - panel read 입력 (기존 schema 호환 + atocid/assocnote 신규)
...
- `data/dart/panel/{code}.parquet` 은 *derived* — zip 으로부터 언제든 재빌드.
...
- 추론 0 — legacy parser layer 의 regex inline split 폐기 가능.

**1.3 zip 비공개 (사용자 결정 2026-05-21)**
- zip 은 panel.parquet 와 중복 정보 → HF 미공개.
...
- hierarchy 없음 → legacy parser layer 가 regex 추론
...
**호환:** 기존 컬럼 모두 유지. atocid/assocnote 만 신규 추가. legacy parser layer 가
...
**5.1 dart-panel-only audit (PR gate)**
```powershell
python -X utf8 tests/audit/dartPanelOnly.py
```
- 폐기된 DART docs/sections runtime surface 재등장 0

**5.2 panel text/unit gate**
```powershell
python -X utf8 -m pytest tests/providers/dart/panel/test_text.py tests/architecture/test_no_raw_cross_scan.py -q --tb=short
```
- panel text helper + raw cross-scan 회귀 0
...
- artifact upload: `data/dart/panel/` 만 (zip 제외)

**6.2 폐기 workflow 상태**
- 옛 DART section parity workflow 는 삭제 상태를 유지한다.
- 현행 fast CI 는 `dart-panel-only` + Guard Index + panel 단위 테스트를 실행한다.

**6.3 ci-fast 게이트**
- 폐기됨 — DART 본문 읽기 SSOT 는 `data/dart/panel/{code}.parquet` 이며, fast CI 는 Guard Index + panel 단위 테스트가 맡는다.
...
3. GitHub Actions artifact upload path — `data/dart/panel/` 만

## §8 — 호출 cookbook

**8.1 단일 종목 수집 (사용자):**
```powershell
python -X utf8 -m dartlab.providers.dart.panel.build --codes 005930
python .github/scripts/sync/onlinePanel.py --codes 005930
```
...
c = Company("005930")
sec = panelTextWide(c.stockCode)   # 내부 새 schema 입력으로 자동 동작
```
...
panel.parquet 완벽 판단 (legacy raw-compare drift 0 종목 비율 ≥ 95%) 시:
- zip 저장 단계 폐기
...
- legacy section parity audit: 0 violations / 5 codes
- legacy raw compare audit: spurious=11 → **Phase B-1 후 spurious=2** (commit 153bbdd45)
- polars panic: 0
- OOM: 0 (035720 broken builder 2.4GB → 14MB fixed)
- panel builder unit: 7 passed
...
복원. panel builder unit 8 passed.
...
- `collectAllOriginalZips(codes=None)` — 고수준 진입점. `data/dart/panel/*.parquet` 의
...
- R3: 5 baseline legacy parity drift 0 / raw-compare spurious 0.
...
**옵션 3 검증 — panel batch build:**
- POC (sectionsParallelPoc.py): 5 baseline 직렬 61.6s → 병렬 5 worker 19.0s = **3.25×**
- 본진 API `python -m dartlab.providers.dart.panel.build --codes ...`:
  - panel build SSOT 로 corp 별 panel artifact 생성
  - 결과는 panel parquet + index/label 계층에 저장
- 검증: 5 corps via 본진 API **16.42s** (3.7× speedup, 모두 success)
- 후속 호출은 디스크 cache hit (~1.3s)

**사용 예:**
```python
# 단일 corp — panel artifact build
sec = panelTextWide("005930")
```

```powershell
python -m dartlab.providers.dart.panel.build --codes 005930 035720 005380 207940 000660
```
