...
category: operation
purpose: Recipe sub-spec 의 status frontmatter 변경은 `scripts/dev/recipe_promote.py` CLI 단독 권한. AI · 도구 · 운영자 수동 편집 모두 금지. 6-stage (drafted→unverified→tested→verified→curated→deprecated) + scorecard 6 신호 게이트로 자기개선 사다리 회귀 차단.
whenToUse:
  - recipe 승급
  - status 변경
  - "verified 로 올려"
...
procedure:
  ...
  - 시장 N 일 reflection + scorecard 6 신호 통과 시 verified
...
requiredEvidence:
  ...
  - marketResultRef (시장 reflection 결과 — verified 이상 필수)
...
failureModes:
  ...
  - 시장 reflection 없이 curated 승급 (사람 검토 누락)
...
| `tested` | 실행 OK, 시장 검증 전 | 시장 N 일 reflection + scorecard 6 신호 |
...
4. `webRef` — 외부 출처 (있으면).
5. `marketResult` — 시장 N 일 후 reflection 결과 ref.
...
[recipe_promote] verified 승급 차단 — scorecard 6 신호 중 2 개 미달:
  - marketResult: ref 없음 (시장 reflection 7 일 후 재시도)
...
## 자기개선 사다리 회귀 차단 — 왜 이 가드가 있는가

- 과거 `kind=generated` 자기진화 skill 0 promoted 로 dormant. `propose_skill` 폐기 ([feedback_recipe_lifecycle](file://C:/Users/MSI/.claude/projects/C--Users-MSI-OneDrive-Desktop-sideProject-dartlab/memory/feedback_recipe_lifecycle.md)).
- AI 가 자율적으로 recipe 를 verified 로 마크하면 *시장 검증 없이* 사다리 완성 → 환각 강화 루프.
- 본 룰은 그 회귀의 코드화 — status 변경 권한을 CLI 한 곳으로 모은다.

## 다음 단계

- 새 recipe 작성 → 강제 섹션 + 4 단계: [operation.skillDevelopmentLoop](/skills/operation.skillDevelopmentLoop).
- recipe scorecard 정의 + 시장 reflection 절차: `operation.coreloop`.
- 신규 recipe 15 종 카탈로그: [project_recipe_catalog_v1](file://C:/Users/MSI/.claude/projects/C--Users-MSI-OneDrive-Desktop-sideProject-dartlab/memory/project_recipe_catalog_v1.md).
...
- tested 건너뛰고 verified 점프.
- AI 자율 status 변경 (CLI 호출 제외 — CLI 안에서도 사용자/도구 명시 인자 필요).

근본: `operation.coreloop` · [feedback_recipe_lifecycle](file://C:/Users/MSI/.claude/projects/C--Users-MSI-OneDrive-Desktop-sideProject-dartlab/memory/feedback_recipe_lifecycle.md) · CLAUDE.md "통합 아키텍처" 3 자 진화 루프.
