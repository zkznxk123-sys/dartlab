...
procedure:
  ...
  - Forge 는 intent, inputs, dataSources, procedure, outputs, outputSchema, criteria, forbidden, completionCriteria 를 marketIndex 에 보존한다.
...
Skill Market 의 완성 상태는 `marketCurated` 이다. 이는 Discussion 에서 완성된 공유스킬이 정적 market index 에 들어가 랜딩과 AI 검색에서 사용 가능하다는 뜻이다. 패키지 builtin Skill OS 에 편입한다는 뜻이 아니다.
...
| 파일 | 용도 |
|---|---|
| `marketIndex.json` | Pages 와 AI 가 읽는 검색용 인덱스. 완성된 공유스킬의 절차·데이터·출력 스키마·금지 조건을 포함한다. |
...
Market 결과를 답변에 사용할 때는 `sourceUrl`, `trustTier`, `author`, `missingDetails` 를 함께 드러낸다. `marketDraft` 는 직접 실행하지 않는다. `marketRunnable` 이상도 외부 작성물이라는 사실을 유지한다. `marketCurated` 는 Skill Market 안에서 완성된 공유스킬이므로 `procedure`, `dataSources`, `outputSchema`, `forbidden` 을 함께 읽어 실행 경계를 고정한다.
