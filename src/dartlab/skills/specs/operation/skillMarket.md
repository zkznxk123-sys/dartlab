...
procedure:
  ...
  - Forge 는 intent, inputs, dataSources, procedure, executionPlan, outputs, outputSchema, criteria, forbidden, completionCriteria 를 구조화한다.
  - executionPlan 은 실제 DartLab engine 또는 recipe caller 를 어떤 순서로 쓰는지 적는 실행 계약이며, mappedBuiltinSkills 와 다르다.
  ...
  - maintainer 만 marketRunnable, marketCurated, builtinCandidate 상태를 확정한다.
  - 확정 명령은 executionPlan, 예시 입력과 기대 출력, 판단 기준, 실패 조건이 갖춰진 revision 에만 snapshot 을 만든다.
...
failureModes:
  ...
  - mappedBuiltinSkills 를 executionPlan 으로 취급
...
3. Forge 는 `intent`, `inputs`, `dataSources`, `procedure`, `executionPlan`, `outputs`, `outputSchema`, `criteria`, `forbidden`, `completionCriteria`, `examples`, `mappedBuiltinSkills`, `missingDetails` 를 추출한다.
4. Forge 는 자동 댓글로 구조화 초안을 남긴다. 이 댓글은 공식 승인이나 curated 판정이 아니다.
5. 작성자와 커뮤니티는 Caller Audit 으로 실제 DartLab caller, 입력 파라미터, 출력, 실패 조건을 확인한다.
6. 토론에서 확인한 caller 를 `executionPlan` 으로 작성하고 예시 입력과 기대 출력을 붙인다.
7. maintainer 는 `/market runnable`, `/market curated`, `/market builtin-candidate`, `/market blocked` 명령으로 현재 revision 을 확정할 수 있다.
8. 확정 명령이 있어도 executionPlan 또는 예시가 부족하면 snapshot 을 만들지 않고 `missingDetails` 로 남긴다.
9. 상태 확정 뒤 새 댓글이 달리면 Forge 는 최종 스킬을 즉시 바꾸지 않고 `revisionStatus: pendingReview` 로 표시한다.
10. maintainer 가 revision draft 를 검토하고 다시 상태를 확정하면 `items/{id}.json`, 정적 index, 랜딩 카드가 갱신된다.
...
- `executionPlan` 에 실제 DartLab engine 또는 recipe caller 가 있다.
- 예시 입력과 기대 출력이 있다.
- 입력, 데이터 소스, 출력 스키마, 판단 기준, 금지·실패 조건이 있다.
- maintainer 가 `/market runnable`, `/market curated`, `/market builtin-candidate` 중 하나로 현재 revision 을 확정했다.
- 조건 미달이면 확정 명령은 기록되지만 snapshot 은 생성하지 않는다.
...
| 파일 | 용도 |
|---|---|
| `items/{id}.json` | 승인된 최종 공유스킬 snapshot. detail page 와 AI 실행 절차의 canonical source 다. 조건 미달 draft 에는 생성하지 않는다. |
