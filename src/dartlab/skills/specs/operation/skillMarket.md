...
Skill Market 은 DartLab 의 공식 Skill OS 를 확장하는 별도 커뮤니티 지식층이다. 등록 방식은 manifest 작성이나 PR 제출이 아니라 GitHub Discussions 의 `Skill Market` 카테고리에 분석 질문을 자연어로 쓰는 것이다.
...
1. 사용자는 GitHub Discussions 의 `Skill Market` 카테고리에 분석 질문을 쓴다.
