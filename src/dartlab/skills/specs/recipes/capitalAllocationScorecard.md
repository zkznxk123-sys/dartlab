---
id: recipes.capitalAllocationScorecard
...
2. ROIIC &lt; SGR 이면 capital 비효율 신호 → `recipes.dupontDriver` 로 ROE 동인 점검.
3. M&A 비중 큰 회사 → 인수 후 통합 효율 (goodwill 추적) 별도.
4. 자사주 비중 큰 회사 → 소각 vs 보유 구분, EPS 변화로 검증.
5. SGR &lt; actual growth → 외부 자본조달 의존 (유상증자·차입) 확인.
6. `recipes.compounderCandidates` 와 상호 검증 — 진짜 compounder 는 ROIIC &gt; 15% 일관.
