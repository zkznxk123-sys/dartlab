---
id: recipes.compounderCandidates
...
2. 후보 → `recipes.qualityValueScreen` 의 GP/A 게이트 추가 검증.
3. `recipes.garpScreen` 의 PEG 게이트 — compounder 가 비싸지 않은지 가치 검증.
4. `recipes.distressFilter` 통과 (안전성).
