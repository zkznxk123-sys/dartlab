---
id: recipes.creditCycleStressMap
title: 신용사이클 스트레스 지도
category: recipes
kind: recipe
scope: builtin
...
1. 기업 신용 질문이면 `engines.credit` 또는 `recipes.creditMacroStress`.
2. 역사적 유사 사례가 필요하면 `recipes.historicalPositioning`.
3. 꼬리손실 분포가 필요하면 `recipes.tailRiskScenarioScan`.
