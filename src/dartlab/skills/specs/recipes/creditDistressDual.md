...
linkedSkills:
  ...
  - recipes.earningsQualityTriad
  - recipes.leverageSensitivity
  - engines.credit
toolRefs:
  - EngineCall
  - RunPython
...
3. `recipes.earningsQualityTriad` 와 결합 — 분식 의심 + 부도 위험 = 강한 회피.
4. `recipes.leverageSensitivity` 와 결합 — DCL 큰 회사가 distressed 면 매크로 충격 시 적자 전환 임박.
5. `recipes.distressFilter` 의 횡단 블랙리스트와 본 recipe 의 단일 회사 결과 교차 검증.
