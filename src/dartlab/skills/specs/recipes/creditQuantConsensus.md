---
id: recipes.creditQuantConsensus
...
# 2. Altman Z″ (비제조업 변형) — recipes.creditDistressDual 본문 참조
bs = c.show("BS", freq="Y")
...
2. consensus = TripleAgreement → `recipes.creditMacroStress` 와 결합 — 매크로 충격 시 추가 악화 위험.
3. consensus = DualAgreement → `engines.analysis.earningsQuality` 로 분식 의심 별도 검증.
4. universe 적용은 `recipes.distressCandidateScreen` 와 결합.
