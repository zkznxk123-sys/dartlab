---
id: recipes.dollarFundingStress
...
1. 달러 압력이 유동성 문제라면 `recipes.globalLiquidityPulse` 로 원인을 확인한다.
2. 한국 시장 영향은 `recipes.koreaMacroStressMap` 또는 `recipes.koreaExportCycleNowcast` 로 연결한다.
3. 신용위험이 같이 커지면 `recipes.creditCycleStressMap` 으로 확장한다.
