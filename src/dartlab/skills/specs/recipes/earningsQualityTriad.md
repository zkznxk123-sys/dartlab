---
id: recipes.earningsQualityTriad
...
2. riskScore = 2 (양 모델 의심) → `recipes.workingCapitalQuality` 의 AR/Inv vs Sales gap 점검.
3. 실제 분식 의심 회사 → 공시 (`c.disclosure(...)`) 의 감사보고서 의견·정정공시 빈도 검증.
4. GP/A 강 (상위 10%) + Sloan 약 (하위 30%) = 진짜 quality. `recipes.qualityValueScreen` 와 상호 검증.
5. `recipes.creditDistressDual` 와 결합 — 분식 의심 + 부도 위험 = 강한 회피 신호.
