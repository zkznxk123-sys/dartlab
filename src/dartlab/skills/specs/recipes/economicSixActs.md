...
1. 현재 위치가 불명확하면 `recipes.historicalPositioning` 으로 과거 위기 대비 위치를 비교한다.
2. 신용 취약성이 핵심이면 `recipes.creditCycleStressMap` 으로 이동한다.
3. 꼬리위험 질문이면 `recipes.tailRiskScenarioScan` 으로 이동한다.
