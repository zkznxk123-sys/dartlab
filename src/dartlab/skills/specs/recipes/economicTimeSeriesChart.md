---
id: recipes.economicTimeSeriesChart
title: 경제 원자료 시계열 차트
category: recipes
kind: recipe
scope: builtin
...
linkedSkills:
  ...
  - engines.viz
  - recipes.yieldCurveStress
  - recipes.inflationBreadthWatch
toolRefs:
  - RunPython
  - CompileVisual
requiredEvidence:
...
1. 금리곡선이면 `recipes.yieldCurveStress` 로 해석한다.
2. 물가 계열이면 `recipes.inflationBreadthWatch` 로 확산성을 확인한다.
