...
- **단일 연도 성장률 의존** — Lynch 원전은 5 년 평균 EPS 성장률 사용. 본 recipe 는 YoY 1 년 (분기/연간) 만. 5 년 일관성은 `recipes.compounderCandidates` 로 보완.
...
2. PEG ≤ 1.0 종목 (강한 신호) 에 대해 `recipes.compounderCandidates` 로 5 년 일관성 추가 검증.
3. `recipes.distressFilter` 로 부도 위험 종목 제외 (성장률 통과해도 부채 급증 종목 위험).
