---
id: recipes.incubator.forensics.deepDive
...
sourceRefs:
  - dartlab://skills/recipes.incubator.forensics.deepDive
requiredEvidence:
  - skillRef
  - target
  - tableRef
...
examples:
  ...
  - L1.5 회계 포렌식 전체 실행
audiences:
  llm: 이 스킬은 L2 금지 조건이 핵심이다. 공개 호출 블록 그대로 RunPython을 실행하고 falsifier와 candidate memo를 함께 답한다.
  agent: ReadSkill 결과에서 이 스킬이 상위에 오면 Company.show와 disclosure만 실행하고 L2 capability를 쓰지 않는다.
...
1. recipes.incubator.forensics.dataCoverageAudit - 원표 coverage.
2. recipes.incubator.forensics.accountTraceLedger - 계정 trace.
3. recipes.incubator.forensics.revenueToCashBridge - 매출-현금 bridge.
4. recipes.incubator.forensics.workingCapitalPressureMap - 운전자본 pressure.
5. recipes.incubator.forensics.noteSignalExtractor - 공시 text signal.
6. recipes.incubator.forensics.eventToStatementMatcher - 이벤트 매칭.
7. recipes.incubator.forensics.crossSectionAnomalyRank - scan 후보.
8. recipes.incubator.forensics.falsifierLedger - 반증 ledger.
9. recipes.incubator.forensics.engineCandidateMemo - 엔진 후보 memo.
