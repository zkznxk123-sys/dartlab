---
id: recipes.insiderEarningsLeading
...
2. clusterFlag = True → `recipes.filingTextSignal` 과 결합 — MD&A tone change 동행 검증.
3. universe 검증은 `recipes.distressCandidateScreen` 의 inverse — cluster + low distress = 강한 매수 신호.
