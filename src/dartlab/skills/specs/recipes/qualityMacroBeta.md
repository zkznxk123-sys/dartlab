...
2. phaseAlignment = True → `recipes.industryStageScreen` 으로 같은 산업 stage 후행기 종목 추가 발굴.
3. phaseAlignment = False → 다음 phase 전환까지 대기 (timing 신호).
4. universe 검증은 `recipes.macroQuantScenarioBacktest` 로 backtest.
