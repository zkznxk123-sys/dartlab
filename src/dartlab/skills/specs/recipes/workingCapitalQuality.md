---
id: recipes.workingCapitalQuality
...
2. arVsSalesGap 양수 + 누적 → `recipes.earningsQualityTriad` 의 Sloan accruals 신호 점검.
3. CCC 추세 확장 → 운전자본 자금 부담 증가 → `recipes.distressFilter` 와 상호 검증.
4. DSO·DIO·DPO 동인별 변화 → `recipes.dupontDriver` 의 AssetTurnover 변동과 일치 확인.
