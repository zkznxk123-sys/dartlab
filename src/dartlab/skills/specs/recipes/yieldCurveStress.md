...
1. 금리곡선 역전이 확인되면 `recipes.laborMarketTurningPoint` 로 고용 후행 신호를 확인한다.
2. 금리 상승이 물가 압력 때문이면 `recipes.inflationBreadthWatch` 로 물가 확산성을 확인한다.
3. 정책/유동성 압력은 `recipes.globalLiquidityPulse` 와 함께 본다.
