...
def _deepDiveTable(
    *,
    ...
    blocker_count = sum(1 for gap in gaps if gap.get("status") == "deferredWithBlocker")
    fallback_count = sum(1 for gap in gaps if gap.get("status") == "fallbackAccepted")
    generic_ok = bool(modelFit.get("genericFcffEligible"))
    dcf_status = str(dcf.get("status") or "usable")
    reverse_status = str(reverse.get("status") or "usable")
    market_cap = _number(marketData.get("marketCap"))
    rows = [
        {
            "order": 1,
            "step": "dataAudit",
            "status": "usableWithFallback" if gaps else "usable",
            "evidence": f"{len(panel)} year panel",
            "fallbackCount": fallback_count,
            "blockerCount": blocker_count,
            "nextAction": "resolve deferred blockers before promotion" if blocker_count else "proceed",
        },
        {
            "order": 2,
            "step": "businessModelFit",
            "status": modelFit.get("modelType"),
            "evidence": "generic FCFF eligible" if generic_ok else modelFit.get("fallbackModel"),
            "fallbackCount": 0 if generic_ok else 1,
            "blockerCount": 0 if generic_ok else 1,
            "nextAction": "use FCFF path" if generic_ok else "route to non-generic model",
        },
        {
            "order": 3,
            "step": "lifeCycleClassifier",
            "status": "blocked" if not generic_ok else "usable",
            "evidence": f"growth={_round(assumptions.get('recentGrowthPct'), 2)}%; margin={_round(assumptions.get('normalizedMarginPct'), 2)}%",
            "fallbackCount": 0,
            "blockerCount": 0 if generic_ok else 1,
            "nextAction": "bind phase to growth and margin assumptions",
        },
        {
            "order": 4,
            "step": "narrativeMap",
            "status": "usableWithFallback",
            "evidence": "company name, market, industryKey, normalized drivers",
            "fallbackCount": 1,
            "blockerCount": 0,
            "nextAction": "replace heuristic narrative with richer topic evidence when available",
        },
        {
            "order": 5,
            "step": "storyToDrivers",
            "status": "usableWithFallback",
            "evidence": "growth, margin, reinvestment, risk assumptions",
            "fallbackCount": 1,
            "blockerCount": 0,
            "nextAction": "keep every story claim tied to a numeric driver",
        },
        {
            "order": 6,
            "step": "normalizedFinancials",
            "status": "usable" if len(panel) >= 5 else "usableWithFallback",
            "evidence": f"{len(panel)} normalized rows",
            "fallbackCount": 0 if len(panel) >= 5 else 1,
            "blockerCount": 0,
            "nextAction": "audit missing statement accounts" if len(panel) < 5 else "proceed",
        },
        {
            "order": 7,
            "step": "accountTraceAudit",
            "status": "usable",
            "evidence": "trace keys generated for valuation inputs",
            "fallbackCount": 0,
            "blockerCount": 0,
            "nextAction": "explain every valuation number from source accounts",
        },
        {
            "order": 8,
            "step": "rdCapitalization",
            "status": "fallbackAccepted",
            "evidence": "R&D line-item token scan",
            "fallbackCount": 1,
            "blockerCount": 0,
            "nextAction": "capitalize R&D only when line-item evidence is present",
        },
        {
            "order": 9,
            "step": "leaseDebtAdjustment",
            "status": "fallbackAccepted",
            "evidence": "lease/right-of-use token scan",
            "fallbackCount": 1,
            "blockerCount": 0,
            "nextAction": "adjust debt only when lease evidence is present",
        },
        {
            "order": 10,
            "step": "oneOffAdjustment",
            "status": "fallbackAccepted",
            "evidence": "one-off token scan",
            "fallbackCount": 1,
            "blockerCount": 0,
            "nextAction": "normalize EBIT only when one-off evidence is present",
        },
        {
            "order": 11,
            "step": "reinvestmentRoc",
            "status": "usable" if generic_ok else "blocked",
            "evidence": f"salesToCapital={_round(assumptions.get('salesToCapital'), 2)}; ROC={_round(assumptions.get('rocPct'), 2)}%",
            "fallbackCount": 0,
            "blockerCount": 0 if generic_ok else 1,
            "nextAction": "check growth = reinvestment x ROC",
        },
        {
            "order": 12,
            "step": "growthFeasibility",
            "status": reverse_status if reverse_status == "blocked" else "usable",
            "evidence": f"reverseRequiredGrowth={_round(reverse.get('requiredGrowthPct'), 2)}%",
            "fallbackCount": 0,
            "blockerCount": 1 if reverse_status == "blocked" else 0,
            "nextAction": "treat price-implied story as unverified if reverse DCF is blocked",
        },
        {
            "order": 13,
            "step": "costOfCapital",
            "status": "usableWithFallback",
            "evidence": f"WACC={_round(assumptions.get('waccPct'), 2)}%; COE={_round(assumptions.get('costOfEquityPct'), 2)}%",
            "fallbackCount": 1,
            "blockerCount": 0,
            "nextAction": "refresh Damodaran reference inputs before official promotion",
        },
        {
            "order": 14,
            "step": "fcffDcf",
            "status": dcf_status,
            "evidence": f"baseEquityValue={_round(dcf.get('base', {}).get('equityValue'))}",
            "fallbackCount": 0,
            "blockerCount": 1 if dcf_status == "blocked" else 0,
            "nextAction": "do not report FCFF value when blocked",
        },
        {
            "order": 15,
            "step": "relativeCheck",
            "status": relative.get("status", "usableWithFallback"),
            "evidence": f"marketCap={_round(market_cap)}",
            "fallbackCount": 0 if market_cap else 1,
            "blockerCount": 0,
            "nextAction": "use only as DCF sanity check",
        },
        {
            "order": 16,
            "step": "peerMultipleDecomposition",
            "status": "partial" if relative.get("status") == "partial" else "usableWithFallback",
            "evidence": "multiple driver decomposition",
            "fallbackCount": 1,
            "blockerCount": 0,
            "nextAction": "avoid simple peer average without driver explanation",
        },
        {
            "order": 17,
            "step": "financialFirmExcessReturn",
            "status": "usableWithFallback" if not generic_ok else "notApplicable",
            "evidence": modelFit.get("fallbackModel") or "generic FCFF preferred",
            "fallbackCount": 1 if not generic_ok else 0,
            "blockerCount": 0 if not generic_ok else 0,
            "nextAction": "use financial route for banks/insurers only",
        },
        {
            "order": 18,
            "step": "sumOfParts",
            "status": "singleSegmentFallback",
            "evidence": "reported company aggregate financials",
            "fallbackCount": 1,
            "blockerCount": 0,
            "nextAction": "require segment evidence for real SOTP",
        },
        {
            "order": 19,
            "step": "distressAdjustedDcf",
            "status": "notApplicable" if not generic_ok else "usable",
            "evidence": "debt-to-equity and negative FCFF route",
            "fallbackCount": 0,
            "blockerCount": 0,
            "nextAction": "switch route only when distress indicators trigger",
        },
        {
            "order": 20,
            "step": "scenarioFalsifier",
            "status": reverse_status,
            "evidence": f"plausibility={reverse.get('plausibility')}",
            "fallbackCount": 0,
            "blockerCount": 1 if reverse_status == "blocked" else 0,
            "nextAction": "make conclusion about price-implied story, not point target price",
        },
        {
            "order": 21,
            "step": "finalDecision",
            "status": decisionStatus,
            "evidence": f"gaps={len(gaps)}; blockers={blocker_count}; fallbacks={fallback_count}",
            "fallbackCount": fallback_count,
            "blockerCount": blocker_count,
            "nextAction": "eligible for operator review"
            if not blocker_count
            else "not eligible for official promotion",
        },
    ]
    return rows
