"""Horizon Meaning Learner V102 - suffix row directory nearest-order mass.

V102 실제 기록
--------------
아이디어:
    V102 는 V101 의 남은 비용을 겨냥한다. V101 은 fixed TARGETS suffix prefilter 없이도 현재 route target 의
    local suffix anchor mass 를 사용해 V97 수준의 오염 억제를 회복했다. 다만 flat sampled row 를 suffix key 마다
    반복 scan 하면서 4,000 표본에서 cohortSampleRows=1,719,010 이 발생했고 totalSeconds=77.3 으로 V97/V88 76.4 보다
    약간 느렸다.

    V102 는 V98 처럼 key별 surface/position bucket 을 모두 물리화하지 않는다. signature sampler 가 고른 flat row 는
    유지하되, 각 suffix key 에 대해 row index 목록만 저장하는 compact directory 를 만든다. route-time cohort 생성은
    전체 flat row 가 아니라 `nearestOrderKeyRowIndex[key]` 만 훑는다. 목표는 target 고정 없이 V101 의 신호와
    cache invalidation 을 유지하면서 반복 scan 비용을 줄이는 것이다.
    ...
    나열하는 핵심 표현이라 내용은 유지해야 한다. V102 는 line atom 문자열도 canonical cache 로 재사용해,
...
시도 방법:
    ...
    80. V102 는 V101 의 flat sampled row 와 target-local anchor mass 를 유지한다.
    81. 각 sampled row 가 속한 suffix key 에 row index 만 걸어 route-time cohort 가 전체 flat row 대신 key subset 만 훑게 한다.
    82. atom text, selected occurrence, scoring, acceptance/search gate 는 그대로 두어 변화 원인을 suffix row directory 로 분리한다.

실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV102Test.py

    $env:DARTLAB_HORIZON_V102_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V102_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V102_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V102_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV102Test.py

    $env:DARTLAB_HORIZON_V102_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V102_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V102_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V102_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV102Test.py

판정 기준:
    V101 의 품질과 오염 억제를 유지해야 한다. suffix key -> row index directory 로 route-time 반복 scan 을 줄이되,
    V98 처럼 key별 surface/position bucket 전체를 물리화해 build 비용을 키우면 실패다.

결과:
    1,200 units + direct side payload 600개:
        nearestOrderLazySeed 는 mode=keyRows, surfaces=9,528, positions=77,673, keys=10,121,
        rowLinks=175,872 이었다.
        orderProbe 는 `복구충당금 -> 대손충당금` sim=0.000/cm=0.000/pen=0.152,
        `손실충당금 -> 대손충당금` cm=1.000/pen=0.000 이었다.
        buildSketches=6.0s, buildSignatures=12.7s, buildFocusedRelationIndexes=2.1s,
        buildSideBoundPayloadIndex=2.0s, buildUnitIndex=1.1s, modelSeconds=33.8, totalSeconds=36.0.
        `복구충당금 -> 대손충당금` 은 score 0.918 로 V101 수준을 유지했다.
        `대출채권 -> 매출채권` 은 -0.151 로 유지했고 positiveHits=4/4, badAccepted=0/7,
        searchTop1=5/5, reliableSearch=5/5 를 유지했다.
        cohortSampleRows 는 V101 776,730 에서 3,904 로 줄었다.

    4,000 units + direct side payload 600개:
        nearestOrderLazySeed 는 mode=keyRows, surfaces=14,475, positions=171,901, keys=14,607,
        rowLinks=388,957 이었다.
        orderProbe 는 `복구충당금 -> 대손충당금` sim=0.000/cm=0.000/pen=0.152,
        `손실충당금 -> 대손충당금` cm=1.000/pen=0.000 이었다.
        buildSketches=14.4s, buildSignatures=22.8s, buildFocusedRelationIndexes=6.9s,
        buildSideBoundPayloadIndex=2.0s, buildUnitIndex=3.3s, modelSeconds=71.3, totalSeconds=77.9.
        ...
        lazy stats 는 profileBuild=236, profilePositions=5,014, cohortSampleBuild=10,
        cohortSampleRows=5,850, cohortBuild=10, cohortCommonAtoms=16 이었다.

판정:
    실패/진단 성공. suffix row directory 는 route-time scan row 수를 크게 줄였지만, key rowLinks 388,957 을
    buildSignatures hot path 에 추가로 만드는 비용이 절감보다 크다. 품질과 오염 억제는 유지되지만 totalSeconds 는
    V101 77.3 -> V102 77.9 로 후퇴했다. 다음은 build-time directory 없이 route-time scan 자체를 더 싸게 묶는 방향이다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V102_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V102_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V102_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V102_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V102_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V102_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V102_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V102_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V102_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V102_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V102_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V102_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V102_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V102_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V102_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V102_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V102_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V102_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V102_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V102_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V102_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V102_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
LANE_MISMATCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V102_LANE_MISMATCH_PENALTY", "0.18"))
LANE_ARTIFACT_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V102_LANE_ARTIFACT_PENALTY", "0.10"))
NEAREST_ORDER_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V102_NEAREST_ORDER_SIGNATURE_LIMIT", "24"))
NEAREST_ORDER_PENALTY_MIN = float(os.environ.get("DARTLAB_HORIZON_V102_NEAREST_ORDER_PENALTY_MIN", "0.05"))
NEAREST_ORDER_PENALTY_SCALE = float(os.environ.get("DARTLAB_HORIZON_V102_NEAREST_ORDER_PENALTY_SCALE", "0.16"))
NEAREST_ORDER_COHORT_SURFACE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V102_NEAREST_ORDER_COHORT_SURFACE_LIMIT", "384")
)
NEAREST_ORDER_COHORT_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V102_NEAREST_ORDER_COHORT_POSITION_LIMIT", "8192")
)
NEAREST_ORDER_SURFACE_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V102_NEAREST_ORDER_SURFACE_POSITION_LIMIT", "48")
)
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V102_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V102_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V102_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V102_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V102_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V102_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V102_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V102_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V102_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V102_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V102_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V102_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V102_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V102_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V102_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V102_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V102_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V102_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V102_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V102_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V102_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V102_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V102_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V102_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V102_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V102_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V102_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V102_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V102_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V102_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V102_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V102_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V102_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V102_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V102_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
RELAY_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V102_RELAY_ROW_LIMIT", "160"))
RELAY_SPECIFIC_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V102_RELAY_SPECIFIC_ROW_LIMIT", "320"))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V102_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V102_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V102_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V102_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V102_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V102_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V102_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V102_SKETCH_OCC_BUCKETS", "16"))
...
class Model:
    ...
    nearestOrderKeyRowIndex: dict[str, tuple[int, ...]]
...
def buildSignatures(
    ...
    nearestOrderKeyRowBuckets: dict[str, list[int]] = defaultdict(list)
    ...
    for cache, occ in sampledOccs:
        ...
        keys = suffixCohortKeys(occ.surface)
        if keys:
            rowIndex = len(nearestOrderSampleRows)
            nearestOrderSampleRows.append((occ.surface, cache.unit.unitId, occ.position))
            for key in keys:
                nearestOrderKeyRowBuckets[key].append(rowIndex)
    ...
    print(
        f"[nearestOrderLazySeed] mode=keyRows surfaces={len(nearestOrderSurfaces)} "
        f"positions={len(nearestOrderSampleRows)} keys={len(nearestOrderKeyRowBuckets)} "
        f"rowLinks={sum(len(rows) for rows in nearestOrderKeyRowBuckets.values())}"
    )
    nearestOrderKeyRowIndex = {key: tuple(rows) for key, rows in nearestOrderKeyRowBuckets.items()}
    return signatures, postings, lineTokenViews, {}, tuple(nearestOrderSampleRows), nearestOrderKeyRowIndex
...
def ensureNearestOrderCohort(key: str, model: Model) -> None:
    ...
    keptPositions = 0
    scannedRows = 0
    for rowIndex in model.nearestOrderKeyRowIndex.get(key, ()):
        scannedRows += 1
        surface, unitId, position = model.nearestOrderSampleRows[rowIndex]
