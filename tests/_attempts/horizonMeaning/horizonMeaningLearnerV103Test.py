"""Horizon Meaning Learner V103 - batched lazy nearest-order cohorts.

V103 실제 기록
--------------
아이디어:
    V103 은 V102 의 실패 원인을 반영한다. V102 는 suffix key -> row index directory 로 V101 의 반복 flat-row scan 을
    줄였지만, key directory 를 build-time 에 만드는 비용이 route-time 절감보다 커서 4,000 totalSeconds 가 77.9 로
    후퇴했다.

    V103 은 V101 의 flat sampled row 구조로 돌아가되, route-time 에 여러 suffix key 를 하나씩 ensure 하지 않는다.
    현재 비교에 필요한 unseen key 묶음을 한 번의 flat-row pass 로 batch 생성한다. 즉 추가 인덱스는 만들지 않고,
    `shared suffix + target-local suffix` cohort 들을 동시에 채워 V101 의 target-anchor nearest-order 신호와 cache
    invalidation 은 유지하면서 `cohortSampleRows` 반복 scan 을 줄인다.
    ...
    나열하는 핵심 표현이라 내용은 유지해야 한다. V103 는 line atom 문자열도 canonical cache 로 재사용해,
...
시도 방법:
    ...
    80. V103 은 V101 의 flat sampled row 와 target-local anchor mass 를 유지한다.
    81. route-time 에 필요한 unseen suffix key 묶음을 한 번의 flat-row pass 로 batch 생성해 key별 반복 scan 을 줄인다.
    82. atom text, selected occurrence, scoring, acceptance/search gate 는 그대로 두어 변화 원인을 batched lazy cohort build 로 분리한다.

실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV103Test.py

    $env:DARTLAB_HORIZON_V103_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V103_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V103_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V103_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV103Test.py

    $env:DARTLAB_HORIZON_V103_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V103_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V103_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V103_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV103Test.py

판정 기준:
    V101 의 품질과 오염 억제를 유지해야 한다. build-time row directory 없이 여러 unseen suffix key 를 한 번의
    flat-row pass 로 채워 V101 의 반복 scan 비용을 낮춰야 한다.

결과:
    1,200 units + direct side payload 600개:
        nearestOrderLazySeed 는 mode=flatSamples, surfaces=9,528, positions=77,673 이었다.
        orderProbe 는 `복구충당금 -> 대손충당금` sim=0.000/cm=1.000/pen=0.152,
        `손실충당금 -> 대손충당금` cm=1.000/pen=0.000 이었다.
        buildSketches=6.1s, buildSignatures=12.3s, buildFocusedRelationIndexes=2.1s,
        buildSideBoundPayloadIndex=1.8s, buildUnitIndex=1.1s, modelSeconds=32.9, totalSeconds=35.4.
        `복구충당금 -> 대손충당금` 은 score 0.917 로 V101 수준을 유지했다.
        `대출채권 -> 매출채권` 은 -0.151 로 유지했고 positiveHits=4/4, badAccepted=0/7,
        searchTop1=5/5, reliableSearch=5/5 를 유지했다.
        lazy stats 는 cohortBatchBuild=7, cohortBatchKeys=14, cohortSampleRows=543,711 이었다.

    4,000 units + direct side payload 600개:
        nearestOrderLazySeed 는 mode=flatSamples, surfaces=14,475, positions=171,901 이었다.
        orderProbe 는 `복구충당금 -> 대손충당금` sim=0.000/cm=1.000/pen=0.152,
        `손실충당금 -> 대손충당금` cm=1.000/pen=0.000 이었다.
        buildSketches=13.9s, buildSignatures=22.2s, buildFocusedRelationIndexes=6.5s,
        buildSideBoundPayloadIndex=2.0s, buildUnitIndex=3.9s, modelSeconds=70.5, totalSeconds=77.5.
        `복구충당금 -> 대손충당금` 은 score 0.025 로 V88 0.177, V93 0.105 보다 낮아졌다.
        `대출채권 -> 대손충당금` -0.134, `대출채권 -> 매출채권` -0.124 유지.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5.
        lazy stats 는 cohortBatchBuild=7, cohortBatchKeys=14, cohortSampleRows=1,203,307,
        profileBuild=236, profilePositions=5,014, cohortBuild=10, cohortCommonAtoms=16 이었다.

판정:
    실패/부분 진단. build-time directory 없이 cohortSampleRows 를 V101 1,719,010 -> 1,203,307 로 줄였고 1,200 은
    35.4 로 V101 35.6 보다 빨랐다. 그러나 4,000 은 batch surface map 구성 비용 때문에 V101 77.3 -> V103 77.5 로
    후퇴했다. 다음은 여러 key 를 묶는 map 이 아니라 surface 단위 row lookup 을 lazy 로 한 번 만드는 방향이다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V103_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V103_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V103_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V103_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V103_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V103_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V103_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V103_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V103_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V103_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V103_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V103_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V103_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V103_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V103_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V103_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V103_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V103_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V103_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V103_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V103_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V103_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
LANE_MISMATCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V103_LANE_MISMATCH_PENALTY", "0.18"))
LANE_ARTIFACT_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V103_LANE_ARTIFACT_PENALTY", "0.10"))
NEAREST_ORDER_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V103_NEAREST_ORDER_SIGNATURE_LIMIT", "24"))
NEAREST_ORDER_PENALTY_MIN = float(os.environ.get("DARTLAB_HORIZON_V103_NEAREST_ORDER_PENALTY_MIN", "0.05"))
NEAREST_ORDER_PENALTY_SCALE = float(os.environ.get("DARTLAB_HORIZON_V103_NEAREST_ORDER_PENALTY_SCALE", "0.16"))
NEAREST_ORDER_COHORT_SURFACE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V103_NEAREST_ORDER_COHORT_SURFACE_LIMIT", "384")
)
NEAREST_ORDER_COHORT_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V103_NEAREST_ORDER_COHORT_POSITION_LIMIT", "8192")
)
NEAREST_ORDER_SURFACE_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V103_NEAREST_ORDER_SURFACE_POSITION_LIMIT", "48")
)
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V103_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V103_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V103_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V103_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V103_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V103_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V103_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V103_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V103_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V103_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V103_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V103_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V103_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V103_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V103_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V103_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V103_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V103_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V103_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V103_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V103_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V103_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V103_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V103_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V103_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V103_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V103_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V103_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V103_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V103_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V103_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V103_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V103_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V103_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V103_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
RELAY_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V103_RELAY_ROW_LIMIT", "160"))
RELAY_SPECIFIC_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V103_RELAY_SPECIFIC_ROW_LIMIT", "320"))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V103_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V103_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V103_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V103_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V103_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V103_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V103_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V103_SKETCH_OCC_BUCKETS", "16"))
...
def buildSignatures(
    ...
    sampled = time.perf_counter()
    nearestOrderSampleRows: list[tuple[str, int, int]] = []
    nearestOrderSurfaces: set[str] = set()
    for cache, occ in sampledOccs:
        ...
        if suffixCohortKeys(occ.surface):
            nearestOrderSampleRows.append((occ.surface, cache.unit.unitId, occ.position))
            nearestOrderSurfaces.add(occ.surface)
    for surface, counter in raw.items():
        for atom in coordAtoms(surface):
            counter[atom] += 1
    rawBuilt = time.perf_counter()
    ...
    print(
        f"[nearestOrderLazySeed] mode=flatSamples surfaces={len(nearestOrderSurfaces)} "
        f"positions={len(nearestOrderSampleRows)}"
    )
    return signatures, postings, lineTokenViews, {}, tuple(nearestOrderSampleRows)
...
def ensureNearestOrderCohorts(keys: tuple[str, ...], model: Model) -> None:
    pending = tuple(dict.fromkeys(key for key in keys if key not in model.nearestOrderCohortSurfaces))
    if not pending:
        return
    started = time.perf_counter()
    rowsByKeySurface: dict[str, dict[str, list[tuple[int, int]]]] = {}
    keptPositionsByKey: Counter[str] = Counter()
    surfaceToKeys: dict[str, list[str]] = defaultdict(list)
    for key in pending:
        ...
        rowsByKeySurface[key] = {surface: [] for surface in surfaces}
        for surface in surfaces:
            surfaceToKeys[surface].append(key)

    scannedRows = 0
    for surface, unitId, position in model.nearestOrderSampleRows:
        scannedRows += 1
        rowKeys = surfaceToKeys.get(surface)
        if not rowKeys:
            continue
        for key in rowKeys:
            if keptPositionsByKey[key] >= NEAREST_ORDER_COHORT_POSITION_LIMIT:
                continue
            rows = rowsByKeySurface[key][surface]
            if len(rows) >= NEAREST_ORDER_SURFACE_POSITION_LIMIT:
                continue
            rows.append((unitId, position))
            keptPositionsByKey[key] += 1

    totalKeptSurfaces = 0
    totalKeptPositions = 0
    for key, rowsBySurface in rowsByKeySurface.items():
        ...
        totalKeptSurfaces += len(keptSurfaces)
        totalKeptPositions += keptPositionsByKey[key]

    model.nearestOrderStats["cohortBatchBuild"] += 1
    model.nearestOrderStats["cohortBatchKeys"] += len(pending)
    model.nearestOrderStats["cohortSampleBuild"] += len(pending)
    model.nearestOrderStats["cohortSampleSurfaces"] += totalKeptSurfaces
    model.nearestOrderStats["cohortSamplePositions"] += totalKeptPositions
...
def ensureNearestOrderCohort(key: str, model: Model) -> None:
    ensureNearestOrderCohorts((key,), model)
...
def nearestOrderCommonRatio(surface: str, atom: str, model: Model) -> float:
    ratios: list[float] = []
    keys = suffixCohortKeys(surface)
    ensureNearestOrderCohorts(keys, model)
    for key in keys:
...
def nearestOrderGatePenalty(surface: str, target: str, sameSuffix: bool, resonance: float, model: Model) -> float:
    if not sameSuffix or resonance >= RESONANCE_ACCEPT_MIN:
        return 0.0
    sharedKeys = sharedSuffixCohortKeys(surface, target)
    targetKeys = suffixCohortKeys(target)
    gateKeys = mergedSuffixKeys(sharedKeys, targetKeys)
    if not gateKeys:
        return 0.0
    ensureNearestOrderCohorts(gateKeys, model)
    signal = max(
        nearestOrderSimilarity(surface, target, model),
        nearestOrderCommonMassForKeys(surface, sharedKeys, model),
        nearestOrderCommonMassForKeys(target, gateKeys, model),
    )
