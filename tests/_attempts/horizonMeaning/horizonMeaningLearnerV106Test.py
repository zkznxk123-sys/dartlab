"""Horizon Meaning Learner V106 - target-local suffix support gate.

V106 실제 기록
--------------
아이디어:
    V106 은 V105 의 결론을 더 거칠게 검증한다. V105 는 nearest-order profile/cohort 호출 수를 크게 줄였지만
    wall-clock 은 V101 보다 빨라지지 않았다. 더 중요한 질문은 V105 의 성공 신호가 정말 앞뒤 순서 경험 공통성에서
    나왔는지, 아니면 현재 target 이 query surface 와 공유하지 않는 local suffix cohort 를 갖는다는 압축 통계만으로
    충분했는지다.

    V106 은 nearest-order sampled row, lazy profile, lazy cohort build 를 route gate 에서 쓰지 않는다.
    same-suffix + low-resonance 후보일 때 현재 target 의 local suffix key 가 기존 suffix cohort surface count 에
    존재하면 anchor signal=1.0 으로 보고 V105 와 같은 penalty scale 을 적용한다. 즉 "대손충당금" 의
    `손충당금` 같은 target-local suffix anchor 가 corpus 안에 표면 경험을 가졌는지만 본다.

    이 실험은 의미 그래프를 포기하자는 뜻이 아니다. 오히려 nearest-order gate 의 효과가 실제 순서 경험인지,
    또는 훨씬 싼 suffix support 통계로도 대체 가능한지 분리해 다음 구조 투입 위치를 결정하기 위한 진단이다.
...
시도 방법:
    ...
    82. V106 은 nearest-order sampled row/profile/cohort 를 만들지 않고, target-local suffix key 의 cohort support 만
        penalty anchor 로 쓴다.
    83. atom text, selected occurrence, scoring, acceptance/search gate 는 그대로 두어 변화 원인을 suffix support
        compression 으로 분리한다.

실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV106Test.py

    $env:DARTLAB_HORIZON_V106_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V106_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V106_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V106_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV106Test.py

    $env:DARTLAB_HORIZON_V106_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V106_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V106_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V106_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV106Test.py

판정 기준:
    V105 와 같은 품질 기준과 오염 억제를 유지해야 한다. nearest-order sampled row/profile/cohort 를 만들지 않고도
    `복구충당금 -> 대손충당금` 같은 same-suffix/no-resonance 오염을 낮추면, V105 의 order graph gate 는 더 싼
    suffix support 통계로 대체 가능하다는 뜻이다. 품질이 깨지면 target-local suffix support 만으로는 부족하고
    nearest-order 경험 공통성이 실제로 필요하다는 뜻이다. wall-clock 은 V105 보다 좋아야 구조 승격이다.

결과:
    1,200 units + direct side payload 600개:
        nearestOrderLazySeed 는 mode=disabled, suffixAnchorSurfaces=9,528, sampleRows=0 이었다.
        orderProbe 는 `복구충당금 -> 대손충당금` support=1.000/pen=0.152,
        `손실충당금 -> 대손충당금` support=1.000/pen=0.000,
        `대출채권 -> 매출채권` support=1.000/pen=0.152,
        `외상매출금 -> 매출채권` support=1.000/pen=0.000 이었다.
        buildSketches=6.0s, buildSignatures=12.2s, buildFocusedRelationIndexes=2.2s,
        buildSideBoundPayloadIndex=1.9s, buildUnitIndex=1.2s, modelSeconds=33.1, totalSeconds=35.3.
        `복구충당금 -> 대손충당금` 은 score 0.917 로 accepted=False 를 유지했다.
        `대출채권 -> 매출채권` 은 -0.286 으로 V105 보다 더 강하게 낮아졌고,
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5 를 유지했다.
        suffix support stats 는 calls=16, keys=22, hits=16, supportSurfaces=227 이었다.

    4,000 units + direct side payload 600개:
        nearestOrderLazySeed 는 mode=disabled, suffixAnchorSurfaces=14,475, sampleRows=0 이었다.
        orderProbe 는 `복구충당금 -> 대손충당금` support=1.000/pen=0.152,
        `손실충당금 -> 대손충당금` support=1.000/pen=0.000,
        `대출채권 -> 매출채권` support=1.000/pen=0.152,
        `외상매출금 -> 매출채권` support=1.000/pen=0.000 이었다.
        buildSketches=14.0s, buildSignatures=22.3s, buildFocusedRelationIndexes=6.3s,
        buildSideBoundPayloadIndex=2.0s, buildUnitIndex=4.0s, modelSeconds=71.5, totalSeconds=78.3.
        `복구충당금 -> 대손충당금` 은 score 0.025 로 V105 와 같은 억제 수준을 유지했다.
        `대출채권 -> 대손충당금` -0.134, `대출채권 -> 매출채권` -0.276 이었고,
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5 를 유지했다.
        suffix support stats 는 calls=16, keys=22, hits=16, supportSurfaces=269 이었다.

판정:
    부분 성공/진단 성공. nearest-order sampled row/profile/cohort 를 전부 쓰지 않아도 현재 probe set 의
    same-suffix/no-resonance 오염 억제는 target-local suffix support 만으로 재현됐다. 즉 V105 의 테스트된
    penalty 효과는 "순서 경험 공통성" 이 아니라 훨씬 싼 target-local suffix anchor 존재 여부만으로도 설명된다.
    그러나 totalSeconds 는 V105 77.6s 보다 느린 78.3s 로, route lazy gate 제거가 wall-clock 병목을 해결하지
    못했다. 이 gate 는 본진에 넣더라도 negative guard 로만 다루고, 의미 학습의 핵심 개선은 buildSignatures,
    focusedRelation, 또는 suffix support 로 설명되지 않는 더 넓은 의미 probe 에서 찾아야 한다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V106_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V106_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V106_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V106_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V106_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V106_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V106_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V106_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V106_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V106_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V106_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V106_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V106_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V106_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V106_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V106_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V106_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V106_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V106_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V106_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V106_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V106_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
LANE_MISMATCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V106_LANE_MISMATCH_PENALTY", "0.18"))
LANE_ARTIFACT_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V106_LANE_ARTIFACT_PENALTY", "0.10"))
NEAREST_ORDER_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V106_NEAREST_ORDER_SIGNATURE_LIMIT", "24"))
NEAREST_ORDER_PENALTY_MIN = float(os.environ.get("DARTLAB_HORIZON_V106_NEAREST_ORDER_PENALTY_MIN", "0.05"))
NEAREST_ORDER_PENALTY_SCALE = float(os.environ.get("DARTLAB_HORIZON_V106_NEAREST_ORDER_PENALTY_SCALE", "0.16"))
NEAREST_ORDER_COHORT_SURFACE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V106_NEAREST_ORDER_COHORT_SURFACE_LIMIT", "384")
)
NEAREST_ORDER_COHORT_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V106_NEAREST_ORDER_COHORT_POSITION_LIMIT", "8192")
)
NEAREST_ORDER_SURFACE_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V106_NEAREST_ORDER_SURFACE_POSITION_LIMIT", "48")
)
SUFFIX_ANCHOR_SUPPORT_MIN = int(os.environ.get("DARTLAB_HORIZON_V106_SUFFIX_ANCHOR_SUPPORT_MIN", "1"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V106_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V106_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V106_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V106_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V106_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V106_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V106_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V106_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V106_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V106_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V106_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V106_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V106_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V106_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V106_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V106_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V106_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V106_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V106_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V106_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V106_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V106_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V106_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V106_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V106_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V106_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V106_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V106_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V106_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V106_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V106_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V106_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V106_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V106_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V106_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
RELAY_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V106_RELAY_ROW_LIMIT", "160"))
RELAY_SPECIFIC_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V106_RELAY_SPECIFIC_ROW_LIMIT", "320"))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V106_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V106_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V106_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V106_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V106_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V106_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V106_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V106_SKETCH_OCC_BUCKETS", "16"))
