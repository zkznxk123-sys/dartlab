"""Horizon Meaning Learner V107 - dynamic route candidate audit.

V107 실제 기록
--------------
아이디어:
    V106 은 현재 probe set 의 same-suffix/no-resonance 오염 억제가 nearest-order 경험 공통성이 아니라
    target-local suffix support 만으로도 재현됨을 보였다. 따라서 다음 병목은 gate 미세조정이 아니라
    "정답 target 을 미리 TARGETS 에 넣어 둔 상태에서만 의미 검색이 되는가" 를 검증하는 것이다.

    V107 은 기존 TARGETS route 와 별도로 `dynamicRoute()` 를 추가한다. query surface 의 coordinate atom posting,
    compound gram posting, 그리고 surface signature 의 top meaning atom posting 으로 corpus surface 후보를 뽑고,
    같은 route scoring 을 그 후보군에 적용한다.
    즉 target 후보군을 고정된 7개 label 에서 시작하지 않고 역인덱스가 직접 만든 surface shortlist 로 시작한다.
    target alias map, 수작업 family lock, 정답별 예외는 넣지 않는다.

    dynamic 후보에는 rawBridge pseudo surface 조각도 섞이므로, query 자체의 strict substring 은 후보에서 제외하고
    더 긴 후보 안에 포함된 fragment surface 는 일반 penalty 로 낮춘다. 이는 정답별 규칙이 아니라 incomplete
    surface 가 완성 계정명을 이기는 문제를 막는 surface canonicalization 이다.

    성공하면 경험 그래프가 "미리 지정한 label 분류기" 를 넘어 corpus surface 후보 생성에도 쓸 수 있다는 뜻이다.
    실패하면 현재 구조의 의미성은 TARGETS 후보 집합에 강하게 의존하고 있으며, 다음은 dynamic candidate generation 에
    경험 atom posting 또는 relation-bound posting 을 결합해야 한다는 결론이 된다.
...
시도 방법:
    ...
    84. query strict substring 과 longer surface 안의 fragment 후보를 generic canonicalization 으로 낮춘다.
    85. dynamic route 의 top1/top5 를 기존 positive/negative probe 로 기록해, 현 구조가 label 후보 고정에 얼마나
        의존하는지 분리한다.

실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV107Test.py

    $env:DARTLAB_HORIZON_V107_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V107_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V107_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V107_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV107Test.py

    $env:DARTLAB_HORIZON_V107_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V107_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V107_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V107_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV107Test.py

판정 기준:
    기존 fixed TARGETS route 의 품질은 V106 수준을 유지해야 한다. dynamicRoute 는 정답 target 을 후보군에 미리
    넣지 않고도 positive probe 의 expected target 을 top1 또는 적어도 top5 안에 올려야 한다. negative probe 에서는
    forbidden target 이 top1 으로 accepted 되면 안 된다. 실패하면 현 구조의 의미성은 route 후보 생성보다 fixed
    TARGETS 분류에 더 의존한다는 판정이다.

결과:
    1,200 units + direct side payload 600개:
        dynamicMeaningPostings 는 keys=169,289, rawLinks=387,238, keptLinks=387,238, maxRow=209 이었다.
        fixed route 는 positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5 를 유지했다.
        dynamicRoute 는 positive dynamicTop1=0/4, dynamicTop5=0/4, dynamicBadTop1=0/7, dynamicBadTop5=0/7 이었다.
        positive rank 는 `외상매출금 -> 매출채권` 33, `영업손익 -> 영업이익` NA,
        `현금성자산 -> 현금및현금성자산` 8, `손실충당금 -> 대손충당금` 38 이었다.
        top 후보는 `매출금액`, `당기누계실`, `금및현금성자`, `신용손실충당` 같은 fragment/boilerplate/near-surface 로
        밀렸다. modelSeconds=34.6, totalSeconds=37.9.

    4,000 units + direct side payload 600개:
        dynamicMeaningPostings 는 keys=253,278, rawLinks=582,712, keptLinks=582,661, prunedRows=1, maxRow=220 이었다.
        fixed route 는 positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5 를 유지했다.
        dynamicRoute 는 positive dynamicTop1=1/4, dynamicTop5=1/4, dynamicBadTop1=0/7, dynamicBadTop5=0/7 이었다.
        `외상매출금 -> 매출채권` 은 dynamic rank 1 로 성공했다.
        `영업손익 -> 영업이익` 은 rank=NA 이고 `타영업손익`, `영업외손익` 이 앞섰다.
        `현금성자산 -> 현금및현금성자산` 은 rank=8 이고 `금및현금성자`, `현금및현금성자` 같은 fragment 가 앞섰다.
        `손실충당금 -> 대손충당금` 은 rank=51 이고 `측정하도록`, `기대신용손실` 같은 경험/문장 조각이 앞섰다.
        modelSeconds=70.8, totalSeconds=78.7.

판정:
    실패/진단 성공. fixed TARGETS route 는 여전히 품질을 유지하지만, 역인덱스가 직접 만든 dynamic candidate 에서는
    4,000 표본에서도 positiveTop1=1/4, top5=1/4 에 그쳤다. meaning atom posting 과 fragment canonicalization 을
    추가해도 완성 계정명보다 pseudo surface fragment, boilerplate, 가까운 표면명이 앞선다. 따라서 현재 의미성은
    "좋은 scorer" 보다 "좋은 target 후보 집합" 에 크게 의존한다. 다음 구조는 scorer 미세조정이 아니라
    candidate generation 단계에서 pseudo surface 와 독립 관측 surface 를 분리하고, relation-bound evidence 를
    candidate shortlist 에 직접 결합하는 방향이어야 한다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V107_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V107_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V107_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V107_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V107_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V107_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V107_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V107_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V107_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V107_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V107_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V107_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V107_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V107_SEARCH_CANDIDATE_LIMIT", "420"))
DYNAMIC_TARGET_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V107_DYNAMIC_TARGET_LIMIT", "80"))
DYNAMIC_COORD_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V107_DYNAMIC_COORD_ROW_LIMIT", "220"))
DYNAMIC_COMPOUND_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V107_DYNAMIC_COMPOUND_ROW_LIMIT", "260"))
DYNAMIC_QUERY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V107_DYNAMIC_QUERY_ATOM_LIMIT", "48"))
DYNAMIC_MEANING_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V107_DYNAMIC_MEANING_ATOM_LIMIT", "36"))
DYNAMIC_MEANING_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V107_DYNAMIC_MEANING_ROW_LIMIT", "220"))
DYNAMIC_FRAGMENT_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V107_DYNAMIC_FRAGMENT_PENALTY", "0.55"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V107_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V107_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V107_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V107_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V107_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V107_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V107_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V107_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
LANE_MISMATCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V107_LANE_MISMATCH_PENALTY", "0.18"))
LANE_ARTIFACT_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V107_LANE_ARTIFACT_PENALTY", "0.10"))
NEAREST_ORDER_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V107_NEAREST_ORDER_SIGNATURE_LIMIT", "24"))
NEAREST_ORDER_PENALTY_MIN = float(os.environ.get("DARTLAB_HORIZON_V107_NEAREST_ORDER_PENALTY_MIN", "0.05"))
NEAREST_ORDER_PENALTY_SCALE = float(os.environ.get("DARTLAB_HORIZON_V107_NEAREST_ORDER_PENALTY_SCALE", "0.16"))
NEAREST_ORDER_COHORT_SURFACE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V107_NEAREST_ORDER_COHORT_SURFACE_LIMIT", "384")
)
NEAREST_ORDER_COHORT_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V107_NEAREST_ORDER_COHORT_POSITION_LIMIT", "8192")
)
NEAREST_ORDER_SURFACE_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V107_NEAREST_ORDER_SURFACE_POSITION_LIMIT", "48")
)
SUFFIX_ANCHOR_SUPPORT_MIN = int(os.environ.get("DARTLAB_HORIZON_V107_SUFFIX_ANCHOR_SUPPORT_MIN", "1"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V107_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V107_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V107_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V107_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V107_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V107_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V107_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V107_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V107_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V107_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V107_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V107_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V107_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V107_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V107_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V107_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V107_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V107_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V107_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V107_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V107_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V107_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V107_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V107_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V107_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V107_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V107_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V107_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V107_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V107_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V107_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V107_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V107_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V107_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V107_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
RELAY_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V107_RELAY_ROW_LIMIT", "160"))
RELAY_SPECIFIC_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V107_RELAY_SPECIFIC_ROW_LIMIT", "320"))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V107_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V107_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V107_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V107_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V107_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V107_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V107_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V107_SKETCH_OCC_BUCKETS", "16"))
...
def dynamicRouteCandidates(surface: str, model: Model) -> tuple[str, ...]:
    ...
    for atom, weight in query.most_common(DYNAMIC_QUERY_ATOM_LIMIT):
        ...
        for rank, other in enumerate(model.meaningPostings.get(atom, ())[:DYNAMIC_MEANING_ROW_LIMIT]):
            ...
            if otherStem == stem or not isContentStem(otherStem):
                continue
            if otherStem in stem and len(otherStem) < len(stem):
                continue
            scores[otherStem] += (
    ...
    for atom in coordAtoms(stem):
        ...
        for rank, other in enumerate(model.coordPostings.get(atom, ())[:DYNAMIC_COORD_ROW_LIMIT]):
            ...
            if otherStem in stem and len(otherStem) < len(stem):
                continue
            scores[otherStem] += weight * (1.0 - min(rank, DYNAMIC_COORD_ROW_LIMIT) / (DYNAMIC_COORD_ROW_LIMIT + 1.0))
    for gram in compoundGrams(stem):
        for rank, other in enumerate(model.compoundGramPostings.get(gram, ())[:DYNAMIC_COMPOUND_ROW_LIMIT]):
            ...
            if similarity <= 0:
                continue
            scores[otherStem] += similarity * (
                2.0 - min(rank, DYNAMIC_COMPOUND_ROW_LIMIT) / (DYNAMIC_COMPOUND_ROW_LIMIT + 1.0)
            )
    return tuple(target for target, _ in scores.most_common(DYNAMIC_TARGET_LIMIT))


def dynamicFragmentPenalty(target: str, candidates: tuple[str, ...]) -> float:
    value = normStem(target)
    if len(value) < 4:
        return 0.0
    for other in candidates:
        otherValue = normStem(other)
        if value == otherValue or len(otherValue) <= len(value):
            continue
        if value in otherValue and compoundSimilarity(value, otherValue) >= 0.28:
            return DYNAMIC_FRAGMENT_PENALTY
    return 0.0
...
def dynamicRoute(surface: str, model: Model):
    ...
    for target in candidates:
        ...
        penalty = dynamicFragmentPenalty(target, candidates)
        if penalty > 0:
            score -= penalty
