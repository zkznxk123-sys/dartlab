"""Horizon Meaning Learner V109 - owner-role dynamic candidates.

V109 실제 기록
--------------
아이디어:
    V108 은 pseudo-only fragment 를 dynamic 후보에서 제거해 4,000 표본 dynamicTop1=2/4, top5=3/4 까지
    올렸다. 그러나 `손실충당금 -> 대손충당금` 은 rank 5 에 머물렀고, `영업손익 -> 영업이익` 은 rank 49 로
    실패했다. 남은 문제는 독립 관측 surface 안에서도 계정명/측정대상 surface 와 설명/행위/문장 surface 가
    섞인다는 점이다.

    V109 는 relation-bound evidence 가 강한 독립 surface 만 다시 owner-role sketch 로 만든다. 각 surface 가
    relation owner 로 잡힌 unit 의 sparse experience atom 과 relation atom 을 `ownerRoleSignatures` 로 압축하고,
    atom -> surface posting 을 `ownerRolePostings` 로 만든다. dynamic 후보 생성은 이 owner-role posting 을
    추가로 조회하되, `relationOwnerCandidate()` 를 통과한 surface 만 허용한다. route 보너스도 무조건 주지 않고,
    coordinate/compound/resonance 중 하나라도 있는 후보에만 적용하며 same-suffix/no-resonance 오염에는 적용하지 않는다.

    성공하면 V108 의 fragment 제거 다음 단계로, 경험 그래프가 "완성 surface" 를 넘어 "역할이 맞는 계정 surface" 를
    더 잘 골라낸다는 증거가 된다. 실패하면 owner-role unit signature 가 너무 일반적이거나 literal-near 후보를
    분리할 role atom 이 아직 부족하다는 판정이다.
...
실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV109Test.py

    $env:DARTLAB_HORIZON_V109_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V109_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V109_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V109_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV109Test.py

    $env:DARTLAB_HORIZON_V109_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V109_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V109_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V109_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV109Test.py

판정 기준:
    기존 fixed TARGETS route 의 품질은 V106/V108 수준을 유지해야 한다. dynamicRoute 는 정답 target 을 후보군에
    미리 넣지 않고도 positive probe 의 expected target 을 top1 또는 적어도 top5 안에 올려야 한다. negative probe 에서는
    forbidden target 이 top1 또는 top5 로 올라오면 안 된다. owner-role sketch 가 positive rank 를 올리면서
    `대출채권 -> 매출채권`, `복구충당금 -> 대손충당금`, `대출채권 -> 대손충당금` 오염을 다시 열지 않는지가 핵심이다.

결과:
    1,200 units + direct side payload 600개:
        surfaceOrigin 은 independent=7,045, bridge=4,225, pseudoOnly=3,857 이었고,
        ownerRole 은 surfaces=196, boundRows=1,128, unitAtomLinks=40,600, atoms=2,978,
        rawLinks=9,764, keptLinks=9,764 이었다.
        ...
        `영업손익` 은 V108 1,200 rank 10 -> 7, `손실충당금` 은 rank 4 -> 3 으로 개선됐지만,
        `복구충당금 -/-> 대손충당금` 은 여전히 forbiddenRank=3 이라 dynamicBadTop5=1/7 을 유지했다.
        modelSeconds=35.8, totalSeconds=39.3.

    4,000 units + direct side payload 600개:
        surfaceOrigin 은 independent=11,448, bridge=5,599, pseudoOnly=4,938 이었고,
        ownerRole 은 surfaces=323, boundRows=3,993, unitAtomLinks=143,672, atoms=4,451,
        rawLinks=17,017, keptLinks=17,010, prunedRows=1 이었다.
        dynamicMeaningPostings 는 keys=253,317, rawLinks=582,788, keptLinks=582,737, prunedRows=1, maxRow=220 이었다.
        fixed route 는 positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5 를 유지했다.
        dynamicRoute 는 positive dynamicTop1=3/4, dynamicTop5=3/4, dynamicBadTop1=0/7, dynamicBadTop5=0/7 이었다.
        `외상매출금 -> 매출채권` 은 dynamic rank 1 로 성공했고,
        `현금성자산 -> 현금및현금성자산` 도 rank 1 로 유지됐다.
        `손실충당금 -> 대손충당금` 은 V108 rank 5 에서 rank 1 로 올라왔다.
        `영업손익 -> 영업이익` 은 V108 rank 49 에서 rank 45 로 소폭 개선됐지만 여전히 실패했고,
        `영업외손익`, `단기차입금의차입`, `손익누계액` 같은 surface 가 앞섰다.
        dynamic negative 는 forbiddenRank 가 `대출채권 -> 매출채권` 64, `현금배당금 -> 현금및현금성자산` 45,
        `당기순이익 -> 영업이익` 54, `복구충당금 -> 대손충당금` 19,
        `대출채권 -> 대손충당금` 55, 나머지 2개 NA 로 top5 오염은 없었다.
        modelSeconds=72.1, totalSeconds=80.0.

판정:
    성공/부분 구조 개선. owner-role sketch 는 4,000 표본에서 V108 의 dynamicTop1=2/4 를 3/4 로 올렸고,
    `손실충당금 -> 대손충당금` 을 rank 5 에서 rank 1 로 끌어올렸다. negative top5 오염도 0/7 을 유지했다.
    이는 역인덱스 후보 생성에서 단순 independent surface 분리 다음에, relation owner 로 경험한 surface 를 별도
    role sketch 로 압축하는 방향이 실제 의미 rank 개선으로 이어진다는 증거다.

    다만 `영업손익 -> 영업이익` 은 여전히 rank 45 다. owner-role sketch 는 같은 손익 계열 literal-near surface 와
    영업외/누계/차입 같은 문맥 surface 를 충분히 분리하지 못한다. 다음 구조는 owner-role atom 을 unit 전체 signature 가
    아니라 owner frame 안의 좌우 stem 경험만으로 더 좁히거나, query/target 의 relation owner frame 에서
    "동일 계정 역할" 과 "반대/외부 손익 역할" 을 분리하는 local owner-frame sketch 여야 한다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V109_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V109_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V109_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V109_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V109_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V109_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V109_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V109_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V109_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_SEARCH_CANDIDATE_LIMIT", "420"))
DYNAMIC_TARGET_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_DYNAMIC_TARGET_LIMIT", "80"))
DYNAMIC_COORD_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_DYNAMIC_COORD_ROW_LIMIT", "220"))
DYNAMIC_COMPOUND_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_DYNAMIC_COMPOUND_ROW_LIMIT", "260"))
DYNAMIC_QUERY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_DYNAMIC_QUERY_ATOM_LIMIT", "48"))
DYNAMIC_MEANING_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_DYNAMIC_MEANING_ATOM_LIMIT", "36"))
DYNAMIC_MEANING_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_DYNAMIC_MEANING_ROW_LIMIT", "220"))
DYNAMIC_RELATION_SURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_DYNAMIC_RELATION_SURFACE_LIMIT", "420"))
DYNAMIC_RELATION_UNIT_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_DYNAMIC_RELATION_UNIT_LIMIT", "160"))
DYNAMIC_BRIDGE_ONLY_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V109_DYNAMIC_BRIDGE_ONLY_PENALTY", "0.80"))
DYNAMIC_OWNER_ROLE_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V109_DYNAMIC_OWNER_ROLE_SIGNATURE_LIMIT", "64")
)
DYNAMIC_OWNER_ROLE_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_DYNAMIC_OWNER_ROLE_ATOM_LIMIT", "40"))
DYNAMIC_OWNER_ROLE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_DYNAMIC_OWNER_ROLE_ROW_LIMIT", "220"))
DYNAMIC_OWNER_ROLE_UNIT_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V109_DYNAMIC_OWNER_ROLE_UNIT_ATOM_LIMIT", "36")
)
DYNAMIC_OWNER_ROLE_MIN_BOUND = float(os.environ.get("DARTLAB_HORIZON_V109_DYNAMIC_OWNER_ROLE_MIN_BOUND", "0.55"))
DYNAMIC_OWNER_ROLE_CANDIDATE_BONUS = float(
    os.environ.get("DARTLAB_HORIZON_V109_DYNAMIC_OWNER_ROLE_CANDIDATE_BONUS", "2.40")
)
DYNAMIC_OWNER_ROLE_ROUTE_BONUS = float(os.environ.get("DARTLAB_HORIZON_V109_DYNAMIC_OWNER_ROLE_ROUTE_BONUS", "0.90"))
DYNAMIC_OWNER_ROLE_WEAK_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V109_DYNAMIC_OWNER_ROLE_WEAK_PENALTY", "0.16"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V109_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V109_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V109_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V109_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V109_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V109_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V109_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V109_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
LANE_MISMATCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V109_LANE_MISMATCH_PENALTY", "0.18"))
LANE_ARTIFACT_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V109_LANE_ARTIFACT_PENALTY", "0.10"))
NEAREST_ORDER_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_NEAREST_ORDER_SIGNATURE_LIMIT", "24"))
NEAREST_ORDER_PENALTY_MIN = float(os.environ.get("DARTLAB_HORIZON_V109_NEAREST_ORDER_PENALTY_MIN", "0.05"))
NEAREST_ORDER_PENALTY_SCALE = float(os.environ.get("DARTLAB_HORIZON_V109_NEAREST_ORDER_PENALTY_SCALE", "0.16"))
NEAREST_ORDER_COHORT_SURFACE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V109_NEAREST_ORDER_COHORT_SURFACE_LIMIT", "384")
)
NEAREST_ORDER_COHORT_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V109_NEAREST_ORDER_COHORT_POSITION_LIMIT", "8192")
)
NEAREST_ORDER_SURFACE_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V109_NEAREST_ORDER_SURFACE_POSITION_LIMIT", "48")
)
SUFFIX_ANCHOR_SUPPORT_MIN = int(os.environ.get("DARTLAB_HORIZON_V109_SUFFIX_ANCHOR_SUPPORT_MIN", "1"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V109_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V109_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V109_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V109_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V109_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V109_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V109_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V109_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V109_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V109_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V109_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V109_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V109_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V109_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V109_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V109_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V109_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V109_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V109_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V109_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V109_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V109_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V109_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
RELAY_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_RELAY_ROW_LIMIT", "160"))
RELAY_SPECIFIC_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_RELAY_SPECIFIC_ROW_LIMIT", "320"))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V109_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V109_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V109_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V109_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V109_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V109_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V109_SKETCH_OCC_BUCKETS", "16"))
...
def buildOwnerRoleIndexes(model: Model) -> tuple[dict[str, Counter[str]], dict[str, tuple[str, ...]], Counter[str]]:
    ...
    boundRows = 0
    unitAtomLinks = 0
    for (unitId, surface, relation), boundScore in model.relationBoundScores.items():
        ...
        if not relationOwnerCandidate(surface):
            continue
        if model.relationFrameLeaks.get((unitId, surface, relation), 0.0) >= 0.82:
            continue
        unitSig = model.unitSignatures.get(unitId, Counter())
        sentenceLane, artifactLane, ownerLane = surfaceLaneProfile(surface, model)
        raw[surface][ownerRoleAtom(f"rel:{relation}")] += boundScore * 4.0
        raw[surface][ownerRoleAtom("role:owner")] += boundScore * (1.0 + ownerLane)
        raw[surface][ownerRoleAtom("role:sentence")] += boundScore * sentenceLane * 0.35
        if artifactLane < 0.45:
            raw[surface][ownerRoleAtom("role:nonArtifact")] += boundScore * (0.45 - artifactLane)
        for atom, weight in unitSig.most_common(DYNAMIC_OWNER_ROLE_UNIT_ATOM_LIMIT):
            if atom.startswith("surf:"):
                continue
            if not atom.startswith(("xp:", "el:", "hx:", "relay:", "rel:")):
                continue
            raw[surface][ownerRoleAtom(atom)] += min(float(weight), 4.0) * boundScore
            unitAtomLinks += 1
    ...
    print(
        ...
        f"unitAtomLinks={unitAtomLinks} atoms={len(postings)} rawLinks={rawLinks} "
