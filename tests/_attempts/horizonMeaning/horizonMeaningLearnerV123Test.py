"""Horizon Meaning Learner V123 - role-transition masked frame graph.

V123 실제 기록
--------------
아이디어:
    V122 는 maskedFrameSignatures 를 owner/artifact/context/mixed role 별 graph 로 hard partition 했다.
    그 결과 familyTop5 는 유지됐지만 exactTop1 이 244/373 에서 156/373 으로 무너졌다. 역할 분리는 의미군을
    보존하지만, candidate role 을 잘라내면 center identity 예측 경로가 끊긴다는 뜻이다.

    V123 은 role 을 자르지 않고 전이를 학습한다. corpus 안의 relation/window 주변 surface pair 에서
    sourceRole -> targetRole 전이 count 를 만들고, role별 masked graph 를 이 전이 행렬로 혼합한다. 예를 들어
    context surface 의 주변 경험이 owner/artifact surface 를 예측해야 하는 경우, 그 통로가 데이터에서 관측된
    transition weight 로 열린다.

    목표는 "역할별 공간 분리" 가 아니라 "역할 사이 의미 이동" 이다. V112 의 masked local frame prediction 이
    중심 stem을 예측할 수 있다는 신호를 유지하면서, V122 의 hard partition 손실 없이 context query 가
    account/metric target 으로 넘어가는 sparse role-pair prediction 이 가능한지 검증한다.

실행 코드:
    $env:DARTLAB_HORIZON_V123_MAX_RECORDS_PER_SOURCE=120
    $env:DARTLAB_HORIZON_V123_MAX_UNITS=1200
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV123Test.py

검증 기준:
    1. maskedRoleTransition matrix 가 corpus window pair 에서 생성되어야 한다.
    2. transition masked holdout exact/family top-k 가 V122 hard partition 보다 회복되어야 한다.
    3. transition positive probe 가 V122 maskedRoleProbeTop5=0/4 보다 개선되거나, 최소 negative top5=0/7 을 유지해야 한다.
    4. 특정 단어 하드코딩 없이 surface lane role 과 corpus pair count 만 사용해야 한다.
    5. fixed route/search guard 는 positiveHits=3/4, badAccepted=0/7 수준을 유지해야 한다.

결과:
    1,200 unit smoke 에서 role-transition matrix 는 실제 corpus pair 로 생성됐다.
    `pairs=4,420,492` 이고 전이 weight 는
    owner->owner:0.29/artifact:0.06/context:0.63/mixed:0.02,
    artifact->owner:0.18/artifact:0.16/context:0.64/mixed:0.03,
    context->owner:0.23/artifact:0.08/context:0.68/mixed:0.01,
    mixed->owner:0.19/artifact:0.10/context:0.48/mixed:0.23 이었다.

    하지만 transition 혼합은 V122 hard partition 보다 더 나빴다. 기존 maskedFrame 은
    exactTop1=244/373, exactTop5=348/373, familyTop5=372/373 이고 V122 maskedRoleFrame 은
    exactTop1=156/373, exactTop5=208/373, familyTop5=372/373 이었다. V123 maskedRoleTransition 은
    exactTop1=82/373, exactTop5=96/373, familyTop1=368/373, familyTop5=370/373 으로 더 하락했다.

    probe 도 개선되지 않았다. maskedTransitionProbeTop1=0/4, Top5=0/4,
    BadTop1=0/7, BadTop5=0/7 이었다. `외상매출금 -> 매출채권`,
    `영업손익 -> 영업이익`, `현금성자산 -> 현금및현금성자산`, `손실충당금 -> 대손충당금` 모두
    rank NA 였고 top 은 `매출금액`, `영업외손익`, `금융자산`, `신용손실충당금` 같은 generic/context
    또는 가까운 표면으로 새었다. fixed route/search guard 는 positiveHits=3/4, badAccepted=0/7,
    searchTop1=4/5, reliableSearch=4/5 를 유지했다.

    비용도 악화됐다. buildMaskedRoleTransitionWeights 가 21.7s 를 추가해 modelSeconds=64.5,
    totalSeconds=70.7 이었다.

결론:
    실패/진단 성공. raw co-window surface pair 로 만든 role-transition 은 의미 이동이 아니라 corpus 의
    주변 문맥 분포를 학습했다. 특히 context->context 0.68, owner->context 0.63 이 지배해 query 가
    target owner/artifact 로 넘어가기보다 generic context lane 에 머물렀다.

    폐기할 것은 "모든 frame pair 에서 sourceRole->targetRole 을 세는 raw transition matrix" 이다.
    살아남은 신호는 role 전이가 필요하다는 점이 아니라, 전이를 relation-bound/owner-conditioned/directive
    조건으로 묶어야 한다는 점이다. 다음 개념은 모든 co-window pair 가 아니라 relation anchor 주변에서
    source surface 와 target owner/artifact 가 방향성 있게 연결된 role-pair evidence 만 세는
    relation-bound role-pair transition 이다.
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V123_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V123_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V123_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V123_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V123_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V123_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V123_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V123_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V123_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_SEARCH_CANDIDATE_LIMIT", "420"))
DYNAMIC_TARGET_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_DYNAMIC_TARGET_LIMIT", "80"))
DYNAMIC_COORD_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_DYNAMIC_COORD_ROW_LIMIT", "220"))
DYNAMIC_COMPOUND_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_DYNAMIC_COMPOUND_ROW_LIMIT", "260"))
DYNAMIC_QUERY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_DYNAMIC_QUERY_ATOM_LIMIT", "48"))
DYNAMIC_MEANING_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_DYNAMIC_MEANING_ATOM_LIMIT", "36"))
DYNAMIC_MEANING_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_DYNAMIC_MEANING_ROW_LIMIT", "220"))
DYNAMIC_RELATION_SURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_DYNAMIC_RELATION_SURFACE_LIMIT", "420"))
DYNAMIC_RELATION_UNIT_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_DYNAMIC_RELATION_UNIT_LIMIT", "160"))
DYNAMIC_BRIDGE_ONLY_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V123_DYNAMIC_BRIDGE_ONLY_PENALTY", "0.80"))
DYNAMIC_OWNER_ROLE_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V123_DYNAMIC_OWNER_ROLE_SIGNATURE_LIMIT", "64")
)
DYNAMIC_OWNER_ROLE_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_DYNAMIC_OWNER_ROLE_ATOM_LIMIT", "40"))
DYNAMIC_OWNER_ROLE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_DYNAMIC_OWNER_ROLE_ROW_LIMIT", "220"))
DYNAMIC_OWNER_ROLE_UNIT_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V123_DYNAMIC_OWNER_ROLE_UNIT_ATOM_LIMIT", "36")
)
DYNAMIC_OWNER_ROLE_MIN_BOUND = float(os.environ.get("DARTLAB_HORIZON_V123_DYNAMIC_OWNER_ROLE_MIN_BOUND", "0.55"))
DYNAMIC_OWNER_ROLE_CANDIDATE_BONUS = float(
    os.environ.get("DARTLAB_HORIZON_V123_DYNAMIC_OWNER_ROLE_CANDIDATE_BONUS", "2.40")
)
DYNAMIC_OWNER_ROLE_ROUTE_BONUS = float(os.environ.get("DARTLAB_HORIZON_V123_DYNAMIC_OWNER_ROLE_ROUTE_BONUS", "0.90"))
DYNAMIC_OWNER_ROLE_WEAK_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V123_DYNAMIC_OWNER_ROLE_WEAK_PENALTY", "0.16"))
DYNAMIC_OWNER_FRAME_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V123_DYNAMIC_OWNER_FRAME_RADIUS", "5"))
DYNAMIC_OWNER_FRAME_BETWEEN_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_DYNAMIC_OWNER_FRAME_BETWEEN_LIMIT", "7"))
MASKED_FRAME_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_MASKED_FRAME_SIGNATURE_LIMIT", "72"))
MASKED_FRAME_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_MASKED_FRAME_ATOM_LIMIT", "48"))
MASKED_FRAME_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_MASKED_FRAME_ROW_LIMIT", "240"))
MASKED_FRAME_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_MASKED_FRAME_EVAL_LIMIT", "800"))
MASKED_FRAME_HOLDOUT_MOD = int(os.environ.get("DARTLAB_HORIZON_V123_MASKED_FRAME_HOLDOUT_MOD", "5"))
MASKED_FRAME_PROBE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_MASKED_FRAME_PROBE_LIMIT", "12"))
MASKED_CONTEXT_SURFACE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_MASKED_CONTEXT_SURFACE_ROW_LIMIT", "12"))
MASKED_CONTEXT_WEIGHT = float(os.environ.get("DARTLAB_HORIZON_V123_MASKED_CONTEXT_WEIGHT", "0.35"))
MASKED_CONTEXT_RELATION_WINDOW = int(os.environ.get("DARTLAB_HORIZON_V123_MASKED_CONTEXT_RELATION_WINDOW", "140"))
MASKED_FRAME_SURPRISAL_POWER = float(os.environ.get("DARTLAB_HORIZON_V123_MASKED_FRAME_SURPRISAL_POWER", "2.0"))
MASKED_FRAME_SURFACE_PRIOR_POWER = float(
    os.environ.get("DARTLAB_HORIZON_V123_MASKED_FRAME_SURFACE_PRIOR_POWER", "0.45")
)
RELATION_OWNER_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_RELATION_OWNER_SIGNATURE_LIMIT", "64"))
RELATION_OWNER_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_RELATION_OWNER_ROW_LIMIT", "220"))
RELATION_OWNER_BRIDGE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_RELATION_OWNER_BRIDGE_LIMIT", "12"))
RELATION_OWNER_PROJECTION_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_RELATION_OWNER_PROJECTION_LIMIT", "24"))
RELATION_OWNER_PROJECTION_OWNER_ROLE_MIN = float(
    os.environ.get("DARTLAB_HORIZON_V123_RELATION_OWNER_PROJECTION_OWNER_ROLE_MIN", "0.18")
)
RELATION_OWNER_PROJECTION_QUERY_ROLE_MIN = float(
    os.environ.get("DARTLAB_HORIZON_V123_RELATION_OWNER_PROJECTION_QUERY_ROLE_MIN", "0.05")
)
RELATION_OWNER_GENERATED_BRIDGE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V123_RELATION_OWNER_GENERATED_BRIDGE_LIMIT", "48")
)
RELATION_OWNER_GENERATED_BRIDGE_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V123_RELATION_OWNER_GENERATED_BRIDGE_WEIGHT", "2.4")
)
RELATION_OWNER_ROLE_COMPAT_MIN = float(os.environ.get("DARTLAB_HORIZON_V123_RELATION_OWNER_ROLE_COMPAT_MIN", "0.16"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V123_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V123_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V123_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V123_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V123_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V123_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V123_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V123_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
LANE_MISMATCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V123_LANE_MISMATCH_PENALTY", "0.18"))
LANE_ARTIFACT_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V123_LANE_ARTIFACT_PENALTY", "0.10"))
NEAREST_ORDER_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_NEAREST_ORDER_SIGNATURE_LIMIT", "24"))
NEAREST_ORDER_PENALTY_MIN = float(os.environ.get("DARTLAB_HORIZON_V123_NEAREST_ORDER_PENALTY_MIN", "0.05"))
NEAREST_ORDER_PENALTY_SCALE = float(os.environ.get("DARTLAB_HORIZON_V123_NEAREST_ORDER_PENALTY_SCALE", "0.16"))
NEAREST_ORDER_COHORT_SURFACE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V123_NEAREST_ORDER_COHORT_SURFACE_LIMIT", "384")
)
NEAREST_ORDER_COHORT_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V123_NEAREST_ORDER_COHORT_POSITION_LIMIT", "8192")
)
NEAREST_ORDER_SURFACE_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V123_NEAREST_ORDER_SURFACE_POSITION_LIMIT", "48")
)
SUFFIX_ANCHOR_SUPPORT_MIN = int(os.environ.get("DARTLAB_HORIZON_V123_SUFFIX_ANCHOR_SUPPORT_MIN", "1"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V123_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V123_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V123_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V123_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V123_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V123_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V123_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V123_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V123_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V123_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V123_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V123_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V123_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V123_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V123_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V123_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V123_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V123_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V123_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V123_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V123_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V123_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V123_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
RELAY_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_RELAY_ROW_LIMIT", "160"))
RELAY_SPECIFIC_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_RELAY_SPECIFIC_ROW_LIMIT", "320"))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V123_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V123_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V123_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V123_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V123_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V123_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V123_SKETCH_OCC_BUCKETS", "16"))
...
def buildModel() -> Model:
    ...
    maskedRoleTransitionWeights = buildMaskedRoleTransitionWeights(model)
    model.maskedRoleTransitionWeights = maskedRoleTransitionWeights
    stage("buildMaskedRoleTransitionWeights")
    (
        relationOwnerGraphSignatures,
        relationOwnerGraphPostings,
        relationOwnerGraphScores,
...
def main() -> None:
    ...
    maskedTransitionMetrics = maskedRoleTransitionPredictionMetrics(model)
    print(
        f"[maskedRoleTransitionEval] evaluated={maskedTransitionMetrics.get('evaluated', 0)} "
        f"empty={maskedTransitionMetrics.get('empty', 0)} "
        f"exactTop1={maskedTransitionMetrics.get('exactTop1', 0)}/{max(1, maskedTransitionMetrics.get('evaluated', 0))} "
        f"exactTop5={maskedTransitionMetrics.get('exactTop5', 0)}/{max(1, maskedTransitionMetrics.get('evaluated', 0))} "
        f"familyTop1={maskedTransitionMetrics.get('familyTop1', 0)}/{max(1, maskedTransitionMetrics.get('evaluated', 0))} "
        f"familyTop5={maskedTransitionMetrics.get('familyTop5', 0)}/{max(1, maskedTransitionMetrics.get('evaluated', 0))}"
    )
    maskedProbeTop1 = 0
    maskedProbeTop5 = 0
    ...
    maskedTransitionProbeTop1 = 0
    maskedTransitionProbeTop5 = 0
    maskedTransitionProbeBadTop1 = 0
    maskedTransitionProbeBadTop5 = 0
    print("[maskedRoleTransitionProbes:positive]")
    for surface, expected in POSITIVE_PROBES:
        rows = maskedRoleTransitionRoute(surface, model)
        rank = maskedFrameRank(rows, expected)
        maskedTransitionProbeTop1 += int(rank == 1)
        maskedTransitionProbeTop5 += int(rank is not None and rank <= 5)
        roleWeights = model.maskedRoleTransitionWeights.get(maskedFrameSurfaceRole(surface, model), {})
        transitionPreview = ",".join(f"{role}:{roleWeights.get(role, 0.0):.2f}" for role in MASKED_ROLE_FRAME_ROLES)
        print(
            f"  {surface}->{expected} sourceRole={maskedFrameSurfaceRole(surface, model)} "
            f"transition={transitionPreview} rank={rank if rank is not None else 'NA'} "
            f"top={previewMaskedFrame(rows)}"
        )
    print("[maskedRoleTransitionProbes:negative]")
    for surface, forbidden in NEGATIVE_PROBES:
        rows = maskedRoleTransitionRoute(surface, model)
        rank = maskedFrameRank(rows, forbidden)
        topForbidden = bool(rows and rows[0][0] == forbidden)
        maskedTransitionProbeBadTop1 += int(topForbidden)
        maskedTransitionProbeBadTop5 += int(rank is not None and rank <= 5)
        roleWeights = model.maskedRoleTransitionWeights.get(maskedFrameSurfaceRole(surface, model), {})
        transitionPreview = ",".join(f"{role}:{roleWeights.get(role, 0.0):.2f}" for role in MASKED_ROLE_FRAME_ROLES)
        print(
            f"  {surface}-/->{forbidden} sourceRole={maskedFrameSurfaceRole(surface, model)} "
            f"transition={transitionPreview} forbiddenRank={rank if rank is not None else 'NA'} "
    ...
    print(
        ...
        f"maskedTransitionExactTop1={maskedTransitionMetrics.get('exactTop1', 0)}/{max(1, maskedTransitionMetrics.get('evaluated', 0))} "
        f"maskedTransitionExactTop5={maskedTransitionMetrics.get('exactTop5', 0)}/{max(1, maskedTransitionMetrics.get('evaluated', 0))} "
        f"maskedTransitionProbeTop1={maskedTransitionProbeTop1}/{len(POSITIVE_PROBES)} maskedTransitionProbeTop5={maskedTransitionProbeTop5}/{len(POSITIVE_PROBES)} "
        f"maskedTransitionProbeBadTop1={maskedTransitionProbeBadTop1}/{len(NEGATIVE_PROBES)} maskedTransitionProbeBadTop5={maskedTransitionProbeBadTop5}/{len(NEGATIVE_PROBES)} "
