"""Horizon Meaning Learner V131 - negative contrast semantic-unit masked head.

V131 실제 기록
--------------
아이디어:
    V130 은 relation/action profile 과 suffix cohort contrast 로 semantic-unit component 를 더 보수적으로
    만들었고, holdout labelTop5 는 793/896 에서 802/896 로 좋아졌다. 하지만 probe negative 는 줄지 않았다.
    원인은 component merge 가 아니라 masked head scoring 이었다. `복구충당금` 과 `대손충당금` 을 다른 unit 으로
    분리해도, query horizon atoms 가 대손충당금 label 을 강하게 예측하면 forbidden rank 는 그대로 1 이 된다.

    V131 은 V130 이 graph union 단계에서 버린 rejected near-pair 를 버리지 않는다. suffix cohort, one-sided
    relation profile, relation mismatch, contrast reject 로 합치지 않은 pair 를 anti-label evidence 로 학습한다.
    예를 들어 A 와 B 가 경험은 가까운데 relation/suffix contrast 로 다른 unit 이 되면, A 의 horizon atoms 는
    B label 을 깎는 sparse posting 에 들어가고 B 의 horizon atoms 는 A label 을 깎는다.

    이것은 특정 단어 금지표가 아니라 corpus 에서 자동 생성된 rejected pair 를 negative sample 로 쓰는 방식이다.
    목표는 positive semantic label prediction 은 보존하면서, 가까운 suffix/경험 때문에 잘못 올라오는 forbidden
    label 을 masked scoring 단계에서 직접 낮추는 것이다.

실행 코드:
    $env:DARTLAB_HORIZON_V131_MAX_RECORDS_PER_SOURCE=120
    $env:DARTLAB_HORIZON_V131_MAX_UNITS=1200
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV131Test.py

검증 기준:
    1. contrastSemanticUnit graph/head 와 별도로 contrastSemanticUnitAnti head 가 생성되어야 한다.
    2. contrastSemanticUnit labelTop5 가 V130 802/896 에서 크게 붕괴하지 않아야 한다.
    3. anti-label scoring 이 실제로 작동해 V130 BadTop1=1/7, BadTop5=2/7 보다 줄어야 한다.
    4. positive probe 는 최소 V130 Top5=1/4 수준을 유지해야 한다.
    5. 특정 surface 하드코딩, forbidden 단어 패널티, 케이스별 gate 없이 rejected near-pair 만 negative sample 로 사용한다.
    6. fixed route/search guard 는 positiveHits=3/4, badAccepted=0/7 수준을 유지해야 한다.

결과:
    1,200 unit smoke 에서 contrastSemanticUnit graph/head 와 별도로 contrastSemanticUnitAnti head 가 생성됐다.
    contrastSemanticUnitGraph 는 surfaces=10,115, units=6,394, singletons=5,063,
    relationProfiles=6,097, candidatePairs=30,007, relationEdges=4,322,
    experienceEdges=4,959, mixedEdges=122, suffixRejects=1,566,
    oneSidedRejects=1,731, relationRejects=322, contrastRejects=3,447,
    antiPairs=2,673 이었다.

    contrastSemanticUnitAnti 는 labels=847, atoms=28,855, inputPairs=2,673,
    antiRows=2,588, rawLinks=52,050, keptLinks=52,050, sameLabelSkips=1,379 로 생성됐다.
    buildContrastSemanticUnitGraph 는 3.7s, buildContrastSemanticUnitMaskedHead 는 2.9s,
    buildContrastSemanticUnitAntiHead 는 0.3s, totalSeconds 는 149.4 였다.

    그러나 holdout 이 크게 무너졌다. V130 contrastSemanticUnitEval 은 labelTop1=705/896,
    labelTop5=802/896, top5UnitContainsFamily=829/896 였지만, V131 은 labelTop1=646/896,
    labelTop5=728/896, top5UnitContainsFamily=765/896 로 떨어졌다.

    probe 도 개선되지 않았다. contrastSemanticUnitProbeTop1=1/4, Top5=1/4,
    BadTop1=1/7, BadTop5=2/7 로 V130 과 같았다. `복구충당금 -> 대손충당금` forbidden 은 여전히
    negative 에 남았다. fixed route/search guard 는 positiveHits=3/4, badAccepted=0/7,
    searchTop1=4/5, reliableSearch=4/5 를 유지했다.

결론:
    실패/진단 성공. rejected near-pair 를 anti-label evidence 로 바꾸는 구조는 생성 가능했지만,
    label 단위로 broad 하게 빼면 의미 예측력을 크게 깎고 실제 forbidden probe 는 줄이지 못한다.
    즉 anti evidence 자체가 필요하다는 V130 의 진단은 맞지만, V131 처럼 label 전체에 전역 subtract 하는 방식은
    너무 거칠다.

    폐기할 것은 rejected pair 의 source atoms 를 target label 전체 anti posting 으로 펼치는 방식이다.
    다음 개념은 pair-local 또는 query-conditioned anti 이다. anti 는 후보 label 전체가 아니라
    query surface 와 candidate unit member 가 같은 suffix/coord cohort 에 놓이고 relation profile 이 충돌할 때만
    활성화되어야 한다. 다시 말해 negative contrast 는 "label prior" 가 아니라 "query-candidate pair 관계" 로
    계산해야 한다.
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V131_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V131_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V131_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V131_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V131_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V131_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V131_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V131_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V131_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_SEARCH_CANDIDATE_LIMIT", "420"))
DYNAMIC_TARGET_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_DYNAMIC_TARGET_LIMIT", "80"))
DYNAMIC_COORD_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_DYNAMIC_COORD_ROW_LIMIT", "220"))
DYNAMIC_COMPOUND_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_DYNAMIC_COMPOUND_ROW_LIMIT", "260"))
DYNAMIC_QUERY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_DYNAMIC_QUERY_ATOM_LIMIT", "48"))
DYNAMIC_MEANING_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_DYNAMIC_MEANING_ATOM_LIMIT", "36"))
DYNAMIC_MEANING_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_DYNAMIC_MEANING_ROW_LIMIT", "220"))
DYNAMIC_RELATION_SURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_DYNAMIC_RELATION_SURFACE_LIMIT", "420"))
DYNAMIC_RELATION_UNIT_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_DYNAMIC_RELATION_UNIT_LIMIT", "160"))
DYNAMIC_BRIDGE_ONLY_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V131_DYNAMIC_BRIDGE_ONLY_PENALTY", "0.80"))
DYNAMIC_OWNER_ROLE_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V131_DYNAMIC_OWNER_ROLE_SIGNATURE_LIMIT", "64")
)
DYNAMIC_OWNER_ROLE_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_DYNAMIC_OWNER_ROLE_ATOM_LIMIT", "40"))
DYNAMIC_OWNER_ROLE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_DYNAMIC_OWNER_ROLE_ROW_LIMIT", "220"))
DYNAMIC_OWNER_ROLE_UNIT_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V131_DYNAMIC_OWNER_ROLE_UNIT_ATOM_LIMIT", "36")
)
DYNAMIC_OWNER_ROLE_MIN_BOUND = float(os.environ.get("DARTLAB_HORIZON_V131_DYNAMIC_OWNER_ROLE_MIN_BOUND", "0.55"))
DYNAMIC_OWNER_ROLE_CANDIDATE_BONUS = float(
    os.environ.get("DARTLAB_HORIZON_V131_DYNAMIC_OWNER_ROLE_CANDIDATE_BONUS", "2.40")
)
DYNAMIC_OWNER_ROLE_ROUTE_BONUS = float(os.environ.get("DARTLAB_HORIZON_V131_DYNAMIC_OWNER_ROLE_ROUTE_BONUS", "0.90"))
DYNAMIC_OWNER_ROLE_WEAK_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V131_DYNAMIC_OWNER_ROLE_WEAK_PENALTY", "0.16"))
DYNAMIC_OWNER_FRAME_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V131_DYNAMIC_OWNER_FRAME_RADIUS", "5"))
DYNAMIC_OWNER_FRAME_BETWEEN_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_DYNAMIC_OWNER_FRAME_BETWEEN_LIMIT", "7"))
MASKED_FRAME_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_MASKED_FRAME_SIGNATURE_LIMIT", "72"))
MASKED_FRAME_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_MASKED_FRAME_ATOM_LIMIT", "48"))
MASKED_FRAME_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_MASKED_FRAME_ROW_LIMIT", "240"))
MASKED_FRAME_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_MASKED_FRAME_EVAL_LIMIT", "800"))
MASKED_FRAME_HOLDOUT_MOD = int(os.environ.get("DARTLAB_HORIZON_V131_MASKED_FRAME_HOLDOUT_MOD", "5"))
MASKED_FRAME_PROBE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_MASKED_FRAME_PROBE_LIMIT", "12"))
MASKED_CONTEXT_SURFACE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_MASKED_CONTEXT_SURFACE_ROW_LIMIT", "12"))
MASKED_CONTEXT_WEIGHT = float(os.environ.get("DARTLAB_HORIZON_V131_MASKED_CONTEXT_WEIGHT", "0.35"))
MASKED_CONTEXT_RELATION_WINDOW = int(os.environ.get("DARTLAB_HORIZON_V131_MASKED_CONTEXT_RELATION_WINDOW", "140"))
MASKED_FRAME_SURPRISAL_POWER = float(os.environ.get("DARTLAB_HORIZON_V131_MASKED_FRAME_SURPRISAL_POWER", "2.0"))
MASKED_FRAME_SURFACE_PRIOR_POWER = float(
    os.environ.get("DARTLAB_HORIZON_V131_MASKED_FRAME_SURFACE_PRIOR_POWER", "0.45")
)
RELATION_OWNER_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_RELATION_OWNER_SIGNATURE_LIMIT", "64"))
RELATION_OWNER_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_RELATION_OWNER_ROW_LIMIT", "220"))
RELATION_OWNER_BRIDGE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_RELATION_OWNER_BRIDGE_LIMIT", "12"))
RELATION_OWNER_PROJECTION_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_RELATION_OWNER_PROJECTION_LIMIT", "24"))
RELATION_OWNER_PROJECTION_OWNER_ROLE_MIN = float(
    os.environ.get("DARTLAB_HORIZON_V131_RELATION_OWNER_PROJECTION_OWNER_ROLE_MIN", "0.18")
)
RELATION_OWNER_PROJECTION_QUERY_ROLE_MIN = float(
    os.environ.get("DARTLAB_HORIZON_V131_RELATION_OWNER_PROJECTION_QUERY_ROLE_MIN", "0.05")
)
RELATION_OWNER_GENERATED_BRIDGE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V131_RELATION_OWNER_GENERATED_BRIDGE_LIMIT", "48")
)
RELATION_OWNER_GENERATED_BRIDGE_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V131_RELATION_OWNER_GENERATED_BRIDGE_WEIGHT", "2.4")
)
RELATION_OWNER_ROLE_COMPAT_MIN = float(os.environ.get("DARTLAB_HORIZON_V131_RELATION_OWNER_ROLE_COMPAT_MIN", "0.16"))
RELATION_BOUND_ROLE_PAIR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_RELATION_BOUND_ROLE_PAIR_LIMIT", "24"))
RELATION_BOUND_ROLE_PAIR_BRIDGE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V131_RELATION_BOUND_ROLE_PAIR_BRIDGE_LIMIT", "10")
)
RELATION_BOUND_ROLE_PAIR_BRIDGE_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V131_RELATION_BOUND_ROLE_PAIR_BRIDGE_WEIGHT", "0.46")
)
RELATION_SOURCE_EXPERIENCE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V131_RELATION_SOURCE_EXPERIENCE_ATOM_LIMIT", "44")
)
RELATION_SOURCE_EXPERIENCE_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V131_RELATION_SOURCE_EXPERIENCE_ROW_LIMIT", "180")
)
RELATION_SOURCE_SHINGLE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V131_RELATION_SOURCE_SHINGLE_ATOM_LIMIT", "28")
)
RELATION_SOURCE_SHINGLE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_RELATION_SOURCE_SHINGLE_ROW_LIMIT", "160"))
RELATION_SOURCE_SHINGLE_KEY_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_RELATION_SOURCE_SHINGLE_KEY_LIMIT", "56"))
RELATION_ANCHORED_COARSE_SHINGLE_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V131_RELATION_ANCHORED_COARSE_SHINGLE_ROW_LIMIT", "180")
)
RELATION_ANCHORED_COARSE_SHINGLE_KEY_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V131_RELATION_ANCHORED_COARSE_SHINGLE_KEY_LIMIT", "72")
)
HORIZON_MASK_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_HORIZON_MASK_SIGNATURE_LIMIT", "96"))
HORIZON_MASK_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_HORIZON_MASK_ATOM_LIMIT", "56"))
HORIZON_MASK_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_HORIZON_MASK_ROW_LIMIT", "220"))
HORIZON_MASK_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_HORIZON_MASK_EVAL_LIMIT", "900"))
HORIZON_MASK_HOLDOUT_MOD = int(os.environ.get("DARTLAB_HORIZON_V131_HORIZON_MASK_HOLDOUT_MOD", "7"))
HORIZON_MASK_SURFACE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_HORIZON_MASK_SURFACE_ROW_LIMIT", "24"))
HORIZON_MASK_NEIGHBOR_EXPERIENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V131_HORIZON_MASK_NEIGHBOR_EXPERIENCE_LIMIT", "5")
)
HORIZON_MASK_COORD_CELL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_HORIZON_MASK_COORD_CELL_LIMIT", "5"))
HORIZON_MASK_COORD_BUCKET_DIGITS = int(os.environ.get("DARTLAB_HORIZON_V131_HORIZON_MASK_COORD_BUCKET_DIGITS", "5"))
SEMANTIC_UNIT_SIGNATURE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V131_SEMANTIC_UNIT_SIGNATURE_ATOM_LIMIT", "18")
)
SEMANTIC_UNIT_POSTING_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_SEMANTIC_UNIT_POSTING_ROW_LIMIT", "90"))
SEMANTIC_UNIT_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_SEMANTIC_UNIT_CANDIDATE_LIMIT", "180"))
SEMANTIC_UNIT_MIN_EXPERIENCE_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V131_SEMANTIC_UNIT_MIN_EXPERIENCE_SIM", "0.46")
)
SEMANTIC_UNIT_MIN_MIXED_SIM = float(os.environ.get("DARTLAB_HORIZON_V131_SEMANTIC_UNIT_MIN_MIXED_SIM", "0.34"))
SEMANTIC_UNIT_MIN_COORD_RESONANCE = float(
    os.environ.get("DARTLAB_HORIZON_V131_SEMANTIC_UNIT_MIN_COORD_RESONANCE", "0.045")
)
SEMANTIC_UNIT_MASK_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V131_SEMANTIC_UNIT_MASK_SIGNATURE_LIMIT", "120")
)
SEMANTIC_UNIT_MASK_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_SEMANTIC_UNIT_MASK_ATOM_LIMIT", "64"))
SEMANTIC_UNIT_MASK_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_SEMANTIC_UNIT_MASK_ROW_LIMIT", "260"))
SEMANTIC_UNIT_ROUTE_MEMBER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_SEMANTIC_UNIT_ROUTE_MEMBER_LIMIT", "4"))
CONTRAST_SEMANTIC_UNIT_MIN_EXPERIENCE_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V131_CONTRAST_SEMANTIC_UNIT_MIN_EXPERIENCE_SIM", "0.46")
)
CONTRAST_SEMANTIC_UNIT_STRONG_EXPERIENCE_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V131_CONTRAST_SEMANTIC_UNIT_STRONG_EXPERIENCE_SIM", "0.62")
)
CONTRAST_SEMANTIC_UNIT_MIN_MIXED_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V131_CONTRAST_SEMANTIC_UNIT_MIN_MIXED_SIM", "0.36")
)
CONTRAST_SEMANTIC_UNIT_MIN_COORD_RESONANCE = float(
    os.environ.get("DARTLAB_HORIZON_V131_CONTRAST_SEMANTIC_UNIT_MIN_COORD_RESONANCE", "0.050")
)
CONTRAST_SEMANTIC_UNIT_MIN_RELATION_COMPAT = float(
    os.environ.get("DARTLAB_HORIZON_V131_CONTRAST_SEMANTIC_UNIT_MIN_RELATION_COMPAT", "0.28")
)
CONTRAST_SEMANTIC_UNIT_SUFFIX_RELATION_COMPAT = float(
    os.environ.get("DARTLAB_HORIZON_V131_CONTRAST_SEMANTIC_UNIT_SUFFIX_RELATION_COMPAT", "0.18")
)
NEGATIVE_CONTRAST_ANTI_PAIR_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V131_NEGATIVE_CONTRAST_ANTI_PAIR_LIMIT", "120000")
)
NEGATIVE_CONTRAST_ANTI_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_NEGATIVE_CONTRAST_ANTI_ATOM_LIMIT", "64"))
NEGATIVE_CONTRAST_ANTI_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_NEGATIVE_CONTRAST_ANTI_ROW_LIMIT", "180"))
NEGATIVE_CONTRAST_ANTI_WEIGHT = float(os.environ.get("DARTLAB_HORIZON_V131_NEGATIVE_CONTRAST_ANTI_WEIGHT", "0.55"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V131_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V131_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V131_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V131_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V131_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V131_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V131_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V131_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
LANE_MISMATCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V131_LANE_MISMATCH_PENALTY", "0.18"))
LANE_ARTIFACT_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V131_LANE_ARTIFACT_PENALTY", "0.10"))
NEAREST_ORDER_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_NEAREST_ORDER_SIGNATURE_LIMIT", "24"))
NEAREST_ORDER_PENALTY_MIN = float(os.environ.get("DARTLAB_HORIZON_V131_NEAREST_ORDER_PENALTY_MIN", "0.05"))
NEAREST_ORDER_PENALTY_SCALE = float(os.environ.get("DARTLAB_HORIZON_V131_NEAREST_ORDER_PENALTY_SCALE", "0.16"))
NEAREST_ORDER_COHORT_SURFACE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V131_NEAREST_ORDER_COHORT_SURFACE_LIMIT", "384")
)
NEAREST_ORDER_COHORT_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V131_NEAREST_ORDER_COHORT_POSITION_LIMIT", "8192")
)
NEAREST_ORDER_SURFACE_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V131_NEAREST_ORDER_SURFACE_POSITION_LIMIT", "48")
)
SUFFIX_ANCHOR_SUPPORT_MIN = int(os.environ.get("DARTLAB_HORIZON_V131_SUFFIX_ANCHOR_SUPPORT_MIN", "1"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V131_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V131_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V131_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V131_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V131_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V131_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V131_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V131_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V131_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V131_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V131_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V131_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V131_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V131_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V131_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V131_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V131_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V131_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V131_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V131_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V131_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V131_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V131_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
RELAY_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_RELAY_ROW_LIMIT", "160"))
RELAY_SPECIFIC_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_RELAY_SPECIFIC_ROW_LIMIT", "320"))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V131_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V131_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V131_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V131_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V131_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V131_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V131_SKETCH_OCC_BUCKETS", "16"))
...
class Model:
    ...
    contrastSemanticUnitAntiSignatures: dict[str, Counter[str]]
    contrastSemanticUnitAntiPostings: dict[str, tuple[str, ...]]
    contrastSemanticUnitAntiAtomDf: Counter[str]
    contrastSemanticUnitAntiStats: Counter[str]
...
def buildContrastSemanticUnitGraph(
    ...
    stats: Counter[str] = Counter()
    antiPairs: list[tuple[str, str, float, str]] = []
    for surface, signature in surfaceSignatures.items():
        candidateScores: Counter[str] = Counter()
        sharedCounts: Counter[str] = Counter()
        for atom, weight in signature.most_common(SEMANTIC_UNIT_SIGNATURE_ATOM_LIMIT):
        ...
        for other, _ in candidateScores.most_common(SEMANTIC_UNIT_CANDIDATE_LIMIT):
            ...
            if suffixCohort and relationCompat < CONTRAST_SEMANTIC_UNIT_SUFFIX_RELATION_COMPAT:
                ...
                if len(antiPairs) < NEGATIVE_CONTRAST_ANTI_PAIR_LIMIT:
                    antiPairs.append((surface, other, max(0.05, experienceSim) + coordSim + 0.15, "suffix"))
                    stats["antiSuffixPairs"] += 1
                continue
            if leftHasRelation != rightHasRelation and experienceSim < CONTRAST_SEMANTIC_UNIT_STRONG_EXPERIENCE_SIM:
                ...
                if (
                    ...
                    and len(antiPairs) < NEGATIVE_CONTRAST_ANTI_PAIR_LIMIT
                ):
                    antiPairs.append((surface, other, max(0.05, experienceSim) + coordSim, "oneSided"))
                    stats["antiOneSidedPairs"] += 1
                continue
            if (
                leftHasRelation
                ...
                if (
                    ...
                    antiPairs.append((surface, other, max(0.05, experienceSim) + coordSim, "relation"))
                    stats["antiRelationPairs"] += 1
                continue
            if experienceSim >= CONTRAST_SEMANTIC_UNIT_MIN_EXPERIENCE_SIM and (
                ...
                if (suffixCohort or experienceSim >= CONTRAST_SEMANTIC_UNIT_MIN_MIXED_SIM) and len(
                    antiPairs
                ) < NEGATIVE_CONTRAST_ANTI_PAIR_LIMIT:
                    antiPairs.append((surface, other, max(0.05, experienceSim) + coordSim, "contrast"))
                    stats["antiContrastPairs"] += 1
    ...
    print(
        ...
        f"antiPairs={stats['antiPairs']} avgEdgeRelationCompat={stats['relationCompatSum'] / edgeCount:.3f} keptLinks={keptLinks}"
    )
    return labels, members, labelSignatures, labelPostings, labelScores, stats, tuple(antiPairs)
...
def buildContrastSemanticUnitAntiHead(
    model: Model,
    antiPairs: tuple[tuple[str, str, float, str], ...],
) -> tuple[dict[str, Counter[str]], dict[str, tuple[str, ...]], Counter[str], Counter[str]]:
    raw: dict[str, Counter[str]] = defaultdict(Counter)
    stats: Counter[str] = Counter()

    def addAntiEvidence(source: str, targetLabel: str, pairWeight: float, reason: str) -> None:
        signature = model.horizonMaskedSignatures.get(source, Counter())
        if not signature or not targetLabel:
            stats["emptySource"] += 1
            return
        scale = min(2.0, max(0.05, pairWeight))
        for atom, weight in signature.most_common(HORIZON_MASK_SIGNATURE_LIMIT):
            raw[targetLabel][atom] += float(weight) * scale
        stats["antiRows"] += 1
        stats[f"{reason}Rows"] += 1

    for left, right, pairWeight, reason in antiPairs:
        leftLabel = model.contrastSemanticUnitLabels.get(left, "")
        rightLabel = model.contrastSemanticUnitLabels.get(right, "")
        if not leftLabel or not rightLabel:
            stats["missingLabel"] += 1
            continue
        if leftLabel == rightLabel:
            stats["sameLabelSkips"] += 1
            continue
        addAntiEvidence(left, rightLabel, pairWeight, reason)
        addAntiEvidence(right, leftLabel, pairWeight, reason)
    ...
    for label, counter in raw.items():
        ...
        for atom, value in counter.items():
            ...
            selectedRows.append((weight, atom))
        selected = Counter(
            {atom: weight for weight, atom in sorted(selectedRows, reverse=True)[:NEGATIVE_CONTRAST_ANTI_ATOM_LIMIT]}
        )
        if not selected:
            continue
        signatures[label] = selected
        for atom, weight in selected.items():
            rawLinks += 1
            rowsByAtom[atom].append((float(weight), label))
    ...
    for atom, rows in rowsByAtom.items():
        rows = sorted(rows, reverse=True)
        if len(rows) > NEGATIVE_CONTRAST_ANTI_ROW_LIMIT:
            rows = rows[:NEGATIVE_CONTRAST_ANTI_ROW_LIMIT]
            prunedRows += 1
        postings[atom] = tuple(label for _, label in rows)
        keptLinks += len(rows)

    stats["labels"] = len(signatures)
    stats["atoms"] = len(postings)
    stats["inputPairs"] = len(antiPairs)
    stats["rawLinks"] = rawLinks
    stats["keptLinks"] = keptLinks
    stats["prunedRows"] = prunedRows
    print(
        f"[contrastSemanticUnitAnti] labels={stats['labels']} atoms={stats['atoms']} "
        f"inputPairs={stats['inputPairs']} antiRows={stats['antiRows']} "
        f"rawLinks={rawLinks} keptLinks={keptLinks} prunedRows={prunedRows} "
        f"sameLabelSkips={stats['sameLabelSkips']} missingLabel={stats['missingLabel']}"
    )
    return signatures, postings, atomDf, stats
...
def contrastSemanticUnitAntiPredictFromAtoms(atoms: Counter[str], model: Model) -> Counter[str]:
    scores: Counter[str] = Counter()
    total = max(1, len(model.contrastSemanticUnitAntiSignatures))
    for atom, weight in atoms.most_common(SEMANTIC_UNIT_MASK_ATOM_LIMIT):
        df = max(1, model.contrastSemanticUnitAntiAtomDf.get(atom, 0))
        surprisal = math.log1p(total / df)
        for rank, label in enumerate(
            model.contrastSemanticUnitAntiPostings.get(atom, ())[:NEGATIVE_CONTRAST_ANTI_ROW_LIMIT]
        ):
            signature = model.contrastSemanticUnitAntiSignatures.get(label)
            if not signature:
                continue
            candidateWeight = float(signature.get(atom, 0.0))
            if candidateWeight <= 0:
                continue
            rankDecay = 1.0 - min(rank, NEGATIVE_CONTRAST_ANTI_ROW_LIMIT) / (NEGATIVE_CONTRAST_ANTI_ROW_LIMIT + 1.0)
            scores[label] += min(float(weight), 4.0) * math.log1p(candidateWeight) * surprisal * rankDecay
    return scores


def contrastSemanticUnitMaskedPredictFromAtoms(atoms: Counter[str], model: Model) -> Counter[str]:
    scores: Counter[str] = Counter()
    for atom, weight in atoms.most_common(SEMANTIC_UNIT_MASK_ATOM_LIMIT):
    ...
    antiScores = contrastSemanticUnitAntiPredictFromAtoms(atoms, model)
    for label, antiScore in antiScores.items():
        scores[label] -= NEGATIVE_CONTRAST_ANTI_WEIGHT * float(antiScore)
    return Counter({label: score for label, score in scores.items() if score > 0})
...
def contrastSemanticUnitMaskedPredictionMetrics(model: Model) -> Counter[str]:
    ...
    for surface, atoms in model.horizonMaskedEvalRows:
        ...
        if not targetLabel:
            metrics["missingLabel"] += 1
            continue
        predictions = contrastSemanticUnitMaskedPredictFromAtoms(atoms, model)
        if not predictions:
            metrics["empty"] += 1
            continue
        ranked = [label for label, _ in predictions.most_common(5)]
...
def contrastSemanticUnitMaskedLabelRoute(surface: str, model: Model) -> tuple[tuple[str, float], ...]:
    signature = inferHorizonMaskedSignature(surface, model)
    predictions = contrastSemanticUnitMaskedPredictFromAtoms(signature, model)
    return tuple(predictions.most_common(MASKED_FRAME_PROBE_LIMIT))
...
def buildModel() -> Model:
    ...
    model = Model(
        ...
        {},  # contrastSemanticUnitAntiSignatures
        {},  # contrastSemanticUnitAntiPostings
        Counter(),  # contrastSemanticUnitAntiAtomDf
        Counter(),  # contrastSemanticUnitAntiStats
    ...
    (
        contrastSemanticUnitAntiSignatures,
        contrastSemanticUnitAntiPostings,
        contrastSemanticUnitAntiAtomDf,
        contrastSemanticUnitAntiStats,
    ) = buildContrastSemanticUnitAntiHead(model, contrastSemanticUnitAntiPairs)
    model.contrastSemanticUnitAntiSignatures = contrastSemanticUnitAntiSignatures
    model.contrastSemanticUnitAntiPostings = contrastSemanticUnitAntiPostings
    model.contrastSemanticUnitAntiAtomDf = contrastSemanticUnitAntiAtomDf
    model.contrastSemanticUnitAntiStats = contrastSemanticUnitAntiStats
    stage("buildContrastSemanticUnitAntiHead")
    (
        maskedFrameSignatures,
        maskedFramePostings,
...
def main() -> None:
    ...
    print(
        ...
        f"antiPairs={model.contrastSemanticUnitStats.get('antiPairs', 0)} "
        f"antiLabels={model.contrastSemanticUnitAntiStats.get('labels', 0)} "
        f"antiRows={model.contrastSemanticUnitAntiStats.get('antiRows', 0)} "
