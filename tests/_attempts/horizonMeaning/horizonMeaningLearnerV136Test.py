"""Horizon Meaning Learner V136 - dual-head agreement semantic-unit gate.

V136 실제 기록
--------------
아이디어:
    V135 는 member-first head 가 holdout labelTop1/Top5 를 올리지만 forbidden member 도 되살린다는 것을
    보여줬다. 따라서 member head 를 단독 label generator 로 쓰면 안 된다. 반대로 V134 의 contrast label head 는
    negative 안정성은 좋지만 unit 내부 어떤 member 가 실제로 활성화되는지 모른다.

    V136 은 두 head 의 역할을 분리한다. 1 차 후보는 V134 contrast label head 가 만든다. member-conditioned
    head 는 후보 생성기가 아니라, contrast 후보 label 안에 실제 member support 가 있는지와 member head 가
    다른 label 로 강하게 disagree 하는지를 재는 agreement/residual gate 로만 쓴다.

    구조적으로는 masked atom -> contrast label score 와 masked atom -> member score 를 동시에 계산한다.
    각 contrast 후보 label 에 대해 member head 의 label mass 가 같은 label 안에 있으면 살리고, top member
    evidence 가 다른 label 로 강하게 쏠리면 candidate label 을 낮춘다. 목표는 V134 의 negative 안정성
    BadTop1=0/7, BadTop5=1/7 을 유지하거나 개선하면서 V135 의 member 복원 신호를 흡수하는 것이다.

실행 코드:
    $env:DARTLAB_HORIZON_V136_MAX_RECORDS_PER_SOURCE=120
    $env:DARTLAB_HORIZON_V136_MAX_UNITS=1200
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV136Test.py

검증 기준:
    1. contrast label head 와 member-conditioned head 를 모두 유지하되 dual-head agreement route 가 별도 생성되어야 한다.
    2. dual-head labelTop5 가 V134 contrast labelTop5=802/896 에서 크게 붕괴하지 않아야 한다.
    3. dual-head negative 가 V134 BadTop1=0/7, BadTop5=1/7 을 유지하거나 개선해야 한다.
    4. positive probe 는 최소 V134 Top5=1/4 수준을 유지해야 한다.
    5. 특정 surface 하드코딩, forbidden 단어 패널티, 케이스별 gate 없이 두 head 의 label/member agreement 만 사용한다.
    6. fixed route/search guard 는 positiveHits=3/4, badAccepted=0/7 수준을 유지해야 한다.

결과:
    1,200 unit smoke 에서 dual-head route 는 별도 생성됐다. contrastSemanticUnitGraph 는
    surfaces=10,115, units=6,392, singletons=5,062, relationProfiles=6,097,
    relationEdges=4,319, experienceEdges=4,936, mixedEdges=122, suffixRejects=1,557 로 생성됐다.
    member head 는 members=10,166, atoms=260,414, rawLinks=913,462, keptLinks=886,401 이었다.

    holdout 은 contrast labelTop1=706/896, labelTop5=802/896, memberConditioned
    labelTop1=723/896, labelTop5=808/896 대비 dualHead labelTop1=699/896,
    labelTop5=803/896, surfaceInTop1Unit=694/896, top5UnitContainsFamily=824/896 였다.
    labelTop5 는 V134 기준 802/896 에서 붕괴하지 않았지만 top1 과 top1 unit member 적중은 낮아졌다.

    semantic probe 는 dualHeadProbeTop1=1/4, Top5=1/4, BadTop1=0/7, BadTop5=2/7 이었다.
    V134 의 BadTop1=0/7 은 유지했지만 BadTop5=1/7 을 유지하지 못했다. fixed route/search guard 는
    positiveHits=3/4, badAccepted=0/7, searchTop1=4/5, reliableSearch=4/5 를 유지했다.
    totalSeconds=202.0.

결론:
    실패/진단 성공. member head 를 label-level agreement 로만 쓰면 holdout Top5 는 유지되지만,
    member head 가 forbidden label 자체를 강하게 support 하는 경우를 막지 못한다. 폐기할 것은
    "candidate label 과 member label mass 가 같으면 살린다" 는 단순 agreement gate 다.
    다음 개념은 member head 가 실제로 활성화한 label 내부 member 들에 대해 V134 의 member-level
    positive/negative evidence balance 를 다시 적용하는 member-supported balance gate 다.
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V136_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V136_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V136_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V136_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V136_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V136_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V136_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V136_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V136_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_SEARCH_CANDIDATE_LIMIT", "420"))
DYNAMIC_TARGET_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_DYNAMIC_TARGET_LIMIT", "80"))
DYNAMIC_COORD_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_DYNAMIC_COORD_ROW_LIMIT", "220"))
DYNAMIC_COMPOUND_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_DYNAMIC_COMPOUND_ROW_LIMIT", "260"))
DYNAMIC_QUERY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_DYNAMIC_QUERY_ATOM_LIMIT", "48"))
DYNAMIC_MEANING_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_DYNAMIC_MEANING_ATOM_LIMIT", "36"))
DYNAMIC_MEANING_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_DYNAMIC_MEANING_ROW_LIMIT", "220"))
DYNAMIC_RELATION_SURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_DYNAMIC_RELATION_SURFACE_LIMIT", "420"))
DYNAMIC_RELATION_UNIT_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_DYNAMIC_RELATION_UNIT_LIMIT", "160"))
DYNAMIC_BRIDGE_ONLY_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V136_DYNAMIC_BRIDGE_ONLY_PENALTY", "0.80"))
DYNAMIC_OWNER_ROLE_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V136_DYNAMIC_OWNER_ROLE_SIGNATURE_LIMIT", "64")
)
DYNAMIC_OWNER_ROLE_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_DYNAMIC_OWNER_ROLE_ATOM_LIMIT", "40"))
DYNAMIC_OWNER_ROLE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_DYNAMIC_OWNER_ROLE_ROW_LIMIT", "220"))
DYNAMIC_OWNER_ROLE_UNIT_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V136_DYNAMIC_OWNER_ROLE_UNIT_ATOM_LIMIT", "36")
)
DYNAMIC_OWNER_ROLE_MIN_BOUND = float(os.environ.get("DARTLAB_HORIZON_V136_DYNAMIC_OWNER_ROLE_MIN_BOUND", "0.55"))
DYNAMIC_OWNER_ROLE_CANDIDATE_BONUS = float(
    os.environ.get("DARTLAB_HORIZON_V136_DYNAMIC_OWNER_ROLE_CANDIDATE_BONUS", "2.40")
)
DYNAMIC_OWNER_ROLE_ROUTE_BONUS = float(os.environ.get("DARTLAB_HORIZON_V136_DYNAMIC_OWNER_ROLE_ROUTE_BONUS", "0.90"))
DYNAMIC_OWNER_ROLE_WEAK_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V136_DYNAMIC_OWNER_ROLE_WEAK_PENALTY", "0.16"))
DYNAMIC_OWNER_FRAME_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V136_DYNAMIC_OWNER_FRAME_RADIUS", "5"))
DYNAMIC_OWNER_FRAME_BETWEEN_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_DYNAMIC_OWNER_FRAME_BETWEEN_LIMIT", "7"))
MASKED_FRAME_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_MASKED_FRAME_SIGNATURE_LIMIT", "72"))
MASKED_FRAME_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_MASKED_FRAME_ATOM_LIMIT", "48"))
MASKED_FRAME_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_MASKED_FRAME_ROW_LIMIT", "240"))
MASKED_FRAME_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_MASKED_FRAME_EVAL_LIMIT", "800"))
MASKED_FRAME_HOLDOUT_MOD = int(os.environ.get("DARTLAB_HORIZON_V136_MASKED_FRAME_HOLDOUT_MOD", "5"))
MASKED_FRAME_PROBE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_MASKED_FRAME_PROBE_LIMIT", "12"))
MASKED_CONTEXT_SURFACE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_MASKED_CONTEXT_SURFACE_ROW_LIMIT", "12"))
MASKED_CONTEXT_WEIGHT = float(os.environ.get("DARTLAB_HORIZON_V136_MASKED_CONTEXT_WEIGHT", "0.35"))
MASKED_CONTEXT_RELATION_WINDOW = int(os.environ.get("DARTLAB_HORIZON_V136_MASKED_CONTEXT_RELATION_WINDOW", "140"))
MASKED_FRAME_SURPRISAL_POWER = float(os.environ.get("DARTLAB_HORIZON_V136_MASKED_FRAME_SURPRISAL_POWER", "2.0"))
MASKED_FRAME_SURFACE_PRIOR_POWER = float(
    os.environ.get("DARTLAB_HORIZON_V136_MASKED_FRAME_SURFACE_PRIOR_POWER", "0.45")
)
RELATION_OWNER_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_RELATION_OWNER_SIGNATURE_LIMIT", "64"))
RELATION_OWNER_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_RELATION_OWNER_ROW_LIMIT", "220"))
RELATION_OWNER_BRIDGE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_RELATION_OWNER_BRIDGE_LIMIT", "12"))
RELATION_OWNER_PROJECTION_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_RELATION_OWNER_PROJECTION_LIMIT", "24"))
RELATION_OWNER_PROJECTION_OWNER_ROLE_MIN = float(
    os.environ.get("DARTLAB_HORIZON_V136_RELATION_OWNER_PROJECTION_OWNER_ROLE_MIN", "0.18")
)
RELATION_OWNER_PROJECTION_QUERY_ROLE_MIN = float(
    os.environ.get("DARTLAB_HORIZON_V136_RELATION_OWNER_PROJECTION_QUERY_ROLE_MIN", "0.05")
)
RELATION_OWNER_GENERATED_BRIDGE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V136_RELATION_OWNER_GENERATED_BRIDGE_LIMIT", "48")
)
RELATION_OWNER_GENERATED_BRIDGE_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V136_RELATION_OWNER_GENERATED_BRIDGE_WEIGHT", "2.4")
)
RELATION_OWNER_ROLE_COMPAT_MIN = float(os.environ.get("DARTLAB_HORIZON_V136_RELATION_OWNER_ROLE_COMPAT_MIN", "0.16"))
RELATION_BOUND_ROLE_PAIR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_RELATION_BOUND_ROLE_PAIR_LIMIT", "24"))
RELATION_BOUND_ROLE_PAIR_BRIDGE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V136_RELATION_BOUND_ROLE_PAIR_BRIDGE_LIMIT", "10")
)
RELATION_BOUND_ROLE_PAIR_BRIDGE_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V136_RELATION_BOUND_ROLE_PAIR_BRIDGE_WEIGHT", "0.46")
)
RELATION_SOURCE_EXPERIENCE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V136_RELATION_SOURCE_EXPERIENCE_ATOM_LIMIT", "44")
)
RELATION_SOURCE_EXPERIENCE_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V136_RELATION_SOURCE_EXPERIENCE_ROW_LIMIT", "180")
)
RELATION_SOURCE_SHINGLE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V136_RELATION_SOURCE_SHINGLE_ATOM_LIMIT", "28")
)
RELATION_SOURCE_SHINGLE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_RELATION_SOURCE_SHINGLE_ROW_LIMIT", "160"))
RELATION_SOURCE_SHINGLE_KEY_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_RELATION_SOURCE_SHINGLE_KEY_LIMIT", "56"))
RELATION_ANCHORED_COARSE_SHINGLE_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V136_RELATION_ANCHORED_COARSE_SHINGLE_ROW_LIMIT", "180")
)
RELATION_ANCHORED_COARSE_SHINGLE_KEY_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V136_RELATION_ANCHORED_COARSE_SHINGLE_KEY_LIMIT", "72")
)
HORIZON_MASK_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_HORIZON_MASK_SIGNATURE_LIMIT", "96"))
HORIZON_MASK_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_HORIZON_MASK_ATOM_LIMIT", "56"))
HORIZON_MASK_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_HORIZON_MASK_ROW_LIMIT", "220"))
HORIZON_MASK_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_HORIZON_MASK_EVAL_LIMIT", "900"))
HORIZON_MASK_HOLDOUT_MOD = int(os.environ.get("DARTLAB_HORIZON_V136_HORIZON_MASK_HOLDOUT_MOD", "7"))
HORIZON_MASK_SURFACE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_HORIZON_MASK_SURFACE_ROW_LIMIT", "24"))
HORIZON_MASK_NEIGHBOR_EXPERIENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V136_HORIZON_MASK_NEIGHBOR_EXPERIENCE_LIMIT", "5")
)
HORIZON_MASK_COORD_CELL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_HORIZON_MASK_COORD_CELL_LIMIT", "5"))
HORIZON_MASK_COORD_BUCKET_DIGITS = int(os.environ.get("DARTLAB_HORIZON_V136_HORIZON_MASK_COORD_BUCKET_DIGITS", "5"))
SEMANTIC_UNIT_SIGNATURE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V136_SEMANTIC_UNIT_SIGNATURE_ATOM_LIMIT", "18")
)
SEMANTIC_UNIT_POSTING_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_SEMANTIC_UNIT_POSTING_ROW_LIMIT", "90"))
SEMANTIC_UNIT_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_SEMANTIC_UNIT_CANDIDATE_LIMIT", "180"))
SEMANTIC_UNIT_MIN_EXPERIENCE_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V136_SEMANTIC_UNIT_MIN_EXPERIENCE_SIM", "0.46")
)
SEMANTIC_UNIT_MIN_MIXED_SIM = float(os.environ.get("DARTLAB_HORIZON_V136_SEMANTIC_UNIT_MIN_MIXED_SIM", "0.34"))
SEMANTIC_UNIT_MIN_COORD_RESONANCE = float(
    os.environ.get("DARTLAB_HORIZON_V136_SEMANTIC_UNIT_MIN_COORD_RESONANCE", "0.045")
)
SEMANTIC_UNIT_MASK_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V136_SEMANTIC_UNIT_MASK_SIGNATURE_LIMIT", "120")
)
SEMANTIC_UNIT_MASK_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_SEMANTIC_UNIT_MASK_ATOM_LIMIT", "64"))
SEMANTIC_UNIT_MASK_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_SEMANTIC_UNIT_MASK_ROW_LIMIT", "260"))
SEMANTIC_UNIT_ROUTE_MEMBER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_SEMANTIC_UNIT_ROUTE_MEMBER_LIMIT", "4"))
CONTRAST_SEMANTIC_UNIT_MIN_EXPERIENCE_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V136_CONTRAST_SEMANTIC_UNIT_MIN_EXPERIENCE_SIM", "0.46")
)
CONTRAST_SEMANTIC_UNIT_STRONG_EXPERIENCE_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V136_CONTRAST_SEMANTIC_UNIT_STRONG_EXPERIENCE_SIM", "0.62")
)
CONTRAST_SEMANTIC_UNIT_MIN_MIXED_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V136_CONTRAST_SEMANTIC_UNIT_MIN_MIXED_SIM", "0.36")
)
CONTRAST_SEMANTIC_UNIT_MIN_COORD_RESONANCE = float(
    os.environ.get("DARTLAB_HORIZON_V136_CONTRAST_SEMANTIC_UNIT_MIN_COORD_RESONANCE", "0.050")
)
CONTRAST_SEMANTIC_UNIT_MIN_RELATION_COMPAT = float(
    os.environ.get("DARTLAB_HORIZON_V136_CONTRAST_SEMANTIC_UNIT_MIN_RELATION_COMPAT", "0.28")
)
CONTRAST_SEMANTIC_UNIT_SUFFIX_RELATION_COMPAT = float(
    os.environ.get("DARTLAB_HORIZON_V136_CONTRAST_SEMANTIC_UNIT_SUFFIX_RELATION_COMPAT", "0.18")
)
MEMBER_BALANCE_MEMBER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_MEMBER_BALANCE_MEMBER_LIMIT", "10"))
MEMBER_BALANCE_MIN_MULTIPLIER = float(os.environ.get("DARTLAB_HORIZON_V136_MEMBER_BALANCE_MIN_MULTIPLIER", "0.015"))
MEMBER_BALANCE_UNSUPPORTED_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V136_MEMBER_BALANCE_UNSUPPORTED_FACTOR", "0.12")
)
MEMBER_BALANCE_POSITIVE_GAIN = float(os.environ.get("DARTLAB_HORIZON_V136_MEMBER_BALANCE_POSITIVE_GAIN", "1.10"))
MEMBER_BALANCE_NEGATIVE_GAIN = float(os.environ.get("DARTLAB_HORIZON_V136_MEMBER_BALANCE_NEGATIVE_GAIN", "1.60"))
MEMBER_BALANCE_POSITIVE_SHIELD_RATIO = float(
    os.environ.get("DARTLAB_HORIZON_V136_MEMBER_BALANCE_POSITIVE_SHIELD_RATIO", "0.85")
)
...
MEMBER_CONDITIONED_MASK_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V136_MEMBER_CONDITIONED_MASK_SIGNATURE_LIMIT", "96")
)
MEMBER_CONDITIONED_MASK_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V136_MEMBER_CONDITIONED_MASK_ATOM_LIMIT", "64")
)
MEMBER_CONDITIONED_MASK_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_MEMBER_CONDITIONED_MASK_ROW_LIMIT", "220"))
MEMBER_CONDITIONED_ROUTE_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V136_MEMBER_CONDITIONED_ROUTE_MEMBER_LIMIT", "180")
)
MEMBER_CONDITIONED_LABEL_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V136_MEMBER_CONDITIONED_LABEL_MEMBER_LIMIT", "6")
)
MEMBER_CONDITIONED_MEMBER_CACHE: dict[tuple[int, str, str], tuple[float, float, float, int]] = {}
DUAL_HEAD_AGREEMENT_LABEL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_DUAL_HEAD_AGREEMENT_LABEL_LIMIT", "80"))
DUAL_HEAD_MEMBER_LABEL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_DUAL_HEAD_MEMBER_LABEL_LIMIT", "80"))
DUAL_HEAD_DISAGREEMENT_RATIO = float(os.environ.get("DARTLAB_HORIZON_V136_DUAL_HEAD_DISAGREEMENT_RATIO", "1.18"))
DUAL_HEAD_STRONG_DISAGREEMENT_RATIO = float(
    os.environ.get("DARTLAB_HORIZON_V136_DUAL_HEAD_STRONG_DISAGREEMENT_RATIO", "1.65")
)
DUAL_HEAD_MIN_MULTIPLIER = float(os.environ.get("DARTLAB_HORIZON_V136_DUAL_HEAD_MIN_MULTIPLIER", "0.18"))
DUAL_HEAD_SOFT_MULTIPLIER = float(os.environ.get("DARTLAB_HORIZON_V136_DUAL_HEAD_SOFT_MULTIPLIER", "0.62"))
DUAL_HEAD_AGREEMENT_BOOST = float(os.environ.get("DARTLAB_HORIZON_V136_DUAL_HEAD_AGREEMENT_BOOST", "0.10"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V136_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V136_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V136_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V136_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V136_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V136_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V136_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V136_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
LANE_MISMATCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V136_LANE_MISMATCH_PENALTY", "0.18"))
LANE_ARTIFACT_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V136_LANE_ARTIFACT_PENALTY", "0.10"))
NEAREST_ORDER_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_NEAREST_ORDER_SIGNATURE_LIMIT", "24"))
NEAREST_ORDER_PENALTY_MIN = float(os.environ.get("DARTLAB_HORIZON_V136_NEAREST_ORDER_PENALTY_MIN", "0.05"))
NEAREST_ORDER_PENALTY_SCALE = float(os.environ.get("DARTLAB_HORIZON_V136_NEAREST_ORDER_PENALTY_SCALE", "0.16"))
NEAREST_ORDER_COHORT_SURFACE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V136_NEAREST_ORDER_COHORT_SURFACE_LIMIT", "384")
)
NEAREST_ORDER_COHORT_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V136_NEAREST_ORDER_COHORT_POSITION_LIMIT", "8192")
)
NEAREST_ORDER_SURFACE_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V136_NEAREST_ORDER_SURFACE_POSITION_LIMIT", "48")
)
SUFFIX_ANCHOR_SUPPORT_MIN = int(os.environ.get("DARTLAB_HORIZON_V136_SUFFIX_ANCHOR_SUPPORT_MIN", "1"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V136_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V136_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V136_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V136_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V136_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V136_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V136_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V136_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V136_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V136_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V136_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V136_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V136_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V136_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V136_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V136_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V136_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V136_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V136_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V136_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V136_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V136_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V136_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
RELAY_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_RELAY_ROW_LIMIT", "160"))
RELAY_SPECIFIC_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_RELAY_SPECIFIC_ROW_LIMIT", "320"))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V136_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V136_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V136_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V136_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V136_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V136_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V136_SKETCH_OCC_BUCKETS", "16"))
...
def dualHeadMemberLabelScores(memberScores: Counter[str], model: Model) -> Counter[str]:
    labels: Counter[str] = Counter()
    labelMembers: Counter[str] = Counter()
    for member, score in memberScores.most_common(DUAL_HEAD_MEMBER_LABEL_LIMIT):
        label = model.contrastSemanticUnitLabels.get(member, "")
        if not label:
            label = contrastSemanticUnitLabelForSurface(member, model)
        if not label:
        ...
        if labelMembers[label] >= MEMBER_CONDITIONED_LABEL_MEMBER_LIMIT:
            continue
        labels[label] += float(score) / (1.0 + 0.20 * labelMembers[label])
        labelMembers[label] += 1
    return labels


def dualHeadAgreementMultiplier(
    label: str,
    memberLabelScores: Counter[str],
) -> tuple[float, float, float, str]:
    if not label or not memberLabelScores:
        return 1.0, 0.0, 0.0, ""
    support = float(memberLabelScores.get(label, 0.0))
    competitorLabel = ""
    competitorScore = 0.0
    for otherLabel, score in memberLabelScores.most_common(4):
        if otherLabel == label:
            continue
        competitorLabel = otherLabel
        competitorScore = float(score)
        break
    if competitorScore <= 0.0:
        return 1.0 + min(DUAL_HEAD_AGREEMENT_BOOST, support * 0.01), support, 0.0, ""
    if support <= 0.0:
        return DUAL_HEAD_MIN_MULTIPLIER, support, competitorScore, competitorLabel
    ratio = competitorScore / max(support, 1e-9)
    if ratio >= DUAL_HEAD_STRONG_DISAGREEMENT_RATIO:
        return DUAL_HEAD_MIN_MULTIPLIER, support, competitorScore, competitorLabel
    if ratio >= DUAL_HEAD_DISAGREEMENT_RATIO:
        return DUAL_HEAD_SOFT_MULTIPLIER, support, competitorScore, competitorLabel
    return (
        1.0 + min(DUAL_HEAD_AGREEMENT_BOOST, support / (support + competitorScore + 1.0) * DUAL_HEAD_AGREEMENT_BOOST),
        support,
        competitorScore,
        competitorLabel,
    )
...
def contrastSemanticUnitDualHeadAgreementPredictFromAtoms(
    atoms: Counter[str],
    model: Model,
    sourceSurface: str = "",
) -> Counter[str]:
    ...
    if not labelScores:
        return Counter()
    memberScores = contrastSemanticUnitMemberMaskedPredictMembersFromAtoms(atoms, model, sourceSurface=sourceSurface)
    memberLabelScores = dualHeadMemberLabelScores(memberScores, model)
    gated: Counter[str] = Counter()
    for label, score in labelScores.most_common(DUAL_HEAD_AGREEMENT_LABEL_LIMIT):
        multiplier, _, _, _ = dualHeadAgreementMultiplier(label, memberLabelScores)
        gated[label] = float(score) * multiplier
    return gated


def contrastSemanticUnitDualHeadAgreementPredictionMetrics(model: Model) -> Counter[str]:
    metrics: Counter[str] = Counter()
    for surface, atoms in model.horizonMaskedEvalRows:
        targetLabel = contrastSemanticUnitLabelForSurface(surface, model)
        if not targetLabel:
            metrics["missingLabel"] += 1
            continue
        predictions = contrastSemanticUnitDualHeadAgreementPredictFromAtoms(atoms, model, sourceSurface=surface)
        if not predictions:
            metrics["empty"] += 1
            continue
        ranked = [label for label, _ in predictions.most_common(5)]
...
def contrastSemanticUnitDualHeadAgreementLabelRoute(surface: str, model: Model) -> tuple[tuple[str, float], ...]:
    signature = inferHorizonMaskedSignature(surface, model)
    predictions = contrastSemanticUnitDualHeadAgreementPredictFromAtoms(signature, model, sourceSurface=surface)
    return tuple(predictions.most_common(MASKED_FRAME_PROBE_LIMIT))


def contrastSemanticUnitDualHeadAgreementSurfaceRoute(surface: str, model: Model) -> tuple[tuple[str, float], ...]:
    scores: Counter[str] = Counter()
    for label, labelScore in contrastSemanticUnitDualHeadAgreementLabelRoute(surface, model):
        members = model.contrastSemanticUnitMembers.get(label, tuple())
        for offset, member in enumerate(members[:SEMANTIC_UNIT_ROUTE_MEMBER_LIMIT]):
            scores[member] = max(scores.get(member, 0.0), float(labelScore) / (1.0 + offset * 0.15))
    return tuple(scores.most_common(MASKED_FRAME_PROBE_LIMIT))
...
def dualHeadAgreementDiagnostics(surface: str, label: str, model: Model) -> tuple[float, float, float, str]:
    signature = inferHorizonMaskedSignature(surface, model)
    memberScores = contrastSemanticUnitMemberMaskedPredictMembersFromAtoms(signature, model, sourceSurface=surface)
    memberLabelScores = dualHeadMemberLabelScores(memberScores, model)
    return dualHeadAgreementMultiplier(label, memberLabelScores)
...
def main() -> None:
    ...
    dualHeadMetrics = contrastSemanticUnitDualHeadAgreementPredictionMetrics(model)
    dualHeadEvaluated = max(1, dualHeadMetrics.get("evaluated", 0))
    print(
        f"[dualHeadSemanticUnitEval] graphSurfaces={model.contrastSemanticUnitStats.get('surfaces', 0)} "
        f"units={model.contrastSemanticUnitStats.get('units', 0)} "
        f"memberMaskedMembers={model.contrastSemanticUnitMemberMaskedStats.get('members', 0)} "
        f"evaluated={dualHeadMetrics.get('evaluated', 0)} "
        f"empty={dualHeadMetrics.get('empty', 0)} "
        f"missingLabel={dualHeadMetrics.get('missingLabel', 0)} "
        f"labelTop1={dualHeadMetrics.get('labelTop1', 0)}/{dualHeadEvaluated} "
        f"labelTop5={dualHeadMetrics.get('labelTop5', 0)}/{dualHeadEvaluated} "
        f"surfaceInTop1Unit={dualHeadMetrics.get('surfaceInTop1Unit', 0)}/{dualHeadEvaluated} "
        f"top5UnitContainsFamily={dualHeadMetrics.get('top5UnitContainsFamily', 0)}/{dualHeadEvaluated}"
    )
    ...
    dualHeadProbeTop1 = 0
    dualHeadProbeTop5 = 0
    dualHeadProbeBadTop1 = 0
    dualHeadProbeBadTop5 = 0
    print("[dualHeadSemanticUnitProbes:positive]")
    for surface, expected in POSITIVE_PROBES:
        labelRows = contrastSemanticUnitDualHeadAgreementLabelRoute(surface, model)
        surfaceRows = contrastSemanticUnitDualHeadAgreementSurfaceRoute(surface, model)
        expectedLabel = contrastSemanticUnitLabelForSurface(expected, model)
        labelRank = semanticUnitRank(labelRows, expectedLabel)
        surfaceRank = maskedFrameRank(surfaceRows, expected)
        agreement = dualHeadAgreementDiagnostics(surface, expectedLabel, model)
        dualHeadProbeTop1 += int(labelRank == 1)
        dualHeadProbeTop5 += int(labelRank is not None and labelRank <= 5)
        print(
            f"  {surface}->{expected} expectedLabelRank={labelRank if labelRank is not None else 'NA'} "
            f"surfaceRank={surfaceRank if surfaceRank is not None else 'NA'} "
            f"agreementMul={agreement[0]:.2f}/support{agreement[1]:.1f}/comp{agreement[2]:.1f}/{agreement[3]} "
            f"labels={previewContrastSemanticUnitLabels(labelRows, model)} surfaces={previewMaskedFrame(surfaceRows)}"
        )
    print("[dualHeadSemanticUnitProbes:negative]")
    for surface, forbidden in NEGATIVE_PROBES:
        labelRows = contrastSemanticUnitDualHeadAgreementLabelRoute(surface, model)
        surfaceRows = contrastSemanticUnitDualHeadAgreementSurfaceRoute(surface, model)
        forbiddenLabel = contrastSemanticUnitLabelForSurface(forbidden, model)
        ...
        surfaceRank = maskedFrameRank(surfaceRows, forbidden)
        topForbidden = bool(labelRows and labelRows[0][0] == forbiddenLabel and forbiddenLabel)
        agreement = dualHeadAgreementDiagnostics(surface, forbiddenLabel, model)
        dualHeadProbeBadTop1 += int(topForbidden)
        dualHeadProbeBadTop5 += int(labelRank is not None and labelRank <= 5)
        print(
            f"  {surface}-/->{forbidden} forbiddenLabelRank={labelRank if labelRank is not None else 'NA'} "
            f"surfaceRank={surfaceRank if surfaceRank is not None else 'NA'} badTop1={topForbidden} "
            f"agreementMul={agreement[0]:.2f}/support{agreement[1]:.1f}/comp{agreement[2]:.1f}/{agreement[3]} "
    ...
    print(
        ...
        f"dualHeadLabelTop1={dualHeadMetrics.get('labelTop1', 0)}/{max(1, dualHeadMetrics.get('evaluated', 0))} "
        f"dualHeadLabelTop5={dualHeadMetrics.get('labelTop5', 0)}/{max(1, dualHeadMetrics.get('evaluated', 0))} "
        f"dualHeadProbeTop1={dualHeadProbeTop1}/{len(POSITIVE_PROBES)} dualHeadProbeTop5={dualHeadProbeTop5}/{len(POSITIVE_PROBES)} "
        f"dualHeadProbeBadTop1={dualHeadProbeBadTop1}/{len(NEGATIVE_PROBES)} dualHeadProbeBadTop5={dualHeadProbeBadTop5}/{len(NEGATIVE_PROBES)} "
