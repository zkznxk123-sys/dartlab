"""Horizon Meaning Learner V138 - member-role residual semantic-unit graph.

V138 실제 기록
--------------
아이디어:
    V137 은 member head 를 boost 가 아니라 conflict veto 로만 쓰면 V134 안정성을 보존한다는 것을 보였다.
    하지만 의미 전이는 늘지 않았다. 이유는 같은 semantic unit 안의 member 를 모두 같은 의미 중심으로 보기
    때문이다. `대손충당금/대손충당/손충당금` 처럼 같은 unit 안에도 중심 member, 표 조각 fragment,
    metric/owner/context 역할이 섞인다.

    V138 은 unit label 만 맞히는 head 에서 한 단계 올려, 중심 stem 을 숨긴 local frame 으로
    `(contrast semantic-unit label, member role)` 을 같이 예측한다. member role 은 수동 단어 목록이 아니라
    surface lane profile, owner-role specificity, independent/bridge origin, 같은 unit 안의 generic
    containment 관계로 owner/metric/modifier/fragment 를 자동 부여한다.

    검색 route 에서는 V134 contrast label score 를 1 차 후보로 유지하고, 별도 role-residual head 가
    같은 label 안에서 source role 을 지지하는지 확인한다. candidate label 이 맞아도 role head 가 같은 label 의
    다른 role 만 강하게 예측하면 residual mismatch 로 낮춘다. 목표는 단순 member veto 를 넘어 "같은 unit 내부
    어떤 member 역할이 의미 중심인가" 를 sparse 경험 그래프가 압축할 수 있는지 검증하는 것이다.

실행 코드:
    $env:DARTLAB_HORIZON_V138_MAX_RECORDS_PER_SOURCE=120
    $env:DARTLAB_HORIZON_V138_MAX_UNITS=1200
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV138Test.py

검증 기준:
    1. contrast semantic-unit label 과 member role 을 결합한 별도 role-residual masked head 가 생성되어야 한다.
    2. role-residual roleKeyTop5 와 collapsed labelTop5 를 모두 기록해야 한다.
    3. role-residual route labelTop5 가 V134 contrast labelTop5=802/896 에서 크게 붕괴하지 않아야 한다.
    4. role-residual negative 가 V134 BadTop1=0/7, BadTop5=1/7 을 유지하거나 개선해야 한다.
    5. positive probe 는 최소 V134 Top5=1/4 수준을 유지해야 한다.
    6. 특정 surface 하드코딩, forbidden 단어 패널티, 케이스별 gate 없이 role 자동 부여와 role residual 만 사용한다.
    7. fixed route/search guard 는 positiveHits=3/4, badAccepted=0/7 수준을 유지해야 한다.

결과:
    1,200 unit smoke 에서 memberRoleResidual head 는 roleKeys=8,839, surfaceRows=11,391,
    roles=owner:928/metric:1,229/modifier:5,753/fragment:3,481 로 생성됐다.
    같은 실행 안의 contrastSemanticUnit 은 labelTop1=607/884, labelTop5=708/884 였고,
    memberSupported 는 같은 607/884, 708/884 로 V137 과 같이 안정성 보존 역할만 했다.

    memberRoleResidual 자체는 roleKeyTop1=440/884, roleKeyTop5=677/884,
    roleCollapsedLabelTop1=590/884, roleCollapsedLabelTop5=700/884 였다. 이를 residual route 로
    얹으면 labelTop1=621/884, labelTop5=722/884 로 같은 실행의 contrast/memberSupported 보다
    각각 +14, +14 개선됐고, top5UnitContainsFamily 도 758/884 로 contrast 745/884 보다 높았다.

    하지만 manual semantic probe 는 개선되지 않았다. contrast/memberSupported 가 Top1=1/4,
    Top5=1/4, BadTop1=0/7, BadTop5=1/7 인 반면 memberRoleResidual route 는 Top1=0/4,
    Top5=1/4, BadTop1=0/7, BadTop5=1/7 이었다. fixed route/search guard 는
    positiveHits=3/4, badAccepted=0/7, searchTop1=4/5, reliableSearch=4/5 를 유지했다.
    totalSeconds=306.9.

결론:
    부분 성공/진단 성공. roleKeyTop5=677/884 와 route labelTop1/Top5 개선은
    "unit 내부 member role 경험" 이 실제 구조 신호임을 보여준다. 그러나 direct residual veto 를
    검색 route 에 붙이면 rare manual probe 의 Top1 이 1/4 에서 0/4 로 떨어진다. 따라서 label-level
    role mismatch veto 는 최종 route 로 폐기한다. 다음 개념은 role residual 을 label 을 깎는 용도가
    아니라, candidate unit 내부에서 어떤 member 를 대표로 비교할지 고르는 target-member selector 또는
    role-conditioned member ranking 으로 쓰는 것이다.
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V138_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V138_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V138_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V138_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V138_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V138_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V138_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V138_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V138_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_SEARCH_CANDIDATE_LIMIT", "420"))
DYNAMIC_TARGET_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_DYNAMIC_TARGET_LIMIT", "80"))
DYNAMIC_COORD_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_DYNAMIC_COORD_ROW_LIMIT", "220"))
DYNAMIC_COMPOUND_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_DYNAMIC_COMPOUND_ROW_LIMIT", "260"))
DYNAMIC_QUERY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_DYNAMIC_QUERY_ATOM_LIMIT", "48"))
DYNAMIC_MEANING_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_DYNAMIC_MEANING_ATOM_LIMIT", "36"))
DYNAMIC_MEANING_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_DYNAMIC_MEANING_ROW_LIMIT", "220"))
DYNAMIC_RELATION_SURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_DYNAMIC_RELATION_SURFACE_LIMIT", "420"))
DYNAMIC_RELATION_UNIT_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_DYNAMIC_RELATION_UNIT_LIMIT", "160"))
DYNAMIC_BRIDGE_ONLY_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V138_DYNAMIC_BRIDGE_ONLY_PENALTY", "0.80"))
DYNAMIC_OWNER_ROLE_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V138_DYNAMIC_OWNER_ROLE_SIGNATURE_LIMIT", "64")
)
DYNAMIC_OWNER_ROLE_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_DYNAMIC_OWNER_ROLE_ATOM_LIMIT", "40"))
DYNAMIC_OWNER_ROLE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_DYNAMIC_OWNER_ROLE_ROW_LIMIT", "220"))
DYNAMIC_OWNER_ROLE_UNIT_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V138_DYNAMIC_OWNER_ROLE_UNIT_ATOM_LIMIT", "36")
)
DYNAMIC_OWNER_ROLE_MIN_BOUND = float(os.environ.get("DARTLAB_HORIZON_V138_DYNAMIC_OWNER_ROLE_MIN_BOUND", "0.55"))
DYNAMIC_OWNER_ROLE_CANDIDATE_BONUS = float(
    os.environ.get("DARTLAB_HORIZON_V138_DYNAMIC_OWNER_ROLE_CANDIDATE_BONUS", "2.40")
)
DYNAMIC_OWNER_ROLE_ROUTE_BONUS = float(os.environ.get("DARTLAB_HORIZON_V138_DYNAMIC_OWNER_ROLE_ROUTE_BONUS", "0.90"))
DYNAMIC_OWNER_ROLE_WEAK_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V138_DYNAMIC_OWNER_ROLE_WEAK_PENALTY", "0.16"))
DYNAMIC_OWNER_FRAME_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V138_DYNAMIC_OWNER_FRAME_RADIUS", "5"))
DYNAMIC_OWNER_FRAME_BETWEEN_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_DYNAMIC_OWNER_FRAME_BETWEEN_LIMIT", "7"))
MASKED_FRAME_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_MASKED_FRAME_SIGNATURE_LIMIT", "72"))
MASKED_FRAME_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_MASKED_FRAME_ATOM_LIMIT", "48"))
MASKED_FRAME_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_MASKED_FRAME_ROW_LIMIT", "240"))
MASKED_FRAME_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_MASKED_FRAME_EVAL_LIMIT", "800"))
MASKED_FRAME_HOLDOUT_MOD = int(os.environ.get("DARTLAB_HORIZON_V138_MASKED_FRAME_HOLDOUT_MOD", "5"))
MASKED_FRAME_PROBE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_MASKED_FRAME_PROBE_LIMIT", "12"))
MASKED_CONTEXT_SURFACE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_MASKED_CONTEXT_SURFACE_ROW_LIMIT", "12"))
MASKED_CONTEXT_WEIGHT = float(os.environ.get("DARTLAB_HORIZON_V138_MASKED_CONTEXT_WEIGHT", "0.35"))
MASKED_CONTEXT_RELATION_WINDOW = int(os.environ.get("DARTLAB_HORIZON_V138_MASKED_CONTEXT_RELATION_WINDOW", "140"))
MASKED_FRAME_SURPRISAL_POWER = float(os.environ.get("DARTLAB_HORIZON_V138_MASKED_FRAME_SURPRISAL_POWER", "2.0"))
MASKED_FRAME_SURFACE_PRIOR_POWER = float(
    os.environ.get("DARTLAB_HORIZON_V138_MASKED_FRAME_SURFACE_PRIOR_POWER", "0.45")
)
RELATION_OWNER_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_RELATION_OWNER_SIGNATURE_LIMIT", "64"))
RELATION_OWNER_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_RELATION_OWNER_ROW_LIMIT", "220"))
RELATION_OWNER_BRIDGE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_RELATION_OWNER_BRIDGE_LIMIT", "12"))
RELATION_OWNER_PROJECTION_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_RELATION_OWNER_PROJECTION_LIMIT", "24"))
RELATION_OWNER_PROJECTION_OWNER_ROLE_MIN = float(
    os.environ.get("DARTLAB_HORIZON_V138_RELATION_OWNER_PROJECTION_OWNER_ROLE_MIN", "0.18")
)
RELATION_OWNER_PROJECTION_QUERY_ROLE_MIN = float(
    os.environ.get("DARTLAB_HORIZON_V138_RELATION_OWNER_PROJECTION_QUERY_ROLE_MIN", "0.05")
)
RELATION_OWNER_GENERATED_BRIDGE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V138_RELATION_OWNER_GENERATED_BRIDGE_LIMIT", "48")
)
RELATION_OWNER_GENERATED_BRIDGE_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V138_RELATION_OWNER_GENERATED_BRIDGE_WEIGHT", "2.4")
)
RELATION_OWNER_ROLE_COMPAT_MIN = float(os.environ.get("DARTLAB_HORIZON_V138_RELATION_OWNER_ROLE_COMPAT_MIN", "0.16"))
RELATION_BOUND_ROLE_PAIR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_RELATION_BOUND_ROLE_PAIR_LIMIT", "24"))
RELATION_BOUND_ROLE_PAIR_BRIDGE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V138_RELATION_BOUND_ROLE_PAIR_BRIDGE_LIMIT", "10")
)
RELATION_BOUND_ROLE_PAIR_BRIDGE_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V138_RELATION_BOUND_ROLE_PAIR_BRIDGE_WEIGHT", "0.46")
)
RELATION_SOURCE_EXPERIENCE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V138_RELATION_SOURCE_EXPERIENCE_ATOM_LIMIT", "44")
)
RELATION_SOURCE_EXPERIENCE_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V138_RELATION_SOURCE_EXPERIENCE_ROW_LIMIT", "180")
)
RELATION_SOURCE_SHINGLE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V138_RELATION_SOURCE_SHINGLE_ATOM_LIMIT", "28")
)
RELATION_SOURCE_SHINGLE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_RELATION_SOURCE_SHINGLE_ROW_LIMIT", "160"))
RELATION_SOURCE_SHINGLE_KEY_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_RELATION_SOURCE_SHINGLE_KEY_LIMIT", "56"))
RELATION_ANCHORED_COARSE_SHINGLE_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V138_RELATION_ANCHORED_COARSE_SHINGLE_ROW_LIMIT", "180")
)
RELATION_ANCHORED_COARSE_SHINGLE_KEY_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V138_RELATION_ANCHORED_COARSE_SHINGLE_KEY_LIMIT", "72")
)
HORIZON_MASK_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_HORIZON_MASK_SIGNATURE_LIMIT", "96"))
HORIZON_MASK_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_HORIZON_MASK_ATOM_LIMIT", "56"))
HORIZON_MASK_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_HORIZON_MASK_ROW_LIMIT", "220"))
HORIZON_MASK_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_HORIZON_MASK_EVAL_LIMIT", "900"))
HORIZON_MASK_HOLDOUT_MOD = int(os.environ.get("DARTLAB_HORIZON_V138_HORIZON_MASK_HOLDOUT_MOD", "7"))
HORIZON_MASK_SURFACE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_HORIZON_MASK_SURFACE_ROW_LIMIT", "24"))
HORIZON_MASK_NEIGHBOR_EXPERIENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V138_HORIZON_MASK_NEIGHBOR_EXPERIENCE_LIMIT", "5")
)
HORIZON_MASK_COORD_CELL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_HORIZON_MASK_COORD_CELL_LIMIT", "5"))
HORIZON_MASK_COORD_BUCKET_DIGITS = int(os.environ.get("DARTLAB_HORIZON_V138_HORIZON_MASK_COORD_BUCKET_DIGITS", "5"))
SEMANTIC_UNIT_SIGNATURE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V138_SEMANTIC_UNIT_SIGNATURE_ATOM_LIMIT", "18")
)
SEMANTIC_UNIT_POSTING_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_SEMANTIC_UNIT_POSTING_ROW_LIMIT", "90"))
SEMANTIC_UNIT_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_SEMANTIC_UNIT_CANDIDATE_LIMIT", "180"))
SEMANTIC_UNIT_MIN_EXPERIENCE_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V138_SEMANTIC_UNIT_MIN_EXPERIENCE_SIM", "0.46")
)
SEMANTIC_UNIT_MIN_MIXED_SIM = float(os.environ.get("DARTLAB_HORIZON_V138_SEMANTIC_UNIT_MIN_MIXED_SIM", "0.34"))
SEMANTIC_UNIT_MIN_COORD_RESONANCE = float(
    os.environ.get("DARTLAB_HORIZON_V138_SEMANTIC_UNIT_MIN_COORD_RESONANCE", "0.045")
)
SEMANTIC_UNIT_MASK_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V138_SEMANTIC_UNIT_MASK_SIGNATURE_LIMIT", "120")
)
SEMANTIC_UNIT_MASK_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_SEMANTIC_UNIT_MASK_ATOM_LIMIT", "64"))
SEMANTIC_UNIT_MASK_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_SEMANTIC_UNIT_MASK_ROW_LIMIT", "260"))
SEMANTIC_UNIT_ROUTE_MEMBER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_SEMANTIC_UNIT_ROUTE_MEMBER_LIMIT", "4"))
CONTRAST_SEMANTIC_UNIT_MIN_EXPERIENCE_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V138_CONTRAST_SEMANTIC_UNIT_MIN_EXPERIENCE_SIM", "0.46")
)
CONTRAST_SEMANTIC_UNIT_STRONG_EXPERIENCE_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V138_CONTRAST_SEMANTIC_UNIT_STRONG_EXPERIENCE_SIM", "0.62")
)
CONTRAST_SEMANTIC_UNIT_MIN_MIXED_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V138_CONTRAST_SEMANTIC_UNIT_MIN_MIXED_SIM", "0.36")
)
CONTRAST_SEMANTIC_UNIT_MIN_COORD_RESONANCE = float(
    os.environ.get("DARTLAB_HORIZON_V138_CONTRAST_SEMANTIC_UNIT_MIN_COORD_RESONANCE", "0.050")
)
CONTRAST_SEMANTIC_UNIT_MIN_RELATION_COMPAT = float(
    os.environ.get("DARTLAB_HORIZON_V138_CONTRAST_SEMANTIC_UNIT_MIN_RELATION_COMPAT", "0.28")
)
CONTRAST_SEMANTIC_UNIT_SUFFIX_RELATION_COMPAT = float(
    os.environ.get("DARTLAB_HORIZON_V138_CONTRAST_SEMANTIC_UNIT_SUFFIX_RELATION_COMPAT", "0.18")
)
MEMBER_BALANCE_MEMBER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_MEMBER_BALANCE_MEMBER_LIMIT", "10"))
MEMBER_BALANCE_MIN_MULTIPLIER = float(os.environ.get("DARTLAB_HORIZON_V138_MEMBER_BALANCE_MIN_MULTIPLIER", "0.015"))
MEMBER_BALANCE_UNSUPPORTED_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V138_MEMBER_BALANCE_UNSUPPORTED_FACTOR", "0.12")
)
MEMBER_BALANCE_POSITIVE_GAIN = float(os.environ.get("DARTLAB_HORIZON_V138_MEMBER_BALANCE_POSITIVE_GAIN", "1.10"))
MEMBER_BALANCE_NEGATIVE_GAIN = float(os.environ.get("DARTLAB_HORIZON_V138_MEMBER_BALANCE_NEGATIVE_GAIN", "1.60"))
MEMBER_BALANCE_POSITIVE_SHIELD_RATIO = float(
    os.environ.get("DARTLAB_HORIZON_V138_MEMBER_BALANCE_POSITIVE_SHIELD_RATIO", "0.85")
)
...
MEMBER_CONDITIONED_MASK_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V138_MEMBER_CONDITIONED_MASK_SIGNATURE_LIMIT", "96")
)
MEMBER_CONDITIONED_MASK_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V138_MEMBER_CONDITIONED_MASK_ATOM_LIMIT", "64")
)
MEMBER_CONDITIONED_MASK_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_MEMBER_CONDITIONED_MASK_ROW_LIMIT", "220"))
MEMBER_CONDITIONED_ROUTE_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V138_MEMBER_CONDITIONED_ROUTE_MEMBER_LIMIT", "180")
)
MEMBER_CONDITIONED_LABEL_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V138_MEMBER_CONDITIONED_LABEL_MEMBER_LIMIT", "6")
)
MEMBER_CONDITIONED_MEMBER_CACHE: dict[tuple[int, str, str], tuple[float, float, float, int]] = {}
MEMBER_SUPPORTED_LABEL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_MEMBER_SUPPORTED_LABEL_LIMIT", "80"))
MEMBER_SUPPORTED_MEMBER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_MEMBER_SUPPORTED_MEMBER_LIMIT", "120"))
MEMBER_SUPPORTED_ACTIVE_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V138_MEMBER_SUPPORTED_ACTIVE_MEMBER_LIMIT", "8")
)
MEMBER_SUPPORTED_MIN_MULTIPLIER = float(
    os.environ.get("DARTLAB_HORIZON_V138_MEMBER_SUPPORTED_MIN_MULTIPLIER", str(MEMBER_BALANCE_MIN_MULTIPLIER))
)
...
MEMBER_ROLE_RESIDUAL_MASK_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V138_MEMBER_ROLE_RESIDUAL_MASK_SIGNATURE_LIMIT", "96")
)
MEMBER_ROLE_RESIDUAL_MASK_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V138_MEMBER_ROLE_RESIDUAL_MASK_ATOM_LIMIT", "64")
)
MEMBER_ROLE_RESIDUAL_MASK_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V138_MEMBER_ROLE_RESIDUAL_MASK_ROW_LIMIT", "220")
)
MEMBER_ROLE_RESIDUAL_LABEL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_MEMBER_ROLE_RESIDUAL_LABEL_LIMIT", "80"))
MEMBER_ROLE_RESIDUAL_MIN_MULTIPLIER = float(
    os.environ.get("DARTLAB_HORIZON_V138_MEMBER_ROLE_RESIDUAL_MIN_MULTIPLIER", "0.25")
)
MEMBER_ROLE_RESIDUAL_CONFLICT_RATIO = float(
    os.environ.get("DARTLAB_HORIZON_V138_MEMBER_ROLE_RESIDUAL_CONFLICT_RATIO", "1.35")
)
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V138_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V138_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V138_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V138_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V138_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V138_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V138_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V138_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
LANE_MISMATCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V138_LANE_MISMATCH_PENALTY", "0.18"))
LANE_ARTIFACT_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V138_LANE_ARTIFACT_PENALTY", "0.10"))
NEAREST_ORDER_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_NEAREST_ORDER_SIGNATURE_LIMIT", "24"))
NEAREST_ORDER_PENALTY_MIN = float(os.environ.get("DARTLAB_HORIZON_V138_NEAREST_ORDER_PENALTY_MIN", "0.05"))
NEAREST_ORDER_PENALTY_SCALE = float(os.environ.get("DARTLAB_HORIZON_V138_NEAREST_ORDER_PENALTY_SCALE", "0.16"))
NEAREST_ORDER_COHORT_SURFACE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V138_NEAREST_ORDER_COHORT_SURFACE_LIMIT", "384")
)
NEAREST_ORDER_COHORT_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V138_NEAREST_ORDER_COHORT_POSITION_LIMIT", "8192")
)
NEAREST_ORDER_SURFACE_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V138_NEAREST_ORDER_SURFACE_POSITION_LIMIT", "48")
)
SUFFIX_ANCHOR_SUPPORT_MIN = int(os.environ.get("DARTLAB_HORIZON_V138_SUFFIX_ANCHOR_SUPPORT_MIN", "1"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V138_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V138_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V138_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V138_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V138_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V138_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V138_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V138_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V138_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V138_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V138_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V138_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V138_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V138_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V138_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V138_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V138_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V138_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V138_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V138_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V138_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V138_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V138_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
RELAY_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_RELAY_ROW_LIMIT", "160"))
RELAY_SPECIFIC_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_RELAY_SPECIFIC_ROW_LIMIT", "320"))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V138_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V138_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V138_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V138_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V138_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V138_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V138_SKETCH_OCC_BUCKETS", "16"))
