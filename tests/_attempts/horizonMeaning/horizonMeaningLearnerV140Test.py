"""Horizon Meaning Learner V140 - source-role selected-target projection.

V140 실제 기록
--------------
아이디어:
    V139 는 role-residual graph 를 label veto 로 쓰지 않고 target-member selector 로 쓰면 holdout labelTop5 와
    surfaceTop5 가 함께 오른다는 것을 보였다. 하지만 manual semantic probe 는 여전히 1/4 에 머물렀다.
    즉 "candidate unit 내부에서 어떤 member 가 중심인가" 는 고를 수 있지만, rare query source 가 그 member 까지
    건너가는 path 가 부족하다.

    V140 은 이 빈칸을 직접 검증한다. relation-bound corpus evidence 에서 source surface 의 role, target member 의
    role, source 주변 horizon/masked 경험 atom 을 묶어
    `(relation, sourceRole, targetRole, sourceExperienceAtom) -> selected target member` posting 을 만든다.
    target 은 raw relation-bound owner surface 를 그대로 쓰지 않고, V139 selector 가 같은 target label 내부에서 고른
    member 로 다시 투영한다. 검색 시에는 query 의 source role 과 relation, 주변 경험 atom 을 넣고 targetRole 별
    posting 을 조회한 뒤, V139 selector 와 source-member evidence balance 로 검산한다.

    목표는 V139 의 "unit 내부 member 선택" 신호와 기존 relation-bound path 를 연결해, embedding 없이도
    source 경험에서 selected target member 로 넘어가는 sparse projection 이 생기는지 보는 것이다.

실행 코드:
    $env:DARTLAB_HORIZON_V140_MAX_RECORDS_PER_SOURCE=120
    $env:DARTLAB_HORIZON_V140_MAX_UNITS=1200
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV140Test.py

검증 기준:
    1. V139 target-member selector 는 유지하고, 별도 selected-target projection posting 을 생성해야 한다.
    2. projection key 는 relation, sourceRole, targetRole, source experience atom 을 모두 포함해야 한다.
    3. target surface 는 relation-bound raw target 을 그대로 쓰지 않고 candidate label 내부 selected member 로 재투영해야 한다.
    4. projection route 의 positive Top1/Top5 와 negative BadTop1/BadTop5 를 기존 relationSourceExperience/selector 와 비교해 기록한다.
    5. manual semantic probe 가 V139 1/4 를 넘지 못하면 matched atoms/keys 와 top 후보로 실패 원인을 기록한다.
    6. 특정 surface 하드코딩, forbidden 단어 패널티, 케이스별 gate, 단순 가중치 미세조정 없이 일반 projection 구조만 사용한다.
    7. fixed route/search guard 는 positiveHits=3/4, badAccepted=0/7 수준을 유지해야 한다.

결과:
    1,200 units smoke 에서 relationSelectedTargetProjection 은 sourceKeys=3,966, keys=384,164,
    links=825,800, atomTargetLinks=886,521 로 생성됐다. sourceRole 은 owner=934, modifier=2,394,
    metric=435, fragment=203 이었고, buildRelationSelectedTargetProjection stage 는 38.5초가 걸렸다.

    기존 semantic unit 계열은 contrast/memberSupported labelTop1=669/886, labelTop5=764/886,
    roleResidual label route=688/886, 768/886 이었다. V139 selector 는 labelTop1=719/886,
    labelTop5=783/886, selectedSurfaceTop5=738/886, selectedFamilyTop5=812/886 를 유지했다.

    그러나 V140 selected-target projection probe 는 Top1=0/4, Top5=1/4, BadTop1=0/7,
    BadTop5=0/7 이었다. relationSourceExperience probe 도 Top1=0/4, Top5=1/4, BadTop1=0/7,
    BadTop5=0/7 이므로 negative 안정성은 유지했지만 positive rare 의미 전이는 늘지 않았다.
    fixed guard 는 positiveHits=3/4, badAccepted=0/7, searchTop1=4/5, reliableSearch=4/5 였고,
    totalSeconds=351.9 였다.

결론:
    실패/진단 성공. sourceRole, targetRole, source 경험 atom, V139 selected member 를 하나의 sparse posting 으로
    물리화할 수 있다는 것은 확인했지만, corpus-wide atom projection 은 key/link 수가 너무 커지고 V139 의
    rare semantic probe 1/4 를 넘지 못했다. broad selected-target atom projection 은 최종 route 로 폐기한다.
    유지할 것은 V139 role-residual target-member selector 이고, 다음 실험은 모든 source atom 을 사전 물리화하지 말고
    query-time selector-conditioned path join 또는 pair-local path join 으로 relation-bound evidence 와 selected member 를
    좁게 연결해야 한다.
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V140_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V140_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V140_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V140_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V140_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V140_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V140_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V140_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V140_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_SEARCH_CANDIDATE_LIMIT", "420"))
DYNAMIC_TARGET_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_DYNAMIC_TARGET_LIMIT", "80"))
DYNAMIC_COORD_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_DYNAMIC_COORD_ROW_LIMIT", "220"))
DYNAMIC_COMPOUND_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_DYNAMIC_COMPOUND_ROW_LIMIT", "260"))
DYNAMIC_QUERY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_DYNAMIC_QUERY_ATOM_LIMIT", "48"))
DYNAMIC_MEANING_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_DYNAMIC_MEANING_ATOM_LIMIT", "36"))
DYNAMIC_MEANING_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_DYNAMIC_MEANING_ROW_LIMIT", "220"))
DYNAMIC_RELATION_SURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_DYNAMIC_RELATION_SURFACE_LIMIT", "420"))
DYNAMIC_RELATION_UNIT_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_DYNAMIC_RELATION_UNIT_LIMIT", "160"))
DYNAMIC_BRIDGE_ONLY_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V140_DYNAMIC_BRIDGE_ONLY_PENALTY", "0.80"))
DYNAMIC_OWNER_ROLE_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_DYNAMIC_OWNER_ROLE_SIGNATURE_LIMIT", "64")
)
DYNAMIC_OWNER_ROLE_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_DYNAMIC_OWNER_ROLE_ATOM_LIMIT", "40"))
DYNAMIC_OWNER_ROLE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_DYNAMIC_OWNER_ROLE_ROW_LIMIT", "220"))
DYNAMIC_OWNER_ROLE_UNIT_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_DYNAMIC_OWNER_ROLE_UNIT_ATOM_LIMIT", "36")
)
DYNAMIC_OWNER_ROLE_MIN_BOUND = float(os.environ.get("DARTLAB_HORIZON_V140_DYNAMIC_OWNER_ROLE_MIN_BOUND", "0.55"))
DYNAMIC_OWNER_ROLE_CANDIDATE_BONUS = float(
    os.environ.get("DARTLAB_HORIZON_V140_DYNAMIC_OWNER_ROLE_CANDIDATE_BONUS", "2.40")
)
DYNAMIC_OWNER_ROLE_ROUTE_BONUS = float(os.environ.get("DARTLAB_HORIZON_V140_DYNAMIC_OWNER_ROLE_ROUTE_BONUS", "0.90"))
DYNAMIC_OWNER_ROLE_WEAK_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V140_DYNAMIC_OWNER_ROLE_WEAK_PENALTY", "0.16"))
DYNAMIC_OWNER_FRAME_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V140_DYNAMIC_OWNER_FRAME_RADIUS", "5"))
DYNAMIC_OWNER_FRAME_BETWEEN_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_DYNAMIC_OWNER_FRAME_BETWEEN_LIMIT", "7"))
MASKED_FRAME_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_MASKED_FRAME_SIGNATURE_LIMIT", "72"))
MASKED_FRAME_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_MASKED_FRAME_ATOM_LIMIT", "48"))
MASKED_FRAME_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_MASKED_FRAME_ROW_LIMIT", "240"))
MASKED_FRAME_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_MASKED_FRAME_EVAL_LIMIT", "800"))
MASKED_FRAME_HOLDOUT_MOD = int(os.environ.get("DARTLAB_HORIZON_V140_MASKED_FRAME_HOLDOUT_MOD", "5"))
MASKED_FRAME_PROBE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_MASKED_FRAME_PROBE_LIMIT", "12"))
MASKED_CONTEXT_SURFACE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_MASKED_CONTEXT_SURFACE_ROW_LIMIT", "12"))
MASKED_CONTEXT_WEIGHT = float(os.environ.get("DARTLAB_HORIZON_V140_MASKED_CONTEXT_WEIGHT", "0.35"))
MASKED_CONTEXT_RELATION_WINDOW = int(os.environ.get("DARTLAB_HORIZON_V140_MASKED_CONTEXT_RELATION_WINDOW", "140"))
MASKED_FRAME_SURPRISAL_POWER = float(os.environ.get("DARTLAB_HORIZON_V140_MASKED_FRAME_SURPRISAL_POWER", "2.0"))
MASKED_FRAME_SURFACE_PRIOR_POWER = float(
    os.environ.get("DARTLAB_HORIZON_V140_MASKED_FRAME_SURFACE_PRIOR_POWER", "0.45")
)
RELATION_OWNER_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_RELATION_OWNER_SIGNATURE_LIMIT", "64"))
RELATION_OWNER_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_RELATION_OWNER_ROW_LIMIT", "220"))
RELATION_OWNER_BRIDGE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_RELATION_OWNER_BRIDGE_LIMIT", "12"))
RELATION_OWNER_PROJECTION_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_RELATION_OWNER_PROJECTION_LIMIT", "24"))
RELATION_OWNER_PROJECTION_OWNER_ROLE_MIN = float(
    os.environ.get("DARTLAB_HORIZON_V140_RELATION_OWNER_PROJECTION_OWNER_ROLE_MIN", "0.18")
)
RELATION_OWNER_PROJECTION_QUERY_ROLE_MIN = float(
    os.environ.get("DARTLAB_HORIZON_V140_RELATION_OWNER_PROJECTION_QUERY_ROLE_MIN", "0.05")
)
RELATION_OWNER_GENERATED_BRIDGE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_RELATION_OWNER_GENERATED_BRIDGE_LIMIT", "48")
)
RELATION_OWNER_GENERATED_BRIDGE_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V140_RELATION_OWNER_GENERATED_BRIDGE_WEIGHT", "2.4")
)
RELATION_OWNER_ROLE_COMPAT_MIN = float(os.environ.get("DARTLAB_HORIZON_V140_RELATION_OWNER_ROLE_COMPAT_MIN", "0.16"))
RELATION_BOUND_ROLE_PAIR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_RELATION_BOUND_ROLE_PAIR_LIMIT", "24"))
RELATION_BOUND_ROLE_PAIR_BRIDGE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_RELATION_BOUND_ROLE_PAIR_BRIDGE_LIMIT", "10")
)
RELATION_BOUND_ROLE_PAIR_BRIDGE_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V140_RELATION_BOUND_ROLE_PAIR_BRIDGE_WEIGHT", "0.46")
)
RELATION_SOURCE_EXPERIENCE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_RELATION_SOURCE_EXPERIENCE_ATOM_LIMIT", "44")
)
RELATION_SOURCE_EXPERIENCE_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_RELATION_SOURCE_EXPERIENCE_ROW_LIMIT", "180")
)
RELATION_SOURCE_SHINGLE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_RELATION_SOURCE_SHINGLE_ATOM_LIMIT", "28")
)
RELATION_SOURCE_SHINGLE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_RELATION_SOURCE_SHINGLE_ROW_LIMIT", "160"))
RELATION_SOURCE_SHINGLE_KEY_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_RELATION_SOURCE_SHINGLE_KEY_LIMIT", "56"))
RELATION_ANCHORED_COARSE_SHINGLE_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_RELATION_ANCHORED_COARSE_SHINGLE_ROW_LIMIT", "180")
)
RELATION_ANCHORED_COARSE_SHINGLE_KEY_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_RELATION_ANCHORED_COARSE_SHINGLE_KEY_LIMIT", "72")
)
HORIZON_MASK_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_HORIZON_MASK_SIGNATURE_LIMIT", "96"))
HORIZON_MASK_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_HORIZON_MASK_ATOM_LIMIT", "56"))
HORIZON_MASK_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_HORIZON_MASK_ROW_LIMIT", "220"))
HORIZON_MASK_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_HORIZON_MASK_EVAL_LIMIT", "900"))
HORIZON_MASK_HOLDOUT_MOD = int(os.environ.get("DARTLAB_HORIZON_V140_HORIZON_MASK_HOLDOUT_MOD", "7"))
HORIZON_MASK_SURFACE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_HORIZON_MASK_SURFACE_ROW_LIMIT", "24"))
HORIZON_MASK_NEIGHBOR_EXPERIENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_HORIZON_MASK_NEIGHBOR_EXPERIENCE_LIMIT", "5")
)
HORIZON_MASK_COORD_CELL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_HORIZON_MASK_COORD_CELL_LIMIT", "5"))
HORIZON_MASK_COORD_BUCKET_DIGITS = int(os.environ.get("DARTLAB_HORIZON_V140_HORIZON_MASK_COORD_BUCKET_DIGITS", "5"))
SEMANTIC_UNIT_SIGNATURE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_SEMANTIC_UNIT_SIGNATURE_ATOM_LIMIT", "18")
)
SEMANTIC_UNIT_POSTING_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_SEMANTIC_UNIT_POSTING_ROW_LIMIT", "90"))
SEMANTIC_UNIT_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_SEMANTIC_UNIT_CANDIDATE_LIMIT", "180"))
SEMANTIC_UNIT_MIN_EXPERIENCE_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V140_SEMANTIC_UNIT_MIN_EXPERIENCE_SIM", "0.46")
)
SEMANTIC_UNIT_MIN_MIXED_SIM = float(os.environ.get("DARTLAB_HORIZON_V140_SEMANTIC_UNIT_MIN_MIXED_SIM", "0.34"))
SEMANTIC_UNIT_MIN_COORD_RESONANCE = float(
    os.environ.get("DARTLAB_HORIZON_V140_SEMANTIC_UNIT_MIN_COORD_RESONANCE", "0.045")
)
SEMANTIC_UNIT_MASK_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_SEMANTIC_UNIT_MASK_SIGNATURE_LIMIT", "120")
)
SEMANTIC_UNIT_MASK_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_SEMANTIC_UNIT_MASK_ATOM_LIMIT", "64"))
SEMANTIC_UNIT_MASK_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_SEMANTIC_UNIT_MASK_ROW_LIMIT", "260"))
SEMANTIC_UNIT_ROUTE_MEMBER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_SEMANTIC_UNIT_ROUTE_MEMBER_LIMIT", "4"))
CONTRAST_SEMANTIC_UNIT_MIN_EXPERIENCE_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V140_CONTRAST_SEMANTIC_UNIT_MIN_EXPERIENCE_SIM", "0.46")
)
CONTRAST_SEMANTIC_UNIT_STRONG_EXPERIENCE_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V140_CONTRAST_SEMANTIC_UNIT_STRONG_EXPERIENCE_SIM", "0.62")
)
CONTRAST_SEMANTIC_UNIT_MIN_MIXED_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V140_CONTRAST_SEMANTIC_UNIT_MIN_MIXED_SIM", "0.36")
)
CONTRAST_SEMANTIC_UNIT_MIN_COORD_RESONANCE = float(
    os.environ.get("DARTLAB_HORIZON_V140_CONTRAST_SEMANTIC_UNIT_MIN_COORD_RESONANCE", "0.050")
)
CONTRAST_SEMANTIC_UNIT_MIN_RELATION_COMPAT = float(
    os.environ.get("DARTLAB_HORIZON_V140_CONTRAST_SEMANTIC_UNIT_MIN_RELATION_COMPAT", "0.28")
)
CONTRAST_SEMANTIC_UNIT_SUFFIX_RELATION_COMPAT = float(
    os.environ.get("DARTLAB_HORIZON_V140_CONTRAST_SEMANTIC_UNIT_SUFFIX_RELATION_COMPAT", "0.18")
)
MEMBER_BALANCE_MEMBER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_MEMBER_BALANCE_MEMBER_LIMIT", "10"))
MEMBER_BALANCE_MIN_MULTIPLIER = float(os.environ.get("DARTLAB_HORIZON_V140_MEMBER_BALANCE_MIN_MULTIPLIER", "0.015"))
MEMBER_BALANCE_UNSUPPORTED_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V140_MEMBER_BALANCE_UNSUPPORTED_FACTOR", "0.12")
)
MEMBER_BALANCE_POSITIVE_GAIN = float(os.environ.get("DARTLAB_HORIZON_V140_MEMBER_BALANCE_POSITIVE_GAIN", "1.10"))
MEMBER_BALANCE_NEGATIVE_GAIN = float(os.environ.get("DARTLAB_HORIZON_V140_MEMBER_BALANCE_NEGATIVE_GAIN", "1.60"))
MEMBER_BALANCE_POSITIVE_SHIELD_RATIO = float(
    os.environ.get("DARTLAB_HORIZON_V140_MEMBER_BALANCE_POSITIVE_SHIELD_RATIO", "0.85")
)
...
MEMBER_CONDITIONED_MASK_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_MEMBER_CONDITIONED_MASK_SIGNATURE_LIMIT", "96")
)
MEMBER_CONDITIONED_MASK_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_MEMBER_CONDITIONED_MASK_ATOM_LIMIT", "64")
)
MEMBER_CONDITIONED_MASK_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_MEMBER_CONDITIONED_MASK_ROW_LIMIT", "220"))
MEMBER_CONDITIONED_ROUTE_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_MEMBER_CONDITIONED_ROUTE_MEMBER_LIMIT", "180")
)
MEMBER_CONDITIONED_LABEL_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_MEMBER_CONDITIONED_LABEL_MEMBER_LIMIT", "6")
)
MEMBER_CONDITIONED_MEMBER_CACHE: dict[tuple[int, str, str], tuple[float, float, float, int]] = {}
MEMBER_SUPPORTED_LABEL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_MEMBER_SUPPORTED_LABEL_LIMIT", "80"))
MEMBER_SUPPORTED_MEMBER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_MEMBER_SUPPORTED_MEMBER_LIMIT", "120"))
MEMBER_SUPPORTED_ACTIVE_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_MEMBER_SUPPORTED_ACTIVE_MEMBER_LIMIT", "8")
)
MEMBER_SUPPORTED_MIN_MULTIPLIER = float(
    os.environ.get("DARTLAB_HORIZON_V140_MEMBER_SUPPORTED_MIN_MULTIPLIER", str(MEMBER_BALANCE_MIN_MULTIPLIER))
)
...
MEMBER_ROLE_RESIDUAL_MASK_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_MEMBER_ROLE_RESIDUAL_MASK_SIGNATURE_LIMIT", "96")
)
MEMBER_ROLE_RESIDUAL_MASK_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_MEMBER_ROLE_RESIDUAL_MASK_ATOM_LIMIT", "64")
)
MEMBER_ROLE_RESIDUAL_MASK_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_MEMBER_ROLE_RESIDUAL_MASK_ROW_LIMIT", "220")
)
MEMBER_ROLE_RESIDUAL_LABEL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_MEMBER_ROLE_RESIDUAL_LABEL_LIMIT", "80"))
MEMBER_ROLE_RESIDUAL_MIN_MULTIPLIER = float(
    os.environ.get("DARTLAB_HORIZON_V140_MEMBER_ROLE_RESIDUAL_MIN_MULTIPLIER", "0.25")
)
MEMBER_ROLE_RESIDUAL_CONFLICT_RATIO = float(
    os.environ.get("DARTLAB_HORIZON_V140_MEMBER_ROLE_RESIDUAL_CONFLICT_RATIO", "1.35")
)
MEMBER_TARGET_SELECTOR_LABEL_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_MEMBER_TARGET_SELECTOR_LABEL_LIMIT", "48")
)
MEMBER_TARGET_SELECTOR_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_MEMBER_TARGET_SELECTOR_MEMBER_LIMIT", "14")
)
MEMBER_TARGET_SELECTOR_BALANCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_MEMBER_TARGET_SELECTOR_BALANCE_LIMIT", "96")
)
MEMBER_TARGET_SELECTOR_SURFACE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_MEMBER_TARGET_SELECTOR_SURFACE_LIMIT", str(MASKED_FRAME_PROBE_LIMIT))
)
SELECTED_TARGET_PROJECTION_SOURCE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_SELECTED_TARGET_PROJECTION_SOURCE_ATOM_LIMIT", "72")
)
SELECTED_TARGET_PROJECTION_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_SELECTED_TARGET_PROJECTION_ROW_LIMIT", "220")
)
SELECTED_TARGET_PROJECTION_TARGET_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_SELECTED_TARGET_PROJECTION_TARGET_LIMIT", "8")
)
SELECTED_TARGET_PROJECTION_SELECTOR_LABEL_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_SELECTED_TARGET_PROJECTION_SELECTOR_LABEL_LIMIT", "12")
)
SELECTED_TARGET_PROJECTION_SELECTOR_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_SELECTED_TARGET_PROJECTION_SELECTOR_MEMBER_LIMIT", "8")
)
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V140_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V140_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V140_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V140_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V140_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V140_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V140_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V140_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
LANE_MISMATCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V140_LANE_MISMATCH_PENALTY", "0.18"))
LANE_ARTIFACT_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V140_LANE_ARTIFACT_PENALTY", "0.10"))
NEAREST_ORDER_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_NEAREST_ORDER_SIGNATURE_LIMIT", "24"))
NEAREST_ORDER_PENALTY_MIN = float(os.environ.get("DARTLAB_HORIZON_V140_NEAREST_ORDER_PENALTY_MIN", "0.05"))
NEAREST_ORDER_PENALTY_SCALE = float(os.environ.get("DARTLAB_HORIZON_V140_NEAREST_ORDER_PENALTY_SCALE", "0.16"))
NEAREST_ORDER_COHORT_SURFACE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_NEAREST_ORDER_COHORT_SURFACE_LIMIT", "384")
)
NEAREST_ORDER_COHORT_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_NEAREST_ORDER_COHORT_POSITION_LIMIT", "8192")
)
NEAREST_ORDER_SURFACE_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V140_NEAREST_ORDER_SURFACE_POSITION_LIMIT", "48")
)
SUFFIX_ANCHOR_SUPPORT_MIN = int(os.environ.get("DARTLAB_HORIZON_V140_SUFFIX_ANCHOR_SUPPORT_MIN", "1"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V140_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V140_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V140_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V140_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V140_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V140_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V140_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V140_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V140_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V140_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V140_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V140_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V140_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V140_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V140_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V140_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V140_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V140_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V140_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V140_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V140_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V140_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V140_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
RELAY_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_RELAY_ROW_LIMIT", "160"))
RELAY_SPECIFIC_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_RELAY_SPECIFIC_ROW_LIMIT", "320"))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V140_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V140_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V140_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V140_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V140_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V140_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V140_SKETCH_OCC_BUCKETS", "16"))
...
def buildModel() -> Model:
    ...
    relationSelectedTargetPostings, relationSelectedTargetStats = buildRelationSelectedTargetProjection(model)
    model.relationSelectedTargetPostings = relationSelectedTargetPostings
    model.relationSelectedTargetStats = relationSelectedTargetStats
    stage("buildRelationSelectedTargetProjection")
