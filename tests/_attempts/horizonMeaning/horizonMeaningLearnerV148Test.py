"""Horizon Meaning Learner V148 - relation-slot constrained compositional frame.

V148 실제 기록
--------------
아이디어:
    V147 은 source surface 가 corpus 에 직접 없거나 얕을 때 내부 contiguous fragment 의 경험장을 합성해
    cold-start source frame 을 만들었다. 그 결과 `외상매출금 -> 매출채권` 은 queryAtoms=0/rank=NA 에서
    queryAtoms=120/rank=7 까지 올라왔지만, fragment 경험을 단순 합산했기 때문에 relation/order 슬롯이
    희석되어 Top5 의미 전이로는 못 넘어갔다.

    V148 은 단순 합산을 폐기하고 relation-slot constrained composition 을 검증한다. source 내부 fragment 는
    자기 문자열 안의 위치(prefix/suffix/inner), corpus occurrence 에서의 role, query relation 과의 정렬 여부,
    그리고 candidate target member 의 실제 frame 과 동시에 맞아야 pair evidence 로 인정된다. 즉 source 의
    "부분 경험" 을 candidate 로 복사하는 것이 아니라, fragment 가 겪은 role/order/relation 경험이 target member 의
    경험장과 같은 slot 에서 겹칠 때만 추가 신호를 만든다.

    핵심은 수평선 좌표가 의미가 아니라 주소계라는 가설을 유지하면서, 의미 비교 단위를 raw atom overlap 에서
    "source 내부 fragment slot -> target member occurrence slot" 의 pair-local sparse agreement 로 올리는 것이다.
    목표는 V147 의 `외상매출금 -> 매출채권` rank=7 을 Top5 쪽으로 끌어올리되, `손실충당금 -> 대손충당금` 처럼
    자기/근접 계열에 밀리는 경우는 peer slot contrast 로 분리하는 것이다.

실행 코드:
    $env:DARTLAB_HORIZON_V148_MAX_RECORDS_PER_SOURCE='120'
    $env:DARTLAB_HORIZON_V148_MAX_UNITS='1200'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV148Test.py
...
검증 기준:
    ...
    6. composition 은 source 문자열 내부 contiguous fragment 만 사용하고 target 후보 surface 를 query frame 으로 복사하지 않는다.
    7. fragment pair evidence 는 source 내부 위치, fragment role, relation alignment, candidate target frame agreement 를 모두 기록한다.
    8. 특정 surface 하드코딩, forbidden 단어 패널티, 케이스별 gate, 단순 가중치 미세조정 없이 일반 cold-start composition 구조만 사용한다.
    9. `외상매출금 -> 매출채권` 은 V147 rank=7 보다 개선되어야 하며, 최소 Top5 진입 여부를 확인한다.
    10. fixed route/search guard 는 positiveHits=3/4, badAccepted=0/7 수준을 유지해야 한다.

결과:
    1,200 unit smoke 는 exit 0 으로 끝났다. 기존 backbone 은 V147 수준을 유지했다.
    queryLocalFrameOccurrenceIndex 는 surfaces=10,883, rows=121,682 이고,
    query-conditioned semantic-unit eval 은 labelTop1=249/260, labelTop5=260/260,
    selectedSurfaceTop5=255/260, selectedSemanticTop5=259/260 이었다.

    새 relation-slot pair evidence 는 실제로 강하게 작동했다. summary 기준
    targetMemberFrameSlotScored=938, SlotRows=27,096, SlotUsedFragments=3,003,
    SlotScoreMilli=127,043 이다. 그러나 방향이 틀렸다. slotPositiveMargins=168 보다
    slotNegativeMargins=744 가 훨씬 많아, 대부분의 후보에서 candidate target 보다 peer slot 이 더 강했다.
    즉 slot pair agreement 는 의미 전이를 만드는 구분자가 아니라 broad multiplier 로 켜졌다.

    핵심 probe 도 개선되지 않았다. targetMemberFrameProbeTop1=1/4, Top5=1/4,
    BadTop1=0/7, BadTop5=1/7 로 V147 과 같다. `외상매출금 -> 매출채권` 은
    candidateHit=True, queryAtoms=120, compFragments=1, compRows=18, slot=0.009,
    slotMargin=-0.008, rank=7 로 V147 rank=7 에서 개선되지 않았다. `영업손익 -> 영업이익` 은
    slot=0.083 이지만 slotMargin=-0.016, rank=NA 이고, `손실충당금 -> 대손충당금` 은
    slot=0.239 이지만 slotMargin=-0.178, rank=NA 로 V147 rank=12 보다 더 나빠졌다.
    fixed guard 는 positiveHits=3/4, badAccepted=0/7, searchTop1=4/5, reliableSearch=4/5 를 유지했다.
    totalSeconds=709.7 로 V147 643.4 보다 느려졌다.

결론:
    실패/진단 성공. relation-slot 을 candidate pair scoring 에 붙이는 방식은 폐기한다.
    source 내부 fragment 의 role/relation/order 경험을 candidate 와 직접 비교하면 신호는 생기지만,
    같은 peer family 와 self/near-surface 계열이 더 강하게 반응한다. 이는 "경험 유사성" 자체가 의미 전이가
    아니라, 같은 slot 안에서 서로 대체되어 등장한 이력, 즉 substitution evidence 가 필요하다는 뜻이다.

    다음 메커니즘은 pair-local multiplier 가 아니라 relation-slot substitution index 여야 한다.
    corpus occurrence 에서 `(relation, role, lane, relation-side/distance, neighbor-role skeleton)` 같은 surface-free
    slot key 를 만들고, 같은 slot 을 점유한 surface 들을 역인덱스로 묶는다. query source 가 없거나 얕으면
    내부 fragment 의 slot key 로 후보를 찾되, 후보는 fragment 와 비슷한 atom 이 아니라 같은 slot 에서 대체 출현한
    surface 여야 한다. 이것이 "경험의 유사" 를 "역할 대체 가능성" 으로 한 단계 올리는 방향이다.
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V148_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V148_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V148_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V148_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V148_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V148_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V148_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V148_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V148_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_SEARCH_CANDIDATE_LIMIT", "420"))
DYNAMIC_TARGET_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_DYNAMIC_TARGET_LIMIT", "80"))
DYNAMIC_COORD_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_DYNAMIC_COORD_ROW_LIMIT", "220"))
DYNAMIC_COMPOUND_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_DYNAMIC_COMPOUND_ROW_LIMIT", "260"))
DYNAMIC_QUERY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_DYNAMIC_QUERY_ATOM_LIMIT", "48"))
DYNAMIC_MEANING_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_DYNAMIC_MEANING_ATOM_LIMIT", "36"))
DYNAMIC_MEANING_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_DYNAMIC_MEANING_ROW_LIMIT", "220"))
DYNAMIC_RELATION_SURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_DYNAMIC_RELATION_SURFACE_LIMIT", "420"))
DYNAMIC_RELATION_UNIT_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_DYNAMIC_RELATION_UNIT_LIMIT", "160"))
DYNAMIC_BRIDGE_ONLY_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V148_DYNAMIC_BRIDGE_ONLY_PENALTY", "0.80"))
DYNAMIC_OWNER_ROLE_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_DYNAMIC_OWNER_ROLE_SIGNATURE_LIMIT", "64")
)
DYNAMIC_OWNER_ROLE_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_DYNAMIC_OWNER_ROLE_ATOM_LIMIT", "40"))
DYNAMIC_OWNER_ROLE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_DYNAMIC_OWNER_ROLE_ROW_LIMIT", "220"))
DYNAMIC_OWNER_ROLE_UNIT_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_DYNAMIC_OWNER_ROLE_UNIT_ATOM_LIMIT", "36")
)
DYNAMIC_OWNER_ROLE_MIN_BOUND = float(os.environ.get("DARTLAB_HORIZON_V148_DYNAMIC_OWNER_ROLE_MIN_BOUND", "0.55"))
DYNAMIC_OWNER_ROLE_CANDIDATE_BONUS = float(
    os.environ.get("DARTLAB_HORIZON_V148_DYNAMIC_OWNER_ROLE_CANDIDATE_BONUS", "2.40")
)
DYNAMIC_OWNER_ROLE_ROUTE_BONUS = float(os.environ.get("DARTLAB_HORIZON_V148_DYNAMIC_OWNER_ROLE_ROUTE_BONUS", "0.90"))
DYNAMIC_OWNER_ROLE_WEAK_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V148_DYNAMIC_OWNER_ROLE_WEAK_PENALTY", "0.16"))
DYNAMIC_OWNER_FRAME_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V148_DYNAMIC_OWNER_FRAME_RADIUS", "5"))
DYNAMIC_OWNER_FRAME_BETWEEN_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_DYNAMIC_OWNER_FRAME_BETWEEN_LIMIT", "7"))
MASKED_FRAME_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_MASKED_FRAME_SIGNATURE_LIMIT", "72"))
MASKED_FRAME_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_MASKED_FRAME_ATOM_LIMIT", "48"))
MASKED_FRAME_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_MASKED_FRAME_ROW_LIMIT", "240"))
MASKED_FRAME_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_MASKED_FRAME_EVAL_LIMIT", "800"))
MASKED_FRAME_HOLDOUT_MOD = int(os.environ.get("DARTLAB_HORIZON_V148_MASKED_FRAME_HOLDOUT_MOD", "5"))
MASKED_FRAME_PROBE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_MASKED_FRAME_PROBE_LIMIT", "12"))
MASKED_CONTEXT_SURFACE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_MASKED_CONTEXT_SURFACE_ROW_LIMIT", "12"))
MASKED_CONTEXT_WEIGHT = float(os.environ.get("DARTLAB_HORIZON_V148_MASKED_CONTEXT_WEIGHT", "0.35"))
MASKED_CONTEXT_RELATION_WINDOW = int(os.environ.get("DARTLAB_HORIZON_V148_MASKED_CONTEXT_RELATION_WINDOW", "140"))
MASKED_FRAME_SURPRISAL_POWER = float(os.environ.get("DARTLAB_HORIZON_V148_MASKED_FRAME_SURPRISAL_POWER", "2.0"))
MASKED_FRAME_SURFACE_PRIOR_POWER = float(
    os.environ.get("DARTLAB_HORIZON_V148_MASKED_FRAME_SURFACE_PRIOR_POWER", "0.45")
)
RELATION_OWNER_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_RELATION_OWNER_SIGNATURE_LIMIT", "64"))
RELATION_OWNER_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_RELATION_OWNER_ROW_LIMIT", "220"))
RELATION_OWNER_BRIDGE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_RELATION_OWNER_BRIDGE_LIMIT", "12"))
RELATION_OWNER_PROJECTION_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_RELATION_OWNER_PROJECTION_LIMIT", "24"))
RELATION_OWNER_PROJECTION_OWNER_ROLE_MIN = float(
    os.environ.get("DARTLAB_HORIZON_V148_RELATION_OWNER_PROJECTION_OWNER_ROLE_MIN", "0.18")
)
RELATION_OWNER_PROJECTION_QUERY_ROLE_MIN = float(
    os.environ.get("DARTLAB_HORIZON_V148_RELATION_OWNER_PROJECTION_QUERY_ROLE_MIN", "0.05")
)
RELATION_OWNER_GENERATED_BRIDGE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_RELATION_OWNER_GENERATED_BRIDGE_LIMIT", "48")
)
RELATION_OWNER_GENERATED_BRIDGE_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V148_RELATION_OWNER_GENERATED_BRIDGE_WEIGHT", "2.4")
)
RELATION_OWNER_ROLE_COMPAT_MIN = float(os.environ.get("DARTLAB_HORIZON_V148_RELATION_OWNER_ROLE_COMPAT_MIN", "0.16"))
RELATION_BOUND_ROLE_PAIR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_RELATION_BOUND_ROLE_PAIR_LIMIT", "24"))
RELATION_BOUND_ROLE_PAIR_BRIDGE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_RELATION_BOUND_ROLE_PAIR_BRIDGE_LIMIT", "10")
)
RELATION_BOUND_ROLE_PAIR_BRIDGE_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V148_RELATION_BOUND_ROLE_PAIR_BRIDGE_WEIGHT", "0.46")
)
RELATION_SOURCE_EXPERIENCE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_RELATION_SOURCE_EXPERIENCE_ATOM_LIMIT", "44")
)
RELATION_SOURCE_EXPERIENCE_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_RELATION_SOURCE_EXPERIENCE_ROW_LIMIT", "180")
)
RELATION_SOURCE_SHINGLE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_RELATION_SOURCE_SHINGLE_ATOM_LIMIT", "28")
)
RELATION_SOURCE_SHINGLE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_RELATION_SOURCE_SHINGLE_ROW_LIMIT", "160"))
RELATION_SOURCE_SHINGLE_KEY_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_RELATION_SOURCE_SHINGLE_KEY_LIMIT", "56"))
RELATION_ANCHORED_COARSE_SHINGLE_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_RELATION_ANCHORED_COARSE_SHINGLE_ROW_LIMIT", "180")
)
RELATION_ANCHORED_COARSE_SHINGLE_KEY_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_RELATION_ANCHORED_COARSE_SHINGLE_KEY_LIMIT", "72")
)
HORIZON_MASK_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_HORIZON_MASK_SIGNATURE_LIMIT", "96"))
HORIZON_MASK_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_HORIZON_MASK_ATOM_LIMIT", "56"))
HORIZON_MASK_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_HORIZON_MASK_ROW_LIMIT", "220"))
HORIZON_MASK_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_HORIZON_MASK_EVAL_LIMIT", "900"))
HORIZON_MASK_HOLDOUT_MOD = int(os.environ.get("DARTLAB_HORIZON_V148_HORIZON_MASK_HOLDOUT_MOD", "7"))
HORIZON_MASK_SURFACE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_HORIZON_MASK_SURFACE_ROW_LIMIT", "24"))
HORIZON_MASK_NEIGHBOR_EXPERIENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_HORIZON_MASK_NEIGHBOR_EXPERIENCE_LIMIT", "5")
)
HORIZON_MASK_COORD_CELL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_HORIZON_MASK_COORD_CELL_LIMIT", "5"))
HORIZON_MASK_COORD_BUCKET_DIGITS = int(os.environ.get("DARTLAB_HORIZON_V148_HORIZON_MASK_COORD_BUCKET_DIGITS", "5"))
LOCAL_FRAME_PREDICTION_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_LOCAL_FRAME_PREDICTION_SIGNATURE_LIMIT", "132")
)
LOCAL_FRAME_PREDICTION_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_LOCAL_FRAME_PREDICTION_ATOM_LIMIT", "76"))
LOCAL_FRAME_PREDICTION_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_LOCAL_FRAME_PREDICTION_ROW_LIMIT", "260"))
LOCAL_FRAME_PREDICTION_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_LOCAL_FRAME_PREDICTION_EVAL_LIMIT", "900"))
LOCAL_FRAME_PREDICTION_SURFACE_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_LOCAL_FRAME_PREDICTION_SURFACE_ROW_LIMIT", "18")
)
LOCAL_FRAME_PREDICTION_HOLDOUT_MOD = int(os.environ.get("DARTLAB_HORIZON_V148_LOCAL_FRAME_PREDICTION_HOLDOUT_MOD", "9"))
LOCAL_FRAME_PREDICTION_NEIGHBOR_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_LOCAL_FRAME_PREDICTION_NEIGHBOR_LIMIT", "5")
)
LOCAL_FRAME_PREDICTION_NEIGHBOR_EXPERIENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_LOCAL_FRAME_PREDICTION_NEIGHBOR_EXPERIENCE_LIMIT", "5")
)
LOCAL_FRAME_PREDICTION_SEQUENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_LOCAL_FRAME_PREDICTION_SEQUENCE_LIMIT", "5")
)
QUERY_LOCAL_FRAME_OCCURRENCE_INDEX_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_QUERY_LOCAL_FRAME_OCCURRENCE_INDEX_ROW_LIMIT", "96")
)
QUERY_LOCAL_FRAME_OCCURRENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_QUERY_LOCAL_FRAME_OCCURRENCE_LIMIT", "72")
)
QUERY_LOCAL_FRAME_PROXY_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_QUERY_LOCAL_FRAME_PROXY_LIMIT", "8"))
QUERY_LOCAL_FRAME_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_QUERY_LOCAL_FRAME_ATOM_LIMIT", "132"))
QUERY_LOCAL_FRAME_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_QUERY_LOCAL_FRAME_EVAL_LIMIT", "260"))
QUERY_SEMANTIC_UNIT_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_QUERY_SEMANTIC_UNIT_ATOM_LIMIT", "120"))
QUERY_SEMANTIC_UNIT_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_QUERY_SEMANTIC_UNIT_EVAL_LIMIT", "260"))
QUERY_SEMANTIC_UNIT_SURFACE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_QUERY_SEMANTIC_UNIT_SURFACE_LIMIT", str(MASKED_FRAME_PROBE_LIMIT))
)
SEMANTIC_UNIT_SIGNATURE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_SEMANTIC_UNIT_SIGNATURE_ATOM_LIMIT", "18")
)
SEMANTIC_UNIT_POSTING_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_SEMANTIC_UNIT_POSTING_ROW_LIMIT", "90"))
SEMANTIC_UNIT_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_SEMANTIC_UNIT_CANDIDATE_LIMIT", "180"))
SEMANTIC_UNIT_MIN_EXPERIENCE_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V148_SEMANTIC_UNIT_MIN_EXPERIENCE_SIM", "0.46")
)
SEMANTIC_UNIT_MIN_MIXED_SIM = float(os.environ.get("DARTLAB_HORIZON_V148_SEMANTIC_UNIT_MIN_MIXED_SIM", "0.34"))
SEMANTIC_UNIT_MIN_COORD_RESONANCE = float(
    os.environ.get("DARTLAB_HORIZON_V148_SEMANTIC_UNIT_MIN_COORD_RESONANCE", "0.045")
)
SEMANTIC_UNIT_MASK_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_SEMANTIC_UNIT_MASK_SIGNATURE_LIMIT", "120")
)
SEMANTIC_UNIT_MASK_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_SEMANTIC_UNIT_MASK_ATOM_LIMIT", "64"))
SEMANTIC_UNIT_MASK_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_SEMANTIC_UNIT_MASK_ROW_LIMIT", "260"))
SEMANTIC_UNIT_ROUTE_MEMBER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_SEMANTIC_UNIT_ROUTE_MEMBER_LIMIT", "4"))
CONTRAST_SEMANTIC_UNIT_MIN_EXPERIENCE_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V148_CONTRAST_SEMANTIC_UNIT_MIN_EXPERIENCE_SIM", "0.46")
)
CONTRAST_SEMANTIC_UNIT_STRONG_EXPERIENCE_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V148_CONTRAST_SEMANTIC_UNIT_STRONG_EXPERIENCE_SIM", "0.62")
)
CONTRAST_SEMANTIC_UNIT_MIN_MIXED_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V148_CONTRAST_SEMANTIC_UNIT_MIN_MIXED_SIM", "0.36")
)
CONTRAST_SEMANTIC_UNIT_MIN_COORD_RESONANCE = float(
    os.environ.get("DARTLAB_HORIZON_V148_CONTRAST_SEMANTIC_UNIT_MIN_COORD_RESONANCE", "0.050")
)
CONTRAST_SEMANTIC_UNIT_MIN_RELATION_COMPAT = float(
    os.environ.get("DARTLAB_HORIZON_V148_CONTRAST_SEMANTIC_UNIT_MIN_RELATION_COMPAT", "0.28")
)
CONTRAST_SEMANTIC_UNIT_SUFFIX_RELATION_COMPAT = float(
    os.environ.get("DARTLAB_HORIZON_V148_CONTRAST_SEMANTIC_UNIT_SUFFIX_RELATION_COMPAT", "0.18")
)
MEMBER_BALANCE_MEMBER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_MEMBER_BALANCE_MEMBER_LIMIT", "10"))
MEMBER_BALANCE_MIN_MULTIPLIER = float(os.environ.get("DARTLAB_HORIZON_V148_MEMBER_BALANCE_MIN_MULTIPLIER", "0.015"))
MEMBER_BALANCE_UNSUPPORTED_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V148_MEMBER_BALANCE_UNSUPPORTED_FACTOR", "0.12")
)
MEMBER_BALANCE_POSITIVE_GAIN = float(os.environ.get("DARTLAB_HORIZON_V148_MEMBER_BALANCE_POSITIVE_GAIN", "1.10"))
MEMBER_BALANCE_NEGATIVE_GAIN = float(os.environ.get("DARTLAB_HORIZON_V148_MEMBER_BALANCE_NEGATIVE_GAIN", "1.60"))
MEMBER_BALANCE_POSITIVE_SHIELD_RATIO = float(
    os.environ.get("DARTLAB_HORIZON_V148_MEMBER_BALANCE_POSITIVE_SHIELD_RATIO", "0.85")
)
...
MEMBER_CONDITIONED_MASK_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_MEMBER_CONDITIONED_MASK_SIGNATURE_LIMIT", "96")
)
MEMBER_CONDITIONED_MASK_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_MEMBER_CONDITIONED_MASK_ATOM_LIMIT", "64")
)
MEMBER_CONDITIONED_MASK_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_MEMBER_CONDITIONED_MASK_ROW_LIMIT", "220"))
MEMBER_CONDITIONED_ROUTE_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_MEMBER_CONDITIONED_ROUTE_MEMBER_LIMIT", "180")
)
MEMBER_CONDITIONED_LABEL_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_MEMBER_CONDITIONED_LABEL_MEMBER_LIMIT", "6")
)
MEMBER_CONDITIONED_MEMBER_CACHE: dict[tuple[int, str, str], tuple[float, float, float, int]] = {}
MEMBER_SUPPORTED_LABEL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_MEMBER_SUPPORTED_LABEL_LIMIT", "80"))
MEMBER_SUPPORTED_MEMBER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_MEMBER_SUPPORTED_MEMBER_LIMIT", "120"))
MEMBER_SUPPORTED_ACTIVE_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_MEMBER_SUPPORTED_ACTIVE_MEMBER_LIMIT", "8")
)
MEMBER_SUPPORTED_MIN_MULTIPLIER = float(
    os.environ.get("DARTLAB_HORIZON_V148_MEMBER_SUPPORTED_MIN_MULTIPLIER", str(MEMBER_BALANCE_MIN_MULTIPLIER))
)
...
MEMBER_ROLE_RESIDUAL_MASK_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_MEMBER_ROLE_RESIDUAL_MASK_SIGNATURE_LIMIT", "96")
)
MEMBER_ROLE_RESIDUAL_MASK_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_MEMBER_ROLE_RESIDUAL_MASK_ATOM_LIMIT", "64")
)
MEMBER_ROLE_RESIDUAL_MASK_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_MEMBER_ROLE_RESIDUAL_MASK_ROW_LIMIT", "220")
)
MEMBER_ROLE_RESIDUAL_LABEL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_MEMBER_ROLE_RESIDUAL_LABEL_LIMIT", "80"))
MEMBER_ROLE_RESIDUAL_MIN_MULTIPLIER = float(
    os.environ.get("DARTLAB_HORIZON_V148_MEMBER_ROLE_RESIDUAL_MIN_MULTIPLIER", "0.25")
)
MEMBER_ROLE_RESIDUAL_CONFLICT_RATIO = float(
    os.environ.get("DARTLAB_HORIZON_V148_MEMBER_ROLE_RESIDUAL_CONFLICT_RATIO", "1.35")
)
MEMBER_TARGET_SELECTOR_LABEL_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_MEMBER_TARGET_SELECTOR_LABEL_LIMIT", "48")
)
MEMBER_TARGET_SELECTOR_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_MEMBER_TARGET_SELECTOR_MEMBER_LIMIT", "14")
)
MEMBER_TARGET_SELECTOR_BALANCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_MEMBER_TARGET_SELECTOR_BALANCE_LIMIT", "96")
)
MEMBER_TARGET_SELECTOR_SURFACE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_MEMBER_TARGET_SELECTOR_SURFACE_LIMIT", str(MASKED_FRAME_PROBE_LIMIT))
)
SELECTED_TARGET_PROJECTION_SOURCE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_SELECTED_TARGET_PROJECTION_SOURCE_ATOM_LIMIT", "72")
)
SELECTED_TARGET_PROJECTION_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_SELECTED_TARGET_PROJECTION_ROW_LIMIT", "220")
)
SELECTED_TARGET_PROJECTION_TARGET_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_SELECTED_TARGET_PROJECTION_TARGET_LIMIT", "8")
)
SELECTED_TARGET_PROJECTION_SELECTOR_LABEL_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_SELECTED_TARGET_PROJECTION_SELECTOR_LABEL_LIMIT", "12")
)
SELECTED_TARGET_PROJECTION_SELECTOR_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_SELECTED_TARGET_PROJECTION_SELECTOR_MEMBER_LIMIT", "8")
)
SELECTOR_PATH_JOIN_SOURCE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_SELECTOR_PATH_JOIN_SOURCE_LIMIT", "12"))
SELECTOR_PATH_JOIN_PAIR_TARGET_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_SELECTOR_PATH_JOIN_PAIR_TARGET_LIMIT", "16")
)
SELECTOR_PATH_JOIN_SELECTED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_SELECTOR_PATH_JOIN_SELECTED_LIMIT", "8"))
SELECTOR_PATH_JOIN_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_SELECTOR_PATH_JOIN_ROW_LIMIT", "36"))
BIDIRECTIONAL_PAIR_CANDIDATE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_BIDIRECTIONAL_PAIR_CANDIDATE_LIMIT", "96")
)
BIDIRECTIONAL_PAIR_ROUTE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_BIDIRECTIONAL_PAIR_ROUTE_LIMIT", "12"))
BIDIRECTIONAL_PAIR_DYNAMIC_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_BIDIRECTIONAL_PAIR_DYNAMIC_LIMIT", "80"))
BIDIRECTIONAL_PAIR_REVERSE_RELATION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_BIDIRECTIONAL_PAIR_REVERSE_RELATION_LIMIT", "2")
)
PAIR_LOCAL_CONTRAST_CANDIDATE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_PAIR_LOCAL_CONTRAST_CANDIDATE_LIMIT", "96")
)
PAIR_LOCAL_CONTRAST_PEER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_PAIR_LOCAL_CONTRAST_PEER_LIMIT", "18"))
PAIR_LOCAL_CONTRAST_NEAR_BONUS = float(os.environ.get("DARTLAB_HORIZON_V148_PAIR_LOCAL_CONTRAST_NEAR_BONUS", "0.42"))
PAIR_LOCAL_CONTRAST_MIN_MARGIN = float(os.environ.get("DARTLAB_HORIZON_V148_PAIR_LOCAL_CONTRAST_MIN_MARGIN", "0.015"))
TARGET_MEMBER_FRAME_OCCURRENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_TARGET_MEMBER_FRAME_OCCURRENCE_LIMIT", "56")
)
TARGET_MEMBER_FRAME_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_TARGET_MEMBER_FRAME_ATOM_LIMIT", "120"))
TARGET_MEMBER_FRAME_PEER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_TARGET_MEMBER_FRAME_PEER_LIMIT", "8"))
TARGET_MEMBER_FRAME_ROUTE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_TARGET_MEMBER_FRAME_ROUTE_LIMIT", str(MASKED_FRAME_PROBE_LIMIT))
)
TARGET_MEMBER_FRAME_NEIGHBOR_RADIUS = int(
    os.environ.get("DARTLAB_HORIZON_V148_TARGET_MEMBER_FRAME_NEIGHBOR_RADIUS", "3")
)
TARGET_MEMBER_FRAME_NEIGHBOR_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_TARGET_MEMBER_FRAME_NEIGHBOR_ATOM_LIMIT", "14")
)
TARGET_MEMBER_FRAME_RELATION_WINDOW = int(
    os.environ.get("DARTLAB_HORIZON_V148_TARGET_MEMBER_FRAME_RELATION_WINDOW", "8")
)
TARGET_MEMBER_FRAME_CANDIDATE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_TARGET_MEMBER_FRAME_CANDIDATE_LIMIT", str(BIDIRECTIONAL_PAIR_CANDIDATE_LIMIT))
)
TARGET_MEMBER_FRAME_QUERY_ROUTE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_TARGET_MEMBER_FRAME_QUERY_ROUTE_LIMIT", "24")
)
COMPOSITIONAL_FRAME_MIN_QUERY_ROWS = int(
    os.environ.get("DARTLAB_HORIZON_V148_COMPOSITIONAL_FRAME_MIN_QUERY_ROWS", "24")
)
COMPOSITIONAL_FRAME_FRAGMENT_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_COMPOSITIONAL_FRAME_FRAGMENT_LIMIT", "10")
)
COMPOSITIONAL_FRAME_FRAGMENT_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_COMPOSITIONAL_FRAME_FRAGMENT_ROW_LIMIT", "18")
)
COMPOSITIONAL_FRAME_MIN_FRAGMENT_LEN = int(
    os.environ.get("DARTLAB_HORIZON_V148_COMPOSITIONAL_FRAME_MIN_FRAGMENT_LEN", "2")
)
COMPOSITIONAL_FRAME_MAX_FRAGMENT_LEN = int(
    os.environ.get("DARTLAB_HORIZON_V148_COMPOSITIONAL_FRAME_MAX_FRAGMENT_LEN", "6")
)
COMPOSITIONAL_FRAME_ATOM_SCALE = float(os.environ.get("DARTLAB_HORIZON_V148_COMPOSITIONAL_FRAME_ATOM_SCALE", "0.58"))
COMPOSITIONAL_FRAME_SIGNATURE_SCALE = float(
    os.environ.get("DARTLAB_HORIZON_V148_COMPOSITIONAL_FRAME_SIGNATURE_SCALE", "0.24")
)
RELATION_SLOT_COMPOSITION_FRAGMENT_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_RELATION_SLOT_COMPOSITION_FRAGMENT_LIMIT", "8")
)
RELATION_SLOT_COMPOSITION_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_RELATION_SLOT_COMPOSITION_ROW_LIMIT", "10")
)
RELATION_SLOT_COMPOSITION_PEER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_RELATION_SLOT_COMPOSITION_PEER_LIMIT", "5")
)
RELATION_SLOT_COMPOSITION_SCORE_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V148_RELATION_SLOT_COMPOSITION_SCORE_GAIN", "2.8")
)
RELATION_SLOT_COMPOSITION_PEER_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V148_RELATION_SLOT_COMPOSITION_PEER_GAIN", "1.8")
)
RELATION_SLOT_COMPOSITION_NEGATIVE_PENALTY = float(
    os.environ.get("DARTLAB_HORIZON_V148_RELATION_SLOT_COMPOSITION_NEGATIVE_PENALTY", "0.76")
)
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V148_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V148_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V148_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V148_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V148_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V148_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V148_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V148_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
LANE_MISMATCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V148_LANE_MISMATCH_PENALTY", "0.18"))
LANE_ARTIFACT_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V148_LANE_ARTIFACT_PENALTY", "0.10"))
NEAREST_ORDER_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_NEAREST_ORDER_SIGNATURE_LIMIT", "24"))
NEAREST_ORDER_PENALTY_MIN = float(os.environ.get("DARTLAB_HORIZON_V148_NEAREST_ORDER_PENALTY_MIN", "0.05"))
NEAREST_ORDER_PENALTY_SCALE = float(os.environ.get("DARTLAB_HORIZON_V148_NEAREST_ORDER_PENALTY_SCALE", "0.16"))
NEAREST_ORDER_COHORT_SURFACE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_NEAREST_ORDER_COHORT_SURFACE_LIMIT", "384")
)
NEAREST_ORDER_COHORT_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_NEAREST_ORDER_COHORT_POSITION_LIMIT", "8192")
)
NEAREST_ORDER_SURFACE_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V148_NEAREST_ORDER_SURFACE_POSITION_LIMIT", "48")
)
SUFFIX_ANCHOR_SUPPORT_MIN = int(os.environ.get("DARTLAB_HORIZON_V148_SUFFIX_ANCHOR_SUPPORT_MIN", "1"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V148_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V148_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V148_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V148_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V148_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V148_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V148_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V148_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V148_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V148_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V148_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V148_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V148_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V148_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V148_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V148_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V148_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V148_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V148_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V148_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V148_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V148_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V148_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
RELAY_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_RELAY_ROW_LIMIT", "160"))
RELAY_SPECIFIC_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_RELAY_SPECIFIC_ROW_LIMIT", "320"))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V148_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V148_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V148_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V148_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V148_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V148_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V148_SKETCH_OCC_BUCKETS", "16"))
...
RELATION_SLOT_COMPOSITION_PAIR_CACHE: dict[tuple[int, str, str, tuple[str, ...]], tuple[float, Counter[str]]] = {}
...
def compositionalColdStartFragmentSlotCandidates(
    surface: str, model: Model
) -> tuple[tuple[str, float, str, str, float], ...]:
    stem = normStem(surface)
    if len(stem) <= COMPOSITIONAL_FRAME_MIN_FRAGMENT_LEN:
        return tuple()
    scores: dict[tuple[str, str], tuple[float, str, float]] = {}
    maxSize = min(COMPOSITIONAL_FRAME_MAX_FRAGMENT_LEN, len(stem) - 1)
    for size in range(COMPOSITIONAL_FRAME_MIN_FRAGMENT_LEN, maxSize + 1):
        for start in range(0, len(stem) - size + 1):
            end = start + size
            ...
            if start == 0:
                ...
                slot = "inner"
            centerRatio = (start + end) / max(1.0, float(len(stem) * 2))
            sizeMass = (size / max(1, len(stem))) ** 1.35
            boundaryMass = 1.18 if slot in {"prefix", "suffix"} else 0.88
            originMass = 1.0 if independent > 0 else 0.78
            evidenceRows = len(rows) + independent + bridge + (len(signature) if signature else 0)
            evidenceMass = 1.0 + min(1.6, math.log1p(evidenceRows) / 5.0)
            score = sizeMass * boundaryMass * originMass * evidenceMass
            kind = "edge" if slot in {"prefix", "suffix"} else "inner"
            key = (fragment, slot)
            current = scores.get(key)
            if current is None or score > current[0]:
                scores[key] = (score, kind, centerRatio)
    ordered = sorted(
        ((fragment, score, kind, slot, centerRatio) for (fragment, slot), (score, kind, centerRatio) in scores.items()),
        key=lambda row: (row[1], len(row[0])),
        reverse=True,
    )
    return tuple(ordered[:RELATION_SLOT_COMPOSITION_FRAGMENT_LIMIT])
...
def relationSlotCompositionRoleScale(fragmentRole: str, candidateRole: str) -> float:
    if fragmentRole == candidateRole:
        return 1.0
    if "fragment" in {fragmentRole, candidateRole}:
        return 0.62
    if "modifier" in {fragmentRole, candidateRole}:
        return 0.54
    return 0.46


def relationSlotCompositionRelationScale(relationKind: str) -> float:
    return {
        "aligned": 1.0,
        "relationless": 0.52,
        "unconditioned": 0.72,
        "unaligned": 0.20,
    }.get(relationKind, 0.46)


def relationSlotCompositionPairAgreement(
    source: str,
    candidate: str,
    relations: tuple[str, ...],
    model: Model,
    candidateFrame: Counter[str] | None = None,
) -> tuple[float, Counter[str]]:
    source = normStem(source)
    candidate = normStem(candidate)
    cacheKey = (id(model), source, candidate, tuple(relations))
    cached = RELATION_SLOT_COMPOSITION_PAIR_CACHE.get(cacheKey)
    if cached is not None:
        score, stats = cached
        return float(score), Counter(stats)

    stats: Counter[str] = Counter()
    fragments = compositionalColdStartFragmentSlotCandidates(source, model)
    if not fragments:
        stats["slotNoFragments"] += 1
        RELATION_SLOT_COMPOSITION_PAIR_CACHE[cacheKey] = (0.0, Counter(stats))
        return 0.0, stats

    if candidateFrame is None:
        candidateFrame, _ = targetMemberLocalRelationFrame(candidate, relations, model, includeProxies=False)
    if not candidateFrame:
        stats["slotEmptyCandidateFrame"] += 1
        RELATION_SLOT_COMPOSITION_PAIR_CACHE[cacheKey] = (0.0, Counter(stats))
        return 0.0, stats

    candidateRole = memberRoleResidualSurfaceRole(
        candidate, model, contrastSemanticUnitLabelForSurface(candidate, model)
    )
    total = 0.0
    for fragmentIndex, (fragment, fragmentWeight, kind, slot, centerRatio) in enumerate(fragments):
        fragmentRole = memberRoleResidualSurfaceRole(
            fragment, model, contrastSemanticUnitLabelForSurface(fragment, model)
        )
        roleScale = relationSlotCompositionRoleScale(fragmentRole, candidateRole)
        slotScale = 1.0 if slot in {"prefix", "suffix"} else 0.78
        rows = model.queryLocalFrameOccurrenceIndex.get(fragment, ())
        stats["slotFragments"] += 1
        stats[f"slotFragmentKind:{kind}"] += 1
        stats[f"slot:{slot}"] += 1
        bestFragment = 0.0
        usedRows = 0
        for rowIndex, (unitId, position, marker) in enumerate(rows[:RELATION_SLOT_COMPOSITION_ROW_LIMIT]):
            if unitId < 0 or unitId >= len(model.caches):
                stats["slotMissingCacheRows"] += 1
                continue
            cache = model.caches[unitId]
            fragmentFrame = targetMemberLocalTypedExperienceAtoms(cache, position, model)
            if not fragmentFrame:
                stats["slotEmptyFragmentRows"] += 1
                continue
            relationScale, relationKind = targetMemberLocalRelationSupport(cache, position, relations)
            relationSlotScale = relationSlotCompositionRelationScale(relationKind)
            aligned, full, relation, order, recursive, role = targetMemberRelationFrameAgreement(
                fragmentFrame, candidateFrame
            )
            channelAgreement = relation * 0.26 + order * 0.20 + recursive * 0.26 + role * 0.16 + full * 0.12
            markerScale = 0.86 if marker == "~" else 1.0
            rowScore = (
                channelAgreement
                ...
                * relationSlotScale
                * markerScale
                / math.sqrt(1.0 + rowIndex)
            )
            bestFragment = max(bestFragment, rowScore)
            usedRows += 1
            stats["slotRows"] += 1
            stats[f"slot{relationKind[0].upper()}{relationKind[1:]}Rows"] += 1
            if relation > 0.0:
                stats["slotRelationAgreementRows"] += 1
            if order > 0.0:
                stats["slotOrderAgreementRows"] += 1
            if recursive > 0.0:
                stats["slotRecursiveAgreementRows"] += 1
        if rows and len(rows) > RELATION_SLOT_COMPOSITION_ROW_LIMIT:
            stats["slotRowLimitSkips"] += len(rows) - RELATION_SLOT_COMPOSITION_ROW_LIMIT
        if usedRows:
            total += bestFragment / math.sqrt(1.0 + fragmentIndex * 0.35)
            stats["slotUsedFragments"] += 1
            stats["slotCenterMilli"] += int(centerRatio * 1000)

    stats["slotScoreMilli"] = int(total * 1000)
    RELATION_SLOT_COMPOSITION_PAIR_CACHE[cacheKey] = (float(total), Counter(stats))
    return total, stats


def relationSlotCompositionPeerMargin(
    source: str,
    candidate: str,
    relations: tuple[str, ...],
    slotScore: float,
    model: Model,
) -> tuple[float, str, float, Counter[str]]:
    stats: Counter[str] = Counter()
    bestPeer = ""
    bestPeerScore = 0.0
    for peer in targetMemberRelationFramePeers(candidate, source, model)[:RELATION_SLOT_COMPOSITION_PEER_LIMIT]:
        peerScore, peerStats = relationSlotCompositionPairAgreement(source, peer, relations, model)
        stats["slotPeerComparisons"] += 1
        stats["slotPeerRows"] += peerStats.get("slotRows", 0)
        if peerScore > bestPeerScore:
            bestPeerScore = peerScore
            bestPeer = peer
    return slotScore - bestPeerScore, bestPeer, bestPeerScore, stats
...
def targetMemberRelationFrameScores(
    ...
    for candidate, seedScore in seeds.most_common(TARGET_MEMBER_FRAME_CANDIDATE_LIMIT):
        ...
        evidenceRatio = max(0.0, positive) / (1.0 + max(0.0, negative))
        balanceMass = max(MEMBER_BALANCE_MIN_MULTIPLIER, multiplier) * (1.0 + math.log1p(evidenceRatio))
        slotScore, slotStats = relationSlotCompositionPairAgreement(source, candidate, relations, model, candidateFrame)
        slotMargin, slotPeer, slotPeerScore, slotPeerStats = relationSlotCompositionPeerMargin(
            source,
            candidate,
            relations,
            slotScore,
        ...
        slotMass = 1.0
        if slotScore > 0.0:
            slotMass += min(2.4, slotScore * RELATION_SLOT_COMPOSITION_SCORE_GAIN)
            if slotMargin > 0.0:
                slotMass += min(1.6, slotMargin * RELATION_SLOT_COMPOSITION_PEER_GAIN)
            elif slotMargin < -PAIR_LOCAL_CONTRAST_MIN_MARGIN:
                slotMass *= RELATION_SLOT_COMPOSITION_NEGATIVE_PENALTY
        seedMass = math.log1p(max(0.0, float(seedScore)))
        agreementMass = 0.08 + aligned * 4.2 + relation * 0.8 + recursive * 1.0 + order * 0.7 + role * 0.35
        score = seedMass * agreementMass * balanceMass * relationMass * contrastMass * slotMass
        if score <= 0.0:
            continue
        ...
        stats["slotScored"] += int(slotScore > 0.0)
        stats["slotScoreMilli"] += int(slotScore * 1000)
        stats["slotPositiveMargins"] += int(slotMargin > 0.0)
        stats["slotNegativeMargins"] += int(slotMargin < 0.0)
        stats["slotPeerComparisons"] += slotPeerStats.get("slotPeerComparisons", 0)
        stats["slotRows"] += slotStats.get("slotRows", 0)
        stats["slotUsedFragments"] += slotStats.get("slotUsedFragments", 0)
        stats["slotRelationAgreementRows"] += slotStats.get("slotRelationAgreementRows", 0)
        stats["slotOrderAgreementRows"] += slotStats.get("slotOrderAgreementRows", 0)
        stats["slotRecursiveAgreementRows"] += slotStats.get("slotRecursiveAgreementRows", 0)
        stats[
        ...
        if slotPeerScore > 0.0:
            stats["slotPeerAgreementRows"] += 1
        if slotPeer:
            stats[f"slotPeer:{slotPeer}"] += 1
        if full > 0.0:
...
def targetMemberRelationFrameDiagnostics(
    ...
    slotScore, _ = relationSlotCompositionPairAgreement(source, target, relations, model, targetFrame)
    slotMargin, slotPeer, _, _ = relationSlotCompositionPeerMargin(source, target, relations, slotScore, model)
    return (
...
def main() -> None:
    ...
    print(
        ...
        f"slotFragments={RELATION_SLOT_COMPOSITION_FRAGMENT_LIMIT} "
        f"slotRows={RELATION_SLOT_COMPOSITION_ROW_LIMIT} "
        f"slotPeers={RELATION_SLOT_COMPOSITION_PEER_LIMIT}"
    )
    print(
    ...
    targetMemberFrameSlotScored = 0
    targetMemberFrameSlotScoreMilli = 0
    targetMemberFrameSlotPositiveMargins = 0
    targetMemberFrameSlotNegativeMargins = 0
    targetMemberFrameSlotRows = 0
    targetMemberFrameSlotUsedFragments = 0
    ...
    for surface, expected in POSITIVE_PROBES:
        ...
        targetMemberFrameSlotScored += frameStats.get("slotScored", 0)
        targetMemberFrameSlotScoreMilli += frameStats.get("slotScoreMilli", 0)
        targetMemberFrameSlotPositiveMargins += frameStats.get("slotPositiveMargins", 0)
        targetMemberFrameSlotNegativeMargins += frameStats.get("slotNegativeMargins", 0)
        targetMemberFrameSlotRows += frameStats.get("slotRows", 0)
        targetMemberFrameSlotUsedFragments += frameStats.get("slotUsedFragments", 0)
        print(
            f"  {surface}->{expected} relations={','.join(relations)} candidateHit={candidateHit} "
            ...
            f"slot={slotScore:.3f}/margin{slotMargin:.3f}/peer{slotPeer} "
    ...
    print(
        ...
        f"targetMemberFrameSlotScored={targetMemberFrameSlotScored} "
        f"targetMemberFrameSlotScoreMilli={targetMemberFrameSlotScoreMilli} "
        f"targetMemberFrameSlotPositiveMargins={targetMemberFrameSlotPositiveMargins} "
        f"targetMemberFrameSlotNegativeMargins={targetMemberFrameSlotNegativeMargins} "
        f"targetMemberFrameSlotRows={targetMemberFrameSlotRows} "
        f"targetMemberFrameSlotUsedFragments={targetMemberFrameSlotUsedFragments} "
