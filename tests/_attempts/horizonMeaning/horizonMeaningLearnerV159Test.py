"""Horizon Meaning Learner V159 - mutual transport proof over target experience field.

V159 실제 기록
--------------
아이디어:
    V158 은 source path 에서 candidate 로 경험이 운반되는지를 conditional ratio 로 보았지만 실패했다.
    이유는 같은 anchor 계열의 역할 차이를 candidate 쪽에서 검산하지 않았기 때문이다.
    `대출채권 -> 매출채권` 은 source 쪽 transport ratio 가 높아도 target 인 `매출채권` 자신의 경험 field 가
    "대출채권 역할" 을 받아들이는지 증명하지 못했다. `현금배당금 -> 현금및현금성자산` 도 같은 cash anchor
    근처라는 이유로 forbiddenRank=5 까지 살아났다.

    수평선 좌표는 의미단위가 아니다. stem 을 빠르게 찾아가기 위한 고정 주소계다.
    의미는 그 주소가 겪은 주변 stem, 앞뒤 순서, 거리, role, relation, 문장/표 위치 경험이 반복적으로
    겹쳐 만들어지는 누적 경험 graph/field 다. 따라서 "source 가 candidate 쪽으로 갈 수 있다" 는
    절반의 조건이고, candidate/target family 자신의 경험 field 가 그 source fragment path 를 받아들이는지
    역방향으로도 증명해야 한다.

    V159 는 V158 conditional ratio 를 primary 의미 신호에서 내리고, target-side mutual proof 를 추가한다.

    1. candidate 가 relationBoundRolePair 에서 실제 target 으로 등장한 역방향 source field 를 만든다.
    2. query source 의 direct/fragment path(position, source role, relation, ledger source sketch) 와
       candidate reverse-source field 를 비교한다.
    3. candidate 의 같은-anchor/같은-family peer 도 같은 mutual proof 를 계산해 anti-cohort 로 둔다.
       peer 가 더 잘 받아들이면 candidate 를 줄이고, candidate 가 peer 보다 더 잘 받아들일 때만 올린다.
    4. family route 대표 선택도 raw surface score 뒤처리가 아니라 mutual proof 가 있는 canonical/member 를
       우선하게 한다.
    5. 특정 단어, forbidden 패널티, 케이스별 gate 없이 target 경험 field 의 수용 여부만 본다.

    즉 V159 의 판단 단위는 수평선 좌표가 아니라 target 주소에 누적된 역방향 경험 field 이다.

실행 코드:
    $env:DARTLAB_HORIZON_V159_MAX_RECORDS_PER_SOURCE='120'
    $env:DARTLAB_HORIZON_V159_MAX_UNITS='1200'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV159Test.py
...
검증 기준:
    ...
    16. mutual proof 는 candidate target field 의 reverse-source 경험과 query source fragment path 의
        relation/role/ledger/anchor 수용성을 비교해야 한다.
    17. 같은 anchor/family peer 가 더 강하게 source path 를 받아들이면 anti-cohort margin 으로 감점한다.
    18. route 는 surface score 를 누적 경험 family 로 먼저 collapse 한 뒤 mutual proof 대표를 고른다.
    19. 특정 surface 하드코딩, forbidden 단어 패널티, 케이스별 gate, 단순 가중치 미세조정 없이 일반 cold-start composition 구조만 사용한다.
    20. `외상매출금 -> 매출채권` 은 V147/V148 rank=7 보다 개선되어야 하며, mutualProof 진단을 기록한다.
    21. fixed route/search guard 는 positiveHits=3/4, badAccepted=0/7 수준을 유지해야 한다.

결과:
    1,200 unit smoke 에서 relationSlotSubstitution 은 rawKeys=951, keptKeys=863, surfaceKeys=3,870,
    rawLinks=70,366, keptLinks=16,011, ownerSources=5,460, ownerTargets=11,326 로 V158 보다 recall 이
    조금 늘었다.

    targetMemberFrame 은 Top1=1/4, Top5=1/4, BadTop1=0/7, BadTop5=1/7, candidateHits=5/11,
    candidateRows=1,046, scored=1,032 였다. V158 의 Top1=0/4 에서 cash case 하나가 Top1 로 올라왔지만,
    guard 는 BadTop5=1/7 로 유지됐고 candidate hit 는 6/11 에서 5/11 로 줄었다.

    mutual proof layer 는 pairScored=1,032, proofMilli=185,008, rawMilli=9,505,630,
    opportunityMilli=37,747,609, reverseRows=32,065, pairRows=398,706, acceptedRows=700,
    positiveMargins=86, negativeMargins=673 으로 실제 계산됐다. 하지만 margin 총합은 -56,210 milli 로
    peer/anti-cohort 에 대체로 졌다. family route 는 canonicalRepresentativeRows=0,
    rawRepresentativeRows=792 로 V158 과 같이 canonical 대표 학습에는 실패했다.

    `현금성자산 -> 현금및현금성자산` 은 rank=1 로 개선됐고 mutual=0.485, reverseRows=48, pairs=864 로
    target-side proof 가 실제로 열렸다. 반면 `외상매출금 -> 매출채권` 은 candidateHit=True 이지만
    mutual=0.000, reverseRows=0, rank=NA 였다. `영업손익 -> 영업이익` 도 candidateHit=False,
    mutual=0.000, reverseRows=0, rank=NA 였다. `손실충당금 -> 대손충당금` 은 candidateHit=True 이지만
    mutual=0.000, reverseRows=0 이고 V158 rank=5 에서 rank=8 로 악화됐다.

    negative 에서는 `현금배당금 -> 현금및현금성자산` 이 forbiddenRank=NA 로 V158 의 forbiddenRank=5 를
    제거했지만, `당기순이익 -> 영업이익` 이 forbiddenRank=2 로 BadTop5 를 만들었다. `대출채권 -> 매출채권` 은
    candidateHit=False, mutual=0.000, forbiddenRank=NA 였다. fixed route/search guard 는
    positiveHits=3/4, badAccepted=0/7, searchTop1=4/5, reliableSearch=4/5, totalSeconds=823.9 이었다.

결론:
    실패/부분 개념 신호. target-side mutual proof 는 reverse field 가 존재하는 target 에서는 강한
    확인 신호가 된다. `현금성자산 -> 현금및현금성자산` 을 Top1 로 복구했고, V158 의
    `현금배당금 -> 현금및현금성자산` forbidden top5 도 제거했다.

    하지만 rare semantic transfer 의 핵심 병목은 해결하지 못했다. `매출채권`, `영업이익`, `대손충당금` 의
    exact target reverseRows 가 0 이라 mutual proof 가 작동하지 않았다. 즉 target surface 자체가 target 으로
    충분히 관측되지 않으면, exact reverse field 방식은 cold-start 의미장을 만들 수 없다. 또한
    `당기순이익 -> 영업이익` forbiddenRank=2 는 같은 손익/성과 축에서 target-side proof 없이 relation/frame
    점수만으로 잘못 살아나는 anti-cohort 병목을 보여준다.

    keep 할 것은 target-side 경험 field 로 source path 를 검산하는 방향이다. 폐기할 것은 exact target
    surface 에 reverse field 가 있어야만 proof 가 생기는 구조와, raw score 이후 family 대표를 고르는 방식이다.
    다음은 candidate surface 가 아니라 candidate family/role/ledger-axis slot 에 역방향 경험을 누적해야 한다.
    `매출채권` exact reverseRows 가 없어도 `매출`, `채권`, owner-bound receivable 역할 field 에서 target-side
    family proof 를 만들고, 같은 손익 축의 `당기순이익 -> 영업이익` 은 role/relation anti-cohort 로 분리해야 한다.
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V159_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V159_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V159_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V159_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V159_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V159_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V159_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V159_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V159_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_SEARCH_CANDIDATE_LIMIT", "420"))
DYNAMIC_TARGET_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_DYNAMIC_TARGET_LIMIT", "80"))
DYNAMIC_COORD_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_DYNAMIC_COORD_ROW_LIMIT", "220"))
DYNAMIC_COMPOUND_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_DYNAMIC_COMPOUND_ROW_LIMIT", "260"))
DYNAMIC_QUERY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_DYNAMIC_QUERY_ATOM_LIMIT", "48"))
DYNAMIC_MEANING_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_DYNAMIC_MEANING_ATOM_LIMIT", "36"))
DYNAMIC_MEANING_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_DYNAMIC_MEANING_ROW_LIMIT", "220"))
DYNAMIC_RELATION_SURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_DYNAMIC_RELATION_SURFACE_LIMIT", "420"))
DYNAMIC_RELATION_UNIT_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_DYNAMIC_RELATION_UNIT_LIMIT", "160"))
DYNAMIC_BRIDGE_ONLY_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V159_DYNAMIC_BRIDGE_ONLY_PENALTY", "0.80"))
DYNAMIC_OWNER_ROLE_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_DYNAMIC_OWNER_ROLE_SIGNATURE_LIMIT", "64")
)
DYNAMIC_OWNER_ROLE_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_DYNAMIC_OWNER_ROLE_ATOM_LIMIT", "40"))
DYNAMIC_OWNER_ROLE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_DYNAMIC_OWNER_ROLE_ROW_LIMIT", "220"))
DYNAMIC_OWNER_ROLE_UNIT_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_DYNAMIC_OWNER_ROLE_UNIT_ATOM_LIMIT", "36")
)
DYNAMIC_OWNER_ROLE_MIN_BOUND = float(os.environ.get("DARTLAB_HORIZON_V159_DYNAMIC_OWNER_ROLE_MIN_BOUND", "0.55"))
DYNAMIC_OWNER_ROLE_CANDIDATE_BONUS = float(
    os.environ.get("DARTLAB_HORIZON_V159_DYNAMIC_OWNER_ROLE_CANDIDATE_BONUS", "2.40")
)
DYNAMIC_OWNER_ROLE_ROUTE_BONUS = float(os.environ.get("DARTLAB_HORIZON_V159_DYNAMIC_OWNER_ROLE_ROUTE_BONUS", "0.90"))
DYNAMIC_OWNER_ROLE_WEAK_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V159_DYNAMIC_OWNER_ROLE_WEAK_PENALTY", "0.16"))
DYNAMIC_OWNER_FRAME_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V159_DYNAMIC_OWNER_FRAME_RADIUS", "5"))
DYNAMIC_OWNER_FRAME_BETWEEN_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_DYNAMIC_OWNER_FRAME_BETWEEN_LIMIT", "7"))
MASKED_FRAME_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_MASKED_FRAME_SIGNATURE_LIMIT", "72"))
MASKED_FRAME_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_MASKED_FRAME_ATOM_LIMIT", "48"))
MASKED_FRAME_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_MASKED_FRAME_ROW_LIMIT", "240"))
MASKED_FRAME_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_MASKED_FRAME_EVAL_LIMIT", "800"))
MASKED_FRAME_HOLDOUT_MOD = int(os.environ.get("DARTLAB_HORIZON_V159_MASKED_FRAME_HOLDOUT_MOD", "5"))
MASKED_FRAME_PROBE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_MASKED_FRAME_PROBE_LIMIT", "12"))
MASKED_CONTEXT_SURFACE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_MASKED_CONTEXT_SURFACE_ROW_LIMIT", "12"))
MASKED_CONTEXT_WEIGHT = float(os.environ.get("DARTLAB_HORIZON_V159_MASKED_CONTEXT_WEIGHT", "0.35"))
MASKED_CONTEXT_RELATION_WINDOW = int(os.environ.get("DARTLAB_HORIZON_V159_MASKED_CONTEXT_RELATION_WINDOW", "140"))
MASKED_FRAME_SURPRISAL_POWER = float(os.environ.get("DARTLAB_HORIZON_V159_MASKED_FRAME_SURPRISAL_POWER", "2.0"))
MASKED_FRAME_SURFACE_PRIOR_POWER = float(
    os.environ.get("DARTLAB_HORIZON_V159_MASKED_FRAME_SURFACE_PRIOR_POWER", "0.45")
)
RELATION_OWNER_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_RELATION_OWNER_SIGNATURE_LIMIT", "64"))
RELATION_OWNER_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_RELATION_OWNER_ROW_LIMIT", "220"))
RELATION_OWNER_BRIDGE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_RELATION_OWNER_BRIDGE_LIMIT", "12"))
RELATION_OWNER_PROJECTION_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_RELATION_OWNER_PROJECTION_LIMIT", "24"))
RELATION_OWNER_PROJECTION_OWNER_ROLE_MIN = float(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_OWNER_PROJECTION_OWNER_ROLE_MIN", "0.18")
)
RELATION_OWNER_PROJECTION_QUERY_ROLE_MIN = float(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_OWNER_PROJECTION_QUERY_ROLE_MIN", "0.05")
)
RELATION_OWNER_GENERATED_BRIDGE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_OWNER_GENERATED_BRIDGE_LIMIT", "48")
)
RELATION_OWNER_GENERATED_BRIDGE_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_OWNER_GENERATED_BRIDGE_WEIGHT", "2.4")
)
RELATION_OWNER_ROLE_COMPAT_MIN = float(os.environ.get("DARTLAB_HORIZON_V159_RELATION_OWNER_ROLE_COMPAT_MIN", "0.16"))
RELATION_BOUND_ROLE_PAIR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_RELATION_BOUND_ROLE_PAIR_LIMIT", "24"))
RELATION_BOUND_ROLE_PAIR_BRIDGE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_BOUND_ROLE_PAIR_BRIDGE_LIMIT", "10")
)
RELATION_BOUND_ROLE_PAIR_BRIDGE_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_BOUND_ROLE_PAIR_BRIDGE_WEIGHT", "0.46")
)
RELATION_SOURCE_EXPERIENCE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_SOURCE_EXPERIENCE_ATOM_LIMIT", "44")
)
RELATION_SOURCE_EXPERIENCE_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_SOURCE_EXPERIENCE_ROW_LIMIT", "180")
)
RELATION_SOURCE_SHINGLE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_SOURCE_SHINGLE_ATOM_LIMIT", "28")
)
RELATION_SOURCE_SHINGLE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_RELATION_SOURCE_SHINGLE_ROW_LIMIT", "160"))
RELATION_SOURCE_SHINGLE_KEY_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_RELATION_SOURCE_SHINGLE_KEY_LIMIT", "56"))
RELATION_ANCHORED_COARSE_SHINGLE_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_ANCHORED_COARSE_SHINGLE_ROW_LIMIT", "180")
)
RELATION_ANCHORED_COARSE_SHINGLE_KEY_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_ANCHORED_COARSE_SHINGLE_KEY_LIMIT", "72")
)
HORIZON_MASK_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_HORIZON_MASK_SIGNATURE_LIMIT", "96"))
HORIZON_MASK_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_HORIZON_MASK_ATOM_LIMIT", "56"))
HORIZON_MASK_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_HORIZON_MASK_ROW_LIMIT", "220"))
HORIZON_MASK_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_HORIZON_MASK_EVAL_LIMIT", "900"))
HORIZON_MASK_HOLDOUT_MOD = int(os.environ.get("DARTLAB_HORIZON_V159_HORIZON_MASK_HOLDOUT_MOD", "7"))
HORIZON_MASK_SURFACE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_HORIZON_MASK_SURFACE_ROW_LIMIT", "24"))
HORIZON_MASK_NEIGHBOR_EXPERIENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_HORIZON_MASK_NEIGHBOR_EXPERIENCE_LIMIT", "5")
)
HORIZON_MASK_COORD_CELL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_HORIZON_MASK_COORD_CELL_LIMIT", "5"))
HORIZON_MASK_COORD_BUCKET_DIGITS = int(os.environ.get("DARTLAB_HORIZON_V159_HORIZON_MASK_COORD_BUCKET_DIGITS", "5"))
LOCAL_FRAME_PREDICTION_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_LOCAL_FRAME_PREDICTION_SIGNATURE_LIMIT", "132")
)
LOCAL_FRAME_PREDICTION_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_LOCAL_FRAME_PREDICTION_ATOM_LIMIT", "76"))
LOCAL_FRAME_PREDICTION_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_LOCAL_FRAME_PREDICTION_ROW_LIMIT", "260"))
LOCAL_FRAME_PREDICTION_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_LOCAL_FRAME_PREDICTION_EVAL_LIMIT", "900"))
LOCAL_FRAME_PREDICTION_SURFACE_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_LOCAL_FRAME_PREDICTION_SURFACE_ROW_LIMIT", "18")
)
LOCAL_FRAME_PREDICTION_HOLDOUT_MOD = int(os.environ.get("DARTLAB_HORIZON_V159_LOCAL_FRAME_PREDICTION_HOLDOUT_MOD", "9"))
LOCAL_FRAME_PREDICTION_NEIGHBOR_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_LOCAL_FRAME_PREDICTION_NEIGHBOR_LIMIT", "5")
)
LOCAL_FRAME_PREDICTION_NEIGHBOR_EXPERIENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_LOCAL_FRAME_PREDICTION_NEIGHBOR_EXPERIENCE_LIMIT", "5")
)
LOCAL_FRAME_PREDICTION_SEQUENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_LOCAL_FRAME_PREDICTION_SEQUENCE_LIMIT", "5")
)
QUERY_LOCAL_FRAME_OCCURRENCE_INDEX_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_QUERY_LOCAL_FRAME_OCCURRENCE_INDEX_ROW_LIMIT", "96")
)
QUERY_LOCAL_FRAME_OCCURRENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_QUERY_LOCAL_FRAME_OCCURRENCE_LIMIT", "72")
)
QUERY_LOCAL_FRAME_PROXY_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_QUERY_LOCAL_FRAME_PROXY_LIMIT", "8"))
QUERY_LOCAL_FRAME_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_QUERY_LOCAL_FRAME_ATOM_LIMIT", "132"))
QUERY_LOCAL_FRAME_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_QUERY_LOCAL_FRAME_EVAL_LIMIT", "260"))
QUERY_SEMANTIC_UNIT_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_QUERY_SEMANTIC_UNIT_ATOM_LIMIT", "120"))
QUERY_SEMANTIC_UNIT_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_QUERY_SEMANTIC_UNIT_EVAL_LIMIT", "260"))
QUERY_SEMANTIC_UNIT_SURFACE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_QUERY_SEMANTIC_UNIT_SURFACE_LIMIT", str(MASKED_FRAME_PROBE_LIMIT))
)
SEMANTIC_UNIT_SIGNATURE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_SEMANTIC_UNIT_SIGNATURE_ATOM_LIMIT", "18")
)
SEMANTIC_UNIT_POSTING_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_SEMANTIC_UNIT_POSTING_ROW_LIMIT", "90"))
SEMANTIC_UNIT_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_SEMANTIC_UNIT_CANDIDATE_LIMIT", "180"))
SEMANTIC_UNIT_MIN_EXPERIENCE_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V159_SEMANTIC_UNIT_MIN_EXPERIENCE_SIM", "0.46")
)
SEMANTIC_UNIT_MIN_MIXED_SIM = float(os.environ.get("DARTLAB_HORIZON_V159_SEMANTIC_UNIT_MIN_MIXED_SIM", "0.34"))
SEMANTIC_UNIT_MIN_COORD_RESONANCE = float(
    os.environ.get("DARTLAB_HORIZON_V159_SEMANTIC_UNIT_MIN_COORD_RESONANCE", "0.045")
)
SEMANTIC_UNIT_MASK_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_SEMANTIC_UNIT_MASK_SIGNATURE_LIMIT", "120")
)
SEMANTIC_UNIT_MASK_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_SEMANTIC_UNIT_MASK_ATOM_LIMIT", "64"))
SEMANTIC_UNIT_MASK_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_SEMANTIC_UNIT_MASK_ROW_LIMIT", "260"))
SEMANTIC_UNIT_ROUTE_MEMBER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_SEMANTIC_UNIT_ROUTE_MEMBER_LIMIT", "4"))
CONTRAST_SEMANTIC_UNIT_MIN_EXPERIENCE_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V159_CONTRAST_SEMANTIC_UNIT_MIN_EXPERIENCE_SIM", "0.46")
)
CONTRAST_SEMANTIC_UNIT_STRONG_EXPERIENCE_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V159_CONTRAST_SEMANTIC_UNIT_STRONG_EXPERIENCE_SIM", "0.62")
)
CONTRAST_SEMANTIC_UNIT_MIN_MIXED_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V159_CONTRAST_SEMANTIC_UNIT_MIN_MIXED_SIM", "0.36")
)
CONTRAST_SEMANTIC_UNIT_MIN_COORD_RESONANCE = float(
    os.environ.get("DARTLAB_HORIZON_V159_CONTRAST_SEMANTIC_UNIT_MIN_COORD_RESONANCE", "0.050")
)
CONTRAST_SEMANTIC_UNIT_MIN_RELATION_COMPAT = float(
    os.environ.get("DARTLAB_HORIZON_V159_CONTRAST_SEMANTIC_UNIT_MIN_RELATION_COMPAT", "0.28")
)
CONTRAST_SEMANTIC_UNIT_SUFFIX_RELATION_COMPAT = float(
    os.environ.get("DARTLAB_HORIZON_V159_CONTRAST_SEMANTIC_UNIT_SUFFIX_RELATION_COMPAT", "0.18")
)
MEMBER_BALANCE_MEMBER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_MEMBER_BALANCE_MEMBER_LIMIT", "10"))
MEMBER_BALANCE_MIN_MULTIPLIER = float(os.environ.get("DARTLAB_HORIZON_V159_MEMBER_BALANCE_MIN_MULTIPLIER", "0.015"))
MEMBER_BALANCE_UNSUPPORTED_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V159_MEMBER_BALANCE_UNSUPPORTED_FACTOR", "0.12")
)
MEMBER_BALANCE_POSITIVE_GAIN = float(os.environ.get("DARTLAB_HORIZON_V159_MEMBER_BALANCE_POSITIVE_GAIN", "1.10"))
MEMBER_BALANCE_NEGATIVE_GAIN = float(os.environ.get("DARTLAB_HORIZON_V159_MEMBER_BALANCE_NEGATIVE_GAIN", "1.60"))
MEMBER_BALANCE_POSITIVE_SHIELD_RATIO = float(
    os.environ.get("DARTLAB_HORIZON_V159_MEMBER_BALANCE_POSITIVE_SHIELD_RATIO", "0.85")
)
...
MEMBER_CONDITIONED_MASK_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_MEMBER_CONDITIONED_MASK_SIGNATURE_LIMIT", "96")
)
MEMBER_CONDITIONED_MASK_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_MEMBER_CONDITIONED_MASK_ATOM_LIMIT", "64")
)
MEMBER_CONDITIONED_MASK_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_MEMBER_CONDITIONED_MASK_ROW_LIMIT", "220"))
MEMBER_CONDITIONED_ROUTE_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_MEMBER_CONDITIONED_ROUTE_MEMBER_LIMIT", "180")
)
MEMBER_CONDITIONED_LABEL_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_MEMBER_CONDITIONED_LABEL_MEMBER_LIMIT", "6")
)
MEMBER_CONDITIONED_MEMBER_CACHE: dict[tuple[int, str, str], tuple[float, float, float, int]] = {}
MEMBER_SUPPORTED_LABEL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_MEMBER_SUPPORTED_LABEL_LIMIT", "80"))
MEMBER_SUPPORTED_MEMBER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_MEMBER_SUPPORTED_MEMBER_LIMIT", "120"))
MEMBER_SUPPORTED_ACTIVE_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_MEMBER_SUPPORTED_ACTIVE_MEMBER_LIMIT", "8")
)
MEMBER_SUPPORTED_MIN_MULTIPLIER = float(
    os.environ.get("DARTLAB_HORIZON_V159_MEMBER_SUPPORTED_MIN_MULTIPLIER", str(MEMBER_BALANCE_MIN_MULTIPLIER))
)
...
MEMBER_ROLE_RESIDUAL_MASK_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_MEMBER_ROLE_RESIDUAL_MASK_SIGNATURE_LIMIT", "96")
)
MEMBER_ROLE_RESIDUAL_MASK_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_MEMBER_ROLE_RESIDUAL_MASK_ATOM_LIMIT", "64")
)
MEMBER_ROLE_RESIDUAL_MASK_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_MEMBER_ROLE_RESIDUAL_MASK_ROW_LIMIT", "220")
)
MEMBER_ROLE_RESIDUAL_LABEL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_MEMBER_ROLE_RESIDUAL_LABEL_LIMIT", "80"))
MEMBER_ROLE_RESIDUAL_MIN_MULTIPLIER = float(
    os.environ.get("DARTLAB_HORIZON_V159_MEMBER_ROLE_RESIDUAL_MIN_MULTIPLIER", "0.25")
)
MEMBER_ROLE_RESIDUAL_CONFLICT_RATIO = float(
    os.environ.get("DARTLAB_HORIZON_V159_MEMBER_ROLE_RESIDUAL_CONFLICT_RATIO", "1.35")
)
MEMBER_TARGET_SELECTOR_LABEL_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_MEMBER_TARGET_SELECTOR_LABEL_LIMIT", "48")
)
MEMBER_TARGET_SELECTOR_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_MEMBER_TARGET_SELECTOR_MEMBER_LIMIT", "14")
)
MEMBER_TARGET_SELECTOR_BALANCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_MEMBER_TARGET_SELECTOR_BALANCE_LIMIT", "96")
)
MEMBER_TARGET_SELECTOR_SURFACE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_MEMBER_TARGET_SELECTOR_SURFACE_LIMIT", str(MASKED_FRAME_PROBE_LIMIT))
)
SELECTED_TARGET_PROJECTION_SOURCE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_SELECTED_TARGET_PROJECTION_SOURCE_ATOM_LIMIT", "72")
)
SELECTED_TARGET_PROJECTION_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_SELECTED_TARGET_PROJECTION_ROW_LIMIT", "220")
)
SELECTED_TARGET_PROJECTION_TARGET_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_SELECTED_TARGET_PROJECTION_TARGET_LIMIT", "8")
)
SELECTED_TARGET_PROJECTION_SELECTOR_LABEL_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_SELECTED_TARGET_PROJECTION_SELECTOR_LABEL_LIMIT", "12")
)
SELECTED_TARGET_PROJECTION_SELECTOR_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_SELECTED_TARGET_PROJECTION_SELECTOR_MEMBER_LIMIT", "8")
)
SELECTOR_PATH_JOIN_SOURCE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_SELECTOR_PATH_JOIN_SOURCE_LIMIT", "12"))
SELECTOR_PATH_JOIN_PAIR_TARGET_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_SELECTOR_PATH_JOIN_PAIR_TARGET_LIMIT", "16")
)
SELECTOR_PATH_JOIN_SELECTED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_SELECTOR_PATH_JOIN_SELECTED_LIMIT", "8"))
SELECTOR_PATH_JOIN_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_SELECTOR_PATH_JOIN_ROW_LIMIT", "36"))
BIDIRECTIONAL_PAIR_CANDIDATE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_BIDIRECTIONAL_PAIR_CANDIDATE_LIMIT", "96")
)
BIDIRECTIONAL_PAIR_ROUTE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_BIDIRECTIONAL_PAIR_ROUTE_LIMIT", "12"))
BIDIRECTIONAL_PAIR_DYNAMIC_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_BIDIRECTIONAL_PAIR_DYNAMIC_LIMIT", "80"))
BIDIRECTIONAL_PAIR_REVERSE_RELATION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_BIDIRECTIONAL_PAIR_REVERSE_RELATION_LIMIT", "2")
)
PAIR_LOCAL_CONTRAST_CANDIDATE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_PAIR_LOCAL_CONTRAST_CANDIDATE_LIMIT", "96")
)
PAIR_LOCAL_CONTRAST_PEER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_PAIR_LOCAL_CONTRAST_PEER_LIMIT", "18"))
PAIR_LOCAL_CONTRAST_NEAR_BONUS = float(os.environ.get("DARTLAB_HORIZON_V159_PAIR_LOCAL_CONTRAST_NEAR_BONUS", "0.42"))
PAIR_LOCAL_CONTRAST_MIN_MARGIN = float(os.environ.get("DARTLAB_HORIZON_V159_PAIR_LOCAL_CONTRAST_MIN_MARGIN", "0.015"))
TARGET_MEMBER_FRAME_OCCURRENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_FRAME_OCCURRENCE_LIMIT", "56")
)
TARGET_MEMBER_FRAME_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_FRAME_ATOM_LIMIT", "120"))
TARGET_MEMBER_FRAME_PEER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_FRAME_PEER_LIMIT", "8"))
TARGET_MEMBER_FRAME_ROUTE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_FRAME_ROUTE_LIMIT", str(MASKED_FRAME_PROBE_LIMIT))
)
TARGET_MEMBER_FRAME_NEIGHBOR_RADIUS = int(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_FRAME_NEIGHBOR_RADIUS", "3")
)
TARGET_MEMBER_FRAME_NEIGHBOR_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_FRAME_NEIGHBOR_ATOM_LIMIT", "14")
)
TARGET_MEMBER_FRAME_RELATION_WINDOW = int(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_FRAME_RELATION_WINDOW", "8")
)
TARGET_MEMBER_FRAME_CANDIDATE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_FRAME_CANDIDATE_LIMIT", str(BIDIRECTIONAL_PAIR_CANDIDATE_LIMIT))
)
TARGET_MEMBER_FRAME_QUERY_ROUTE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_FRAME_QUERY_ROUTE_LIMIT", "24")
)
COMPOSITIONAL_FRAME_MIN_QUERY_ROWS = int(
    os.environ.get("DARTLAB_HORIZON_V159_COMPOSITIONAL_FRAME_MIN_QUERY_ROWS", "24")
)
COMPOSITIONAL_FRAME_FRAGMENT_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_COMPOSITIONAL_FRAME_FRAGMENT_LIMIT", "10")
)
COMPOSITIONAL_FRAME_FRAGMENT_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_COMPOSITIONAL_FRAME_FRAGMENT_ROW_LIMIT", "18")
)
COMPOSITIONAL_FRAME_MIN_FRAGMENT_LEN = int(
    os.environ.get("DARTLAB_HORIZON_V159_COMPOSITIONAL_FRAME_MIN_FRAGMENT_LEN", "2")
)
COMPOSITIONAL_FRAME_MAX_FRAGMENT_LEN = int(
    os.environ.get("DARTLAB_HORIZON_V159_COMPOSITIONAL_FRAME_MAX_FRAGMENT_LEN", "6")
)
COMPOSITIONAL_FRAME_ATOM_SCALE = float(os.environ.get("DARTLAB_HORIZON_V159_COMPOSITIONAL_FRAME_ATOM_SCALE", "0.58"))
COMPOSITIONAL_FRAME_SIGNATURE_SCALE = float(
    os.environ.get("DARTLAB_HORIZON_V159_COMPOSITIONAL_FRAME_SIGNATURE_SCALE", "0.24")
)
RELATION_SLOT_SUBSTITUTION_WINDOW = int(os.environ.get("DARTLAB_HORIZON_V159_RELATION_SLOT_SUBSTITUTION_WINDOW", "8"))
RELATION_SLOT_SUBSTITUTION_MIN_KEY_DF = int(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_SLOT_SUBSTITUTION_MIN_KEY_DF", "2")
)
RELATION_SLOT_SUBSTITUTION_MAX_KEY_DF = int(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_SLOT_SUBSTITUTION_MAX_KEY_DF", "260")
)
RELATION_SLOT_SUBSTITUTION_POSTING_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_SLOT_SUBSTITUTION_POSTING_LIMIT", "80")
)
RELATION_SLOT_SUBSTITUTION_SURFACE_KEY_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_SLOT_SUBSTITUTION_SURFACE_KEY_LIMIT", "36")
)
RELATION_SLOT_SUBSTITUTION_QUERY_KEY_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_SLOT_SUBSTITUTION_QUERY_KEY_LIMIT", "64")
)
RELATION_SLOT_SUBSTITUTION_FRAGMENT_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_SLOT_SUBSTITUTION_FRAGMENT_LIMIT", "8")
)
RELATION_SLOT_SUBSTITUTION_ROUTE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_SLOT_SUBSTITUTION_ROUTE_LIMIT", str(MASKED_FRAME_PROBE_LIMIT))
)
RELATION_SLOT_SUBSTITUTION_SEED_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_SLOT_SUBSTITUTION_SEED_WEIGHT", "1.15")
)
RELATION_SLOT_SUBSTITUTION_TYPED_SPECIFICITY_MIN = float(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_SLOT_SUBSTITUTION_TYPED_SPECIFICITY_MIN", "0.16")
)
RELATION_SLOT_SUBSTITUTION_TYPED_HUBNESS_MAX = float(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_SLOT_SUBSTITUTION_TYPED_HUBNESS_MAX", "0.66")
)
RELATION_SLOT_SUBSTITUTION_TYPED_ENTROPY_MAX = float(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_SLOT_SUBSTITUTION_TYPED_ENTROPY_MAX", "0.86")
)
RELATION_SLOT_SUBSTITUTION_TYPED_CONCEPT_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_SLOT_SUBSTITUTION_TYPED_CONCEPT_WEIGHT", "1.18")
)
RELATION_SLOT_SUBSTITUTION_TYPED_GENERIC_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_SLOT_SUBSTITUTION_TYPED_GENERIC_WEIGHT", "0.42")
)
RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_TARGET_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_TARGET_LIMIT", "10")
)
RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_WEAK_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_WEAK_WEIGHT", "0.52")
)
RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_GENERIC_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_GENERIC_WEIGHT", "0.24")
)
RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_TARGET_KEY_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_TARGET_KEY_WEIGHT", "0.32")
)
RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_MIN_PAIR_SCORE = float(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_MIN_PAIR_SCORE", "0.035")
)
RELATION_SLOT_SUBSTITUTION_LEDGER_OCCURRENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_SLOT_SUBSTITUTION_LEDGER_OCCURRENCE_LIMIT", "48")
)
RELATION_SLOT_SUBSTITUTION_LEDGER_TARGET_KEY_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_SLOT_SUBSTITUTION_LEDGER_TARGET_KEY_WEIGHT", "0.38")
)
RELATION_SLOT_SUBSTITUTION_LEDGER_BROAD_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V159_RELATION_SLOT_SUBSTITUTION_LEDGER_BROAD_WEIGHT", "0.34")
)
PAIR_LOCAL_LEDGER_FRAGMENT_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_PAIR_LOCAL_LEDGER_FRAGMENT_LIMIT", "10"))
PAIR_LOCAL_LEDGER_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_PAIR_LOCAL_LEDGER_ATOM_LIMIT", "48"))
PAIR_LOCAL_LEDGER_BAND_SIZE = int(os.environ.get("DARTLAB_HORIZON_V159_PAIR_LOCAL_LEDGER_BAND_SIZE", "3"))
PAIR_LOCAL_LEDGER_BAND_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_PAIR_LOCAL_LEDGER_BAND_LIMIT", "12"))
PAIR_LOCAL_LEDGER_GAIN = float(os.environ.get("DARTLAB_HORIZON_V159_PAIR_LOCAL_LEDGER_GAIN", "1.45"))
PAIR_LOCAL_LEDGER_MARGIN_GAIN = float(os.environ.get("DARTLAB_HORIZON_V159_PAIR_LOCAL_LEDGER_MARGIN_GAIN", "2.20"))
PAIR_LOCAL_LEDGER_NEGATIVE_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V159_PAIR_LOCAL_LEDGER_NEGATIVE_FACTOR", "0.74")
)
PAIR_LOCAL_LEDGER_EMPTY_FACTOR = float(os.environ.get("DARTLAB_HORIZON_V159_PAIR_LOCAL_LEDGER_EMPTY_FACTOR", "0.88"))
PAIR_LOCAL_LEDGER_MIN_MARGIN = float(os.environ.get("DARTLAB_HORIZON_V159_PAIR_LOCAL_LEDGER_MIN_MARGIN", "0.018"))
PAIR_LOCAL_LEDGER_RESIDUAL_COMMON_POWER = float(
    os.environ.get("DARTLAB_HORIZON_V159_PAIR_LOCAL_LEDGER_RESIDUAL_COMMON_POWER", "0.72")
)
PAIR_LOCAL_LEDGER_RESIDUAL_IDF_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V159_PAIR_LOCAL_LEDGER_RESIDUAL_IDF_GAIN", "0.58")
)
PAIR_LOCAL_LEDGER_RESIDUAL_MIN_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V159_PAIR_LOCAL_LEDGER_RESIDUAL_MIN_WEIGHT", "0.018")
)
PAIR_LOCAL_LEDGER_RESIDUAL_PEER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_PAIR_LOCAL_LEDGER_RESIDUAL_PEER_LIMIT", "8")
)
DIRECTIONAL_LEDGER_FRAGMENT_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_DIRECTIONAL_LEDGER_FRAGMENT_LIMIT", "8"))
DIRECTIONAL_LEDGER_SOURCE_TARGET_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_DIRECTIONAL_LEDGER_SOURCE_TARGET_LIMIT", "10")
)
DIRECTIONAL_LEDGER_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_DIRECTIONAL_LEDGER_ATOM_LIMIT", "64"))
DIRECTIONAL_LEDGER_GAIN = float(os.environ.get("DARTLAB_HORIZON_V159_DIRECTIONAL_LEDGER_GAIN", "1.72"))
DIRECTIONAL_LEDGER_MARGIN_GAIN = float(os.environ.get("DARTLAB_HORIZON_V159_DIRECTIONAL_LEDGER_MARGIN_GAIN", "2.45"))
DIRECTIONAL_LEDGER_NEGATIVE_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V159_DIRECTIONAL_LEDGER_NEGATIVE_FACTOR", "0.72")
)
DIRECTIONAL_LEDGER_EMPTY_FACTOR = float(os.environ.get("DARTLAB_HORIZON_V159_DIRECTIONAL_LEDGER_EMPTY_FACTOR", "0.90"))
DIRECTIONAL_LEDGER_MIN_MARGIN = float(os.environ.get("DARTLAB_HORIZON_V159_DIRECTIONAL_LEDGER_MIN_MARGIN", "0.014"))
TARGET_MEMBER_PRIOR_SOURCE_TARGET_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_PRIOR_SOURCE_TARGET_LIMIT", "16")
)
TARGET_MEMBER_PRIOR_HARD_NEGATIVE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_PRIOR_HARD_NEGATIVE_LIMIT", "14")
)
TARGET_MEMBER_PRIOR_GAIN = float(os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_PRIOR_GAIN", "2.35"))
TARGET_MEMBER_PRIOR_MARGIN_GAIN = float(os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_PRIOR_MARGIN_GAIN", "2.10"))
TARGET_MEMBER_PRIOR_NEGATIVE_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_PRIOR_NEGATIVE_FACTOR", "0.68")
)
TARGET_MEMBER_PRIOR_EMPTY_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_PRIOR_EMPTY_FACTOR", "0.92")
)
TARGET_MEMBER_PRIOR_MIN_MARGIN = float(os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_PRIOR_MIN_MARGIN", "0.012"))
TARGET_MEMBER_TRANSPORT_GAIN = float(os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_TRANSPORT_GAIN", "3.20"))
TARGET_MEMBER_TRANSPORT_ANCHOR_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_TRANSPORT_ANCHOR_GAIN", "1.35")
)
TARGET_MEMBER_TRANSPORT_ROLE_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_TRANSPORT_ROLE_GAIN", "0.86")
)
TARGET_MEMBER_TRANSPORT_EMPTY_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_TRANSPORT_EMPTY_FACTOR", "0.94")
)
TARGET_MEMBER_TRANSPORT_LOW_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_TRANSPORT_LOW_FACTOR", "0.78")
)
TARGET_MEMBER_TRANSPORT_MIN_LIKELIHOOD = float(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_TRANSPORT_MIN_LIKELIHOOD", "0.045")
)
TARGET_MEMBER_CONDITIONAL_TRANSPORT_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_CONDITIONAL_TRANSPORT_GAIN", "2.35")
)
TARGET_MEMBER_CONDITIONAL_TRANSPORT_RESIDUAL_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_CONDITIONAL_TRANSPORT_RESIDUAL_GAIN", "1.15")
)
TARGET_MEMBER_CONDITIONAL_ANCHOR_RESIDUAL_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_CONDITIONAL_ANCHOR_RESIDUAL_GAIN", "0.72")
)
TARGET_MEMBER_CONDITIONAL_TRANSPORT_EMPTY_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_CONDITIONAL_TRANSPORT_EMPTY_FACTOR", "0.92")
)
TARGET_MEMBER_CONDITIONAL_TRANSPORT_LOW_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_CONDITIONAL_TRANSPORT_LOW_FACTOR", "0.82")
)
TARGET_MEMBER_CONDITIONAL_TRANSPORT_PEER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_CONDITIONAL_TRANSPORT_PEER_LIMIT", "10")
)
TARGET_MEMBER_EXPERIENCE_FAMILY_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_EXPERIENCE_FAMILY_MEMBER_LIMIT", "8")
)
TARGET_MEMBER_EXPERIENCE_FAMILY_SCORE_FLOOR = float(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_EXPERIENCE_FAMILY_SCORE_FLOOR", "0.045")
)
TARGET_MEMBER_MUTUAL_PROOF_SOURCE_PATH_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_MUTUAL_PROOF_SOURCE_PATH_LIMIT", "18")
)
TARGET_MEMBER_MUTUAL_PROOF_REVERSE_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_MUTUAL_PROOF_REVERSE_ROW_LIMIT", "48")
)
TARGET_MEMBER_MUTUAL_PROOF_FAMILY_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_MUTUAL_PROOF_FAMILY_MEMBER_LIMIT", "8")
)
TARGET_MEMBER_MUTUAL_PROOF_PEER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_MUTUAL_PROOF_PEER_LIMIT", "8")
)
TARGET_MEMBER_MUTUAL_PROOF_GAIN = float(os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_MUTUAL_PROOF_GAIN", "2.60"))
TARGET_MEMBER_MUTUAL_PROOF_MARGIN_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_MUTUAL_PROOF_MARGIN_GAIN", "1.85")
)
TARGET_MEMBER_MUTUAL_PROOF_NEGATIVE_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_MUTUAL_PROOF_NEGATIVE_FACTOR", "0.72")
)
TARGET_MEMBER_MUTUAL_PROOF_EMPTY_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_MUTUAL_PROOF_EMPTY_FACTOR", "0.94")
)
TARGET_MEMBER_MUTUAL_PROOF_MIN_MARGIN = float(
    os.environ.get("DARTLAB_HORIZON_V159_TARGET_MEMBER_MUTUAL_PROOF_MIN_MARGIN", "0.010")
)
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V159_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V159_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V159_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V159_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V159_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V159_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V159_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V159_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
LANE_MISMATCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V159_LANE_MISMATCH_PENALTY", "0.18"))
LANE_ARTIFACT_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V159_LANE_ARTIFACT_PENALTY", "0.10"))
NEAREST_ORDER_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_NEAREST_ORDER_SIGNATURE_LIMIT", "24"))
NEAREST_ORDER_PENALTY_MIN = float(os.environ.get("DARTLAB_HORIZON_V159_NEAREST_ORDER_PENALTY_MIN", "0.05"))
NEAREST_ORDER_PENALTY_SCALE = float(os.environ.get("DARTLAB_HORIZON_V159_NEAREST_ORDER_PENALTY_SCALE", "0.16"))
NEAREST_ORDER_COHORT_SURFACE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_NEAREST_ORDER_COHORT_SURFACE_LIMIT", "384")
)
NEAREST_ORDER_COHORT_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_NEAREST_ORDER_COHORT_POSITION_LIMIT", "8192")
)
NEAREST_ORDER_SURFACE_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V159_NEAREST_ORDER_SURFACE_POSITION_LIMIT", "48")
)
SUFFIX_ANCHOR_SUPPORT_MIN = int(os.environ.get("DARTLAB_HORIZON_V159_SUFFIX_ANCHOR_SUPPORT_MIN", "1"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V159_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V159_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V159_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V159_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V159_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V159_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V159_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V159_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V159_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V159_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V159_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V159_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V159_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V159_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V159_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V159_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V159_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V159_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V159_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V159_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V159_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V159_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V159_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
RELAY_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_RELAY_ROW_LIMIT", "160"))
RELAY_SPECIFIC_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_RELAY_SPECIFIC_ROW_LIMIT", "320"))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V159_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V159_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V159_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V159_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V159_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V159_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V159_SKETCH_OCC_BUCKETS", "16"))
