"""Horizon Meaning Learner V161 - residual target acceptance slot proof.

V161 실제 기록
--------------
아이디어:
    V160 은 candidate surface exact reverseRows=0 병목을 generalized reverse slot 으로 깼다.
    `외상매출금 -> 매출채권` 은 mutual=0/reverseRows=0 에서 mutual=0.310/reverseRows=48 로 올라왔다.
    하지만 slot 이 너무 넓어 같은 family/role peer 도 함께 켜졌다. `매출채권` 은
    `단기매출채권및기타채권` 에 proof margin 으로 졌고, `당기순이익 -> 영업이익` forbiddenRank=2 도
    유지됐다.

    수평선 좌표는 의미단위가 아니다. stem 을 빠르게 찾아가기 위한 고정 주소계다.
    의미는 그 주소가 겪은 주변 stem, 앞뒤 순서, 거리, role, relation, 문장/표 위치 경험이 반복적으로
    겹쳐 만들어지는 누적 경험 graph/field 다. V161 의 질문은 "slot 이 켜지는가" 가 아니라
    "그 slot 안에서 candidate target family 가 source path 를 residual 하게 받아들이는가" 다.

    V161 은 V160 generalized reverse slot 을 recall layer 로 유지하고, 그 위에 target-side residual
    acceptance ledger 를 추가한다.

    1. slot reverse index 의 각 slot row 를 target family/role 분포로 다시 본다.
    2. candidate family share, candidate role share, slot row saturation, same-role peer slot overlap 을
       corpus 통계로 계산한다.
    3. candidate 와 sibling peer 가 공통으로 갖는 broad slot atom 은 감산하고, candidate family 쪽으로
       실제 target rows 가 모이는 slot 만 acceptance proof 에 남긴다.
    4. exact surface reverseRows 는 그대로 살리되, generalized slot rows 는 residual acceptance weight 를
       통과해야 source path proof 에 쓰인다.
    5. 특정 단어, forbidden 패널티, 케이스별 gate 없이 broad slot proof 를 primary 의미 신호에서 분리한다.

    즉 V161 의 판단 단위는 수평선 좌표도 exact surface 도 broad slot 도 아니라,
    target 주소 주변 경험 field 안에서 peer 공통 경험을 제거하고 남는 residual acceptance 다.

실행 코드:
    $env:DARTLAB_HORIZON_V161_MAX_RECORDS_PER_SOURCE='120'
    $env:DARTLAB_HORIZON_V161_MAX_UNITS='1200'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV161Test.py
...
검증 기준:
    ...
    9. substitution key 에 target ledger-axis sketch 를 직접 넣지 않는다.
    10. target ledger-axis sketch 는 target literal 없이 occurrence lane/value/relation/bound/role/profile 경험으로만 만든다.
    11. source 가 없거나 얕으면 source 내부 contiguous fragment 를 prefix/suffix/inner 위치와 role 로 유지한다.
    12. source/fragment 의 relationBoundRolePair outgoing target 들을 평균 ledger field 로 뭉개지 않고
        path별 target-member prior 로 계산한다.
    13. V156 prior margin 은 진단으로만 남기고 primary multiplier 로 쓰지 않는다.
    14. V157 absolute transport boost 는 primary multiplier 로 쓰지 않는다.
    15. V158 conditional ratio 는 진단값으로 유지하되 primary 의미 신호는 target-side mutual proof 로 옮긴다.
    16. mutual proof 는 exact target reverseRows 가 0 이어도 family/role/profile/ledger slot reverseRows 를
        통해 source fragment path 수용성을 비교해야 한다.
    17. slot key 에 target literal 을 직접 넣지 않는다. surface literal 은 exact 진단용 key 에만 남긴다.
    18. 같은 anchor/family peer 가 더 강하게 source path 를 받아들이면 anti-cohort margin 으로 감점한다.
    19. route 는 surface score 를 누적 경험 family 로 먼저 collapse 한 뒤 mutual proof 대표를 고른다.
    20. 특정 surface 하드코딩, forbidden 단어 패널티, 케이스별 gate, 단순 가중치 미세조정 없이 일반 cold-start composition 구조만 사용한다.
    21. `외상매출금 -> 매출채권` 은 V159 의 reverseRows=0 병목을 깨야 한다.
    22. V160 generalized reverse slot proof 는 recall layer 로 유지한다.
    23. residual acceptance 는 slot row 의 candidate-family share, role share, row saturation,
        peer common slot mass 를 사용해야 한다.
    24. broad slot 이 켜졌다는 이유만으로 multiplier 를 주지 않는다.
    25. fixed route/search guard 는 positiveHits=3/4, badAccepted=0/7 수준을 유지해야 한다.

결과:
    부분 실행. 1,200 units 에서 build/model 단계는 total=214.5s 까지 완료됐다.
    relationSlotSubstitution 은 V160 과 같은 rawKeys=951, keptKeys=863, surfaceKeys=3,870,
    rawLinks=70,366, keptLinks=16,011, ownerSources=5,460, ownerTargets=11,326 이었다.

    targetMemberRelationFrame positive 4건:
        외상매출금 -> 매출채권: candidateHit=True, rank=NA, mutual=0.310/revRows48,
        acceptance=0.000/revRows0/slotAcc1/slotRej60/famShare32/roleShare1000.
        영업손익 -> 영업이익: candidateHit=False, rank=NA, mutual=0.284/revRows48,
        acceptance=0.140/revRows48/slotAcc11/slotRej24/famShare0/roleShare9601.
        현금성자산 -> 현금및현금성자산: candidateHit=True, rank=1, mutual=0.509/revRows48,
        acceptance=0.303/revRows48/slotAcc36/slotRej36.
        손실충당금 -> 대손충당금: candidateHit=True, rank=6, mutual=0.287/revRows48,
        acceptance=0.134/revRows48/slotAcc10/slotRej38/famShare0/roleShare10250.

    targetMemberRelationFrame negative 는 3건까지 출력됐다.
        대출채권 -/-> 매출채권: forbiddenRank=NA, acceptance=0.079.
        현금배당금 -/-> 현금및현금성자산: forbiddenRank=NA, acceptance=0.122.
        당기순이익 -/-> 영업이익: forbiddenRank=2, acceptance=0.098.

    4번째 negative 복구충당금 -/-> 대손충당금 계산 중 로그가 11:16 이후 증가하지 않았고,
    python 단일 프로세스가 CPU 2,040s, working set 약 3.3GB 까지 사용해 수동 중단했다.
    이후 slot quality row/sample cap, slot eval cap, peer mass cap 을 추가해 재실행 폭주를 막았다.

결론:
    실패/진단 성공. V160 의 generalized reverse slot recall 은 keep 이지만,
    candidate-family share 로 slot 을 residual accept 하는 방식은 cold-start 정답을 눌러버린다.
    외상매출금 -> 매출채권의 경우 V160 이 만든 mutual=0.310/revRows48 신호가
    acceptance=0.000/revRows0 으로 사라졌다. 즉 정답 family 가 아직 충분히 관측되지 않은 상황에서
    family share 를 acceptance 조건으로 삼으면 "없는 stem 도 경험으로 예측"한다는 목표와 충돌한다.

    폐기할 것은 family-share residual acceptance 를 primary multiplier/gate 로 쓰는 방식이다.
    남길 것은 slotAcc/slotRej/roleShare/famShare 같은 진단 계측이다.
    다음 메커니즘은 slot 을 reject 하지 말고, generalized slot 이 켠 receivable/allowance 같은 role field
    안에서 canonical target member 를 고르는 selector 여야 한다. 후보군 내부 중심성, source role transition,
    relation polarity, target family centrality 를 써서 `매출채권` 과 `단기매출채권및기타채권` 을 분리해야 한다.
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V161_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V161_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V161_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V161_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V161_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V161_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V161_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V161_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V161_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_SEARCH_CANDIDATE_LIMIT", "420"))
DYNAMIC_TARGET_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_DYNAMIC_TARGET_LIMIT", "80"))
DYNAMIC_COORD_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_DYNAMIC_COORD_ROW_LIMIT", "220"))
DYNAMIC_COMPOUND_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_DYNAMIC_COMPOUND_ROW_LIMIT", "260"))
DYNAMIC_QUERY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_DYNAMIC_QUERY_ATOM_LIMIT", "48"))
DYNAMIC_MEANING_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_DYNAMIC_MEANING_ATOM_LIMIT", "36"))
DYNAMIC_MEANING_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_DYNAMIC_MEANING_ROW_LIMIT", "220"))
DYNAMIC_RELATION_SURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_DYNAMIC_RELATION_SURFACE_LIMIT", "420"))
DYNAMIC_RELATION_UNIT_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_DYNAMIC_RELATION_UNIT_LIMIT", "160"))
DYNAMIC_BRIDGE_ONLY_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V161_DYNAMIC_BRIDGE_ONLY_PENALTY", "0.80"))
DYNAMIC_OWNER_ROLE_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_DYNAMIC_OWNER_ROLE_SIGNATURE_LIMIT", "64")
)
DYNAMIC_OWNER_ROLE_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_DYNAMIC_OWNER_ROLE_ATOM_LIMIT", "40"))
DYNAMIC_OWNER_ROLE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_DYNAMIC_OWNER_ROLE_ROW_LIMIT", "220"))
DYNAMIC_OWNER_ROLE_UNIT_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_DYNAMIC_OWNER_ROLE_UNIT_ATOM_LIMIT", "36")
)
DYNAMIC_OWNER_ROLE_MIN_BOUND = float(os.environ.get("DARTLAB_HORIZON_V161_DYNAMIC_OWNER_ROLE_MIN_BOUND", "0.55"))
DYNAMIC_OWNER_ROLE_CANDIDATE_BONUS = float(
    os.environ.get("DARTLAB_HORIZON_V161_DYNAMIC_OWNER_ROLE_CANDIDATE_BONUS", "2.40")
)
DYNAMIC_OWNER_ROLE_ROUTE_BONUS = float(os.environ.get("DARTLAB_HORIZON_V161_DYNAMIC_OWNER_ROLE_ROUTE_BONUS", "0.90"))
DYNAMIC_OWNER_ROLE_WEAK_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V161_DYNAMIC_OWNER_ROLE_WEAK_PENALTY", "0.16"))
DYNAMIC_OWNER_FRAME_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V161_DYNAMIC_OWNER_FRAME_RADIUS", "5"))
DYNAMIC_OWNER_FRAME_BETWEEN_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_DYNAMIC_OWNER_FRAME_BETWEEN_LIMIT", "7"))
MASKED_FRAME_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_MASKED_FRAME_SIGNATURE_LIMIT", "72"))
MASKED_FRAME_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_MASKED_FRAME_ATOM_LIMIT", "48"))
MASKED_FRAME_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_MASKED_FRAME_ROW_LIMIT", "240"))
MASKED_FRAME_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_MASKED_FRAME_EVAL_LIMIT", "800"))
MASKED_FRAME_HOLDOUT_MOD = int(os.environ.get("DARTLAB_HORIZON_V161_MASKED_FRAME_HOLDOUT_MOD", "5"))
MASKED_FRAME_PROBE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_MASKED_FRAME_PROBE_LIMIT", "12"))
MASKED_CONTEXT_SURFACE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_MASKED_CONTEXT_SURFACE_ROW_LIMIT", "12"))
MASKED_CONTEXT_WEIGHT = float(os.environ.get("DARTLAB_HORIZON_V161_MASKED_CONTEXT_WEIGHT", "0.35"))
MASKED_CONTEXT_RELATION_WINDOW = int(os.environ.get("DARTLAB_HORIZON_V161_MASKED_CONTEXT_RELATION_WINDOW", "140"))
MASKED_FRAME_SURPRISAL_POWER = float(os.environ.get("DARTLAB_HORIZON_V161_MASKED_FRAME_SURPRISAL_POWER", "2.0"))
MASKED_FRAME_SURFACE_PRIOR_POWER = float(
    os.environ.get("DARTLAB_HORIZON_V161_MASKED_FRAME_SURFACE_PRIOR_POWER", "0.45")
)
RELATION_OWNER_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_RELATION_OWNER_SIGNATURE_LIMIT", "64"))
RELATION_OWNER_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_RELATION_OWNER_ROW_LIMIT", "220"))
RELATION_OWNER_BRIDGE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_RELATION_OWNER_BRIDGE_LIMIT", "12"))
RELATION_OWNER_PROJECTION_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_RELATION_OWNER_PROJECTION_LIMIT", "24"))
RELATION_OWNER_PROJECTION_OWNER_ROLE_MIN = float(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_OWNER_PROJECTION_OWNER_ROLE_MIN", "0.18")
)
RELATION_OWNER_PROJECTION_QUERY_ROLE_MIN = float(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_OWNER_PROJECTION_QUERY_ROLE_MIN", "0.05")
)
RELATION_OWNER_GENERATED_BRIDGE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_OWNER_GENERATED_BRIDGE_LIMIT", "48")
)
RELATION_OWNER_GENERATED_BRIDGE_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_OWNER_GENERATED_BRIDGE_WEIGHT", "2.4")
)
RELATION_OWNER_ROLE_COMPAT_MIN = float(os.environ.get("DARTLAB_HORIZON_V161_RELATION_OWNER_ROLE_COMPAT_MIN", "0.16"))
RELATION_BOUND_ROLE_PAIR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_RELATION_BOUND_ROLE_PAIR_LIMIT", "24"))
RELATION_BOUND_ROLE_PAIR_BRIDGE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_BOUND_ROLE_PAIR_BRIDGE_LIMIT", "10")
)
RELATION_BOUND_ROLE_PAIR_BRIDGE_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_BOUND_ROLE_PAIR_BRIDGE_WEIGHT", "0.46")
)
RELATION_SOURCE_EXPERIENCE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_SOURCE_EXPERIENCE_ATOM_LIMIT", "44")
)
RELATION_SOURCE_EXPERIENCE_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_SOURCE_EXPERIENCE_ROW_LIMIT", "180")
)
RELATION_SOURCE_SHINGLE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_SOURCE_SHINGLE_ATOM_LIMIT", "28")
)
RELATION_SOURCE_SHINGLE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_RELATION_SOURCE_SHINGLE_ROW_LIMIT", "160"))
RELATION_SOURCE_SHINGLE_KEY_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_RELATION_SOURCE_SHINGLE_KEY_LIMIT", "56"))
RELATION_ANCHORED_COARSE_SHINGLE_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_ANCHORED_COARSE_SHINGLE_ROW_LIMIT", "180")
)
RELATION_ANCHORED_COARSE_SHINGLE_KEY_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_ANCHORED_COARSE_SHINGLE_KEY_LIMIT", "72")
)
HORIZON_MASK_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_HORIZON_MASK_SIGNATURE_LIMIT", "96"))
HORIZON_MASK_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_HORIZON_MASK_ATOM_LIMIT", "56"))
HORIZON_MASK_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_HORIZON_MASK_ROW_LIMIT", "220"))
HORIZON_MASK_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_HORIZON_MASK_EVAL_LIMIT", "900"))
HORIZON_MASK_HOLDOUT_MOD = int(os.environ.get("DARTLAB_HORIZON_V161_HORIZON_MASK_HOLDOUT_MOD", "7"))
HORIZON_MASK_SURFACE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_HORIZON_MASK_SURFACE_ROW_LIMIT", "24"))
HORIZON_MASK_NEIGHBOR_EXPERIENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_HORIZON_MASK_NEIGHBOR_EXPERIENCE_LIMIT", "5")
)
HORIZON_MASK_COORD_CELL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_HORIZON_MASK_COORD_CELL_LIMIT", "5"))
HORIZON_MASK_COORD_BUCKET_DIGITS = int(os.environ.get("DARTLAB_HORIZON_V161_HORIZON_MASK_COORD_BUCKET_DIGITS", "5"))
LOCAL_FRAME_PREDICTION_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_LOCAL_FRAME_PREDICTION_SIGNATURE_LIMIT", "132")
)
LOCAL_FRAME_PREDICTION_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_LOCAL_FRAME_PREDICTION_ATOM_LIMIT", "76"))
LOCAL_FRAME_PREDICTION_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_LOCAL_FRAME_PREDICTION_ROW_LIMIT", "260"))
LOCAL_FRAME_PREDICTION_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_LOCAL_FRAME_PREDICTION_EVAL_LIMIT", "900"))
LOCAL_FRAME_PREDICTION_SURFACE_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_LOCAL_FRAME_PREDICTION_SURFACE_ROW_LIMIT", "18")
)
LOCAL_FRAME_PREDICTION_HOLDOUT_MOD = int(os.environ.get("DARTLAB_HORIZON_V161_LOCAL_FRAME_PREDICTION_HOLDOUT_MOD", "9"))
LOCAL_FRAME_PREDICTION_NEIGHBOR_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_LOCAL_FRAME_PREDICTION_NEIGHBOR_LIMIT", "5")
)
LOCAL_FRAME_PREDICTION_NEIGHBOR_EXPERIENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_LOCAL_FRAME_PREDICTION_NEIGHBOR_EXPERIENCE_LIMIT", "5")
)
LOCAL_FRAME_PREDICTION_SEQUENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_LOCAL_FRAME_PREDICTION_SEQUENCE_LIMIT", "5")
)
QUERY_LOCAL_FRAME_OCCURRENCE_INDEX_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_QUERY_LOCAL_FRAME_OCCURRENCE_INDEX_ROW_LIMIT", "96")
)
QUERY_LOCAL_FRAME_OCCURRENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_QUERY_LOCAL_FRAME_OCCURRENCE_LIMIT", "72")
)
QUERY_LOCAL_FRAME_PROXY_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_QUERY_LOCAL_FRAME_PROXY_LIMIT", "8"))
QUERY_LOCAL_FRAME_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_QUERY_LOCAL_FRAME_ATOM_LIMIT", "132"))
QUERY_LOCAL_FRAME_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_QUERY_LOCAL_FRAME_EVAL_LIMIT", "260"))
QUERY_SEMANTIC_UNIT_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_QUERY_SEMANTIC_UNIT_ATOM_LIMIT", "120"))
QUERY_SEMANTIC_UNIT_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_QUERY_SEMANTIC_UNIT_EVAL_LIMIT", "260"))
QUERY_SEMANTIC_UNIT_SURFACE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_QUERY_SEMANTIC_UNIT_SURFACE_LIMIT", str(MASKED_FRAME_PROBE_LIMIT))
)
SEMANTIC_UNIT_SIGNATURE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_SEMANTIC_UNIT_SIGNATURE_ATOM_LIMIT", "18")
)
SEMANTIC_UNIT_POSTING_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_SEMANTIC_UNIT_POSTING_ROW_LIMIT", "90"))
SEMANTIC_UNIT_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_SEMANTIC_UNIT_CANDIDATE_LIMIT", "180"))
SEMANTIC_UNIT_MIN_EXPERIENCE_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V161_SEMANTIC_UNIT_MIN_EXPERIENCE_SIM", "0.46")
)
SEMANTIC_UNIT_MIN_MIXED_SIM = float(os.environ.get("DARTLAB_HORIZON_V161_SEMANTIC_UNIT_MIN_MIXED_SIM", "0.34"))
SEMANTIC_UNIT_MIN_COORD_RESONANCE = float(
    os.environ.get("DARTLAB_HORIZON_V161_SEMANTIC_UNIT_MIN_COORD_RESONANCE", "0.045")
)
SEMANTIC_UNIT_MASK_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_SEMANTIC_UNIT_MASK_SIGNATURE_LIMIT", "120")
)
SEMANTIC_UNIT_MASK_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_SEMANTIC_UNIT_MASK_ATOM_LIMIT", "64"))
SEMANTIC_UNIT_MASK_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_SEMANTIC_UNIT_MASK_ROW_LIMIT", "260"))
SEMANTIC_UNIT_ROUTE_MEMBER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_SEMANTIC_UNIT_ROUTE_MEMBER_LIMIT", "4"))
CONTRAST_SEMANTIC_UNIT_MIN_EXPERIENCE_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V161_CONTRAST_SEMANTIC_UNIT_MIN_EXPERIENCE_SIM", "0.46")
)
CONTRAST_SEMANTIC_UNIT_STRONG_EXPERIENCE_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V161_CONTRAST_SEMANTIC_UNIT_STRONG_EXPERIENCE_SIM", "0.62")
)
CONTRAST_SEMANTIC_UNIT_MIN_MIXED_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V161_CONTRAST_SEMANTIC_UNIT_MIN_MIXED_SIM", "0.36")
)
CONTRAST_SEMANTIC_UNIT_MIN_COORD_RESONANCE = float(
    os.environ.get("DARTLAB_HORIZON_V161_CONTRAST_SEMANTIC_UNIT_MIN_COORD_RESONANCE", "0.050")
)
CONTRAST_SEMANTIC_UNIT_MIN_RELATION_COMPAT = float(
    os.environ.get("DARTLAB_HORIZON_V161_CONTRAST_SEMANTIC_UNIT_MIN_RELATION_COMPAT", "0.28")
)
CONTRAST_SEMANTIC_UNIT_SUFFIX_RELATION_COMPAT = float(
    os.environ.get("DARTLAB_HORIZON_V161_CONTRAST_SEMANTIC_UNIT_SUFFIX_RELATION_COMPAT", "0.18")
)
MEMBER_BALANCE_MEMBER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_MEMBER_BALANCE_MEMBER_LIMIT", "10"))
MEMBER_BALANCE_MIN_MULTIPLIER = float(os.environ.get("DARTLAB_HORIZON_V161_MEMBER_BALANCE_MIN_MULTIPLIER", "0.015"))
MEMBER_BALANCE_UNSUPPORTED_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V161_MEMBER_BALANCE_UNSUPPORTED_FACTOR", "0.12")
)
MEMBER_BALANCE_POSITIVE_GAIN = float(os.environ.get("DARTLAB_HORIZON_V161_MEMBER_BALANCE_POSITIVE_GAIN", "1.10"))
MEMBER_BALANCE_NEGATIVE_GAIN = float(os.environ.get("DARTLAB_HORIZON_V161_MEMBER_BALANCE_NEGATIVE_GAIN", "1.60"))
MEMBER_BALANCE_POSITIVE_SHIELD_RATIO = float(
    os.environ.get("DARTLAB_HORIZON_V161_MEMBER_BALANCE_POSITIVE_SHIELD_RATIO", "0.85")
)
...
MEMBER_CONDITIONED_MASK_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_MEMBER_CONDITIONED_MASK_SIGNATURE_LIMIT", "96")
)
MEMBER_CONDITIONED_MASK_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_MEMBER_CONDITIONED_MASK_ATOM_LIMIT", "64")
)
MEMBER_CONDITIONED_MASK_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_MEMBER_CONDITIONED_MASK_ROW_LIMIT", "220"))
MEMBER_CONDITIONED_ROUTE_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_MEMBER_CONDITIONED_ROUTE_MEMBER_LIMIT", "180")
)
MEMBER_CONDITIONED_LABEL_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_MEMBER_CONDITIONED_LABEL_MEMBER_LIMIT", "6")
)
MEMBER_CONDITIONED_MEMBER_CACHE: dict[tuple[int, str, str], tuple[float, float, float, int]] = {}
MEMBER_SUPPORTED_LABEL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_MEMBER_SUPPORTED_LABEL_LIMIT", "80"))
MEMBER_SUPPORTED_MEMBER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_MEMBER_SUPPORTED_MEMBER_LIMIT", "120"))
MEMBER_SUPPORTED_ACTIVE_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_MEMBER_SUPPORTED_ACTIVE_MEMBER_LIMIT", "8")
)
MEMBER_SUPPORTED_MIN_MULTIPLIER = float(
    os.environ.get("DARTLAB_HORIZON_V161_MEMBER_SUPPORTED_MIN_MULTIPLIER", str(MEMBER_BALANCE_MIN_MULTIPLIER))
)
...
MEMBER_ROLE_RESIDUAL_MASK_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_MEMBER_ROLE_RESIDUAL_MASK_SIGNATURE_LIMIT", "96")
)
MEMBER_ROLE_RESIDUAL_MASK_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_MEMBER_ROLE_RESIDUAL_MASK_ATOM_LIMIT", "64")
)
MEMBER_ROLE_RESIDUAL_MASK_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_MEMBER_ROLE_RESIDUAL_MASK_ROW_LIMIT", "220")
)
MEMBER_ROLE_RESIDUAL_LABEL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_MEMBER_ROLE_RESIDUAL_LABEL_LIMIT", "80"))
MEMBER_ROLE_RESIDUAL_MIN_MULTIPLIER = float(
    os.environ.get("DARTLAB_HORIZON_V161_MEMBER_ROLE_RESIDUAL_MIN_MULTIPLIER", "0.25")
)
MEMBER_ROLE_RESIDUAL_CONFLICT_RATIO = float(
    os.environ.get("DARTLAB_HORIZON_V161_MEMBER_ROLE_RESIDUAL_CONFLICT_RATIO", "1.35")
)
MEMBER_TARGET_SELECTOR_LABEL_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_MEMBER_TARGET_SELECTOR_LABEL_LIMIT", "48")
)
MEMBER_TARGET_SELECTOR_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_MEMBER_TARGET_SELECTOR_MEMBER_LIMIT", "14")
)
MEMBER_TARGET_SELECTOR_BALANCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_MEMBER_TARGET_SELECTOR_BALANCE_LIMIT", "96")
)
MEMBER_TARGET_SELECTOR_SURFACE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_MEMBER_TARGET_SELECTOR_SURFACE_LIMIT", str(MASKED_FRAME_PROBE_LIMIT))
)
SELECTED_TARGET_PROJECTION_SOURCE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_SELECTED_TARGET_PROJECTION_SOURCE_ATOM_LIMIT", "72")
)
SELECTED_TARGET_PROJECTION_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_SELECTED_TARGET_PROJECTION_ROW_LIMIT", "220")
)
SELECTED_TARGET_PROJECTION_TARGET_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_SELECTED_TARGET_PROJECTION_TARGET_LIMIT", "8")
)
SELECTED_TARGET_PROJECTION_SELECTOR_LABEL_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_SELECTED_TARGET_PROJECTION_SELECTOR_LABEL_LIMIT", "12")
)
SELECTED_TARGET_PROJECTION_SELECTOR_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_SELECTED_TARGET_PROJECTION_SELECTOR_MEMBER_LIMIT", "8")
)
SELECTOR_PATH_JOIN_SOURCE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_SELECTOR_PATH_JOIN_SOURCE_LIMIT", "12"))
SELECTOR_PATH_JOIN_PAIR_TARGET_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_SELECTOR_PATH_JOIN_PAIR_TARGET_LIMIT", "16")
)
SELECTOR_PATH_JOIN_SELECTED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_SELECTOR_PATH_JOIN_SELECTED_LIMIT", "8"))
SELECTOR_PATH_JOIN_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_SELECTOR_PATH_JOIN_ROW_LIMIT", "36"))
BIDIRECTIONAL_PAIR_CANDIDATE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_BIDIRECTIONAL_PAIR_CANDIDATE_LIMIT", "96")
)
BIDIRECTIONAL_PAIR_ROUTE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_BIDIRECTIONAL_PAIR_ROUTE_LIMIT", "12"))
BIDIRECTIONAL_PAIR_DYNAMIC_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_BIDIRECTIONAL_PAIR_DYNAMIC_LIMIT", "80"))
BIDIRECTIONAL_PAIR_REVERSE_RELATION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_BIDIRECTIONAL_PAIR_REVERSE_RELATION_LIMIT", "2")
)
PAIR_LOCAL_CONTRAST_CANDIDATE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_PAIR_LOCAL_CONTRAST_CANDIDATE_LIMIT", "96")
)
PAIR_LOCAL_CONTRAST_PEER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_PAIR_LOCAL_CONTRAST_PEER_LIMIT", "18"))
PAIR_LOCAL_CONTRAST_NEAR_BONUS = float(os.environ.get("DARTLAB_HORIZON_V161_PAIR_LOCAL_CONTRAST_NEAR_BONUS", "0.42"))
PAIR_LOCAL_CONTRAST_MIN_MARGIN = float(os.environ.get("DARTLAB_HORIZON_V161_PAIR_LOCAL_CONTRAST_MIN_MARGIN", "0.015"))
TARGET_MEMBER_FRAME_OCCURRENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_FRAME_OCCURRENCE_LIMIT", "56")
)
TARGET_MEMBER_FRAME_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_FRAME_ATOM_LIMIT", "120"))
TARGET_MEMBER_FRAME_PEER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_FRAME_PEER_LIMIT", "8"))
TARGET_MEMBER_FRAME_ROUTE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_FRAME_ROUTE_LIMIT", str(MASKED_FRAME_PROBE_LIMIT))
)
TARGET_MEMBER_FRAME_NEIGHBOR_RADIUS = int(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_FRAME_NEIGHBOR_RADIUS", "3")
)
TARGET_MEMBER_FRAME_NEIGHBOR_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_FRAME_NEIGHBOR_ATOM_LIMIT", "14")
)
TARGET_MEMBER_FRAME_RELATION_WINDOW = int(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_FRAME_RELATION_WINDOW", "8")
)
TARGET_MEMBER_FRAME_CANDIDATE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_FRAME_CANDIDATE_LIMIT", str(BIDIRECTIONAL_PAIR_CANDIDATE_LIMIT))
)
TARGET_MEMBER_FRAME_QUERY_ROUTE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_FRAME_QUERY_ROUTE_LIMIT", "24")
)
COMPOSITIONAL_FRAME_MIN_QUERY_ROWS = int(
    os.environ.get("DARTLAB_HORIZON_V161_COMPOSITIONAL_FRAME_MIN_QUERY_ROWS", "24")
)
COMPOSITIONAL_FRAME_FRAGMENT_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_COMPOSITIONAL_FRAME_FRAGMENT_LIMIT", "10")
)
COMPOSITIONAL_FRAME_FRAGMENT_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_COMPOSITIONAL_FRAME_FRAGMENT_ROW_LIMIT", "18")
)
COMPOSITIONAL_FRAME_MIN_FRAGMENT_LEN = int(
    os.environ.get("DARTLAB_HORIZON_V161_COMPOSITIONAL_FRAME_MIN_FRAGMENT_LEN", "2")
)
COMPOSITIONAL_FRAME_MAX_FRAGMENT_LEN = int(
    os.environ.get("DARTLAB_HORIZON_V161_COMPOSITIONAL_FRAME_MAX_FRAGMENT_LEN", "6")
)
COMPOSITIONAL_FRAME_ATOM_SCALE = float(os.environ.get("DARTLAB_HORIZON_V161_COMPOSITIONAL_FRAME_ATOM_SCALE", "0.58"))
COMPOSITIONAL_FRAME_SIGNATURE_SCALE = float(
    os.environ.get("DARTLAB_HORIZON_V161_COMPOSITIONAL_FRAME_SIGNATURE_SCALE", "0.24")
)
RELATION_SLOT_SUBSTITUTION_WINDOW = int(os.environ.get("DARTLAB_HORIZON_V161_RELATION_SLOT_SUBSTITUTION_WINDOW", "8"))
RELATION_SLOT_SUBSTITUTION_MIN_KEY_DF = int(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_SLOT_SUBSTITUTION_MIN_KEY_DF", "2")
)
RELATION_SLOT_SUBSTITUTION_MAX_KEY_DF = int(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_SLOT_SUBSTITUTION_MAX_KEY_DF", "260")
)
RELATION_SLOT_SUBSTITUTION_POSTING_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_SLOT_SUBSTITUTION_POSTING_LIMIT", "80")
)
RELATION_SLOT_SUBSTITUTION_SURFACE_KEY_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_SLOT_SUBSTITUTION_SURFACE_KEY_LIMIT", "36")
)
RELATION_SLOT_SUBSTITUTION_QUERY_KEY_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_SLOT_SUBSTITUTION_QUERY_KEY_LIMIT", "64")
)
RELATION_SLOT_SUBSTITUTION_FRAGMENT_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_SLOT_SUBSTITUTION_FRAGMENT_LIMIT", "8")
)
RELATION_SLOT_SUBSTITUTION_ROUTE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_SLOT_SUBSTITUTION_ROUTE_LIMIT", str(MASKED_FRAME_PROBE_LIMIT))
)
RELATION_SLOT_SUBSTITUTION_SEED_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_SLOT_SUBSTITUTION_SEED_WEIGHT", "1.15")
)
RELATION_SLOT_SUBSTITUTION_TYPED_SPECIFICITY_MIN = float(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_SLOT_SUBSTITUTION_TYPED_SPECIFICITY_MIN", "0.16")
)
RELATION_SLOT_SUBSTITUTION_TYPED_HUBNESS_MAX = float(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_SLOT_SUBSTITUTION_TYPED_HUBNESS_MAX", "0.66")
)
RELATION_SLOT_SUBSTITUTION_TYPED_ENTROPY_MAX = float(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_SLOT_SUBSTITUTION_TYPED_ENTROPY_MAX", "0.86")
)
RELATION_SLOT_SUBSTITUTION_TYPED_CONCEPT_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_SLOT_SUBSTITUTION_TYPED_CONCEPT_WEIGHT", "1.18")
)
RELATION_SLOT_SUBSTITUTION_TYPED_GENERIC_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_SLOT_SUBSTITUTION_TYPED_GENERIC_WEIGHT", "0.42")
)
RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_TARGET_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_TARGET_LIMIT", "10")
)
RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_WEAK_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_WEAK_WEIGHT", "0.52")
)
RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_GENERIC_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_GENERIC_WEIGHT", "0.24")
)
RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_TARGET_KEY_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_TARGET_KEY_WEIGHT", "0.32")
)
RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_MIN_PAIR_SCORE = float(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_MIN_PAIR_SCORE", "0.035")
)
RELATION_SLOT_SUBSTITUTION_LEDGER_OCCURRENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_SLOT_SUBSTITUTION_LEDGER_OCCURRENCE_LIMIT", "48")
)
RELATION_SLOT_SUBSTITUTION_LEDGER_TARGET_KEY_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_SLOT_SUBSTITUTION_LEDGER_TARGET_KEY_WEIGHT", "0.38")
)
RELATION_SLOT_SUBSTITUTION_LEDGER_BROAD_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V161_RELATION_SLOT_SUBSTITUTION_LEDGER_BROAD_WEIGHT", "0.34")
)
PAIR_LOCAL_LEDGER_FRAGMENT_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_PAIR_LOCAL_LEDGER_FRAGMENT_LIMIT", "10"))
PAIR_LOCAL_LEDGER_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_PAIR_LOCAL_LEDGER_ATOM_LIMIT", "48"))
PAIR_LOCAL_LEDGER_BAND_SIZE = int(os.environ.get("DARTLAB_HORIZON_V161_PAIR_LOCAL_LEDGER_BAND_SIZE", "3"))
PAIR_LOCAL_LEDGER_BAND_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_PAIR_LOCAL_LEDGER_BAND_LIMIT", "12"))
PAIR_LOCAL_LEDGER_GAIN = float(os.environ.get("DARTLAB_HORIZON_V161_PAIR_LOCAL_LEDGER_GAIN", "1.45"))
PAIR_LOCAL_LEDGER_MARGIN_GAIN = float(os.environ.get("DARTLAB_HORIZON_V161_PAIR_LOCAL_LEDGER_MARGIN_GAIN", "2.20"))
PAIR_LOCAL_LEDGER_NEGATIVE_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V161_PAIR_LOCAL_LEDGER_NEGATIVE_FACTOR", "0.74")
)
PAIR_LOCAL_LEDGER_EMPTY_FACTOR = float(os.environ.get("DARTLAB_HORIZON_V161_PAIR_LOCAL_LEDGER_EMPTY_FACTOR", "0.88"))
PAIR_LOCAL_LEDGER_MIN_MARGIN = float(os.environ.get("DARTLAB_HORIZON_V161_PAIR_LOCAL_LEDGER_MIN_MARGIN", "0.018"))
PAIR_LOCAL_LEDGER_RESIDUAL_COMMON_POWER = float(
    os.environ.get("DARTLAB_HORIZON_V161_PAIR_LOCAL_LEDGER_RESIDUAL_COMMON_POWER", "0.72")
)
PAIR_LOCAL_LEDGER_RESIDUAL_IDF_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V161_PAIR_LOCAL_LEDGER_RESIDUAL_IDF_GAIN", "0.58")
)
PAIR_LOCAL_LEDGER_RESIDUAL_MIN_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V161_PAIR_LOCAL_LEDGER_RESIDUAL_MIN_WEIGHT", "0.018")
)
PAIR_LOCAL_LEDGER_RESIDUAL_PEER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_PAIR_LOCAL_LEDGER_RESIDUAL_PEER_LIMIT", "8")
)
DIRECTIONAL_LEDGER_FRAGMENT_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_DIRECTIONAL_LEDGER_FRAGMENT_LIMIT", "8"))
DIRECTIONAL_LEDGER_SOURCE_TARGET_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_DIRECTIONAL_LEDGER_SOURCE_TARGET_LIMIT", "10")
)
DIRECTIONAL_LEDGER_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_DIRECTIONAL_LEDGER_ATOM_LIMIT", "64"))
DIRECTIONAL_LEDGER_GAIN = float(os.environ.get("DARTLAB_HORIZON_V161_DIRECTIONAL_LEDGER_GAIN", "1.72"))
DIRECTIONAL_LEDGER_MARGIN_GAIN = float(os.environ.get("DARTLAB_HORIZON_V161_DIRECTIONAL_LEDGER_MARGIN_GAIN", "2.45"))
DIRECTIONAL_LEDGER_NEGATIVE_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V161_DIRECTIONAL_LEDGER_NEGATIVE_FACTOR", "0.72")
)
DIRECTIONAL_LEDGER_EMPTY_FACTOR = float(os.environ.get("DARTLAB_HORIZON_V161_DIRECTIONAL_LEDGER_EMPTY_FACTOR", "0.90"))
DIRECTIONAL_LEDGER_MIN_MARGIN = float(os.environ.get("DARTLAB_HORIZON_V161_DIRECTIONAL_LEDGER_MIN_MARGIN", "0.014"))
TARGET_MEMBER_PRIOR_SOURCE_TARGET_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_PRIOR_SOURCE_TARGET_LIMIT", "16")
)
TARGET_MEMBER_PRIOR_HARD_NEGATIVE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_PRIOR_HARD_NEGATIVE_LIMIT", "14")
)
TARGET_MEMBER_PRIOR_GAIN = float(os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_PRIOR_GAIN", "2.35"))
TARGET_MEMBER_PRIOR_MARGIN_GAIN = float(os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_PRIOR_MARGIN_GAIN", "2.10"))
TARGET_MEMBER_PRIOR_NEGATIVE_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_PRIOR_NEGATIVE_FACTOR", "0.68")
)
TARGET_MEMBER_PRIOR_EMPTY_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_PRIOR_EMPTY_FACTOR", "0.92")
)
TARGET_MEMBER_PRIOR_MIN_MARGIN = float(os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_PRIOR_MIN_MARGIN", "0.012"))
TARGET_MEMBER_TRANSPORT_GAIN = float(os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_TRANSPORT_GAIN", "3.20"))
TARGET_MEMBER_TRANSPORT_ANCHOR_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_TRANSPORT_ANCHOR_GAIN", "1.35")
)
TARGET_MEMBER_TRANSPORT_ROLE_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_TRANSPORT_ROLE_GAIN", "0.86")
)
TARGET_MEMBER_TRANSPORT_EMPTY_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_TRANSPORT_EMPTY_FACTOR", "0.94")
)
TARGET_MEMBER_TRANSPORT_LOW_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_TRANSPORT_LOW_FACTOR", "0.78")
)
TARGET_MEMBER_TRANSPORT_MIN_LIKELIHOOD = float(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_TRANSPORT_MIN_LIKELIHOOD", "0.045")
)
TARGET_MEMBER_CONDITIONAL_TRANSPORT_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_CONDITIONAL_TRANSPORT_GAIN", "2.35")
)
TARGET_MEMBER_CONDITIONAL_TRANSPORT_RESIDUAL_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_CONDITIONAL_TRANSPORT_RESIDUAL_GAIN", "1.15")
)
TARGET_MEMBER_CONDITIONAL_ANCHOR_RESIDUAL_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_CONDITIONAL_ANCHOR_RESIDUAL_GAIN", "0.72")
)
TARGET_MEMBER_CONDITIONAL_TRANSPORT_EMPTY_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_CONDITIONAL_TRANSPORT_EMPTY_FACTOR", "0.92")
)
TARGET_MEMBER_CONDITIONAL_TRANSPORT_LOW_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_CONDITIONAL_TRANSPORT_LOW_FACTOR", "0.82")
)
TARGET_MEMBER_CONDITIONAL_TRANSPORT_PEER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_CONDITIONAL_TRANSPORT_PEER_LIMIT", "10")
)
TARGET_MEMBER_EXPERIENCE_FAMILY_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_EXPERIENCE_FAMILY_MEMBER_LIMIT", "8")
)
TARGET_MEMBER_EXPERIENCE_FAMILY_SCORE_FLOOR = float(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_EXPERIENCE_FAMILY_SCORE_FLOOR", "0.045")
)
TARGET_MEMBER_MUTUAL_PROOF_SOURCE_PATH_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_MUTUAL_PROOF_SOURCE_PATH_LIMIT", "18")
)
TARGET_MEMBER_MUTUAL_PROOF_REVERSE_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_MUTUAL_PROOF_REVERSE_ROW_LIMIT", "48")
)
TARGET_MEMBER_MUTUAL_PROOF_FAMILY_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_MUTUAL_PROOF_FAMILY_MEMBER_LIMIT", "8")
)
TARGET_MEMBER_MUTUAL_PROOF_PEER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_MUTUAL_PROOF_PEER_LIMIT", "8")
)
TARGET_MEMBER_MUTUAL_PROOF_SLOT_KEY_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_MUTUAL_PROOF_SLOT_KEY_LIMIT", "56")
)
TARGET_MEMBER_MUTUAL_PROOF_SLOT_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_MUTUAL_PROOF_SLOT_ROW_LIMIT", "96")
)
TARGET_MEMBER_MUTUAL_PROOF_SLOT_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_MUTUAL_PROOF_SLOT_MEMBER_LIMIT", "6")
)
TARGET_MEMBER_MUTUAL_PROOF_SLOT_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_MUTUAL_PROOF_SLOT_WEIGHT", "0.74")
)
TARGET_MEMBER_MUTUAL_PROOF_GAIN = float(os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_MUTUAL_PROOF_GAIN", "2.60"))
TARGET_MEMBER_MUTUAL_PROOF_MARGIN_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_MUTUAL_PROOF_MARGIN_GAIN", "1.85")
)
TARGET_MEMBER_MUTUAL_PROOF_NEGATIVE_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_MUTUAL_PROOF_NEGATIVE_FACTOR", "0.72")
)
TARGET_MEMBER_MUTUAL_PROOF_EMPTY_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_MUTUAL_PROOF_EMPTY_FACTOR", "0.94")
)
TARGET_MEMBER_MUTUAL_PROOF_MIN_MARGIN = float(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_MUTUAL_PROOF_MIN_MARGIN", "0.010")
)
TARGET_MEMBER_RESIDUAL_ACCEPTANCE_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_RESIDUAL_ACCEPTANCE_GAIN", "2.10")
)
TARGET_MEMBER_RESIDUAL_ACCEPTANCE_MARGIN_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_RESIDUAL_ACCEPTANCE_MARGIN_GAIN", "1.70")
)
TARGET_MEMBER_RESIDUAL_ACCEPTANCE_NEGATIVE_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_RESIDUAL_ACCEPTANCE_NEGATIVE_FACTOR", "0.74")
)
TARGET_MEMBER_RESIDUAL_ACCEPTANCE_EMPTY_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_RESIDUAL_ACCEPTANCE_EMPTY_FACTOR", "1.00")
)
TARGET_MEMBER_RESIDUAL_ACCEPTANCE_MIN_MARGIN = float(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_RESIDUAL_ACCEPTANCE_MIN_MARGIN", "0.006")
)
TARGET_MEMBER_RESIDUAL_ACCEPTANCE_MIN_SLOT_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_RESIDUAL_ACCEPTANCE_MIN_SLOT_WEIGHT", "0.030")
)
TARGET_MEMBER_RESIDUAL_ACCEPTANCE_PEER_COMMON_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_RESIDUAL_ACCEPTANCE_PEER_COMMON_FACTOR", "0.78")
)
TARGET_MEMBER_RESIDUAL_ACCEPTANCE_ROW_SCALE = float(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_RESIDUAL_ACCEPTANCE_ROW_SCALE", "30.0")
)
TARGET_MEMBER_RESIDUAL_ACCEPTANCE_SLOT_EVAL_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_RESIDUAL_ACCEPTANCE_SLOT_EVAL_LIMIT", "36")
)
TARGET_MEMBER_RESIDUAL_ACCEPTANCE_SLOT_QUALITY_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_RESIDUAL_ACCEPTANCE_SLOT_QUALITY_ROW_LIMIT", "48")
)
TARGET_MEMBER_RESIDUAL_ACCEPTANCE_PEER_MASS_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_TARGET_MEMBER_RESIDUAL_ACCEPTANCE_PEER_MASS_LIMIT", "4")
)
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V161_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V161_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V161_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V161_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V161_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V161_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V161_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V161_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
LANE_MISMATCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V161_LANE_MISMATCH_PENALTY", "0.18"))
LANE_ARTIFACT_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V161_LANE_ARTIFACT_PENALTY", "0.10"))
NEAREST_ORDER_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_NEAREST_ORDER_SIGNATURE_LIMIT", "24"))
NEAREST_ORDER_PENALTY_MIN = float(os.environ.get("DARTLAB_HORIZON_V161_NEAREST_ORDER_PENALTY_MIN", "0.05"))
NEAREST_ORDER_PENALTY_SCALE = float(os.environ.get("DARTLAB_HORIZON_V161_NEAREST_ORDER_PENALTY_SCALE", "0.16"))
NEAREST_ORDER_COHORT_SURFACE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_NEAREST_ORDER_COHORT_SURFACE_LIMIT", "384")
)
NEAREST_ORDER_COHORT_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_NEAREST_ORDER_COHORT_POSITION_LIMIT", "8192")
)
NEAREST_ORDER_SURFACE_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V161_NEAREST_ORDER_SURFACE_POSITION_LIMIT", "48")
)
SUFFIX_ANCHOR_SUPPORT_MIN = int(os.environ.get("DARTLAB_HORIZON_V161_SUFFIX_ANCHOR_SUPPORT_MIN", "1"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V161_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V161_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V161_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V161_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V161_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V161_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V161_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V161_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V161_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V161_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V161_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V161_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V161_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V161_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V161_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V161_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V161_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V161_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V161_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V161_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V161_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V161_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V161_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
RELAY_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_RELAY_ROW_LIMIT", "160"))
RELAY_SPECIFIC_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_RELAY_SPECIFIC_ROW_LIMIT", "320"))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V161_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V161_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V161_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V161_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V161_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V161_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V161_SKETCH_OCC_BUCKETS", "16"))
...
TARGET_MEMBER_RESIDUAL_ACCEPTANCE_SLOT_WEIGHT_CACHE: dict[
    tuple[int, str, str, tuple[str, ...]], tuple[dict[str, float], Counter[str]]
] = {}
TARGET_MEMBER_RESIDUAL_ACCEPTANCE_CACHE: dict[tuple[int, str, str, tuple[str, ...]], tuple[float, Counter[str]]] = {}
TARGET_MEMBER_RESIDUAL_ACCEPTANCE_MARGIN_CACHE: dict[
    tuple[int, str, str, tuple[str, ...]], tuple[float, str, float, int]
] = {}
...
def targetMemberResidualAcceptanceFamily(surface: str, model: Model) -> str:
    surface = normStem(surface)
    label = contrastSemanticUnitLabelForSurface(surface, model)
    if label:
        return label
...
def targetMemberResidualAcceptanceSlotMass(
    surface: str,
    relations: tuple[str, ...],
    model: Model,
) -> Counter[str]:
    mass: Counter[str] = Counter()
    for targetSurface, targetWeight in targetMemberMutualProofTargetSurfaces(surface, model):
        slotRelationKeys = tuple(dict.fromkeys((*(relations or ("",)), "")))
        for slotRelation in slotRelationKeys:
            for slotKey, slotWeight in targetMemberMutualProofSlotKeys(targetSurface, slotRelation, model):
                mass[slotKey] += max(0.0, float(targetWeight)) * max(0.0, float(slotWeight))
    return mass


def targetMemberResidualAcceptanceSlotQuality(
    slotKey: str,
    candidate: str,
    model: Model,
) -> tuple[float, Counter[str]]:
    ...
    stats["acceptanceSlotRows"] = len(rows)
    if not rows:
        return 0.0, stats

    candidate = normStem(candidate)
    candidateFamily = targetMemberResidualAcceptanceFamily(candidate, model)
    candidateRole = relationSlotSubstitutionTargetRole(candidate, model)
    familyMass: Counter[str] = Counter()
    roleMass: Counter[str] = Counter()
    total = 0.0
    candidateFamilyMass = 0.0
    candidateRoleMass = 0.0
    sampledRows = rows[:TARGET_MEMBER_RESIDUAL_ACCEPTANCE_SLOT_QUALITY_ROW_LIMIT]
    stats["acceptanceSlotSampleRows"] = len(sampledRows)
    for _sourceSurface, _relation, reverseTarget, score, _sourceRole, targetRole in sampledRows:
        rowMass = max(0.0, float(score))
        if rowMass <= 0.0:
            continue
        total += rowMass
        targetFamily = targetMemberResidualAcceptanceFamily(reverseTarget, model)
        familyMass[targetFamily] += rowMass
        roleMass[targetRole] += rowMass
        if targetFamily == candidateFamily or maskedFrameFamilyMatch(candidate, reverseTarget, model):
            candidateFamilyMass += rowMass
        if candidateRole and targetRole == candidateRole:
            candidateRoleMass += rowMass
    if total <= 0.0:
        return 0.0, stats

    familyShare = candidateFamilyMass / total
    roleShare = candidateRoleMass / total if candidateRole else 0.0
    maxFamilyShare = max(familyMass.values(), default=0.0) / total
    maxRoleShare = max(roleMass.values(), default=0.0) / total
    rowSaturation = 1.0 / math.sqrt(1.0 + len(rows) / max(1.0, TARGET_MEMBER_RESIDUAL_ACCEPTANCE_ROW_SCALE))
    coldStartFamilyFloor = max(0.22, maxFamilyShare * 0.48)
    quality = (
        (0.42 + 0.58 * max(familyShare, coldStartFamilyFloor))
        * (0.30 + 0.70 * roleShare)
        * (0.32 + 0.68 * maxFamilyShare)
        * (0.40 + 0.60 * maxRoleShare)
        * rowSaturation
    )
    stats["acceptanceSlotFamilyShareMilli"] = int(familyShare * 1000)
    stats["acceptanceSlotRoleShareMilli"] = int(roleShare * 1000)
    stats["acceptanceSlotMaxFamilyShareMilli"] = int(maxFamilyShare * 1000)
    stats["acceptanceSlotMaxRoleShareMilli"] = int(maxRoleShare * 1000)
    stats["acceptanceSlotSaturationMilli"] = int(rowSaturation * 1000)
    stats["acceptanceSlotQualityMilli"] = int(quality * 1000)
    return max(0.0, min(1.0, quality)), stats


def targetMemberResidualAcceptanceSlotWeights(
    source: str,
    candidate: str,
    relations: tuple[str, ...],
    model: Model,
    ...
    candidate = normStem(candidate)
    cacheKey = (id(model), source, candidate, tuple(relations))
    cached = TARGET_MEMBER_RESIDUAL_ACCEPTANCE_SLOT_WEIGHT_CACHE.get(cacheKey)
    if cached is not None:
        weights, stats = cached
        return dict(weights), Counter(stats)

    stats: Counter[str] = Counter()
    candidateMass = targetMemberResidualAcceptanceSlotMass(candidate, relations, model)
    peerMass: Counter[str] = Counter()
    for peerRank, peer in enumerate(targetMemberMutualProofPeers(source, candidate, relations, model), start=1):
        if peerRank > TARGET_MEMBER_RESIDUAL_ACCEPTANCE_PEER_MASS_LIMIT:
            break
        peerScale = 1.0 / math.sqrt(float(peerRank))
        for slotKey, slotMass in targetMemberResidualAcceptanceSlotMass(peer, relations, model).items():
            peerMass[slotKey] += max(0.0, float(slotMass)) * peerScale

    weights: dict[str, float] = {}
    for slotKey, slotMass in candidateMass.most_common(TARGET_MEMBER_RESIDUAL_ACCEPTANCE_SLOT_EVAL_LIMIT):
        slotMass = max(0.0, float(slotMass))
        if slotMass <= 0.0:
            continue
        commonMass = max(0.0, float(peerMass.get(slotKey, 0.0)))
        residualMass = max(0.0, slotMass - commonMass * TARGET_MEMBER_RESIDUAL_ACCEPTANCE_PEER_COMMON_FACTOR)
        residualRatio = (residualMass + slotMass * 0.10) / max(1e-9, slotMass + commonMass + 0.18)
        slotQuality, qualityStats = targetMemberResidualAcceptanceSlotQuality(slotKey, candidate, model)
        stats["acceptanceSlotCandidateMilli"] += int(slotMass * 1000)
        stats["acceptanceSlotPeerMilli"] += int(commonMass * 1000)
        stats["acceptanceSlotResidualMilli"] += int(residualMass * 1000)
        stats["acceptanceSlotResidualRatioMilli"] += int(residualRatio * 1000)
        stats["acceptanceSlotQualityMilli"] += qualityStats.get("acceptanceSlotQualityMilli", 0)
        stats["acceptanceSlotFamilyShareMilli"] += qualityStats.get("acceptanceSlotFamilyShareMilli", 0)
        stats["acceptanceSlotRoleShareMilli"] += qualityStats.get("acceptanceSlotRoleShareMilli", 0)
        stats["acceptanceSlotSaturationMilli"] += qualityStats.get("acceptanceSlotSaturationMilli", 0)
        stats["acceptanceSlotRows"] += qualityStats.get("acceptanceSlotRows", 0)
        stats["acceptanceSlotEvaluated"] += 1
        weight = slotQuality * (0.36 + 0.64 * min(1.0, residualRatio))
        if weight < TARGET_MEMBER_RESIDUAL_ACCEPTANCE_MIN_SLOT_WEIGHT:
            stats["acceptanceSlotRejected"] += 1
            continue
        weights[slotKey] = weight
        stats["acceptanceSlotAccepted"] += 1
        stats["acceptanceSlotWeightMilli"] += int(weight * 1000)

    TARGET_MEMBER_RESIDUAL_ACCEPTANCE_SLOT_WEIGHT_CACHE[cacheKey] = (dict(weights), Counter(stats))
    return weights, stats


def targetMemberResidualAcceptanceReverseRows(
    source: str,
    candidate: str,
    relations: tuple[str, ...],
    model: Model,
) -> tuple[tuple[tuple[str, str, str, float, str, str], ...], Counter[str]]:
    relationSet = {relation for relation in relations if relation}
    exactRows: list[tuple[str, str, str, float, str, str]] = []
    fallbackRows: list[tuple[str, str, str, float, str, str]] = []
    reverseIndex = targetMemberMutualProofReverseIndex(model)
    reverseSlotIndex = targetMemberMutualProofReverseSlotIndex(model)
    slotWeights, stats = targetMemberResidualAcceptanceSlotWeights(source, candidate, relations, model)
    seen: set[tuple[str, str, str]] = set()
    for targetSurface, targetWeight in targetMemberMutualProofTargetSurfaces(candidate, model):
        for sourceSurface, relation, reverseTarget, score, sourceRole, targetRole in reverseIndex.get(
            targetSurface, tuple()
            ...
            if not relationSet or relation in relationSet:
                ...
                fallbackRows.append(weighted)
            stats["acceptanceExactRows"] += 1

        slotRelationKeys = tuple(dict.fromkeys((*(relations or ("",)), "")))
        for slotRelation in slotRelationKeys:
            for slotKey, slotWeight in targetMemberMutualProofSlotKeys(targetSurface, slotRelation, model):
                acceptanceWeight = slotWeights.get(slotKey, 0.0)
                if acceptanceWeight <= 0.0:
                    stats["acceptanceSkippedSlotRows"] += 1
                    continue
                for sourceSurface, relation, reverseTarget, score, sourceRole, targetRole in reverseSlotIndex.get(
                    slotKey, tuple()
                ):
                    ...
                    weighted = (
                        ...
                        relation,
                        reverseTarget,
                        float(score) * float(targetWeight) * float(slotWeight) * float(acceptanceWeight),
                        sourceRole,
                        targetRole,
                    )
                    if not relationSet or relation in relationSet:
                        ...
                        fallbackRows.append(weighted)
                    stats["acceptanceResidualSlotRows"] += 1
    exactRows.sort(key=lambda row: row[3], reverse=True)
    fallbackRows.sort(key=lambda row: row[3], reverse=True)
    selected = exactRows[:TARGET_MEMBER_MUTUAL_PROOF_REVERSE_ROW_LIMIT]
    if len(selected) < TARGET_MEMBER_MUTUAL_PROOF_REVERSE_ROW_LIMIT:
        selected.extend(fallbackRows[: TARGET_MEMBER_MUTUAL_PROOF_REVERSE_ROW_LIMIT - len(selected)])
    stats["acceptanceReverseRows"] = len(selected)
    return tuple(selected), stats


def targetMemberResidualAcceptanceProof(
    source: str,
    candidate: str,
    relations: tuple[str, ...],
    model: Model,
    ...
    candidate = normStem(candidate)
    cacheKey = (id(model), source, candidate, tuple(relations))
    cached = TARGET_MEMBER_RESIDUAL_ACCEPTANCE_CACHE.get(cacheKey)
    if cached is not None:
        score, stats = cached
        return score, Counter(stats)
    ...
    stats["acceptanceSourcePaths"] = len(sourcePaths)
    slotWeights, slotStats = targetMemberResidualAcceptanceSlotWeights(source, candidate, relations, model)
    ...
    exactRows = 0
    residualSlotRows = 0
    seenRows: set[tuple[str, str, str]] = set()
    for targetSurface, _targetWeight in targetMemberMutualProofTargetSurfaces(candidate, model):
        for sourceSurface, relation, reverseTarget, _score, _sourceRole, _targetRole in reverseIndex.get(
            targetSurface, tuple()
        ):
            if relationSet and relation not in relationSet:
                continue
            rowKey = (sourceSurface, relation, reverseTarget)
            if rowKey in seenRows:
                continue
            seenRows.add(rowKey)
            exactRows += 1
        slotRelationKeys = tuple(dict.fromkeys((*(relations or ("",)), "")))
        for slotRelation in slotRelationKeys:
            for slotKey, _slotWeight in targetMemberMutualProofSlotKeys(targetSurface, slotRelation, model):
                if slotKey not in slotWeights:
                    continue
                for sourceSurface, relation, reverseTarget, _score, _sourceRole, _targetRole in reverseSlotIndex.get(
                    slotKey, tuple()
                ):
                    if relationSet and relation not in relationSet:
                        continue
                    ...
                    seenRows.add(rowKey)
                    residualSlotRows += 1
                    if residualSlotRows >= TARGET_MEMBER_MUTUAL_PROOF_REVERSE_ROW_LIMIT:
                        break
                if residualSlotRows >= TARGET_MEMBER_MUTUAL_PROOF_REVERSE_ROW_LIMIT:
                    break
            if residualSlotRows >= TARGET_MEMBER_MUTUAL_PROOF_REVERSE_ROW_LIMIT:
    ...
    reverseRowCount = min(TARGET_MEMBER_MUTUAL_PROOF_REVERSE_ROW_LIMIT, exactRows + residualSlotRows)
    stats["acceptanceExactRows"] = exactRows
    stats["acceptanceResidualSlotRows"] = residualSlotRows
    stats["acceptanceReverseRows"] = reverseRowCount
    if not sourcePaths or reverseRowCount <= 0:
        stats["acceptanceEmptyRows"] += 1
        TARGET_MEMBER_RESIDUAL_ACCEPTANCE_CACHE[cacheKey] = (0.0, Counter(stats))
        return 0.0, stats

    mutualProof, mutualStats = targetMemberMutualTransportProof(source, candidate, relations, model)
    acceptedSlotCount = max(0, stats.get("acceptanceSlotAccepted", 0))
    evaluatedSlotCount = max(1, stats.get("acceptanceSlotEvaluated", 0))
    slotCoverage = acceptedSlotCount / evaluatedSlotCount
    slotWeightMean = stats.get("acceptanceSlotWeightMilli", 0) / max(1.0, acceptedSlotCount * 1000.0)
    familyShareMean = stats.get("acceptanceSlotFamilyShareMilli", 0) / max(1.0, evaluatedSlotCount * 1000.0)
    roleShareMean = stats.get("acceptanceSlotRoleShareMilli", 0) / max(1.0, evaluatedSlotCount * 1000.0)
    rowCoverage = reverseRowCount / max(1.0, TARGET_MEMBER_MUTUAL_PROOF_REVERSE_ROW_LIMIT)
    exactCoverage = min(1.0, exactRows / max(1.0, TARGET_MEMBER_MUTUAL_PROOF_REVERSE_ROW_LIMIT))
    acceptanceGate = (
        min(1.0, slotWeightMean * 1.65) * 0.44
        + math.sqrt(max(0.0, slotCoverage)) * 0.24
        + min(1.0, roleShareMean) * 0.16
        + min(1.0, familyShareMean + exactCoverage) * 0.10
        + rowCoverage * 0.06
    )
    proof = max(0.0, min(1.85, mutualProof * (0.30 + 0.70 * acceptanceGate)))
    rawProof = mutualStats.get("mutualRawMilli", 0) / 1000.0 * acceptanceGate
    opportunity = mutualStats.get("mutualOpportunityMilli", 0) / 1000.0
    stats["acceptanceProofMilli"] = int(proof * 1000)
    stats["acceptanceRawMilli"] = int(rawProof * 1000)
    stats["acceptanceOpportunityMilli"] = int(opportunity * 1000)
    stats["acceptanceAnchorMilli"] = int(mutualStats.get("mutualAnchorMilli", 0) * acceptanceGate)
    stats["acceptanceLedgerMilli"] = int(mutualStats.get("mutualLedgerMilli", 0) * acceptanceGate)
    stats["acceptanceRoleMilli"] = int(mutualStats.get("mutualRoleMilli", 0) * acceptanceGate)
    stats["acceptanceRelationMilli"] = int(mutualStats.get("mutualRelationMilli", 0) * acceptanceGate)
    stats["acceptanceFamilyMilli"] = int(mutualStats.get("mutualFamilyMilli", 0) * acceptanceGate)
    stats["acceptancePairRows"] = len(sourcePaths) * reverseRowCount
    stats["acceptanceRelationRows"] = int(mutualStats.get("mutualRelationRows", 0) * acceptanceGate)
    stats["acceptanceAnchorRows"] = int(mutualStats.get("mutualAnchorRows", 0) * acceptanceGate)
    stats["acceptanceLedgerRows"] = int(mutualStats.get("mutualLedgerRows", 0) * acceptanceGate)
    stats["acceptanceWeakAnchorRows"] = int(mutualStats.get("mutualWeakAnchorRows", 0) * acceptanceGate)
    stats["acceptanceGateMilli"] = int(acceptanceGate * 1000)
    stats["acceptanceAcceptedRows"] = int(proof > 0.0)
    TARGET_MEMBER_RESIDUAL_ACCEPTANCE_CACHE[cacheKey] = (proof, Counter(stats))
    return proof, stats


def targetMemberResidualAcceptanceMargin(
    source: str,
    candidate: str,
    relations: tuple[str, ...],
    ...
    candidate = normStem(candidate)
    cacheKey = (id(model), source, candidate, tuple(relations))
    cached = TARGET_MEMBER_RESIDUAL_ACCEPTANCE_MARGIN_CACHE.get(cacheKey)
    if cached is not None:
        return cached
    ...
    for peer in targetMemberMutualProofPeers(source, candidate, relations, model):
        peerProof, _ = targetMemberResidualAcceptanceProof(source, peer, relations, model)
        inspected += 1
        if peerProof > bestPeerProof:
            bestPeerProof = peerProof
            bestPeer = peer
    result = (candidateProof - bestPeerProof, bestPeer, bestPeerProof, inspected)
    TARGET_MEMBER_RESIDUAL_ACCEPTANCE_MARGIN_CACHE[cacheKey] = result
    return result
...
def targetMemberExperienceFamilyRouteScores(
    ...
    source = normStem(source)
    familyMembers: dict[str, Counter[str]] = defaultdict(Counter)
    familyTotals: Counter[str] = Counter()
    familyMax: Counter[str] = Counter()
    for member, score in rawScores.items():
        family = targetMemberExperienceFamilyKey(member, model)
        value = max(0.0, float(score))
        familyMembers[family][member] = max(familyMembers[family].get(member, 0.0), value)
        familyTotals[family] += value
        familyMax[family] = max(familyMax.get(family, 0.0), value)
        stats["experienceFamilyInputRows"] += 1

    routed: Counter[str] = Counter()
    for family, memberScores in familyMembers.items():
        totalScore = familyTotals[family]
        if totalScore <= 0.0:
            continue
        familyScore = familyMax[family] + math.log1p(totalScore) * 0.24
        candidatePool: list[str] = []
        seen: set[str] = set()
        if not family.startswith("surface:"):
            for member in model.contrastSemanticUnitMembers.get(family, tuple())[
                :TARGET_MEMBER_EXPERIENCE_FAMILY_MEMBER_LIMIT
            ]:
                member = normStem(member)
                if member and member not in seen:
                    seen.add(member)
                    candidatePool.append(member)
        for member, _score in memberScores.most_common(TARGET_MEMBER_EXPERIENCE_FAMILY_MEMBER_LIMIT):
            member = normStem(member)
            if member and member not in seen:
                seen.add(member)
                candidatePool.append(member)
        if not candidatePool:
            candidatePool = [member for member, _score in memberScores.most_common(1)]
        ...
        bestAcceptanceProof = 0.0
        bestAcceptanceMargin = 0.0
        for offset, member in enumerate(candidatePool):
            if not member or member == source:
                continue
            raw = float(memberScores.get(member, 0.0))
            conditional, conditionalStats = targetMemberConditionalTransportRatio(source, member, relations, model)
            ...
            acceptanceProof, acceptanceStats = targetMemberResidualAcceptanceProof(source, member, relations, model)
            acceptanceMargin = 0.0
            canonicalPrior = 1.0 / math.sqrt(1.0 + offset * 0.42)
            injected = 0.0 if raw > 0.0 else 1.0
            memberScore = raw + familyScore * (
                0.14 * canonicalPrior
                + 0.07 * conditional
                + 0.28 * mutualProof
                + 0.18 * max(0.0, mutualMargin)
                + 0.30 * acceptanceProof
                + 0.24 * max(0.0, acceptanceMargin)
            )
            if injected:
                memberScore *= 0.68
                stats["experienceFamilyInjectedRows"] += 1
            if conditionalStats.get("conditionalTransportMilli", 0) > 0:
                stats["experienceFamilyConditionalRows"] += 1
            if mutualStats.get("mutualProofMilli", 0) > 0:
                stats["experienceFamilyMutualRows"] += 1
            if mutualMargin > 0.0:
                stats["experienceFamilyMutualPositiveMargins"] += 1
            if acceptanceStats.get("acceptanceProofMilli", 0) > 0:
                stats["experienceFamilyAcceptanceRows"] += 1
            if acceptanceMargin > 0.0:
                stats["experienceFamilyAcceptancePositiveMargins"] += 1
            if memberScore > bestMemberScore:
                bestMemberScore = memberScore
                bestMember = member
                bestConditional = conditional
                bestMutualProof = mutualProof
                bestMutualMargin = mutualMargin
                bestAcceptanceProof = acceptanceProof
                bestAcceptanceMargin = acceptanceMargin
        if not bestMember:
        ...
        outputScore = familyScore * (
            1.0
            + min(0.32, bestConditional * 0.12)
            + min(0.58, bestMutualProof * 0.30 + max(0.0, bestMutualMargin) * 0.22)
            + min(0.50, bestAcceptanceProof * 0.26 + max(0.0, bestAcceptanceMargin) * 0.22)
        )
        ...
        if bestMember in memberScores:
            stats["experienceFamilyRawRepresentativeRows"] += 1
        else:
            stats["experienceFamilyCanonicalRepresentativeRows"] += 1
        stats["experienceFamilyRepresentativeConditionalMilli"] += int(bestConditional * 1000)
        stats["experienceFamilyRepresentativeMutualMilli"] += int(bestMutualProof * 1000)
        stats["experienceFamilyRepresentativeMutualMarginMilli"] += int(bestMutualMargin * 1000)
        stats["experienceFamilyRepresentativeAcceptanceMilli"] += int(bestAcceptanceProof * 1000)
        stats["experienceFamilyRepresentativeAcceptanceMarginMilli"] += int(bestAcceptanceMargin * 1000)
...
def targetMemberRelationFrameScores(
    ...
    for candidate, seedScore in seeds.most_common(TARGET_MEMBER_FRAME_CANDIDATE_LIMIT):
        ...
        acceptanceProof, acceptanceStats = targetMemberResidualAcceptanceProof(source, candidate, relations, model)
        acceptanceMargin = 0.0
        acceptancePeer = ""
        acceptancePeerProof = 0.0
        acceptancePeerCount = 0
        multiplier, positive, negative, inspected = contrastSemanticUnitSourceMemberEvidenceBalance(
        ...
        if mutualStats.get("mutualReverseRows", 0) > 0 and mutualStats.get("mutualSourcePaths", 0) > 0:
            ...
            mutualMass = 1.0
        if acceptanceStats.get("acceptanceReverseRows", 0) > 0 and acceptanceStats.get("acceptanceSourcePaths", 0) > 0:
            acceptanceMass = 1.0 + acceptanceProof * TARGET_MEMBER_RESIDUAL_ACCEPTANCE_GAIN
            acceptanceMass = min(3.15, max(0.52, acceptanceMass))
        elif sourceLedgerSketch:
            acceptanceMass = TARGET_MEMBER_RESIDUAL_ACCEPTANCE_EMPTY_FACTOR
        else:
            acceptanceMass = 1.0
        seedMass = math.log1p(max(0.0, float(seedScore)))
        ...
        stats["acceptancePairScored"] += 1
        stats["acceptanceProofMilli"] += acceptanceStats.get("acceptanceProofMilli", 0)
        stats["acceptanceRawMilli"] += acceptanceStats.get("acceptanceRawMilli", 0)
        stats["acceptanceOpportunityMilli"] += acceptanceStats.get("acceptanceOpportunityMilli", 0)
        stats["acceptanceAnchorMilli"] += acceptanceStats.get("acceptanceAnchorMilli", 0)
        stats["acceptanceLedgerMilli"] += acceptanceStats.get("acceptanceLedgerMilli", 0)
        stats["acceptanceRoleMilli"] += acceptanceStats.get("acceptanceRoleMilli", 0)
        stats["acceptanceRelationMilli"] += acceptanceStats.get("acceptanceRelationMilli", 0)
        stats["acceptanceFamilyMilli"] += acceptanceStats.get("acceptanceFamilyMilli", 0)
        stats["acceptanceSourcePaths"] += acceptanceStats.get("acceptanceSourcePaths", 0)
        stats["acceptanceReverseRows"] += acceptanceStats.get("acceptanceReverseRows", 0)
        stats["acceptancePairRows"] += acceptanceStats.get("acceptancePairRows", 0)
        stats["acceptanceRelationRows"] += acceptanceStats.get("acceptanceRelationRows", 0)
        stats["acceptanceAnchorRows"] += acceptanceStats.get("acceptanceAnchorRows", 0)
        stats["acceptanceLedgerRows"] += acceptanceStats.get("acceptanceLedgerRows", 0)
        stats["acceptanceWeakAnchorRows"] += acceptanceStats.get("acceptanceWeakAnchorRows", 0)
        stats["acceptanceAcceptedRows"] += acceptanceStats.get("acceptanceAcceptedRows", 0)
        stats["acceptanceExactRows"] += acceptanceStats.get("acceptanceExactRows", 0)
        stats["acceptanceResidualSlotRows"] += acceptanceStats.get("acceptanceResidualSlotRows", 0)
        stats["acceptanceSkippedSlotRows"] += acceptanceStats.get("acceptanceSkippedSlotRows", 0)
        stats["acceptanceSlotEvaluated"] += acceptanceStats.get("acceptanceSlotEvaluated", 0)
        stats["acceptanceSlotAccepted"] += acceptanceStats.get("acceptanceSlotAccepted", 0)
        stats["acceptanceSlotRejected"] += acceptanceStats.get("acceptanceSlotRejected", 0)
        stats["acceptanceSlotRows"] += acceptanceStats.get("acceptanceSlotRows", 0)
        stats["acceptanceSlotFamilyShareMilli"] += acceptanceStats.get("acceptanceSlotFamilyShareMilli", 0)
        stats["acceptanceSlotRoleShareMilli"] += acceptanceStats.get("acceptanceSlotRoleShareMilli", 0)
        stats["acceptanceSlotSaturationMilli"] += acceptanceStats.get("acceptanceSlotSaturationMilli", 0)
        stats["acceptanceSlotWeightMilli"] += acceptanceStats.get("acceptanceSlotWeightMilli", 0)
        stats["acceptancePositiveMargins"] += int(acceptanceMargin > 0.0)
        stats["acceptanceNegativeMargins"] += int(acceptanceMargin < 0.0)
        stats["acceptanceMarginMilli"] += int(acceptanceMargin * 1000)
        stats["acceptancePeerProofMilli"] += int(acceptancePeerProof * 1000)
        stats["acceptancePeerRows"] += acceptancePeerCount
        if acceptancePeer:
            stats["acceptancePeerComparisons"] += 1
        stats[
...
def targetMemberRelationFrameDiagnostics(
    ...
    acceptanceProof, acceptanceStats = targetMemberResidualAcceptanceProof(source, target, relations, model)
    acceptanceMargin, acceptancePeer, acceptancePeerProof, acceptancePeerCount = targetMemberResidualAcceptanceMargin(
        source,
        target,
        relations,
        acceptanceProof,
    ...
    return (
        ...
        acceptanceProof,
        acceptanceMargin,
        acceptancePeer or "none",
        acceptancePeerProof,
        acceptancePeerCount,
        acceptanceStats.get("acceptanceSourcePaths", 0),
        acceptanceStats.get("acceptanceReverseRows", 0),
        acceptanceStats.get("acceptancePairRows", 0),
        acceptanceStats.get("acceptanceAnchorMilli", 0),
        acceptanceStats.get("acceptanceLedgerMilli", 0),
        acceptanceStats.get("acceptanceRoleMilli", 0),
        acceptanceStats.get("acceptanceRelationMilli", 0),
        acceptanceStats.get("acceptanceFamilyMilli", 0),
        acceptanceStats.get("acceptanceResidualSlotRows", 0),
        acceptanceStats.get("acceptanceSlotAccepted", 0),
        acceptanceStats.get("acceptanceSlotRejected", 0),
        acceptanceStats.get("acceptanceSlotWeightMilli", 0),
        acceptanceStats.get("acceptanceSlotFamilyShareMilli", 0),
        acceptanceStats.get("acceptanceSlotRoleShareMilli", 0),
    )
...
def main() -> None:
    ...
    targetMemberFrameAcceptancePairScored = 0
    targetMemberFrameAcceptanceProofMilli = 0
    targetMemberFrameAcceptanceRawMilli = 0
    targetMemberFrameAcceptanceOpportunityMilli = 0
    targetMemberFrameAcceptanceAnchorMilli = 0
    targetMemberFrameAcceptanceLedgerMilli = 0
    targetMemberFrameAcceptanceRoleMilli = 0
    targetMemberFrameAcceptanceRelationMilli = 0
    targetMemberFrameAcceptanceFamilyMilli = 0
    targetMemberFrameAcceptanceSourcePaths = 0
    targetMemberFrameAcceptanceReverseRows = 0
    targetMemberFrameAcceptancePairRows = 0
    targetMemberFrameAcceptanceRelationRows = 0
    targetMemberFrameAcceptanceAnchorRows = 0
    targetMemberFrameAcceptanceLedgerRows = 0
    targetMemberFrameAcceptanceWeakAnchorRows = 0
    targetMemberFrameAcceptanceAcceptedRows = 0
    targetMemberFrameAcceptanceExactRows = 0
    targetMemberFrameAcceptanceResidualSlotRows = 0
    targetMemberFrameAcceptanceSkippedSlotRows = 0
    targetMemberFrameAcceptanceSlotEvaluated = 0
    targetMemberFrameAcceptanceSlotAccepted = 0
    targetMemberFrameAcceptanceSlotRejected = 0
    targetMemberFrameAcceptanceSlotRows = 0
    targetMemberFrameAcceptanceSlotFamilyShareMilli = 0
    targetMemberFrameAcceptanceSlotRoleShareMilli = 0
    targetMemberFrameAcceptanceSlotSaturationMilli = 0
    targetMemberFrameAcceptanceSlotWeightMilli = 0
    targetMemberFrameAcceptancePositiveMargins = 0
    targetMemberFrameAcceptanceNegativeMargins = 0
    targetMemberFrameAcceptanceMarginMilli = 0
    targetMemberFrameAcceptancePeerProofMilli = 0
    targetMemberFrameAcceptancePeerRows = 0
    ...
    targetMemberFrameExperienceFamilyAcceptanceRows = 0
    targetMemberFrameExperienceFamilyAcceptancePositiveMargins = 0
    ...
    targetMemberFrameExperienceFamilyRepresentativeAcceptanceMilli = 0
    targetMemberFrameExperienceFamilyRepresentativeAcceptanceMarginMilli = 0
    ...
    for surface, expected in POSITIVE_PROBES:
        ...
        (
            ...
            acceptancePeerProof,
            acceptancePeerCount,
            acceptanceSourcePaths,
            acceptanceReverseRows,
            acceptancePairRows,
            acceptanceAnchorMilli,
            acceptanceLedgerMilli,
            acceptanceRoleMilli,
            acceptanceRelationMilli,
            acceptanceFamilyMilli,
            acceptanceResidualSlotRows,
            acceptanceSlotAccepted,
            acceptanceSlotRejected,
            acceptanceSlotWeightMilli,
            acceptanceSlotFamilyShareMilli,
            acceptanceSlotRoleShareMilli,
        ) = diag
        ...
        targetMemberFrameAcceptancePairScored += frameStats.get("acceptancePairScored", 0)
        targetMemberFrameAcceptanceProofMilli += frameStats.get("acceptanceProofMilli", 0)
        targetMemberFrameAcceptanceRawMilli += frameStats.get("acceptanceRawMilli", 0)
        targetMemberFrameAcceptanceOpportunityMilli += frameStats.get("acceptanceOpportunityMilli", 0)
        targetMemberFrameAcceptanceAnchorMilli += frameStats.get("acceptanceAnchorMilli", 0)
        targetMemberFrameAcceptanceLedgerMilli += frameStats.get("acceptanceLedgerMilli", 0)
        targetMemberFrameAcceptanceRoleMilli += frameStats.get("acceptanceRoleMilli", 0)
        targetMemberFrameAcceptanceRelationMilli += frameStats.get("acceptanceRelationMilli", 0)
        targetMemberFrameAcceptanceFamilyMilli += frameStats.get("acceptanceFamilyMilli", 0)
        targetMemberFrameAcceptanceSourcePaths += frameStats.get("acceptanceSourcePaths", 0)
        targetMemberFrameAcceptanceReverseRows += frameStats.get("acceptanceReverseRows", 0)
        targetMemberFrameAcceptancePairRows += frameStats.get("acceptancePairRows", 0)
        targetMemberFrameAcceptanceRelationRows += frameStats.get("acceptanceRelationRows", 0)
        targetMemberFrameAcceptanceAnchorRows += frameStats.get("acceptanceAnchorRows", 0)
        targetMemberFrameAcceptanceLedgerRows += frameStats.get("acceptanceLedgerRows", 0)
        targetMemberFrameAcceptanceWeakAnchorRows += frameStats.get("acceptanceWeakAnchorRows", 0)
        targetMemberFrameAcceptanceAcceptedRows += frameStats.get("acceptanceAcceptedRows", 0)
        targetMemberFrameAcceptanceExactRows += frameStats.get("acceptanceExactRows", 0)
        targetMemberFrameAcceptanceResidualSlotRows += frameStats.get("acceptanceResidualSlotRows", 0)
        targetMemberFrameAcceptanceSkippedSlotRows += frameStats.get("acceptanceSkippedSlotRows", 0)
        targetMemberFrameAcceptanceSlotEvaluated += frameStats.get("acceptanceSlotEvaluated", 0)
        targetMemberFrameAcceptanceSlotAccepted += frameStats.get("acceptanceSlotAccepted", 0)
        targetMemberFrameAcceptanceSlotRejected += frameStats.get("acceptanceSlotRejected", 0)
        targetMemberFrameAcceptanceSlotRows += frameStats.get("acceptanceSlotRows", 0)
        targetMemberFrameAcceptanceSlotFamilyShareMilli += frameStats.get("acceptanceSlotFamilyShareMilli", 0)
        targetMemberFrameAcceptanceSlotRoleShareMilli += frameStats.get("acceptanceSlotRoleShareMilli", 0)
        targetMemberFrameAcceptanceSlotSaturationMilli += frameStats.get("acceptanceSlotSaturationMilli", 0)
        targetMemberFrameAcceptanceSlotWeightMilli += frameStats.get("acceptanceSlotWeightMilli", 0)
        targetMemberFrameAcceptancePositiveMargins += frameStats.get("acceptancePositiveMargins", 0)
        targetMemberFrameAcceptanceNegativeMargins += frameStats.get("acceptanceNegativeMargins", 0)
        targetMemberFrameAcceptanceMarginMilli += frameStats.get("acceptanceMarginMilli", 0)
        targetMemberFrameAcceptancePeerProofMilli += frameStats.get("acceptancePeerProofMilli", 0)
        targetMemberFrameAcceptancePeerRows += frameStats.get("acceptancePeerRows", 0)
        ...
        targetMemberFrameExperienceFamilyAcceptanceRows += frameStats.get("experienceFamilyAcceptanceRows", 0)
        targetMemberFrameExperienceFamilyAcceptancePositiveMargins += frameStats.get(
            "experienceFamilyAcceptancePositiveMargins", 0
        )
        ...
        targetMemberFrameExperienceFamilyRepresentativeAcceptanceMilli += frameStats.get(
            "experienceFamilyRepresentativeAcceptanceMilli", 0
        )
        targetMemberFrameExperienceFamilyRepresentativeAcceptanceMarginMilli += frameStats.get(
            "experienceFamilyRepresentativeAcceptanceMarginMilli", 0
        )
        print(
            f"  {surface}->{expected} relations={','.join(relations)} candidateHit={candidateHit} "
            ...
            f"acceptance={acceptanceProof:.3f}/margin{acceptanceMargin:.3f}/peer{acceptancePeer}/peerProof{acceptancePeerProof:.3f}/peers{acceptancePeerCount}/srcPaths{acceptanceSourcePaths}/revRows{acceptanceReverseRows}/pairs{acceptancePairRows}/anchor{acceptanceAnchorMilli}/ledger{acceptanceLedgerMilli}/role{acceptanceRoleMilli}/rel{acceptanceRelationMilli}/family{acceptanceFamilyMilli}/slotRows{acceptanceResidualSlotRows}/slotAcc{acceptanceSlotAccepted}/slotRej{acceptanceSlotRejected}/slotW{acceptanceSlotWeightMilli}/famShare{acceptanceSlotFamilyShareMilli}/roleShare{acceptanceSlotRoleShareMilli} "
    ...
    print(
        ...
        f"targetMemberFrameAcceptancePairScored={targetMemberFrameAcceptancePairScored} "
        f"targetMemberFrameAcceptanceProofMilli={targetMemberFrameAcceptanceProofMilli} "
        f"targetMemberFrameAcceptanceRawMilli={targetMemberFrameAcceptanceRawMilli} "
        f"targetMemberFrameAcceptanceOpportunityMilli={targetMemberFrameAcceptanceOpportunityMilli} "
        f"targetMemberFrameAcceptanceAnchorMilli={targetMemberFrameAcceptanceAnchorMilli} "
        f"targetMemberFrameAcceptanceLedgerMilli={targetMemberFrameAcceptanceLedgerMilli} "
        f"targetMemberFrameAcceptanceRoleMilli={targetMemberFrameAcceptanceRoleMilli} "
        f"targetMemberFrameAcceptanceRelationMilli={targetMemberFrameAcceptanceRelationMilli} "
        f"targetMemberFrameAcceptanceFamilyMilli={targetMemberFrameAcceptanceFamilyMilli} "
        f"targetMemberFrameAcceptanceSourcePaths={targetMemberFrameAcceptanceSourcePaths} "
        f"targetMemberFrameAcceptanceReverseRows={targetMemberFrameAcceptanceReverseRows} "
        f"targetMemberFrameAcceptancePairRows={targetMemberFrameAcceptancePairRows} "
        f"targetMemberFrameAcceptanceRelationRows={targetMemberFrameAcceptanceRelationRows} "
        f"targetMemberFrameAcceptanceAnchorRows={targetMemberFrameAcceptanceAnchorRows} "
        f"targetMemberFrameAcceptanceLedgerRows={targetMemberFrameAcceptanceLedgerRows} "
        f"targetMemberFrameAcceptanceWeakAnchorRows={targetMemberFrameAcceptanceWeakAnchorRows} "
        f"targetMemberFrameAcceptanceAcceptedRows={targetMemberFrameAcceptanceAcceptedRows} "
        f"targetMemberFrameAcceptanceExactRows={targetMemberFrameAcceptanceExactRows} "
        f"targetMemberFrameAcceptanceResidualSlotRows={targetMemberFrameAcceptanceResidualSlotRows} "
        f"targetMemberFrameAcceptanceSkippedSlotRows={targetMemberFrameAcceptanceSkippedSlotRows} "
        f"targetMemberFrameAcceptanceSlotEvaluated={targetMemberFrameAcceptanceSlotEvaluated} "
        f"targetMemberFrameAcceptanceSlotAccepted={targetMemberFrameAcceptanceSlotAccepted} "
        f"targetMemberFrameAcceptanceSlotRejected={targetMemberFrameAcceptanceSlotRejected} "
        f"targetMemberFrameAcceptanceSlotRows={targetMemberFrameAcceptanceSlotRows} "
        f"targetMemberFrameAcceptanceSlotFamilyShareMilli={targetMemberFrameAcceptanceSlotFamilyShareMilli} "
        f"targetMemberFrameAcceptanceSlotRoleShareMilli={targetMemberFrameAcceptanceSlotRoleShareMilli} "
        f"targetMemberFrameAcceptanceSlotSaturationMilli={targetMemberFrameAcceptanceSlotSaturationMilli} "
        f"targetMemberFrameAcceptanceSlotWeightMilli={targetMemberFrameAcceptanceSlotWeightMilli} "
        f"targetMemberFrameAcceptancePositiveMargins={targetMemberFrameAcceptancePositiveMargins} "
        f"targetMemberFrameAcceptanceNegativeMargins={targetMemberFrameAcceptanceNegativeMargins} "
        f"targetMemberFrameAcceptanceMarginMilli={targetMemberFrameAcceptanceMarginMilli} "
        f"targetMemberFrameAcceptancePeerProofMilli={targetMemberFrameAcceptancePeerProofMilli} "
        f"targetMemberFrameAcceptancePeerRows={targetMemberFrameAcceptancePeerRows} "
        ...
        f"targetMemberFrameExperienceFamilyAcceptanceRows={targetMemberFrameExperienceFamilyAcceptanceRows} "
        f"targetMemberFrameExperienceFamilyAcceptancePositiveMargins={targetMemberFrameExperienceFamilyAcceptancePositiveMargins} "
        ...
        f"targetMemberFrameExperienceFamilyRepresentativeAcceptanceMilli={targetMemberFrameExperienceFamilyRepresentativeAcceptanceMilli} "
        f"targetMemberFrameExperienceFamilyRepresentativeAcceptanceMarginMilli={targetMemberFrameExperienceFamilyRepresentativeAcceptanceMarginMilli} "
