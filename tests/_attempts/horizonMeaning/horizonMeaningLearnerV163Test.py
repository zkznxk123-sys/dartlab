"""Horizon Meaning Learner V163 - residual experience-field family canonicalization.

V163 실제 기록
--------------
아이디어:
    V160 은 generalized reverse slot 으로 exact target reverseRows=0 병목을 깼다. `외상매출금 -> 매출채권` 은
    mutual=0.310/revRows48 이 되었고 `손실충당금 -> 대손충당금` 도 mutual=0.287/revRows48 이 되었다.
    하지만 broad slot 과 sibling peer 가 더 강해서 대표 member 는 고르지 못했다.

    V161 은 slot 을 candidate-family share 로 reject 하다가 cold-start 정답 field 를 꺼버렸고,
    V162 는 corpus-general support/density/length prior 로 후단 canonical selector 를 만들었지만
    대표 member 를 고르지 못하고 비용만 늘었다. 둘 다 "경험이 의미" 라는 본체가 아니라 후단 보정이었다.

    수평선 좌표는 의미단위가 아니다. stem 을 빠르게 찾아가기 위한 고정 주소계다.
    의미는 그 주소가 겪은 주변 stem, 앞뒤 순서, 거리, role, relation, 문장/표 위치 경험이 반복적으로
    겹쳐 만들어지는 누적 경험 graph/field 다.

    V163 은 후보 surface 를 바로 점수화하지 않는다. 먼저 각 target member 의 경험 field 를 만든다.
    이 field 는 role/profile bucket, relation-local ledger sketch, semantic experience signature,
    generalized reverse source role/relation 경험을 묶은 것이다. 그 다음 query source 가 기대하는 target
    field 를 만들고, 후보들을 field 유사도로 local family 로 먼저 접는다.

    family 내부에서는 공통 field atom 을 제거한다. 남은 residual field 와 source expected field 가 겹치는
    member 를 representative 로 고른다. 즉 `매출`, `매출금액`, `매출채권`, `단기매출채권및기타채권` 을
    raw surface 경쟁자로 두지 않고, 같은 경험 family 안의 공통 경험을 뺀 뒤 source path 가 실제로 지지하는
    잔여 경험을 비교한다.

    즉 V163 의 판단 단위는 수평선 좌표도 exact surface 도 후단 prior 도 아니라,
    source-conditioned residual experience-field family 다.

실행 코드:
    $env:DARTLAB_HORIZON_V163_MAX_RECORDS_PER_SOURCE='120'
    $env:DARTLAB_HORIZON_V163_MAX_UNITS='1200'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV163Test.py
...
검증 기준:
    ...
    21. field family 는 contrast unit label 만 쓰지 않고 role/profile/ledger/reverse-source/semantic experience atom 으로 만들어야 한다.
    22. representative 선택은 support/length prior 가 아니라 family 공통 atom 제거 후 source expected field 와의 residual overlap 을 써야 한다.
    23. `외상매출금 -> 매출채권` 은 V160 의 mutual=0.310/revRows48 recall 을 유지하면서 rank 를 V147 rank=7 쪽으로 회복해야 한다.
    24. `손실충당금 -> 대손충당금` 은 V160 rank=6 보다 나빠지면 안 된다.
    25. fixed route/search guard 는 positiveHits=3/4, badAccepted=0/7 수준을 유지해야 한다.

결과:
    1,200 unit smoke 는 stderr 없이 완료됐지만 totalSeconds=1319.7 로 V162 1270.8 보다 더 비싸졌다.
    targetMemberFrame 은 Top1=1/4, Top5=1/4, BadTop1=0/7, BadTop5=0/7, candidateHits=5/11 로
    V160/V162 의 recall 수준을 넘지 못했다. fixed guard 는 positiveHits=3/4, badAccepted=0/7,
    searchTop1=4/5, reliableSearch=4/5 를 유지했다.

    field family 는 실제로 계산됐다. fieldFamilyGroups=14, expandedRows=1056, commonAtoms=582,
    sourceAtoms=1294, residualAtoms=12051, residualSupportMilli=46928, fullSupportMilli=73329,
    distinctMilli=126655 이었다. 하지만 representative 는 rawRepresentativeRows=14,
    injectedRepresentativeRows=0 이라 새 canonical member 를 만들지 못했다.

    `외상매출금 -> 매출채권` 은 candidateHit=True, mutual=0.310/revRows48 을 유지했지만
    fieldFam=grp1/expanded96/common47/resid335/full518/distinct731/injRep0, rank=NA, top=`매출` 이었다.
    `손실충당금 -> 대손충당금` 은 mutual=0.287/revRows48 이지만
    fieldFam=grp1/expanded96/common32/resid205/full409/distinct681/injRep0, rank=NA, top=`손실충당` 으로
    V160 rank=6 보다 악화됐다. `현금성자산 -> 현금및현금성자산` 만 rank=1 을 유지했다.
    negative `당기순이익 -> 영업이익` 은 forbiddenRank=NA 로 V160/V162 의 badTop5 문제는 제거됐다.

결론:
    실패/진단 성공. 경험 field 를 만들고 residual/common atom 도 계산할 수 있다는 것은 확인했다.
    그러나 현재 field similarity 는 후보를 의미 있는 여러 family 로 나누지 못하고 대부분 `grp1` 큰 묶음으로
    collapse 한다. 그래서 representative contest 는 raw 후보를 그대로 고를 뿐 source 가 원하는 canonical
    member 를 새로 주입하지 못한다.

    keep 할 것은 "수평선은 주소, 의미는 누적 경험 field" 라는 정의와 generalized reverse slot recall 이다.
    discard 할 것은 broad field family 를 먼저 만든 뒤 residual overlap 으로 대표를 고르는 방식이다.
    다음 메커니즘은 family 를 score 이후에 묶지 말고, 학습 단계에서 role/ledger/relation/event polarity 로
    typed micro-field 를 만들고 각 micro-field 사이의 typed edge 를 비교해야 한다. 즉 `매출채권` 류 계정명,
    `매출` 류 실적/수익, `손실충당` 류 modifier 가 같은 큰 field 에 섞이지 않도록 field 생성 자체를
    typed relation graph 로 바꿔야 한다.
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V163_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V163_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V163_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V163_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V163_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V163_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V163_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V163_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V163_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_SEARCH_CANDIDATE_LIMIT", "420"))
DYNAMIC_TARGET_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_DYNAMIC_TARGET_LIMIT", "80"))
DYNAMIC_COORD_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_DYNAMIC_COORD_ROW_LIMIT", "220"))
DYNAMIC_COMPOUND_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_DYNAMIC_COMPOUND_ROW_LIMIT", "260"))
DYNAMIC_QUERY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_DYNAMIC_QUERY_ATOM_LIMIT", "48"))
DYNAMIC_MEANING_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_DYNAMIC_MEANING_ATOM_LIMIT", "36"))
DYNAMIC_MEANING_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_DYNAMIC_MEANING_ROW_LIMIT", "220"))
DYNAMIC_RELATION_SURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_DYNAMIC_RELATION_SURFACE_LIMIT", "420"))
DYNAMIC_RELATION_UNIT_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_DYNAMIC_RELATION_UNIT_LIMIT", "160"))
DYNAMIC_BRIDGE_ONLY_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V163_DYNAMIC_BRIDGE_ONLY_PENALTY", "0.80"))
DYNAMIC_OWNER_ROLE_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_DYNAMIC_OWNER_ROLE_SIGNATURE_LIMIT", "64")
)
DYNAMIC_OWNER_ROLE_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_DYNAMIC_OWNER_ROLE_ATOM_LIMIT", "40"))
DYNAMIC_OWNER_ROLE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_DYNAMIC_OWNER_ROLE_ROW_LIMIT", "220"))
DYNAMIC_OWNER_ROLE_UNIT_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_DYNAMIC_OWNER_ROLE_UNIT_ATOM_LIMIT", "36")
)
DYNAMIC_OWNER_ROLE_MIN_BOUND = float(os.environ.get("DARTLAB_HORIZON_V163_DYNAMIC_OWNER_ROLE_MIN_BOUND", "0.55"))
DYNAMIC_OWNER_ROLE_CANDIDATE_BONUS = float(
    os.environ.get("DARTLAB_HORIZON_V163_DYNAMIC_OWNER_ROLE_CANDIDATE_BONUS", "2.40")
)
DYNAMIC_OWNER_ROLE_ROUTE_BONUS = float(os.environ.get("DARTLAB_HORIZON_V163_DYNAMIC_OWNER_ROLE_ROUTE_BONUS", "0.90"))
DYNAMIC_OWNER_ROLE_WEAK_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V163_DYNAMIC_OWNER_ROLE_WEAK_PENALTY", "0.16"))
DYNAMIC_OWNER_FRAME_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V163_DYNAMIC_OWNER_FRAME_RADIUS", "5"))
DYNAMIC_OWNER_FRAME_BETWEEN_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_DYNAMIC_OWNER_FRAME_BETWEEN_LIMIT", "7"))
MASKED_FRAME_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_MASKED_FRAME_SIGNATURE_LIMIT", "72"))
MASKED_FRAME_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_MASKED_FRAME_ATOM_LIMIT", "48"))
MASKED_FRAME_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_MASKED_FRAME_ROW_LIMIT", "240"))
MASKED_FRAME_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_MASKED_FRAME_EVAL_LIMIT", "800"))
MASKED_FRAME_HOLDOUT_MOD = int(os.environ.get("DARTLAB_HORIZON_V163_MASKED_FRAME_HOLDOUT_MOD", "5"))
MASKED_FRAME_PROBE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_MASKED_FRAME_PROBE_LIMIT", "12"))
MASKED_CONTEXT_SURFACE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_MASKED_CONTEXT_SURFACE_ROW_LIMIT", "12"))
MASKED_CONTEXT_WEIGHT = float(os.environ.get("DARTLAB_HORIZON_V163_MASKED_CONTEXT_WEIGHT", "0.35"))
MASKED_CONTEXT_RELATION_WINDOW = int(os.environ.get("DARTLAB_HORIZON_V163_MASKED_CONTEXT_RELATION_WINDOW", "140"))
MASKED_FRAME_SURPRISAL_POWER = float(os.environ.get("DARTLAB_HORIZON_V163_MASKED_FRAME_SURPRISAL_POWER", "2.0"))
MASKED_FRAME_SURFACE_PRIOR_POWER = float(
    os.environ.get("DARTLAB_HORIZON_V163_MASKED_FRAME_SURFACE_PRIOR_POWER", "0.45")
)
RELATION_OWNER_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_RELATION_OWNER_SIGNATURE_LIMIT", "64"))
RELATION_OWNER_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_RELATION_OWNER_ROW_LIMIT", "220"))
RELATION_OWNER_BRIDGE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_RELATION_OWNER_BRIDGE_LIMIT", "12"))
RELATION_OWNER_PROJECTION_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_RELATION_OWNER_PROJECTION_LIMIT", "24"))
RELATION_OWNER_PROJECTION_OWNER_ROLE_MIN = float(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_OWNER_PROJECTION_OWNER_ROLE_MIN", "0.18")
)
RELATION_OWNER_PROJECTION_QUERY_ROLE_MIN = float(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_OWNER_PROJECTION_QUERY_ROLE_MIN", "0.05")
)
RELATION_OWNER_GENERATED_BRIDGE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_OWNER_GENERATED_BRIDGE_LIMIT", "48")
)
RELATION_OWNER_GENERATED_BRIDGE_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_OWNER_GENERATED_BRIDGE_WEIGHT", "2.4")
)
RELATION_OWNER_ROLE_COMPAT_MIN = float(os.environ.get("DARTLAB_HORIZON_V163_RELATION_OWNER_ROLE_COMPAT_MIN", "0.16"))
RELATION_BOUND_ROLE_PAIR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_RELATION_BOUND_ROLE_PAIR_LIMIT", "24"))
RELATION_BOUND_ROLE_PAIR_BRIDGE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_BOUND_ROLE_PAIR_BRIDGE_LIMIT", "10")
)
RELATION_BOUND_ROLE_PAIR_BRIDGE_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_BOUND_ROLE_PAIR_BRIDGE_WEIGHT", "0.46")
)
RELATION_SOURCE_EXPERIENCE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_SOURCE_EXPERIENCE_ATOM_LIMIT", "44")
)
RELATION_SOURCE_EXPERIENCE_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_SOURCE_EXPERIENCE_ROW_LIMIT", "180")
)
RELATION_SOURCE_SHINGLE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_SOURCE_SHINGLE_ATOM_LIMIT", "28")
)
RELATION_SOURCE_SHINGLE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_RELATION_SOURCE_SHINGLE_ROW_LIMIT", "160"))
RELATION_SOURCE_SHINGLE_KEY_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_RELATION_SOURCE_SHINGLE_KEY_LIMIT", "56"))
RELATION_ANCHORED_COARSE_SHINGLE_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_ANCHORED_COARSE_SHINGLE_ROW_LIMIT", "180")
)
RELATION_ANCHORED_COARSE_SHINGLE_KEY_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_ANCHORED_COARSE_SHINGLE_KEY_LIMIT", "72")
)
HORIZON_MASK_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_HORIZON_MASK_SIGNATURE_LIMIT", "96"))
HORIZON_MASK_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_HORIZON_MASK_ATOM_LIMIT", "56"))
HORIZON_MASK_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_HORIZON_MASK_ROW_LIMIT", "220"))
HORIZON_MASK_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_HORIZON_MASK_EVAL_LIMIT", "900"))
HORIZON_MASK_HOLDOUT_MOD = int(os.environ.get("DARTLAB_HORIZON_V163_HORIZON_MASK_HOLDOUT_MOD", "7"))
HORIZON_MASK_SURFACE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_HORIZON_MASK_SURFACE_ROW_LIMIT", "24"))
HORIZON_MASK_NEIGHBOR_EXPERIENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_HORIZON_MASK_NEIGHBOR_EXPERIENCE_LIMIT", "5")
)
HORIZON_MASK_COORD_CELL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_HORIZON_MASK_COORD_CELL_LIMIT", "5"))
HORIZON_MASK_COORD_BUCKET_DIGITS = int(os.environ.get("DARTLAB_HORIZON_V163_HORIZON_MASK_COORD_BUCKET_DIGITS", "5"))
LOCAL_FRAME_PREDICTION_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_LOCAL_FRAME_PREDICTION_SIGNATURE_LIMIT", "132")
)
LOCAL_FRAME_PREDICTION_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_LOCAL_FRAME_PREDICTION_ATOM_LIMIT", "76"))
LOCAL_FRAME_PREDICTION_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_LOCAL_FRAME_PREDICTION_ROW_LIMIT", "260"))
LOCAL_FRAME_PREDICTION_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_LOCAL_FRAME_PREDICTION_EVAL_LIMIT", "900"))
LOCAL_FRAME_PREDICTION_SURFACE_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_LOCAL_FRAME_PREDICTION_SURFACE_ROW_LIMIT", "18")
)
LOCAL_FRAME_PREDICTION_HOLDOUT_MOD = int(os.environ.get("DARTLAB_HORIZON_V163_LOCAL_FRAME_PREDICTION_HOLDOUT_MOD", "9"))
LOCAL_FRAME_PREDICTION_NEIGHBOR_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_LOCAL_FRAME_PREDICTION_NEIGHBOR_LIMIT", "5")
)
LOCAL_FRAME_PREDICTION_NEIGHBOR_EXPERIENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_LOCAL_FRAME_PREDICTION_NEIGHBOR_EXPERIENCE_LIMIT", "5")
)
LOCAL_FRAME_PREDICTION_SEQUENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_LOCAL_FRAME_PREDICTION_SEQUENCE_LIMIT", "5")
)
QUERY_LOCAL_FRAME_OCCURRENCE_INDEX_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_QUERY_LOCAL_FRAME_OCCURRENCE_INDEX_ROW_LIMIT", "96")
)
QUERY_LOCAL_FRAME_OCCURRENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_QUERY_LOCAL_FRAME_OCCURRENCE_LIMIT", "72")
)
QUERY_LOCAL_FRAME_PROXY_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_QUERY_LOCAL_FRAME_PROXY_LIMIT", "8"))
QUERY_LOCAL_FRAME_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_QUERY_LOCAL_FRAME_ATOM_LIMIT", "132"))
QUERY_LOCAL_FRAME_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_QUERY_LOCAL_FRAME_EVAL_LIMIT", "260"))
QUERY_SEMANTIC_UNIT_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_QUERY_SEMANTIC_UNIT_ATOM_LIMIT", "120"))
QUERY_SEMANTIC_UNIT_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_QUERY_SEMANTIC_UNIT_EVAL_LIMIT", "260"))
QUERY_SEMANTIC_UNIT_SURFACE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_QUERY_SEMANTIC_UNIT_SURFACE_LIMIT", str(MASKED_FRAME_PROBE_LIMIT))
)
SEMANTIC_UNIT_SIGNATURE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_SEMANTIC_UNIT_SIGNATURE_ATOM_LIMIT", "18")
)
SEMANTIC_UNIT_POSTING_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_SEMANTIC_UNIT_POSTING_ROW_LIMIT", "90"))
SEMANTIC_UNIT_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_SEMANTIC_UNIT_CANDIDATE_LIMIT", "180"))
SEMANTIC_UNIT_MIN_EXPERIENCE_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V163_SEMANTIC_UNIT_MIN_EXPERIENCE_SIM", "0.46")
)
SEMANTIC_UNIT_MIN_MIXED_SIM = float(os.environ.get("DARTLAB_HORIZON_V163_SEMANTIC_UNIT_MIN_MIXED_SIM", "0.34"))
SEMANTIC_UNIT_MIN_COORD_RESONANCE = float(
    os.environ.get("DARTLAB_HORIZON_V163_SEMANTIC_UNIT_MIN_COORD_RESONANCE", "0.045")
)
SEMANTIC_UNIT_MASK_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_SEMANTIC_UNIT_MASK_SIGNATURE_LIMIT", "120")
)
SEMANTIC_UNIT_MASK_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_SEMANTIC_UNIT_MASK_ATOM_LIMIT", "64"))
SEMANTIC_UNIT_MASK_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_SEMANTIC_UNIT_MASK_ROW_LIMIT", "260"))
SEMANTIC_UNIT_ROUTE_MEMBER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_SEMANTIC_UNIT_ROUTE_MEMBER_LIMIT", "4"))
CONTRAST_SEMANTIC_UNIT_MIN_EXPERIENCE_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V163_CONTRAST_SEMANTIC_UNIT_MIN_EXPERIENCE_SIM", "0.46")
)
CONTRAST_SEMANTIC_UNIT_STRONG_EXPERIENCE_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V163_CONTRAST_SEMANTIC_UNIT_STRONG_EXPERIENCE_SIM", "0.62")
)
CONTRAST_SEMANTIC_UNIT_MIN_MIXED_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V163_CONTRAST_SEMANTIC_UNIT_MIN_MIXED_SIM", "0.36")
)
CONTRAST_SEMANTIC_UNIT_MIN_COORD_RESONANCE = float(
    os.environ.get("DARTLAB_HORIZON_V163_CONTRAST_SEMANTIC_UNIT_MIN_COORD_RESONANCE", "0.050")
)
CONTRAST_SEMANTIC_UNIT_MIN_RELATION_COMPAT = float(
    os.environ.get("DARTLAB_HORIZON_V163_CONTRAST_SEMANTIC_UNIT_MIN_RELATION_COMPAT", "0.28")
)
CONTRAST_SEMANTIC_UNIT_SUFFIX_RELATION_COMPAT = float(
    os.environ.get("DARTLAB_HORIZON_V163_CONTRAST_SEMANTIC_UNIT_SUFFIX_RELATION_COMPAT", "0.18")
)
MEMBER_BALANCE_MEMBER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_MEMBER_BALANCE_MEMBER_LIMIT", "10"))
MEMBER_BALANCE_MIN_MULTIPLIER = float(os.environ.get("DARTLAB_HORIZON_V163_MEMBER_BALANCE_MIN_MULTIPLIER", "0.015"))
MEMBER_BALANCE_UNSUPPORTED_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V163_MEMBER_BALANCE_UNSUPPORTED_FACTOR", "0.12")
)
MEMBER_BALANCE_POSITIVE_GAIN = float(os.environ.get("DARTLAB_HORIZON_V163_MEMBER_BALANCE_POSITIVE_GAIN", "1.10"))
MEMBER_BALANCE_NEGATIVE_GAIN = float(os.environ.get("DARTLAB_HORIZON_V163_MEMBER_BALANCE_NEGATIVE_GAIN", "1.60"))
MEMBER_BALANCE_POSITIVE_SHIELD_RATIO = float(
    os.environ.get("DARTLAB_HORIZON_V163_MEMBER_BALANCE_POSITIVE_SHIELD_RATIO", "0.85")
)
...
MEMBER_CONDITIONED_MASK_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_MEMBER_CONDITIONED_MASK_SIGNATURE_LIMIT", "96")
)
MEMBER_CONDITIONED_MASK_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_MEMBER_CONDITIONED_MASK_ATOM_LIMIT", "64")
)
MEMBER_CONDITIONED_MASK_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_MEMBER_CONDITIONED_MASK_ROW_LIMIT", "220"))
MEMBER_CONDITIONED_ROUTE_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_MEMBER_CONDITIONED_ROUTE_MEMBER_LIMIT", "180")
)
MEMBER_CONDITIONED_LABEL_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_MEMBER_CONDITIONED_LABEL_MEMBER_LIMIT", "6")
)
MEMBER_CONDITIONED_MEMBER_CACHE: dict[tuple[int, str, str], tuple[float, float, float, int]] = {}
MEMBER_SUPPORTED_LABEL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_MEMBER_SUPPORTED_LABEL_LIMIT", "80"))
MEMBER_SUPPORTED_MEMBER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_MEMBER_SUPPORTED_MEMBER_LIMIT", "120"))
MEMBER_SUPPORTED_ACTIVE_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_MEMBER_SUPPORTED_ACTIVE_MEMBER_LIMIT", "8")
)
MEMBER_SUPPORTED_MIN_MULTIPLIER = float(
    os.environ.get("DARTLAB_HORIZON_V163_MEMBER_SUPPORTED_MIN_MULTIPLIER", str(MEMBER_BALANCE_MIN_MULTIPLIER))
)
...
MEMBER_ROLE_RESIDUAL_MASK_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_MEMBER_ROLE_RESIDUAL_MASK_SIGNATURE_LIMIT", "96")
)
MEMBER_ROLE_RESIDUAL_MASK_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_MEMBER_ROLE_RESIDUAL_MASK_ATOM_LIMIT", "64")
)
MEMBER_ROLE_RESIDUAL_MASK_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_MEMBER_ROLE_RESIDUAL_MASK_ROW_LIMIT", "220")
)
MEMBER_ROLE_RESIDUAL_LABEL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_MEMBER_ROLE_RESIDUAL_LABEL_LIMIT", "80"))
MEMBER_ROLE_RESIDUAL_MIN_MULTIPLIER = float(
    os.environ.get("DARTLAB_HORIZON_V163_MEMBER_ROLE_RESIDUAL_MIN_MULTIPLIER", "0.25")
)
MEMBER_ROLE_RESIDUAL_CONFLICT_RATIO = float(
    os.environ.get("DARTLAB_HORIZON_V163_MEMBER_ROLE_RESIDUAL_CONFLICT_RATIO", "1.35")
)
MEMBER_TARGET_SELECTOR_LABEL_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_MEMBER_TARGET_SELECTOR_LABEL_LIMIT", "48")
)
MEMBER_TARGET_SELECTOR_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_MEMBER_TARGET_SELECTOR_MEMBER_LIMIT", "14")
)
MEMBER_TARGET_SELECTOR_BALANCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_MEMBER_TARGET_SELECTOR_BALANCE_LIMIT", "96")
)
MEMBER_TARGET_SELECTOR_SURFACE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_MEMBER_TARGET_SELECTOR_SURFACE_LIMIT", str(MASKED_FRAME_PROBE_LIMIT))
)
SELECTED_TARGET_PROJECTION_SOURCE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_SELECTED_TARGET_PROJECTION_SOURCE_ATOM_LIMIT", "72")
)
SELECTED_TARGET_PROJECTION_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_SELECTED_TARGET_PROJECTION_ROW_LIMIT", "220")
)
SELECTED_TARGET_PROJECTION_TARGET_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_SELECTED_TARGET_PROJECTION_TARGET_LIMIT", "8")
)
SELECTED_TARGET_PROJECTION_SELECTOR_LABEL_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_SELECTED_TARGET_PROJECTION_SELECTOR_LABEL_LIMIT", "12")
)
SELECTED_TARGET_PROJECTION_SELECTOR_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_SELECTED_TARGET_PROJECTION_SELECTOR_MEMBER_LIMIT", "8")
)
SELECTOR_PATH_JOIN_SOURCE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_SELECTOR_PATH_JOIN_SOURCE_LIMIT", "12"))
SELECTOR_PATH_JOIN_PAIR_TARGET_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_SELECTOR_PATH_JOIN_PAIR_TARGET_LIMIT", "16")
)
SELECTOR_PATH_JOIN_SELECTED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_SELECTOR_PATH_JOIN_SELECTED_LIMIT", "8"))
SELECTOR_PATH_JOIN_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_SELECTOR_PATH_JOIN_ROW_LIMIT", "36"))
BIDIRECTIONAL_PAIR_CANDIDATE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_BIDIRECTIONAL_PAIR_CANDIDATE_LIMIT", "96")
)
BIDIRECTIONAL_PAIR_ROUTE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_BIDIRECTIONAL_PAIR_ROUTE_LIMIT", "12"))
BIDIRECTIONAL_PAIR_DYNAMIC_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_BIDIRECTIONAL_PAIR_DYNAMIC_LIMIT", "80"))
BIDIRECTIONAL_PAIR_REVERSE_RELATION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_BIDIRECTIONAL_PAIR_REVERSE_RELATION_LIMIT", "2")
)
PAIR_LOCAL_CONTRAST_CANDIDATE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_PAIR_LOCAL_CONTRAST_CANDIDATE_LIMIT", "96")
)
PAIR_LOCAL_CONTRAST_PEER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_PAIR_LOCAL_CONTRAST_PEER_LIMIT", "18"))
PAIR_LOCAL_CONTRAST_NEAR_BONUS = float(os.environ.get("DARTLAB_HORIZON_V163_PAIR_LOCAL_CONTRAST_NEAR_BONUS", "0.42"))
PAIR_LOCAL_CONTRAST_MIN_MARGIN = float(os.environ.get("DARTLAB_HORIZON_V163_PAIR_LOCAL_CONTRAST_MIN_MARGIN", "0.015"))
TARGET_MEMBER_FRAME_OCCURRENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_FRAME_OCCURRENCE_LIMIT", "56")
)
TARGET_MEMBER_FRAME_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_FRAME_ATOM_LIMIT", "120"))
TARGET_MEMBER_FRAME_PEER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_FRAME_PEER_LIMIT", "8"))
TARGET_MEMBER_FRAME_ROUTE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_FRAME_ROUTE_LIMIT", str(MASKED_FRAME_PROBE_LIMIT))
)
TARGET_MEMBER_FRAME_NEIGHBOR_RADIUS = int(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_FRAME_NEIGHBOR_RADIUS", "3")
)
TARGET_MEMBER_FRAME_NEIGHBOR_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_FRAME_NEIGHBOR_ATOM_LIMIT", "14")
)
TARGET_MEMBER_FRAME_RELATION_WINDOW = int(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_FRAME_RELATION_WINDOW", "8")
)
TARGET_MEMBER_FRAME_CANDIDATE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_FRAME_CANDIDATE_LIMIT", str(BIDIRECTIONAL_PAIR_CANDIDATE_LIMIT))
)
TARGET_MEMBER_FRAME_QUERY_ROUTE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_FRAME_QUERY_ROUTE_LIMIT", "24")
)
COMPOSITIONAL_FRAME_MIN_QUERY_ROWS = int(
    os.environ.get("DARTLAB_HORIZON_V163_COMPOSITIONAL_FRAME_MIN_QUERY_ROWS", "24")
)
COMPOSITIONAL_FRAME_FRAGMENT_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_COMPOSITIONAL_FRAME_FRAGMENT_LIMIT", "10")
)
COMPOSITIONAL_FRAME_FRAGMENT_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_COMPOSITIONAL_FRAME_FRAGMENT_ROW_LIMIT", "18")
)
COMPOSITIONAL_FRAME_MIN_FRAGMENT_LEN = int(
    os.environ.get("DARTLAB_HORIZON_V163_COMPOSITIONAL_FRAME_MIN_FRAGMENT_LEN", "2")
)
COMPOSITIONAL_FRAME_MAX_FRAGMENT_LEN = int(
    os.environ.get("DARTLAB_HORIZON_V163_COMPOSITIONAL_FRAME_MAX_FRAGMENT_LEN", "6")
)
COMPOSITIONAL_FRAME_ATOM_SCALE = float(os.environ.get("DARTLAB_HORIZON_V163_COMPOSITIONAL_FRAME_ATOM_SCALE", "0.58"))
COMPOSITIONAL_FRAME_SIGNATURE_SCALE = float(
    os.environ.get("DARTLAB_HORIZON_V163_COMPOSITIONAL_FRAME_SIGNATURE_SCALE", "0.24")
)
RELATION_SLOT_SUBSTITUTION_WINDOW = int(os.environ.get("DARTLAB_HORIZON_V163_RELATION_SLOT_SUBSTITUTION_WINDOW", "8"))
RELATION_SLOT_SUBSTITUTION_MIN_KEY_DF = int(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_SLOT_SUBSTITUTION_MIN_KEY_DF", "2")
)
RELATION_SLOT_SUBSTITUTION_MAX_KEY_DF = int(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_SLOT_SUBSTITUTION_MAX_KEY_DF", "260")
)
RELATION_SLOT_SUBSTITUTION_POSTING_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_SLOT_SUBSTITUTION_POSTING_LIMIT", "80")
)
RELATION_SLOT_SUBSTITUTION_SURFACE_KEY_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_SLOT_SUBSTITUTION_SURFACE_KEY_LIMIT", "36")
)
RELATION_SLOT_SUBSTITUTION_QUERY_KEY_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_SLOT_SUBSTITUTION_QUERY_KEY_LIMIT", "64")
)
RELATION_SLOT_SUBSTITUTION_FRAGMENT_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_SLOT_SUBSTITUTION_FRAGMENT_LIMIT", "8")
)
RELATION_SLOT_SUBSTITUTION_ROUTE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_SLOT_SUBSTITUTION_ROUTE_LIMIT", str(MASKED_FRAME_PROBE_LIMIT))
)
RELATION_SLOT_SUBSTITUTION_SEED_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_SLOT_SUBSTITUTION_SEED_WEIGHT", "1.15")
)
RELATION_SLOT_SUBSTITUTION_TYPED_SPECIFICITY_MIN = float(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_SLOT_SUBSTITUTION_TYPED_SPECIFICITY_MIN", "0.16")
)
RELATION_SLOT_SUBSTITUTION_TYPED_HUBNESS_MAX = float(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_SLOT_SUBSTITUTION_TYPED_HUBNESS_MAX", "0.66")
)
RELATION_SLOT_SUBSTITUTION_TYPED_ENTROPY_MAX = float(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_SLOT_SUBSTITUTION_TYPED_ENTROPY_MAX", "0.86")
)
RELATION_SLOT_SUBSTITUTION_TYPED_CONCEPT_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_SLOT_SUBSTITUTION_TYPED_CONCEPT_WEIGHT", "1.18")
)
RELATION_SLOT_SUBSTITUTION_TYPED_GENERIC_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_SLOT_SUBSTITUTION_TYPED_GENERIC_WEIGHT", "0.42")
)
RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_TARGET_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_TARGET_LIMIT", "10")
)
RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_WEAK_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_WEAK_WEIGHT", "0.52")
)
RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_GENERIC_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_GENERIC_WEIGHT", "0.24")
)
RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_TARGET_KEY_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_TARGET_KEY_WEIGHT", "0.32")
)
RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_MIN_PAIR_SCORE = float(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_MIN_PAIR_SCORE", "0.035")
)
RELATION_SLOT_SUBSTITUTION_LEDGER_OCCURRENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_SLOT_SUBSTITUTION_LEDGER_OCCURRENCE_LIMIT", "48")
)
RELATION_SLOT_SUBSTITUTION_LEDGER_TARGET_KEY_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_SLOT_SUBSTITUTION_LEDGER_TARGET_KEY_WEIGHT", "0.38")
)
RELATION_SLOT_SUBSTITUTION_LEDGER_BROAD_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V163_RELATION_SLOT_SUBSTITUTION_LEDGER_BROAD_WEIGHT", "0.34")
)
PAIR_LOCAL_LEDGER_FRAGMENT_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_PAIR_LOCAL_LEDGER_FRAGMENT_LIMIT", "10"))
PAIR_LOCAL_LEDGER_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_PAIR_LOCAL_LEDGER_ATOM_LIMIT", "48"))
PAIR_LOCAL_LEDGER_BAND_SIZE = int(os.environ.get("DARTLAB_HORIZON_V163_PAIR_LOCAL_LEDGER_BAND_SIZE", "3"))
PAIR_LOCAL_LEDGER_BAND_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_PAIR_LOCAL_LEDGER_BAND_LIMIT", "12"))
PAIR_LOCAL_LEDGER_GAIN = float(os.environ.get("DARTLAB_HORIZON_V163_PAIR_LOCAL_LEDGER_GAIN", "1.45"))
PAIR_LOCAL_LEDGER_MARGIN_GAIN = float(os.environ.get("DARTLAB_HORIZON_V163_PAIR_LOCAL_LEDGER_MARGIN_GAIN", "2.20"))
PAIR_LOCAL_LEDGER_NEGATIVE_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V163_PAIR_LOCAL_LEDGER_NEGATIVE_FACTOR", "0.74")
)
PAIR_LOCAL_LEDGER_EMPTY_FACTOR = float(os.environ.get("DARTLAB_HORIZON_V163_PAIR_LOCAL_LEDGER_EMPTY_FACTOR", "0.88"))
PAIR_LOCAL_LEDGER_MIN_MARGIN = float(os.environ.get("DARTLAB_HORIZON_V163_PAIR_LOCAL_LEDGER_MIN_MARGIN", "0.018"))
PAIR_LOCAL_LEDGER_RESIDUAL_COMMON_POWER = float(
    os.environ.get("DARTLAB_HORIZON_V163_PAIR_LOCAL_LEDGER_RESIDUAL_COMMON_POWER", "0.72")
)
PAIR_LOCAL_LEDGER_RESIDUAL_IDF_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V163_PAIR_LOCAL_LEDGER_RESIDUAL_IDF_GAIN", "0.58")
)
PAIR_LOCAL_LEDGER_RESIDUAL_MIN_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V163_PAIR_LOCAL_LEDGER_RESIDUAL_MIN_WEIGHT", "0.018")
)
PAIR_LOCAL_LEDGER_RESIDUAL_PEER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_PAIR_LOCAL_LEDGER_RESIDUAL_PEER_LIMIT", "8")
)
DIRECTIONAL_LEDGER_FRAGMENT_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_DIRECTIONAL_LEDGER_FRAGMENT_LIMIT", "8"))
DIRECTIONAL_LEDGER_SOURCE_TARGET_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_DIRECTIONAL_LEDGER_SOURCE_TARGET_LIMIT", "10")
)
DIRECTIONAL_LEDGER_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_DIRECTIONAL_LEDGER_ATOM_LIMIT", "64"))
DIRECTIONAL_LEDGER_GAIN = float(os.environ.get("DARTLAB_HORIZON_V163_DIRECTIONAL_LEDGER_GAIN", "1.72"))
DIRECTIONAL_LEDGER_MARGIN_GAIN = float(os.environ.get("DARTLAB_HORIZON_V163_DIRECTIONAL_LEDGER_MARGIN_GAIN", "2.45"))
DIRECTIONAL_LEDGER_NEGATIVE_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V163_DIRECTIONAL_LEDGER_NEGATIVE_FACTOR", "0.72")
)
DIRECTIONAL_LEDGER_EMPTY_FACTOR = float(os.environ.get("DARTLAB_HORIZON_V163_DIRECTIONAL_LEDGER_EMPTY_FACTOR", "0.90"))
DIRECTIONAL_LEDGER_MIN_MARGIN = float(os.environ.get("DARTLAB_HORIZON_V163_DIRECTIONAL_LEDGER_MIN_MARGIN", "0.014"))
TARGET_MEMBER_PRIOR_SOURCE_TARGET_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_PRIOR_SOURCE_TARGET_LIMIT", "16")
)
TARGET_MEMBER_PRIOR_HARD_NEGATIVE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_PRIOR_HARD_NEGATIVE_LIMIT", "14")
)
TARGET_MEMBER_PRIOR_GAIN = float(os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_PRIOR_GAIN", "2.35"))
TARGET_MEMBER_PRIOR_MARGIN_GAIN = float(os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_PRIOR_MARGIN_GAIN", "2.10"))
TARGET_MEMBER_PRIOR_NEGATIVE_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_PRIOR_NEGATIVE_FACTOR", "0.68")
)
TARGET_MEMBER_PRIOR_EMPTY_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_PRIOR_EMPTY_FACTOR", "0.92")
)
TARGET_MEMBER_PRIOR_MIN_MARGIN = float(os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_PRIOR_MIN_MARGIN", "0.012"))
TARGET_MEMBER_TRANSPORT_GAIN = float(os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_TRANSPORT_GAIN", "3.20"))
TARGET_MEMBER_TRANSPORT_ANCHOR_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_TRANSPORT_ANCHOR_GAIN", "1.35")
)
TARGET_MEMBER_TRANSPORT_ROLE_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_TRANSPORT_ROLE_GAIN", "0.86")
)
TARGET_MEMBER_TRANSPORT_EMPTY_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_TRANSPORT_EMPTY_FACTOR", "0.94")
)
TARGET_MEMBER_TRANSPORT_LOW_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_TRANSPORT_LOW_FACTOR", "0.78")
)
TARGET_MEMBER_TRANSPORT_MIN_LIKELIHOOD = float(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_TRANSPORT_MIN_LIKELIHOOD", "0.045")
)
TARGET_MEMBER_CONDITIONAL_TRANSPORT_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_CONDITIONAL_TRANSPORT_GAIN", "2.35")
)
TARGET_MEMBER_CONDITIONAL_TRANSPORT_RESIDUAL_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_CONDITIONAL_TRANSPORT_RESIDUAL_GAIN", "1.15")
)
TARGET_MEMBER_CONDITIONAL_ANCHOR_RESIDUAL_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_CONDITIONAL_ANCHOR_RESIDUAL_GAIN", "0.72")
)
TARGET_MEMBER_CONDITIONAL_TRANSPORT_EMPTY_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_CONDITIONAL_TRANSPORT_EMPTY_FACTOR", "0.92")
)
TARGET_MEMBER_CONDITIONAL_TRANSPORT_LOW_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_CONDITIONAL_TRANSPORT_LOW_FACTOR", "0.82")
)
TARGET_MEMBER_CONDITIONAL_TRANSPORT_PEER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_CONDITIONAL_TRANSPORT_PEER_LIMIT", "10")
)
TARGET_MEMBER_EXPERIENCE_FAMILY_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_EXPERIENCE_FAMILY_MEMBER_LIMIT", "8")
)
TARGET_MEMBER_EXPERIENCE_FAMILY_SCORE_FLOOR = float(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_EXPERIENCE_FAMILY_SCORE_FLOOR", "0.045")
)
TARGET_MEMBER_FIELD_FAMILY_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_FIELD_FAMILY_MEMBER_LIMIT", "14")
)
TARGET_MEMBER_FIELD_FAMILY_EXPANSION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_FIELD_FAMILY_EXPANSION_LIMIT", "6")
)
TARGET_MEMBER_FIELD_FAMILY_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_FIELD_FAMILY_ATOM_LIMIT", "72")
)
TARGET_MEMBER_FIELD_FAMILY_SOURCE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_FIELD_FAMILY_SOURCE_ATOM_LIMIT", "96")
)
TARGET_MEMBER_FIELD_FAMILY_SIM_MIN = float(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_FIELD_FAMILY_SIM_MIN", "0.215")
)
TARGET_MEMBER_FIELD_FAMILY_COMMON_RATIO = float(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_FIELD_FAMILY_COMMON_RATIO", "0.46")
)
TARGET_MEMBER_FIELD_FAMILY_COMMON_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_FIELD_FAMILY_COMMON_FACTOR", "0.78")
)
TARGET_MEMBER_FIELD_FAMILY_RESIDUAL_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_FIELD_FAMILY_RESIDUAL_GAIN", "1.35")
)
TARGET_MEMBER_FIELD_FAMILY_FULL_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_FIELD_FAMILY_FULL_GAIN", "0.34")
)
TARGET_MEMBER_FIELD_FAMILY_MUTUAL_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_FIELD_FAMILY_MUTUAL_GAIN", "0.22")
)
TARGET_MEMBER_MUTUAL_PROOF_SOURCE_PATH_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_MUTUAL_PROOF_SOURCE_PATH_LIMIT", "18")
)
TARGET_MEMBER_MUTUAL_PROOF_REVERSE_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_MUTUAL_PROOF_REVERSE_ROW_LIMIT", "48")
)
TARGET_MEMBER_MUTUAL_PROOF_FAMILY_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_MUTUAL_PROOF_FAMILY_MEMBER_LIMIT", "8")
)
TARGET_MEMBER_MUTUAL_PROOF_PEER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_MUTUAL_PROOF_PEER_LIMIT", "8")
)
TARGET_MEMBER_MUTUAL_PROOF_SLOT_KEY_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_MUTUAL_PROOF_SLOT_KEY_LIMIT", "56")
)
TARGET_MEMBER_MUTUAL_PROOF_SLOT_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_MUTUAL_PROOF_SLOT_ROW_LIMIT", "96")
)
TARGET_MEMBER_MUTUAL_PROOF_SLOT_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_MUTUAL_PROOF_SLOT_MEMBER_LIMIT", "6")
)
TARGET_MEMBER_MUTUAL_PROOF_SLOT_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_MUTUAL_PROOF_SLOT_WEIGHT", "0.74")
)
TARGET_MEMBER_MUTUAL_PROOF_GAIN = float(os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_MUTUAL_PROOF_GAIN", "2.60"))
TARGET_MEMBER_MUTUAL_PROOF_MARGIN_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_MUTUAL_PROOF_MARGIN_GAIN", "1.85")
)
TARGET_MEMBER_MUTUAL_PROOF_NEGATIVE_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_MUTUAL_PROOF_NEGATIVE_FACTOR", "0.72")
)
TARGET_MEMBER_MUTUAL_PROOF_EMPTY_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_MUTUAL_PROOF_EMPTY_FACTOR", "0.94")
)
TARGET_MEMBER_MUTUAL_PROOF_MIN_MARGIN = float(
    os.environ.get("DARTLAB_HORIZON_V163_TARGET_MEMBER_MUTUAL_PROOF_MIN_MARGIN", "0.010")
)
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V163_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V163_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V163_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V163_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V163_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V163_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V163_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V163_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
LANE_MISMATCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V163_LANE_MISMATCH_PENALTY", "0.18"))
LANE_ARTIFACT_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V163_LANE_ARTIFACT_PENALTY", "0.10"))
NEAREST_ORDER_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_NEAREST_ORDER_SIGNATURE_LIMIT", "24"))
NEAREST_ORDER_PENALTY_MIN = float(os.environ.get("DARTLAB_HORIZON_V163_NEAREST_ORDER_PENALTY_MIN", "0.05"))
NEAREST_ORDER_PENALTY_SCALE = float(os.environ.get("DARTLAB_HORIZON_V163_NEAREST_ORDER_PENALTY_SCALE", "0.16"))
NEAREST_ORDER_COHORT_SURFACE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_NEAREST_ORDER_COHORT_SURFACE_LIMIT", "384")
)
NEAREST_ORDER_COHORT_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_NEAREST_ORDER_COHORT_POSITION_LIMIT", "8192")
)
NEAREST_ORDER_SURFACE_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V163_NEAREST_ORDER_SURFACE_POSITION_LIMIT", "48")
)
SUFFIX_ANCHOR_SUPPORT_MIN = int(os.environ.get("DARTLAB_HORIZON_V163_SUFFIX_ANCHOR_SUPPORT_MIN", "1"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V163_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V163_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V163_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V163_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V163_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V163_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V163_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V163_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V163_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V163_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V163_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V163_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V163_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V163_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V163_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V163_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V163_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V163_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V163_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V163_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V163_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V163_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V163_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
RELAY_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_RELAY_ROW_LIMIT", "160"))
RELAY_SPECIFIC_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_RELAY_SPECIFIC_ROW_LIMIT", "320"))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V163_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V163_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V163_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V163_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V163_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V163_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V163_SKETCH_OCC_BUCKETS", "16"))
