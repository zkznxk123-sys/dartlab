"""Horizon Meaning Learner V168 - path-local event trace transducer.

V168 path-local trace 기록
--------------
아이디어:
    V167 은 target field df/IDF 로 broad event grammar 를 낮췄지만, event field 를 하나의 전역 bag 으로
    펼쳐 비교했다. 그 방식은 "경험이 겹친다" 는 사실은 보지만, 어떤 owner/table/statement/window 경로에서
    source 가 target 으로 변환됐는지의 로컬 trace 를 잃는다.

    V168 은 relation extraction/build 단계에서 occurrence 단위 path-local trace 를 별도로 저장한다.
    trace 는 relation/polarity, sourceRole/targetRole, relation 기준 좌우 side, direction, distance bucket,
    lane pair, profile/ledger/owner-shape bucket, 순서 atom 을 묶는다. source outgoing trace 와 target incoming
    trace 는 같은 namespace 를 쓰되, 후보 비교는 전역 field overlap 이 아니라 trace row 쌍의 IDF weighted
    overlap/top-pair aggregate 로 계산한다.

    수평선 좌표는 의미가 아니라 fixed lookup address 이다. 의미는 그 주소가 누적한 경험 trace 이고, 이번 실험은
    같은 stem/fragment 표면이 아니라 relation path 안에서 겪은 trace 의 순서/역할/거리 경험이 비교 가능한
    매개체가 될 수 있는지를 본다.

실행 코드:
    $env:DARTLAB_HORIZON_V168_MAX_RECORDS_PER_SOURCE='120'
    $env:DARTLAB_HORIZON_V168_MAX_UNITS='1200'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV168Test.py
...
결과:
    stderr 없이 완료. totalSeconds=1362.4.
    relationPathEventBundles=76390, SourceKeys=5645, TargetKeys=240,
    TargetAtomDfKeys=6784 로 V167 event recall 을 유지했다.
    새 path-local trace 는 relationPathLocalTraces=76390, SourceKeys=5645, TargetKeys=240,
    TargetTraceRows=9495, AtomDfKeys=2097 로 생성됐다.

    targetMemberPathLocal route 는 Top1=1/4, Top5=1/4, BadTop1=0/7, BadTop5=0/7,
    queryTraceRows=189, scored=706, routeRows=706, matchedAtoms=23297,
    sequenceMatchedAtoms=10318, rareMatchedAtoms=1252, tracePairs=111160,
    acceptedTracePairs=4074, targetTraceRows=16936, proofMilli=125124 이었다.
    V167 path event route 의 proofMilli=66736/sequenceMatchedAtoms=4139 보다 trace 신호는 크게 늘었다.

    그러나 positive direct route 는 여전히 `현금성자산 -> 현금및현금성자산` 만 rank=1 이고,
    `외상매출금 -> 매출채권`, `영업손익 -> 영업이익`, `손실충당금 -> 대손충당금` 은 rank=NA 였다.

    targetMemberFrame 은 Top1=1/4, Top5=1/4, BadTop1=0/7, BadTop5=0/7,
    candidateHits=4/11, candidateRows=1056, scored=1031 로 V167 과 같은 수준이다.
    frame 내부 path-local trace 는 pairScored=1031, queryTraceRows=17853,
    targetTraceRows=36080, matchedAtoms=46913, sequenceMatchedAtoms=20708,
    rareMatchedAtoms=2541, tracePairs=224840, acceptedTracePairs=8135,
    proofMilli=254123, acceptedRows=706 으로 실제 sequence 경험을 많이 만들었다.

    그러나 frame rank 는 개선되지 않았다. `외상매출금 -> 매출채권` 은 candidateHit=True 이지만
    rank=NA/top=`매출`, `영업손익 -> 영업이익` 은 candidateHit=False/rank=NA,
    `손실충당금 -> 대손충당금` 도 candidateHit=False/rank=NA/top=`손실충당` 이었다.
    `현금성자산 -> 현금및현금성자산` 만 rank=1 을 유지했다.
    fixed guard 는 positiveHits=3/4, badAccepted=0/7, searchTop1=4/5, reliableSearch=4/5 를 유지했다.
    rare semantic transfer 는 V147 의 `외상매출금 -> 매출채권 rank=7` 을 넘지 못했다.

결론:
    실패/개념 진단 성공. path-local trace 는 "경험" 을 비교 가능한 매개체로 만들 수 있다는 신호를 냈다.
    특히 sequenceMatchedAtoms 가 direct 10318, frame 20708 로 커져서 V165 의 sequenceMatchedAtoms=0 병목은
    완전히 해소됐다. 따라서 build-time trace 저장과 trace-pair proof 는 keep 한다.

    하지만 trace proof 를 direct route 또는 frame multiplier 로 더하는 것만으로는 의미 전이가 되지 않는다.
    병목은 trace 생성이 아니라 target 정렬/candidate hit/peer suppression 이다. 정답 target 이 후보에 없거나
    같은 ledger/role peer(`매출`, `손실충당`, `신용손실충당금`, `영업이익률`) 에 proof mass 가 먼저 흡수된다.

    다음은 trace overlap 을 더 키우는 얕은 실험이 아니라, path-local trace 를 같은 ledger/role sibling anti-cohort 에서
    residualize 하고, source trace 가 candidate target member 로 "변환" 되는지를 target family/role slot 내부의
    경쟁으로 판정해야 한다. 즉 trace 는 evidence 이고, 필요한 본체는 trace-conditioned target alignment/selector 이다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V168_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V168_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V168_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V168_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V168_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V168_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V168_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V168_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V168_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_SEARCH_CANDIDATE_LIMIT", "420"))
DYNAMIC_TARGET_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_DYNAMIC_TARGET_LIMIT", "80"))
DYNAMIC_COORD_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_DYNAMIC_COORD_ROW_LIMIT", "220"))
DYNAMIC_COMPOUND_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_DYNAMIC_COMPOUND_ROW_LIMIT", "260"))
DYNAMIC_QUERY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_DYNAMIC_QUERY_ATOM_LIMIT", "48"))
DYNAMIC_MEANING_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_DYNAMIC_MEANING_ATOM_LIMIT", "36"))
DYNAMIC_MEANING_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_DYNAMIC_MEANING_ROW_LIMIT", "220"))
DYNAMIC_RELATION_SURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_DYNAMIC_RELATION_SURFACE_LIMIT", "420"))
DYNAMIC_RELATION_UNIT_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_DYNAMIC_RELATION_UNIT_LIMIT", "160"))
DYNAMIC_BRIDGE_ONLY_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V168_DYNAMIC_BRIDGE_ONLY_PENALTY", "0.80"))
DYNAMIC_OWNER_ROLE_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_DYNAMIC_OWNER_ROLE_SIGNATURE_LIMIT", "64")
)
DYNAMIC_OWNER_ROLE_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_DYNAMIC_OWNER_ROLE_ATOM_LIMIT", "40"))
DYNAMIC_OWNER_ROLE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_DYNAMIC_OWNER_ROLE_ROW_LIMIT", "220"))
DYNAMIC_OWNER_ROLE_UNIT_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_DYNAMIC_OWNER_ROLE_UNIT_ATOM_LIMIT", "36")
)
DYNAMIC_OWNER_ROLE_MIN_BOUND = float(os.environ.get("DARTLAB_HORIZON_V168_DYNAMIC_OWNER_ROLE_MIN_BOUND", "0.55"))
DYNAMIC_OWNER_ROLE_CANDIDATE_BONUS = float(
    os.environ.get("DARTLAB_HORIZON_V168_DYNAMIC_OWNER_ROLE_CANDIDATE_BONUS", "2.40")
)
DYNAMIC_OWNER_ROLE_ROUTE_BONUS = float(os.environ.get("DARTLAB_HORIZON_V168_DYNAMIC_OWNER_ROLE_ROUTE_BONUS", "0.90"))
DYNAMIC_OWNER_ROLE_WEAK_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V168_DYNAMIC_OWNER_ROLE_WEAK_PENALTY", "0.16"))
DYNAMIC_OWNER_FRAME_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V168_DYNAMIC_OWNER_FRAME_RADIUS", "5"))
DYNAMIC_OWNER_FRAME_BETWEEN_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_DYNAMIC_OWNER_FRAME_BETWEEN_LIMIT", "7"))
MASKED_FRAME_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_MASKED_FRAME_SIGNATURE_LIMIT", "72"))
MASKED_FRAME_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_MASKED_FRAME_ATOM_LIMIT", "48"))
MASKED_FRAME_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_MASKED_FRAME_ROW_LIMIT", "240"))
MASKED_FRAME_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_MASKED_FRAME_EVAL_LIMIT", "800"))
MASKED_FRAME_HOLDOUT_MOD = int(os.environ.get("DARTLAB_HORIZON_V168_MASKED_FRAME_HOLDOUT_MOD", "5"))
MASKED_FRAME_PROBE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_MASKED_FRAME_PROBE_LIMIT", "12"))
MASKED_CONTEXT_SURFACE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_MASKED_CONTEXT_SURFACE_ROW_LIMIT", "12"))
MASKED_CONTEXT_WEIGHT = float(os.environ.get("DARTLAB_HORIZON_V168_MASKED_CONTEXT_WEIGHT", "0.35"))
MASKED_CONTEXT_RELATION_WINDOW = int(os.environ.get("DARTLAB_HORIZON_V168_MASKED_CONTEXT_RELATION_WINDOW", "140"))
MASKED_FRAME_SURPRISAL_POWER = float(os.environ.get("DARTLAB_HORIZON_V168_MASKED_FRAME_SURPRISAL_POWER", "2.0"))
MASKED_FRAME_SURFACE_PRIOR_POWER = float(
    os.environ.get("DARTLAB_HORIZON_V168_MASKED_FRAME_SURFACE_PRIOR_POWER", "0.45")
)
RELATION_OWNER_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_RELATION_OWNER_SIGNATURE_LIMIT", "64"))
RELATION_OWNER_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_RELATION_OWNER_ROW_LIMIT", "220"))
RELATION_OWNER_BRIDGE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_RELATION_OWNER_BRIDGE_LIMIT", "12"))
RELATION_OWNER_PROJECTION_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_RELATION_OWNER_PROJECTION_LIMIT", "24"))
RELATION_OWNER_PROJECTION_OWNER_ROLE_MIN = float(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_OWNER_PROJECTION_OWNER_ROLE_MIN", "0.18")
)
RELATION_OWNER_PROJECTION_QUERY_ROLE_MIN = float(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_OWNER_PROJECTION_QUERY_ROLE_MIN", "0.05")
)
RELATION_OWNER_GENERATED_BRIDGE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_OWNER_GENERATED_BRIDGE_LIMIT", "48")
)
RELATION_OWNER_GENERATED_BRIDGE_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_OWNER_GENERATED_BRIDGE_WEIGHT", "2.4")
)
RELATION_OWNER_ROLE_COMPAT_MIN = float(os.environ.get("DARTLAB_HORIZON_V168_RELATION_OWNER_ROLE_COMPAT_MIN", "0.16"))
RELATION_BOUND_ROLE_PAIR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_RELATION_BOUND_ROLE_PAIR_LIMIT", "24"))
RELATION_BOUND_ROLE_PAIR_BRIDGE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_BOUND_ROLE_PAIR_BRIDGE_LIMIT", "10")
)
RELATION_BOUND_ROLE_PAIR_BRIDGE_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_BOUND_ROLE_PAIR_BRIDGE_WEIGHT", "0.46")
)
RELATION_SOURCE_EXPERIENCE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_SOURCE_EXPERIENCE_ATOM_LIMIT", "44")
)
RELATION_SOURCE_EXPERIENCE_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_SOURCE_EXPERIENCE_ROW_LIMIT", "180")
)
RELATION_SOURCE_SHINGLE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_SOURCE_SHINGLE_ATOM_LIMIT", "28")
)
RELATION_SOURCE_SHINGLE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_RELATION_SOURCE_SHINGLE_ROW_LIMIT", "160"))
RELATION_SOURCE_SHINGLE_KEY_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_RELATION_SOURCE_SHINGLE_KEY_LIMIT", "56"))
RELATION_ANCHORED_COARSE_SHINGLE_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_ANCHORED_COARSE_SHINGLE_ROW_LIMIT", "180")
)
RELATION_ANCHORED_COARSE_SHINGLE_KEY_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_ANCHORED_COARSE_SHINGLE_KEY_LIMIT", "72")
)
HORIZON_MASK_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_HORIZON_MASK_SIGNATURE_LIMIT", "96"))
HORIZON_MASK_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_HORIZON_MASK_ATOM_LIMIT", "56"))
HORIZON_MASK_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_HORIZON_MASK_ROW_LIMIT", "220"))
HORIZON_MASK_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_HORIZON_MASK_EVAL_LIMIT", "900"))
HORIZON_MASK_HOLDOUT_MOD = int(os.environ.get("DARTLAB_HORIZON_V168_HORIZON_MASK_HOLDOUT_MOD", "7"))
HORIZON_MASK_SURFACE_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_HORIZON_MASK_SURFACE_ROW_LIMIT", "24"))
HORIZON_MASK_NEIGHBOR_EXPERIENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_HORIZON_MASK_NEIGHBOR_EXPERIENCE_LIMIT", "5")
)
HORIZON_MASK_COORD_CELL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_HORIZON_MASK_COORD_CELL_LIMIT", "5"))
HORIZON_MASK_COORD_BUCKET_DIGITS = int(os.environ.get("DARTLAB_HORIZON_V168_HORIZON_MASK_COORD_BUCKET_DIGITS", "5"))
LOCAL_FRAME_PREDICTION_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_LOCAL_FRAME_PREDICTION_SIGNATURE_LIMIT", "132")
)
LOCAL_FRAME_PREDICTION_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_LOCAL_FRAME_PREDICTION_ATOM_LIMIT", "76"))
LOCAL_FRAME_PREDICTION_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_LOCAL_FRAME_PREDICTION_ROW_LIMIT", "260"))
LOCAL_FRAME_PREDICTION_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_LOCAL_FRAME_PREDICTION_EVAL_LIMIT", "900"))
LOCAL_FRAME_PREDICTION_SURFACE_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_LOCAL_FRAME_PREDICTION_SURFACE_ROW_LIMIT", "18")
)
LOCAL_FRAME_PREDICTION_HOLDOUT_MOD = int(os.environ.get("DARTLAB_HORIZON_V168_LOCAL_FRAME_PREDICTION_HOLDOUT_MOD", "9"))
LOCAL_FRAME_PREDICTION_NEIGHBOR_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_LOCAL_FRAME_PREDICTION_NEIGHBOR_LIMIT", "5")
)
LOCAL_FRAME_PREDICTION_NEIGHBOR_EXPERIENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_LOCAL_FRAME_PREDICTION_NEIGHBOR_EXPERIENCE_LIMIT", "5")
)
LOCAL_FRAME_PREDICTION_SEQUENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_LOCAL_FRAME_PREDICTION_SEQUENCE_LIMIT", "5")
)
QUERY_LOCAL_FRAME_OCCURRENCE_INDEX_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_QUERY_LOCAL_FRAME_OCCURRENCE_INDEX_ROW_LIMIT", "96")
)
QUERY_LOCAL_FRAME_OCCURRENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_QUERY_LOCAL_FRAME_OCCURRENCE_LIMIT", "72")
)
QUERY_LOCAL_FRAME_PROXY_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_QUERY_LOCAL_FRAME_PROXY_LIMIT", "8"))
QUERY_LOCAL_FRAME_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_QUERY_LOCAL_FRAME_ATOM_LIMIT", "132"))
QUERY_LOCAL_FRAME_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_QUERY_LOCAL_FRAME_EVAL_LIMIT", "260"))
QUERY_SEMANTIC_UNIT_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_QUERY_SEMANTIC_UNIT_ATOM_LIMIT", "120"))
QUERY_SEMANTIC_UNIT_EVAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_QUERY_SEMANTIC_UNIT_EVAL_LIMIT", "260"))
QUERY_SEMANTIC_UNIT_SURFACE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_QUERY_SEMANTIC_UNIT_SURFACE_LIMIT", str(MASKED_FRAME_PROBE_LIMIT))
)
SEMANTIC_UNIT_SIGNATURE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_SEMANTIC_UNIT_SIGNATURE_ATOM_LIMIT", "18")
)
SEMANTIC_UNIT_POSTING_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_SEMANTIC_UNIT_POSTING_ROW_LIMIT", "90"))
SEMANTIC_UNIT_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_SEMANTIC_UNIT_CANDIDATE_LIMIT", "180"))
SEMANTIC_UNIT_MIN_EXPERIENCE_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V168_SEMANTIC_UNIT_MIN_EXPERIENCE_SIM", "0.46")
)
SEMANTIC_UNIT_MIN_MIXED_SIM = float(os.environ.get("DARTLAB_HORIZON_V168_SEMANTIC_UNIT_MIN_MIXED_SIM", "0.34"))
SEMANTIC_UNIT_MIN_COORD_RESONANCE = float(
    os.environ.get("DARTLAB_HORIZON_V168_SEMANTIC_UNIT_MIN_COORD_RESONANCE", "0.045")
)
SEMANTIC_UNIT_MASK_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_SEMANTIC_UNIT_MASK_SIGNATURE_LIMIT", "120")
)
SEMANTIC_UNIT_MASK_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_SEMANTIC_UNIT_MASK_ATOM_LIMIT", "64"))
SEMANTIC_UNIT_MASK_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_SEMANTIC_UNIT_MASK_ROW_LIMIT", "260"))
SEMANTIC_UNIT_ROUTE_MEMBER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_SEMANTIC_UNIT_ROUTE_MEMBER_LIMIT", "4"))
CONTRAST_SEMANTIC_UNIT_MIN_EXPERIENCE_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V168_CONTRAST_SEMANTIC_UNIT_MIN_EXPERIENCE_SIM", "0.46")
)
CONTRAST_SEMANTIC_UNIT_STRONG_EXPERIENCE_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V168_CONTRAST_SEMANTIC_UNIT_STRONG_EXPERIENCE_SIM", "0.62")
)
CONTRAST_SEMANTIC_UNIT_MIN_MIXED_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V168_CONTRAST_SEMANTIC_UNIT_MIN_MIXED_SIM", "0.36")
)
CONTRAST_SEMANTIC_UNIT_MIN_COORD_RESONANCE = float(
    os.environ.get("DARTLAB_HORIZON_V168_CONTRAST_SEMANTIC_UNIT_MIN_COORD_RESONANCE", "0.050")
)
CONTRAST_SEMANTIC_UNIT_MIN_RELATION_COMPAT = float(
    os.environ.get("DARTLAB_HORIZON_V168_CONTRAST_SEMANTIC_UNIT_MIN_RELATION_COMPAT", "0.28")
)
CONTRAST_SEMANTIC_UNIT_SUFFIX_RELATION_COMPAT = float(
    os.environ.get("DARTLAB_HORIZON_V168_CONTRAST_SEMANTIC_UNIT_SUFFIX_RELATION_COMPAT", "0.18")
)
MEMBER_BALANCE_MEMBER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_MEMBER_BALANCE_MEMBER_LIMIT", "10"))
MEMBER_BALANCE_MIN_MULTIPLIER = float(os.environ.get("DARTLAB_HORIZON_V168_MEMBER_BALANCE_MIN_MULTIPLIER", "0.015"))
MEMBER_BALANCE_UNSUPPORTED_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V168_MEMBER_BALANCE_UNSUPPORTED_FACTOR", "0.12")
)
MEMBER_BALANCE_POSITIVE_GAIN = float(os.environ.get("DARTLAB_HORIZON_V168_MEMBER_BALANCE_POSITIVE_GAIN", "1.10"))
MEMBER_BALANCE_NEGATIVE_GAIN = float(os.environ.get("DARTLAB_HORIZON_V168_MEMBER_BALANCE_NEGATIVE_GAIN", "1.60"))
MEMBER_BALANCE_POSITIVE_SHIELD_RATIO = float(
    os.environ.get("DARTLAB_HORIZON_V168_MEMBER_BALANCE_POSITIVE_SHIELD_RATIO", "0.85")
)
...
MEMBER_CONDITIONED_MASK_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_MEMBER_CONDITIONED_MASK_SIGNATURE_LIMIT", "96")
)
MEMBER_CONDITIONED_MASK_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_MEMBER_CONDITIONED_MASK_ATOM_LIMIT", "64")
)
MEMBER_CONDITIONED_MASK_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_MEMBER_CONDITIONED_MASK_ROW_LIMIT", "220"))
MEMBER_CONDITIONED_ROUTE_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_MEMBER_CONDITIONED_ROUTE_MEMBER_LIMIT", "180")
)
MEMBER_CONDITIONED_LABEL_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_MEMBER_CONDITIONED_LABEL_MEMBER_LIMIT", "6")
)
MEMBER_CONDITIONED_MEMBER_CACHE: dict[tuple[int, str, str], tuple[float, float, float, int]] = {}
MEMBER_SUPPORTED_LABEL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_MEMBER_SUPPORTED_LABEL_LIMIT", "80"))
MEMBER_SUPPORTED_MEMBER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_MEMBER_SUPPORTED_MEMBER_LIMIT", "120"))
MEMBER_SUPPORTED_ACTIVE_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_MEMBER_SUPPORTED_ACTIVE_MEMBER_LIMIT", "8")
)
MEMBER_SUPPORTED_MIN_MULTIPLIER = float(
    os.environ.get("DARTLAB_HORIZON_V168_MEMBER_SUPPORTED_MIN_MULTIPLIER", str(MEMBER_BALANCE_MIN_MULTIPLIER))
)
...
MEMBER_ROLE_RESIDUAL_MASK_SIGNATURE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_MEMBER_ROLE_RESIDUAL_MASK_SIGNATURE_LIMIT", "96")
)
MEMBER_ROLE_RESIDUAL_MASK_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_MEMBER_ROLE_RESIDUAL_MASK_ATOM_LIMIT", "64")
)
MEMBER_ROLE_RESIDUAL_MASK_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_MEMBER_ROLE_RESIDUAL_MASK_ROW_LIMIT", "220")
)
MEMBER_ROLE_RESIDUAL_LABEL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_MEMBER_ROLE_RESIDUAL_LABEL_LIMIT", "80"))
MEMBER_ROLE_RESIDUAL_MIN_MULTIPLIER = float(
    os.environ.get("DARTLAB_HORIZON_V168_MEMBER_ROLE_RESIDUAL_MIN_MULTIPLIER", "0.25")
)
MEMBER_ROLE_RESIDUAL_CONFLICT_RATIO = float(
    os.environ.get("DARTLAB_HORIZON_V168_MEMBER_ROLE_RESIDUAL_CONFLICT_RATIO", "1.35")
)
MEMBER_TARGET_SELECTOR_LABEL_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_MEMBER_TARGET_SELECTOR_LABEL_LIMIT", "48")
)
MEMBER_TARGET_SELECTOR_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_MEMBER_TARGET_SELECTOR_MEMBER_LIMIT", "14")
)
MEMBER_TARGET_SELECTOR_BALANCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_MEMBER_TARGET_SELECTOR_BALANCE_LIMIT", "96")
)
MEMBER_TARGET_SELECTOR_SURFACE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_MEMBER_TARGET_SELECTOR_SURFACE_LIMIT", str(MASKED_FRAME_PROBE_LIMIT))
)
SELECTED_TARGET_PROJECTION_SOURCE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_SELECTED_TARGET_PROJECTION_SOURCE_ATOM_LIMIT", "72")
)
SELECTED_TARGET_PROJECTION_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_SELECTED_TARGET_PROJECTION_ROW_LIMIT", "220")
)
SELECTED_TARGET_PROJECTION_TARGET_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_SELECTED_TARGET_PROJECTION_TARGET_LIMIT", "8")
)
SELECTED_TARGET_PROJECTION_SELECTOR_LABEL_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_SELECTED_TARGET_PROJECTION_SELECTOR_LABEL_LIMIT", "12")
)
SELECTED_TARGET_PROJECTION_SELECTOR_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_SELECTED_TARGET_PROJECTION_SELECTOR_MEMBER_LIMIT", "8")
)
SELECTOR_PATH_JOIN_SOURCE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_SELECTOR_PATH_JOIN_SOURCE_LIMIT", "12"))
SELECTOR_PATH_JOIN_PAIR_TARGET_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_SELECTOR_PATH_JOIN_PAIR_TARGET_LIMIT", "16")
)
SELECTOR_PATH_JOIN_SELECTED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_SELECTOR_PATH_JOIN_SELECTED_LIMIT", "8"))
SELECTOR_PATH_JOIN_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_SELECTOR_PATH_JOIN_ROW_LIMIT", "36"))
BIDIRECTIONAL_PAIR_CANDIDATE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_BIDIRECTIONAL_PAIR_CANDIDATE_LIMIT", "96")
)
BIDIRECTIONAL_PAIR_ROUTE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_BIDIRECTIONAL_PAIR_ROUTE_LIMIT", "12"))
BIDIRECTIONAL_PAIR_DYNAMIC_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_BIDIRECTIONAL_PAIR_DYNAMIC_LIMIT", "80"))
BIDIRECTIONAL_PAIR_REVERSE_RELATION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_BIDIRECTIONAL_PAIR_REVERSE_RELATION_LIMIT", "2")
)
PAIR_LOCAL_CONTRAST_CANDIDATE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_PAIR_LOCAL_CONTRAST_CANDIDATE_LIMIT", "96")
)
PAIR_LOCAL_CONTRAST_PEER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_PAIR_LOCAL_CONTRAST_PEER_LIMIT", "18"))
PAIR_LOCAL_CONTRAST_NEAR_BONUS = float(os.environ.get("DARTLAB_HORIZON_V168_PAIR_LOCAL_CONTRAST_NEAR_BONUS", "0.42"))
PAIR_LOCAL_CONTRAST_MIN_MARGIN = float(os.environ.get("DARTLAB_HORIZON_V168_PAIR_LOCAL_CONTRAST_MIN_MARGIN", "0.015"))
TARGET_MEMBER_FRAME_OCCURRENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_FRAME_OCCURRENCE_LIMIT", "56")
)
TARGET_MEMBER_FRAME_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_FRAME_ATOM_LIMIT", "120"))
TARGET_MEMBER_FRAME_PEER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_FRAME_PEER_LIMIT", "8"))
TARGET_MEMBER_FRAME_ROUTE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_FRAME_ROUTE_LIMIT", str(MASKED_FRAME_PROBE_LIMIT))
)
TARGET_MEMBER_FRAME_NEIGHBOR_RADIUS = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_FRAME_NEIGHBOR_RADIUS", "3")
)
TARGET_MEMBER_FRAME_NEIGHBOR_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_FRAME_NEIGHBOR_ATOM_LIMIT", "14")
)
TARGET_MEMBER_FRAME_RELATION_WINDOW = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_FRAME_RELATION_WINDOW", "8")
)
TARGET_MEMBER_FRAME_CANDIDATE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_FRAME_CANDIDATE_LIMIT", str(BIDIRECTIONAL_PAIR_CANDIDATE_LIMIT))
)
TARGET_MEMBER_FRAME_QUERY_ROUTE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_FRAME_QUERY_ROUTE_LIMIT", "24")
)
COMPOSITIONAL_FRAME_MIN_QUERY_ROWS = int(
    os.environ.get("DARTLAB_HORIZON_V168_COMPOSITIONAL_FRAME_MIN_QUERY_ROWS", "24")
)
COMPOSITIONAL_FRAME_FRAGMENT_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_COMPOSITIONAL_FRAME_FRAGMENT_LIMIT", "10")
)
COMPOSITIONAL_FRAME_FRAGMENT_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_COMPOSITIONAL_FRAME_FRAGMENT_ROW_LIMIT", "18")
)
COMPOSITIONAL_FRAME_MIN_FRAGMENT_LEN = int(
    os.environ.get("DARTLAB_HORIZON_V168_COMPOSITIONAL_FRAME_MIN_FRAGMENT_LEN", "2")
)
COMPOSITIONAL_FRAME_MAX_FRAGMENT_LEN = int(
    os.environ.get("DARTLAB_HORIZON_V168_COMPOSITIONAL_FRAME_MAX_FRAGMENT_LEN", "6")
)
COMPOSITIONAL_FRAME_ATOM_SCALE = float(os.environ.get("DARTLAB_HORIZON_V168_COMPOSITIONAL_FRAME_ATOM_SCALE", "0.58"))
COMPOSITIONAL_FRAME_SIGNATURE_SCALE = float(
    os.environ.get("DARTLAB_HORIZON_V168_COMPOSITIONAL_FRAME_SIGNATURE_SCALE", "0.24")
)
RELATION_SLOT_SUBSTITUTION_WINDOW = int(os.environ.get("DARTLAB_HORIZON_V168_RELATION_SLOT_SUBSTITUTION_WINDOW", "8"))
RELATION_SLOT_SUBSTITUTION_MIN_KEY_DF = int(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_SLOT_SUBSTITUTION_MIN_KEY_DF", "2")
)
RELATION_SLOT_SUBSTITUTION_MAX_KEY_DF = int(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_SLOT_SUBSTITUTION_MAX_KEY_DF", "260")
)
RELATION_SLOT_SUBSTITUTION_POSTING_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_SLOT_SUBSTITUTION_POSTING_LIMIT", "80")
)
RELATION_SLOT_SUBSTITUTION_SURFACE_KEY_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_SLOT_SUBSTITUTION_SURFACE_KEY_LIMIT", "36")
)
RELATION_SLOT_SUBSTITUTION_QUERY_KEY_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_SLOT_SUBSTITUTION_QUERY_KEY_LIMIT", "64")
)
RELATION_SLOT_SUBSTITUTION_FRAGMENT_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_SLOT_SUBSTITUTION_FRAGMENT_LIMIT", "8")
)
RELATION_SLOT_SUBSTITUTION_ROUTE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_SLOT_SUBSTITUTION_ROUTE_LIMIT", str(MASKED_FRAME_PROBE_LIMIT))
)
RELATION_SLOT_SUBSTITUTION_SEED_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_SLOT_SUBSTITUTION_SEED_WEIGHT", "1.15")
)
RELATION_SLOT_SUBSTITUTION_TYPED_SPECIFICITY_MIN = float(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_SLOT_SUBSTITUTION_TYPED_SPECIFICITY_MIN", "0.16")
)
RELATION_SLOT_SUBSTITUTION_TYPED_HUBNESS_MAX = float(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_SLOT_SUBSTITUTION_TYPED_HUBNESS_MAX", "0.66")
)
RELATION_SLOT_SUBSTITUTION_TYPED_ENTROPY_MAX = float(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_SLOT_SUBSTITUTION_TYPED_ENTROPY_MAX", "0.86")
)
RELATION_SLOT_SUBSTITUTION_TYPED_CONCEPT_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_SLOT_SUBSTITUTION_TYPED_CONCEPT_WEIGHT", "1.18")
)
RELATION_SLOT_SUBSTITUTION_TYPED_GENERIC_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_SLOT_SUBSTITUTION_TYPED_GENERIC_WEIGHT", "0.42")
)
RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_TARGET_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_TARGET_LIMIT", "10")
)
RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_WEAK_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_WEAK_WEIGHT", "0.52")
)
RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_GENERIC_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_GENERIC_WEIGHT", "0.24")
)
RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_TARGET_KEY_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_TARGET_KEY_WEIGHT", "0.32")
)
RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_MIN_PAIR_SCORE = float(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_SLOT_SUBSTITUTION_OWNER_BOUND_MIN_PAIR_SCORE", "0.035")
)
RELATION_SLOT_SUBSTITUTION_LEDGER_OCCURRENCE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_SLOT_SUBSTITUTION_LEDGER_OCCURRENCE_LIMIT", "48")
)
RELATION_SLOT_SUBSTITUTION_LEDGER_TARGET_KEY_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_SLOT_SUBSTITUTION_LEDGER_TARGET_KEY_WEIGHT", "0.38")
)
RELATION_SLOT_SUBSTITUTION_LEDGER_BROAD_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V168_RELATION_SLOT_SUBSTITUTION_LEDGER_BROAD_WEIGHT", "0.34")
)
PAIR_LOCAL_LEDGER_FRAGMENT_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_PAIR_LOCAL_LEDGER_FRAGMENT_LIMIT", "10"))
PAIR_LOCAL_LEDGER_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_PAIR_LOCAL_LEDGER_ATOM_LIMIT", "48"))
PAIR_LOCAL_LEDGER_BAND_SIZE = int(os.environ.get("DARTLAB_HORIZON_V168_PAIR_LOCAL_LEDGER_BAND_SIZE", "3"))
PAIR_LOCAL_LEDGER_BAND_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_PAIR_LOCAL_LEDGER_BAND_LIMIT", "12"))
PAIR_LOCAL_LEDGER_GAIN = float(os.environ.get("DARTLAB_HORIZON_V168_PAIR_LOCAL_LEDGER_GAIN", "1.45"))
PAIR_LOCAL_LEDGER_MARGIN_GAIN = float(os.environ.get("DARTLAB_HORIZON_V168_PAIR_LOCAL_LEDGER_MARGIN_GAIN", "2.20"))
PAIR_LOCAL_LEDGER_NEGATIVE_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V168_PAIR_LOCAL_LEDGER_NEGATIVE_FACTOR", "0.74")
)
PAIR_LOCAL_LEDGER_EMPTY_FACTOR = float(os.environ.get("DARTLAB_HORIZON_V168_PAIR_LOCAL_LEDGER_EMPTY_FACTOR", "0.88"))
PAIR_LOCAL_LEDGER_MIN_MARGIN = float(os.environ.get("DARTLAB_HORIZON_V168_PAIR_LOCAL_LEDGER_MIN_MARGIN", "0.018"))
PAIR_LOCAL_LEDGER_RESIDUAL_COMMON_POWER = float(
    os.environ.get("DARTLAB_HORIZON_V168_PAIR_LOCAL_LEDGER_RESIDUAL_COMMON_POWER", "0.72")
)
PAIR_LOCAL_LEDGER_RESIDUAL_IDF_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V168_PAIR_LOCAL_LEDGER_RESIDUAL_IDF_GAIN", "0.58")
)
PAIR_LOCAL_LEDGER_RESIDUAL_MIN_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V168_PAIR_LOCAL_LEDGER_RESIDUAL_MIN_WEIGHT", "0.018")
)
PAIR_LOCAL_LEDGER_RESIDUAL_PEER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_PAIR_LOCAL_LEDGER_RESIDUAL_PEER_LIMIT", "8")
)
DIRECTIONAL_LEDGER_FRAGMENT_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_DIRECTIONAL_LEDGER_FRAGMENT_LIMIT", "8"))
DIRECTIONAL_LEDGER_SOURCE_TARGET_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_DIRECTIONAL_LEDGER_SOURCE_TARGET_LIMIT", "10")
)
DIRECTIONAL_LEDGER_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_DIRECTIONAL_LEDGER_ATOM_LIMIT", "64"))
DIRECTIONAL_LEDGER_GAIN = float(os.environ.get("DARTLAB_HORIZON_V168_DIRECTIONAL_LEDGER_GAIN", "1.72"))
DIRECTIONAL_LEDGER_MARGIN_GAIN = float(os.environ.get("DARTLAB_HORIZON_V168_DIRECTIONAL_LEDGER_MARGIN_GAIN", "2.45"))
DIRECTIONAL_LEDGER_NEGATIVE_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V168_DIRECTIONAL_LEDGER_NEGATIVE_FACTOR", "0.72")
)
DIRECTIONAL_LEDGER_EMPTY_FACTOR = float(os.environ.get("DARTLAB_HORIZON_V168_DIRECTIONAL_LEDGER_EMPTY_FACTOR", "0.90"))
DIRECTIONAL_LEDGER_MIN_MARGIN = float(os.environ.get("DARTLAB_HORIZON_V168_DIRECTIONAL_LEDGER_MIN_MARGIN", "0.014"))
TARGET_MEMBER_PRIOR_SOURCE_TARGET_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PRIOR_SOURCE_TARGET_LIMIT", "16")
)
TARGET_MEMBER_PRIOR_HARD_NEGATIVE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PRIOR_HARD_NEGATIVE_LIMIT", "14")
)
TARGET_MEMBER_PRIOR_GAIN = float(os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PRIOR_GAIN", "2.35"))
TARGET_MEMBER_PRIOR_MARGIN_GAIN = float(os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PRIOR_MARGIN_GAIN", "2.10"))
TARGET_MEMBER_PRIOR_NEGATIVE_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PRIOR_NEGATIVE_FACTOR", "0.68")
)
TARGET_MEMBER_PRIOR_EMPTY_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PRIOR_EMPTY_FACTOR", "0.92")
)
TARGET_MEMBER_PRIOR_MIN_MARGIN = float(os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PRIOR_MIN_MARGIN", "0.012"))
TARGET_MEMBER_TRANSPORT_GAIN = float(os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_TRANSPORT_GAIN", "3.20"))
TARGET_MEMBER_TRANSPORT_ANCHOR_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_TRANSPORT_ANCHOR_GAIN", "1.35")
)
TARGET_MEMBER_TRANSPORT_ROLE_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_TRANSPORT_ROLE_GAIN", "0.86")
)
TARGET_MEMBER_TRANSPORT_EMPTY_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_TRANSPORT_EMPTY_FACTOR", "0.94")
)
TARGET_MEMBER_TRANSPORT_LOW_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_TRANSPORT_LOW_FACTOR", "0.78")
)
TARGET_MEMBER_TRANSPORT_MIN_LIKELIHOOD = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_TRANSPORT_MIN_LIKELIHOOD", "0.045")
)
TARGET_MEMBER_CONDITIONAL_TRANSPORT_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_CONDITIONAL_TRANSPORT_GAIN", "2.35")
)
TARGET_MEMBER_CONDITIONAL_TRANSPORT_RESIDUAL_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_CONDITIONAL_TRANSPORT_RESIDUAL_GAIN", "1.15")
)
TARGET_MEMBER_CONDITIONAL_ANCHOR_RESIDUAL_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_CONDITIONAL_ANCHOR_RESIDUAL_GAIN", "0.72")
)
TARGET_MEMBER_CONDITIONAL_TRANSPORT_EMPTY_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_CONDITIONAL_TRANSPORT_EMPTY_FACTOR", "0.92")
)
TARGET_MEMBER_CONDITIONAL_TRANSPORT_LOW_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_CONDITIONAL_TRANSPORT_LOW_FACTOR", "0.82")
)
TARGET_MEMBER_CONDITIONAL_TRANSPORT_PEER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_CONDITIONAL_TRANSPORT_PEER_LIMIT", "10")
)
TARGET_MEMBER_EXPERIENCE_FAMILY_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_EXPERIENCE_FAMILY_MEMBER_LIMIT", "8")
)
TARGET_MEMBER_EXPERIENCE_FAMILY_SCORE_FLOOR = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_EXPERIENCE_FAMILY_SCORE_FLOOR", "0.045")
)
TARGET_MEMBER_FIELD_FAMILY_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_FIELD_FAMILY_MEMBER_LIMIT", "14")
)
TARGET_MEMBER_FIELD_FAMILY_EXPANSION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_FIELD_FAMILY_EXPANSION_LIMIT", "6")
)
TARGET_MEMBER_FIELD_FAMILY_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_FIELD_FAMILY_ATOM_LIMIT", "72")
)
TARGET_MEMBER_FIELD_FAMILY_SOURCE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_FIELD_FAMILY_SOURCE_ATOM_LIMIT", "96")
)
TARGET_MEMBER_FIELD_FAMILY_SIM_MIN = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_FIELD_FAMILY_SIM_MIN", "0.215")
)
TARGET_MEMBER_FIELD_FAMILY_COMMON_RATIO = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_FIELD_FAMILY_COMMON_RATIO", "0.46")
)
TARGET_MEMBER_FIELD_FAMILY_COMMON_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_FIELD_FAMILY_COMMON_FACTOR", "0.78")
)
TARGET_MEMBER_FIELD_FAMILY_RESIDUAL_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_FIELD_FAMILY_RESIDUAL_GAIN", "1.35")
)
TARGET_MEMBER_FIELD_FAMILY_FULL_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_FIELD_FAMILY_FULL_GAIN", "0.34")
)
TARGET_MEMBER_FIELD_FAMILY_MUTUAL_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_FIELD_FAMILY_MUTUAL_GAIN", "0.22")
)
TARGET_MEMBER_TYPED_MICRO_EDGE_LEDGER_KEY_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_TYPED_MICRO_EDGE_LEDGER_KEY_LIMIT", "10")
)
TARGET_MEMBER_TYPED_MICRO_EDGE_PROFILE_KEY_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_TYPED_MICRO_EDGE_PROFILE_KEY_LIMIT", "4")
)
TARGET_MEMBER_TYPED_MICRO_EDGE_POSTING_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_TYPED_MICRO_EDGE_POSTING_LIMIT", "96")
)
TARGET_MEMBER_TYPED_MICRO_EDGE_QUERY_KEY_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_TYPED_MICRO_EDGE_QUERY_KEY_LIMIT", "96")
)
TARGET_MEMBER_TYPED_MICRO_EDGE_ROUTE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_TYPED_MICRO_EDGE_ROUTE_LIMIT", str(MASKED_FRAME_PROBE_LIMIT))
)
TARGET_MEMBER_TYPED_MICRO_EDGE_SEED_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_TYPED_MICRO_EDGE_SEED_WEIGHT", "1.28")
)
TARGET_MEMBER_TYPED_MICRO_EDGE_MIN_KEY_DF = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_TYPED_MICRO_EDGE_MIN_KEY_DF", "1")
)
TARGET_MEMBER_TYPED_MICRO_EDGE_MAX_KEY_DF = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_TYPED_MICRO_EDGE_MAX_KEY_DF", "220")
)
TARGET_MEMBER_TYPED_MICRO_EDGE_RARITY_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_TYPED_MICRO_EDGE_RARITY_GAIN", "0.22")
)
TARGET_MEMBER_TYPED_MICRO_EDGE_BACKOFF_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_TYPED_MICRO_EDGE_BACKOFF_WEIGHT", "0.38")
)
TARGET_MEMBER_PATH_BOUND_HYPEREDGE_LEDGER_KEY_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_BOUND_HYPEREDGE_LEDGER_KEY_LIMIT", "6")
)
TARGET_MEMBER_PATH_BOUND_HYPEREDGE_PROFILE_KEY_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_BOUND_HYPEREDGE_PROFILE_KEY_LIMIT", "3")
)
TARGET_MEMBER_PATH_BOUND_HYPEREDGE_SOURCE_PATH_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_BOUND_HYPEREDGE_SOURCE_PATH_LIMIT", "16")
)
TARGET_MEMBER_PATH_BOUND_HYPEREDGE_REVERSE_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_BOUND_HYPEREDGE_REVERSE_ROW_LIMIT", "48")
)
TARGET_MEMBER_PATH_BOUND_HYPEREDGE_TARGET_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_BOUND_HYPEREDGE_TARGET_ATOM_LIMIT", "96")
)
TARGET_MEMBER_PATH_BOUND_HYPEREDGE_QUERY_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_BOUND_HYPEREDGE_QUERY_ATOM_LIMIT", "96")
)
TARGET_MEMBER_PATH_BOUND_HYPEREDGE_POSTING_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_BOUND_HYPEREDGE_POSTING_LIMIT", "96")
)
TARGET_MEMBER_PATH_BOUND_HYPEREDGE_MAX_ATOM_DF = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_BOUND_HYPEREDGE_MAX_ATOM_DF", "180")
)
TARGET_MEMBER_PATH_BOUND_HYPEREDGE_ROUTE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_BOUND_HYPEREDGE_ROUTE_LIMIT", str(MASKED_FRAME_PROBE_LIMIT))
)
TARGET_MEMBER_PATH_BOUND_HYPEREDGE_SEED_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_BOUND_HYPEREDGE_SEED_WEIGHT", "1.42")
)
TARGET_MEMBER_PATH_BOUND_HYPEREDGE_RARITY_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_BOUND_HYPEREDGE_RARITY_GAIN", "0.38")
)
TARGET_MEMBER_PATH_BOUND_HYPEREDGE_PROOF_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_BOUND_HYPEREDGE_PROOF_GAIN", "1.08")
)
TARGET_MEMBER_PATH_BOUND_HYPEREDGE_SEQUENCE_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_BOUND_HYPEREDGE_SEQUENCE_GAIN", "0.18")
)
TARGET_MEMBER_PATH_EVENT_FIELD_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_EVENT_FIELD_LIMIT", "128")
)
TARGET_MEMBER_PATH_EVENT_FRAGMENT_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_EVENT_FRAGMENT_LIMIT", "18")
)
TARGET_MEMBER_PATH_EVENT_ROUTE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_EVENT_ROUTE_LIMIT", str(MASKED_FRAME_PROBE_LIMIT))
)
TARGET_MEMBER_PATH_EVENT_SEED_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_EVENT_SEED_WEIGHT", "1.18")
)
TARGET_MEMBER_PATH_EVENT_PROOF_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_EVENT_PROOF_GAIN", "1.04")
)
TARGET_MEMBER_PATH_EVENT_SEQUENCE_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_EVENT_SEQUENCE_GAIN", "0.24")
)
TARGET_MEMBER_PATH_EVENT_RESIDUAL_IDF_FLOOR = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_EVENT_RESIDUAL_IDF_FLOOR", "0.08")
)
TARGET_MEMBER_PATH_EVENT_RESIDUAL_IDF_SELECT_FLOOR = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_EVENT_RESIDUAL_IDF_SELECT_FLOOR", "0.18")
)
TARGET_MEMBER_PATH_EVENT_RESIDUAL_SEQUENCE_BOOST = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_EVENT_RESIDUAL_SEQUENCE_BOOST", "1.16")
)
TARGET_MEMBER_PATH_EVENT_RESIDUAL_LOW_IDF_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_EVENT_RESIDUAL_LOW_IDF_FACTOR", "0.58")
)
TARGET_MEMBER_PATH_LOCAL_TRACE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_LOCAL_TRACE_LIMIT", "40")
)
TARGET_MEMBER_PATH_LOCAL_TRACE_ATOM_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_LOCAL_TRACE_ATOM_LIMIT", "34")
)
TARGET_MEMBER_PATH_LOCAL_TRACE_QUERY_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_LOCAL_TRACE_QUERY_LIMIT", "22")
)
TARGET_MEMBER_PATH_LOCAL_TRACE_TARGET_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_LOCAL_TRACE_TARGET_LIMIT", "24")
)
TARGET_MEMBER_PATH_LOCAL_TRACE_PAIR_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_LOCAL_TRACE_PAIR_LIMIT", "160")
)
TARGET_MEMBER_PATH_LOCAL_TRACE_ROUTE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_LOCAL_TRACE_ROUTE_LIMIT", str(MASKED_FRAME_PROBE_LIMIT))
)
TARGET_MEMBER_PATH_LOCAL_TRACE_SEED_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_LOCAL_TRACE_SEED_WEIGHT", "1.22")
)
TARGET_MEMBER_PATH_LOCAL_TRACE_PROOF_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_LOCAL_TRACE_PROOF_GAIN", "1.10")
)
TARGET_MEMBER_PATH_LOCAL_TRACE_SEQUENCE_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_LOCAL_TRACE_SEQUENCE_GAIN", "0.30")
)
TARGET_MEMBER_PATH_LOCAL_TRACE_RARE_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_LOCAL_TRACE_RARE_GAIN", "0.18")
)
TARGET_MEMBER_PATH_LOCAL_TRACE_LOW_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_PATH_LOCAL_TRACE_LOW_FACTOR", "0.88")
)
TARGET_MEMBER_MUTUAL_PROOF_SOURCE_PATH_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_MUTUAL_PROOF_SOURCE_PATH_LIMIT", "18")
)
TARGET_MEMBER_MUTUAL_PROOF_REVERSE_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_MUTUAL_PROOF_REVERSE_ROW_LIMIT", "48")
)
TARGET_MEMBER_MUTUAL_PROOF_FAMILY_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_MUTUAL_PROOF_FAMILY_MEMBER_LIMIT", "8")
)
TARGET_MEMBER_MUTUAL_PROOF_PEER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_MUTUAL_PROOF_PEER_LIMIT", "8")
)
TARGET_MEMBER_MUTUAL_PROOF_SLOT_KEY_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_MUTUAL_PROOF_SLOT_KEY_LIMIT", "56")
)
TARGET_MEMBER_MUTUAL_PROOF_SLOT_ROW_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_MUTUAL_PROOF_SLOT_ROW_LIMIT", "96")
)
TARGET_MEMBER_MUTUAL_PROOF_SLOT_MEMBER_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_MUTUAL_PROOF_SLOT_MEMBER_LIMIT", "6")
)
TARGET_MEMBER_MUTUAL_PROOF_SLOT_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_MUTUAL_PROOF_SLOT_WEIGHT", "0.74")
)
TARGET_MEMBER_MUTUAL_PROOF_GAIN = float(os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_MUTUAL_PROOF_GAIN", "2.60"))
TARGET_MEMBER_MUTUAL_PROOF_MARGIN_GAIN = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_MUTUAL_PROOF_MARGIN_GAIN", "1.85")
)
TARGET_MEMBER_MUTUAL_PROOF_NEGATIVE_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_MUTUAL_PROOF_NEGATIVE_FACTOR", "0.72")
)
TARGET_MEMBER_MUTUAL_PROOF_EMPTY_FACTOR = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_MUTUAL_PROOF_EMPTY_FACTOR", "0.94")
)
TARGET_MEMBER_MUTUAL_PROOF_MIN_MARGIN = float(
    os.environ.get("DARTLAB_HORIZON_V168_TARGET_MEMBER_MUTUAL_PROOF_MIN_MARGIN", "0.010")
)
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V168_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V168_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V168_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V168_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V168_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V168_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V168_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V168_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
LANE_MISMATCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V168_LANE_MISMATCH_PENALTY", "0.18"))
LANE_ARTIFACT_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V168_LANE_ARTIFACT_PENALTY", "0.10"))
NEAREST_ORDER_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_NEAREST_ORDER_SIGNATURE_LIMIT", "24"))
NEAREST_ORDER_PENALTY_MIN = float(os.environ.get("DARTLAB_HORIZON_V168_NEAREST_ORDER_PENALTY_MIN", "0.05"))
NEAREST_ORDER_PENALTY_SCALE = float(os.environ.get("DARTLAB_HORIZON_V168_NEAREST_ORDER_PENALTY_SCALE", "0.16"))
NEAREST_ORDER_COHORT_SURFACE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_NEAREST_ORDER_COHORT_SURFACE_LIMIT", "384")
)
NEAREST_ORDER_COHORT_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_NEAREST_ORDER_COHORT_POSITION_LIMIT", "8192")
)
NEAREST_ORDER_SURFACE_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V168_NEAREST_ORDER_SURFACE_POSITION_LIMIT", "48")
)
SUFFIX_ANCHOR_SUPPORT_MIN = int(os.environ.get("DARTLAB_HORIZON_V168_SUFFIX_ANCHOR_SUPPORT_MIN", "1"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V168_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V168_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V168_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V168_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V168_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V168_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V168_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V168_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V168_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V168_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V168_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V168_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V168_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V168_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V168_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V168_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V168_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V168_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V168_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V168_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V168_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V168_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V168_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
RELAY_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_RELAY_ROW_LIMIT", "160"))
RELAY_SPECIFIC_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_RELAY_SPECIFIC_ROW_LIMIT", "320"))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V168_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V168_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V168_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V168_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V168_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V168_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V168_SKETCH_OCC_BUCKETS", "16"))
...
class Model:
    ...
    relationPathLocalSourceTraces: dict[tuple[str, str], tuple[tuple[tuple[str, float], ...], ...]]
    relationPathLocalTargetTraces: dict[tuple[str, str], tuple[tuple[tuple[str, float], ...], ...]]
    relationPathLocalAtomDf: Counter[str]
...
def relationPathLocalSideBucket(surfacePos: int, surfaceSize: int, relationPos: int, relationSize: int) -> str:
    surfaceCenter = surfacePos + surfaceSize / 2.0
    relationCenter = relationPos + relationSize / 2.0
    if surfaceCenter + 2 < relationCenter:
        return "left"
    if relationCenter + 2 < surfaceCenter:
        return "right"
    return "overlap"


def relationPathLocalPackTrace(trace: Counter[str]) -> tuple[tuple[str, float], ...]:
    if not trace:
        return tuple()
    return tuple((atom, float(weight)) for atom, weight in trace.most_common(TARGET_MEMBER_PATH_LOCAL_TRACE_ATOM_LIMIT))


def relationPathLocalTraceNorm(trace: tuple[tuple[str, float], ...]) -> float:
    return math.sqrt(sum(float(weight) * float(weight) for _atom, weight in trace))


def relationPathLocalTraceAtoms(
    sourceSurface: str,
    targetSurface: str,
    relation: str,
    sourceRole: str,
    ...
    relationDistance: int,
    targetDistance: int,
    bestOwnerSurfaces: tuple[str, ...] | list[str],
    model: Model,
) -> Counter[str]:
    relationKey = relation or "any"
    polarity = targetMemberTypedMicroPolarity(relation)
    ...
    direction = direction or "unknown"
    lanePair = relationPathEventLanePair(sourceLane, targetLane)
    sourceSide = relationPathLocalSideBucket(sourcePos, sourceSize, relationPos, relationSize)
    targetSide = relationPathLocalSideBucket(targetPos, targetSize, relationPos, relationSize)
    relationBucket = relationPathEventDistanceBucket(relationDistance)
    targetBucket = relationPathEventDistanceBucket(targetDistance)
    sourceProfile = inferredOwnerRoleSpecificityProfile(sourceSurface, model)
    targetProfile = inferredOwnerRoleSpecificityProfile(targetSurface, model)
    atoms: Counter[str] = Counter()

    pathBase = (
        f"plt:rel:{relationKey}:pol:{polarity}:srcRole:{sourceRole}:"
        f"tgtRole:{targetRole}:srcSide:{sourceSide}:tgtSide:{targetSide}"
    )
    atoms[pathBase] += 1.00
    atoms[f"{pathBase}:dir:{direction}"] += 0.86
    atoms[f"{pathBase}:lane:{lanePair}"] += 0.70
    atoms[f"{pathBase}:rd:{relationBucket}:td:{targetBucket}"] += 0.66
    atoms[f"pltseq:role:{sourceRole}>{targetRole}:side:{sourceSide}>{targetSide}:dir:{direction}"] += 0.92
    atoms[f"pltseq:lane:{lanePair}:rd:{relationBucket}:td:{targetBucket}"] += 0.62
    atoms[f"pltprof:src:{sourceProfile.role}:tgt:{targetProfile.role}:pol:{polarity}"] += 0.58
    atoms[
        "pltprof:spec:"
        + f"{targetMemberMutualProofBucket(sourceProfile.specificity)}>"
        + f"{targetMemberMutualProofBucket(targetProfile.specificity)}"
    ] += 0.42

    ownerRoles: Counter[str] = Counter()
    ownerSpecificity = 0.0
    ownerHubness = 0.0
    inspectedOwners = 0
    for ownerSurface in bestOwnerSurfaces[:3]:
        ownerSurface = normStem(ownerSurface)
        if not ownerSurface:
            continue
        ownerProfile = inferredOwnerRoleSpecificityProfile(ownerSurface, model)
        ownerRoles[ownerProfile.role] += 1
        ownerSpecificity += ownerProfile.specificity
        ownerHubness += ownerProfile.hubness
        inspectedOwners += 1
    if inspectedOwners:
        ownerRole = ownerRoles.most_common(1)[0][0]
        atoms[f"pltowner:role:{ownerRole}:pol:{polarity}:lane:{lanePair}"] += 0.48
        atoms[
            "pltowner:shape:"
            + f"spec:{targetMemberMutualProofBucket(ownerSpecificity / inspectedOwners)}:"
            + f"hub:{targetMemberMutualProofBucket(ownerHubness / inspectedOwners)}"
        ] += 0.34

    for ledgerKey, ledgerWeight in targetMemberTypedMicroLedgerKeys(targetSurface, relation, model)[:4]:
        if ledgerWeight <= 0.0:
            continue
        atoms[f"pltseq:tled:{ledgerKey}:role:{sourceRole}>{targetRole}:side:{targetSide}"] += ledgerWeight * 0.58
    for ledgerKey, ledgerWeight in targetMemberTypedMicroLedgerKeys(sourceSurface, relation, model)[:3]:
        if ledgerWeight <= 0.0:
            continue
        atoms[f"pltseq:sled:{ledgerKey}:role:{sourceRole}:side:{sourceSide}"] += ledgerWeight * 0.36
    for profileKey, profileWeight in targetMemberTypedMicroProfileKeys(targetSurface, model):
        if profileWeight <= 0.0:
            continue
        atoms[f"plt:tprof:{profileKey}:role:{targetRole}:side:{targetSide}"] += profileWeight * 0.30
    return atoms
...
def buildRelationBoundRolePairIndex(
    ...
    pathLocalSourceRaw: dict[tuple[str, str], list[tuple[float, Counter[str]]]] = defaultdict(list)
    pathLocalTargetRaw: dict[tuple[str, str], list[tuple[float, Counter[str]]]] = defaultdict(list)
    stats: Counter[str] = Counter()
            ...
            for relationPos, relationSize in relPositions:
                ...
                for sourcePos, sourceSize, sourceSurface in frameRows:
                    ...
                    for (
                        ...
                        localTrace = relationPathLocalTraceAtoms(
                            sourceSurface,
                            targetSurface,
                            relation,
                            sourceRole,
                        ...
                        if localTrace:
                            scaledTrace = Counter(
                                {
                                    atom: eventScale * float(atomWeight)
                                    for atom, atomWeight in localTrace.items()
                                    if atomWeight > 0.0
                                }
                            )
                            pathLocalSourceRaw[(sourceSurface, relation)].append((score, scaledTrace))
                            pathLocalTargetRaw[(targetSurface, relation)].append((score, scaledTrace))
                            stats["pathLocalTraces"] += 1
                            stats["pathLocalTraceAtoms"] += len(scaledTrace)
    ...
    pathLocalSourceTraces: dict[tuple[str, str], tuple[tuple[tuple[str, float], ...], ...]] = {}
    for key, traceRows in pathLocalSourceRaw.items():
        if not traceRows:
            continue
        packedRows = [
            relationPathLocalPackTrace(trace)
            for _score, trace in sorted(traceRows, key=lambda item: item[0], reverse=True)[
                :TARGET_MEMBER_PATH_LOCAL_TRACE_LIMIT
            ]
        ]
        packedRows = [trace for trace in packedRows if trace]
        if not packedRows:
            continue
        pathLocalSourceTraces[key] = tuple(packedRows)
        stats["pathLocalSourceKeys"] += 1
        stats["pathLocalSourceTraceRows"] += len(packedRows)
    pathLocalTargetTraces: dict[tuple[str, str], tuple[tuple[tuple[str, float], ...], ...]] = {}
    pathLocalAtomDf: Counter[str] = Counter()
    for key, traceRows in pathLocalTargetRaw.items():
        if not traceRows:
            continue
        packedRows = [
            relationPathLocalPackTrace(trace)
        ...
        if not packedRows:
            continue
        pathLocalTargetTraces[key] = tuple(packedRows)
        for trace in packedRows:
            for atom, _weight in trace:
                pathLocalAtomDf[atom] += 1
        stats["pathLocalTargetKeys"] += 1
        stats["pathLocalTargetTraceRows"] += len(packedRows)
    stats["pathLocalAtomDfKeys"] = len(pathLocalAtomDf)
    ...
    model.relationPathLocalSourceTraces = pathLocalSourceTraces
    model.relationPathLocalTargetTraces = pathLocalTargetTraces
    model.relationPathLocalAtomDf = pathLocalAtomDf
    ...
    print(
        ...
        + f"pathLocalSourceKeys={stats.get('pathLocalSourceKeys', 0)} "
        + f"pathLocalTargetKeys={stats.get('pathLocalTargetKeys', 0)} "
        + f"pathLocalTraceRows={stats.get('pathLocalTargetTraceRows', 0)} "
...
TARGET_MEMBER_PATH_LOCAL_TRACE_QUERY_CACHE: dict[
    tuple[int, str, tuple[str, ...]], tuple[tuple[tuple[tuple[str, float], ...], ...], Counter[str]]
] = {}
TARGET_MEMBER_PATH_LOCAL_TRACE_TARGET_CACHE: dict[
    tuple[int, str, tuple[str, ...]], tuple[tuple[tuple[tuple[str, float], ...], ...], Counter[str]]
] = {}
TARGET_MEMBER_PATH_LOCAL_TRACE_PROOF_CACHE: dict[tuple[int, str, str, tuple[str, ...]], tuple[float, Counter[str]]] = {}
...
def buildModel() -> Model:
    ...
    model = Model(
        ...
        {},  # relationPathLocalSourceTraces
        {},  # relationPathLocalTargetTraces
        Counter(),  # relationPathLocalAtomDf
...
def main() -> None:
    ...
    for surface, expected in POSITIVE_PROBES:
        ...
        print(
            ...
            f"localMatched={frameStats.get('pathLocalMatchedAtoms', 0)} localSeq={frameStats.get('pathLocalSequenceMatchedAtoms', 0)} localRare={frameStats.get('pathLocalRareMatchedAtoms', 0)} localPairs={frameStats.get('pathLocalAcceptedTracePairs', 0)} localProof={frameStats.get('pathLocalProofMilli', 0)} "
    ...
    print(
        ...
        f"relationPathLocalTraces={model.relationPathEventStats.get('pathLocalTraces', 0)} "
        f"relationPathLocalSourceKeys={model.relationPathEventStats.get('pathLocalSourceKeys', 0)} "
        f"relationPathLocalTargetKeys={model.relationPathEventStats.get('pathLocalTargetKeys', 0)} "
        f"relationPathLocalTargetTraceRows={model.relationPathEventStats.get('pathLocalTargetTraceRows', 0)} "
        f"relationPathLocalAtomDfKeys={model.relationPathEventStats.get('pathLocalAtomDfKeys', 0)} "
        f"targetMemberPathLocalProbeTop1={targetMemberPathLocalProbeTop1}/{len(POSITIVE_PROBES)} "
        f"targetMemberPathLocalProbeTop5={targetMemberPathLocalProbeTop5}/{len(POSITIVE_PROBES)} "
        f"targetMemberPathLocalProbeBadTop1={targetMemberPathLocalProbeBadTop1}/{len(NEGATIVE_PROBES)} "
        f"targetMemberPathLocalProbeBadTop5={targetMemberPathLocalProbeBadTop5}/{len(NEGATIVE_PROBES)} "
        f"targetMemberPathLocalQueryTraceRows={targetMemberPathLocalQueryTraceRows} "
        f"targetMemberPathLocalScored={targetMemberPathLocalScored} "
        f"targetMemberPathLocalRouteRows={targetMemberPathLocalRouteRows} "
        f"targetMemberPathLocalMatchedAtoms={targetMemberPathLocalMatchedAtoms} "
        f"targetMemberPathLocalSequenceMatchedAtoms={targetMemberPathLocalSequenceMatchedAtoms} "
        f"targetMemberPathLocalRareMatchedAtoms={targetMemberPathLocalRareMatchedAtoms} "
        f"targetMemberPathLocalTracePairs={targetMemberPathLocalTracePairs} "
        ...
        f"targetMemberFramePathLocalPairScored={targetMemberFramePathLocalPairScored} "
        f"targetMemberFramePathLocalQueryTraceRows={targetMemberFramePathLocalQueryTraceRows} "
        f"targetMemberFramePathLocalTargetTraceRows={targetMemberFramePathLocalTargetTraceRows} "
        f"targetMemberFramePathLocalMatchedAtoms={targetMemberFramePathLocalMatchedAtoms} "
        f"targetMemberFramePathLocalSequenceMatchedAtoms={targetMemberFramePathLocalSequenceMatchedAtoms} "
        f"targetMemberFramePathLocalRareMatchedAtoms={targetMemberFramePathLocalRareMatchedAtoms} "
        f"targetMemberFramePathLocalTracePairs={targetMemberFramePathLocalTracePairs} "
