"""Horizon Meaning Learner V188 - explicit counter path object proof.

아이디어:
    V187 은 corpus fragment 주변 경험을 residual atom field 로 투영해 V186 alias-only path 보다 좋은 open held-out
    MRR 을 만들었다. V188 은 그 다음 병목인 "비슷하지만 반대인 경험"을 다룬다. 수평선 좌표는 여전히 의미가 아니라
    고정 주소이고, 의미는 주변 경험의 support/counter 관계다.

    첫 구현은 slot/statement/missing modifier 까지 counter 로 넣었지만 320 held-out 에서 Top1/MRR 을 낮춰 실패했다.
    최종 V188 은 넓은 counter 를 버리고 train alias 경험에서 얻은 명시적 반대 path object 만 사용한다.

    - supportScore: V187 corpus fragment residual overlap 유지
    - counterScore: 유입/유출, 증가/감소, 완성/미완성, 유동/비유동 같은 explicit contrast term group 만 penalty
    - profile term hint: train alias 의 반대/필수 term 경험을 snake profile 에 미리 누적해 candidate loop 에서 확인
    - counter scale: 0.50. 0.70 은 497 MRR 은 올렸지만 Top1 을 하나 낮춰 폐기했다.

    목표는 V187 위에 더 많은 규칙을 덧붙이는 것이 아니라, 경험의 유사와 충돌을 분리할 수 있는 최소 counter proof 가
    open held-out 에서 MRR 을 올리면서 Top1 을 해치지 않는지 확인하는 것이다.

실행 코드:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV188Test.py
    $env:DARTLAB_HORIZON_V188_HELDOUT_LIMIT='80'; uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV188Test.py
    $env:DARTLAB_HORIZON_V188_HELDOUT_LIMIT='600'; uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV188Test.py
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV188Test.py
...
검증 기준:
    ...
    - baseline, V186 alias path, V187 corpus path, V188 counter path 를 같은 split 에서 비교한다.
    - V188 counterPathWithSj 가 V187 corpusPathWithSj 의 MRR 을 넘고 Top1 을 떨어뜨리지 않으면 counter 신호로 본다.

결과:
    20 smoke(row-limited): corpusPathWithSj Top1=8/20 Top5=16/20 MRR=0.5796,
    counterPathWithSj Top1=9/20 Top5=16/20 MRR=0.6087, movement=2/18/0.

    80 held-out(scale=0.50): corpusPathWithSj Top1=33/80 Top3=51/80 Top5=59/80 MRR=0.5625,
    counterPathWithSj Top1=34/80 Top3=52/80 Top5=59/80 MRR=0.5700, movement=6/74/0.

    320 held-out(scale=0.50): corpusPathWithSj Top1=131/320 Top3=214/320 Top5=241/320 MRR=0.5647,
    counterPathWithSj Top1=131/320 Top3=215/320 Top5=241/320 MRR=0.5658, movement=20/298/2.

    HELDOUT_LIMIT=600 produced 497 eligible held-out cases(scale=0.50): corpus docFiles=12, allFilingsFiles=6,
    rowsScanned=15173, aliasMatches=65571, leakedAliases=0. corpusPathWithSj Top1=210/497 Top3=333/497
    Top5=380/497 MRR=0.5716, counterPathWithSj Top1=210/497 Top3=336/497 Top5=381/497 MRR=0.5729,
    movement=33/460/4. noSj 는 corpusPath MRR=0.5258 에서 counterPath MRR=0.5248 로 소폭 하락했다.

결론:
    부분 성공/개념 상승. 넓은 counter object 는 실패했지만, 명시적 반대 path object 를 작게 걸면 withSj open
    held-out 80/320/497 에서 Top1 을 해치지 않고 MRR 을 올린다. 효과는 작지만 방향은 의미 있다. 다만 noSj 는 하락했고,
    완성/미완성 같은 강한 반대도 아직 rank1 을 뒤집지 못한다. 다음은 hand contrast group 을 늘리는 것이 아니라
    같은 object family 안에서 실제로 서로를 밀어내는 sibling contrast 를 train cluster/corpus row 에서 자동 학습하는 것이다.
"""
...
HELDOUT_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V188_HELDOUT_LIMIT", "320"))
MIN_CLUSTER_SIZE = int(os.environ.get("DARTLAB_HORIZON_V188_MIN_CLUSTER_SIZE", "4"))
MAX_TRAIN_ALIASES_PER_SNAKE = int(os.environ.get("DARTLAB_HORIZON_V188_MAX_TRAIN_ALIASES_PER_SNAKE", "80"))
POOL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V188_POOL_LIMIT", "5200"))
POSTING_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V188_POSTING_ROW_LIMIT", "2600"))
TOP_K = int(os.environ.get("DARTLAB_HORIZON_V188_TOP_K", "10"))
NUM_MH = int(os.environ.get("DARTLAB_HORIZON_V188_NUM_MH", "32"))
PATH_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V188_PATH_ATOM_LIMIT", "96"))
PATH_RESIDUAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V188_PATH_RESIDUAL_LIMIT", "64"))
CORPUS_DOC_FILE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V188_DOC_FILE_LIMIT", "18"))
CORPUS_ALL_FILINGS_FILE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V188_ALL_FILINGS_FILE_LIMIT", "6"))
CORPUS_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V188_CORPUS_ROW_LIMIT", "18000"))
CORPUS_TEXT_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V188_CORPUS_TEXT_LIMIT", "3600"))
CORPUS_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V188_CORPUS_GRAM_POSTING_LIMIT", "160"))
CORPUS_ALIAS_MATCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V188_CORPUS_ALIAS_MATCH_LIMIT", "90000"))
CORPUS_PROFILE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V188_CORPUS_PROFILE_LIMIT", "96"))
CORPUS_RESIDUAL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V188_CORPUS_RESIDUAL_LIMIT", "64"))
CORPUS_FRAGMENT_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V188_CORPUS_FRAGMENT_LIMIT", "64"))
COUNTER_SCALE = float(os.environ.get("DARTLAB_HORIZON_V188_COUNTER_SCALE", "0.50"))
...
def main() -> None:
    ...
    print("V188 corpus counter path object proof")
    print(
    ...
    for case in heldout:
        ...
        if counterValue < corpusValue:
            ...
            same += 1
        if counterNo != 1 and len(wrongNoSj) < 12:
            wrongNoSj.append((case, counterNo, counterRowsNo, candidateCountNo))
        if counterSj != 1 and len(wrongWithSj) < 12:
            wrongWithSj.append((case, counterSj, counterRowsSj, candidateCountSj))
