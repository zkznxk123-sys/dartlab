"""Horizon Meaning Learner V194 - local span meaning capsule.

아이디어:
    V193 은 accountMappings 중심에서 벗어나 panel/allFilings fragment-first capsule 로 돌아왔지만,
    panel block 하나를 하나의 의미 object 로 보는 순간 같은 block 안의 유입/유출, 영업/투자, target/action span 이
    섞였다. 특히 현금흐름표 block 은 여러 행의 반대 방향 경험을 한 capsule 에 같이 넣었고, 영업권 손상 probe 는
    target(goodwill) 만 맞고 action(impairment) 이 빠진 변동표를 높였다.

    V194 는 학습 단위를 block 에서 local span 으로 더 낮춘다. 수평선 좌표는 여전히 의미가 아니라 빠른 주소이고,
    의미는 panel block 안의 sentence/table-row/window span 과 그 주변 block/cell/allFilings 경험이 겹치거나
    충돌하는 sparse proof object 로 응결된다고 본다.
...
실행 코드:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV194Test.py
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV194Test.py
    $env:DARTLAB_HORIZON_V194_PANEL_FILE_LIMIT='220'; $env:DARTLAB_HORIZON_V194_ALL_FILINGS_FILE_LIMIT='32'; uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV194Test.py
...
검증 기준:
    ...
    - V193 의 block capsule 대비 local span 이 topK evidence 를 더 잘 분리하는지 본다.

결과:
    첫 실행은 local span 만 넣고 V193 과 같은 baselineTop1Semantic=6/8, capsuleTop1Semantic=6/8 에 머물렀다.
    실패 양상은 선명해졌다. `현금 유입 투자활동` 은 같은 span 안에 유입/유출이 같이 있는 mixed role 후보가 높았고,
    `영업권 손상` 은 target 만 있거나 action 만 있는 후보가 갈라졌다.

    최종판은 contrast state atom(`state:{group}:mixed`) 과 missing core action gate 를 추가했다. 기본 smoke 는
    fragments=17,293, panelFiles=120, panelRows=11,428, panelSpans=16,020, allFilingFiles=18,
    allFilingRows=504, allFilingSpans=1,273, cellFiles=0, cellRows=0, cellHintTerms=0, atoms=83,133,
    postings=79,002, buildMs=40,744 였다. `data/dart/panelCell` 은 현재 로컬에 없어 cell anchor 는 비활성이다.

    최종 결과는 baselineTop1Semantic=7/8, baselineTop3Semantic=7/8, capsuleTop1Semantic=7/8,
    capsuleTop3Semantic=7/8 이었다. 더 엄격하게 query 의 expected role 을 모두 만족하는 completeRole 로 보면
    baselineTop1CompleteRole=5/8, capsuleTop1CompleteRole=6/8 로 local capsule 이 surface baseline 을 하나 넘었다.
    capsuleTop1ExactOnly=0, capsuleTop1Contradiction=0 이고 accountGuard 는 cases=40,
    top5SiblingEvidence=2/40 으로 V193 의 4/40 보다 낮아졌다.

결론:
    부분 성공/진단 상승. block 을 local span 으로 쪼개고 mixed role purity 를 proof object 로 만들면 cash-flow
    반대 의미 오염은 줄어든다. 다만 accountMappings guard 는 약해졌고, `영업권 손상` 처럼 target span 과 action span 이
    같은 local span 에 나타나지 않는 질의는 여전히 실패한다. 다음은 같은 parent block/table 안에서 target span 과
    action span 을 event capsule 로 잇는 cross-span stitch 가 필요하다. 즉 의미 object 를 더 작게 쪼개되,
    최종 evidence 는 parent 안의 span graph 로 재결합해야 한다.
"""
...
PANEL_FILE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V194_PANEL_FILE_LIMIT", "120"))
PANEL_ROWS_PER_FILE = int(os.environ.get("DARTLAB_HORIZON_V194_PANEL_ROWS_PER_FILE", "180"))
PANEL_SPANS_PER_BLOCK = int(os.environ.get("DARTLAB_HORIZON_V194_PANEL_SPANS_PER_BLOCK", "4"))
PANEL_SPAN_CHARS = int(os.environ.get("DARTLAB_HORIZON_V194_PANEL_SPAN_CHARS", "620"))
PANEL_CELL_FILE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V194_PANEL_CELL_FILE_LIMIT", "160"))
PANEL_CELL_ROWS_PER_FILE = int(os.environ.get("DARTLAB_HORIZON_V194_PANEL_CELL_ROWS_PER_FILE", "220"))
ALL_FILINGS_FILE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V194_ALL_FILINGS_FILE_LIMIT", "18"))
ALL_FILINGS_ROWS_PER_FILE = int(os.environ.get("DARTLAB_HORIZON_V194_ALL_FILINGS_ROWS_PER_FILE", "28"))
ALL_FILINGS_CHARS = int(os.environ.get("DARTLAB_HORIZON_V194_ALL_FILINGS_CHARS", "2600"))
ALL_FILINGS_SPANS_PER_ROW = int(os.environ.get("DARTLAB_HORIZON_V194_ALL_FILINGS_SPANS_PER_ROW", "3"))
MAX_FRAGMENTS = int(os.environ.get("DARTLAB_HORIZON_V194_MAX_FRAGMENTS", "22000"))
POSTING_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V194_POSTING_ATOM_LIMIT", "360"))
CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V194_CANDIDATE_LIMIT", "900"))
TOP_K = int(os.environ.get("DARTLAB_HORIZON_V194_TOP_K", "5"))
ACCOUNT_GUARD_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V194_ACCOUNT_GUARD_LIMIT", "40"))
...
def missingCoreActions(qCapsule: Counter[str], fCapsule: Counter[str]) -> tuple[str, ...]:
    ...
    fActions = roleKeySet(fCapsule, "action")
...
def main() -> None:
    ...
    print("V194 local span meaning capsule")
    print(
