"""Horizon Meaning Learner V196 - typed stitch gate capsule.

아이디어:
    V195 는 local span 을 parent graph event capsule 로 다시 묶어 문서 의미 probe 에서 surface baseline 을 처음 넘겼다.
    하지만 `sectionNear` stitch 는 아직 넓다. 가까운 span 이라는 이유만으로 event 를 공유하면 cash-flow 같은 표에서
    방향성은 hard veto 로 막아도 statement/modifier edge 가 흐릴 수 있다.

    V196 은 모든 가까운 span 을 잇지 않고 typed boundary 가 증명한 span 만 잇는다. 같은 statement role, table-row
    anchor overlap, block/section label overlap, xbrl/meta boundary 같은 경계가 있을 때만 section-level event 를 통과시킨다.
    수평선 좌표는 빠른 주소이고, 의미는 typed boundary 를 통과한 fragment/block/cell 경험의 sparse proof object 다.
    ...
    - 같은 section 의 가까운 blockOrder(+/-1)는 typed boundary gate 를 통과할 때만 약한 `event:*` bridge 를 만든다.
...
실행 코드:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV196Test.py
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV196Test.py
    $env:DARTLAB_HORIZON_V196_PANEL_FILE_LIMIT='220'; $env:DARTLAB_HORIZON_V196_ALL_FILINGS_FILE_LIMIT='32'; uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV196Test.py
...
검증 기준:
    ...
    - V195 의 capsuleTop1Semantic=8/8 을 유지하면서 sectionNear 오염과 completeRole 이 개선되는지 본다.

결과:
    기본 smoke 는 fragments=17,293, panelFiles=120, panelRows=11,428, panelSpans=16,020,
    allFilingFiles=18, allFilingRows=504, allFilingSpans=1,273, blockEventGroups=1,290,
    sectionNearEventSpans=4,540, sectionNearRejectedSpans=6,573, filingEventGroups=157,
    cellFiles=0, cellRows=0, cellHintTerms=0, atoms=84,025, postings=79,858, buildMs=85,950 이었다.
    `data/dart/panelCell` 은 현재 로컬에 없어 cell anchor 는 비활성이다.

    결과는 baselineTop1Semantic=7/8, baselineTop3Semantic=7/8, capsuleTop1Semantic=8/8,
    capsuleTop3Semantic=8/8, baselineTop1CompleteRole=5/8, capsuleTop1CompleteRole=7/8 이었다.
    accountGuard 는 cases=40, top5SiblingEvidence=2/40 이었다. V195 대비 semantic/completeRole 수치는 유지됐고,
    sectionNearEventSpans 는 6,112 에서 4,540 으로 줄었다.

결론:
    부분 성공/진단 성공. typed boundary gate 는 V195 의 문서 probe 성능을 유지하면서 sectionNear edge 폭을 줄였다.
    그러나 completeRole 은 7/8 로 오르지 않았고, `현금 유입 투자활동` 은 여전히 cashflow/inflow/cash 까지는 맞지만
    investing modifier 를 top1 에서 완성하지 못한다. 다음은 sectionNear 를 더 줄이는 것이 아니라 실제 table cell 또는
    row label/value/axis 경계를 쓰는 stitch 가 필요하다. 현재 로컬에는 `data/dart/panelCell` 이 없어 이 부분은
    데이터 생성/적재 후 검증해야 한다.
"""
...
PANEL_FILE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V196_PANEL_FILE_LIMIT", "120"))
PANEL_ROWS_PER_FILE = int(os.environ.get("DARTLAB_HORIZON_V196_PANEL_ROWS_PER_FILE", "180"))
PANEL_SPANS_PER_BLOCK = int(os.environ.get("DARTLAB_HORIZON_V196_PANEL_SPANS_PER_BLOCK", "4"))
PANEL_SPAN_CHARS = int(os.environ.get("DARTLAB_HORIZON_V196_PANEL_SPAN_CHARS", "620"))
PANEL_CELL_FILE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V196_PANEL_CELL_FILE_LIMIT", "160"))
PANEL_CELL_ROWS_PER_FILE = int(os.environ.get("DARTLAB_HORIZON_V196_PANEL_CELL_ROWS_PER_FILE", "220"))
ALL_FILINGS_FILE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V196_ALL_FILINGS_FILE_LIMIT", "18"))
ALL_FILINGS_ROWS_PER_FILE = int(os.environ.get("DARTLAB_HORIZON_V196_ALL_FILINGS_ROWS_PER_FILE", "28"))
ALL_FILINGS_CHARS = int(os.environ.get("DARTLAB_HORIZON_V196_ALL_FILINGS_CHARS", "2600"))
ALL_FILINGS_SPANS_PER_ROW = int(os.environ.get("DARTLAB_HORIZON_V196_ALL_FILINGS_SPANS_PER_ROW", "3"))
MAX_FRAGMENTS = int(os.environ.get("DARTLAB_HORIZON_V196_MAX_FRAGMENTS", "22000"))
POSTING_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V196_POSTING_ATOM_LIMIT", "360"))
CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V196_CANDIDATE_LIMIT", "900"))
TOP_K = int(os.environ.get("DARTLAB_HORIZON_V196_TOP_K", "5"))
ACCOUNT_GUARD_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V196_ACCOUNT_GUARD_LIMIT", "40"))
...
def typedStitchAllowed(anchor: Fragment, peer: Fragment) -> bool:
    if anchor.idx == peer.idx:
        return True
    anchorStatements = roleKeySet(anchor.capsule, "statement")
    peerStatements = roleKeySet(peer.capsule, "statement")
    if anchorStatements and anchorStatements & peerStatements:
        return True
    anchorXbrl = {atom for atom in anchor.capsule if atom.startswith("xbrl:")}
    peerXbrl = {atom for atom in peer.capsule if atom.startswith("xbrl:")}
    if anchorXbrl and len(anchorXbrl & peerXbrl) >= 2:
        return True
    if anchor.spanKind == "tableRow" and peer.spanKind == "tableRow":
        shared = stitchBoundaryTerms(anchor) & stitchBoundaryTerms(peer)
        if len(shared) >= 2:
            return True
...
def addParentEventExperience(fragments: list[Fragment]) -> Counter[str]:
    ...
    for rows in sectionGroups.values():
        ...
        for index, fragment in enumerate(rows):
            ...
            peers = [other for other in nearby if typedStitchAllowed(fragment, other)]
            if len(peers) < 2:
                stats["sectionNearRejectedSpans"] += 1
                continue
...
def main() -> None:
    ...
    print("V196 typed stitch gate capsule")
    print(
        ...
        f"sectionNearRejectedSpans={buildStats['sectionNearRejectedSpans']} "
