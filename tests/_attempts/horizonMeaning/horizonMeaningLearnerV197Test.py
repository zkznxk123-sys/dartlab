"""Horizon Meaning Learner V197 - textual experience chain capsule.

아이디어:
    V193~V196 은 panel/allFilings 텍스트로 돌아와 block -> local span -> cross-span event 로 발전했다.
    V195/V196 은 문서 의미 probe 에서 8/8 을 만들었지만, 아직 parent stitch 가 넓고 completeRole 은 7/8 에 머물렀다.
    여기서 중요한 correction 은 panelCell/value/axis 로 빠지지 않는 것이다. 이번 라인의 의미는 `panel.contentRaw` 와
    `allFilings.content_raw` 텍스트 자체에서 만든 경험이어야 한다.

    V197 은 텍스트 span 들이 같은 block/report 안에서 만드는 "사건 사슬"을 sparse proof object 로 만든다.
    수평선 좌표는 빠른 주소이고, 의미는 stem 의 앞뒤 순서, 거리, 반복 anchor, report title, block/section 위치,
    역할(action/target/modifier/statement), 방향성 충돌, 같은 사건 안의 연결 관계가 누적된 경험 capsule 이다.
    ...
    - 같은 block/report 안에서 target/action/modifier 가 떨어져 있으면 무조건 stitch 하지 않고 텍스트 거리, 순서,
      반복 anchor, report/block title 공유, 방향성 일관성으로만 textual event chain 을 만든다.
    - allFilings report_nm 은 사건 제목 경험, content_raw 는 사건 설명 경험으로 넣는다.
    - local span 의미가 우선이고 parent/report chain 은 보조 경험이다. parent 가 local 반대 의미를 덮어쓰면 실패로 본다.
    - query 도 같은 capsule 공간에서 만들고 posting 후보를 capsule overlap + contradiction + residual 로 rerank 한다.

실행 코드:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV197Test.py
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV197Test.py
    $env:DARTLAB_HORIZON_V197_PANEL_FILE_LIMIT='220'; $env:DARTLAB_HORIZON_V197_ALL_FILINGS_FILE_LIMIT='32'; uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV197Test.py
...
검증 기준:
    ...
    - rubric 은 semantic hit, completeRole, exact-only, contradiction, boilerplate, textual chain 사용 여부를 본다.
    - V196 의 capsuleTop1Semantic=8/8 을 유지하면서 completeRole 7/8 을 넘길 수 있는지 본다.
...
결과:
    ...
    - textChainSpans=16,388, textChainAtoms=660,814
    - textChainBlockGroups=2,213, textChainSectionSpans=8,769, textChainSectionRejectedSpans=4,934,
      textChainFilingGroups=416
    - atoms=235,786, postings=223,629, buildMs=272,513, totalSeconds=459.0
    - baselineTop1Semantic=6/8, baselineTop3Semantic=6/8
    - capsuleTop1Semantic=5/8, capsuleTop3Semantic=6/8
    - baselineTop1CompleteRole=6/8, capsuleTop1CompleteRole=5/8
    - capsuleTop1Contradiction=2, capsuleTop1TextAnchored=8, queryTextAnchored=8
    - accountGuard top5SiblingEvidence=0/40

    첫 실행은 parent/local 구분이 약해 noData/표제 경험이 과증폭됐다. 이후 local text 공유, noData chain 차단,
    target residual 을 넣어 재실행했지만, textChain atom 이 여전히 66 만 개까지 번져 V196 의 8/8 을 잃었다.

결론:
    실패/진단 성공. 텍스트 사건 사슬은 방향 자체는 맞지만, event atom 을 postings/ranker 의 1 차 proof 로
    직접 주입하면 block/report 안의 반복 표제와 broad 경험이 의미를 덮어쓴다. 다음 실험은 chain 을 후보 proof 로
    물리화하지 말고, local capsule 이 먼저 target/action 을 잡은 뒤 빠진 role 을 보강하는 residual edge ledger 로만
    써야 한다.
"""
...
PANEL_FILE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V197_PANEL_FILE_LIMIT", "120"))
PANEL_ROWS_PER_FILE = int(os.environ.get("DARTLAB_HORIZON_V197_PANEL_ROWS_PER_FILE", "180"))
PANEL_SPANS_PER_BLOCK = int(os.environ.get("DARTLAB_HORIZON_V197_PANEL_SPANS_PER_BLOCK", "4"))
PANEL_SPAN_CHARS = int(os.environ.get("DARTLAB_HORIZON_V197_PANEL_SPAN_CHARS", "620"))
ALL_FILINGS_FILE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V197_ALL_FILINGS_FILE_LIMIT", "18"))
ALL_FILINGS_ROWS_PER_FILE = int(os.environ.get("DARTLAB_HORIZON_V197_ALL_FILINGS_ROWS_PER_FILE", "28"))
ALL_FILINGS_CHARS = int(os.environ.get("DARTLAB_HORIZON_V197_ALL_FILINGS_CHARS", "2600"))
ALL_FILINGS_SPANS_PER_ROW = int(os.environ.get("DARTLAB_HORIZON_V197_ALL_FILINGS_SPANS_PER_ROW", "3"))
MAX_FRAGMENTS = int(os.environ.get("DARTLAB_HORIZON_V197_MAX_FRAGMENTS", "22000"))
POSTING_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V197_POSTING_ATOM_LIMIT", "360"))
CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V197_CANDIDATE_LIMIT", "900"))
TOP_K = int(os.environ.get("DARTLAB_HORIZON_V197_TOP_K", "5"))
ACCOUNT_GUARD_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V197_ACCOUNT_GUARD_LIMIT", "40"))
    ...
    *,
    ...
    for fragment in fragments:
        for atom, value in atoms.items():
            ...
            if scope.startswith("textChain"):
                fragment.capsule[f"textChainScope:{scope}"] += 0.26 * weight
    return added


def addTextualChainAtoms(
    rows: list[Fragment],
    *,
    scope: str,
    weight: float,
    ...
    for index, fragment in enumerate(rows):
        ...
        added = addEventAtoms([fragment], hits, weight=weight, scope=scope)
        if added:
            stats[f"{scope}Spans"] += 1
            stats[f"{scope}Atoms"] += added
            stats["textChainSpans"] += 1
            stats["textChainAtoms"] += added
    return stats
...
def addParentEventExperience(fragments: list[Fragment]) -> Counter[str]:
    ...
    for rows in blockGroups.values():
        chainStats = addTextualChainAtoms(
            rows,
            scope="textChainBlock",
            weight=1.0,
            maxOrderDistance=0,
        )
        stats.update(chainStats)
        if chainStats.get("textChainBlockAtoms"):
            stats["textChainBlockGroups"] += 1

    for rows in filingGroups.values():
        chainStats = addTextualChainAtoms(
            rows,
            scope="textChainFiling",
            weight=0.8,
            maxOrderDistance=2,
        )
        stats.update(chainStats)
        if chainStats.get("textChainFilingAtoms"):
            stats["textChainFilingGroups"] += 1
    ...
    for rows in sectionGroups.values():
        ...
        for index, fragment in enumerate(rows):
            ...
            if len(peers) < 2:
                stats["textChainSectionRejectedSpans"] += 1
                continue
            hits = mergedRoleHits(peers, maxPerRole=4)
            added = addEventAtoms([fragment], hits, weight=0.32, scope="textChainSection")
            if added:
                stats["textChainSectionSpans"] += 1
                stats["textChainSectionAtoms"] += added
                stats["textChainSpans"] += 1
                stats["textChainAtoms"] += added
    return stats
    ...
    for idx, vote in pool.items():
        ...
        if contradictions:
            ...
            if any(item.startswith(("flow:", "delta:", "deal:", "issueRedeem:")) for item in contradictions):
                capsuleScore -= 38.0
        if missingRoles:
            targetMisses = sum(1 for item in missingRoles if item.startswith("target:"))
            actionMisses = sum(1 for item in missingRoles if item.startswith("action:"))
            modifierMisses = sum(1 for item in missingRoles if item.startswith("modifier:"))
        ...
        row = SearchRow(
            ...
            residual=tuple(list(missingRoles) + list(residual)),
        )
...
def main() -> None:
    ...
    print("V197 textual experience chain capsule")
    print(
        ...
        f"textChainSpans={buildStats['textChainSpans']} textChainAtoms={buildStats['textChainAtoms']} "
        f"textChainBlockGroups={buildStats['textChainBlockGroups']} "
        f"textChainSectionSpans={buildStats['textChainSectionSpans']} "
        f"textChainSectionRejectedSpans={buildStats['textChainSectionRejectedSpans']} "
        f"textChainFilingGroups={buildStats['textChainFilingGroups']} "
