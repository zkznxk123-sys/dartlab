"""Horizon Meaning Learner V199 - target-cohort residual ledger capsule.

아이디어:
    V197 은 textual chain 을 capsule/postings 의 1 차 proof 로 직접 넣어 broad 경험이 의미를 덮었다.
    V198 은 ledger 를 별도 필드로 분리했지만, target/object cohort 없이 block/report peer role 전체를 ledger 에
    적어 87 만 atom 규모의 넓은 경험장이 생겼고 V196 의 8/8 을 회복하지 못했다.

    V199 는 local target object 를 먼저 의미의 중심으로 고정한다. ledger 는 target 이 local span 에 있는 fragment 에만
    붙고, 같은 target cohort 안에서 빠진 action/modifier 를 보강하는 용도로만 쓰인다. target 자체는 ledger 로 보강하지
    않는다. 수평선 좌표는 빠른 주소이고, 의미는 local target 경험 + target-cohort residual 충족 여부다.
    ...
    - 같은 block/report 안에서 target/action/modifier 가 떨어져 있으면 event atom 을 capsule 에 넣지 않는다.
    - local target 이 있는 fragment 만 target-cohort ledger 를 갖는다. peer target 이 다르면 버리고, peer target 이
      없으면 같은 block/report 안의 action/modifier 보조 경험으로만 쓴다.
    - ranker 는 target 이 local capsule 에 없으면 강하게 낮춘다. action/modifier missing 만 ledger 로 보강한다.
...
실행 코드:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV199Test.py
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV199Test.py
    $env:DARTLAB_HORIZON_V199_PANEL_FILE_LIMIT='220'; $env:DARTLAB_HORIZON_V199_ALL_FILINGS_FILE_LIMIT='32'; uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV199Test.py
...
검증 기준:
    ...
    - rubric 은 semantic hit, completeRole, exact-only, contradiction, boilerplate, target-cohort ledger 사용 여부를 본다.
    - V196 의 capsuleTop1Semantic=8/8 을 회복하면서 completeRole 7/8 을 넘길 수 있는지 본다.

결과:
    미실행. 실행 후 이 영역에 수치를 기록한다.

결론:
    미실행. 실행 후 성공/실패를 기록한다.
"""
...
PANEL_FILE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V199_PANEL_FILE_LIMIT", "120"))
PANEL_ROWS_PER_FILE = int(os.environ.get("DARTLAB_HORIZON_V199_PANEL_ROWS_PER_FILE", "180"))
PANEL_SPANS_PER_BLOCK = int(os.environ.get("DARTLAB_HORIZON_V199_PANEL_SPANS_PER_BLOCK", "4"))
PANEL_SPAN_CHARS = int(os.environ.get("DARTLAB_HORIZON_V199_PANEL_SPAN_CHARS", "620"))
ALL_FILINGS_FILE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V199_ALL_FILINGS_FILE_LIMIT", "18"))
ALL_FILINGS_ROWS_PER_FILE = int(os.environ.get("DARTLAB_HORIZON_V199_ALL_FILINGS_ROWS_PER_FILE", "28"))
ALL_FILINGS_CHARS = int(os.environ.get("DARTLAB_HORIZON_V199_ALL_FILINGS_CHARS", "2600"))
ALL_FILINGS_SPANS_PER_ROW = int(os.environ.get("DARTLAB_HORIZON_V199_ALL_FILINGS_SPANS_PER_ROW", "3"))
MAX_FRAGMENTS = int(os.environ.get("DARTLAB_HORIZON_V199_MAX_FRAGMENTS", "22000"))
POSTING_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V199_POSTING_ATOM_LIMIT", "360"))
CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V199_CANDIDATE_LIMIT", "900"))
TOP_K = int(os.environ.get("DARTLAB_HORIZON_V199_TOP_K", "5"))
ACCOUNT_GUARD_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V199_ACCOUNT_GUARD_LIMIT", "40"))
...
def localTargetKeys(fragment: Fragment) -> set[str]:
    return roleKeySet(fragment.capsule, "target")


def targetCohortPeerAllowed(anchor: Fragment, peer: Fragment, *, maxOrderDistance: int) -> bool:
    if anchor.idx == peer.idx:
        return False
    if "role:negation:noData" in anchor.capsule or "role:negation:noData" in peer.capsule:
        return False
    ...
    if not directionCompatible(anchor, peer):
        return False
    anchorTargets = localTargetKeys(anchor)
    if not anchorTargets:
        return False
    peerHits = capsuleRoleHits(peer.capsule)
    peerTargets = peerHits.get("target", set())
    if peerTargets and not (anchorTargets & peerTargets):
        return False
    if not (peerHits.get("action", set()) or peerHits.get("modifier", set()) or peerHits.get("statement", set())):
        return False
    if peerTargets & anchorTargets:
        return True
    if localStitchTerms(anchor) & localStitchTerms(peer):
        return True
    anchorStatements = capsuleRoleHits(anchor.capsule).get("statement", set())
    peerStatements = peerHits.get("statement", set())
    if anchorStatements and anchorStatements & peerStatements:
        return True
    if anchor.block and anchor.block == peer.block:
        return True
    if anchor.source.startswith("allFilings") and anchor.reportNm == peer.reportNm:
        return True
    return False
...
def targetCohortLedgerAtoms(
    ...
    *,
    ...
    for peer in peers:
        hits = capsuleRoleHits(peer.capsule)
        actions.update(hits.get("action", set()))
        modifiers.update(hits.get("modifier", set()))
        statements.update(hits.get("statement", set()))
    directionalKeys = actions | modifiers
    for _group, keys in CONTRAST_GROUPS:
        active = directionalKeys & set(keys)
        if len(active) > 1:
            actions.difference_update(active)
            modifiers.difference_update(active)
    for target in sorted(targets):
        atoms[f"ledgerCohort:{target}"] += 0.7 * weight
        for action in sorted(actions):
            atoms[f"ledger:action:{action}"] += weight
            atoms[f"ledgerEvent:targetAction:{target}|{action}"] += 2.2 * weight
        for modifier in sorted(modifiers):
            atoms[f"ledger:modifier:{modifier}"] += 0.85 * weight
            atoms[f"ledgerEvent:targetModifier:{target}|{modifier}"] += 1.2 * weight
        for statement in sorted(statements):
            atoms[f"ledger:statement:{statement}"] += 0.65 * weight
            atoms[f"ledgerEvent:targetStatement:{target}|{statement}"] += 0.8 * weight
    atoms[f"ledgerScope:{scope}"] += 0.35 * weight
    return atoms
...
def addTargetCohortLedgerEdges(
    rows: list[Fragment],
    *,
    scope: str,
    weight: float,
    ...
    for index, fragment in enumerate(rows):
        ...
        peers = [
            ...
            if targetCohortPeerAllowed(fragment, other, maxOrderDistance=maxOrderDistance)
        ]
        if not peers:
            stats[f"{scope}RejectedSpans"] += 1
            continue
        atoms = targetCohortLedgerAtoms(targets, peers, weight=weight, scope=scope)
        added = 0
        for atom, value in atoms.items():
        ...
        if added:
            ...
            stats["targetCohortLedgerSpans"] += 1
            stats["targetCohortLedgerAtoms"] += added
    return stats


def addTargetCohortLedger(fragments: list[Fragment]) -> Counter[str]:
    stats: Counter[str] = Counter()
    ...
    for rows in blockGroups.values():
        ledgerStats = addTargetCohortLedgerEdges(
            rows,
            scope="targetLedgerBlock",
            weight=1.0,
            maxOrderDistance=0,
        )
        stats.update(ledgerStats)
        if ledgerStats.get("targetLedgerBlockAtoms"):
            stats["targetCohortLedgerBlockGroups"] += 1

    for rows in filingGroups.values():
        ledgerStats = addTargetCohortLedgerEdges(
            rows,
            scope="targetLedgerFiling",
            weight=0.8,
            maxOrderDistance=2,
        )
        stats.update(ledgerStats)
        if ledgerStats.get("targetLedgerFilingAtoms"):
            stats["targetCohortLedgerFilingGroups"] += 1
    ...
    for rows in sectionGroups.values():
        ...
        for index, fragment in enumerate(rows):
            ...
            targets = localTargetKeys(fragment)
            if not targets:
                stats["targetCohortLedgerSectionNoTargetSpans"] += 1
                continue
            peers = [other for other in nearby if targetCohortPeerAllowed(fragment, other, maxOrderDistance=1)]
            if not peers:
                stats["targetCohortLedgerSectionRejectedSpans"] += 1
                continue
            atoms = targetCohortLedgerAtoms(targets, peers, weight=0.32, scope="targetLedgerSection")
            added = 0
            for atom, value in atoms.items():
                if ledgerAtomContradicts(fragment, atom):
                    fragment.ledger["ledgerConflictSkipped:targetLedgerSection"] += 0.1
                    continue
                fragment.ledger[atom] += value
                added += 1
            if added:
                stats["targetCohortLedgerSectionSpans"] += 1
                stats["targetCohortLedgerSectionAtoms"] += added
                stats["targetCohortLedgerSpans"] += 1
                stats["targetCohortLedgerAtoms"] += added
    return stats
...
def buildFragments() -> tuple[list[Fragment], dict[str, Counter[str]], Counter[str]]:
    ...
    for index, fragment in enumerate(fragments):
        fragment.idx = index
    ledgerStats = addTargetCohortLedger(fragments)
    stats = Counter()
...
def directionContradictions(qCapsule: Counter[str], fCapsule: Counter[str]) -> tuple[str, ...]:
    contradictions: list[str] = []
    for group, keys in CONTRAST_GROUPS:
        qKeys = {key for key in keys if f"role:action:{key}" in qCapsule or f"role:modifier:{key}" in qCapsule}
        if not qKeys:
        ...
        if f"state:{group}:mixed" in fCapsule:
            contradictions.append(f"direction:{group}:{'/'.join(sorted(qKeys))}!=mixed")
            continue
        foreign = fKeys - qKeys
        if foreign:
            contradictions.append(f"direction:{group}:{'/'.join(sorted(qKeys))}!={'/'.join(sorted(foreign))}")
    return tuple(contradictions[:4])
...
def ledgerSupportedMissing(
    ...
    ledger: Counter[str],
    *,
    localTargetOverlap: bool,
) -> tuple[str, ...]:
    if not localTargetOverlap:
        return ()
    supported: list[str] = []
...
def rankRows(
    ...
    for idx, vote in pool.items():
        ...
        contradictions = tuple(
            dict.fromkeys(contrastAtoms(qCapsule, fCapsule) + directionContradictions(qCapsule, fCapsule))
        )
        ...
        supportedMissing = ledgerSupportedMissing(
            missingRoles,
            fragment.ledger,
            localTargetOverlap=bool(qTargets & fTargets),
        )
        ...
        if contradictions:
            capsuleScore -= len(contradictions) * 12.0
            capsuleScore -= sharedScore * 0.58
            if any(
                item.startswith(("flow:", "delta:", "deal:", "issueRedeem:", "direction:")) for item in contradictions
            ):
                capsuleScore -= 80.0
        if unsupportedMissing:
            ...
            capsuleScore -= targetMisses * 18.0
            capsuleScore -= actionMisses * 8.5
            capsuleScore -= modifierMisses * 5.0
            capsuleScore -= sharedScore * (0.42 if targetMisses else 0.22)
        if supportedMissing:
...
def semanticEvidence(probe: Probe, row: SearchRow) -> dict[str, object]:
    ...
    for expected in probe.expected:
        ...
        ledgerHit = role != "target" and key in ledgerRoleKeySet(row.fragment.ledger, role)
...
def main() -> None:
    ...
    print("V199 target-cohort residual ledger capsule")
    print(
        ...
        f"targetCohortLedgerSpans={buildStats['targetCohortLedgerSpans']} "
        f"targetCohortLedgerAtoms={buildStats['targetCohortLedgerAtoms']} "
        f"targetCohortLedgerBlockGroups={buildStats['targetCohortLedgerBlockGroups']} "
        f"targetCohortLedgerSectionSpans={buildStats['targetCohortLedgerSectionSpans']} "
        f"targetCohortLedgerSectionRejectedSpans={buildStats['targetCohortLedgerSectionRejectedSpans']} "
        f"targetCohortLedgerFilingGroups={buildStats['targetCohortLedgerFilingGroups']} "
