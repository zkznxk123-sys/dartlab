"""Horizon Meaning Learner v20 — automatic role clusters and answer skeleton.
...
---------
기본 샘플 실행:
    uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV20Test.py
...
- allFilings 는 section_content/content_html/content_xml/content_raw fallback 으로 읽고,
  docs 와 합쳐 24,636 units(allFilings 4,636 + docs 20,000) 를 만들었다.
- build: stems 42,745, postings 647,818, assocEdges 1,677,
  autoRoleFrameSlotHits 18,559, autoCoreAnchorHits 13,404,
  roughMemoryMb 31.8, buildSeconds 26.6.
- strict rubric: exact 57, fire 57, assoc 64, proxySlot 78, lexSlot 72,
  autoSlot 70, prunedAutoSlot 72, supportAutoSlot 72, triggerSupportSlot 58,
  cohortSlot 62, roleFrameSlot 64, meaningLearner(autoRoleFrameSlot+diversity) 71.
- meaningLearner 는 rubric improved/same/worsened = 6/1/1, avg 20.28ms,
  duplicateTop5 0, tableTop5 2, learnedRoleClusters 18, answerSkeletons 8 이다.

결론:
수동 role marker 없이 query stem 별 association/fire 근접도와 contrastive cohort 로 만든
자동 role cluster 만으로 exact 대비 +14, cohortSlot 대비 +9 strict rubric 을 만들었다.
v19 에 기록된 roleFrameSlot 71 과 같은 수준이고 hand lexSlot/supportAutoSlot 72 에 1점
차이라, "의미학습 -> 의미검색 -> 근거 skeleton" 루틴이 embedding 없이 실제로 연결된
첫 신호다. answer skeleton 은 8개 query 모두에서 core anchor, learned role cluster,
evidence unit 을 한 줄 frame 으로 조립했다.

남은 한계:
role cluster 안에 표/재무항목/범용 사업 stem 이 섞인다. 특히 유상증자 목적, 매출채권 회수
지연, 배당 지급은 관련 evidence 를 찾지만 cluster purity 가 낮고 skeleton 은 아직 자연어 답변이
아니라 근거 조립 frame 이다. 다음 단계는 cluster term 을 role coverage 와 relation pair 로
정제하고, 같은 corp/report 반복뿐 아니라 같은 공시 양식 문장 반복을 줄이는 diversity gate 를
추가하는 것이다.
"""
...
def buildAnswerSkeleton(
    ...
    evidence = " || ".join(evidenceBits) or "-"
    answerFrame = (
        f"query='{spec.query}' core='{coreAnchors}' roles='{roleClusters}' "
        f"claim='top evidence units connect the query core with learned role clusters' "
        f"evidenceCount={len(rows[:3])}"
    )
    return f"[skeleton] {answerFrame} evidence={evidence}"


def evaluateExperiment(model: Model) -> None:
    print("\n" + "=" * 88)
    print(
        "[evaluation] exact-only vs exact+fire vs exact+association vs proxySlot vs lexSlot vs autoSlot vs prunedAutoSlot vs supportAutoSlot vs triggerSupportSlot vs cohortSlot vs roleFrameSlot vs meaningLearner(autoRoleFrameSlot+diversity)"
    )
    print("=" * 88)
    ...
    learnedRoleClusters = sum(len(groups) for groups in model.queryAutoRoleFrameSlotGroups.values())
    answerSkeletons = 0
    ...
    for spec in PROBE_QUERIES:
        ...
        print(
            buildAnswerSkeleton(
                model,
                spec,
                modes["autoRoleFrameSlot"]["rows"],  # type: ignore[arg-type]
            )
        )
        answerSkeletons += 1
    ...
    print(
        ...
        f"learnedRoleClusters={learnedRoleClusters} answerSkeletons={answerSkeletons}"
    )


def main() -> None:
    print("=" * 88)
    print("Horizon Meaning Learner v20")
    print("=" * 88)
    print(
