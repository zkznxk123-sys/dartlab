"""Horizon Meaning Learner v21 — relation-purified role clusters.
...
---------
기본 샘플 실행:
    uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV21Test.py
...
- allFilings 는 section_content/content_html/content_xml/content_raw fallback 으로 읽고,
  docs 와 합쳐 24,862 units(allFilings 4,862 + docs 20,000) 를 만들었다.
- build: stems 43,231, postings 653,133, assocEdges 1,678,
  autoRoleFrameSlotHits 16,457, purifiedRoleFrameSlotHits 16,102,
  autoCoreAnchorHits 13,487, roughMemoryMb 32.2, buildSeconds 40.5.
- strict rubric: exact 57, fire 57, assoc 64, proxySlot 78, lexSlot 72,
  ...
  purifiedMeaningLearner 72.
- purifiedMeaningLearner 는 rubric improved/same/worsened = 6/2/0,
  avg 24.40ms, duplicateTop5 0, tableTop5 2, answerSkeletons 8 이다.
- cluster 정제: learnedRoleClusters 18, learnedRoleTerms 216, learnedFormLikeTerms 30 에서
  purifiedRoleClusters 18, purifiedRoleTerms 137, purifiedFormLikeTerms 0 으로 줄었다.

결론:
relation pair 와 core anchor 동시 support 로 auto role cluster 를 정제하면 strict rubric 이
71 -> 72 로 올라가 hand lexSlot/supportAutoSlot 과 동률이 된다. 동시에 role term 을 37% 줄이고
form-like term 을 0 으로 만들었으므로, "의미학습" 단계가 단순 확장어 수집이 아니라
role purity 를 학습하는 방향으로 한 단계 전진했다. answer skeleton 도 8개 query 모두
정제 cluster 기준으로 만들어졌다.

남은 한계:
form-like term 은 줄었지만 반도체 query 의 사업부문 stem, 매출채권 query 의 금융자산 stem 처럼
도메인상 넓은 관련어가 여전히 섞인다. 또 purified mode 는 avg 24.40ms 로 v20 보다 약간 느리다.
다음 단계는 role cluster 를 object/action/change/risk/purpose 로 명명하지 않고도 role 간
관계 방향을 추론하는 answer frame builder 를 붙여, skeleton 을 "근거 묶음" 에서
"대상-행동-변화-근거" 구조로 끌어올리는 것이다.
"""
    ...
    for spec in PROBE_QUERIES:
        ...
        print(
            buildAnswerSkeleton(
                model,
                spec,
                modes["purifiedMeaningLearner"]["rows"],  # type: ignore[arg-type]
            )
        )
        answerSkeletons += 1
...
def main() -> None:
    print("=" * 88)
    print("Horizon Meaning Learner v21")
    print("=" * 88)
    print(
