"""Horizon Meaning Learner v22 — evidence-grounded answer frames.
...
---------
기본 샘플 실행:
    uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV22Test.py
...
- build: stems 43,231, postings 653,133, assocEdges 1,678,
  ...
  autoCoreAnchorHits 13,487, roughMemoryMb 32.2, buildSeconds 38.4.
- strict rubric: exact 57, fire 57, assoc 64, proxySlot 78, lexSlot 72,
  ...
  cohortSlot 62, roleFrameSlot 64, meaningLearner 71, purifiedMeaningLearner 72.
- purifiedMeaningLearner 는 rubric improved/same/worsened = 6/2/0,
  avg 25.12ms, duplicateTop5 0, tableTop5 2 이다.
- answer frame: answerFrames 8, fullAnswerFrames 8, framedTargets 8,
  framedRelations 8, frameCompletenessAvg 3.00, frameConfidenceAvg 1.00.
- cluster 정제 상태는 v21 과 동일하게 learnedRoleTerms 216 -> purifiedRoleTerms 137,
  learnedFormLikeTerms 30 -> purifiedFormLikeTerms 0 이다.

결론:
v22 는 검색 품질을 떨어뜨리지 않고 v21 의 hand-slot 수준 strict rubric 72 를 유지하면서,
정제 role cluster 와 evidence unit 을 target/action/change/risk/purpose/evidence frame 으로
조립했다. 8개 probe 모두 target, relation slot, evidence 가 채워졌으므로
"의미학습 -> 의미검색 -> 의미생성 skeleton" 의 마지막 단계를 단순 skeleton 에서
구조화 answer frame 으로 끌어올렸다.

남은 한계:
answer frame 의 target 후보에는 반도체 query 의 화합물, 환율 query 의 국제곡물처럼
도메인상 관련은 있지만 답변 대상이라기엔 넓은 stem 이 섞인다. confidence 도 현재 슬롯
충족도 기반이라 모두 1.00 으로 포화된다. 다음 단계는 frame confidence 를 rubric/evidence
강도와 연결하고, target slot 을 core anchor 중심으로 더 보수적으로 제한하는 것이다.
"""
...
def buildEvidenceGroundedAnswerFrame(
    ...
    for term in queryTerms | clusterTerms:
        ...
        if slot != "target" and term not in queryTerms and term not in evidenceSurface:
            continue
        if slot != "target" and score < 5.0:
            continue
        if score <= 0:
            continue
        candidates[slot][term] += score
    if not candidates["target"]:
        for anchor in model.queryAutoCoreAnchorTerms.get(spec.query, ()):
            if slotForTerm(anchor) == "target":
                candidates["target"][anchor] += 3.0
    evidence: list[str] = []
    ...
    target = selectSlotTerms(candidates, "target", 3)
    ...
    confidence = min(1.0, (len(target) * 0.20) + (relationSlots * 0.18) + (len(evidence) * 0.10))
    return AnswerFrame(
        query=spec.query,
        target=target,
        action=action,
...
def main() -> None:
    print("=" * 88)
    print("Horizon Meaning Learner v22")
    print("=" * 88)
    print(
