"""Horizon Meaning Learner v24 — slot confidence and compressed answer skeleton.
...
---------
기본 샘플 실행:
    uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV24Test.py
...
- build: stems 59,905, postings 1,022,451, assocEdges 1,698,
  ...
  autoCoreAnchorHits 24,357, roughMemoryMb 47.9, buildSeconds 60.8.
...
- purifiedMeaningLearner 는 rubric improved/same/worsened = 3/5/0,
  avg 32.07ms, duplicateTop5 0, tableTop5 1 이다.
...
- answer skeleton: answerSkeletons 8, slotConfidenceAvg 0.92,
  weakSlotFrames 0, skeletonCharsAvg 162.5.
- 매출채권 회수 지연 probe 는 반례다. evidenceRubric 3, strongHits 0,
  frameConfidence 0.67 로 낮아졌지만 slotConfidence 는 target/action/change 가
  0.81/0.81/0.60 으로 남아 surface 기반 slot confidence 가 evidence 약함을 충분히
  반영하지 못했다.

결론:
v24 는 v23 frame 을 target/action/change/risk/purpose slot confidence 와 압축 answer
skeleton 으로 접었다. 8개 probe 모두 skeleton 을 만들었고, 좋은 probe 에서는
"대상-관계-근거강도-slotConf-frameConf-근거" 형태가 바로 답변 골격으로 읽힌다. 다만
검색 품질을 올리는 실험이 아니라 생성 skeleton 검증 실험이므로, 의미검색 지표 자체는
현재 40,000 unit cap 의 표본 구성 영향을 받는다.

남은 한계:
slotConfidence 가 term surface support 에 강하게 반응해 evidenceStrongHits 0 인 frame 에서도
높게 남을 수 있다. 다음 단계는 skeleton 생성을 evidenceStrongHits 또는 evidenceRubric 으로
gate 하고, slotConfidence 계산에서 evidenceScore 가 더 강하게 작동하도록 보정하는 것이다.
"""
...
def slotConfidenceValue(
    ...
    return round(min(0.99, (0.62 * surfaceScore) + (0.38 * evidenceScore)), 2)
...
def buildSlotConfidence(
    ...
    return (
        ("target", slotConfidenceValue(candidates, "target", target, evidenceScore)),
        ("action", slotConfidenceValue(candidates, "action", action, evidenceScore)),
        ("change", slotConfidenceValue(candidates, "change", change, evidenceScore)),
        ("risk", slotConfidenceValue(candidates, "risk", risk, evidenceScore)),
        ("purpose", slotConfidenceValue(candidates, "purpose", purpose, evidenceScore)),
    )
...
def buildEvidenceGroundedAnswerFrame(
    ...
    skeleton = buildCompressedAnswerSkeleton(
        target,
        action,
        change,
        risk,
...
def main() -> None:
    print("=" * 88)
    print("Horizon Meaning Learner v24")
    print("=" * 88)
    print(
