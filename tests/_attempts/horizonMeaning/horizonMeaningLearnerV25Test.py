"""Horizon Meaning Learner v25 — evidence-gated answer skeleton.
...
---------
기본 샘플 실행:
    uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV25Test.py
...
- build: stems 59,905, postings 1,022,451, assocEdges 1,698,
  ...
  autoCoreAnchorHits 24,357, roughMemoryMb 47.9, buildSeconds 66.2.
...
- purifiedMeaningLearner 는 rubric improved/same/worsened = 3/5/0,
  avg 31.22ms, duplicateTop5 0, tableTop5 1 이다.
- answer frame: answerFrames 8, fullAnswerFrames 8, framedTargets 8,
  framedRelations 8, frameCompletenessAvg 3.00, frameConfidenceAvg 0.88,
  frameEvidenceRubricAvg 5.62, frameStrongHitAvg 2.62.
- evidence-gated answer skeleton: answerSkeletons 8, reliableSkeletons 7,
  weakEvidenceSkeletons 1, slotConfidenceAvg 0.85, weakSlotFrames 1,
  skeletonCharsAvg 170.4.
- v24 반례였던 매출채권 회수 지연은 evidenceRubric 3, strongHits 0,
  evidenceGate 0.20, skeletonStatus weak-evidence 로 분리됐다. slotConfidence 도
  target/action/change = 0.27/0.27/0.20 으로 낮아져 skeleton 에서는 relation 을
  확정하지 않고 관계=- 로 출력했다.

결론:
v25 는 v24 의 "surface slot 은 강한데 근거가 약한 skeleton" 문제를 evidence gate 로 분리했다.
검색 지표는 v24 와 같은 계열을 유지하면서, reliable skeleton 을 7개와 weak-evidence skeleton
1개로 나눠 답변 가능한 frame 과 보류해야 할 frame 을 구분했다. 이는 embedding 없이 만든
answer skeleton 이 근거 품질과 연결될 수 있다는 검증 단계다.

남은 한계:
weak-evidence skeleton 은 이제 표시되지만 검색 자체가 그 query 에서 좋은 근거를 다시 찾지는
못한다. 다음 단계는 weak-evidence 로 판정된 query 에 한해 fallback retrieval 을 실행해
role frame 후보를 넓히거나, table/주석 잡음을 더 강하게 감점해 strong evidence 를 회복하는 것이다.
"""
...
def main() -> None:
    print("=" * 88)
    print("Horizon Meaning Learner v25")
    print("=" * 88)
    print(
