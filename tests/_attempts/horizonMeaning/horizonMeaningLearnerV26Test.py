"""Horizon Meaning Learner v26 — weak-evidence fallback retrieval.
...
---------
기본 샘플 실행:
    uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV26Test.py
...
- build: stems 59,905, postings 1,022,451, assocEdges 1,698,
  ...
  autoCoreAnchorHits 24,357, roughMemoryMb 47.9, buildSeconds 60.4.
...
- purifiedMeaningLearner 는 rubric improved/same/worsened = 3/5/0,
  avg 32.53ms, duplicateTop5 0, tableTop5 1 이다.
...
- answer skeleton: answerSkeletons 8, reliableSkeletons 8, weakEvidenceSkeletons 0,
  slotConfidenceAvg 0.88, weakSlotFrames 0, skeletonCharsAvg 175.5.
- 매출채권 회수 지연은 purifiedMeaningLearner 기준 evidenceRubric 3, strongHits 0,
  skeletonStatus weak-evidence 였지만 supportAutoSlotFallback 으로 evidenceRubric 4,
  strongHits 1, evidenceGate 0.47, skeletonStatus reliable 로 회복했다.

결론:
v26 은 weak-evidence 로 분리된 query 에만 fallback retrieval 을 적용해 reliable skeleton 을
7개에서 8개로 늘렸다. fallback 은 query relation pair posting 과 support slot 후보를 다시
탐색했고, 매출채권 회수 지연 probe 에서 strong evidence 1개를 top-3 안에 회복했다. 이로써
"약한 답변 골격을 표시만 하는 단계" 에서 "약한 query 만 재탐색해 답변 가능 상태로 올리는 단계" 로
한 칸 전진했다.

남은 한계:
회복된 매출채권 frame 은 top-3 안에 strong evidence 를 포함하지만 첫 evidence snippet 은 여전히
약한 거래내용 조각이다. skeleton 의 근거 문장은 현재 첫 row 를 그대로 쓰므로, 다음 단계는
fallback 이후 evidence row 자체를 evidence-first 로 재정렬하거나 대표 snippet 을 strong row 에서
뽑는 것이다.
"""
...
def main() -> None:
    print("=" * 88)
    print("Horizon Meaning Learner v26")
    print("=" * 88)
    print(
