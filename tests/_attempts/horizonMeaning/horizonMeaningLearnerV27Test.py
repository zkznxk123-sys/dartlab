"""Horizon Meaning Learner v27 — evidence-first skeleton representative.
...
---------
기본 샘플 실행:
    uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV27Test.py
...
- build: stems 59,905, postings 1,022,451, assocEdges 1,698,
  ...
  autoCoreAnchorHits 24,357, roughMemoryMb 47.9, buildSeconds 59.9.
...
- purifiedMeaningLearner 는 rubric improved/same/worsened = 3/5/0,
  avg 31.65ms, duplicateTop5 0, tableTop5 1 이다.
...
- answer skeleton: answerSkeletons 8, reliableSkeletons 8, weakEvidenceSkeletons 0,
  slotConfidenceAvg 0.88, weakSlotFrames 0, skeletonCharsAvg 173.0.
- 매출채권 회수 지연은 supportAutoSlotFallback 후 primaryEvidenceRank 2,
  primaryEvidenceRubric 2 로 대표 근거가 바뀌었고, skeleton 근거가
  "거래상대방의 재무상태 변화에 따른 회수위험" 문장으로 교체되었다.

결론:
v27 은 검색 결과 top-3 자체를 바꾸지 않고도 answer skeleton 의 대표 근거 선택을 개선했다.
v26 에서 회복된 strong evidence 가 top-3 안에 있었지만 첫 row 때문에 skeleton 이 약한 거래내용
조각을 인용하던 문제가 해소됐다. 특히 매출채권 회수 지연 probe 에서 대표 evidence rank 가
1에서 2로 이동했고, skeleton 의 근거 문장이 실제 회수위험 문장으로 바뀌었다. 따라서
"근거 있는 검색" 다음 단계로 "근거를 답변에 제대로 붙이는 단계" 가 필요하다는 점과, 그 단계가
embedding/LLM 없이도 별도 scoring 으로 가능하다는 신호를 확인했다.

남은 한계:
primary evidence scoring 은 top-3 내부 대표 선택일 뿐 검색 recall 을 새로 만들지는 않는다.
또한 반도체 HBM 투자처럼 rubric 상 strong 이지만 snippet 이 HBM 보다 넓은 반도체/투자 문장을
대표로 고르는 경우가 있어, 다음 단계는 query hard term coverage 와 specificity penalty 를 넣어
대표 근거가 query 핵심어를 더 촘촘히 포함하도록 만드는 것이다.
"""
...
def primaryEvidenceScore(model: Model, spec: QuerySpec, unitId: int, exact: list[str], semantic: list[str]) -> float:
    unit = model.units[unitId]
    surface = compactText(f"{unit.reportName} {unit.sectionTitle} {unit.text}")
...
def buildEvidenceGroundedAnswerFrame(
    ...
    *,
    ...
    for rank, (_, unitId, exact, semantic) in enumerate(rows[:3], start=1):
        ...
        evidenceRows.append((primaryEvidenceScore(model, spec, unitId, exact, semantic), rank, unitId, evidenceText))
...
def main() -> None:
    print("=" * 88)
    print("Horizon Meaning Learner v27")
    print("=" * 88)
    print(
