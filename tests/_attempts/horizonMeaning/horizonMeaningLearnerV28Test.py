"""Horizon Meaning Learner v28 — hard-term specificity gate.
...
---------
기본 샘플 실행:
    uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV28Test.py
...
- build: stems 59,905, postings 1,022,451, assocEdges 1,698,
  autoRoleFrameSlotHits 27,678, purifiedRoleFrameSlotHits 27,550,
  autoCoreAnchorHits 24,357, roughMemoryMb 47.9, buildSeconds 61.2.
- strict rubric: exact 66, fire 66, assoc 69, proxySlot 79, lexSlot 74,
  autoSlot 71, prunedAutoSlot 74, supportAutoSlot 74, triggerSupportSlot 67,
  cohortSlot 71, roleFrameSlot 70, meaningLearner 74, purifiedMeaningLearner 74.
- purifiedMeaningLearner 는 rubric improved/same/worsened = 3/5/0,
  avg 31.68ms, duplicateTop5 0, tableTop5 1 이다.
- answer frame after fallback: answerFrames 8, fullAnswerFrames 8, framedTargets 8,
  framedRelations 8, frameCompletenessAvg 3.00, frameConfidenceAvg 0.89,
  frameEvidenceRubricAvg 5.75, frameStrongHitAvg 2.75.
- hard-term specificity: primaryEvidenceHardCoverageAvg 0.94,
  hardTermFrames 8, hardTermCoveredFrames 7, broadEvidenceSkeletons 1.
- evidence-first representative: primaryEvidenceRubricAvg 2.00,
  primaryEvidenceStrongFrames 8, primaryEvidenceChanged 4.
- fallback: fallbackAttempts 1, fallbackApplied 1, fallbackRecovered 1,
  fallbackEvidenceRubricGain 1, fallbackStrongHitGain 1.
- answer skeleton: answerSkeletons 8, reliableSkeletons 7, weakEvidenceSkeletons 0,
  broadEvidenceSkeletons 1, slotConfidenceAvg 0.88, weakSlotFrames 0.
- 반도체 HBM 투자는 primaryEvidenceHardTerms hbm,반도체 중 hbm 이 빠져
  hardCoverage 0.50, skeletonStatus broad-evidence 로 분리됐다. relation 은 확정하지 않고
  missingHard=hbm 을 skeleton 에 남겼다.
- 매출채권 회수 지연은 v27 과 같이 supportAutoSlotFallback 으로 reliable 을 유지했고,
  primaryEvidenceRank 2, primaryEvidenceHardCoverage 1.00 으로 회수위험 문장을 대표 근거로 썼다.

결론:
v28 은 v27 의 answer skeleton 을 더 엄격하게 만들었다. strict rubric 기준 strong evidence 라도
대표 근거가 query hard term 을 빠뜨리면 reliable 로 확정하지 않고 broad-evidence 로 분리한다.
이로써 반도체 HBM 투자처럼 "반도체/투자" 는 맞지만 "HBM" 이 없는 넓은 문장을 근거로
언어모델처럼 확정 답변하는 과신을 막았다. 다른 7개 probe 는 reliable 을 유지해 specificity gate 가
과도하게 보수적으로 작동하지 않는 것도 확인했다.

남은 한계:
v28 은 broad-evidence 를 발견하고 관계 확정을 막지만, HBM 이 포함된 더 좋은 evidence 를 새로
찾지는 못한다. 다음 단계는 hard-term missing query 에 한정해 hbm posting, support term, relation
pair 를 다시 섞는 hard-term fallback retrieval 을 붙여 broad-evidence 를 reliable 로 회복할 수
있는지 검증하는 것이다.
"""
...
def fallbackFrameRank(frame: AnswerFrame) -> tuple[int, int, int, float]:
    reliable = int(frame.skeletonStatus == "reliable")
    return reliable, frame.evidenceStrongHits, frame.evidenceRubric, frame.confidence


def recoverWeakEvidenceFrame(model: Model, spec: QuerySpec, baseFrame: AnswerFrame) -> AnswerFrame:
    if baseFrame.skeletonStatus != "weak-evidence":
        return baseFrame
    bestFrame = baseFrame
    bestRank = fallbackFrameRank(baseFrame)
    for mode, rows in fallbackCandidateRows(model, spec):
        if not rows:
            continue
...
def buildFallbackLine(spec: QuerySpec, baseFrame: AnswerFrame, finalFrame: AnswerFrame) -> str:
    recovered = int(baseFrame.skeletonStatus == "weak-evidence" and finalFrame.skeletonStatus == "reliable")
    return (
...
def evaluateExperiment(model: Model) -> None:
    ...
    for spec in PROBE_QUERIES:
        ...
        frame = recoverWeakEvidenceFrame(model, spec, baseFrame)
        if baseFrame.skeletonStatus == "weak-evidence":
            fallbackAttempts += 1
...
def main() -> None:
    print("=" * 88)
    print("Horizon Meaning Learner v28")
    print("=" * 88)
    print(
