"""Horizon Meaning Learner v29 — hard-term fallback retrieval.
...
---------
기본 샘플 실행:
    uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV29Test.py
...
---------
2026-05-28 기본 샘플 실행.
- collect: units=40,000(allFilings 20,000 + docs 20,000), dupSkips=107,641.
- build: stems=59,905, postings=1,022,451, roughMemoryMb=47.9, buildSeconds=60.6.
- search: exactQuality=74, purifiedMeaningLearnerQuality=78, exactRubric=66,
  purifiedMeaningLearnerRubric=74, purifiedMeaningLearnerAvgMs=28.01,
...
- fallback: fallbackAttempts=2, fallbackApplied=2, fallbackRecovered=2.
  hardFallbackAttempts=1, hardFallbackApplied=1, hardFallbackRecovered=1,
  hardFallbackHardCoverageGain=1.00, hardFallbackMissingHardTermGain=1,
  hardFallbackSeedUnits=80, hardFallbackSeedFiles=39.
- HBM probe: "반도체 HBM 투자" 는 base purifiedMeaningLearner 에서
  hardCoverage=0.00, missingHard=hbm, skeletonStatus=broad-evidence 였고,
  hardTermFallback 이 SK하이닉스 HBM seed posting 을 찾아 hardCoverage=1.00,
  skeletonStatus=reliable 로 승격했다.

결론:
v28 의 specificity gate 가 만든 broad-evidence 상태를 실제 회복하는 첫 결과다.
capped sample 에 없는 HBM posting 을 fallback 직전에 원본 parquet 에서 소량 seed 로
주입하면, embedding 없이도 missing hard term posting + relation/support 신호만으로
근거 skeleton 을 broad-evidence 에서 reliable 로 바꿀 수 있었다. 특히 HBM probe 는
기존 넓은 반도체/투자 문장이 아니라 SK하이닉스의 HBM/투자 근거를 조합했다.

남은 한계:
hard-term seed scan 은 39개 docs 파일을 추가로 읽어 전체 실행 시간이 212.4초까지 늘었다.
또한 seed 는 fallback 시점에 model 을 동적으로 확장하므로 roughMemoryMb 는 seed 주입 전
모델 기준이고, HBM 대표 근거 1순위는 기술 설명 문장, 2순위가 투자 문장이라 relation
대표 snippet 선택은 아직 더 정교화해야 한다. 다음은 hard-term seed 를 build-time sparse
rare-term side index 로 바꾸고, fallback 대표 evidence 를 hard term + relation 동시 충족
순서로 재정렬하는 것이다.
"""
...
HARD_FALLBACK_SEED_UNITS_PER_TERM = int(os.environ.get("DARTLAB_HORIZON_HARD_FALLBACK_SEED_UNITS_PER_TERM", "80"))
HARD_FALLBACK_SEED_MAX_FILES = int(os.environ.get("DARTLAB_HORIZON_HARD_FALLBACK_SEED_MAX_FILES", "60"))
...
def main() -> None:
    print("=" * 88)
    print("Horizon Meaning Learner v29")
    print("=" * 88)
    print(
