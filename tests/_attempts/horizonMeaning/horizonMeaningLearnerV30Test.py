"""Horizon Meaning Learner v30 — build-time rare hard-term side index.
...
---------
기본 샘플 실행:
    uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV30Test.py
...
---------
2026-05-28 기본 샘플 실행.
- hard-side-index: terms=hbm, added=80, filesScanned=39, dupSkips=92.
- collect: units=40,080(allFilings 20,000 + docs 20,000 + hardSide 80),
  dupSkips=107,733, collectSeconds=153.9.
- build: stems=60,341, postings=1,027,960, roughMemoryMb=48.2, buildSeconds=59.2.
- search: exactQuality=74, purifiedMeaningLearnerQuality=79, exactRubric=65,
  purifiedMeaningLearnerRubric=73, purifiedMeaningLearnerAvgMs=28.23,
  purifiedMeaningLearnerDuplicateTop5=0, purifiedMeaningLearnerTableTop5=1.
- answer frame: answerFrames=8, reliableSkeletons=8, weakEvidenceSkeletons=0,
  broadEvidenceSkeletons=0, primaryEvidenceHardCoverageAvg=1.00,
  hardTermCoveredFrames=8/8.
- fallback: fallbackAttempts=1, fallbackRecovered=1, hardFallbackAttempts=0,
  hardSideIndexUnits=80, hardSideIndexFiles=39, hardFallbackSeedUnits=0.
- HBM probe: "반도체 HBM 투자" 는 fallback 없이 purifiedMeaningLearner 단계에서
  target=hbm,반도체, action=투자, hardCoverage=1.00, skeletonStatus=reliable 이 됐다.
  즉 v29 의 dynamic hardTermFallback 이 하던 HBM 회복을 build-time side index 가
  postingTable/stemTable 내부 신호로 앞당겼다.

결론:
build-time rare hard-term side index 는 HBM 같은 capped sample 밖 희귀 핵심어를
동적 model mutation 없이 기본 semantic retrieval 단계로 끌어오는 데 성공했다.
v29 와 달리 hardFallbackAttempts=0 이고, HBM skeleton 은 처음부터 reliable 로 조립됐다.
다만 side index 를 실행 중 원본 parquet scan 으로 만드는 방식은 collectSeconds=153.9,
totalSeconds=215.4 로 느려 실시간 검색 구조로는 부적합하다. 개념은 유효하지만 구현은
prebuilt rare-term side index 또는 파일별 term bloom/header index 로 옮겨야 한다.

남은 한계:
side index scan 이 docs 39개 파일을 직접 읽어 v29 의 fallback-time seed scan 비용을
build-time 으로 옮겼을 뿐 총 실행 시간을 줄이지 못했다. 또한 HBM 대표 evidence 는
hard term 설명 문장이 1순위가 되고 투자 문장은 top-3 안에 섞이는 수준이라, 다음은
prebuilt side index + hard term/relation 동시 충족 representative rerank 를 검증해야 한다.
"""
...
def buildEvidenceGroundedAnswerFrame(
    ...
    *,
    ...
    evidence = [evidenceText for _, _, _, evidenceText in rankedEvidenceRows]
    ...
    evidenceRubrics = [rubricQuality(model.units[unitId], spec) for _, _, unitId, _ in evidenceRows]
...
def main() -> None:
    print("=" * 88)
    print("Horizon Meaning Learner v30")
    print("=" * 88)
    print(
