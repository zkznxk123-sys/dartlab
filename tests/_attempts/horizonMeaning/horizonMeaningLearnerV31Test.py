"""Horizon Meaning Learner v31 — hard-role representative evidence.
...
22. horizonMeaningLearnerV23 은 v22 의 answer frame 에서 넓은 cluster-only target 과 confidence
    ...
    보정한다.
23. horizonMeaningLearnerV24 는 v23 frame 을 한 단계 더 압축해 slot별 confidence 와
    answer skeleton 을 만든다. skeleton 은 생성 모델이 아니라 target/action/change/risk/purpose
    slot, slot confidence, top evidence anchor 를 정해진 구조로 접은 결과다.
24. horizonMeaningLearnerV25 는 v24 반례였던 weak evidence skeleton 을 분리한다.
    slot confidence 에 strong evidence gate 를 곱하고, evidenceStrongHits/evidenceRubric 이 낮은
    frame 은 skeletonStatus=weak-evidence 로 표시해 reliable skeleton 집계에서 제외한다.
25. horizonMeaningLearnerV26 은 weak-evidence 로 판정된 query 에만 fallback retrieval 을 건다.
    fallback 은 query relation pair posting, support stem posting, section hint 를 다시 조합해
    top evidence 를 재탐색하고, strong evidence 를 회복하면 skeleton 을 reliable 로 승격한다.
26. horizonMeaningLearnerV27 은 top-3 evidence 를 첫 row 순서대로 skeleton 에 넣지 않고,
    query/relation/support/section/table 신호로 대표 evidence 를 먼저 고른다. 목표는 fallback 이
    strong evidence 를 회복했을 때 skeleton 근거 문장도 weak row 가 아니라 strong row 를 인용하는지
    검증하는 것이다.
27. horizonMeaningLearnerV28 은 대표 evidence 가 strict rubric 상 strong 이더라도 query 의
    hard term 을 놓치면 reliable skeleton 으로 확정하지 않는다. HBM 처럼 드문 핵심 대상 stem 을
    primary evidence specificity gate 로 분리해 broad-evidence 상태를 만들고, 다음 단계 fallback
    대상이 드러나는지 검증한다.
28. horizonMeaningLearnerV29 는 broad-evidence query 에 한정해 hard-term fallback retrieval 을 건다.
    missing hard term posting 을 강한 anchor 로 삼고, support stem, relation pair, section hint 를
    다시 조합해 HBM 같은 누락 핵심어를 포함한 근거 unit 을 회복할 수 있는지 검증한다. capped
    sample 에 hard term posting 이 없으면 fallback 직전에 원본 parquet 에서 해당 hard term unit 을
    소량 seed 로 주입해 full-corpus index 에서라면 존재했을 rare posting 을 재현한다.
29. horizonMeaningLearnerV30 은 v29 의 fallback-time seed scan 을 build-time rare hard-term
    side index 로 옮긴다. query/mustAny 에서 ASCII hard term 을 먼저 모으고, capped sample 과
    별도로 원본 parquet 에서 해당 term 의 meaning unit 을 소량 주입한 뒤 모델을 빌드한다.
    목표는 HBM 같은 희귀 핵심어를 동적 fallback mutation 없이 postingTable/stemTable 안에
    포함시키고, 기본 semantic retrieval 단계부터 reliable skeleton 으로 들어오는지 검증하는 것이다.
30. horizonMeaningLearnerV31 은 v30 의 HBM 회복 이후 남은 대표 근거 문제를 다룬다.
    query hard term 과 action/change/risk/purpose role term 사이의 동적 hard-role relation
    pair 를 자동 파생하고, 검색 점수와 primary evidence 선택에 반영한다. 목표는 HBM 설명 문장만
    대표로 고르는 것이 아니라 HBM 과 투자 같은 관계가 동시에 살아있는 meaning unit 을 skeleton
    대표 근거로 끌어올릴 수 있는지 검증하는 것이다.

실행 코드
---------
기본 샘플 실행:
    uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV31Test.py
...
ANSWER_FRAME_MIN_SLOT_CONFIDENCE = float(os.environ.get("DARTLAB_HORIZON_ANSWER_FRAME_MIN_SLOT_CONFIDENCE", "0.35"))
ANSWER_FRAME_EVIDENCE_POOL = int(os.environ.get("DARTLAB_HORIZON_ANSWER_FRAME_EVIDENCE_POOL", "5"))
ANSWER_SKELETON_EVIDENCE_CHARS = int(os.environ.get("DARTLAB_HORIZON_ANSWER_SKELETON_EVIDENCE_CHARS", "72"))
ANSWER_SKELETON_MIN_STRONG_HITS = int(os.environ.get("DARTLAB_HORIZON_ANSWER_SKELETON_MIN_STRONG_HITS", "1"))
ANSWER_SKELETON_MIN_EVIDENCE_RUBRIC = int(os.environ.get("DARTLAB_HORIZON_ANSWER_SKELETON_MIN_EVIDENCE_RUBRIC", "4"))
FALLBACK_POSTINGS_PER_STEM = int(os.environ.get("DARTLAB_HORIZON_FALLBACK_POSTINGS_PER_STEM", "9000"))
FALLBACK_PAIR_UNITS_PER_PAIR = int(os.environ.get("DARTLAB_HORIZON_FALLBACK_PAIR_UNITS_PER_PAIR", "400"))
FALLBACK_SECTION_HINT_WEIGHT = float(os.environ.get("DARTLAB_HORIZON_FALLBACK_SECTION_HINT_WEIGHT", "10.0"))
FALLBACK_TABLE_PENALTY = float(os.environ.get("DARTLAB_HORIZON_FALLBACK_TABLE_PENALTY", "18.0"))
PRIMARY_EVIDENCE_RELATION_WEIGHT = float(os.environ.get("DARTLAB_HORIZON_PRIMARY_EVIDENCE_RELATION_WEIGHT", "28.0"))
PRIMARY_EVIDENCE_SUPPORT_WEIGHT = float(os.environ.get("DARTLAB_HORIZON_PRIMARY_EVIDENCE_SUPPORT_WEIGHT", "8.0"))
PRIMARY_EVIDENCE_SECTION_WEIGHT = float(os.environ.get("DARTLAB_HORIZON_PRIMARY_EVIDENCE_SECTION_WEIGHT", "6.0"))
PRIMARY_EVIDENCE_TABLE_PENALTY = float(os.environ.get("DARTLAB_HORIZON_PRIMARY_EVIDENCE_TABLE_PENALTY", "12.0"))
PRIMARY_EVIDENCE_HARD_TERM_WEIGHT = float(os.environ.get("DARTLAB_HORIZON_PRIMARY_EVIDENCE_HARD_TERM_WEIGHT", "16.0"))
PRIMARY_EVIDENCE_MISSING_HARD_TERM_PENALTY = float(
    os.environ.get("DARTLAB_HORIZON_PRIMARY_EVIDENCE_MISSING_HARD_TERM_PENALTY", "30.0")
)
PRIMARY_EVIDENCE_HARD_ROLE_RELATION_WEIGHT = float(
    os.environ.get("DARTLAB_HORIZON_PRIMARY_EVIDENCE_HARD_ROLE_RELATION_WEIGHT", "42.0")
)
HARD_ROLE_RELATION_SEARCH_WEIGHT = float(os.environ.get("DARTLAB_HORIZON_HARD_ROLE_RELATION_SEARCH_WEIGHT", "18.0"))
PRIMARY_EVIDENCE_HARD_TERM_MAX_DF_RATIO = float(
    os.environ.get("DARTLAB_HORIZON_PRIMARY_EVIDENCE_HARD_TERM_MAX_DF_RATIO", "0.035")
)
PRIMARY_EVIDENCE_MIN_HARD_COVERAGE = float(os.environ.get("DARTLAB_HORIZON_PRIMARY_EVIDENCE_MIN_HARD_COVERAGE", "1.0"))
HARD_FALLBACK_POSTINGS_PER_STEM = int(os.environ.get("DARTLAB_HORIZON_HARD_FALLBACK_POSTINGS_PER_STEM", "16000"))
HARD_FALLBACK_SUPPORT_POSTINGS_PER_STEM = int(
    os.environ.get("DARTLAB_HORIZON_HARD_FALLBACK_SUPPORT_POSTINGS_PER_STEM", "5000")
)
HARD_FALLBACK_PAIR_UNITS_PER_PAIR = int(os.environ.get("DARTLAB_HORIZON_HARD_FALLBACK_PAIR_UNITS_PER_PAIR", "600"))
HARD_FALLBACK_HARD_TERM_WEIGHT = float(os.environ.get("DARTLAB_HORIZON_HARD_FALLBACK_HARD_TERM_WEIGHT", "95.0"))
HARD_FALLBACK_ALL_HARD_TERM_WEIGHT = float(os.environ.get("DARTLAB_HORIZON_HARD_FALLBACK_ALL_HARD_TERM_WEIGHT", "34.0"))
HARD_FALLBACK_MISSING_HARD_TERM_PENALTY = float(
    os.environ.get("DARTLAB_HORIZON_HARD_FALLBACK_MISSING_HARD_TERM_PENALTY", "70.0")
)
HARD_FALLBACK_RELATION_WEIGHT = float(os.environ.get("DARTLAB_HORIZON_HARD_FALLBACK_RELATION_WEIGHT", "34.0"))
HARD_FALLBACK_SECTION_HINT_WEIGHT = float(os.environ.get("DARTLAB_HORIZON_HARD_FALLBACK_SECTION_HINT_WEIGHT", "12.0"))
HARD_FALLBACK_TABLE_PENALTY = float(os.environ.get("DARTLAB_HORIZON_HARD_FALLBACK_TABLE_PENALTY", "18.0"))
HARD_SIDE_INDEX_UNITS_PER_TERM = int(os.environ.get("DARTLAB_HORIZON_HARD_SIDE_INDEX_UNITS_PER_TERM", "80"))
HARD_SIDE_INDEX_MAX_FILES = int(os.environ.get("DARTLAB_HORIZON_HARD_SIDE_INDEX_MAX_FILES", "60"))
HARD_SIDE_INDEX_STATS: Counter[str] = Counter()
HARD_FALLBACK_SEED_UNITS_PER_TERM = int(os.environ.get("DARTLAB_HORIZON_HARD_FALLBACK_SEED_UNITS_PER_TERM", "0"))
HARD_FALLBACK_SEED_MAX_FILES = int(os.environ.get("DARTLAB_HORIZON_HARD_FALLBACK_SEED_MAX_FILES", "0"))
HARD_FALLBACK_SEED_PAIR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_HARD_FALLBACK_SEED_PAIR_LIMIT", "500"))
HARD_FALLBACK_SEEDED_TERMS: set[str] = set()
HARD_FALLBACK_SEED_STATS: Counter[str] = Counter()
KOREAN_SUFFIXES = (
    "으로부터",
    "로부터",
    "에서는",
...
class AnswerFrame:
    ...
    purpose: tuple[str, ...]
    evidence: tuple[str, ...]
    primaryEvidenceRank: int
    primaryEvidenceRubric: int
    primaryEvidenceHardTerms: tuple[str, ...]
    primaryEvidenceMissingHardTerms: tuple[str, ...]
    primaryEvidenceHardCoverage: float
    primaryEvidenceHardRoleHits: int
    evidenceRubric: int
    evidenceStrongHits: int
    slotConfidence: tuple[tuple[str, float], ...]
    skeleton: str
    skeletonStatus: str
    evidenceGate: float
    retrievalMode: str
...
def hardSideIndexTerms() -> tuple[str, ...]:
    terms: list[str] = []
    for spec in PROBE_QUERIES:
        surfaces = (spec.query, *spec.mustAny)
        for surface in surfaces:
            for stem in extractStems(surface, cap=16):
                if any("a" <= char.lower() <= "z" for char in stem):
                    terms.append(stem.lower())
    return tuple(dict.fromkeys(terms))


def hardSideIndexUnitFromRow(rowSource: str, row: dict[str, object], text: str) -> Unit:
    return Unit(
        source=f"{rowSource}:hardSide",
        corpName=safeValue(row.get("corp_name")),
        stockCode=safeValue(row.get("stock_code")),
        rceptNo=safeValue(row.get("rcept_no")),
        rceptDate=safeValue(row.get("rcept_dt") or row.get("rcept_date")),
        reportName=safeValue(row.get("report_nm") or row.get("report_type")),
        sectionTitle=safeValue(row.get("section_title") or row.get("report_nm") or row.get("report_type")),
        text=text,
    )


def collectHardSideIndexUnits(seenUnitKeys: set[str]) -> tuple[list[Unit], Counter[str], int, int]:
    if HARD_SIDE_INDEX_UNITS_PER_TERM <= 0:
        return [], Counter(), 0, 0
    terms = hardSideIndexTerms()
    if not terms:
        return [], Counter(), 0, 0

    units: list[Unit] = []
    termCounts: Counter[str] = Counter()
    duplicateSkips = 0
    filesScanned = 0
    stopScan = False

    for iterator in (iterDocsRows(), iterAllFilingRows()):
        for rowSource, path, columns in iterator:
            if filesScanned >= HARD_SIDE_INDEX_MAX_FILES:
                stopScan = True
                break
            filesScanned += 1
            for row in readRows(rowSource, path, columns):
                content = hardTermRowContent(row)
                if not content:
                    continue
                surface = compactText(content)
                for term in terms:
                    if termCounts[term] >= HARD_SIDE_INDEX_UNITS_PER_TERM or term not in surface:
                        continue
                    for text in hardTermSeedUnits(content, term):
                        key = unitDedupeKey(text)
                        if key in seenUnitKeys:
                            duplicateSkips += 1
                            continue
                        seenUnitKeys.add(key)
                        units.append(hardSideIndexUnitFromRow(rowSource, row, text))
                        termCounts[term] += 1
                        if termCounts[term] >= HARD_SIDE_INDEX_UNITS_PER_TERM:
                            break
                if all(termCounts[term] >= HARD_SIDE_INDEX_UNITS_PER_TERM for term in terms):
                    stopScan = True
                    break
            if stopScan:
                break
    ...
    HARD_SIDE_INDEX_STATS["units"] += len(units)
    HARD_SIDE_INDEX_STATS["files"] += filesScanned
    HARD_SIDE_INDEX_STATS["duplicateSkips"] += duplicateSkips
    for term, count in termCounts.items():
        HARD_SIDE_INDEX_STATS[f"term:{term}"] += count
    counts = ",".join(f"{term}:{termCounts[term]}" for term in terms)
    print(
        f"[hard-side-index] terms={','.join(terms)} added={len(units)} "
        f"counts={counts or '-'} filesScanned={filesScanned} dupSkips={duplicateSkips}"
    )
    return units, termCounts, duplicateSkips, filesScanned
...
def collectUnits() -> list[Unit]:
    ...
    hardSideUnits, _, hardSideDuplicateSkips, _ = collectHardSideIndexUnits(seenUnitKeys)
    if hardSideUnits:
        units.extend(hardSideUnits)
        sourceCounts["hardSide"] += len(hardSideUnits)
        duplicateSkips += hardSideDuplicateSkips

    print(
        f"[collect] units={len(units):,} "
        f"allFilings={sourceCounts['allFilings']:,} docs={sourceCounts['docs']:,} "
        f"hardSide={sourceCounts['hardSide']:,} "
...
def surfacePairHits(surface: str, pairs: tuple[tuple[str, str], ...], *, maxDistance: int = 160) -> int:
    hits = 0
    for left, right in pairs:
        ...
        if leftAt < 0 or rightAt < 0:
            continue
        if abs(leftAt - rightAt) <= maxDistance:
            hits += 1
    return hits


def hardRoleRelationPairs(spec: QuerySpec, hardTerms: tuple[str, ...]) -> tuple[tuple[str, str], ...]:
    if not hardTerms:
        return ()
    queryTerms = set(extractStems(spec.query, cap=32))
    roleTerms = {term for term in queryTerms if slotForTerm(term) in {"action", "change", "risk", "purpose"}}
    for surface in (*spec.mustAny, *spec.supportAny):
        for term in extractStems(surface, cap=8):
            if slotForTerm(term) in {"action", "change", "risk", "purpose"}:
                roleTerms.add(term)
    pairs: list[tuple[str, str]] = []
    for hardTerm in hardTerms:
        for roleTerm in sorted(roleTerms):
            if hardTerm != roleTerm:
                pairs.append((hardTerm, roleTerm))
    return tuple(dict.fromkeys(pairs))


def hardRoleRelationSurfaceHits(surface: str, spec: QuerySpec, hardTerms: tuple[str, ...]) -> int:
    return surfacePairHits(surface, hardRoleRelationPairs(spec, hardTerms), maxDistance=180)
...
def search(
    ...
    *,
    ...
    hardTerms = queryHardTerms(model, spec, set(extractStems(query, cap=32))) if spec is not None else ()
    ...
    for unitId in list(scores):
        ...
        if usePurifiedMeaningLearner:
            ...
            hardRoleHits = (
                hardRoleRelationSurfaceHits(
                    compactText(f"{unit.reportName} {unit.sectionTitle} {unit.text}"),
                    spec,
                    hardTerms,
                )
                if spec is not None
            ...
            scores[unitId] += HARD_ROLE_RELATION_SEARCH_WEIGHT * hardRoleHits
...
def slotConfidenceValue(
    ...
    if not terms:
        return 0.0
    rawScores = [float(candidates.get(slot, Counter()).get(term, 0.0)) for term in terms]
    surfaceScore = min(1.0, sum(rawScores) / max(len(rawScores) * 9.0, 1.0))
    evidenceAdjustedSurface = surfaceScore * (0.35 + (0.65 * evidenceGate))
    return round(min(0.99, (0.45 * evidenceAdjustedSurface) + (0.55 * evidenceScore * evidenceGate)), 2)


def buildSlotConfidence(
    candidates: dict[str, Counter[str]],
    target: tuple[str, ...],
    action: tuple[str, ...],
    change: tuple[str, ...],
    ...
    return (
        ("target", slotConfidenceValue(candidates, "target", target, evidenceScore, evidenceGate)),
        ("action", slotConfidenceValue(candidates, "action", action, evidenceScore, evidenceGate)),
        ("change", slotConfidenceValue(candidates, "change", change, evidenceScore, evidenceGate)),
        ("risk", slotConfidenceValue(candidates, "risk", risk, evidenceScore, evidenceGate)),
        ("purpose", slotConfidenceValue(candidates, "purpose", purpose, evidenceScore, evidenceGate)),
    )


def formatSlotConfidence(slotConfidence: tuple[tuple[str, float], ...]) -> str:
    values = [f"{slot}={score:.2f}" for slot, score in slotConfidence if score > 0]
    return ",".join(values) or "-"


def evidenceSnippet(evidence: tuple[str, ...]) -> str:
    if not evidence:
        return "-"
    snippet = evidence[0].split("/snippet=", 1)[-1]
    if len(snippet) > ANSWER_SKELETON_EVIDENCE_CHARS:
        return snippet[:ANSWER_SKELETON_EVIDENCE_CHARS] + "..."
    return snippet


def hasAsciiSignal(term: str) -> bool:
    return any(char.isascii() and char.isalnum() for char in term)


def queryHardTerms(model: Model, spec: QuerySpec, queryTerms: set[str]) -> tuple[str, ...]:
    candidates: dict[str, int] = {}
    for term in queryTerms:
        candidates[term] = 0
    for marker in spec.mustAny:
        for term in extractStems(marker, cap=8):
            candidates[term] = 0
    hardTerms: list[str] = []
    unitCount = max(len(model.units), 1)
    for term in sorted(candidates, key=lambda value: (slotForTerm(value) != "target", value)):
        if not term or isFormLikeRoleStem(term):
            continue
        stemId = model.stemToId.get(term)
        dfRatio = model.stemFreq[stemId] / unitCount if stemId is not None else 0.0
        isHardTarget = slotForTerm(term) == "target" and (
            hasAsciiSignal(term) or len(term) >= 4 or dfRatio <= PRIMARY_EVIDENCE_HARD_TERM_MAX_DF_RATIO
        )
        if isHardTarget:
            hardTerms.append(term)
    asciiHardTerms = [term for term in hardTerms if hasAsciiSignal(term)]
    if asciiHardTerms:
        return tuple(asciiHardTerms)
    return tuple(hardTerms)


def hardTermCoverage(surface: str, hardTerms: tuple[str, ...]) -> tuple[float, tuple[str, ...]]:
    if not hardTerms:
        return 1.0, ()
    missing = tuple(term for term in hardTerms if term.lower() not in surface)
    coverage = (len(hardTerms) - len(missing)) / max(len(hardTerms), 1)
    return round(coverage, 2), missing


def hardTermSeedUnits(content: str, term: str) -> list[str]:
    termLower = term.lower()
    units = [text for text in splitUnits(content) if termLower in compactText(text)]
    if units:
        return units
    ...
    cursor = 0
    while len(out) < 3:
        found = lowered.find(termLower, cursor)
        if found < 0:
            break
        left = max(0, found - 260)
        right = min(len(cleaned), found + 560)
        window = cleaned[left:right].strip()
        if len(window) >= 20:
            out.append(window)
        cursor = found + max(len(termLower), 1)
    return out


def appendHardTermSeedUnit(model: Model, unit: Unit) -> bool:
    stems = extractStems(f"{unit.reportName} {unit.sectionTitle} {unit.text}")
    if not stems:
        return False
    unitId = len(model.units)
    sequence: list[int] = []
    seen: set[int] = set()
    for stem in stems:
        stemId = model.stemToId.get(stem)
        if stemId is None:
            stemId = getStemId(stem, model.stemToId, model.idToStem, model.stemUnits, model.stemFreq)
            position = stemPosition(stem)
            bucket = sum(position) % HORIZON_BUCKETS if position else 0
            model.stemPosMap.append(position)
            model.stemBucket.append(bucket)
            model.bucketToStems[bucket].append(stemId)
            model.fireTable.append(defaultdict(float))
            model.fireNorm.append(0.0)
        sequence.append(stemId)
        model.stemFreq[stemId] += 1
        if stemId not in seen:
            model.stemUnits[stemId].append(unitId)
            seen.add(stemId)
    if not sequence:
        return False

    model.units.append(unit)
    model.postingTable.append(tuple(sequence))
    unitTableLike = isTableLike(unit)
    model.unitTableLike.append(unitTableLike)
    model.unitNegativeEvidenceBase.append(buildUnitNegativeEvidenceBase([unit], [unitTableLike])[0])

    seenPairs: set[tuple[int, int]] = set()
    seqLen = len(sequence)
    for leftPos, leftStem in enumerate(sequence):
        if len(seenPairs) >= HARD_FALLBACK_SEED_PAIR_LIMIT:
            break
        rightLimit = min(seqLen, leftPos + PAIR_INDEX_WINDOW + 1)
        for rightStem in sequence[leftPos + 1 : rightLimit]:
            if leftStem == rightStem:
            ...
            if len(seenPairs) >= HARD_FALLBACK_SEED_PAIR_LIMIT:
                break
    for pair in seenPairs:
        postings = model.pairUnits.setdefault(pair, [])
        if len(postings) < PAIR_INDEX_MAX_UNITS:
            postings.append(unitId)
    return True


def hardTermRowContent(row: dict[str, object]) -> str:
    contentParts = [
        safeValue(row.get("section_content")),
        safeValue(row.get("content_html")),
        safeValue(row.get("content_xml")),
        safeValue(row.get("content_raw")),
    ]
    return " ".join(part for part in contentParts if part)


def hardTermSeedUnitFromRow(rowSource: str, row: dict[str, object], text: str) -> Unit:
    return Unit(
        source=f"{rowSource}:hardSeed",
        corpName=safeValue(row.get("corp_name")),
        stockCode=safeValue(row.get("stock_code")),
        rceptNo=safeValue(row.get("rcept_no")),
        rceptDate=safeValue(row.get("rcept_dt") or row.get("rcept_date")),
...
def ensureHardTermFallbackSeeds(model: Model, missingHardTerms: tuple[str, ...]) -> None:
    if HARD_FALLBACK_SEED_UNITS_PER_TERM <= 0:
        return
    terms = tuple(
        dict.fromkeys(
            term.lower()
            for term in missingHardTerms
            if term and hasAsciiSignal(term) and term.lower() not in HARD_FALLBACK_SEEDED_TERMS
        )
    )
    if not terms:
        return

    seedCounts: Counter[str] = Counter()
    seenUnitKeys = {unitDedupeKey(unit.text) for unit in model.units}
    duplicateSkips = 0
    filesScanned = 0
    stopScan = False
    ...
    for iterator in (iterDocsRows(), iterAllFilingRows()):
        for rowSource, path, columns in iterator:
            if filesScanned >= HARD_FALLBACK_SEED_MAX_FILES:
                stopScan = True
                break
            filesScanned += 1
            for row in readRows(rowSource, path, columns):
                ...
                surface = compactText(content)
                for term in terms:
                    if seedCounts[term] >= HARD_FALLBACK_SEED_UNITS_PER_TERM or term not in surface:
                        continue
                    for text in hardTermSeedUnits(content, term):
                        key = unitDedupeKey(text)
                        if key in seenUnitKeys:
                            ...
                            continue
                        seenUnitKeys.add(key)
                        if appendHardTermSeedUnit(model, hardTermSeedUnitFromRow(rowSource, row, text)):
                            seedCounts[term] += 1
                            HARD_FALLBACK_SEED_STATS["units"] += 1
                        if seedCounts[term] >= HARD_FALLBACK_SEED_UNITS_PER_TERM:
                            break
                if all(seedCounts[term] >= HARD_FALLBACK_SEED_UNITS_PER_TERM for term in terms):
                    stopScan = True
                    break
            if stopScan:
                break
    ...
    for term in terms:
        HARD_FALLBACK_SEEDED_TERMS.add(term)
    HARD_FALLBACK_SEED_STATS["files"] += filesScanned
    HARD_FALLBACK_SEED_STATS["duplicateSkips"] += duplicateSkips
    counts = ",".join(f"{term}:{seedCounts[term]}" for term in terms)
    print(
        f"[hard-seed] terms={','.join(terms)} added={sum(seedCounts.values())} "
        f"counts={counts or '-'} filesScanned={filesScanned} dupSkips={duplicateSkips}"
    )


def primaryEvidenceScore(
    model: Model,
    spec: QuerySpec,
    unitId: int,
    exact: list[str],
    ...
    unit = model.units[unitId]
    surface = compactText(f"{unit.reportName} {unit.sectionTitle} {unit.text}")
    mustHits = sum(1 for term in spec.mustAny if term.lower() in surface)
    supportHits = sum(1 for term in spec.supportAny if term.lower() in surface)
    relationHits = relationSurfaceHits(unit, spec)
    hardRoleHits = hardRoleRelationSurfaceHits(surface, spec, hardTerms)
    sectionHits = sum(1 for hint in spec.typeHints if hint.lower() in surface)
    hardCoverage, missingHardTerms = hardTermCoverage(surface, hardTerms)
    score = 10.0 * min(mustHits, 3)
    score += PRIMARY_EVIDENCE_RELATION_WEIGHT * relationHits
    score += PRIMARY_EVIDENCE_HARD_ROLE_RELATION_WEIGHT * hardRoleHits
    score += PRIMARY_EVIDENCE_SUPPORT_WEIGHT * min(supportHits, 4)
    score += PRIMARY_EVIDENCE_SECTION_WEIGHT * min(sectionHits, 2)
    score += PRIMARY_EVIDENCE_HARD_TERM_WEIGHT * (len(hardTerms) - len(missingHardTerms))
    score -= PRIMARY_EVIDENCE_MISSING_HARD_TERM_PENALTY * len(missingHardTerms)
    score += 2.0 * len(exact) + 1.0 * len(semantic)
    if hardTerms and hardCoverage < PRIMARY_EVIDENCE_MIN_HARD_COVERAGE:
        score *= 0.78
    if relationHits == 0 and hardRoleHits == 0 and supportHits <= 1:
        score *= 0.78
    if model.unitTableLike[unitId]:
        score -= PRIMARY_EVIDENCE_TABLE_PENALTY
    return score


def buildCompressedAnswerSkeleton(
    target: tuple[str, ...],
    action: tuple[str, ...],
    change: tuple[str, ...],
    risk: tuple[str, ...],
    ...
    if evidenceStrongHits < ANSWER_SKELETON_MIN_STRONG_HITS or evidenceRubric < ANSWER_SKELETON_MIN_EVIDENCE_RUBRIC:
        status = "weak-evidence"
    elif primaryEvidenceMissingHardTerms and primaryEvidenceHardCoverage < PRIMARY_EVIDENCE_MIN_HARD_COVERAGE:
        status = "broad-evidence"
    else:
        status = "reliable"
    relationParts: list[str] = []
    if action and status == "reliable":
        relationParts.append(f"행동={','.join(action)}")
    if change and status == "reliable":
        relationParts.append(f"변화={','.join(change)}")
    if risk and status == "reliable":
        relationParts.append(f"위험={','.join(risk)}")
    if purpose and status == "reliable":
        relationParts.append(f"목적={','.join(purpose)}")
    reliableSlots = [
        f"{slot}:{score:.2f}" for slot, score in slotConfidence if score >= ANSWER_FRAME_MIN_SLOT_CONFIDENCE
    ]
    skeleton = (
        f"상태={status}; "
        f"대상={','.join(target) or '-'}; "
        f"관계={' | '.join(relationParts) or '-'}; "
        f"근거강도={evidenceStrongHits}/3; "
        f"hardCoverage={primaryEvidenceHardCoverage:.2f}; "
        f"missingHard={','.join(primaryEvidenceMissingHardTerms) or '-'}; "
        f"slotConf={','.join(reliableSlots) or '-'}; "
        f"frameConf={confidence:.2f}; "
        f"근거={evidenceSnippet(evidence)}"
    )
...
def buildEvidenceGroundedAnswerFrame(
    ...
    *,
    retrievalMode: str = "purifiedMeaningLearner",
) -> AnswerFrame:
    queryTerms = set(extractStems(spec.query, cap=32))
    hardTerms = queryHardTerms(model, spec, queryTerms)
    ...
    evidenceSurface = compactText(
        " ".join(
            ...
            for _, unitId, _, _ in rows[:ANSWER_FRAME_EVIDENCE_POOL]
        )
    )
    candidates: dict[str, Counter[str]] = {
        "target": Counter(),
    ...
    evidenceRows: list[tuple[float, int, int, str]] = []
    for rank, (_, unitId, exact, semantic) in enumerate(rows[:ANSWER_FRAME_EVIDENCE_POOL], start=1):
        unit = model.units[unitId]
        ...
        evidenceRows.append(
            (primaryEvidenceScore(model, spec, unitId, exact, semantic, hardTerms), rank, unitId, evidenceText)
        )
    rankedEvidenceRows = sorted(evidenceRows, key=lambda item: (-item[0], item[1]))
    evidenceRowsForFrame = rankedEvidenceRows[:3]
    evidence = [evidenceText for _, _, _, evidenceText in evidenceRowsForFrame]
    primaryEvidenceRank = rankedEvidenceRows[0][1] if rankedEvidenceRows else 0
    primaryEvidenceUnitId = rankedEvidenceRows[0][2] if rankedEvidenceRows else -1
    primaryEvidenceRubric = rubricQuality(model.units[primaryEvidenceUnitId], spec) if primaryEvidenceUnitId >= 0 else 0
    primaryEvidenceSurface = (
        compactText(
            f"{model.units[primaryEvidenceUnitId].reportName} "
            f"{model.units[primaryEvidenceUnitId].sectionTitle} "
            f"{model.units[primaryEvidenceUnitId].text}"
        )
        if primaryEvidenceUnitId >= 0
        else ""
    )
    primaryEvidenceHardCoverage, primaryEvidenceMissingHardTerms = hardTermCoverage(primaryEvidenceSurface, hardTerms)
    primaryEvidenceHardRoleHits = hardRoleRelationSurfaceHits(primaryEvidenceSurface, spec, hardTerms)
    ...
    evidenceRubrics = [rubricQuality(model.units[unitId], spec) for _, _, unitId, _ in evidenceRowsForFrame]
    evidenceRubric = sum(evidenceRubrics)
    evidenceStrongHits = sum(1 for quality in evidenceRubrics if quality == 2)
    evidenceGate = round(
        min(1.0, (0.40 * min(1.0, evidenceRubric / 6.0)) + (0.60 * min(1.0, evidenceStrongHits / 3.0))),
        2,
    )
    ...
    slotConfidence = buildSlotConfidence(
        candidates,
        target,
        action,
        change,
    ...
    skeleton, skeletonStatus = buildCompressedAnswerSkeleton(
        target,
        action,
        change,
        risk,
        ...
        evidenceRubric,
        evidenceStrongHits,
        primaryEvidenceHardCoverage,
        primaryEvidenceMissingHardTerms,
        slotConfidence,
        confidence,
    )
    return AnswerFrame(
        ...
        primaryEvidenceRank=primaryEvidenceRank,
        primaryEvidenceRubric=primaryEvidenceRubric,
        primaryEvidenceHardTerms=hardTerms,
        primaryEvidenceMissingHardTerms=primaryEvidenceMissingHardTerms,
        primaryEvidenceHardCoverage=primaryEvidenceHardCoverage,
        primaryEvidenceHardRoleHits=primaryEvidenceHardRoleHits,
...
def buildAnswerFrameLine(
    ...
    return (
        f"[frame] query='{frame.query}' mode={frame.retrievalMode} core='{coreAnchors}' "
        ...
        f"evidenceGate={frame.evidenceGate:.2f} skeletonStatus={frame.skeletonStatus} "
        f"primaryEvidenceRank={frame.primaryEvidenceRank} "
        f"primaryEvidenceRubric={frame.primaryEvidenceRubric} "
        f"primaryEvidenceHardCoverage={frame.primaryEvidenceHardCoverage:.2f} "
        f"primaryEvidenceHardRoleHits={frame.primaryEvidenceHardRoleHits} "
        f"primaryEvidenceHardTerms='{','.join(frame.primaryEvidenceHardTerms) or '-'}' "
        f"primaryEvidenceMissingHardTerms='{','.join(frame.primaryEvidenceMissingHardTerms) or '-'}' "
        f"slotConfidence='{formatSlotConfidence(frame.slotConfidence)}' "
        f"roles='{roleClusters}' evidenceCount={len(frame.evidence)} evidence={evidence}"
    )


def buildAnswerSkeletonLine(frame: AnswerFrame) -> str:
    return f"[skeleton] query='{frame.query}' skeleton='{frame.skeleton}'"


def resolveTermStemIds(model: Model, terms: tuple[str, ...]) -> list[int]:
    stemIds: list[int] = []
    seen: set[int] = set()
    for term in terms:
        for stem in extractStems(term, cap=8):
            stemId = model.stemToId.get(stem)
            if stemId is None or stemId in seen:
                continue
            seen.add(stemId)
...
def pairKey(leftStemId: int, rightStemId: int) -> tuple[int, int]:
    return (leftStemId, rightStemId) if leftStemId < rightStemId else (rightStemId, leftStemId)


def fallbackRelationSearch(
    model: Model,
    spec: QuerySpec,
    *,
    topK: int = TOP_K,
) -> list[tuple[float, int, list[str], list[str]]]:
    queryIds = resolveQueryIds(model, spec.query)
    querySet = set(queryIds)
    supportIds = resolveTermStemIds(model, spec.supportAny)
    supportSet = set(supportIds)
    scores: Counter[int] = Counter()

    for left, right in spec.relationPairs:
        leftIds = resolveTermStemIds(model, (left,))
        rightIds = resolveTermStemIds(model, (right,))
        for leftId in leftIds:
            for rightId in rightIds:
                if leftId == rightId:
                    continue
                for unitId in model.pairUnits.get(pairKey(leftId, rightId), ())[:FALLBACK_PAIR_UNITS_PER_PAIR]:
                    scores[unitId] += 60.0

    for stemId in queryIds:
        for unitId in model.stemUnits[stemId][:FALLBACK_POSTINGS_PER_STEM]:
            scores[unitId] += 10.0
    for stemId in supportIds:
        for unitId in model.stemUnits[stemId][: max(FALLBACK_POSTINGS_PER_STEM // 2, 500)]:
            scores[unitId] += 5.0

    for unitId in list(scores):
        unit = model.units[unitId]
        surface = compactText(f"{unit.reportName} {unit.sectionTitle} {unit.text}")
        unitStemIds = set(model.postingTable[unitId])
        coverage = len(querySet & unitStemIds) / max(len(querySet), 1)
        mustHits = sum(1 for term in spec.mustAny if term.lower() in surface)
        supportHits = sum(1 for term in spec.supportAny if term.lower() in surface)
        relationHits = relationSurfaceHits(unit, spec)
        sectionHits = sum(1 for hint in spec.typeHints if hint.lower() in surface)
        scores[unitId] += 16.0 * min(mustHits, 3)
        scores[unitId] += 24.0 * relationHits
        scores[unitId] += 7.0 * min(supportHits, 4)
        scores[unitId] += FALLBACK_SECTION_HINT_WEIGHT * min(sectionHits, 2)
        scores[unitId] += 18.0 * coverage
        if relationHits == 0 and supportHits <= 1:
            scores[unitId] *= 0.72
        if model.unitTableLike[unitId] and relationHits == 0:
            scores[unitId] -= FALLBACK_TABLE_PENALTY
        elif model.unitTableLike[unitId]:
            scores[unitId] -= FALLBACK_TABLE_PENALTY * 0.35

    ranked: list[tuple[float, int, list[str], list[str]]] = []
    for unitId, score in scores.most_common(topK * 8):
        unitStemIds = set(model.postingTable[unitId])
        exact = [model.idToStem[stemId] for stemId in sorted(querySet & unitStemIds)]
        semantic = [model.idToStem[stemId] for stemId in sorted(supportSet & unitStemIds)[:10]]
        ranked.append((float(score), unitId, exact, semantic))
    ranked.sort(key=lambda item: item[0], reverse=True)
    return applyDiversityRerank(model, ranked, topK)


def hardTermFallbackSearch(
    model: Model,
    spec: QuerySpec,
    baseFrame: AnswerFrame,
    *,
    topK: int = TOP_K,
) -> list[tuple[float, int, list[str], list[str]]]:
    hardTerms = baseFrame.primaryEvidenceHardTerms or queryHardTerms(model, spec, set(extractStems(spec.query, cap=32)))
    missingHardTerms = baseFrame.primaryEvidenceMissingHardTerms or hardTerms
    ensureHardTermFallbackSeeds(model, missingHardTerms)
    queryIds = resolveQueryIds(model, spec.query)
    querySet = set(queryIds)
    supportIds = resolveTermStemIds(model, spec.supportAny)
    supportSet = set(supportIds)
    hardIds = resolveTermStemIds(model, hardTerms)
    missingHardIds = resolveTermStemIds(model, missingHardTerms)
    scores: Counter[int] = Counter()

    for stemId in missingHardIds:
        for unitId in model.stemUnits[stemId][:HARD_FALLBACK_POSTINGS_PER_STEM]:
            scores[unitId] += HARD_FALLBACK_HARD_TERM_WEIGHT
    for stemId in hardIds:
        for unitId in model.stemUnits[stemId][:HARD_FALLBACK_SUPPORT_POSTINGS_PER_STEM]:
            scores[unitId] += HARD_FALLBACK_ALL_HARD_TERM_WEIGHT
    for stemId in queryIds:
        for unitId in model.stemUnits[stemId][: max(HARD_FALLBACK_SUPPORT_POSTINGS_PER_STEM // 2, 500)]:
            scores[unitId] += 8.0
    for stemId in supportIds:
        for unitId in model.stemUnits[stemId][: max(HARD_FALLBACK_SUPPORT_POSTINGS_PER_STEM // 2, 500)]:
            scores[unitId] += 5.0

    pairPartners = list(dict.fromkeys(queryIds + supportIds + hardIds))
    for hardId in missingHardIds or hardIds:
        for partnerId in pairPartners:
            if hardId == partnerId:
                continue
            for unitId in model.pairUnits.get(pairKey(hardId, partnerId), ())[:HARD_FALLBACK_PAIR_UNITS_PER_PAIR]:
                scores[unitId] += 26.0
    ...
    for left, right in spec.relationPairs:
        ...
        for leftId in leftIds:
            for rightId in rightIds:
                if leftId == rightId:
                    continue
                for unitId in model.pairUnits.get(pairKey(leftId, rightId), ())[:HARD_FALLBACK_PAIR_UNITS_PER_PAIR]:
                    scores[unitId] += HARD_FALLBACK_RELATION_WEIGHT
    ...
    for unitId in list(scores):
        ...
        unitStemIds = set(model.postingTable[unitId])
        coverage = len(querySet & unitStemIds) / max(len(querySet), 1)
        hardCoverage, stillMissingHardTerms = hardTermCoverage(surface, hardTerms)
        recoveredMissingCount = sum(1 for term in missingHardTerms if term.lower() in surface)
        supportHits = sum(1 for term in spec.supportAny if term.lower() in surface)
        relationHits = relationSurfaceHits(unit, spec)
        sectionHits = sum(1 for hint in spec.typeHints if hint.lower() in surface)
        scores[unitId] += HARD_FALLBACK_HARD_TERM_WEIGHT * recoveredMissingCount
        scores[unitId] += HARD_FALLBACK_ALL_HARD_TERM_WEIGHT * (len(hardTerms) - len(stillMissingHardTerms))
        scores[unitId] -= HARD_FALLBACK_MISSING_HARD_TERM_PENALTY * len(stillMissingHardTerms)
        scores[unitId] += HARD_FALLBACK_RELATION_WEIGHT * relationHits
        scores[unitId] += 8.0 * min(supportHits, 4)
        scores[unitId] += HARD_FALLBACK_SECTION_HINT_WEIGHT * min(sectionHits, 2)
        scores[unitId] += 18.0 * coverage
        if missingHardTerms and recoveredMissingCount == 0:
            scores[unitId] *= 0.25
        if model.unitTableLike[unitId] and hardCoverage < 1.0:
            scores[unitId] -= HARD_FALLBACK_TABLE_PENALTY
        elif model.unitTableLike[unitId]:
            scores[unitId] -= HARD_FALLBACK_TABLE_PENALTY * 0.35

    ranked: list[tuple[float, int, list[str], list[str]]] = []
    for unitId, score in scores.most_common(topK * 10):
        unitStemIds = set(model.postingTable[unitId])
        exact = [model.idToStem[stemId] for stemId in sorted(querySet & unitStemIds)]
        semantic = [model.idToStem[stemId] for stemId in sorted(supportSet & unitStemIds)[:10]]
        ranked.append((float(score), unitId, exact, semantic))
...
def fallbackCandidateRows(
    model: Model,
    spec: QuerySpec,
    baseFrame: AnswerFrame,
) -> list[tuple[str, list[tuple[float, int, list[str], list[str]]]]]:
    candidates = [
        ("relationPairFallback", fallbackRelationSearch(model, spec)),
        (
            "supportAutoSlotFallback",
            search(model, spec.query, useFire=False, useSupportAutoSlot=True, useDiversity=True),
        ),
        ("prunedAutoSlotFallback", search(model, spec.query, useFire=False, usePrunedAutoSlot=True, useDiversity=True)),
        ("lexSlotFallback", search(model, spec.query, useFire=False, useLexiconSlot=True, useDiversity=True)),
        ("purifiedNoDiversityFallback", search(model, spec.query, useFire=False, usePurifiedMeaningLearner=True)),
    ]
    if baseFrame.skeletonStatus == "broad-evidence":
        candidates.insert(0, ("hardTermFallback", hardTermFallbackSearch(model, spec, baseFrame)))
    return candidates


def skeletonRankValue(status: str) -> int:
    if status == "reliable":
        return 2
    if status == "broad-evidence":
        return 1
    return 0


def fallbackFrameRank(frame: AnswerFrame) -> tuple[int, int, float, int, int, float]:
    return (
        skeletonRankValue(frame.skeletonStatus),
        int(not frame.primaryEvidenceMissingHardTerms),
        frame.primaryEvidenceHardCoverage,
        frame.evidenceStrongHits,
        frame.evidenceRubric,
        frame.confidence,
    )


def recoverEvidenceFrame(model: Model, spec: QuerySpec, baseFrame: AnswerFrame) -> AnswerFrame:
    if baseFrame.skeletonStatus not in {"weak-evidence", "broad-evidence"}:
        return baseFrame
    bestFrame = baseFrame
    bestRank = fallbackFrameRank(baseFrame)
    for mode, rows in fallbackCandidateRows(model, spec, baseFrame):
        if not rows:
            continue
        candidateFrame = buildEvidenceGroundedAnswerFrame(model, spec, rows, retrievalMode=mode)
        candidateRank = fallbackFrameRank(candidateFrame)
...
def buildFallbackLine(spec: QuerySpec, baseFrame: AnswerFrame, finalFrame: AnswerFrame) -> str:
    recovered = int(baseFrame.skeletonStatus != "reliable" and finalFrame.skeletonStatus == "reliable")
    return (
        f"[fallback] query='{spec.query}' recovered={recovered} "
        f"mode={finalFrame.retrievalMode} "
        f"status={baseFrame.skeletonStatus}->{finalFrame.skeletonStatus} "
        f"evidenceRubric={baseFrame.evidenceRubric}->{finalFrame.evidenceRubric} "
        f"strongHits={baseFrame.evidenceStrongHits}->{finalFrame.evidenceStrongHits} "
        f"evidenceGate={baseFrame.evidenceGate:.2f}->{finalFrame.evidenceGate:.2f} "
        f"hardCoverage={baseFrame.primaryEvidenceHardCoverage:.2f}->{finalFrame.primaryEvidenceHardCoverage:.2f} "
        f"missingHard={','.join(baseFrame.primaryEvidenceMissingHardTerms) or '-'}"
        f"->{','.join(finalFrame.primaryEvidenceMissingHardTerms) or '-'}"
    )
...
def evaluateExperiment(model: Model) -> None:
    ...
    primaryEvidenceRubricTotal = 0
    primaryEvidenceStrongFrames = 0
    primaryEvidenceChanged = 0
    primaryEvidenceHardCoverageTotal = 0.0
    primaryEvidenceHardRoleTotal = 0
    primaryEvidenceHardRoleFrames = 0
    hardTermFrames = 0
    hardTermCoveredFrames = 0
    answerSkeletons = 0
    reliableSkeletons = 0
    weakEvidenceSkeletons = 0
    broadEvidenceSkeletons = 0
    fallbackAttempts = 0
    fallbackApplied = 0
    fallbackRecovered = 0
    fallbackEvidenceRubricGain = 0
    fallbackStrongHitGain = 0
    hardFallbackAttempts = 0
    hardFallbackApplied = 0
    hardFallbackRecovered = 0
    hardFallbackHardCoverageGain = 0.0
    hardFallbackMissingHardTermGain = 0
    frameSlotConfidenceTotal = 0.0
    frameSlotConfidenceCount = 0
    weakSlotFrames = 0
    skeletonCharsTotal = 0
    ...
    for spec in PROBE_QUERIES:
        ...
        baseFrame = buildEvidenceGroundedAnswerFrame(model, spec, frameRows)  # type: ignore[arg-type]
        frame = recoverEvidenceFrame(model, spec, baseFrame)
        if baseFrame.skeletonStatus in {"weak-evidence", "broad-evidence"}:
            fallbackAttempts += 1
            fallbackApplied += int(frame.retrievalMode != baseFrame.retrievalMode)
            fallbackRecovered += int(frame.skeletonStatus == "reliable")
            fallbackEvidenceRubricGain += frame.evidenceRubric - baseFrame.evidenceRubric
            fallbackStrongHitGain += frame.evidenceStrongHits - baseFrame.evidenceStrongHits
            if baseFrame.skeletonStatus == "broad-evidence":
                hardFallbackAttempts += 1
                hardFallbackApplied += int(frame.retrievalMode != baseFrame.retrievalMode)
                hardFallbackRecovered += int(frame.skeletonStatus == "reliable")
                hardFallbackHardCoverageGain += (
                    frame.primaryEvidenceHardCoverage - baseFrame.primaryEvidenceHardCoverage
                )
                hardFallbackMissingHardTermGain += len(baseFrame.primaryEvidenceMissingHardTerms) - len(
                    frame.primaryEvidenceMissingHardTerms
                )
            print(buildFallbackLine(spec, baseFrame, frame))
        ...
        primaryEvidenceRubricTotal += frame.primaryEvidenceRubric
        primaryEvidenceStrongFrames += int(frame.primaryEvidenceRubric == 2)
        primaryEvidenceChanged += int(frame.primaryEvidenceRank > 1)
        primaryEvidenceHardCoverageTotal += frame.primaryEvidenceHardCoverage
        primaryEvidenceHardRoleTotal += frame.primaryEvidenceHardRoleHits
        primaryEvidenceHardRoleFrames += int(frame.primaryEvidenceHardRoleHits > 0)
        hardTermFrames += int(bool(frame.primaryEvidenceHardTerms))
        hardTermCoveredFrames += int(bool(frame.primaryEvidenceHardTerms) and not frame.primaryEvidenceMissingHardTerms)
        nonEmptySlotScores = [score for _, score in frame.slotConfidence if score > 0]
        frameSlotConfidenceTotal += sum(nonEmptySlotScores)
        frameSlotConfidenceCount += len(nonEmptySlotScores)
        weakSlotFrames += int(any(0 < score < ANSWER_FRAME_MIN_SLOT_CONFIDENCE for score in nonEmptySlotScores))
        answerSkeletons += int(bool(frame.skeleton))
        reliableSkeletons += int(frame.skeletonStatus == "reliable")
        weakEvidenceSkeletons += int(frame.skeletonStatus == "weak-evidence")
        broadEvidenceSkeletons += int(frame.skeletonStatus == "broad-evidence")
        skeletonCharsTotal += len(frame.skeleton)
        print(
        ...
        print(buildAnswerSkeletonLine(frame))
    ...
    print(
        ...
        f"primaryEvidenceRubricAvg={primaryEvidenceRubricTotal / max(answerFrames, 1):.2f} "
        f"primaryEvidenceStrongFrames={primaryEvidenceStrongFrames} "
        f"primaryEvidenceChanged={primaryEvidenceChanged} "
        f"primaryEvidenceHardCoverageAvg={primaryEvidenceHardCoverageTotal / max(answerFrames, 1):.2f} "
        f"primaryEvidenceHardRoleAvg={primaryEvidenceHardRoleTotal / max(answerFrames, 1):.2f} "
        f"primaryEvidenceHardRoleFrames={primaryEvidenceHardRoleFrames} "
        f"hardTermFrames={hardTermFrames} "
        f"hardTermCoveredFrames={hardTermCoveredFrames} "
        f"answerSkeletons={answerSkeletons} "
        f"reliableSkeletons={reliableSkeletons} "
        f"weakEvidenceSkeletons={weakEvidenceSkeletons} "
        f"broadEvidenceSkeletons={broadEvidenceSkeletons} "
        f"fallbackAttempts={fallbackAttempts} "
        f"fallbackApplied={fallbackApplied} "
        f"fallbackRecovered={fallbackRecovered} "
        f"fallbackEvidenceRubricGain={fallbackEvidenceRubricGain} "
        f"fallbackStrongHitGain={fallbackStrongHitGain} "
        f"hardFallbackAttempts={hardFallbackAttempts} "
        f"hardFallbackApplied={hardFallbackApplied} "
        f"hardFallbackRecovered={hardFallbackRecovered} "
        f"hardFallbackHardCoverageGain={hardFallbackHardCoverageGain:.2f} "
        f"hardFallbackMissingHardTermGain={hardFallbackMissingHardTermGain} "
        f"hardSideIndexUnits={HARD_SIDE_INDEX_STATS['units']} "
        f"hardSideIndexFiles={HARD_SIDE_INDEX_STATS['files']} "
        f"hardFallbackSeedUnits={HARD_FALLBACK_SEED_STATS['units']} "
        f"hardFallbackSeedFiles={HARD_FALLBACK_SEED_STATS['files']} "
        f"slotConfidenceAvg={frameSlotConfidenceTotal / max(frameSlotConfidenceCount, 1):.2f} "
        f"weakSlotFrames={weakSlotFrames} "
        f"skeletonCharsAvg={skeletonCharsTotal / max(answerSkeletons, 1):.1f}"
    )


def main() -> None:
    print("=" * 88)
    print("Horizon Meaning Learner v31")
    print("=" * 88)
    print(
        ...
        f"answerFrameMinSlotConfidence={ANSWER_FRAME_MIN_SLOT_CONFIDENCE:g} "
        f"answerFrameEvidencePool={ANSWER_FRAME_EVIDENCE_POOL} "
        f"answerSkeletonEvidenceChars={ANSWER_SKELETON_EVIDENCE_CHARS} "
        f"answerSkeletonMinStrongHits={ANSWER_SKELETON_MIN_STRONG_HITS} "
        f"answerSkeletonMinEvidenceRubric={ANSWER_SKELETON_MIN_EVIDENCE_RUBRIC} "
        f"fallbackPostingsPerStem={FALLBACK_POSTINGS_PER_STEM} "
        f"fallbackPairUnitsPerPair={FALLBACK_PAIR_UNITS_PER_PAIR} "
        f"fallbackSectionHintWeight={FALLBACK_SECTION_HINT_WEIGHT:g} "
        f"fallbackTablePenalty={FALLBACK_TABLE_PENALTY:g} "
        f"primaryEvidenceRelationWeight={PRIMARY_EVIDENCE_RELATION_WEIGHT:g} "
        f"primaryEvidenceSupportWeight={PRIMARY_EVIDENCE_SUPPORT_WEIGHT:g} "
        f"primaryEvidenceSectionWeight={PRIMARY_EVIDENCE_SECTION_WEIGHT:g} "
        f"primaryEvidenceTablePenalty={PRIMARY_EVIDENCE_TABLE_PENALTY:g} "
        f"primaryEvidenceHardTermWeight={PRIMARY_EVIDENCE_HARD_TERM_WEIGHT:g} "
        f"primaryEvidenceMissingHardTermPenalty={PRIMARY_EVIDENCE_MISSING_HARD_TERM_PENALTY:g} "
        f"primaryEvidenceHardRoleRelationWeight={PRIMARY_EVIDENCE_HARD_ROLE_RELATION_WEIGHT:g} "
        f"hardRoleRelationSearchWeight={HARD_ROLE_RELATION_SEARCH_WEIGHT:g} "
        f"primaryEvidenceHardTermMaxDfRatio={PRIMARY_EVIDENCE_HARD_TERM_MAX_DF_RATIO:g} "
        f"primaryEvidenceMinHardCoverage={PRIMARY_EVIDENCE_MIN_HARD_COVERAGE:g} "
        f"hardFallbackPostingsPerStem={HARD_FALLBACK_POSTINGS_PER_STEM} "
        f"hardFallbackSupportPostingsPerStem={HARD_FALLBACK_SUPPORT_POSTINGS_PER_STEM} "
        f"hardFallbackPairUnitsPerPair={HARD_FALLBACK_PAIR_UNITS_PER_PAIR} "
        f"hardFallbackHardTermWeight={HARD_FALLBACK_HARD_TERM_WEIGHT:g} "
        f"hardFallbackAllHardTermWeight={HARD_FALLBACK_ALL_HARD_TERM_WEIGHT:g} "
        f"hardFallbackMissingHardTermPenalty={HARD_FALLBACK_MISSING_HARD_TERM_PENALTY:g} "
        f"hardFallbackRelationWeight={HARD_FALLBACK_RELATION_WEIGHT:g} "
        f"hardFallbackSectionHintWeight={HARD_FALLBACK_SECTION_HINT_WEIGHT:g} "
        f"hardFallbackTablePenalty={HARD_FALLBACK_TABLE_PENALTY:g} "
        f"hardSideIndexUnitsPerTerm={HARD_SIDE_INDEX_UNITS_PER_TERM} "
        f"hardSideIndexMaxFiles={HARD_SIDE_INDEX_MAX_FILES} "
        f"hardFallbackSeedUnitsPerTerm={HARD_FALLBACK_SEED_UNITS_PER_TERM} "
        f"hardFallbackSeedMaxFiles={HARD_FALLBACK_SEED_MAX_FILES} "
        f"hardFallbackSeedPairLimit={HARD_FALLBACK_SEED_PAIR_LIMIT}"
    )
