"""Horizon Meaning Learner V33 - suffix cohort contrastive experience line.

V33 실제 기록
--------------
아이디어:
    V32 는 stem 좌표와 ordered experience-line 을 만들었지만, 같은 suffix 를 가진 surface 들이 표/계정과목
    장면을 함께 경험하면서 `복구충당금 -> 대손충당금` 이 강하게 오염됐다. 반대로 `손실충당금 -> 대손충당금` 은
    top1 이지만 accepted 될 만큼의 순수 경험 overlap 이 없었다.

    V33 은 같은 coordinate suffix cohort 안에서 "모두가 공유하는 경험" 과 "각 surface 를 가르는 경험" 을 분리한다.
    특정 family 이름이나 `충당/충당금` literal 은 쓰지 않는다. 문자열 수평선에서 자동으로 suffix cohort 를 만들고,
    그 cohort 내부에서 널리 반복되는 `xp/el/hx` atom 은 공통 장면으로 penalty 를 주며, cohort 안에서도 덜 넓게
    나타나는 atom 만 contrast 경험으로 남긴다.

시도 방법:
    1. V32 와 같은 방식으로 DART allFilings/docs 에서 meaning window 를 균형 수집한다.
    2. surface 의 Unicode coordinate suffix path 로 cohort 를 자동 생성한다.
    3. 각 surface signature 의 `xp/el/hx/relay` atom 이 같은 suffix cohort 안에서 몇 surface 에 나타나는지 센다.
    4. route scoring 에서 raw experience cosine 과 contrast experience cosine 을 분리한다.
    5. accepted 조건은 raw score 만으로 열지 않고, contrast score 또는 충분한 coordinate resonance 가 있어야 한다.
       coordinate resonance 도 수동 target 예외가 아니라 공통 code point gram IDF 로 계산한다.
    6. 목표는 `복구충당금` 같은 broad suffix 오염을 낮추면서 `손실충당금` 의 top1 진단을 보존하는 것이다.

실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV33Test.py

    $env:DARTLAB_HORIZON_V33_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V33_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V33_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V33_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV33Test.py

판정 기준:
    수동 family lock, `충당/충당금` 전용 rescue atom, target 별 예외처리 없이
    V32 의 `복구충당금 -> 대손충당금` bad accepted 를 낮춰야 한다. 동시에 `손실충당금 -> 대손충당금` 은
    top1 또는 accepted route 로 남아야 한다. positive route 4/4, bad accepted 0/7 이 장기 성공선이다.

결과:
    py_compile 통과.

    smoke(1,200 units, allFilings 600 + docs 600):
        surfaces=7,238, coordKeys=56,951, suffixCohorts=7,301, coordGrams=26,299, model seconds=48.8.
        `손실충당금 -> 대손충당금` accepted=True, score=0.115, xp=0.005, contrast=0.000,
        el=0.002, cx=0.435, nonSuffix resonance=0.061.
        V32 의 대표 오염이던 `복구충당금 -> 대손충당금` 은 score=1.959, xp=0.657,
        contrast=0.639, el=0.361 로 높지만 nonSuffix resonance=0.000 이라 accepted=False.
        `대출채권 -> 매출채권`, `당기순이익 -> 영업이익` 도 accepted=False 로 내려갔다.
        positiveHits=3/4, badAccepted=0/7, searchTop1=4/5, totalSeconds=49.1.

    larger smoke(4,000 units, allFilings 2,000 + docs 2,000):
        surfaces=11,303, coordKeys=84,838, suffixCohorts=10,927, coordGrams=38,826, model seconds=133.6.
        `손실충당금 -> 대손충당금` accepted=True, score=0.128, xp=0.003, contrast=0.003,
        el=0.003, cx=0.435, nonSuffix resonance=0.062.
        `복구충당금 -> 대손충당금` 은 score=2.214, xp=0.663, contrast=0.713,
        el=0.401 로 여전히 broad suffix 경험은 높지만 nonSuffix resonance=0.000 이라 accepted=False.
        `대출채권 -> 매출채권`, `당기순이익 -> 영업이익` 도 accepted=False.
        positiveHits=3/4, badAccepted=0/7, searchTop1=4/5, totalSeconds=133.9.

판정:
    성공에 가까운 구조 개선. V32 의 핵심 실패였던 same-suffix broad scene contamination 을 accepted 단계에서
    끊었다. 특히 `복구충당금 -> 대손충당금` 은 raw/contrast 경험 score 자체는 여전히 높으므로, 단순 경험
    overlap 만으로는 의미가 아니라 장면 오염을 학습한다는 점이 확인됐다. nonSuffix coordinate resonance gate 는
    `손실충당금 -> 대손충당금` 을 보존하면서 `복구충당금`, `대출채권`, `당기순이익` 오염을 0/7 로 낮췄다.
    남은 실패는 `외상매출금 -> 매출채권` 이며, 다음은 suffix contrast 가 아니라 substring role equivalence
    (`외상/매출/금` 과 `매출/채권`) 를 경험 그래프에서 자동으로 만들 수 있는지 검증해야 한다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V33_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V33_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V33_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V33_MAX_WINDOWS_PER_RECORD", "3"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V33_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V33_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V33_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V33_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V33_POSTING_LIMIT", "1200"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V33_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V33_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V33_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V33_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V33_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V33_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V33_RESONANCE_ACCEPT_MIN", "0.030"))
...
def meaningAtom(atom: str) -> bool:
    return atom.startswith(("xp:", "el:", "hx:", "relay:xp", "relay:el", "relay:hx"))
...
def route(surface: str, model: Model):
    ...
    for target in TARGETS:
        ...
        score = contrast * 2.6 + el * 1.2 + cx * 0.20 + resonance * 0.45 - commonPenalty - suffixPenalty
        accepted = (
            score >= ROUTE_MIN_SCORE
            and not suffixNoResonance
            and ((contrast + el) >= ROUTE_MIN_EXPERIENCE or (sameSuffix and resonance >= RESONANCE_ACCEPT_MIN))
            and (contrast >= CONTRAST_ACCEPT_MIN or resonance >= RESONANCE_ACCEPT_MIN)
        )
        rows.append((score, target, xp, contrast, el, cx, resonance, accepted))
...
def preview(rows, limit: int = 3) -> str:
    return " | ".join(
        f"{target}:{score:.3f}/xp{xp:.3f}/ct{contrast:.3f}/el{el:.3f}/cx{cx:.3f}/rs{resonance:.3f}/{'Y' if ok else 'N'}"
        for score, target, xp, contrast, el, cx, resonance, ok in rows[:limit]
    )
...
def main() -> None:
    ...
    for surface, expected in POSITIVE_PROBES:
        ...
        ok = rows[0][1] == expected and rows[0][7]
        pos += int(ok)
    ...
    for surface, forbidden in NEGATIVE_PROBES:
        ...
        isBad = rows[0][1] == forbidden and targetRow[7]
        bad += int(isBad)
        print(
            f"  {surface}-/->{forbidden} bad={isBad} "
            f"forbidden={targetRow[0]:.3f}/xp{targetRow[2]:.3f}/ct{targetRow[3]:.3f}/el{targetRow[4]:.3f} "
    ...
    for query, expected, polarity in SEARCH_PROBES:
        ...
        searchOk += int(ok)
        print(
            f"  {query} route={rows[0][1]} expected={expected} ok={ok} accepted={rows[0][7]} hit={(hits[0][0] if hits else 0):.2f} text={(hits[0][3] if hits else '')}"
        )
    print(
