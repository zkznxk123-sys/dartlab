"""Horizon Meaning Learner V34 - compound substring co-view equivalence.
...
아이디어:
    ...
    V34 는 shared non-suffix substring 과 실제 문서 window co-view association 이 동시에 있을 때만
    compound role equivalence 를 추가한다. 특정 표면명, `외상`, `채권`, `매출` literal 은 쓰지 않는다.
    V33 의 same-suffix gate 는 그대로 유지하므로 `대출채권 -> 매출채권`, `당기순이익 -> 영업이익` 같은
    suffix contamination 은 다시 열리지 않아야 한다.

시도 방법:
    1. V33 의 coordinate experience line, suffix cohort contrast, nonSuffix resonance gate 를 유지한다.
    2. 각 meaning window 안에서 같이 나온 surface pair 를 세고 surface document frequency 를 계산한다.
    3. 두 surface 가 suffix 공유가 아닌 내부 coordinate substring gram 을 공유할 때만 pair association 을 본다.
    4. association 은 pair count, surface df, 전체 unit 수로 계산한 positive PMI 계열 점수다.
    5. route score 에 compound association 을 보조 신호로 더하고, accepted 조건에도 compound path 를 추가한다.

실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV34Test.py

    $env:DARTLAB_HORIZON_V34_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V34_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V34_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V34_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV34Test.py
...
결과:
    ...
    1,200 units(allFilings 600 + docs 600, files=8, rows=180, windows=2):
        surfaces=7,163, coordKeys=56,233, suffixCohorts=7,221, coordGrams=26,047,
        surfacePairs=602,281, compoundGrams=30,047, modelSeconds=59.8, totalSeconds=60.2.
        `외상매출금 -> 매출채권` 은 route top1 이지만 score=0.055, cp=0.000, accepted=False.
        `손실충당금 -> 대손충당금` 은 accepted=True(score=0.146, resonance=0.062).
        `복구충당금 -> 대손충당금`, `대출채권 -> 매출채권`, `당기순이익 -> 영업이익` 은 accepted=False.
        positiveHits=3/4, badAccepted=0/7, searchTop1=5/5.

판정:
    실패/진단 성공. shared substring + tokenized surface co-view 만으로는 `외상매출금 -> 매출채권`
    accepted 를 만들지 못했다. raw 본문에는 붙은 복합어가 있어도 pair index 는 tokenized surface 만 보므로
    compound path 가 0 으로 남는다. 다음은 raw token 내부 substring 을 pseudo-occurrence 로 올려 pair/experience
    index 에 직접 넣어야 한다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V34_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V34_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V34_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V34_MAX_WINDOWS_PER_RECORD", "3"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V34_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V34_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V34_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V34_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V34_POSTING_LIMIT", "1200"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V34_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V34_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V34_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V34_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V34_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V34_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V34_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V34_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
...
def inferSignature(surface: str, model: Model) -> Counter[str]:
    ...
    for similarity, proxy in compoundProxySurfaces(stem, model):
        scale = min(0.20, similarity * 0.12)
        for atom, weight in model.signatures.get(proxy, Counter()).most_common(42):
            if meaningAtom(atom):
                out[f"compoundProxy:{atom}"] += weight * scale
    return out
