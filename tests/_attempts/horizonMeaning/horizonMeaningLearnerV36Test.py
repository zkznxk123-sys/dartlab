"""Horizon Meaning Learner V36 - competitive acceptance margin.
...
실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV36Test.py

    $env:DARTLAB_HORIZON_V36_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V36_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V36_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V36_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV36Test.py
...
판정 기준:
    ...
    `외상매출금` route 2순위 `매출액` accepted 도 닫혀야 한다.

결과:
    py_compile 통과.

    1,200 units(allFilings 600 + docs 600, files=8, rows=180, windows=2):
        rawBridge surfaces=308, rawBridge hits=7,432, surfaces=7,558, coordKeys=57,682,
        suffixCohorts=7,481, coordGrams=26,384, surfacePairs=666,545, compoundGrams=30,440,
        modelSeconds=42.6, totalSeconds=42.8.
        `외상매출금 -> 매출채권` accepted=True(score=0.148, cp=0.058).
        `영업손익 -> 영업이익`, `현금성자산 -> 현금및현금성자산`, `손실충당금 -> 대손충당금` 도 accepted=True.
        `매출액`, `차입금` 등 2순위/3순위 후보는 accepted=False 로 닫혔다.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5.

    4,000 units(allFilings 2,000 + docs 2,000, files=20, rows=600, windows=3):
        rawBridge surfaces=430, rawBridge hits=20,362, surfaces=11,906, coordKeys=87,375,
        suffixCohorts=11,428, coordGrams=39,592, surfacePairs=1,204,700, compoundGrams=46,741,
        modelSeconds=118.8, totalSeconds=119.0.
        `외상매출금 -> 매출채권` accepted=True(score=1.297, cp=0.696).
        V35 에서 남았던 2순위 `매출액` 은 score=0.282, cp=0.127 이지만 accepted=False 로 닫혔다.
        `손실충당금 -> 대손충당금` accepted=True(score=0.116), `복구충당금 -> 대손충당금` 은 suffix gate 로
        accepted=False, `대출채권 -> 매출채권` 도 accepted=False.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5.

판정:
    성공적인 구조 개선. V35 의 raw substring pseudo-occurrence bridge 는 유지하면서, accepted 를 개별 후보의
    절대 점수가 아니라 query 내부 경쟁 margin 으로 보정해 top-k 오염을 줄였다. 특히 `외상매출금` 에서
    `매출채권` top1 accepted 는 유지하고 `매출액` 2순위 accepted 를 닫았다. 다음은 검색 hit snippet 이
    여전히 차입금 표 조각으로 튀는 문제를 unit rerank 의 role evidence gate 로 줄이는 방향이다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V36_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V36_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V36_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V36_MAX_WINDOWS_PER_RECORD", "3"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V36_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V36_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V36_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V36_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V36_POSTING_LIMIT", "1200"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V36_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V36_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V36_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V36_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V36_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V36_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V36_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V36_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V36_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V36_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V36_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V36_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V36_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V36_RAW_BRIDGE_MAX_TOKEN", "18"))
