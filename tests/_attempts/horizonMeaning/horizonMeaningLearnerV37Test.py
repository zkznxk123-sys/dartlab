"""Horizon Meaning Learner V37 - role evidence unit rerank.
...
실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV37Test.py

    $env:DARTLAB_HORIZON_V37_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V37_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V37_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V37_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV37Test.py
...
결과:
    ...
    1,200 units(allFilings 600 + docs 600, files=8, rows=180, windows=2):
        rawBridge surfaces=291, rawBridge hits=7,416, surfaces=7,464, coordKeys=57,018,
        suffixCohorts=7,380, coordGrams=26,113, surfacePairs=660,105, compoundGrams=30,141,
        modelSeconds=46.6, totalSeconds=47.3.
        `외상매출금 -> 매출채권` accepted=True(score=0.141, cp=0.054).
        `영업손익 -> 영업이익`, `현금성자산 -> 현금및현금성자산`, `손실충당금 -> 대손충당금` 도 accepted=True.
        search 는 5/5 로 유지됐고, `매출채권 증가` top snippet 은 `매출채권이 126억원 증가` 문맥으로 잡혔다.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5.

    4,000 units(allFilings 2,000 + docs 2,000, files=20, rows=600, windows=3):
        rawBridge surfaces=422, rawBridge hits=20,945, surfaces=11,926, coordKeys=87,356,
        suffixCohorts=11,414, coordGrams=39,539, surfacePairs=1,224,325, compoundGrams=46,662,
        modelSeconds=131.2, totalSeconds=132.4.
        `외상매출금 -> 매출채권` accepted=True(score=1.286, cp=0.690).
        `영업손익 -> 영업이익` 은 coordinate-resonance floor 로 accepted=True(score=0.063, cx=0.243, rs=0.051).
        V35 잔여 오염이던 2순위 `매출액` 은 accepted=False 로 유지됐다.
        search 는 5/5 로 유지됐고, V36 의 차입금 약정 표 조각은 `매출채권/대손충당금` evidence 문맥으로 바뀌었다.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5.

판정:
    부분 성공. target/query/relation evidence rerank 와 evidence 중심 snippet 추출로 `매출채권할인약정` 같은
    순수 약정 표 조각은 낮췄고, route 의 4/4 positive 와 badAccepted 0/7 은 유지했다. 다만 4,000-unit 의
    `매출채권 증가` 는 아직 증가 자체보다 대손충당금 설정 문맥을 고르는 경향이 남았다. 다음 실험은
    target-relation proximity 를 chunk 단위가 아니라 span/posting 단위로 별도 인덱싱해야 한다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V37_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V37_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V37_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V37_MAX_WINDOWS_PER_RECORD", "3"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V37_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V37_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V37_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V37_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V37_POSTING_LIMIT", "1200"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V37_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V37_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V37_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V37_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V37_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V37_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V37_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V37_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V37_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V37_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V37_SEARCH_EVIDENCE_MIN", "0.34"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V37_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V37_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V37_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V37_RAW_BRIDGE_MAX_TOKEN", "18"))
...
def unitEvidenceScore(
    unitSig: Counter[str], text: str, surface: str, target: str, polarity: str, model: Model
) -> float:
...
def search(query: str, polarity: str, model: Model):
    ...
    for unitId, base in candidates.most_common(160):
        ...
        evidence = unitEvidenceScore(unitSig, unit.text, surface, best[1], polarity, model)
        ...
        hits.append((score, evidence, best[1], unit.ref, evidenceSnippet(unit.text, surface, best[1], polarity, model)))
