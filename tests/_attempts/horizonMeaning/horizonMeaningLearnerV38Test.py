"""Horizon Meaning Learner V38 - target-relation span posting index.
...
실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV38Test.py

    $env:DARTLAB_HORIZON_V38_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V38_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V38_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V38_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV38Test.py
...
결과:
    ...
    1,200 units(allFilings 600 + docs 600, files=8, rows=180, windows=2):
        rawBridge surfaces=303, rawBridge hits=7,135, surfaces=7,412, coordKeys=56,774,
        suffixCohorts=7,375, coordGrams=25,992, surfacePairs=657,834, compoundGrams=29,957,
        spanKeys=6,033, spanHits=34,197, modelSeconds=47.2, totalSeconds=48.3.
        `외상매출금 -> 매출채권` accepted=True(score=0.143, cp=0.055).
        `영업손익 -> 영업이익`, `현금성자산 -> 현금및현금성자산`, `손실충당금 -> 대손충당금` 도 accepted=True.
        search 는 5/5 로 유지됐고, 모든 search top hit 이 span evidence sp=1.00 을 받았다.
        `외상매출금 감소` 는 `전기말 대비 55백만원 감소 ... 매출채권` span 을 반환했다.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5.

    4,000 units(allFilings 2,000 + docs 2,000, files=20, rows=600, windows=3):
        rawBridge surfaces=419, rawBridge hits=20,775, surfaces=11,880, coordKeys=87,044,
        suffixCohorts=11,356, coordGrams=39,356, surfacePairs=1,232,599, compoundGrams=46,393,
        spanKeys=9,296, spanHits=108,420, modelSeconds=134.0, totalSeconds=136.1.
        `외상매출금 -> 매출채권` accepted=True(score=1.287, cp=0.690).
        `영업손익 -> 영업이익` accepted=True(score=0.106), `손실충당금 -> 대손충당금` accepted=True(score=0.116).
        V35 잔여 오염이던 2순위 `매출액` 은 accepted=False 로 유지됐다.
        search 는 5/5 로 유지됐고, 모든 search top hit 이 span evidence sp=1.00 을 받았다.
        V37 의 차입금 약정 표 조각은 밀어냈지만, `매출채권 증가` top span 은 여전히 `대손충당금 설정금액 증가`
        문맥과 `매출채권` 표 항목이 가까운 사례로 잡혔다.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5.

판정:
    부분 성공. build-time target-relation span posting 을 추가하자 search 후보 생성이 span evidence 를 직접
    사용하게 되었고, `매출채권할인약정` 같은 target-only 약정 표 조각은 밀려났다. route 의 4/4 positive,
    badAccepted 0/7, searchTop1 5/5 도 유지했다. 그러나 거리 기반 span 은 아직 관계가 어떤 명사를 수식하는지
    모른다. `매출채권 증가` 에서 증가가 `대손충당금 설정금액` 에 붙어도 같은 span 으로 잡히는 한계가 남았다.
    다음 실험은 target-relation distance 가 아니라 target-value-relation slot span 또는 dependency-like order
    frame 을 만들어야 한다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V38_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V38_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V38_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V38_MAX_WINDOWS_PER_RECORD", "3"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V38_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V38_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V38_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V38_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V38_POSTING_LIMIT", "1200"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V38_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V38_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V38_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V38_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V38_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V38_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V38_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V38_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V38_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V38_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V38_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V38_SPAN_MAX_DISTANCE", "160"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V38_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V38_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V38_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V38_RAW_BRIDGE_MAX_TOKEN", "18"))
...
def unitEvidenceScore(
    ...
    if polarity:
        if targetHit and spanScore >= 0.82:
            score = 1.0
...
def main() -> None:
    ...
    for query, expected, polarity in SEARCH_PROBES:
        ...
        print(
            ...
            f"hit={(hits[0][0] if hits else 0):.2f} ev={(hits[0][1] if hits else 0):.2f} sp={(hits[0][2] if hits else 0):.2f} "
            f"text={(hits[0][5] if hits else '')}"
        )
    print(
