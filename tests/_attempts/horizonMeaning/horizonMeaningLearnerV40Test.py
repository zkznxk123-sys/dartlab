"""Horizon Meaning Learner V40 - table-row aware slot frame.
...
실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV40Test.py

    $env:DARTLAB_HORIZON_V40_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V40_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V40_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V40_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV40Test.py

    $env:DARTLAB_HORIZON_V40_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V40_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V40_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V40_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV40Test.py
...
결과:
    ...
    coordKeys=56,088, spanKeys=5,963, frameKeys=6,314, frameLeaks=6,677. `외상매출금 -> 매출채권`
    accepted=True(score 0.145, cp 0.056), `손실충당금 -> 대손충당금` accepted=True(score 0.117),
    `복구충당금 -> 대손충당금` 은 score 2.085 이지만 accepted=False. positiveHits=4/4,
    badAccepted=0/7, searchTop1=5/5, totalSeconds=59.4.

    4,000 units(allFilings 2,000 + docs 2,000): rawBridge surfaces=411/hits=20,878, surfaces=11,833,
    coordKeys=86,668, spanKeys=9,399, frameKeys=9,918, frameLeaks=27,046. `외상매출금 -> 매출채권`
    accepted=True(score 1.290, cp 0.692), `손실충당금 -> 대손충당금` accepted=True(score 0.115),
    `매출액` 2순위는 score 0.283/cp 0.127 이지만 accepted=False. positiveHits=4/4,
    badAccepted=0/7, searchTop1=5/5, totalSeconds=157.6. `외상매출금 감소` 검색은 후보 recall 확장 후
    `매출채권 감소로 인한 대손충당금이 감소` 문장으로 회복했다.

판정:
    성공/구조 개선. table-row leak 을 별도 negative evidence 로 분리해 frame 이 약한 row-leak 후보를 낮추고,
    accepted 후보 우선 route ordering 과 relation 후보 recall 확장으로 route/search 안정성을 유지했다. 다만 leak 은
    아직 의미 그래프 자체의 학습이라기보다 표 구조 오염을 낮추는 방어 채널이다. 다음은 relation 을 surface 에 귀속시키는
    role-bound frame 을 만들어 `영업이익/당기순이익/매출채권` 처럼 같은 문장에 여러 변화가 있을 때 관계 주어를 더 엄격히
    분리해야 한다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V40_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V40_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V40_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V40_MAX_WINDOWS_PER_RECORD", "3"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V40_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V40_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V40_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V40_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V40_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V40_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V40_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V40_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V40_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V40_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V40_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V40_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V40_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V40_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V40_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V40_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V40_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V40_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V40_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V40_FRAME_MAX_DISTANCE", "180"))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V40_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V40_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V40_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V40_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V40_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V40_RAW_BRIDGE_MAX_TOKEN", "18"))
...
def buildRelationFrameIndex(
    ...
    return dict(postings), scores, leaks
...
def buildModel() -> Model:
    ...
    relationFramePostings, relationFrameScores, relationFrameLeaks = buildRelationFrameIndex(caches)
    print(
    ...
    if polarity:
        ...
        if focusPositions and relPositions:
            focusPos, focusSize, relPos, relSize = min(
                ...
                key=lambda item: (
                    relationTableLeakStrength(compact, item[0], item[1], item[2], item[3]) >= 0.82,
                    relationTableLeakStrength(compact, item[0], item[1], item[2], item[3]),
                    abs(item[0] - item[2]),
                ),
            )
...
def main() -> None:
    ...
    for query, expected, polarity in SEARCH_PROBES:
        ...
        print(
            ...
            f"lk={(hits[0][4] if hits else 0):.2f} text={(hits[0][7] if hits else '')}"
        )
    print(
