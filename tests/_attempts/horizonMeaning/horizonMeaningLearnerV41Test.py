"""Horizon Meaning Learner V41 - role-bound relation owner frame.
...
아이디어:
    ...
    오인할 수 있다. V41 은 relation 과 target 사이의 table fence (`구분`, `계정과목`, `채권금액`, `설정률`, `단위`)
...
실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV41Test.py

    $env:DARTLAB_HORIZON_V41_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V41_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V41_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V41_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV41Test.py

    $env:DARTLAB_HORIZON_V41_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V41_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V41_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V41_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV41Test.py
...
결과:
    ...
    score 2.092 이지만 accepted=False. positiveHits=4/4, badAccepted=0/7, searchTop1=5/5,
    totalSeconds=60.7. `매출채권 증가`, `영업손익 감소`, `현금성자산 증가`, `손실충당금 증가` 는
    bd=1.00 직접 귀속 근거를 잡았다. 단, 작은 표본의 `외상매출금 감소` 는 직접 감소 근거가 부족해
    bd=0.18 약한 근거로 남았다.
    ...
    이지만 accepted=False. positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, totalSeconds=186.8.
    `영업손익 감소` 검색은 V40 의 비교/감소폭성 근거 대신 `영업이익은 ... 감소하였습니다` 직접 문장으로
    바뀌었고 bd=1.00 이다.

판정:
    부분 성공/구조 개선. role-bound frame 은 route 안전성(4/4, bad 0/7)을 유지하면서 같은 문장 안 여러
    변화가 섞인 4,000 표본의 `영업손익 감소` 근거를 직접 귀속 문장으로 교체했다. build-time owner 후보를
    미리 필터링해 4,000 build 총시간은 186.8초로 V40(157.6초) 대비 약 19% 증가 수준이다. 한계는 작은 표본에서
    직접 polarity-bound 근거가 없을 때도 약한 span 후보를 반환한다는 점이다. 다음은 bound evidence 가 약하면
    강제로 답하지 않고 fallback/abstain 하거나, polarity-bound 후보 recall 을 별도 확장하는 메커니즘이 필요하다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V41_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V41_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V41_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V41_MAX_WINDOWS_PER_RECORD", "3"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V41_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V41_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V41_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V41_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V41_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V41_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V41_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V41_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V41_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V41_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V41_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V41_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V41_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V41_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V41_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V41_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V41_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V41_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V41_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V41_FRAME_MAX_DISTANCE", "180"))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V41_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V41_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V41_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V41_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V41_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V41_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V41_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V41_RAW_BRIDGE_MAX_TOKEN", "18"))
...
def main() -> None:
    ...
    for query, expected, polarity in SEARCH_PROBES:
        ...
        print(
            ...
            f"lk={(hits[0][4] if hits else 0):.2f} bd={(hits[0][5] if hits else 0):.2f} text={(hits[0][8] if hits else '')}"
        )
    print(
