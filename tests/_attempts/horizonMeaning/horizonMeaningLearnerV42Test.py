"""Horizon Meaning Learner V42 - polarity-bound abstain/reliable search.
...
실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV42Test.py

    $env:DARTLAB_HORIZON_V42_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V42_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V42_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V42_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV42Test.py

    $env:DARTLAB_HORIZON_V42_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V42_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V42_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V42_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV42Test.py
...
결과:
    1,200 units(allFilings 600 + docs 600): rawBridge surfaces=303/hits=7,214, surfaces=7,328,
    coordKeys=56,088, spanKeys=5,963, frameKeys=6,314, frameLeaks=6,677, boundKeys=6,365,
    boundHits=33,980. `외상매출금 -> 매출채권` accepted=True(score 0.145, cp 0.056),
    `손실충당금 -> 대손충당금` accepted=True(score 0.117), `복구충당금 -> 대손충당금` 은
    score 2.085 이지만 accepted=False. positiveHits=4/4, badAccepted=0/7, searchTop1=5/5,
    reliableSearch=4/5, totalSeconds=61.3. `외상매출금 감소` 는 route 는 맞지만 bd=0.18 이어서
    status=abstain 으로 내려갔다. 즉 작은 표본에 직접 polarity-bound 근거가 없으면 답처럼 확정하지 않는다.

    4,000 units(allFilings 2,000 + docs 2,000): rawBridge surfaces=411/hits=20,878, surfaces=11,833,
    coordKeys=86,668, spanKeys=9,399, frameKeys=9,918, frameLeaks=27,046, boundKeys=9,988,
    boundHits=114,641. `외상매출금 -> 매출채권` accepted=True(score 1.290, cp 0.692),
    `손실충당금 -> 대손충당금` accepted=True(score 0.115), `매출액` 2순위는 score 0.283/cp 0.127
    이지만 accepted=False. positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5,
    totalSeconds=185.8. `외상매출금 감소` 는 `매출채권 감소로 인한 대손충당금이 감소` 직접 문장을
    bd=1.00/status=reliable 로 회복했다.

판정:
    성공/구조 개선. V42 는 route 정확도를 유지하면서 검색 근거 신뢰도를 별도 출력한다. 1,200 표본에서는
    `외상매출금 감소` 를 억지 답변하지 않고 abstain 으로 분리했고, 4,000 표본에서는 실제 bound 근거가 나타나면
    reliable 로 승격했다. 이는 점수만 올리는 개선이 아니라 의미 검색기가 "근거가 없으면 모른다"고 말하는
    검증 채널이다. 다음은 abstain 상태에서 전체 후보를 다시 훑지 않고 CPU 친화적으로 polarity-bound 후보를
    보강하는 side posting/fallback 인덱스가 필요하다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V42_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V42_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V42_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V42_MAX_WINDOWS_PER_RECORD", "3"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V42_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V42_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V42_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V42_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V42_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V42_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V42_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V42_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V42_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V42_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V42_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V42_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V42_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V42_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V42_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V42_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V42_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V42_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V42_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V42_FRAME_MAX_DISTANCE", "180"))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V42_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V42_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V42_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V42_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V42_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V42_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V42_RELIABLE_EVIDENCE_MIN", "0.70"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V42_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V42_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V42_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V42_RAW_BRIDGE_MAX_TOKEN", "18"))
...
def search(query: str, polarity: str, model: Model):
    ...
    for unitId, base in candidates.most_common(SEARCH_CANDIDATE_LIMIT):
        ...
        leakScore = frameLeakScore(unitId, surface, best[1], polarity, model)
        boundScore = boundEvidenceScore(unitId, surface, best[1], polarity, model)
        nearScore = relationNearEvidence(unit.text, searchEvidenceTerms(surface, best[1], "", model), polarity)
