"""Horizon Meaning Learner V43 - polarity-bound side fallback posting.
...
실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV43Test.py

    $env:DARTLAB_HORIZON_V43_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V43_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V43_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V43_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV43Test.py

    $env:DARTLAB_HORIZON_V43_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V43_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V43_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V43_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV43Test.py
...
결과:
    1,200 units(allFilings 600 + docs 600) + side 600 units(allFilings 300 + docs 300):
    rawBridge surfaces=303/hits=7,214, surfaces=7,328, coordKeys=56,088, spanKeys=5,963,
    frameKeys=6,314, frameLeaks=6,677, boundKeys=6,365, boundHits=33,980.
    side spanKeys=7,380, side frameKeys=7,790, side boundKeys=7,849, side boundHits=39,833.
    `외상매출금 -> 매출채권` accepted=True(score 0.145, cp 0.056), `손실충당금 -> 대손충당금`
    accepted=True(score 0.117), `복구충당금 -> 대손충당금` 은 score 2.095 이지만 accepted=False.
    positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5, totalSeconds=80.3.
    V42 에서 abstain 이던 `외상매출금 감소` 는 side posting fallback 으로 `매출채권 감소로 인한
    대손충당금이 감소` 근거를 찾아 bd=1.00/status=reliable 로 회복했다.

    4,000 units(allFilings 2,000 + docs 2,000) + side 600 units(allFilings 300 + docs 300):
    rawBridge surfaces=411/hits=20,878, surfaces=11,833, coordKeys=86,668, spanKeys=9,399,
    frameKeys=9,918, frameLeaks=27,046, boundKeys=9,988, boundHits=114,641.
    side spanKeys=7,420, side frameKeys=7,816, side boundKeys=7,870, side boundHits=43,078.
    `외상매출금 -> 매출채권` accepted=True(score 1.290, cp 0.692), `손실충당금 -> 대손충당금`
    accepted=True(score 0.115), `매출액` 2순위는 score 0.283/cp 0.127 이지만 accepted=False.
    positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5, totalSeconds=210.9.
    V42 의 4,000 기준 성능은 유지했고, side 600 budget 의 추가 비용은 약 25초 수준이다.

판정:
    성공/구조 개선. main route/signature 학습셋을 흔들지 않고 side-bound posting 만 추가해 V42 의
    1,200 표본 abstain 을 reliable 로 회복했다. 핵심은 전체 재스캔이 아니라 build-time 에 relation-bound side
    unit 을 제한된 600개로 만들어 두고, main search 가 reliable 을 못 찾을 때만 조회하는 점이다. 4,000 표본에서는
    기존 route/search 안전성을 유지했지만 build 비용은 V42 대비 증가했다. 다음은 side index 를 full unit signature 가
    아니라 compact posting payload 로 압축해 build/메모리 비용을 줄이는 방향이 필요하다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V43_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V43_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V43_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V43_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V43_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V43_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V43_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V43_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V43_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V43_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V43_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V43_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V43_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V43_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V43_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V43_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V43_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V43_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V43_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V43_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V43_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V43_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V43_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V43_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V43_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V43_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V43_FRAME_MAX_DISTANCE", "180"))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V43_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V43_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V43_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V43_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V43_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V43_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V43_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V43_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V43_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V43_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V43_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V43_RAW_BRIDGE_MAX_TOKEN", "18"))
...
class Model:
    ...
    sideRelationSpanPostings: dict[tuple[str, str], list[int]]
    sideRelationSpanScores: dict[tuple[int, str, str], float]
    sideRelationFramePostings: dict[tuple[str, str], list[int]]
    sideRelationFrameScores: dict[tuple[int, str, str], float]
    sideRelationFrameLeaks: dict[tuple[int, str, str], float]
...
def collectSideUnits(startUnitId: int, seenTexts: set[str]) -> list[Unit]:
    units: list[Unit] = []
    counts: Counter[str] = Counter()
        ...
        for row, textCol in sideParquetRows(source, folder):
            ...
            for chunk in windows(row.get(textCol)):
                ...
                if key in seenTexts:
                    continue
                seenTexts.add(key)
                units.append(Unit(startUnitId + len(units), source, ref, chunk))
                counts[source] += 1
                if len(units) >= SIDE_MAX_UNITS or counts[source] >= perSource:
                    break
            if len(units) >= SIDE_MAX_UNITS or counts[source] >= perSource:
                break
    print(f"[sideCollect] units={len(units)} sourceCounts={dict(counts)} seconds={time.perf_counter() - started:.1f}")
    return units
...
def unitSignatureForCache(cache: Cache, model: Model) -> Counter[str]:
    sig: Counter[str] = Counter()
    for occ in cache.occs:
        sig[f"surf:{occ.surface}"] += 2
        for atom, weight in model.signatures.get(occ.surface, Counter()).most_common(12):
...
def buildSideUnitSignatures(model: Model) -> None:
    for cache in model.sideCaches:
        model.unitSignatures[cache.unit.unitId] = unitSignatureForCache(cache, model)


def buildModel() -> Model:
    started = time.perf_counter()
    units = collectUnits()
    sideUnits = collectSideUnits(len(units), {stableHash(unit.text, 16) for unit in units})
    caches = [tokenize(unit) for unit in units]
    sideCaches = [tokenize(unit) for unit in sideUnits]
    print(f"[tokenize] caches={len(caches)}")
    print(f"[sideTokenize] caches={len(sideCaches)}")
    ...
    relationFramePostings, relationFrameScores, relationFrameLeaks, relationBoundPostings, relationBoundScores = (
        buildRelationFrameIndex(caches)
    )
    sideRelationSpanPostings, sideRelationSpanScores = buildRelationSpanIndex(sideCaches)
    (
        sideRelationFramePostings,
        sideRelationFrameScores,
        sideRelationFrameLeaks,
        sideRelationBoundPostings,
        sideRelationBoundScores,
    ) = buildRelationFrameIndex(sideCaches)
    print(
    ...
    print(
        f"[side] spanKeys={len(sideRelationSpanPostings)} frameKeys={len(sideRelationFramePostings)} "
        f"boundKeys={len(sideRelationBoundPostings)} boundHits={sum(len(values) for values in sideRelationBoundPostings.values())}"
    )
    model = Model(
        units + sideUnits,
        caches,
        ...
        relationBoundScores,
        sideRelationSpanPostings,
        sideRelationSpanScores,
        sideRelationFramePostings,
        sideRelationFrameScores,
        sideRelationFrameLeaks,
        sideRelationBoundPostings,
...
def spanEvidenceScore(unitId: int, surface: str, target: str, polarity: str, model: Model) -> float:
    ...
    scores.extend(model.sideRelationSpanScores.get((unitId, term, polarity), 0.0) for term in terms)
...
def frameEvidenceScore(unitId: int, surface: str, target: str, polarity: str, model: Model) -> float:
    ...
    scores.extend(model.sideRelationFrameScores.get((unitId, term, polarity), 0.0) for term in terms)
    return max(scores) if scores else 0.0
...
def frameLeakScore(unitId: int, surface: str, target: str, polarity: str, model: Model) -> float:
    ...
    scores.extend(model.sideRelationFrameLeaks.get((unitId, term, polarity), 0.0) for term in terms)
    return max(scores) if scores else 0.0
...
def boundEvidenceScore(unitId: int, surface: str, target: str, polarity: str, model: Model) -> float:
    ...
    scores.extend(model.sideRelationBoundScores.get((unitId, term, polarity), 0.0) for term in terms)
    return max(scores) if scores else 0.0
...
def search(query: str, polarity: str, model: Model):
    ...
    if polarity and not any(hit[6] == "reliable" for hit in hits):
        ...
        for term in searchEvidenceTerms(surface, target, "", model):
            ...
            for unitId in model.sideRelationFramePostings.get((term, polarity), ())[:SIDE_FALLBACK_LIMIT]:
                sideCandidates[unitId] += 12
            for unitId in model.sideRelationSpanPostings.get((term, polarity), ())[:SIDE_FALLBACK_LIMIT]:
                sideCandidates[unitId] += 7
        for unitId, base in sideCandidates.most_common(SIDE_FALLBACK_LIMIT):
            if unitId in considered:
                continue
            hit = scoreSearchHit(unitId, base, seed, surface, target, polarity, model, sideFallback=True)
            if hit is not None:
                hits.append(hit)
