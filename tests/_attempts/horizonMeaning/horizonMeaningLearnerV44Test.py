"""Horizon Meaning Learner V44 - compact side fallback payload.
...
실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV44Test.py

    $env:DARTLAB_HORIZON_V44_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V44_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V44_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V44_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV44Test.py

    $env:DARTLAB_HORIZON_V44_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V44_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V44_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V44_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV44Test.py
...
결과:
    1,200 units(allFilings 600 + docs 600) + compact side payload 600개(allFilings 300 + docs 300):
    ...
    side payloads=600, side boundKeys=7,849, side boundHits=39,833.
    ...
    positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5, totalSeconds=75.7.
    V43 1,200 의 80.3초보다 줄었고, `외상매출금 감소` 는 compact side fallback 으로
    bd=1.00/status=reliable 을 유지했다.

    4,000 units(allFilings 2,000 + docs 2,000) + compact side payload 600개(allFilings 300 + docs 300):
    ...
    side payloads=600, side boundKeys=7,870, side boundHits=43,078.
    ...
    positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5, totalSeconds=200.7.
    V43 4,000 의 210.9초보다 줄었고, V42 의 reliableSearch 5/5 및 V43 의 side 회복 효과를 유지했다.

판정:
    성공/구조 개선. V44 는 side unit 을 정규 `model.units` 나 `unitSignatures` 에 넣지 않고,
    `SidePayload(ref,text)` 와 relation-bound posting/score 만 저장한다. 따라서 route/signature 학습 분포를
    건드리지 않고 V43 의 abstain 회복 효과를 유지하면서 build 시간을 일부 줄였다. 다만 side bound index 자체를
    만들기 위해 side cache/tokenize 는 여전히 필요하므로 시간 개선은 제한적이다. 다음은 side bound index 를 더
    압축하거나, side 수집 단계에서 relation-bound surface 후보만 바로 추출해 cache 생성을 줄이는 방향이 필요하다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V44_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V44_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V44_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V44_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V44_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V44_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V44_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V44_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V44_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V44_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V44_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V44_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V44_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V44_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V44_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V44_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V44_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V44_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V44_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V44_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V44_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V44_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V44_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V44_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V44_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V44_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V44_FRAME_MAX_DISTANCE", "180"))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V44_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V44_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V44_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V44_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V44_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V44_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V44_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V44_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V44_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V44_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V44_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V44_RAW_BRIDGE_MAX_TOKEN", "18"))
...
def collectSideUnits(seenTexts: set[str]) -> list[Unit]:
    units: list[Unit] = []
    counts: Counter[str] = Counter()
...
def buildModel() -> Model:
    started = time.perf_counter()
    units = collectUnits()
    sideUnits = collectSideUnits({stableHash(unit.text, 16) for unit in units})
    caches = [tokenize(unit) for unit in units]
    sideCaches = [tokenize(unit) for unit in sideUnits]
    sidePayloads = [SidePayload(unit.unitId, unit.ref, unit.text) for unit in sideUnits]
    ...
    sideRelationBoundPostings, sideRelationBoundScores = buildSideBoundIndex(sideCaches)
    print(
