"""Horizon Meaning Learner V49 - sparse common contrast index.
...
실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV49Test.py

    $env:DARTLAB_HORIZON_V49_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V49_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V49_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V49_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV49Test.py

    $env:DARTLAB_HORIZON_V49_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V49_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V49_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V49_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV49Test.py
...
결과:
    ...
    rawBridge surfaces=303/hits=7,214, surfaces=7,328, coordKeys=56,088.
    contrastPrune atomLimit=48, rawUpdates=611,597, commonAtoms=80,807.
    surfacePair targetPairChecks=56,340, surfacePairs=7,166.
    focusedRelation relationOcc=1,312/spanPairs=64,923/localPairs=117,570.
    spanKeys=5,838, frameKeys=6,188, boundKeys=6,240, sideBoundKeys=7,720.
    `외상매출금 -> 매출채권` accepted=True(score 0.145, cp 0.056), `손실충당금 -> 대손충당금`
    accepted=True(score 0.118), `복구충당금 -> 대손충당금` 은 score 2.161 이지만 accepted=False.
    positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5, totalSeconds=65.8.
    V48 1,200 의 cohortAtoms=3,056,497 을 80,807 로 줄였고 총 시간도 66.5초에서 65.8초로 줄었다.

    4,000 units(allFilings 2,000 + docs 2,000) + direct side payload 600개(allFilings 300 + docs 300):
    rawBridge surfaces=411/hits=20,878, surfaces=11,833, coordKeys=86,668.
    contrastPrune atomLimit=48, rawUpdates=1,025,415, commonAtoms=141,354.
    surfacePair targetPairChecks=170,013, surfacePairs=11,686.
    focusedRelation relationOcc=4,825/spanPairs=236,233/localPairs=432,514.
    spanKeys=9,225, frameKeys=9,747, boundKeys=9,816, sideBoundKeys=7,742.
    ...
    positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5, totalSeconds=176.4.
    V48 4,000 의 cohortAtoms=4,809,080 을 141,354 로 줄였고 총 시간도 180.7초에서 176.4초로 줄었다.

판정:
    성공/구조 개선. V49 는 suffix cohort contrast 를 full atom DF 에서 sparse common atom index 로 바꿨다.
    같은 suffix 경험 오염을 막는 핵심 기능은 유지하면서 cohort atom footprint 를 4,000 표본 기준 약 34분의 1로
    줄였고, wall-clock 도 4.3초 줄였다. 특히 `복구충당금 -> 대손충당금` 의 score 가 2.468(V48) 에서 0.297 로
    크게 내려갔지만 accepted=False 를 유지해 오염 차단은 더 안정화됐다. 다음은 stage timing 을 넣어 남은 병목이
    signature 생성인지 relation index/side scan 인지 분리하거나, signature 생성의 top-atom/relay 계산을 줄이는 쪽이다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V49_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V49_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V49_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V49_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V49_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V49_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V49_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V49_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V49_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V49_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V49_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V49_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V49_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V49_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V49_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V49_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V49_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V49_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V49_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V49_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V49_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V49_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V49_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V49_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V49_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V49_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V49_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V49_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V49_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V49_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V49_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V49_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V49_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V49_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V49_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V49_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V49_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V49_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V49_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V49_RAW_BRIDGE_MAX_TOKEN", "18"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V49_COHORT_CONTRAST_ATOM_LIMIT", "48"))
