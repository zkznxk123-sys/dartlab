"""Horizon Meaning Learner V50 - shared horizon atom cache.
...
실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV50Test.py

    $env:DARTLAB_HORIZON_V50_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V50_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V50_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V50_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV50Test.py

    $env:DARTLAB_HORIZON_V50_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V50_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V50_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V50_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV50Test.py
...
결과:
    ...
    horizon atom cache 는 buildSketches 에서 miss=96,864/hit=10,240, buildSignatures 에서 miss=0/hit=107,104.
    stage timing 은 collect 0.9s, sideCollect 2.9s, tokenize 1.7s, buildSketches 7.7s,
    buildSignatures 27.7s, contrast 0.7s, surfacePair 0.1s, relation 7.8s, sideBound 7.7s,
    unitIndex 4.9s, modelSeconds=62.1.
    contrastPrune atomLimit=48, commonAtoms=80,936, surfacePairs=7,166.
    ...
    positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5, totalSeconds=64.8.
    V49 1,200 의 65.8초보다 1.0초 줄었다.

    4,000 units(allFilings 2,000 + docs 2,000) + direct side payload 600개(allFilings 300 + docs 300):
    horizon atom cache 는 buildSketches 에서 miss=319,845/hit=31,339, buildSignatures 에서 miss=0/hit=351,184.
    stage timing 은 collect 1.6s, sideCollect 4.1s, tokenize 4.9s, buildSketches 22.6s,
    buildSignatures 76.0s, contrast 1.1s, surfacePair 0.2s, compoundGram 0.2s, relation 27.9s,
    sideBound 9.3s, unitIndex 13.8s, modelSeconds=161.6.
    contrastPrune atomLimit=48, commonAtoms=141,410, surfacePairs=11,686.
    `외상매출금 -> 매출채권` accepted=True(score 1.290, cp 0.692), `손실충당금 -> 대손충당금`
    accepted=True(score 0.116), `복구충당금 -> 대손충당금` 은 score 0.297 이지만 accepted=False,
    `매출액` 2순위는 score 0.283/cp 0.127 이지만 accepted=False.
    positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5, totalSeconds=171.0.
    V49 4,000 의 176.4초보다 5.4초 줄었다.

판정:
    성공/구조 개선. V50 은 unit/position horizon atom cache 를 공유해 sketch 와 signature 사이의 중복
    horizon atom 계산을 제거했고, 품질 지표를 모두 유지하면서 4,000 표본 wall-clock 을 5.4초 줄였다.
    stage timing 으로 다음 병목도 확인했다. 현재 가장 큰 비용은 buildSignatures 76.0초이며, 그 안에서는
    lineAtoms 와 relayExperience/weightCounters 가 다음 후보로 보인다. relation index 는 27.9초, sketch 는 22.6초로
    그다음이다. 다음은 signature raw atom 생성 자체를 top-position/role-position 중심으로 줄이거나,
    relayExperience 를 target/bridge 후보군 중심으로 제한하는 실험이 필요하다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V50_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V50_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V50_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V50_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V50_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V50_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V50_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V50_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V50_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V50_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V50_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V50_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V50_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V50_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V50_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V50_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V50_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V50_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V50_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V50_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V50_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V50_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V50_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V50_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V50_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V50_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V50_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V50_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V50_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V50_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V50_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V50_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V50_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V50_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V50_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V50_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V50_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V50_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V50_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V50_RAW_BRIDGE_MAX_TOKEN", "18"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V50_COHORT_CONTRAST_ATOM_LIMIT", "48"))
