"""Horizon Meaning Learner V51 - shared line atom cache.
...
아이디어:
    ...
    V50 timing 은 buildSignatures 가 여전히 4,000 표본에서 76초로 가장 크다고 보여줬다. 그 안의 `lineAtoms()`
    역시 surface 자체가 아니라 unit/position 과 이미 만들어진 sketches 로만 결정된다. rawBridge pseudo surface 는
    원 token 과 같은 position 에 놓이므로 같은 line atom 을 반복 계산한다. V51 은 unitId+position 별 line atom
    cache 를 추가해 signature raw atom 생성의 남은 중복을 줄인다. scorer 와 index 구조는 그대로 둔다.
...
시도 방법:
    ...
    25. unit/position 단위 line atom cache 를 buildSignatures 내부에서 공유한다.

실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV51Test.py

    $env:DARTLAB_HORIZON_V51_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V51_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V51_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V51_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV51Test.py

    $env:DARTLAB_HORIZON_V51_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V51_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V51_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V51_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV51Test.py
...
결과:
    1,200 units(allFilings 600 + docs 600) + direct side payload 600개(allFilings 300 + docs 300):
    line atom cache 는 buildSignatures 에서 lineMiss=96,864/lineHit=10,240.
    stage timing 은 collect 1.0s, sideCollect 2.9s, tokenize 1.6s, buildSketches 7.6s,
    buildSignatures 27.4s, contrast 0.7s, relation 7.9s, sideBound 7.7s, unitIndex 4.8s,
    modelSeconds=61.8.
    `외상매출금 -> 매출채권` accepted=True(score 0.145, cp 0.056), `손실충당금 -> 대손충당금`
    accepted=True(score 0.118), `복구충당금 -> 대손충당금` 은 score 2.072 이지만 accepted=False.
    positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5, totalSeconds=64.3.
    V50 1,200 의 64.8초보다 0.5초 줄었다.

    4,000 units(allFilings 2,000 + docs 2,000) + direct side payload 600개(allFilings 300 + docs 300):
    line atom cache 는 buildSignatures 에서 lineMiss=319,845/lineHit=31,339.
    stage timing 은 collect 1.7s, sideCollect 3.8s, tokenize 4.6s, buildSketches 23.4s,
    buildSignatures 82.9s, contrast 1.3s, surfacePair 0.2s, compoundGram 1.5s, relation 29.2s,
    sideBound 8.1s, unitIndex 13.6s, modelSeconds=170.3.
    `외상매출금 -> 매출채권` accepted=True(score 1.290, cp 0.692), `손실충당금 -> 대손충당금`
    accepted=True(score 0.116), `복구충당금 -> 대손충당금` 은 score 0.295 이지만 accepted=False,
    `매출액` 2순위는 score 0.283/cp 0.127 이지만 accepted=False.
    positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5, totalSeconds=179.9.
    V50 4,000 의 171.0초보다 느려졌다.

판정:
    실패/진단 성공. V51 은 line atom 을 unit/position 캐시로 공유해 품질은 유지했지만, 4,000 표본에서
    buildSignatures 가 76.0초(V50) 에서 82.9초로 느려졌다. line cache hit 는 31,339/351,184 로 낮아 dict/set
    캐시 오버헤드가 절감액보다 컸다. 따라서 lineAtoms 단순 캐시는 본진 투입 가치가 없고, 다음은
    relayExperience/weightCounters 또는 signature raw atom 생성량 자체를 구조적으로 줄여야 한다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V51_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V51_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V51_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V51_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V51_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V51_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V51_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V51_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V51_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V51_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V51_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V51_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V51_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V51_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V51_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V51_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V51_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V51_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V51_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V51_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V51_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V51_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V51_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V51_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V51_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V51_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V51_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V51_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V51_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V51_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V51_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V51_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V51_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V51_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V51_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V51_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V51_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V51_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V51_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V51_RAW_BRIDGE_MAX_TOKEN", "18"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V51_COHORT_CONTRAST_ATOM_LIMIT", "48"))
...
def cachedLineAtoms(
    ...
    if cached is not None:
        if stats is not None:
            stats["hit"] += 1
        return cached
    atoms = lineAtoms(pos, cache.stems, cache.markers, sketches)
...
def buildSignatures(
    ...
    raw: dict[str, Counter[str]] = defaultdict(Counter)
    stats: Counter[str] = Counter()
    lineStats: Counter[str] = Counter()
    lineAtomCache: dict[tuple[int, int], set[str]] = {}
    for cache in caches:
        for occ in cache.occs:
            raw[occ.surface].update(cachedHorizonAtoms(cache, occ.position, horizonAtomCache, stats))
            raw[occ.surface].update(cachedLineAtoms(cache, occ.position, sketches, lineAtomCache, lineStats))
    for surface, counter in raw.items():
        for atom in coordAtoms(surface):
            counter[atom] += 1
    ...
    print(
        ...
        f"lineMiss={lineStats['miss']} lineHit={lineStats['hit']}"
    )
    return signatures
