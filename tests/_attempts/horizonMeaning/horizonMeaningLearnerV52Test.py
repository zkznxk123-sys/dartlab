"""Horizon Meaning Learner V52 - sparse coordinate relay.
...
아이디어:
    ...
    V51 은 line atom cache 를 시도했지만 cache hit 가 낮아 4,000 표본에서 오히려 느려졌다. 따라서 다음 후보는
    `relayExperience` 다. relay 는 좌표가 가까운 surface 의 경험을 전달해 없는 stem/희소 stem 을 보강하지만,
    모든 surface 에 top10 이웃 × top24 atom 을 전파하면 signature raw graph 가 커진다. V52 는 relay fanout 을
    top6 이웃 × top16 atom 으로 줄이는 sparse coordinate relay 를 적용한다. 이는 relay 를 끊는 것이 아니라,
    경험 전파를 가장 강한 좌표 이웃으로 제한해 CPU 친화적으로 만드는 구조다.
...
실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV52Test.py

    $env:DARTLAB_HORIZON_V52_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V52_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V52_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V52_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV52Test.py

    $env:DARTLAB_HORIZON_V52_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V52_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V52_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V52_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV52Test.py
...
결과:
    1,200 units + direct side payload 600개:
        relay surfaces=7,328, neighbors=6, atoms=16, updates=669,755.
        sketch horizonMiss=96,864/hit=10,240, signature horizonMiss=0/hit=107,104.
        buildSignatures=25.4s, focusedRelation=7.8s, sideBound=7.6s, unitIndex=3.8s,
        modelSeconds=58.4, totalSeconds=61.1.
        `외상매출금 -> 매출채권` accepted=True(score 0.145, cp 0.056), `매출액` 2순위는
        accepted=False. `손실충당금 -> 대손충당금` accepted=True(score 0.117).
        `복구충당금 -> 대손충당금` 은 score 2.291 로 경험 overlap 은 크지만 accepted=False.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5.

    4,000 units + direct side payload 600개:
        relay surfaces=11,832, neighbors=6, atoms=16, updates=1,065,458.
        sketch horizonMiss=319,845/hit=31,339, signature horizonMiss=0/hit=351,184.
        buildSignatures=74.5s, focusedRelation=27.8s, sideBound=9.3s, unitIndex=10.6s,
        modelSeconds=155.9, totalSeconds=165.6.
        `외상매출금 -> 매출채권` accepted=True(score 1.290, cp 0.692), `매출액` 2순위는
        accepted=False. `손실충당금 -> 대손충당금` accepted=True(score 0.115).
        `복구충당금 -> 대손충당금` 은 score 0.306 으로 accepted=False.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5.

판정:
    부분 성공/진단 성공. V51 의 line cache 실패와 달리 sparse relay 는 품질을 해치지 않았고,
    V50 4,000 의 totalSeconds 171.0 대비 165.6 으로 줄었다. 그러나 buildSignatures 는 여전히
    74.5s 로 압도적 병목이고, 1,200 표본의 `복구충당금 -> 대손충당금` forbidden score 가 2.291 로
    높게 남아 같은 suffix 경험 오염을 gate 가 막고 있을 뿐 내부 의미 표현이 깨끗해진 것은 아니다.
    다음 실험은 relay fanout 이 아니라 signature raw atom 생성량/weightCounters 를 줄이고, common
    suffix 경험을 더 일찍 분리하는 구조여야 한다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V52_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V52_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V52_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V52_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V52_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V52_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V52_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V52_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V52_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V52_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V52_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V52_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V52_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V52_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V52_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V52_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V52_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V52_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V52_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V52_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V52_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V52_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V52_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V52_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V52_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V52_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V52_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V52_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V52_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V52_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V52_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V52_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V52_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V52_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V52_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V52_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V52_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V52_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V52_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V52_RAW_BRIDGE_MAX_TOKEN", "18"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V52_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V52_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V52_RELAY_ATOM_LIMIT", "16"))
...
def relayExperience(signatures: dict[str, Counter[str]], postings: dict[str, list[str]]) -> None:
    ...
    for surface, signature in list(signatures.items()):
        ...
        for other, overlap in candidates.most_common(RELAY_NEIGHBOR_LIMIT):
            ...
            for atom, weight in signatures.get(other, Counter()).most_common(RELAY_ATOM_LIMIT):
