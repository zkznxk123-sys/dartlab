"""Horizon Meaning Learner V53 - early signature pruning and relay common quarantine.
...
아이디어:
    ...
    165.6 으로 낮췄다. 그러나 buildSignatures 는 여전히 74.5s 로 가장 크고, 1,200 표본에서
    `복구충당금 -> 대손충당금` forbidden score 가 2.291 로 높았다. 즉 same-suffix 공통 경험이 아직
    signature/relay 내부에 남아 있고, accepted gate 와 contrast 가 뒤에서 막는 구조다.

    V53 은 raw signature atom 을 lane 별로 먼저 줄인 뒤 DF weighting 에 넣는다. 의미 atom 을 전부 버리는
    것이 아니라 xp/hx/el lane 별 상위 경험만 남겨 `weightCounters` 의 입력 폭을 줄인다. 또한 relay source 는
    suffix cohort 에서 흔한 meaning atom 을 quarantine 한 signature 를 사용한다. base signature 와 contrast
    계산은 유지하되, 같은 suffix 공통 경험이 좌표 이웃으로 다시 전파되는 경로만 줄인다.
...
실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV53Test.py

    $env:DARTLAB_HORIZON_V53_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V53_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V53_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V53_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV53Test.py

    $env:DARTLAB_HORIZON_V53_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V53_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V53_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V53_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV53Test.py
...
결과:
    1,200 units + direct side payload 600개:
        rawPrune atoms=2,230,268 -> 1,210,220, relaySource commonAtoms=63,616, removed=60,815,
        relay updates=597,088. signatureStage raw=17.4s, prune=1.8s, weight=1.6s,
        relaySource=1.0s, relay=3.6s. buildSignatures=25.5s, modelSeconds=59.1,
        totalSeconds=61.7.
        `외상매출금 -> 매출채권` accepted=True(score 0.145, cp 0.056), `매출액` 2순위는
        accepted=False. `손실충당금 -> 대손충당금` accepted=True(score 0.155).
        `복구충당금 -> 대손충당금` 은 score 2.269 로 accepted=False.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5.

    4,000 units + direct side payload 600개:
        rawPrune atoms=4,043,447 -> 1,962,887, relaySource commonAtoms=111,392, removed=106,044,
        relay updates=945,454. signatureStage raw=57.8s, prune=3.3s, weight=3.9s,
        relaySource=1.0s, relay=8.4s. buildSignatures=74.5s, modelSeconds=155.4,
        totalSeconds=164.8.
        `외상매출금 -> 매출채권` accepted=True(score 1.290, cp 0.692), `매출액` 2순위는
        accepted=False. `손실충당금 -> 대손충당금` accepted=True(score 0.115).
        `복구충당금 -> 대손충당금` 은 score 0.294 로 accepted=False.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5.

판정:
    부분 성공/진단 성공. 품질은 유지했고 V52 4,000 totalSeconds 165.6 대비 164.8 로 소폭 줄었으며,
    relay update 도 1,065,458 -> 945,454 로 감소했다. 하지만 buildSignatures 는 V52 와 같은 74.5s 이고,
    내부 timing 에서 raw atom 생성이 57.8s 로 대부분을 차지한다. prune/weight/relaySource/relay 를 모두
    합쳐도 raw 생성보다 작다. 즉 weightCounters 를 줄이는 after-prune 은 footprint 진단에는 의미가 있지만
    본질 속도 개선은 아니다. 다음 실험은 per-surface occurrence budget 또는 relation/compound focused
    occurrence sampling 으로 lineAtoms/horizonAtoms 호출 자체를 줄여야 한다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V53_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V53_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V53_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V53_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V53_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V53_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V53_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V53_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V53_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V53_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V53_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V53_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V53_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V53_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V53_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V53_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V53_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V53_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V53_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V53_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V53_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V53_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V53_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V53_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V53_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V53_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V53_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V53_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V53_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V53_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V53_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V53_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V53_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V53_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V53_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V53_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V53_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V53_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V53_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V53_RAW_BRIDGE_MAX_TOKEN", "18"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V53_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V53_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V53_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V53_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V53_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V53_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V53_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V53_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V53_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
...
def buildSignatures(
    ...
    print(
        f"[signatureStage] raw={rawBuilt - started:.1f}s prune={pruned - rawBuilt:.1f}s "
