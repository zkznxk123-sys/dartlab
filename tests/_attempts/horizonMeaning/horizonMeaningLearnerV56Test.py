"""Horizon Meaning Learner V56 - shared occurrence plan.
...
아이디어:
    ...
    V55 이후 남은 중복은 sampler 자체다. sketch 와 signature 가 같은 occurrence 에 대해 relation/value
    근접도와 score 를 각각 계산하고 surface 별 그룹을 다시 만든다. V56 은 `OccurrencePlan` 을 한 번 만들고,
    sketch/signature 가 서로 다른 budget 과 bucket 만 적용해 공유한다. 의미 선택 규칙은 유지하고 CPU overhead 만
    줄이는 구조적 최적화다.
...
시도 방법:
    ...
    30. occurrence score/grouping 을 `OccurrencePlan` 으로 한 번 계산하고 sketch/signature sampler 가 공유한다.

실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV56Test.py

    $env:DARTLAB_HORIZON_V56_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V56_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V56_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V56_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV56Test.py

    $env:DARTLAB_HORIZON_V56_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V56_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V56_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V56_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV56Test.py
...
결과:
    1,200 units + direct side payload 600개:
        occurrencePlan surfaces=7,328, occs=107,104, seconds=1.2.
        sketchSample=83,286, buildSketches=6.2s, signature occSample=66,911,
        buildSignatures=19.9s, modelSeconds=52.6, totalSeconds=55.3.
        `외상매출금 -> 매출채권` accepted=True(score 0.145, cp 0.056), `매출액` 2순위는
        accepted=False. `현금성자산 -> 현금및현금성자산` accepted=True(score 0.175).
        `손실충당금 -> 대손충당금` accepted=True(score 0.117).
        `복구충당금 -> 대손충당금` 은 score 1.683 로 accepted=False.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5.

    4,000 units + direct side payload 600개:
        occurrencePlan surfaces=11,833, occs=351,184, seconds=4.6.
        sketchSample=199,082, buildSketches=17.1s, signature occSample=142,825,
        buildSignatures=43.3s, modelSeconds=125.1, totalSeconds=134.4.
        `외상매출금 -> 매출채권` accepted=True(score 1.290, cp 0.692), `매출액` 2순위는
        accepted=False. `현금성자산 -> 현금및현금성자산` accepted=True(score 0.202).
        `손실충당금 -> 대손충당금` accepted=True(score 0.115).
        `복구충당금 -> 대손충당금` 은 score -0.062 로 accepted=False.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5.

판정:
    실패/진단 성공. 품질은 유지했고 1,200 표본은 totalSeconds 56.2 -> 55.3 으로 소폭 개선됐지만,
    4,000 표본은 V55 totalSeconds 131.4 보다 느린 134.4 가 나왔다. sampler 선택 자체는 sketch 4.0s->0.7s,
    signature 4.9s 수준->1.4s 로 줄었지만, occurrencePlan 생성 4.6s 와 raw/cache 변동이 이득을 상쇄했다.
    따라서 sampler 공유는 현재 본진 투입 가치가 낮다. 다음 구조 개선은 sampler overhead 가 아니라
    focusedRelation 28s 대의 relation-local pair 계산을 줄이는 방향이어야 한다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V56_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V56_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V56_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V56_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V56_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V56_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V56_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V56_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V56_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V56_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V56_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V56_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V56_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V56_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V56_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V56_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V56_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V56_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V56_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V56_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V56_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V56_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V56_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V56_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V56_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V56_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V56_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V56_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V56_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V56_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V56_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V56_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V56_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V56_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V56_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V56_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V56_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V56_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V56_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V56_RAW_BRIDGE_MAX_TOKEN", "18"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V56_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V56_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V56_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V56_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V56_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V56_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V56_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V56_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V56_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V56_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V56_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V56_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V56_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V56_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V56_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V56_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V56_SKETCH_OCC_BUCKETS", "16"))
...
class OccurrenceRow:
    ...
    unitId: int
    position: int
    signatureBucket: tuple[int, int]
    sketchBucket: tuple[int, int]
    cache: Cache
    occ: Occ
    isBridge: bool
    nearRelation: bool
...
class OccurrencePlan:
    grouped: dict[str, list[OccurrenceRow]]
    totalOccs: int
    seconds: float
...
def buildSketches(
    ...
    sampledOccs = selectSketchOccurrences(occurrencePlan)
...
def buildOccurrencePlan(caches: list[Cache]) -> OccurrencePlan:
    started = time.perf_counter()
    ...
    orderedGrouped = {
        surface: sorted(rows, key=lambda row: (-row.score, row.unitId, row.position))
        for surface, rows in grouped.items()
    }
    seconds = time.perf_counter() - started
    print(f"[occPlan] surfaces={len(orderedGrouped)} occs={totalOccs} seconds={seconds:.1f}")
    return OccurrencePlan(orderedGrouped, totalOccs, seconds)


def selectRowsFromPlan(
    plan: OccurrencePlan,
    fullLimit: int,
    budget: int,
    bucketKind: str,
    preserveSelfEcho: bool,
    label: str,
) -> list[OccurrenceRow]:
    selectedRows: list[OccurrenceRow] = []
    limitedSurfaces = 0
    for rows in plan.grouped.values():
        if len(rows) <= fullLimit or (preserveSelfEcho and selfEchoCompoundSurface(rows[0].occ.surface)):
        ...
        for row in rows:
            key = (row.unitId, row.position, row.occ.marker)
            bucket = row.sketchBucket if bucketKind == "sketch" else row.signatureBucket
            if bucket in usedBuckets or key in chosenKeys:
                continue
            chosen.append(row)
            chosenKeys.add(key)
    ...
    selectedRows.sort(key=lambda row: (row.unitId, row.position, row.occ.surface, row.occ.marker))
    bridgeKept = sum(1 for row in selectedRows if row.isBridge)
    relationKept = sum(1 for row in selectedRows if row.nearRelation)
    valueKept = sum(1 for row in selectedRows if row.nearValue)
    print(
        f"[{label}] surfaces={len(plan.grouped)} occs={plan.totalOccs}->{len(selectedRows)} "
        f"limited={limitedSurfaces} budget={budget} fullLimit={fullLimit} "
        f"bridge={bridgeKept} relation={relationKept} value={valueKept}"
    )
    return selectedRows


def selectSketchOccurrences(plan: OccurrencePlan) -> list[tuple[Cache, Occ]]:
    rows = selectRowsFromPlan(
        plan,
        SKETCH_OCC_FULL_LIMIT,
        SKETCH_OCC_BUDGET,
        "sketch",
        True,
        "sketchSample",
    )
    return [(row.cache, row.occ) for row in rows]


def selectSignatureOccurrences(plan: OccurrencePlan) -> list[tuple[Cache, Occ]]:
    rows = selectRowsFromPlan(
        plan,
        SIGNATURE_OCC_FULL_LIMIT,
        SIGNATURE_OCC_BUDGET,
        "signature",
        False,
        "occSample",
    )
...
def buildSignatures(
    ...
    raw: dict[str, Counter[str]] = defaultdict(Counter)
    stats: Counter[str] = Counter()
    sampledOccs = selectSignatureOccurrences(occurrencePlan)
    sampled = time.perf_counter()
    for cache, occ in sampledOccs:
        raw[occ.surface].update(cachedHorizonAtoms(cache, occ.position, horizonAtomCache, stats))
...
def buildModel() -> Model:
    ...
    occurrencePlan = buildOccurrencePlan(caches)
    stage("buildOccurrencePlan")
    horizonAtomCache: dict[tuple[int, int], set[str]] = {}
    sketches = buildSketches(caches, horizonAtomCache, occurrencePlan)
    stage("buildSketches")
    signatures = buildSignatures(caches, sketches, horizonAtomCache, occurrencePlan)
