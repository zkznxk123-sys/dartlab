"""Horizon Meaning Learner V62 - bisected relation intervening scan.
...
아이디어:
    ...
    V61 이후 남은 병목 중 focusedRelation 은 4,000 기준 10.1s 다. 현재 relation-local loop 는 surface-relation
    pair 마다 `intervening` 판정을 위해 relation window 의 localRows 전체를 다시 훑는다. V62 는 localRows 의
    position list 를 만들고, surface 끝과 relation 시작 사이의 실제 구간만 bisect 로 잘라 확인한다. 순서 frame 의
    의미 판정은 유지하면서 불필요한 row scan 을 줄이는 구조다.
...
시도 방법:
    ...
    34. main focusedRelation 의 intervening surface 판정은 localRows 전체 scan 대신 bisect 로 실제 사이 구간만
        검사한다.

실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV62Test.py

    $env:DARTLAB_HORIZON_V62_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V62_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V62_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V62_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV62Test.py

    $env:DARTLAB_HORIZON_V62_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V62_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V62_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V62_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV62Test.py
...
결과:
    1,200 units + direct side payload 600개:
        첫 실행은 buildFocusedRelationIndexes 안에서 `localStarts` 위치가 빠져 NameError 로 중단됐고,
        relation-local loop 안에서 localRows 생성 직후 localStarts 를 만들도록 패치했다.
        재실행 결과 sketchSample=107,104 -> 83,286, buildSketches=8.2s.
        signature occSample=107,104 -> 66,911, buildSignatures=15.1s.
        focusedRelation relationOcc=1,312, spanPairs=64,923, localPairs=117,570,
        ownerFrames=1,312, buildFocusedRelationIndexes=3.0s.
        sideFocusedBound payloads=600, relationOcc=1,575, localPairs=85,334,
        ownerFrames=1,575, buildSideBoundPayloadIndex=2.4s.
        unitIndex surfaceViews=7,328, occs=107,104, uniqueUnitSurfaces=79,324,
        view=0.2s, build=0.7s, stage=1.2s.
        modelSeconds=37.1, totalSeconds=40.1.
    ...
    4,000 units + direct side payload 600개:
        sketchSample=351,184 -> 199,082, buildSketches=20.5s.
        signature occSample=351,184 -> 142,825, buildSignatures=31.6s.
        focusedRelation relationOcc=4,825, spanPairs=236,233, localPairs=432,514,
        ownerFrames=4,825, buildFocusedRelationIndexes=10.0s.
        sideFocusedBound payloads=600, relationOcc=1,870, localPairs=104,185,
        ownerFrames=1,870, buildSideBoundPayloadIndex=2.0s.
        unitIndex surfaceViews=11,833, occs=351,184, uniqueUnitSurfaces=258,358,
        view=0.4s, build=2.2s, stage=3.8s.
        modelSeconds=81.0, totalSeconds=89.8.
...
판정:
    실패/진단 성공. 품질은 유지했지만 V61 4,000 대비 focusedRelation 은 10.1s -> 10.0s 로
    사실상 개선폭이 없고, totalSeconds 는 82.5 -> 89.8 로 나빠졌다. localRows 전체 scan 보다
    bisect start list 생성 비용과 다른 stage 변동이 더 커서 본진 투입 가치가 낮다. 다음은
    `intervening` 미세 최적화가 아니라 buildSketches/buildSignatures 의 occurrence/raw atom 경로 또는
    focusedRelation 의 regex/substring 계산 자체를 줄이는 구조가 필요하다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V62_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V62_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V62_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V62_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V62_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V62_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V62_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V62_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V62_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V62_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V62_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V62_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V62_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V62_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V62_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V62_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V62_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V62_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V62_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V62_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V62_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V62_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V62_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V62_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V62_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V62_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V62_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V62_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V62_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V62_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V62_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V62_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V62_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V62_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V62_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V62_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V62_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V62_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V62_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V62_RAW_BRIDGE_MAX_TOKEN", "18"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V62_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V62_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V62_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V62_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V62_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V62_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V62_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V62_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V62_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V62_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V62_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V62_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V62_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V62_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V62_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V62_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V62_SKETCH_OCC_BUCKETS", "16"))
...
def hasInterveningSurface(
    ...
    if relationPos < surfacePos:
        return False
    start = bisect_left(starts, surfacePos + surfaceSize)
    end = bisect_left(starts, relationPos)
    ...
    for cache in caches:
        ...
        for relation, relPositions in relationPositions.items():
            for relationPos, relationSize in relPositions:
                ...
                if not localRows:
                    continue
                localStarts = [pos for pos, _, _ in localRows]
            ...
            for relationPos, relationSize in relPositions:
                ...
                for surfacePos, surfaceSize, surface in localRows:
                    ...
                    if gapDistance(surfacePos, surfaceSize, relationPos, relationSize) > FOCUSED_FRAME_DISTANCE:
                        continue
                    localPairChecks += 1
                    intervening = hasInterveningSurface(
                        localRows,
                        localStarts,
                        surfacePos,
                        surfaceSize,
