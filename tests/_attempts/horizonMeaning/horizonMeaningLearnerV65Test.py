"""Horizon Meaning Learner V65 - corpus-derived raw bridge seeds.
...
아이디어:
    ...
    V65 는 bridge 없는 base tokenize 를 먼저 수행하고, 그 base cache 의 실제 corpus surface DF/TF 에서
    rawBridge seed 를 뽑는 2-pass 구조를 검증한다. 즉 raw compound token 내부 substring 을 pseudo-occurrence 로
    올리는 메커니즘은 유지하되, substring 이 의미 후보인지 판단하는 seed 는 평가 target/probe 목록이 아니라
    DART corpus 에서 관측된 surface 경험으로 만든다. 정답 예외를 넣지 않고 seed 후보에서 HTML/CSS/문장 종결
    잡음만 일반 필터로 제외한다.
...
시도 방법:
    ...
    35. relay source surface 별 top relay atom view 도 precompute 해 relayExperience 의 반복 `most_common`
        호출을 줄인다.
    36. V65 는 rawBridge 를 target/probe seed 에서 떼어내기 위해 bridge 없는 `tokenizeBase` 를 먼저 만든다.
    37. base cache 의 surface DF/TF 로 corpus bridge seed index 를 만들고, seed gram posting 으로 substring
        match 후보를 좁힌다.
    38. corpus seed 후보는 한글 surface, 길이, content stem, 문장 종결/HTML 잡음 필터만 사용한다. target/probe
        alias, 수작업 family lock, 정답별 예외는 넣지 않는다.
    39. base cache 를 다시 tokenize 하지 않고 같은 stem position 위에 corpus-derived bridge pseudo occurrence 를
        얹어 cache 를 증강한다.
    40. main focused relation 과 side bound position extraction 도 같은 `BridgeSeedIndex` 를 사용해 evidence
        위치 추적과 학습 cache 의 bridge 기준을 맞춘다.

실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV65Test.py

    $env:DARTLAB_HORIZON_V65_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V65_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V65_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V65_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV65Test.py

    $env:DARTLAB_HORIZON_V65_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V65_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V65_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V65_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV65Test.py
...
결과:
    1,200 units + direct side payload 600개:
        tokenizeBase=1.0s, buildCorpusBridgeSeedIndex=0.4s, augmentBridgeSurfaces=0.8s.
        bridgeSeeds corpusSeeds=2,400, candidates=3,745, rawBridge surfaces=7,186, hits=68,552, cache=7,044.
        horizonView caches=1,200, tokens=175,983, buildHorizonTokenViews=0.4s.
        sketchSample=185,298 -> 146,279, buildSketches=8.9s.
        signature occSample=185,298 -> 120,961, buildSignatures=27.1s.
        signatureStage view=0.2s, sample=2.5s, raw=6.8s, prune=3.2s, weight=2.7s,
        relaySource=1.1s, relay=10.6s.
        relay views=12,180, updates=942,749, view=0.7s, build=9.9s.
        focusedRelation relationOcc=1,312, spanPairs=98,190, localPairs=181,295,
        ownerFrames=1,312, buildFocusedRelationIndexes=4.4s.
        sideFocusedBound payloads=600, relationOcc=1,575, localPairs=112,728,
        ownerFrames=1,575, buildSideBoundPayloadIndex=2.1s.
        modelSeconds=52.4, totalSeconds=55.0.
        `외상매출금 -> 매출채권` accepted=True(score 0.149, cp 0.059), `매출액` 2순위는
        accepted=False. `현금성자산 -> 현금및현금성자산` accepted=True(score 0.172).
        `영업손익 -> 영업이익` accepted=True(score 0.203).
        `손실충당금 -> 대손충당금` accepted=True(score 0.116).
        `복구충당금 -> 대손충당금` 은 score 1.280 로 accepted=False.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5.

    4,000 units + direct side payload 600개:
        tokenizeBase=3.3s, buildCorpusBridgeSeedIndex=0.9s, augmentBridgeSurfaces=1.8s.
        bridgeSeeds corpusSeeds=2,400, candidates=6,231, rawBridge surfaces=8,740, hits=229,485, cache=11,447.
        horizonView caches=4,000, tokens=607,533, buildHorizonTokenViews=0.8s.
        sketchSample=618,207 -> 345,481, buildSketches=19.4s.
        signature occSample=618,207 -> 258,675, buildSignatures=51.4s.
        signatureStage view=0.3s, sample=6.8s, raw=13.0s, prune=4.4s, weight=4.1s,
        relaySource=2.5s, relay=20.2s.
        relay views=17,853, updates=1,406,923, view=0.4s, build=19.8s.
        focusedRelation relationOcc=4,825, spanPairs=340,451, localPairs=634,047,
        ownerFrames=4,825, buildFocusedRelationIndexes=15.1s.
        sideFocusedBound payloads=600, relationOcc=1,870, localPairs=146,423,
        ownerFrames=1,870, buildSideBoundPayloadIndex=2.4s.
        modelSeconds=107.0, totalSeconds=115.0.
        `외상매출금 -> 매출채권` accepted=True(score 1.335, cp 0.718), `매출액` 2순위는
        accepted=False. `현금성자산 -> 현금및현금성자산` accepted=True(score 0.266).
        `영업손익 -> 영업이익` accepted=True(score 0.198).
        `손실충당금 -> 대손충당금` accepted=True(score 0.115).
        `복구충당금 -> 대손충당금` 은 score -0.071 로 accepted=False.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5.

판정:
    부분 성공/구조 진단. rawBridge seed 를 target/probe surface 에서 직접 만들지 않고 corpus-derived seed 로
    바꿔도 1,200/4,000 모두 positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5 를 유지했다.
    따라서 "정답 목록 seed 없이는 bridge 의미 연결이 불가능하다" 는 가설은 반박된다.

    하지만 V64 4,000 대비 rawBridge hits 가 크게 늘어 buildSignatures 26.9s -> 51.4s,
    focusedRelation 10.3s -> 15.1s, totalSeconds 80.1 -> 115.0 으로 후퇴했다. 즉 본진 투입용 성능 개선은
    아니고, 하드코딩 의존 축소가 가능하다는 구조 증명이다. 다음 실험은 corpus seed 를 단순 DF/TF top-k 가 아니라
    relation/value/owner frame 근처에서 관측된 noun-like surface 로 더 좁히거나, bridge pseudo occurrence 를
    seed score 별 budget 으로 제한해 V64 수준의 sparse footprint 를 회복해야 한다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V65_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V65_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V65_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V65_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V65_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V65_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V65_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V65_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V65_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V65_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V65_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V65_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V65_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V65_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V65_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V65_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V65_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V65_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V65_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V65_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V65_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V65_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V65_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V65_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V65_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V65_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V65_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V65_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V65_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V65_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V65_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V65_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V65_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V65_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V65_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V65_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V65_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V65_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V65_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V65_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V65_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V65_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V65_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V65_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V65_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V65_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V65_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V65_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V65_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V65_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V65_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V65_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V65_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V65_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V65_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V65_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V65_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V65_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V65_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V65_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V65_SKETCH_OCC_BUCKETS", "16"))
    ...
    for size in range(RAW_BRIDGE_MIN_SIZE, maxSize + 1):
        for index in range(0, len(value) - size + 1):
            ...
            if rawBridgeSeedMatch(part, bridgeSeedIndex):
                out.add(part)
    surfaces = tuple(sorted(out, key=lambda item: (-len(item), item))[:10])
...
def buildCorpusBridgeSeedIndex(caches: list[Cache]) -> BridgeSeedIndex:
    ...
    for surface, df in surfaceDf.items():
        ...
        if not grams:
            continue
        tf = surfaceTf[value]
        score = math.log1p(df) * 1.50 + math.log1p(tf) * 0.35 + min(len(value), 14) * 0.08 + min(len(grams), 16) * 0.02
    ...
    print(
        ...
        f"minDf={CORPUS_BRIDGE_SEED_MIN_DF} top={topSample}"
    )
...
def buildModel() -> Model:
    ...
    print(
        f"[rawBridge] mode=corpusDerivedSeed surfaces={len(bridgeSurfaceUniverse)} "
