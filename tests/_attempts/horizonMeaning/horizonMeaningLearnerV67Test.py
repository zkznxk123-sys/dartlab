"""Horizon Meaning Learner V67 - unit-local bridge application gate.
...
아이디어:
    ...
    실제로 관측된 경험 surface 로 점수화했다. 또한 token 하나에서 그래프에 얹는 bridge pseudo surface 를
    상위 4개로 제한해 V65 의 과도한 pseudo occurrence footprint 를 줄였다. 4,000 표본에서 V65 대비
    totalSeconds 는 115.0 -> 100.4 로 회복했지만, 아직 V64 의 80.1 보다 느렸다.

    V67 은 seed 는 V66 그대로 두고, pseudo surface 를 모든 token 위치에 얹지 않는다. unit 안에서 relation/value
    token 주변 range 를 먼저 mark 하고, 그 evidence range 안에 있는 stem 에만 bridge pseudo occurrence 를 적용한다.
    main cache 증강과 focused/side surface position extraction 이 같은 unit-local gate 를 쓰게 해 학습과 evidence
    기준을 맞춘다. 목적은 target/probe seed 없이 품질을 유지하면서 side/main relation localPairs 와 relay footprint 를
    더 줄일 수 있는지 검증하는 것이다.
...
시도 방법:
    ...
    42. token 하나에서 추가하는 bridge pseudo surface 는 seed anchor/길이 기준 상위 4개로 제한해 V65 의 과도한
        bridge fanout 을 줄인다.
    43. V67 은 relation/value token 주변 range 를 cache 단위로 한 번 mark 하고, 해당 range 밖의 stem 에는 bridge
        pseudo occurrence 를 얹지 않는다.
    44. `sideSurfacePositionMap` 도 같은 range gate 를 사용해 main focused relation 과 side bound extraction 의
        bridge 위치 기준을 cache 증강 기준과 맞춘다.

실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV67Test.py

    $env:DARTLAB_HORIZON_V67_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V67_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V67_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V67_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV67Test.py

    $env:DARTLAB_HORIZON_V67_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V67_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V67_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V67_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV67Test.py
...
결과:
    1,200 units + direct side payload 600개:
        tokenizeBase=1.0s, buildCorpusBridgeSeedIndex=1.7s, augmentBridgeSurfaces=2.0s.
        bridgeSeeds corpusSeeds=2,400, candidates=3,617, evidenceSurfaces=6,525,
        rawBridge surfaces=4,070, hits=33,123, cache=6,412.
        horizonView caches=1,200, tokens=175,983, buildHorizonTokenViews=0.4s.
        sketchSample=141,967 -> 111,162, buildSketches=6.9s.
        signature occSample=141,967 -> 92,219, buildSignatures=18.9s.
        signatureStage view=0.2s, sample=2.3s, raw=4.8s, prune=2.3s, weight=2.2s,
        relaySource=1.2s, relay=5.9s.
        relay views=9,742, updates=796,726, view=0.2s, build=5.7s.
        focusedRelation relationOcc=1,312, spanPairs=80,504, localPairs=147,326,
        ownerFrames=1,312, buildFocusedRelationIndexes=5.4s.
        sideFocusedBound payloads=600, relationOcc=1,575, localPairs=101,179,
        ownerFrames=1,575, buildSideBoundPayloadIndex=2.7s.
        modelSeconds=45.5, totalSeconds=47.5.
        `외상매출금 -> 매출채권` accepted=True(score 0.142, cp 0.055), `매출액` 2순위는
        accepted=False. `현금성자산 -> 현금및현금성자산` accepted=True(score 1.559).
        `영업손익 -> 영업이익` accepted=True(score 0.200).
        `손실충당금 -> 대손충당금` accepted=True(score 0.116).
        `복구충당금 -> 대손충당금` 은 score 1.530 으로 accepted=False.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5.

    4,000 units + direct side payload 600개:
        tokenizeBase=3.3s, buildCorpusBridgeSeedIndex=5.2s, augmentBridgeSurfaces=5.8s.
        bridgeSeeds corpusSeeds=2,400, candidates=6,040, evidenceSurfaces=10,547,
        rawBridge surfaces=5,449, hits=107,174, cache=10,477.
        horizonView caches=4,000, tokens=607,533, buildHorizonTokenViews=0.9s.
        sketchSample=469,393 -> 265,632, buildSketches=17.3s.
        signature occSample=469,393 -> 200,921, buildSignatures=39.3s.
        signatureStage view=0.2s, sample=5.8s, raw=10.6s, prune=3.6s, weight=3.4s,
        relaySource=2.2s, relay=13.2s.
        relay views=14,995, updates=1,235,054, view=0.3s, build=12.9s.
        focusedRelation relationOcc=4,825, spanPairs=286,192, localPairs=528,915,
        ownerFrames=4,825, buildFocusedRelationIndexes=16.2s.
        sideFocusedBound payloads=600, relationOcc=1,870, localPairs=122,460,
        ownerFrames=1,870, buildSideBoundPayloadIndex=2.7s.
        modelSeconds=101.6, totalSeconds=108.2.
        `외상매출금 -> 매출채권` accepted=True(score 1.166, cp 0.624), `매출액` 2순위는
        accepted=False. `현금성자산 -> 현금및현금성자산` accepted=True(score 2.305).
        `영업손익 -> 영업이익` accepted=True(score 1.457).
        `손실충당금 -> 대손충당금` accepted=True(score 0.115).
        `복구충당금 -> 대손충당금` 은 score 0.442 로 accepted=False.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5.

판정:
    실패/진단 성공. 품질은 유지했고 1,200 표본에서는 V66 totalSeconds 49.7 -> 47.5 로 좋아졌다.
    그러나 4,000 표본에서는 rawBridge hits 를 137,244 -> 107,174 로 줄였음에도 totalSeconds 가
    100.4 -> 108.2 로 악화됐다. 원인은 bridge application range mark 와 focused surface position extraction
    비용이다. buildSignatures 는 40.3s -> 39.3s 로 조금 줄었지만, augmentBridgeSurfaces 는 1.5s -> 5.8s,
    buildFocusedRelationIndexes 는 12.8s -> 16.2s 로 늘었다.

    따라서 unit-local bridge application gate 는 본진 투입 가치가 낮다. 다음은 위치별 application gate 가 아니라
    seed index 자체에서 boilerplate 상위 후보를 낮추거나, rawBridgeSubsurfaces cache 결과를 seed evidence score 로
    정렬/압축해 추가 위치 계산 없이 fanout 을 줄이는 방향이 맞다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V67_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V67_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V67_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V67_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V67_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V67_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V67_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V67_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V67_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V67_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V67_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V67_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V67_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V67_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V67_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V67_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V67_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V67_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V67_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V67_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V67_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V67_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V67_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V67_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V67_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V67_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V67_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V67_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V67_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V67_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V67_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V67_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V67_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V67_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V67_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V67_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V67_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V67_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V67_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V67_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V67_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V67_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V67_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V67_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V67_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V67_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V67_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V67_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
CORPUS_BRIDGE_APPLY_RELATION_RADIUS = int(
    os.environ.get("DARTLAB_HORIZON_V67_CORPUS_BRIDGE_APPLY_RELATION_RADIUS", "10")
)
CORPUS_BRIDGE_APPLY_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V67_CORPUS_BRIDGE_APPLY_VALUE_RADIUS", "8"))
CORPUS_BRIDGE_APPLY_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V67_CORPUS_BRIDGE_APPLY_MIN_EVIDENCE", "0.40"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V67_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V67_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V67_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V67_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V67_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V67_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V67_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V67_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V67_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V67_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V67_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V67_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V67_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V67_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V67_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V67_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V67_SKETCH_OCC_BUCKETS", "16"))
...
def relationValuePositions(stems: list[str], markers: list[str]) -> tuple[list[int], list[int]]:
    relationPositions = sorted(
        {index for index, stem in enumerate(stems) if isContentStem(stem) and relationTokenLike(stem)}
    )
    valuePositions = sorted(
        {index for index, (stem, marker) in enumerate(zip(stems, markers)) if valueTokenLike(stem, marker)}
    )
    return relationPositions, valuePositions


def bridgeApplicationEvidence(
    stem: str,
    marker: str,
    position: int,
    ...
    if relationDistance is not None and relationDistance <= CORPUS_BRIDGE_APPLY_RELATION_RADIUS:
        evidence += 2.5 * (1.0 - relationDistance / (CORPUS_BRIDGE_APPLY_RELATION_RADIUS + 1.0))
    valueDistance = nearestTokenDistance(position, valuePositions)
    if valueDistance is not None and valueDistance <= CORPUS_BRIDGE_APPLY_VALUE_RADIUS:
        evidence += 1.5 * (1.0 - valueDistance / (CORPUS_BRIDGE_APPLY_VALUE_RADIUS + 1.0))
    if marker:
        evidence += 0.20
    if relationOwnerCandidate(stem):
        evidence += 0.35
    return evidence


def bridgeEligiblePositions(stems: list[str], markers: list[str]) -> set[int]:
    scores = [0.0] * len(stems)
    relationPositions, valuePositions = relationValuePositions(stems, markers)
    for relationPos in relationPositions:
        left = max(0, relationPos - CORPUS_BRIDGE_APPLY_RELATION_RADIUS)
        right = min(len(stems), relationPos + CORPUS_BRIDGE_APPLY_RELATION_RADIUS + 1)
        for index in range(left, right):
            distance = abs(index - relationPos)
            scores[index] += 2.5 * (1.0 - distance / (CORPUS_BRIDGE_APPLY_RELATION_RADIUS + 1.0))
    for valuePos in valuePositions:
        left = max(0, valuePos - CORPUS_BRIDGE_APPLY_VALUE_RADIUS)
        right = min(len(stems), valuePos + CORPUS_BRIDGE_APPLY_VALUE_RADIUS + 1)
        for index in range(left, right):
            distance = abs(index - valuePos)
            scores[index] += 1.5 * (1.0 - distance / (CORPUS_BRIDGE_APPLY_VALUE_RADIUS + 1.0))
    for index, (stem, marker) in enumerate(zip(stems, markers)):
        if marker:
            scores[index] += 0.20
        if relationOwnerCandidate(stem):
            scores[index] += 0.35
    return {index for index, score in enumerate(scores) if score >= CORPUS_BRIDGE_APPLY_MIN_EVIDENCE}
...
def buildCorpusBridgeSeedIndex(caches: list[Cache]) -> BridgeSeedIndex:
    ...
    for cache in caches:
        ...
        relationPositions, valuePositions = relationValuePositions(cache.stems, cache.markers)
...
def augmentCacheWithBridgeSurfaces(cache: Cache, bridgeSeedIndex: BridgeSeedIndex) -> Cache:
    ...
    eligiblePositions = bridgeEligiblePositions(cache.stems, cache.markers)
    for pos, stem in enumerate(cache.stems):
        if not isContentStem(stem):
            continue
        if pos not in eligiblePositions:
            continue
    ...
    return Cache(cache.unit, cache.stems, cache.markers, occs, bridgeSurfaces, terms)
...
def tokenize(unit: Unit) -> Cache:
    ...
    return Cache(unit, stems, markers, occs, set(), terms)
...
def buildHorizonTokenViews(caches: list[Cache]) -> dict[int, HorizonTokenView]:
    ...
    for cache in caches:
        ...
        views[cache.unit.unitId] = HorizonTokenView(content, markerCells, cells)
    print(f"[horizonView] caches={len(views)} tokens={sum(len(view.content) for view in views.values())}")
    return views


def horizonAtomsFromView(pos: int, view: HorizonTokenView) -> set[str]:
    atoms = {f"hx:selfMarker:{view.markers[pos] if pos < len(view.markers) else '_'}"}
    leftNearest = ""
    rightNearest = ""
...
def lineAtoms(pos: int, stems: list[str], markers: list[str], sketchView: SketchAtomView) -> set[str]:
    atoms: set[str] = set()
    stem = stems[pos]
...
def buildSignatures(
    ...
    for cache, occ in sampledOccs:
        raw[occ.surface].update(cachedHorizonAtoms(cache, occ.position, horizonAtomCache, horizonTokenViews, stats))
        raw[occ.surface].update(lineAtoms(occ.position, cache.stems, cache.markers, sketchView))
    for surface, counter in raw.items():
        for atom in coordAtoms(surface):
            counter[atom] += 1
    rawBuilt = time.perf_counter()
...
def sideSurfacePositionMap(
    ...
    tokenMatches = list(TOKEN_RE.finditer(text))
    stems: list[str] = []
    markers: list[str] = []
    for match in tokenMatches:
        raw = match.group(0)
        stem, marker = splitStemMarker(raw)
        stems.append(normStem(stem))
        markers.append(marker)
    eligiblePositions = bridgeEligiblePositions(stems, markers) if bridgeSeedIndex is not None else set()
    for pos, match in enumerate(tokenMatches):
        stem = stems[pos]
        if not isContentStem(stem):
            continue
...
def buildModel() -> Model:
    ...
    print(
        f"[rawBridge] mode=unitLocalRangeGate surfaces={len(bridgeSurfaceUniverse)} "
    ...
    stage("buildSketches")
    signatures = buildSignatures(caches, sketches, horizonAtomCache, horizonTokenViews)
