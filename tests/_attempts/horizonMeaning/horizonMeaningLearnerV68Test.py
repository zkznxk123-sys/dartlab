"""Horizon Meaning Learner V68 - seed scored raw bridge compression.
...
아이디어:
    ...
    V67 은 V66 의 bridge pseudo surface 를 unit-local relation/value window 안에만 적용했지만, 4,000 표본에서
    위치 range mark 와 focused surface position extraction 비용이 이득을 상쇄했다. rawBridge hits 는 줄었지만
    totalSeconds 는 100.4 -> 108.2 로 악화됐다.

    V68 은 위치 계산을 추가하지 않고 seed index 자체의 evidence score 를 bridge substring 선택에 사용한다.
    V66 은 token 내부 substring 을 `exact seed 먼저, 긴 surface 먼저` 정렬했기 때문에 corpus seed 안의
    boilerplate 또는 약한 seed 도 같은 우선순위를 가졌다. V68 은 corpus seed 후보를 만들 때 계산한 relation/value
    evidence score 와 rank 를 `seedScores` 로 저장하고, V66 의 match universe 와 exact/length 순서는 유지하되
    score floor 로 약한 substring 을 압축한다. score-first 정렬도 같이 시도했지만 작은 표본에서
    `영업손익 -> 영업이익` accepted 를 끊어 최종 코드는 score 를 hard gate 로만 쓴다.
...
시도 방법:
    ...
    42. token 하나에서 추가하는 bridge pseudo surface 는 seed anchor/길이 기준 상위 4개로 제한해 V65 의 과도한
        bridge fanout 을 줄인다.
    43. V68 에서는 corpus seed 의 raw score 와 rank 를 정규화한 `seedScores` 를 보존하고, V66 의 bridge match
        universe 안에서만 score 를 계산한다.
    44. substring 이 정확한 seed 가 아니어도 containment/compound gram 으로 강한 seed 와 닿으면 score 를 받지만,
        score floor 밑이면 bridge pseudo surface 로 올리지 않는다.
    45. exact seed/긴 surface 우선순위는 유지해 기존 의미 recall 을 보존하고, evidence score 는 약한 후보 제거에만 쓴다.

실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV68Test.py

    $env:DARTLAB_HORIZON_V68_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V68_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V68_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V68_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV68Test.py

    $env:DARTLAB_HORIZON_V68_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V68_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V68_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V68_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV68Test.py
...
결과:
    1,200 units + direct side payload 600개, 기본값(scoreFloor=0.38, subsurfaceLimit=4):
        tokenizeBase=1.0s, buildCorpusBridgeSeedIndex=1.8s, augmentBridgeSurfaces=1.0s.
        bridgeSeeds corpusSeeds=2,400, candidates=3,618, evidenceSurfaces=6,526,
        rawBridge surfaces=2,742, hits=36,463, cache=7,044.
        horizonView caches=1,200, tokens=175,983, buildHorizonTokenViews=0.4s.
        sketchSample=146,442 -> 111,933, buildSketches=8.0s.
        signature occSample=146,442 -> 91,604, buildSignatures=20.4s.
        relay views=8,699, updates=729,802, relay=5.8s.
        focusedRelation relationOcc=1,312, spanPairs=78,267, localPairs=143,913,
        ownerFrames=1,312, buildFocusedRelationIndexes=5.5s.
        sideFocusedBound payloads=600, relationOcc=1,575, localPairs=97,726,
        ownerFrames=1,575, buildSideBoundPayloadIndex=2.6s.
        modelSeconds=48.4, totalSeconds=50.7.
        `외상매출금 -> 매출채권` accepted=True(score 0.142, cp 0.055), `매출액` 2순위는
        accepted=False. `현금성자산 -> 현금및현금성자산` accepted=True(score 1.558).
        `영업손익 -> 영업이익` 은 route top1 이지만 accepted=False(score 0.140, cp 0.025).
        `손실충당금 -> 대손충당금` accepted=True(score 0.109).
        `복구충당금 -> 대손충당금` 은 score 1.414 로 accepted=False.
        positiveHits=3/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5.

    진단 실행(scoreFloor=0.00, V66 match universe 사실상 보존):
        rawBridge surfaces=4,306, hits=40,259, cache=7,044.
        buildSketches=8.1s, buildSignatures=22.8s, focusedRelation=4.3s,
        modelSeconds=49.4, totalSeconds=52.0.
        `영업손익 -> 영업이익` accepted=True(score 0.182, cp 0.060) 로 회복했고,
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5.
        하지만 V66 의 1,200 표본 rawBridge surfaces=4,328/hits=40,345, totalSeconds=49.7 과
        거의 같은 footprint 이며 더 느리다.

    제거 surface 진단:
        scoreFloor=0.38 이 제거한 상위 bridge 조각은 `매출액또는손익구`, `규모법인은1`,
        `기매출채권및기타`, `기손익인식금융자`, `감소하였습` 같은 공시 제목/표 boilerplate 조각이었다.
        이 조각을 제거하자 `영업손익 -> 영업이익` 이 accepted 에서 떨어졌다.

판정:
    실패/진단 성공. seed evidence score 로 bridge substring 을 hard-gate 하면 rawBridge hits 는
    V66 40,345 -> 36,463 으로 줄지만 positiveHits 가 4/4 -> 3/4 로 깨진다. 반대로 scoreFloor 를
    0 으로 낮추면 품질은 회복되지만 V66 과 같은 footprint 로 돌아가며 속도도 좋아지지 않는다.

    핵심 진단은 V66 의 `영업손익 -> 영업이익` 성공 일부가 깨끗한 stem 의미 그래프가 아니라
    `매출액또는손익구...` 같은 공시 제목/표 조각의 bridge 경험에 기대고 있었다는 점이다. 따라서 다음 실험은
    bridge 후보를 scalar score 로 자르는 방향이 아니라, 공시 제목/표 boilerplate 경험과 본문 relation-bound
    경험을 lane 으로 분리해야 한다. 즉 bridge pseudo surface 를 더 세게 줄이는 것이 아니라,
    같은 substring 이라도 title/table lane 과 sentence/relation-owner lane 의 경험을 분리해 의미 경로가
    artifact 없이 살아남는지 봐야 한다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V68_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V68_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V68_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V68_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V68_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V68_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V68_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V68_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V68_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V68_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V68_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V68_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V68_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V68_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V68_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V68_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V68_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V68_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V68_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V68_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V68_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V68_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V68_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V68_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V68_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V68_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V68_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V68_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V68_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V68_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V68_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V68_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V68_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V68_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V68_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V68_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V68_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V68_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V68_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V68_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V68_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V68_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V68_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V68_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V68_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V68_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V68_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V68_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
CORPUS_BRIDGE_SUBSURFACE_SCORE_FLOOR = float(
    os.environ.get("DARTLAB_HORIZON_V68_CORPUS_BRIDGE_SUBSURFACE_SCORE_FLOOR", "0.38")
)
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V68_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V68_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V68_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V68_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V68_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V68_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V68_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V68_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V68_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V68_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V68_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V68_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V68_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V68_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V68_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V68_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V68_SKETCH_OCC_BUCKETS", "16"))
...
def rawBridgeSeedScore(surface: str, bridgeSeedIndex: BridgeSeedIndex) -> float:
    value = normStem(surface)
    if len(value) < RAW_BRIDGE_MIN_SIZE or not isContentStem(value):
        return 0.0
    grams = compoundGrams(value)
    ...
    exactScore = bridgeSeedIndex.seedScores.get(value, 0.0)
    if exactScore > 0.0:
        return min(1.25, 0.40 + exactScore * 0.72)
    ...
    for seedValue in candidateSeeds:
        ...
        if len(overlap) < 2:
            continue
        similarity = len(overlap) / math.sqrt(len(grams) * len(seedGrams))
        containment = value in seedValue or seedValue in value
        if not containment and (
            len(overlap) < 3 or similarity < max(RAW_BRIDGE_MIN_SIM, CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM)
        ):
            continue
        seedScore = bridgeSeedIndex.seedScores.get(seedValue, 0.0)
        if seedScore <= 0.0:
            continue
        relationStrength = 0.72 + 0.28 * min(1.0, similarity)
        if containment:
            relationStrength += 0.10
        lengthSpecificity = min(1.0, len(value) / max(RAW_BRIDGE_MIN_SIZE + 2, len(seedValue)))
        score = seedScore * relationStrength * (0.82 + 0.18 * lengthSpecificity)
        best = max(best, score)
    return best
...
def rawBridgeSubsurfaces(stem: str, bridgeSeedIndex: BridgeSeedIndex | None) -> tuple[str, ...]:
    ...
    if len(value) < RAW_BRIDGE_MIN_SIZE + 1 or len(value) > RAW_BRIDGE_MAX_TOKEN:
        ...
        return tuple()
    out: dict[str, tuple[int, int, float, str]] = {}
    ...
    for size in range(RAW_BRIDGE_MIN_SIZE, maxSize + 1):
        for index in range(0, len(value) - size + 1):
            ...
            if part == value:
                continue
            if not rawBridgeSeedMatch(part, bridgeSeedIndex):
                continue
            score = rawBridgeSeedScore(part, bridgeSeedIndex)
            if score >= CORPUS_BRIDGE_SUBSURFACE_SCORE_FLOOR:
                seedAnchor = 0 if part in bridgeSeedIndex.seedValues else 1
                out[part] = min(out.get(part, (9, 0, 0.0, part)), (seedAnchor, -len(part), -score, part))
    surfaces = tuple(part for _, _, _, part in sorted(out.values())[:CORPUS_BRIDGE_SUBSURFACE_LIMIT])
...
def buildCorpusBridgeSeedIndex(caches: list[Cache]) -> BridgeSeedIndex:
    ...
    maxSeedScore = selected[0][0] if selected else 1.0
    seedScores: dict[str, float] = {}
    selectedCount = max(1, len(selected))
    for rank, (score, _, _, value) in enumerate(selected):
        rankScore = 1.0 - (rank / selectedCount)
        evidenceScore = score / maxSeedScore if maxSeedScore else 0.0
        seedScores[value] = 0.58 * evidenceScore + 0.42 * rankScore
    ...
    print(
        ...
        f"evidenceSurfaces={len(evidenceDf)} minEvidence={CORPUS_BRIDGE_MIN_EVIDENCE} "
        f"subsurfaceLimit={CORPUS_BRIDGE_SUBSURFACE_LIMIT} scoreFloor={CORPUS_BRIDGE_SUBSURFACE_SCORE_FLOOR} "
        f"top={topSample}"
    )
...
def buildModel() -> Model:
    ...
    print(
        f"[rawBridge] mode=seedScoredBridgeCompression surfaces={len(bridgeSurfaceUniverse)} "
