"""Horizon Meaning Learner V69 - artifact/owner lane separated experience.
...
아이디어:
    ...
    V69 는 bridge 후보를 더 세게 자르는 대신 경험 lane 을 분리한다. token occurrence 를 `sentence`,
    `artifact`, `owner` lane 으로 분류하고, bridge pseudo occurrence 는 원 token 의 lane 을 물려받는다.
    line atom 에는 artifact/owner lane self/order 신호만 가볍게 추가하고 sentence token 전체에는 lane atom 을
    붙이지 않는다. 목적은 title/table artifact 경험을 sentence/relation-owner 경험과 분리해 의미 recall 을
    유지하면서 `복구충당금`, `대출채권` 같은 오염 점수를 낮출 수 있는지 보는 것이다. target alias, target/probe
    seed, 수동 family lock 은 추가하지 않는다.
...
시도 방법:
    ...
    45. `lineAtoms()` 에 artifact/owner self lane atom 과 lane-aware order atom 만 추가하고, sentence token 전체에는
        lane atom 을 붙이지 않아 lane 분리 비용을 제한한다.
    46. horizon view, sketch sample, signature sample 에 lane count 를 출력해 실제 경험 분포가 분리되는지 확인한다.

실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV69Test.py

    $env:DARTLAB_HORIZON_V69_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V69_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V69_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V69_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV69Test.py

    $env:DARTLAB_HORIZON_V69_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V69_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V69_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V69_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV69Test.py
...
결과:
    1,200 units + direct side payload 600개:
        ...
        lanes={'sentence': 65341, 'artifact': 18653, 'owner': 12265}, buildSignatures=22.7s.
        signatureStage view=0.2s, sample=2.5s, raw=6.3s, prune=2.7s, weight=2.6s,
        relaySource=1.4s, relay=7.0s.
        relay views=10,027, updates=826,994, view=0.3s, build=6.7s.
        focusedRelation relationOcc=1,312, spanPairs=81,891, localPairs=150,820,
        ownerFrames=1,312, buildFocusedRelationIndexes=3.4s.
    ...
    4,000 units + direct side payload 600개:
        ...
        lanes={'sentence': 136450, 'artifact': 36514, 'owner': 35209}, buildSignatures=47.6s.
        signatureStage view=0.2s, sample=7.6s, raw=14.0s, prune=4.2s, weight=4.2s,
        relaySource=1.6s, relay=15.6s.
        relay views=15,332, updates=1,273,751, view=0.4s, build=15.2s.
        ...
        modelSeconds=106.5, totalSeconds=113.4.
        `외상매출금 -> 매출채권` accepted=True(score 1.166, cp 0.624), `매출액` 2순위는
        accepted=False. `현금성자산 -> 현금및현금성자산` accepted=True(score 2.238).
        `영업손익 -> 영업이익` accepted=True(score 1.454).
        `손실충당금 -> 대손충당금` accepted=True(score 0.115).
        `복구충당금 -> 대손충당금` 은 score 0.296 로 accepted=False.
        `대출채권 -> 대손충당금` 은 score 0.023 으로 accepted=False.
        `대출채권 -> 매출채권` 은 forbidden=-0.105 로 accepted=False.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5.

판정:
    부분 성공/진단 성공. V68 의 scalar bridge score gate 는 recall 을 끊었지만, V69 의 lane 분리는
    positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5 를 유지했다. 또한 4,000 기준
    `복구충당금 -> 대손충당금` 오염 점수는 V66 0.442 -> V69 0.296, `대출채권 -> 대손충당금` 은
    V66 0.201 -> V69 0.023 으로 낮아졌다. artifact/owner 경험을 별도 lane 으로 들고 가는 방향은 오염 억제에
    실제 신호가 있다.

    하지만 본진 투입 수준은 아니다. 4,000 기준 totalSeconds 는 V66 100.4 -> V69 113.4 로 악화됐고,
    buildSignatures 는 40.3s -> 47.6s, tokenizeBase 는 3.2s -> 5.6s 로 늘었다. 다음 실험은 lane 을 raw
    signature atom 으로 늘리는 대신, lane count/view 를 acceptance contrast 또는 route penalty 에만 쓰는 방식으로
    비용을 줄여야 한다. 즉 lane materialization 은 유지하되 경험 atom fanout 은 더 얇게 만들어야 한다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V69_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V69_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V69_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V69_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V69_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V69_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V69_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V69_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V69_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V69_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V69_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V69_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V69_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V69_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V69_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V69_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V69_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V69_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V69_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V69_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V69_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V69_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V69_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V69_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V69_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V69_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V69_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V69_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V69_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V69_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V69_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V69_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V69_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V69_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V69_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V69_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V69_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V69_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V69_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V69_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V69_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V69_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V69_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V69_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V69_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V69_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V69_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V69_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V69_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V69_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V69_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V69_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V69_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V69_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V69_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V69_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V69_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V69_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V69_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V69_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V69_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V69_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V69_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V69_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V69_SKETCH_OCC_BUCKETS", "16"))
...
def lineAtoms(
    ...
    atoms: set[str] = set()
    stem = stems[pos]
    lane = lanes[pos] if lanes and pos < len(lanes) else "sentence"
    atoms.update(sketchView.selfAtoms.get(stem, ()))
    selfCell = sketchView.cells.get(stem, coldSketchCell(stem))
    if lane != "sentence":
        atoms.add(f"xp:lane:{lane}:self:{selfCell}")
    cells: dict[int, str] = {}
    laneCells: dict[int, str] = {}
    for index in range(max(0, pos - RADIUS), min(len(stems), pos + RADIUS + 1)):
        neighborStem = stems[index]
        if not isContentStem(neighborStem):
            continue
        neighborLane = lanes[index] if lanes and index < len(lanes) else "sentence"
        cells[index] = sketchView.cells.get(neighborStem, coldSketchCell(neighborStem))
        laneCells[index] = neighborLane
        if index == pos:
            continue
        dist = index - pos
        side = "L" if dist < 0 else "R"
    ...
    for start in range(pos - 2, pos + 1):
        ...
        if all(index in cells for index in idxs):
            ...
            if any(laneCells[index] != "sentence" for index in idxs):
                atoms.add(
                    f"el:lane:{'.'.join(str(index - pos) for index in idxs)}:"
                    f"{'>'.join(laneCells[index] for index in idxs)}:"
                    f"{'>'.join(cells[index] for index in idxs)}"
                )
    if pos - 1 in cells and pos in cells and pos + 1 in cells:
        atoms.add(f"el:lr:{cells[pos - 1]}>{cells[pos]}>{cells[pos + 1]}")
        if any(laneCells[index] != "sentence" for index in (pos - 1, pos, pos + 1)):
            atoms.add(
                f"el:lane:lr:{laneCells[pos - 1]}>{laneCells[pos]}>{laneCells[pos + 1]}:"
                f"{cells[pos - 1]}>{cells[pos]}>{cells[pos + 1]}"
            )
    return atoms
