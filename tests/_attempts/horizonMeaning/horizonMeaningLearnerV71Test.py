"""Horizon Meaning Learner V71 - post-relation lane profile.
...
아이디어:
    ...
    V70 은 lane 을 signature atom 으로 섞지 않고 surface 별 lane profile 을 route penalty 에만 써서
    V69 보다 빨라졌다. 하지만 tokenization 단계에서 모든 token 의 relation/value proximity 를 계산해
    lane 을 붙이므로 tokenizeBase 가 V66 보다 느렸다. V71 은 tokenization 의 lane 계산을 제거하고 모든 occurrence 를
    sentence 로 둔다. 대신 focused relation index 가 이미 계산한 table leak 과 role-bound score 에서
    surface 별 artifact/owner profile 을 사후 집계한다. 목적은 V70 의 profile/penalty 아이디어를 유지하면서
    tokenization overhead 를 없앨 수 있는지 검증하는 것이다. target alias, target/probe seed, 수동 family lock 은
    추가하지 않는다.
...
시도 방법:
    ...
    42. token 하나에서 추가하는 bridge pseudo surface 는 seed anchor/길이 기준 상위 4개로 제한해 V65 의 과도한
        bridge fanout 을 줄인다.
    43. tokenization 에서는 token lane 을 계산하지 않고 모든 cache lane 을 `sentence` 로 둔다.
    44. rawBridge pseudo occurrence 도 position 만 상속하고 lane 은 sentence 로 둔다.
    45. `lineAtoms()` 에서는 lane atom 을 추가하지 않고 기존 coordinate/experience atom 만 만든다.
    46. `buildFocusedRelationIndexes()` 이후 frame leak unit 은 artifact, bound score unit 은 owner evidence 로
        사후 집계해 surface lane profile 을 만든다.
    47. route 단계의 artifact mismatch 와 low-resonance owner transfer penalty 는 V70 과 동일하게 유지한다.
    48. lane profile probe 를 출력해 사후 profile 이 token-level profile 을 대체할 수 있는지 확인한다.

실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV71Test.py

    $env:DARTLAB_HORIZON_V71_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V71_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V71_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V71_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV71Test.py

    $env:DARTLAB_HORIZON_V71_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V71_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V71_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V71_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV71Test.py
...
결과:
    1,200 units + direct side payload 600개:
        tokenizeBase=1.0s, buildCorpusBridgeSeedIndex=1.2s, augmentBridgeSurfaces=0.8s.
        ...
        lanes={'sentence': 175983}.
        sketchSample=150,989 -> 116,480, lanes={'sentence': 116480}, buildSketches=7.8s.
        signature occSample=150,989 -> 96,259,
        lanes={'sentence': 96259}, buildSignatures=22.9s.
        rawPrune atoms=2,933,152 -> 1,812,361, el=58,222.
        signatureStage view=0.5s, sample=2.2s, raw=6.4s, prune=2.8s, weight=2.6s,
        relaySource=1.4s, relay=6.9s.
        relay views=9,967, updates=807,781, view=0.3s, build=6.6s.
        focusedRelation relationOcc=1,312, spanPairs=81,891, localPairs=150,820,
        ownerFrames=1,312, buildFocusedRelationIndexes=3.6s.
        postRelation laneProfile surfaces=10,902, artifactHeavy=933, ownerHeavy=440.
        sideFocusedBound payloads=600, relationOcc=1,575, localPairs=102,451,
        ownerFrames=1,575, buildSideBoundPayloadIndex=1.8s.
        modelSeconds=47.3, totalSeconds=49.8.
        laneProbe: 외상매출금 S1.00/A0.00/O0.00, 매출채권 S0.48/A0.49/O0.02,
        대손충당금 S0.27/A0.65/O0.08, 복구충당금 S1.00/A0.00/O0.00.
        `외상매출금 -> 매출채권` accepted=True(score 0.083, cp 0.055), `매출액` 2순위는
        ...
        `복구충당금 -> 대손충당금` 은 score 1.430 로 accepted=False.
        `대출채권 -> 대손충당금` 은 score 0.193 로 accepted=False.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5.

    4,000 units + direct side payload 600개:
        tokenizeBase=3.7s, buildCorpusBridgeSeedIndex=3.9s, augmentBridgeSurfaces=1.8s.
        ...
        lanes={'sentence': 607533}.
        sketchSample=505,826 -> 275,808, lanes={'sentence': 275808}, buildSketches=19.4s.
        signature occSample=505,826 -> 208,173,
        lanes={'sentence': 208173}, buildSignatures=45.9s.
        rawPrune atoms=4,607,269 -> 2,757,382, el=94,220.
        signatureStage view=0.3s, sample=7.4s, raw=12.6s, prune=4.4s, weight=4.0s,
        relaySource=1.6s, relay=15.6s.
        relay views=15,233, updates=1,247,122, view=0.4s, build=15.2s.
        focusedRelation relationOcc=4,825, spanPairs=290,683, localPairs=539,818,
        ownerFrames=4,825, buildFocusedRelationIndexes=12.0s.
        postRelation laneProfile surfaces=16,386, artifactHeavy=1,746, ownerHeavy=655.
        sideFocusedBound payloads=600, relationOcc=1,870, localPairs=123,609,
        ownerFrames=1,870, buildSideBoundPayloadIndex=2.0s.
        modelSeconds=103.0, totalSeconds=110.1.
        laneProbe: 외상매출금 S1.00/A0.00/O0.00, 매출채권 S0.63/A0.35/O0.01,
        대손충당금 S0.32/A0.65/O0.03, 복구충당금 S1.00/A0.00/O0.00.
        `외상매출금 -> 매출채권` accepted=True(score 1.064, cp 0.624), `매출액` 2순위는
        accepted=False. `현금성자산 -> 현금및현금성자산` accepted=True(score 2.311).
        `영업손익 -> 영업이익` accepted=True(score 1.404).
        `손실충당금 -> 대손충당금` accepted=True(score 0.115).
        `복구충당금 -> 대손충당금` 은 score 0.354 로 accepted=False.
        `대출채권 -> 대손충당금` 은 score 0.023 로 accepted=False.
        `대출채권 -> 매출채권` 은 forbidden=-0.105 로 accepted=False.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5.

판정:
    실패/진단 성공. token lane 계산을 제거해 tokenizeBase 는 4,000 기준 V70 5.6s -> V71 3.7s 로 줄었다.
    그러나 focused relation 이후 leak/bound 로 만든 사후 profile 은 token-level lane profile 을 대체하지 못했다.
    `대손충당금` 이 owner profile 이 아니라 artifact-heavy(S0.32/A0.65/O0.03) 로 잡혔고,
    `복구충당금 -> 대손충당금` 오염 점수는 V70 0.285 -> V71 0.354 로 나빠졌다.

    속도도 본진 가치가 없다. 4,000 기준 totalSeconds 는 V70 104.1 -> V71 110.1 로 악화됐고,
    buildSignatures/relay 변동도 이득을 상쇄했다. 다음은 사후 profile 이 아니라 V70 의 token-level profile 품질을
    유지하되, relation/value nearest distance 계산을 bisect 반복이 아닌 선형 precompute 로 줄이는 방향이 맞다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V71_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V71_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V71_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V71_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V71_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V71_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V71_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V71_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V71_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V71_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V71_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V71_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V71_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V71_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V71_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V71_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V71_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V71_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V71_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V71_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V71_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V71_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
LANE_MISMATCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V71_LANE_MISMATCH_PENALTY", "0.18"))
LANE_ARTIFACT_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V71_LANE_ARTIFACT_PENALTY", "0.10"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V71_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V71_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V71_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V71_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V71_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V71_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V71_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V71_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V71_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V71_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V71_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V71_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V71_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V71_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V71_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V71_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V71_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V71_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V71_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V71_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V71_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V71_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V71_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V71_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V71_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V71_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V71_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V71_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V71_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V71_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V71_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V71_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V71_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V71_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V71_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V71_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V71_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V71_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V71_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V71_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V71_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V71_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V71_SKETCH_OCC_BUCKETS", "16"))
...
def buildSurfaceLaneProfiles(
    caches: list[Cache],
    frameLeaks: dict[tuple[int, str, str], float],
    boundScores: dict[tuple[int, str, str], float],
) -> dict[str, tuple[float, float, float]]:
    surfaceUnits: dict[str, set[int]] = defaultdict(set)
    for cache in caches:
        unitSurfaces = {occ.surface for occ in cache.occs}
        for surface in unitSurfaces:
            surfaceUnits[surface].add(cache.unit.unitId)

    artifactUnits: dict[str, set[int]] = defaultdict(set)
    for unitId, surface, _ in frameLeaks:
        artifactUnits[surface].add(unitId)

    ownerUnits: dict[str, set[int]] = defaultdict(set)
    for (unitId, surface, _), score in boundScores.items():
        if score >= WEAK_BOUND_MIN:
            ownerUnits[surface].add(unitId)

    profiles: dict[str, tuple[float, float, float]] = {}
    for surface, units in surfaceUnits.items():
        df = len(units)
        if df <= 0:
            continue
        artifact = min(0.65, len(artifactUnits.get(surface, ())) * 2.0 / (df + 1.0))
        owner = min(0.55, len(ownerUnits.get(surface, ())) * 2.2 / (df + 1.0))
        if artifact + owner > 0.88:
            scale = 0.88 / (artifact + owner)
            artifact *= scale
            owner *= scale
        sentence = max(0.0, 1.0 - artifact - owner)
        profiles[surface] = (sentence, artifact, owner)
    ...
    print(
        f"[laneProfile] mode=postRelation surfaces={len(profiles)} "
        f"artifactHeavy={artifactHeavy} ownerHeavy={ownerHeavy}"
    )
    return profiles
...
def tokenize(unit: Unit) -> Cache:
    ...
    for pos, (stem, marker, isContent) in enumerate(zip(stems, markers, contentFlags)):
        if isContent:
            occs.append(Occ(stem, marker, pos, "sentence"))
    terms = set(TOKEN_RE.findall(unit.text)) | relKeys(unit.text)
    lanes = ["sentence"] * len(stems)
...
def buildModel() -> Model:
    ...
    surfaceLaneProfiles = buildSurfaceLaneProfiles(caches, relationFrameLeaks, relationBoundScores)
