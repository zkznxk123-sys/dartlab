"""Horizon Meaning Learner V72 - linear token lane distance.
...
아이디어:
    ...
    V70 은 token-level lane profile 품질을 유지하면서 route penalty 로 오염을 낮췄다. V71 은 token lane 계산을
    제거하고 relation index 에서 profile 을 사후 집계했지만, `대손충당금` 을 owner 가 아니라 artifact-heavy 로
    잘못 보아 실패했다. V72 는 V70 의 token-level profile 품질은 유지하되, token 마다 bisect 로 relation/value
    nearest distance 를 찾는 방식을 선형 distance vector precompute 로 바꾼다. 목적은 profile 품질을 잃지 않고
    tokenizeBase 비용만 줄일 수 있는지 검증하는 것이다. target alias, target/probe seed, 수동 family lock 은
    추가하지 않는다.
...
시도 방법:
    ...
    42. token 하나에서 추가하는 bridge pseudo surface 는 seed anchor/길이 기준 상위 4개로 제한해 V65 의 과도한
        bridge fanout 을 줄인다.
    43. relation/value token 위치를 먼저 모으고, 각 token 의 nearest relation/value distance 를 선형 pass 로
        미리 계산한다.
    44. token lane 은 V70 과 같은 규칙을 쓰되, `tokenLaneAt()` 에 precomputed distance 만 넘겨 bisect 반복을 없앤다.
    45. rawBridge pseudo occurrence 는 원 token 의 position 뿐 아니라 lane 도 함께 상속한다.
    46. `lineAtoms()` 에서는 lane atom 을 추가하지 않고 기존 coordinate/experience atom 만 만든다.
    47. cache occurrence 전체에서 surface 별 `sentence/artifact/owner` lane profile 을 별도 집계한다.
    ...
    49. lane profile probe 를 출력해 V70 과 같은 profile 품질이 유지되는지 확인한다.

실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV72Test.py

    $env:DARTLAB_HORIZON_V72_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V72_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V72_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V72_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV72Test.py

    $env:DARTLAB_HORIZON_V72_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V72_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V72_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V72_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV72Test.py
...
결과:
    1,200 units + direct side payload 600개:
        tokenizeBase=1.7s, buildCorpusBridgeSeedIndex=1.3s, augmentBridgeSurfaces=0.8s.
        ...
        lanes={'sentence': 65341, 'artifact': 18653, 'owner': 12265}, buildSignatures=22.4s.
        rawPrune atoms=2,933,152 -> 1,812,361, el=58,222.
        signatureStage view=0.2s, sample=2.6s, raw=5.8s, prune=2.6s, weight=2.6s,
        relaySource=1.4s, relay=7.1s.
        relay views=9,967, updates=806,406, view=0.2s, build=6.9s.
        ...
        `복구충당금 -> 대손충당금` 은 score 1.368 로 accepted=False.
    ...
    4,000 units + direct side payload 600개:
        tokenizeBase=5.7s, buildCorpusBridgeSeedIndex=4.0s, augmentBridgeSurfaces=2.0s.
        ...
        lanes={'sentence': 136450, 'artifact': 36514, 'owner': 35209}, buildSignatures=47.5s.
        rawPrune atoms=4,607,269 -> 2,757,382, el=94,220.
        signatureStage view=0.3s, sample=7.6s, raw=12.9s, prune=4.2s, weight=4.2s,
        relaySource=1.6s, relay=16.8s.
        relay views=15,233, updates=1,246,113, view=0.4s, build=16.4s.
...
판정:
    실패/진단 성공. V72 는 V70 과 같은 lane profile 품질과 품질 카운트
    positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5 를 유지했다. 하지만 nearest distance 를
    선형 precompute 로 바꿔도 tokenizeBase 는 4,000 기준 V70 5.6s -> V72 5.7s 로 줄지 않았다. 거리 lookup 이
    병목이 아니라 lane 판정의 regex/owner 후보 판정 또는 전체 build 변동이 병목이라는 진단이다.

    본진 투입 가치는 없다. 4,000 totalSeconds 는 V70 104.1 -> V72 114.3 으로 악화됐다. 다음은 distance 계산이
    아니라 `tokenLaneAt()` 내부의 artifact regex, digit scan, owner 후보 판정을 cache/view 로 줄이거나,
    lane profile 을 모든 token 이 아니라 route/target 후보 surface 주변 occurrence 에만 제한하는 구조를 검토해야 한다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V72_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V72_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V72_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V72_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V72_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V72_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V72_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V72_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V72_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V72_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V72_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V72_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V72_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V72_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V72_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V72_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V72_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V72_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V72_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V72_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V72_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V72_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
LANE_MISMATCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V72_LANE_MISMATCH_PENALTY", "0.18"))
LANE_ARTIFACT_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V72_LANE_ARTIFACT_PENALTY", "0.10"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V72_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V72_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V72_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V72_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V72_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V72_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V72_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V72_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V72_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V72_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V72_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V72_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V72_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V72_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V72_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V72_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V72_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V72_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V72_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V72_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V72_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V72_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V72_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V72_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V72_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V72_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V72_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V72_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V72_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V72_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V72_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V72_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V72_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V72_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V72_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V72_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V72_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V72_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V72_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V72_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V72_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V72_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V72_SKETCH_OCC_BUCKETS", "16"))
...
def tokenLaneAt(
    stem: str,
    marker: str,
    relationDistance: int | None,
    valueDistance: int | None,
) -> str:
...
def nearestTokenDistanceVector(size: int, positions: list[int]) -> list[int | None]:
    if not positions:
        return [None] * size
    markers = [False] * size
    ...
    for index in range(size - 1, -1, -1):
        ...
        if nearest is not None:
            ...
            distances[index] = distance if current is None else min(current, distance)
    return distances
...
def tokenize(unit: Unit) -> Cache:
    ...
    relationDistances = nearestTokenDistanceVector(len(stems), relationPositions)
    valueDistances = nearestTokenDistanceVector(len(stems), valuePositions)
    for pos, (stem, marker, isContent) in enumerate(zip(stems, markers, contentFlags)):
        lane = tokenLaneAt(stem, marker, relationDistances[pos], valueDistances[pos]) if isContent else "sentence"
        lanes.append(lane)
        if isContent:
