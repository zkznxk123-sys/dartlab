"""Horizon Meaning Learner V74 - lazy route lane profile.
...
아이디어:
    ...
    V70 은 token-level lane profile 을 route penalty 에 사용해 오염을 낮췄지만 build-time 전체 occurrence 에 대해
    lane 을 계산했다. V71 의 사후 profile 은 품질을 잃었고 V72/V73 의 lane 판정 최적화도 속도 개선이 없었다.
    V74 는 tokenization 에서는 모든 occurrence 를 sentence 로 두고, surface -> occurrence ref 만 저장한다.
    route 에서 실제 query/target surface 가 필요할 때만 해당 surface 의 occurrence 를 훑어 V70 과 같은
    tokenLaneAt profile 을 lazy 계산해 캐시한다. 목적은 V70 의 lane profile 품질을 유지하면서 build-time 전체
    lane 계산을 없앨 수 있는지 검증하는 것이다. target alias, target/probe seed, 수동 family lock 은 추가하지 않는다.
...
시도 방법:
    ...
    42. token 하나에서 추가하는 bridge pseudo surface 는 seed anchor/길이 기준 상위 4개로 제한해 V65 의 과도한
        bridge fanout 을 줄인다.
    43. tokenization 에서는 token lane 을 계산하지 않고 모든 occurrence lane 을 sentence 로 둔다.
    44. cache 별 surface occurrence ref 를 저장하고, build-time lane profile 집계는 하지 않는다.
    45. route 에서 `surfaceLaneProfile(surface)` 가 호출될 때 해당 surface occurrence 만 V70 규칙으로 lane 판정한다.
    46. cache 별 relation/value position 은 lazy profile 계산 중 cache 해서 같은 cache 재계산을 줄인다.
    47. `lineAtoms()` 에서는 lane atom 을 추가하지 않고 기존 coordinate/experience atom 만 만든다.
    48. route 단계에서 artifact mismatch 와 resonance 가 낮은 owner-lane 전이를 penalty 로 넣어, 경험 atom fanout 없이
        title/table/owner 오염을 낮출 수 있는지 확인한다.
    49. lane profile probe 를 출력해 lazy profile 이 V70 과 같은 profile 품질을 복원하는지 확인한다.

실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV74Test.py

    $env:DARTLAB_HORIZON_V74_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V74_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V74_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V74_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV74Test.py

    $env:DARTLAB_HORIZON_V74_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V74_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V74_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V74_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV74Test.py
...
결과:
    1,200 units + direct side payload 600개:
        tokenizeBase=1.1s, buildCorpusBridgeSeedIndex=1.2s, augmentBridgeSurfaces=0.8s.
        ...
        sketchSample=150,989 -> 116,480, lanes={'sentence': 116480}, buildSketches=7.9s.
        signature occSample=150,989 -> 96,259,
        lanes={'sentence': 96259}, buildSignatures=22.4s.
        rawPrune atoms=2,933,152 -> 1,812,361, el=58,222.
        signatureStage view=0.5s, sample=2.3s, raw=5.8s, prune=2.6s, weight=2.6s,
        relaySource=1.4s, relay=7.0s.
        relay views=9,967, updates=808,167, view=0.3s, build=6.7s.
        laneRefs surfaces=10,902, refs=150,989.
        ...
        modelSeconds=47.3, totalSeconds=49.9.
        ...
        `복구충당금 -> 대손충당금` 은 score 1.372 로 accepted=False.
    ...
    4,000 units + direct side payload 600개:
        tokenizeBase=3.7s, buildCorpusBridgeSeedIndex=4.0s, augmentBridgeSurfaces=1.9s.
        ...
        sketchSample=505,826 -> 275,808, lanes={'sentence': 275808}, buildSketches=20.1s.
        signature occSample=505,826 -> 208,173,
        lanes={'sentence': 208173}, buildSignatures=46.5s.
        rawPrune atoms=4,607,269 -> 2,757,382, el=94,220.
        signatureStage view=0.3s, sample=7.6s, raw=12.5s, prune=4.3s, weight=4.2s,
        relaySource=1.6s, relay=15.9s.
        relay views=15,233, updates=1,246,617, view=0.4s, build=15.5s.
        laneRefs surfaces=16,386, refs=505,826.
        focusedRelation relationOcc=4,825, spanPairs=290,683, localPairs=539,818,
        ownerFrames=4,825, buildFocusedRelationIndexes=11.8s.
        sideFocusedBound payloads=600, relationOcc=1,870, localPairs=123,609,
        ownerFrames=1,870, buildSideBoundPayloadIndex=2.0s.
        modelSeconds=104.3, totalSeconds=112.8.
        laneProbe: 외상매출금 S1.00/A0.00/O0.00, 매출채권 S0.63/A0.35/O0.01,
        대손충당금 S0.71/A0.00/O0.29, 복구충당금 S1.00/A0.00/O0.00.
        `외상매출금 -> 매출채권` accepted=True(score 1.119, cp 0.624), `매출액` 2순위는
        accepted=False. `현금성자산 -> 현금및현금성자산` accepted=True(score 2.310).
        `영업손익 -> 영업이익` accepted=True(score 1.456).
        `손실충당금 -> 대손충당금` accepted=True(score 0.115).
        `복구충당금 -> 대손충당금` 은 score 0.285 로 accepted=False.
        `대출채권 -> 대손충당금` 은 score -0.134 로 accepted=False.
        `대출채권 -> 매출채권` 은 forbidden=-0.124 로 accepted=False.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5.

판정:
    실패/진단 성공. V74 는 lazy profile 로 V70 과 같은 laneProbe 와 route/search 품질
    positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5 를 복원했다. tokenization 도 4,000 기준
    V70 5.6s -> V74 3.7s 로 줄었다.

    그러나 전체 성능은 실패다. 4,000 totalSeconds 는 V70 104.1 -> V74 112.8 로 악화됐고,
    buildSignatures/relay 변동이 이득을 상쇄했다. lazy profile 은 build-time lane 계산 제거와 품질 보존 가능성을
    보였지만, 현재 파이프라인에서는 signature/relay 병목이 더 커서 본진 투입 가치가 없다. 다음은 lane 계열 최적화보다
    V66/V70 공통 병목인 buildSignatures relay/raw stage 를 다시 줄이는 구조가 맞다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V74_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V74_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V74_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V74_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V74_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V74_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V74_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V74_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V74_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V74_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V74_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V74_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V74_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V74_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V74_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V74_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V74_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V74_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V74_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V74_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V74_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V74_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
LANE_MISMATCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V74_LANE_MISMATCH_PENALTY", "0.18"))
LANE_ARTIFACT_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V74_LANE_ARTIFACT_PENALTY", "0.10"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V74_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V74_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V74_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V74_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V74_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V74_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V74_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V74_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V74_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V74_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V74_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V74_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V74_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V74_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V74_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V74_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V74_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V74_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V74_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V74_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V74_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V74_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V74_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V74_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V74_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V74_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V74_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V74_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V74_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V74_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V74_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V74_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V74_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V74_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V74_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V74_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V74_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V74_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V74_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V74_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V74_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V74_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V74_SKETCH_OCC_BUCKETS", "16"))
...
class Model:
    ...
    surfaceOccurrenceRefs: dict[str, list[tuple[int, int]]]
    lanePositionCache: dict[int, tuple[list[int], list[int]]]
...
def buildSurfaceOccurrenceRefs(caches: list[Cache]) -> dict[str, list[tuple[int, int]]]:
    refs: dict[str, list[tuple[int, int]]] = defaultdict(list)
    totalRefs = 0
    for cacheIndex, cache in enumerate(caches):
        for occ in cache.occs:
            refs[occ.surface].append((cacheIndex, occ.position))
            totalRefs += 1
    print(f"[laneRefs] surfaces={len(refs)} refs={totalRefs}")
    return dict(refs)
...
def surfaceLaneProfile(surface: str, model: Model) -> tuple[float, float, float]:
    ...
    if profile is not None:
        return profile
    refs = model.surfaceOccurrenceRefs.get(stem)
    if not refs:
        profile = (1.0, 0.0, 0.0)
        model.surfaceLaneProfiles[stem] = profile
        return profile

    counts: Counter[str] = Counter()
    for cacheIndex, position in refs:
        cache = model.caches[cacheIndex]
        relationPositions, valuePositions = cachedLanePositions(cacheIndex, cache, model)
        lane = tokenLaneAt(cache.stems[position], cache.markers[position], position, relationPositions, valuePositions)
        counts[lane] += 1
...
def cachedLanePositions(cacheIndex: int, cache: Cache, model: Model) -> tuple[list[int], list[int]]:
    cached = model.lanePositionCache.get(cacheIndex)
    if cached is not None:
        return cached
    relationPositions = sorted(
        index for index, stem in enumerate(cache.stems) if isContentStem(stem) and relationTokenLike(stem)
    ...
    valuePositions = sorted(
        index for index, (stem, marker) in enumerate(zip(cache.stems, cache.markers)) if valueTokenLike(stem, marker)
    )
    cached = (relationPositions, valuePositions)
    model.lanePositionCache[cacheIndex] = cached
    return cached
...
def buildModel() -> Model:
    ...
    surfaceOccurrenceRefs = buildSurfaceOccurrenceRefs(caches)
    stage("buildSurfaceOccurrenceRefs")
