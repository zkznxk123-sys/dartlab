"""Horizon Meaning Learner V75 - sampled signature line atom cache.
...
아이디어:
    ...
    V69~V74 는 lane/profile 계열을 여러 방식으로 분리했지만, V70 의 품질을 유지하면서 V66 보다 빨라지는
    구조까지 가지 못했다. 남은 병목은 `buildSignatures` 의 raw/relay 단계다. V75 는 V70 으로 돌아가
    signature occurrence sampling 이후의 실제 `(unitId, position)` 중복을 이용한다. rawBridge pseudo surface 는
    같은 위치에 여러 표면을 얹으므로 `lineAtoms()` 를 surface 별로 반복 계산한다. V51 의 예전 line cache 는 hit 가
    낮아 실패했지만, V54 이후 focused occurrence sampling 과 V66 corpus bridge 제한 뒤에는 4,000 표본에서
    77,687 회의 중복 line position 이 생긴다. V75 는 정답 alias, target/probe seed, 수동 family lock 없이
    위치 경험 atom 묶음만 캐시해 raw signature 비용을 줄일 수 있는지 검증한다.
...
시도 방법:
    ...
    49. V70 의 `buildSignatures` raw loop 에서 `(unitId, position)` 별 `lineAtoms()` 결과를 캐시한다.
    50. horizon atom cache 와 별도로 line atom cache 의 miss/hit/cache size 를 출력해 bridge pseudo surface 중복이
        실제 raw 비용으로 연결되는지 분리한다.
    51. tuple cache 와 raw 직후 clear 도 진단했지만, 최종 기록은 가장 나았던 set cache 버전으로 남긴다.

실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV75Test.py

    $env:DARTLAB_HORIZON_V75_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V75_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V75_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V75_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV75Test.py

    $env:DARTLAB_HORIZON_V75_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V75_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V75_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V75_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV75Test.py

판정 기준:
    V70 과 같은 품질 기준을 유지해야 한다. 즉 `외상매출금 -> 매출채권`, `영업손익 -> 영업이익`,
    `현금성자산 -> 현금및현금성자산`, `손실충당금 -> 대손충당금` positiveHits=4/4 와 badAccepted=0/7,
    searchTop1=5/5, reliableSearch=5/5 를 유지해야 한다. 동시에 4,000 표본에서 V70 의 raw 11.4s,
    buildSignatures 41.2s, totalSeconds 104.1s 보다 빨라야 본진 후보로 본다.
...
결과:
    1,200 units + direct side payload 600개:
        ...
        lanes={'sentence': 65341, 'artifact': 18653, 'owner': 12265}, buildSignatures=21.1s.
        lineMiss=60,445, lineHit=35,814, lineCache=60,445.
        rawPrune atoms=2,933,152 -> 1,812,361, el=58,222.
        signatureStage view=0.2s, sample=2.6s, raw=4.9s, prune=2.6s, weight=2.5s,
        relaySource=1.5s, relay=6.8s.
        relay views=9,967, updates=806,546, view=0.3s, build=6.6s.
    ...
    4,000 units + direct side payload 600개:
        tokenizeBase=5.3s, buildCorpusBridgeSeedIndex=4.2s, augmentBridgeSurfaces=1.9s.
        ...
        lanes={'sentence': 136450, 'artifact': 36514, 'owner': 35209}, buildSignatures=43.6s.
        lineMiss=130,486, lineHit=77,687, lineCache=130,486.
        rawPrune atoms=4,607,269 -> 2,757,382, el=94,220.
        signatureStage view=0.3s, sample=7.4s, raw=10.5s, prune=4.0s, weight=4.0s,
        relaySource=1.5s, relay=15.6s.
        relay views=15,233, updates=1,245,942, view=0.4s, build=15.2s.
...
판정:
    실패/진단 성공. 품질은 positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5 로
    V70 수준을 유지했고, line cache 도 4,000 표본에서 lineHit=77,687 을 만들었다. raw stage 는 V70 11.4s 에서
    10.5s 로 줄어 "bridge pseudo surface 가 같은 위치 경험 atom 을 중복 계산한다"는 가설은 맞았다.

    그러나 buildSignatures 전체는 41.2s -> 43.6s, totalSeconds 는 104.1 -> 110.2 로 악화됐다. raw 에서 절약한
    0.9s 를 prune/weight/relay 후반 비용 증가가 상쇄했다. tuple cache 진단은 1,200 totalSeconds=53.0 으로 더
    느렸고, raw 직후 cache clear 진단도 4,000 totalSeconds=113.8 로 악화되어 cache materialization 자체가
    본진 투입 가치가 없음을 확인했다. 다음은 line atom 캐시가 아니라 relay 후보 생성을 `coordPostings` 전체
    surface fanout 에서 bounded sparse row 로 바꾸거나, V66 의 빠른 raw 구조에 V70 lane penalty 만 최소 이식하는
    방향이 맞다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V75_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V75_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V75_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V75_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V75_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V75_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V75_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V75_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V75_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V75_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V75_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V75_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V75_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V75_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V75_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V75_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V75_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V75_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V75_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V75_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V75_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V75_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
LANE_MISMATCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V75_LANE_MISMATCH_PENALTY", "0.18"))
LANE_ARTIFACT_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V75_LANE_ARTIFACT_PENALTY", "0.10"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V75_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V75_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V75_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V75_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V75_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V75_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V75_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V75_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V75_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V75_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V75_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V75_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V75_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V75_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V75_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V75_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V75_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V75_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V75_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V75_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V75_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V75_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V75_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V75_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V75_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V75_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V75_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V75_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V75_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V75_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V75_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V75_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V75_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V75_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V75_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V75_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V75_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V75_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V75_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V75_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V75_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V75_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V75_SKETCH_OCC_BUCKETS", "16"))
...
def cachedLineAtoms(
    cache: Cache,
    pos: int,
    lineAtomCache: dict[tuple[int, int], set[str]],
    sketchView: SketchAtomView,
    stats: Counter[str],
) -> set[str]:
    key = (cache.unit.unitId, pos)
    cached = lineAtomCache.get(key)
    if cached is not None:
        stats["lineHit"] += 1
        return cached
    atoms = lineAtoms(pos, cache.stems, cache.markers, cache.lanes, sketchView)
    lineAtomCache[key] = atoms
    stats["lineMiss"] += 1
    return atoms
...
def buildSignatures(
    ...
    for cache, occ in sampledOccs:
        raw[occ.surface].update(cachedHorizonAtoms(cache, occ.position, horizonAtomCache, horizonTokenViews, stats))
        raw[occ.surface].update(cachedLineAtoms(cache, occ.position, lineAtomCache, sketchView, stats))
    for surface, counter in raw.items():
        for atom in coordAtoms(surface):
            counter[atom] += 1
    rawBuilt = time.perf_counter()
    ...
    print(
        ...
        f"horizonMiss={stats['miss']} horizonHit={stats['hit']} "
        f"lineMiss={stats['lineMiss']} lineHit={stats['lineHit']} lineCache={len(lineAtomCache)}"
    )
    print(
