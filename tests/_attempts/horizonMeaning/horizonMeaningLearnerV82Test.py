"""Horizon Meaning Learner V82 - bounded top-k IDF reuse.
...
아이디어:
    ...
    V81 이후 raw atom 생성 자체는 의미 경험을 만드는 핵심이지만, 그 뒤 prune/weight 단계는 같은 raw counter 를
    여러 번 훑고 lane별 전체 sort 와 IDF log 계산을 반복한다. V82 는 의미 atom 정의와 limit 값은 그대로 두고,
    sketch/signature top-k 선택을 full sort 에서 bounded `nlargest()` 로 바꾸며, prune 단계에서 만든 df 를
    weight 단계가 재사용하게 한다. 즉 경험 그래프의 내용은 보존하고 후처리 materialization 비용만 줄인다.
...
시도 방법:
    ...
    69. sketch top atom 선택은 전체 sort 대신 bounded `nlargest(SKETCH_LIMIT)` 로 수행한다.
    70. signature raw prune 은 lane별 전체 sort 대신 bounded `nlargest(limit)` 를 쓰고, 선택된 atom df 를 함께 만든다.
    71. weight 단계는 prune df 를 재사용하고 atom별 IDF 를 한 번만 materialize 한 뒤 bounded top-k 로 signature 를 고른다.

실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV82Test.py

    $env:DARTLAB_HORIZON_V82_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V82_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V82_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V82_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV82Test.py

    $env:DARTLAB_HORIZON_V82_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V82_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V82_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V82_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV82Test.py

판정 기준:
    V81 과 같은 품질 기준을 유지해야 한다. 즉 `외상매출금 -> 매출채권`, `영업손익 -> 영업이익`,
    `현금성자산 -> 현금및현금성자산`, `손실충당금 -> 대손충당금` positiveHits=4/4 와 badAccepted=0/7,
    searchTop1=5/5, reliableSearch=5/5 를 유지해야 한다. 동시에 4,000 표본에서 V81 의 buildSketches 16.9s,
    rawPrune/weight 각 4초대, totalSeconds 88.5s 중 후처리 반복 계산이 실제로 줄어야 본진 후보로 본다.

결과:
    1,200 units + direct side payload 600개:
        tokenizeBase=2.0s, buildCorpusBridgeSeedIndex=1.2s, augmentBridgeSurfaces=0.8s.
        ...
        buildSketches=7.3s, sketch raw=4.0s, select=2.1s.
        lineView caches=1,200, tokens=175,983, content=96,864, learnedCells=96,864,
        selfAtoms=579,264, neighborAtoms=386,304.
        signature occSample=150,989 -> 96,259,
        lanes={'sentence': 65341, 'artifact': 18653, 'owner': 12265}, buildSignatures=17.0s.
        rawPrune atoms=2,933,152 -> 1,812,361, el=58,222, dfAtoms=357,415.
        relayPostings keys=54,339, rawLinks=220,061, keptLinks=139,623,
        skippedBroad=80,277, prunedRows=7, maxRow=160.
        signatureStage view=0.4s, sample=1.4s, raw=4.2s, prune=4.0s, weight=3.3s,
        relaySource=2.3s, relay=1.4s.
        relay views=9,967, updates=717,042, view=0.3s, build=1.1s.
        laneProfile surfaces=10,902, artifactHeavy=2,294, ownerHeavy=1,522.
        focusedRelation relationOcc=1,312, spanPairs=81,891, leakPairs=150,820,
        framePairs=90,869, ownerFrames=1,312, buildFocusedRelationIndexes=2.4s.
        sideFocusedBound payloads=600, relationOcc=1,575, localPairs=102,451,
        ownerFrames=1,575, buildSideBoundPayloadIndex=1.9s.
        buildUnitIndex=1.2s, modelSeconds=39.7, totalSeconds=42.2.
    ...
    4,000 units + direct side payload 600개:
        tokenizeBase=6.8s, buildCorpusBridgeSeedIndex=4.2s, augmentBridgeSurfaces=1.9s.
        ...
        buildSketches=16.5s, sketch raw=10.3s, select=2.9s.
        lineView caches=4,000, tokens=607,533, content=319,845, learnedCells=319,845,
        selfAtoms=1,915,740, neighborAtoms=1,277,382.
        signature occSample=505,826 -> 208,173,
        lanes={'sentence': 136450, 'artifact': 36514, 'owner': 35209}, buildSignatures=30.0s.
        rawPrune atoms=4,607,269 -> 2,757,382, el=94,220, dfAtoms=522,923.
        relayPostings keys=81,530, rawLinks=334,322, keptLinks=211,662,
        skippedBroad=121,533, prunedRows=17, maxRow=219.
        signatureStage view=1.0s, sample=5.0s, raw=8.8s, prune=5.7s, weight=4.4s,
        relaySource=2.7s, relay=2.4s.
        relay views=15,233, updates=1,112,132, view=0.4s, build=2.0s.
        laneProfile surfaces=16,386, artifactHeavy=3,559, ownerHeavy=2,253.
        focusedRelation relationOcc=4,825, spanPairs=290,683, leakPairs=539,818,
        framePairs=324,757, ownerFrames=4,825, buildFocusedRelationIndexes=8.8s.
        sideFocusedBound payloads=600, relationOcc=1,870, localPairs=123,609,
        ownerFrames=1,870, buildSideBoundPayloadIndex=2.0s.
        buildUnitIndex=3.4s, modelSeconds=82.6, totalSeconds=89.7.
...
판정:
    실패/진단 성공. 품질은 positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5 를 유지했지만
    bounded `nlargest()` 와 IDF dict materialization 은 후처리 비용을 줄이지 못했다. 4,000 기준 buildSketches 는
    16.9s -> 16.5s 로 소폭 줄었지만 buildSignatures 는 28.1s -> 30.0s 로 나빠졌고, 특히 prune 이 4.1s -> 5.7s,
    weight 가 4.2s -> 4.4s 로 늘었다. totalSeconds 도 88.5s -> 89.7s 로 후퇴했다.

    원인은 Python `heapq.nlargest()` 가 surface별 수백 atom 규모에서는 전체 sort 보다 이득이 작고, IDF dict 를
    별도로 물리화하는 비용이 df 재사용 이득을 상쇄하기 때문이다. 본진 투입 가치 없음. 다음은 postprocess top-k 가
    아니라 signature sampling 후보 자체를 sketch 의 넓은 evidence funnel 에서 재선택해 raw/prune/weight 입력량을
    줄이는 방향을 검증한다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V82_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V82_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V82_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V82_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V82_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V82_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V82_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V82_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V82_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V82_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V82_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V82_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V82_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V82_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V82_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V82_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V82_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V82_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V82_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V82_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V82_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V82_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
LANE_MISMATCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V82_LANE_MISMATCH_PENALTY", "0.18"))
LANE_ARTIFACT_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V82_LANE_ARTIFACT_PENALTY", "0.10"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V82_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V82_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V82_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V82_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V82_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V82_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V82_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V82_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V82_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V82_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V82_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V82_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V82_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V82_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V82_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V82_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V82_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V82_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V82_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V82_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V82_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V82_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V82_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V82_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V82_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V82_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V82_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V82_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V82_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V82_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V82_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V82_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V82_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V82_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V82_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
RELAY_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V82_RELAY_ROW_LIMIT", "160"))
RELAY_SPECIFIC_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V82_RELAY_SPECIFIC_ROW_LIMIT", "320"))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V82_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V82_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V82_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V82_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V82_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V82_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V82_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V82_SKETCH_OCC_BUCKETS", "16"))
...
def buildSketches(
    ...
    total = max(1, len(raw))
    idf = {atom: math.log(1.0 + total / (1.0 + count)) for atom, count in df.items()}
    sketches: dict[str, Counter[str]] = {}
    for stem, counter in raw.items():
        selected = Counter(
            {
                atom: score
                for score, atom in nlargest(
                    SKETCH_LIMIT,
                    ((math.sqrt(count) * idf[atom], atom) for atom, count in counter.items()),
                )
            }
        )
        if selected:
    ...
    print(
        ...
        f"sample={sampled - started:.1f}s raw={rawBuilt - sampled:.1f}s select={finished - rawBuilt:.1f}s"
    )
    return sketches
...
def pruneRawSignatureCounters(raw: dict[str, Counter[str]]) -> tuple[dict[str, Counter[str]], Counter[str]]:
    limits = {
    ...
    for surface, counter in raw.items():
        ...
        for lane, rows in lanes.items():
            ...
            if limit <= 0:
                continue
            for _, atom in nlargest(limit, rows):
                selected[atom] = counter[atom]
                laneKept[lane] += 1
        afterAtoms += len(selected)
        pruned[surface] = selected
        df.update(selected.keys())
    print(
        f"[rawPrune] surfaces={len(raw)} atoms={beforeAtoms}->{afterAtoms} "
        f"xp={laneKept['xp']} hx={laneKept['hx']} el={laneKept['el']} "
        f"cx={laneKept['cx']} other={laneKept['other']} dfAtoms={len(df)}"
    )
    return pruned, df


def weightCounters(raw: dict[str, Counter[str]], df: Counter[str]) -> dict[str, Counter[str]]:
    total = max(1, len(raw))
    idf = {atom: math.log(1.0 + total / (1.0 + count)) for atom, count in df.items()}
...
def buildSignatures(
    ...
    for surface, counter in raw.items():
        for atom in coordAtoms(surface):
            counter[atom] += 1
    rawBuilt = time.perf_counter()
    raw, prunedDf = pruneRawSignatureCounters(raw)
    pruned = time.perf_counter()
    signatures = weightCounters(raw, prunedDf)
