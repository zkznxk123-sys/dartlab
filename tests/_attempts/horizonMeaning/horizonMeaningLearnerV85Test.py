"""Horizon Meaning Learner V85 - canonical line atom strings.
...
실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV85Test.py

    $env:DARTLAB_HORIZON_V85_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V85_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V85_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V85_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV85Test.py

    $env:DARTLAB_HORIZON_V85_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V85_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V85_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V85_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV85Test.py

판정 기준:
    V84 와 같은 품질 기준을 유지해야 한다. 즉 `외상매출금 -> 매출채권`, `영업손익 -> 영업이익`,
    `현금성자산 -> 현금및현금성자산`, `손실충당금 -> 대손충당금` positiveHits=4/4 와 badAccepted=0/7,
    searchTop1=5/5, reliableSearch=5/5 를 유지해야 한다. 동시에 4,000 표본에서 V84 의 buildSignatures 24.4s,
    signature raw 7.7s, totalSeconds 83.1s 중 line atom 문자열 생성 비용이 실제로 줄어야 한다.
...
결과:
    1,200 units + direct side payload 600개:
        ...
        buildSketches=6.5s, sketch raw=3.8s, atomCache cell=4,873,908/126,545,
        marker=624,998/244, lr=50,608/16,440.
        lineView caches=1,200, tokens=175,983, content=96,864, learnedCells=96,864,
        selfAtoms=579,264, neighborAtoms=386,304.
        signature occSample source=sketchFunnel, 116,480 -> 96,259,
        lanes={'sentence': 65438, 'artifact': 18607, 'owner': 12214}, buildSignatures=13.0s.
        rawPrune atoms=2,941,058 -> 1,812,531, el=58,460.
        relayPostings keys=54,339, rawLinks=220,061, keptLinks=139,623,
        skippedBroad=80,277, prunedRows=7, maxRow=160.
        signatureStage view=0.4s, sample=1.1s, raw=3.0s, prune=2.6s, weight=2.4s,
        relaySource=2.2s, relay=1.3s, lineAtomCache xp=2,968,654/164,950,
        tri=133,724/30,013, lr=44,036/10,007.
        relay views=9,965, updates=718,049, view=0.3s, build=1.0s.
        ...
        framePairs=90,869, ownerFrames=1,312, buildFocusedRelationIndexes=2.3s.
        sideFocusedBound payloads=600, relationOcc=1,575, localPairs=102,451,
        ownerFrames=1,575, buildSideBoundPayloadIndex=2.0s.
        buildUnitIndex=1.2s, modelSeconds=35.4, totalSeconds=37.7.
        ...
        accepted=False. `현금성자산 -> 현금및현금성자산` accepted=True(score 1.459).
        `영업손익 -> 영업이익` accepted=True(score 0.182).
        `손실충당금 -> 대손충당금` accepted=True(score 0.113).
        `복구충당금 -> 대손충당금` 은 score 1.069 로 accepted=False.
    ...
    4,000 units + direct side payload 600개:
        tokenizeBase=6.7s, buildCorpusBridgeSeedIndex=4.0s, augmentBridgeSurfaces=2.1s.
        ...
        buildSketches=15.1s, sketch raw=9.5s, atomCache cell=11,624,571/177,558,
        marker=1,475,395/260, lr=130,202/27,556.
        lineView caches=4,000, tokens=607,533, content=319,845, learnedCells=319,845,
        selfAtoms=1,915,740, neighborAtoms=1,277,382.
        signature occSample source=sketchFunnel, 275,808 -> 208,173,
        lanes={'sentence': 137003, 'artifact': 36491, 'owner': 34679}, buildSignatures=23.1s.
        rawPrune atoms=4,634,185 -> 2,758,395, el=94,847.
        relayPostings keys=81,530, rawLinks=334,322, keptLinks=211,662,
        skippedBroad=121,533, prunedRows=17, maxRow=219.
        signatureStage view=1.0s, sample=2.8s, raw=6.5s, prune=4.2s, weight=3.8s,
        relaySource=2.5s, relay=2.3s, lineAtomCache xp=6,584,855/238,309,
        tri=306,335/48,839, lr=101,583/16,231.
        relay views=15,233, updates=1,113,151, view=0.4s, build=1.9s.
        ...
        framePairs=324,757, ownerFrames=4,825, buildFocusedRelationIndexes=7.7s.
        sideFocusedBound payloads=600, relationOcc=1,870, localPairs=123,609,
        ownerFrames=1,870, buildSideBoundPayloadIndex=2.0s.
        buildUnitIndex=3.3s, modelSeconds=74.2, totalSeconds=81.3.
        laneProbe: 외상매출금 S1.00/A0.00/O0.00, 매출채권 S0.63/A0.35/O0.01,
        대손충당금 S0.71/A0.00/O0.29, 복구충당금 S1.00/A0.00/O0.00.
        `외상매출금 -> 매출채권` accepted=True(score 1.119, cp 0.624), `매출액` 2순위는
        accepted=False. `현금성자산 -> 현금및현금성자산` accepted=True(score 1.920).
        `영업손익 -> 영업이익` accepted=True(score 0.198).
        `손실충당금 -> 대손충당금` accepted=True(score 0.115).
        `복구충당금 -> 대손충당금` 은 score 0.177 으로 accepted=False.
        `대출채권 -> 대손충당금` 은 score -0.134 로 accepted=False.
        `대출채권 -> 매출채권` 은 forbidden=-0.124 로 accepted=False.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5.

판정:
    성공/작은 구조 개선. V85 는 V84 의 horizon atom canonical cache 에 이어 `lineAtomsFromView()` 의
    `xp:n`, `el:tri`, `el:lr` 문자열도 canonical cache 로 재사용했다. atom text, sampling, scoring,
    acceptance/search gate 는 그대로라 품질 변화 원인은 없다. 1,200 기준 buildSignatures 는 13.6s -> 13.0s,
    signature raw 는 3.6s -> 3.0s, totalSeconds 는 38.1s -> 37.7s 로 줄었다. 4,000 기준 buildSignatures 는
    24.4s -> 23.1s, signature raw 는 7.7s -> 6.5s, modelSeconds 는 75.9s -> 74.2s,
    totalSeconds 는 83.1s -> 81.3s 로 낮아졌다.

    line atom cache 는 4,000 기준 xp 6,584,855 hit / 238,309 miss, tri 306,335 / 48,839,
    lr 101,583 / 16,231 로 반복성이 충분했다. buildSketches 는 line atom 대상이 아니므로 거의 유지됐고,
    개선은 의도대로 signature raw 생성 비용에 집중됐다.

    품질은 positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5 를 유지했다.
    `복구충당금 -> 대손충당금` 0.177, `대출채권 -> 대손충당금` -0.134, `대출채권 -> 매출채권` -0.124 도 유지됐다.
    결론적으로 경험 atom 문자열뿐 아니라 line atom 문자열도 canonical object 로 재사용하는 것이 맞다. 다음은 여전히
    큰 focusedRelation 7.7s 또는 buildSketches raw 9.5s 를 줄이는 방향이다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V85_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V85_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V85_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V85_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V85_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V85_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V85_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V85_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V85_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V85_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V85_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V85_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V85_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V85_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V85_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V85_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V85_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V85_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V85_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V85_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V85_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V85_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
LANE_MISMATCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V85_LANE_MISMATCH_PENALTY", "0.18"))
LANE_ARTIFACT_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V85_LANE_ARTIFACT_PENALTY", "0.10"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V85_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V85_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V85_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V85_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V85_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V85_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V85_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V85_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V85_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V85_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V85_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V85_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V85_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V85_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V85_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V85_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V85_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V85_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V85_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V85_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V85_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V85_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V85_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V85_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V85_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V85_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V85_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V85_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V85_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V85_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V85_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V85_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V85_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V85_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V85_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
RELAY_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V85_RELAY_ROW_LIMIT", "160"))
RELAY_SPECIFIC_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V85_RELAY_SPECIFIC_ROW_LIMIT", "320"))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V85_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V85_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V85_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V85_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V85_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V85_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V85_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V85_SKETCH_OCC_BUCKETS", "16"))
