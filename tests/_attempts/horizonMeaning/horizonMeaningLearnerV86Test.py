"""Horizon Meaning Learner V86 - fused focused-relation row windows.
...
아이디어:
    ...
    나열하는 핵심 표현이라 내용은 유지해야 한다. V86 는 line atom 문자열도 canonical cache 로 재사용해,
...
시도 방법:
    ...
    76. focusedRelation 은 relation occurrence 마다 span/leak/frame/owner row slice 를 여러 개 만들고,
        frame row 마다 intervening surface 를 다시 선형 검사한다.
    77. V86 은 가장 큰 leak window 하나를 relation-local row stream 으로 삼아 span/leak/frame 계산을 한 pass 로
        합치고, owner/intervening 판정은 list slice 대신 index range 와 bisect 로 수행한다.
    78. relation score 함수, surface position source, acceptance/search gate 는 그대로 두어 의미 품질 변수를
        window materialization 제거로 분리한다.

실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV86Test.py

    $env:DARTLAB_HORIZON_V86_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V86_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V86_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V86_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV86Test.py

    $env:DARTLAB_HORIZON_V86_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V86_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V86_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V86_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV86Test.py

판정 기준:
    V85 와 같은 품질 기준을 유지해야 한다. 즉 `외상매출금 -> 매출채권`, `영업손익 -> 영업이익`,
    `현금성자산 -> 현금및현금성자산`, `손실충당금 -> 대손충당금` positiveHits=4/4 와 badAccepted=0/7,
    searchTop1=5/5, reliableSearch=5/5 를 유지해야 한다. 동시에 4,000 표본에서 V85 의
    focusedRelation 7.7s, modelSeconds 74.2s, totalSeconds 81.3s 중 focusedRelation window 비용이
    실제로 줄어야 한다.

결과:
    1,200 units + direct side payload 600개:
        buildSketches=7.2s, buildSignatures=12.8s, buildFocusedRelationIndexes=2.1s,
        buildSideBoundPayloadIndex=1.9s, buildUnitIndex=1.2s, modelSeconds=35.5, totalSeconds=37.9.
        focusedRelation relationOcc=1,312, spanPairs=81,891, leakPairs=150,820,
        framePairs=90,869, ownerFrames=1,312, windowRows=150,820.
        ...
        `복구충당금 -> 대손충당금` 은 score 1.065 로 accepted=False.
    ...
    4,000 units + direct side payload 600개:
        buildSketches=15.3s, buildSignatures=24.6s, buildFocusedRelationIndexes=7.9s,
        buildSideBoundPayloadIndex=2.2s, buildUnitIndex=3.9s, modelSeconds=76.8, totalSeconds=83.8.
        focusedRelation relationOcc=4,825, spanPairs=290,683, leakPairs=539,818,
        framePairs=324,757, ownerFrames=4,825, windowRows=539,818.
        `외상매출금 -> 매출채권` accepted=True(score 1.119, cp 0.624), `매출액` 2순위는
        accepted=False. `영업손익 -> 영업이익` accepted=True(score 0.198).
        `현금성자산 -> 현금및현금성자산` accepted=True(score 1.920).
        `손실충당금 -> 대손충당금` accepted=True(score 0.115).
        `복구충당금 -> 대손충당금` 은 score 0.177 로 accepted=False.
        `대출채권 -> 대손충당금` 은 score -0.134 로 accepted=False.
        `대출채권 -> 매출채권` 은 forbidden=-0.124 로 accepted=False.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5.

판정:
    실패/진단 성공. 품질은 positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5 를
    유지했지만, 목표였던 focusedRelation 비용 절감은 4,000 기준 V85 7.7s -> V86 7.9s 로 후퇴했다.
    totalSeconds 도 81.3s -> 83.8s 로 나빠졌다. list slice/window materialization 을 줄여도 pair count 는
    spanPairs=290,683, leakPairs=539,818, framePairs=324,757 로 그대로이고, `gapDistance`, substring fence/value/clause
    regex 판정은 여전히 반복된다. 따라서 병목의 본체는 row slice 가 아니라 pair 별 text gate 판정 비용이다.
    다음 실험은 V85 기준으로 돌아가 relation text 의 fence/value/clause match 를 unit 단위 sparse index 로
    미리 만들고 pair scoring 에서는 bisect 로 조회하는 방향이다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V86_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V86_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V86_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V86_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V86_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V86_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V86_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V86_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V86_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V86_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V86_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V86_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V86_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V86_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V86_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V86_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V86_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V86_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V86_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V86_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V86_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V86_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
LANE_MISMATCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V86_LANE_MISMATCH_PENALTY", "0.18"))
LANE_ARTIFACT_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V86_LANE_ARTIFACT_PENALTY", "0.10"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V86_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V86_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V86_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V86_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V86_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V86_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V86_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V86_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V86_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V86_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V86_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V86_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V86_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V86_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V86_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V86_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V86_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V86_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V86_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V86_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V86_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V86_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V86_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V86_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V86_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V86_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V86_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V86_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V86_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V86_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V86_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V86_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V86_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V86_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V86_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
RELAY_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V86_RELAY_ROW_LIMIT", "160"))
RELAY_SPECIFIC_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V86_RELAY_SPECIFIC_ROW_LIMIT", "320"))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V86_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V86_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V86_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V86_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V86_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V86_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V86_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V86_SKETCH_OCC_BUCKETS", "16"))
...
def positionWindowRange(
    ...
    if high is None:
        high = len(starts)
    start = bisect_left(starts, left, low, high)
    end = bisect_right(starts, right, start, high)
    return start, end


def relationOwnerFrameInRange(
    text: str,
    relationPos: int,
    relationSize: int,
    ownerRows: list[tuple[int, int, str]],
    startIndex: int,
    endIndex: int,
) -> tuple[str, ...]:
    localOwners: list[tuple[int, str]] = []
    for index in range(startIndex, endIndex):
        otherPos, otherSize, otherSurface = ownerRows[index]
        otherDistance = gapDistance(otherPos, otherSize, relationPos, relationSize)
        if otherDistance > FRAME_MAX_DISTANCE:
            continue
        if not sameClause(text, otherPos, otherSize, relationPos, relationSize):
...
def hasInterveningSurfaceInRange(
    rows: list[tuple[int, int, str]],
    starts: list[int],
    startIndex: int,
    endIndex: int,
    ...
    if relationPos <= surfaceEnd:
        return False
    left = bisect_left(starts, surfaceEnd, startIndex, endIndex)
    right = bisect_left(starts, relationPos, left, endIndex)
    for index in range(left, right):
        if rows[index][2] != surface:
            return True
    return False
...
def buildFocusedRelationIndexes(
    ...
    for cache in caches:
        ...
        for relation, relPositions in relationPositions.items():
            for relationPos, relationSize in relPositions:
                relationOccurrences += 1
                spanStart, spanEnd = positionWindowRange(
                    allSurfaceStarts,
                    relationPos - SPAN_MAX_DISTANCE,
                    relationPos + SPAN_MAX_DISTANCE,
                )
                leakStart, leakEnd = positionWindowRange(
                    allSurfaceStarts,
                    relationPos - FOCUSED_FRAME_DISTANCE,
                    relationPos + relationSize + FOCUSED_FRAME_DISTANCE,
                )
                frameStart, frameEnd = positionWindowRange(
                    allSurfaceStarts,
                    relationPos - FRAME_MAX_DISTANCE - frameStartSlack,
                    relationPos + relationSize + FRAME_MAX_DISTANCE,
                )
                if spanStart == spanEnd and leakStart == leakEnd and frameStart == frameEnd:
                    continue
                ownerStart, ownerEnd = positionWindowRange(
                    ownerStarts,
                    relationPos - FRAME_MAX_DISTANCE,
                    relationPos + relationSize + FRAME_MAX_DISTANCE,
                )
                bestOwnerSurfaces = relationOwnerFrameInRange(
                    text,
                    relationPos,
                    relationSize,
                    ownerRows,
                ...
                ownerFrames += 1
                for rowIndex in range(leakStart, leakEnd):
                    windowRowVisits += 1
                    surfacePos, surfaceSize, surface = allSurfaceRows[rowIndex]
                    startDistance = abs(surfacePos - relationPos)
                    distance = gapDistance(surfacePos, surfaceSize, relationPos, relationSize)
                    key = (surface, relation)
                    if rowIndex >= spanStart and rowIndex < spanEnd:
                    ...
                    if distance <= FOCUSED_FRAME_DISTANCE:
                        leakPairChecks += 1
                        leakBest[key] = max(
                            leakBest.get(key, 0.0),
                            relationTableLeakStrength(text, surfacePos, surfaceSize, relationPos, relationSize),
                        )
                    if rowIndex >= frameStart and rowIndex < frameEnd and distance <= FRAME_MAX_DISTANCE:
                        framePairChecks += 1
                        intervening = hasInterveningSurfaceInRange(
                            allSurfaceRows,
                            allSurfaceStarts,
                            frameStart,
                            frameEnd,
                            surface,
                            surfacePos + surfaceSize,
                            relationPos,
                        )
                        frameBest[key] = max(
                            frameBest.get(key, 0.0),
