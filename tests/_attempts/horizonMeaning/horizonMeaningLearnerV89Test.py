"""Horizon Meaning Learner V89 - cache surface aggregate views.
...
아이디어:
    ...
    나열하는 핵심 표현이라 내용은 유지해야 한다. V89 는 line atom 문자열도 canonical cache 로 재사용해,
...
시도 방법:
    ...
    80. V89 은 V87 의 sparse relation text gate 위에서 horizon atom cache 를 `(unitId, position)` tuple-key dict 에서
        unitId 별 dense list 로 바꾼다.
    81. cached horizon atom 은 set 대신 tuple 로 저장해 sketch/signature update 에 재사용하고, 매 occurrence 의 tuple key
        생성과 dict hash lookup 을 줄인다.
    82. atom text, selected occurrence, scoring, acceptance/search gate 는 그대로 두어 의미 품질 변수를 cache layout 으로 분리한다.
    83. V89 는 V88 이후 남은 buildUnitIndex 4.1s 와 surface/lane index 반복 scan 을 겨냥한다.
    84. augment 된 Cache 에 `surfaceCounts`, `contentSurfaces`, `surfaceLaneCounts` 를 붙여
        `buildSurfacePairIndex`, `buildSurfaceLaneProfiles`, `buildUnitIndex` 가 같은 `cache.occs` 를 다시 훑지 않게 한다.
    85. occurrence 자체, surface count 값, route/search gate 는 그대로 두고 aggregation layout 만 공유한다.

실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV89Test.py

    $env:DARTLAB_HORIZON_V89_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V89_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V89_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V89_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV89Test.py

    $env:DARTLAB_HORIZON_V89_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V89_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V89_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V89_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV89Test.py
...
판정 기준:
    ...
    buildUnitIndex 4.1s, buildSurfacePairIndex 0.3s, buildSurfaceLaneProfiles 0.2s,
    totalSeconds 76.4s 중 repeated occurrence aggregation 비용이 실제로 줄어야 한다.

결과:
    1,200 units + direct side payload 600개:
        augmentBridgeSurfaces=1.2s, buildSketches=5.9s, buildSignatures=12.1s,
        buildSurfacePairIndex=0.1s, buildSurfaceLaneProfiles=0.1s, buildUnitIndex=1.2s,
        modelSeconds=33.3, totalSeconds=35.6.
        `외상매출금 -> 매출채권` accepted=True(score 0.080, cp 0.055), `매출액` 2순위는
        accepted=False. `손실충당금 -> 대손충당금` accepted=True(score 0.113).
        `복구충당금 -> 대손충당금` score 1.069 accepted=False,
        `대출채권 -> 대손충당금` score 0.049 accepted=False,
        `대출채권 -> 매출채권` forbidden=-0.134.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5.

    4,000 units + direct side payload 600개:
        augmentBridgeSurfaces=3.7s, buildSketches=13.3s, buildSignatures=22.2s,
        buildSurfacePairIndex=0.2s, buildSurfaceLaneProfiles=0.4s, buildUnitIndex=3.3s,
        modelSeconds=71.1, totalSeconds=77.6.
        `외상매출금 -> 매출채권` accepted=True(score 1.119, cp 0.624), `매출액` 2순위는
        accepted=False. `손실충당금 -> 대손충당금` accepted=True(score 0.115).
        `복구충당금 -> 대손충당금` score 0.177 accepted=False,
        `대출채권 -> 대손충당금` score -0.134 accepted=False,
        `대출채권 -> 매출채권` forbidden=-0.124.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5.

판정:
    실패/진단 성공. 품질은 유지했지만 4,000 기준 V88 totalSeconds 76.4s -> V89 77.6s 로 후퇴했다.
    buildUnitIndex 는 4.1s -> 3.3s 로 줄었지만, surfaceCounts/contentSurfaces/surfaceLaneCounts 를 모두
    augment 단계에서 만들면서 augmentBridgeSurfaces 가 1.9s -> 3.7s 로 늘었다. buildSurfaceLaneProfiles 도
    0.2s -> 0.4s 로 나빠졌다. 즉 aggregate precompute 의 범위가 너무 넓으면 downstream 이득보다 upstream
    비용이 크다. 다음은 lane count 를 제외하고 surface count 만 token/augment 흐름에 좁게 붙이는 방향이다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V89_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V89_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V89_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V89_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V89_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V89_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V89_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V89_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V89_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V89_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V89_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V89_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V89_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V89_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V89_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V89_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V89_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V89_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V89_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V89_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V89_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V89_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
LANE_MISMATCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V89_LANE_MISMATCH_PENALTY", "0.18"))
LANE_ARTIFACT_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V89_LANE_ARTIFACT_PENALTY", "0.10"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V89_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V89_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V89_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V89_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V89_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V89_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V89_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V89_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V89_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V89_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V89_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V89_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V89_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V89_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V89_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V89_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V89_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V89_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V89_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V89_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V89_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V89_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V89_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V89_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V89_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V89_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V89_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V89_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V89_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V89_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V89_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V89_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V89_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V89_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V89_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
RELAY_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V89_RELAY_ROW_LIMIT", "160"))
RELAY_SPECIFIC_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V89_RELAY_SPECIFIC_ROW_LIMIT", "320"))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V89_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V89_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V89_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V89_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V89_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V89_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V89_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V89_SKETCH_OCC_BUCKETS", "16"))
...
class Cache:
    ...
    surfaceCounts: Counter[str] | None = None
    contentSurfaces: tuple[str, ...] | None = None
    surfaceLaneCounts: dict[str, Counter[str]] | None = None
...
def occurrenceSurfaceViews(occs: list[Occ]) -> tuple[Counter[str], tuple[str, ...], dict[str, Counter[str]]]:
    surfaceCounts: Counter[str] = Counter()
    laneCounts: dict[str, Counter[str]] = defaultdict(Counter)
    for occ in occs:
        surfaceCounts[occ.surface] += 1
        laneCounts[occ.surface][occ.lane] += 1
    contentSurfaces = tuple(sorted(surface for surface in surfaceCounts if isContentStem(surface)))
    return surfaceCounts, contentSurfaces, dict(laneCounts)
...
def augmentCacheWithBridgeSurfaces(cache: Cache, bridgeSeedIndex: BridgeSeedIndex) -> Cache:
    ...
    surfaceCounts, contentSurfaces, surfaceLaneCounts = occurrenceSurfaceViews(occs)
    return Cache(
        cache.unit,
        cache.stems,
        cache.markers,
...
def buildSurfacePairIndex(caches: list[Cache]) -> tuple[Counter[str], Counter[tuple[str, str]]]:
    ...
    for cache in caches:
        surfaces = (
            list(cache.contentSurfaces)
            if cache.contentSurfaces is not None
            else sorted({occ.surface for occ in cache.occs})
        )
...
def buildSurfaceLaneProfiles(caches: list[Cache]) -> dict[str, tuple[float, float, float]]:
    laneCounts: dict[str, Counter[str]] = defaultdict(Counter)
    for cache in caches:
        if cache.surfaceLaneCounts is not None:
            for surface, counts in cache.surfaceLaneCounts.items():
                laneCounts[surface].update(counts)
        else:
            for occ in cache.occs:
                laneCounts[occ.surface][occ.lane] += 1
...
def buildUnitIndex(model: Model) -> None:
    ...
    for cache in model.caches:
        ...
        surfaceCounts = (
            cache.surfaceCounts if cache.surfaceCounts is not None else Counter(occ.surface for occ in cache.occs)
        )
