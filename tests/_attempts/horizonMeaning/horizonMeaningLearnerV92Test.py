"""Horizon Meaning Learner V92 - nearest content order line atom.
...
아이디어:
    ...
    V88 은 V87 의 sparse relation text gate 위에서 horizon atom cache 를 `(unitId, position)` tuple-key dict 에서
    unitId 별 dense list 로 바꿔 현재 최고 기준선이 됐다. 이후 V89/V90 의 surface aggregate cache 는 비용이
    앞단으로 이동해 실패했고, V91 의 offset list line atom 조립도 작은 dict 보다 느렸다. 따라서 V92 는 속도
    미세조정보다 사용자가 말한 "자기 stem 의 위치와 앞뒤 stem 순서 경험" 을 더 직접 표현하는 의미 구조를 검증한다.
    기존 `el:lr` 는 `pos-1,pos,pos+1` 즉시 인접 토큰이 모두 content 일 때만 생긴다. 표/문장에는 조사, 숫자,
    HTML 조각, 짧은 기능 token 이 끼므로 의미상 가장 가까운 content 좌/우 stem 의 순서 경험이 빠질 수 있다.
    V92 는 `LineTokenView` 에서 반경 내 nearest content left/right sketch cell 과 자기 sketch cell 을
    `el:nlr:{left}>{center}>{right}` atom 으로 하나 더 추가한다. alias map, target 예외, 수동 family lock 없이
    stem 수평선 경험 그래프의 "자기순서보존 + 앞뒤순서보존" 표현력을 강화하는 실험이다.
...
시도 방법:
    ...
    81. `lineAtomsFromView()` 에서 반경 내 nearest content left/right sketch cell 과 center sketch cell 을 찾는다.
    82. 즉시 인접 `el:lr` 와 별개로 `el:nlr:{left}>{center}>{right}` order atom 을 추가한다.
    83. atom text 추가 외 selected occurrence, route scoring, acceptance/search gate 는 그대로 두어 의미 품질 변수를
        nearest content order atom 하나로 분리한다.

실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV92Test.py

    $env:DARTLAB_HORIZON_V92_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V92_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V92_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V92_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV92Test.py

    $env:DARTLAB_HORIZON_V92_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V92_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V92_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V92_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV92Test.py

판정 기준:
    V88 과 같은 품질 기준을 유지해야 한다. 즉 `외상매출금 -> 매출채권`, `영업손익 -> 영업이익`,
    `현금성자산 -> 현금및현금성자산`, `손실충당금 -> 대손충당금` positiveHits=4/4 와 badAccepted=0/7,
    searchTop1=5/5, reliableSearch=5/5 를 유지해야 한다. 동시에 4,000 표본에서 V88 의
    buildSignatures 21.9s, signature raw 6.0s, totalSeconds 76.4s 와 비교해 nearest content order atom 이
    품질을 개선하거나 최소한 오염 점수를 낮추는지 본다. 품질 개선 없이 totalSeconds 가 후퇴하면 실패다.

결과:
    1,200 units + direct side payload 600개:
        buildSketches=6.5s, sketch sample=1.2s, sketch raw=3.5s.
        buildSignatures=13.0s, signatureStage view=0.4s, sample=0.9s, raw=3.1s,
        prune=2.7s, weight=2.4s, relaySource=2.0s, relay=1.4s.
        buildFocusedRelationIndexes=2.1s, buildSideBoundPayloadIndex=1.9s,
        buildUnitIndex=1.2s, modelSeconds=34.4, totalSeconds=36.6.
        ...
        nearest order atom cache 는 nlr hit/miss=69,001/15,671 이었다.
        `외상매출금 -> 매출채권` accepted=True(score 0.080, cp 0.055), `매출액` 2순위는
        accepted=False. `영업손익 -> 영업이익` accepted=True(score 0.182).
        `현금성자산 -> 현금및현금성자산` accepted=True(score 1.437).
        `손실충당금 -> 대손충당금` accepted=True(score 0.113).
        `복구충당금 -> 대손충당금` 은 score 1.016 로 accepted=False.
        `대출채권 -> 대손충당금` 은 score 0.046 로 accepted=False.
        `대출채권 -> 매출채권` 은 forbidden=-0.134 로 accepted=False.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5.

    4,000 units + direct side payload 600개:
        buildSketches=14.3s, sketch sample=3.3s, sketch raw=8.0s.
        buildSignatures=22.3s, signatureStage view=0.9s, sample=1.5s, raw=6.4s,
        prune=4.2s, weight=3.8s, relaySource=3.1s, relay=2.4s.
        buildFocusedRelationIndexes=6.4s, buildSideBoundPayloadIndex=2.0s,
        buildUnitIndex=4.0s, modelSeconds=71.0, totalSeconds=78.4.
        ...
        nearest order atom cache 는 nlr hit/miss=158,827/25,351 이었다.
        `외상매출금 -> 매출채권` accepted=True(score 1.119, cp 0.624), `매출액` 2순위는
        accepted=False. `영업손익 -> 영업이익` accepted=True(score 0.197).
        `현금성자산 -> 현금및현금성자산` accepted=True(score 2.007).
        `손실충당금 -> 대손충당금` accepted=True(score 0.115).
        `복구충당금 -> 대손충당금` 은 score 0.136 로 accepted=False.
        `대출채권 -> 대손충당금` 은 score -0.134 로 accepted=False.
        `대출채권 -> 매출채권` 은 forbidden=-0.124 로 accepted=False.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5.

판정:
    실패/진단 성공. nearest content order atom 은 사용자가 말한 "자기순서보존 + 앞뒤순서보존" 경험을 더 직접
    표현한다. 품질 게이트는 positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5 로 유지했고,
    4,000 표본에서 `복구충당금 -> 대손충당금` 오염 점수는 V88 0.177 -> V92 0.136 으로 더 낮아졌다.
    `현금성자산 -> 현금및현금성자산` 은 1.920 -> 2.007 로 강해졌다.

    그러나 atom 하나를 전 surface signature 에 투입하니 raw atom 수가 늘어 buildSketches 14.0s -> 14.3s,
    buildSignatures 21.9s -> 22.3s, signature raw 6.0s -> 6.4s, totalSeconds 76.4s -> 78.4s 로 후퇴했다.
    결론적으로 nearest content order 는 의미 신호로는 가치가 있지만, 모든 signature 에 일반 atom 으로 넣는 방식은
    CPU 친화 기준선으로 승격할 수 없다. 다음은 `el:nlr` 를 전면 atom 이 아니라 suffix-cohort contrast/negative
    오염 억제용 보조 게이트로 제한해 비용과 희석을 줄이는 방향이 맞다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V92_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V92_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V92_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V92_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V92_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V92_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V92_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V92_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V92_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V92_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V92_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V92_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V92_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V92_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V92_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V92_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V92_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V92_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V92_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V92_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V92_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V92_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
LANE_MISMATCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V92_LANE_MISMATCH_PENALTY", "0.18"))
LANE_ARTIFACT_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V92_LANE_ARTIFACT_PENALTY", "0.10"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V92_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V92_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V92_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V92_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V92_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V92_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V92_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V92_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V92_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V92_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V92_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V92_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V92_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V92_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V92_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V92_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V92_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V92_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V92_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V92_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V92_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V92_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V92_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V92_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V92_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V92_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V92_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V92_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V92_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V92_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V92_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V92_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V92_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V92_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V92_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
RELAY_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V92_RELAY_ROW_LIMIT", "160"))
RELAY_SPECIFIC_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V92_RELAY_SPECIFIC_ROW_LIMIT", "320"))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V92_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V92_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V92_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V92_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V92_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V92_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V92_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V92_SKETCH_OCC_BUCKETS", "16"))
...
@lru_cache(maxsize=1_000_000)
def lineNearestLrAtom(leftCell: str, centerCell: str, rightCell: str) -> str:
    return f"el:nlr:{leftCell}>{centerCell}>{rightCell}"
...
def lineAtoms(
    ...
    for index in range(max(0, pos - RADIUS), min(len(stems), pos + RADIUS + 1)):
        ...
        if not isContentStem(neighborStem):
            continue
        cell = sketchView.cells.get(neighborStem, coldSketchCell(neighborStem))
        cells[index] = cell
        if index == pos:
            continue
        if index < pos:
    ...
    centerCell = cells.get(pos, "")
    if leftNearest and centerCell and rightNearest:
        atoms.add(lineNearestLrAtom(leftNearest, centerCell, rightNearest))
    return atoms
...
def buildSignatures(
    ...
    lineNearestLrInfo = lineNearestLrAtom.cache_info()
    ...
    print(
        ...
        f"nlr={lineNearestLrInfo.hits}/{lineNearestLrInfo.misses}"
    )
...
def buildModel() -> Model:
    ...
    stage("buildSketches")
    signatures, signatureCoordPostings = buildSignatures(
        caches, sketches, horizonAtomCache, horizonTokenViews, sketchRows
    )
