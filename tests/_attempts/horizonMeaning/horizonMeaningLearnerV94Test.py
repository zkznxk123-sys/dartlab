"""Horizon Meaning Learner V94 - cached nearest order contamination gate.
...
아이디어:
    ...
    나열하는 핵심 표현이라 내용은 유지해야 한다. V85 는 line atom 문자열도 canonical cache 로 재사용해,
    signature raw allocation/hash 비용을 더 줄일 수 있는지 검증한다.

    V88 은 현재 최고 기준선이다. V89/V90 은 surface aggregate cache 가 앞단 비용을 키워 실패했고, V91 은
    offset list line atom 조립이 작은 dict 보다 느려 실패했다. V92 는 nearest content left/right + center
    sketch cell 을 `el:nlr` atom 으로 main signature 에 직접 넣었다. 품질은 유지했고 `복구충당금 -> 대손충당금`
    오염 점수는 0.177 -> 0.136 으로 낮췄지만, 전체 atom 공간이 늘어 totalSeconds 가 76.4s -> 78.4s 로 후퇴했다.

    V93 은 V92 의 의미 신호를 전면 signature atom 으로 쓰지 않고, 같은 suffix + 낮은 non-suffix resonance
    후보의 오염 억제 게이트로만 제한한다. sampled occurrence 에서 nearest content order atom 을 별도 compact
    profile 로 만들고, main `xp/el/hx` signature 에는 섞지 않는다. 같은 suffix 인데 resonance 가 낮은 후보가
    nearest order 경험까지 많이 겹치면 이는 의미 연결이라기보다 같은 표/문장 틀을 공유한 경험 오염일 가능성이
    크므로 route score 에 작은 penalty 만 준다.

    V93 은 4,000 표본에서 `복구충당금 -> 대손충당금` 오염 점수를 V88 0.177, V92 0.136 보다 낮은 0.105 로
    낮췄지만, sampled occurrence 마다 nearest order atom 을 직접 계산해 buildSignatures 21.9s -> 22.8s,
    totalSeconds 76.4s -> 77.8s 로 후퇴했다. V94 는 V93 의 게이트는 유지하되 unitId-position dense cache 로
    nearest order atom 을 재사용한다. rawBridge pseudo occurrence 와 반복 표 row 는 같은 position 을 공유하므로
    V88 의 horizon atom cache 와 같은 방식으로 계산 비용을 줄일 수 있는지 검증한다.
...
시도 방법:
    ...
    80. V88 의 dense horizon atom cache 를 기준선으로 유지한다.
    81. signature sampled occurrence 에서 suffix cohort 를 가진 surface 만 nearest content left/center/right order atom 을
        별도 `nearestOrderSignatures` 로 만든다.
    82. 이 atom 은 main signature 의 `xp/el/hx` 공간과 contrast index 에 넣지 않는다.
    83. route 에서 same-suffix 이면서 resonance 가 낮은 후보에만 nearest order commonness/similarity 기반 penalty 를 더한다.
    84. nearest order atom 은 unitId-position dense list 로 cache 해 rawBridge pseudo occurrence 의 반복 계산을 줄인다.
    85. selected occurrence, rawBridge, scoring의 기존 positive gate, search gate 는 유지해 변화 원인을
        오염 억제 보조 게이트로 분리한다.

실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV94Test.py

    $env:DARTLAB_HORIZON_V94_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V94_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V94_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V94_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV94Test.py

    $env:DARTLAB_HORIZON_V94_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V94_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V94_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V94_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV94Test.py

판정 기준:
    V88 과 같은 품질 기준을 유지해야 한다. 즉 `외상매출금 -> 매출채권`, `영업손익 -> 영업이익`,
    `현금성자산 -> 현금및현금성자산`, `손실충당금 -> 대손충당금` positiveHits=4/4 와 badAccepted=0/7,
    searchTop1=5/5, reliableSearch=5/5 를 유지해야 한다. 동시에 V92 처럼 totalSeconds 를 악화시키지 않고
    4,000 표본에서 V88 의 `복구충당금 -> 대손충당금` 0.177 같은 same-suffix/no-resonance 오염 점수를 낮춰야 한다.
    품질 개선이나 오염 점수 개선 없이 buildSignatures/totalSeconds 가 후퇴하면 실패다.

결과:
    1,200 units + direct side payload 600개:
        buildSketches=6.3s, sketch sample=1.2s, sketch raw=3.3s.
        buildSignatures=13.0s, signatureStage view=0.5s, sample=0.9s, raw=3.5s,
        nearest=0.0s, prune=2.6s, weight=2.3s, relaySource=1.8s, relay=1.2s.
        nearestOrder surfaces=8,869, rawAtoms=18,795, atoms=18,724.
        nearest cache stats 는 nearestMiss=41,897/nearestHit=35,776 이었다.
        nearestOrderCohort cohorts=9,576, rawUpdates=41,595, commonAtoms=7,547.
        buildFocusedRelationIndexes=1.9s, buildSideBoundPayloadIndex=1.9s,
        buildUnitIndex=1.2s, modelSeconds=34.3, totalSeconds=36.5.
        horizon cache stats 는 sketch horizonMiss=73,805/horizonHit=42,675,
        signature horizonMiss=0/horizonHit=96,259 로 V88 과 같은 recall 을 유지했다.
        orderProbe 는 `복구충당금 -> 대손충당금` sim=0.000/commonMass=0.524/penalty=0.076,
        `대출채권 -> 매출채권` sim=0.000/commonMass=0.141/penalty=0.015 였다.
        `외상매출금 -> 매출채권` accepted=True(score 0.080, cp 0.055), `매출액` 2순위는
        accepted=False. `영업손익 -> 영업이익` accepted=True(score 0.182).
        `현금성자산 -> 현금및현금성자산` accepted=True(score 1.459).
        `손실충당금 -> 대손충당금` accepted=True(score 0.113).
        `복구충당금 -> 대손충당금` 은 score 0.993 로 accepted=False.
        `대출채권 -> 대손충당금` 은 score 0.049 로 accepted=False.
        `대출채권 -> 매출채권` 은 forbidden=-0.149 로 accepted=False.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5.

    4,000 units + direct side payload 600개:
        buildSketches=14.2s, sketch sample=3.2s, sketch raw=7.9s.
        buildSignatures=22.8s, signatureStage view=1.0s, sample=1.5s, raw=6.7s,
        nearest=0.1s, prune=4.1s, weight=3.9s, relaySource=3.2s, relay=2.3s.
        nearestOrder surfaces=13,311, rawAtoms=30,973, atoms=30,905.
        nearest cache stats 는 nearestMiss=94,514/nearestHit=77,387 이었다.
        nearestOrderCohort cohorts=13,738, rawUpdates=68,872, commonAtoms=11,264.
        buildFocusedRelationIndexes=7.0s, buildSideBoundPayloadIndex=1.9s,
        buildUnitIndex=3.5s, modelSeconds=71.4, totalSeconds=78.1.
        horizon cache stats 는 sketch horizonMiss=173,982/horizonHit=101,826,
        signature horizonMiss=0/horizonHit=208,173 로 V88 과 같은 recall 을 유지했다.
        orderProbe 는 `복구충당금 -> 대손충당금` sim=0.000/commonMass=0.500/penalty=0.072,
        `손실충당금 -> 대손충당금` commonMass=0.500 이지만 resonance 가 높아 penalty=0.000 이었다.
        `외상매출금 -> 매출채권` accepted=True(score 1.119, cp 0.624), `매출액` 2순위는
        accepted=False. `영업손익 -> 영업이익` accepted=True(score 0.197).
        `현금성자산 -> 현금및현금성자산` accepted=True(score 1.920).
        `손실충당금 -> 대손충당금` accepted=True(score 0.115).
        `복구충당금 -> 대손충당금` 은 score 0.105 로 accepted=False.
        `대출채권 -> 대손충당금` 은 score -0.134 로 accepted=False.
        `대출채권 -> 매출채권` 은 forbidden=-0.124 로 accepted=False.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5.

판정:
    실패/진단 성공. V94 는 V93 의 same-suffix/no-resonance nearest-order commonness gate 를 유지하고,
    nearest order atom 을 unitId-position dense list 로 cache 했다. 품질과 오염 억제 효과는 V93 과 동일하게
    유지되어 4,000 기준 `복구충당금 -> 대손충당금` 은 0.105, positiveHits=4/4, badAccepted=0/7,
    searchTop1=5/5, reliableSearch=5/5 를 기록했다.

    그러나 cache 는 기준선 승격에 실패했다. 4,000 에서 nearestMiss=94,514/nearestHit=77,387 로 hit 는 있었지만
    dense list 준비와 lookup 비용이 이득을 먹었고, totalSeconds 는 V93 77.8s -> V94 78.1s 로 더 나빠졌다.
    1,200 에서도 V93 35.7s -> V94 36.5s 로 후퇴했다. 결론적으로 nearest order gate 자체는 의미 있지만,
    unit-position dense cache 는 이 경로에 맞지 않다. 다음은 sampled occurrence loop 가 아니라 route-time 후보쌍에
    대해서만 지연 계산하거나, 기존 contrast/common atom 통계에서 같은 정보를 더 싸게 추출해야 한다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V94_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V94_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V94_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V94_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V94_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V94_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V94_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V94_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V94_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V94_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V94_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V94_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V94_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V94_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V94_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V94_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V94_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V94_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V94_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V94_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V94_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V94_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
LANE_MISMATCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V94_LANE_MISMATCH_PENALTY", "0.18"))
LANE_ARTIFACT_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V94_LANE_ARTIFACT_PENALTY", "0.10"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V94_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V94_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V94_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V94_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V94_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V94_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V94_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V94_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V94_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V94_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V94_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V94_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V94_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V94_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V94_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V94_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V94_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V94_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V94_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V94_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V94_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V94_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V94_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V94_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V94_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V94_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V94_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V94_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V94_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V94_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V94_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V94_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V94_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V94_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V94_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
RELAY_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V94_RELAY_ROW_LIMIT", "160"))
RELAY_SPECIFIC_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V94_RELAY_SPECIFIC_ROW_LIMIT", "320"))
NEAREST_ORDER_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V94_NEAREST_ORDER_SIGNATURE_LIMIT", "24"))
NEAREST_ORDER_PENALTY_MIN = float(os.environ.get("DARTLAB_HORIZON_V94_NEAREST_ORDER_PENALTY_MIN", "0.05"))
NEAREST_ORDER_PENALTY_SCALE = float(os.environ.get("DARTLAB_HORIZON_V94_NEAREST_ORDER_PENALTY_SCALE", "0.16"))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V94_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V94_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V94_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V94_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V94_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V94_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V94_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V94_SKETCH_OCC_BUCKETS", "16"))
...
def cachedNearestOrderAtom(
    cache: Cache,
    pos: int,
    lineView: LineTokenView,
    nearestOrderAtomCache: dict[int, list[str | None]],
    stats: Counter[str] | None = None,
) -> str:
    unitCache = nearestOrderAtomCache[cache.unit.unitId]
    cached = unitCache[pos] if pos < len(unitCache) else ""
    if cached is not None:
        if stats is not None:
            stats["hit"] += 1
        return cached
    atom = nearestOrderAtomFromView(pos, lineView)
    if pos < len(unitCache):
        unitCache[pos] = atom
    if stats is not None:
        stats["miss"] += 1
...
def buildNearestOrderSignatures(raw: dict[str, Counter[str]]) -> dict[str, Counter[str]]:
    started = time.perf_counter()
    df: Counter[str] = Counter()
    for counter in raw.values():
        df.update(counter.keys())
    ...
    for surface, counter in raw.items():
        ...
        for atom, count in counter.items():
            rows.append((math.sqrt(count) * math.log(1.0 + total / (1.0 + df[atom])), atom))
        selected = Counter({atom: score for score, atom in sorted(rows, reverse=True)[:NEAREST_ORDER_SIGNATURE_LIMIT]})
        if selected:
            signatures[surface] = selected
            atomCount += len(selected)
    finished = time.perf_counter()
    print(
        f"[nearestOrder] surfaces={len(signatures)} rawAtoms={sum(len(counter) for counter in raw.values())} "
        f"atoms={atomCount} limit={NEAREST_ORDER_SIGNATURE_LIMIT} build={finished - started:.1f}s"
    )
    return signatures


def buildNearestOrderCohortIndexes(
    nearestOrderSignatures: dict[str, Counter[str]],
) -> tuple[dict[str, Counter[str]], Counter[str]]:
    cohortAtomDf: dict[str, Counter[str]] = defaultdict(Counter)
    cohortSurfaceCounts: Counter[str] = Counter()
    rawUpdates = 0
    for surface, signature in nearestOrderSignatures.items():
        keys = suffixCohortKeys(surface)
        if not keys:
            continue
        atoms = set(signature)
    ...
    for key, counter in cohortAtomDf.items():
        ...
        if common:
            commonAtomDf[key] = common
            commonAtoms += len(common)
    print(f"[nearestOrderCohort] cohorts={len(cohortSurfaceCounts)} rawUpdates={rawUpdates} commonAtoms={commonAtoms}")
    return commonAtomDf, cohortSurfaceCounts
...
def buildSignatures(
...
) -> tuple[dict[str, Counter[str]], dict[str, list[str]], dict[str, Counter[str]]]:
    started = time.perf_counter()
    raw: dict[str, Counter[str]] = defaultdict(Counter)
    nearestOrderRaw: dict[str, Counter[str]] = defaultdict(Counter)
    stats: Counter[str] = Counter()
    nearestStats: Counter[str] = Counter()
    sketchView = buildSketchAtomView(sketches)
    lineTokenViews = buildLineTokenViews(caches, sketchView)
    nearestOrderAtomCache: dict[int, list[str | None]] = {
        unitId: [None] * len(view.cells) for unitId, view in lineTokenViews.items()
    }
    viewBuilt = time.perf_counter()
    ...
    for cache, occ in sampledOccs:
        ...
        if suffixCohortKeys(occ.surface):
            nearestAtom = cachedNearestOrderAtom(cache, occ.position, lineView, nearestOrderAtomCache, nearestStats)
            if nearestAtom:
                nearestOrderRaw[occ.surface][nearestAtom] += 1
    for surface, counter in raw.items():
        for atom in coordAtoms(surface):
            counter[atom] += 1
    rawBuilt = time.perf_counter()
    nearestOrderSignatures = buildNearestOrderSignatures(nearestOrderRaw)
    nearestBuilt = time.perf_counter()
    lineNeighborInfo = lineNeighborAtom.cache_info()
    lineTriInfo = lineTriAtom.cache_info()
    lineLrInfo = lineLrAtom.cache_info()
    nearestOrderInfo = nearestOrderAtom.cache_info()
    ...
    print(
        ...
        f"horizonMiss={stats['miss']} horizonHit={stats['hit']} "
        f"nearestMiss={nearestStats['miss']} nearestHit={nearestStats['hit']}"
    )
    print(
        f"[signatureStage] view={viewBuilt - started:.1f}s sample={sampled - viewBuilt:.1f}s "
        f"raw={rawBuilt - sampled:.1f}s nearest={nearestBuilt - rawBuilt:.1f}s "
        f"prune={pruned - nearestBuilt:.1f}s "
        ...
        f"ng={nearestOrderInfo.hits}/{nearestOrderInfo.misses}"
    )
    return signatures, postings, nearestOrderSignatures
...
def nearestOrderSimilarity(surface: str, target: str, model: Model) -> float:
    left = model.nearestOrderSignatures.get(normStem(surface))
    right = model.nearestOrderSignatures.get(normStem(target))
...
def nearestOrderCommonRatio(surface: str, atom: str, model: Model) -> float:
    ...
    for key in suffixCohortKeys(surface):
        ...
        if surfaceCount <= 1:
            continue
        ratios.append(model.nearestOrderCohortDf.get(key, Counter()).get(atom, 0) / surfaceCount)
    return max(ratios) if ratios else 0.0


def nearestOrderCommonMass(surface: str, model: Model) -> float:
    signature = model.nearestOrderSignatures.get(normStem(surface))
    if not signature:
        return 0.0
    total = sum(abs(weight) for weight in signature.values())
    if total <= 0:
        return 0.0
    return (
        sum(abs(weight) * nearestOrderCommonRatio(surface, atom, model) for atom, weight in signature.items()) / total
    )
...
def nearestOrderGatePenalty(surface: str, target: str, sameSuffix: bool, resonance: float, model: Model) -> float:
    if not sameSuffix or resonance >= RESONANCE_ACCEPT_MIN:
        return 0.0
    similarity = nearestOrderSimilarity(surface, target, model)
    commonMass = max(nearestOrderCommonMass(surface, model), nearestOrderCommonMass(target, model))
    signal = max(similarity, commonMass)
...
def buildModel() -> Model:
    ...
    stage("buildSketches")
    signatures, signatureCoordPostings, nearestOrderSignatures = buildSignatures(
        caches,
        sketches,
        horizonAtomCache,
        horizonTokenViews,
    ...
    nearestOrderCohortDf, nearestOrderCohortSurfaceCounts = buildNearestOrderCohortIndexes(nearestOrderSignatures)
    stage("buildNearestOrderCohorts")
    ...
    model = Model(
        ...
        nearestOrderSignatures,
        nearestOrderCohortDf,
        nearestOrderCohortSurfaceCounts,
        relationSpanPostings,
        relationSpanScores,
        relationFramePostings,
        relationFrameScores,
...
def main() -> None:
    ...
    print(
        ...
        + " | ".join(
            (
                f"{left}->{right}:sim{nearestOrderSimilarity(left, right, model):.3f}/"
                f"cm{max(nearestOrderCommonMass(left, model), nearestOrderCommonMass(right, model)):.3f}/"
