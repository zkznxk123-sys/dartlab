"""Horizon Meaning Learner V95 - lazy suffix common-mass gate.

V95 실제 기록
--------------
아이디어:
    V95 는 V93/V94 의 nearest-order commonness gate 가 오염 억제 신호로는 유효하지만 전역 build 비용 때문에
    V88 기준선을 넘지 못한 문제를 겨냥한다. 새 nearest-order profile/cohort 를 만들지 않고, V88 이 이미 가진
    suffix cohort contrast 통계에서 "same-suffix 인데 coord resonance 가 낮고, 양쪽 의미 atom 이 cohort 공통 경험에
    과도하게 기대는 경우" 만 route 시점에 벌점화한다. 즉 경험 그래프의 공통성 신호를 전역 추가 인덱스 없이
    재사용해 `복구충당금 -> 대손충당금` 같은 suffix 오염을 낮출 수 있는지 본다.
    ...
    나열하는 핵심 표현이라 내용은 유지해야 한다. V95 는 line atom 문자열도 canonical cache 로 재사용해,
...
시도 방법:
    ...
    80. V95 는 V88 의 existing suffix cohort contrast 통계에서 meaning atom 의 common mass 를 route 시점에 읽는다.
    81. same-suffix 이면서 coord resonance 가 낮은 후보만 벌점화해, 새 nearest-order 전역 인덱스 없이 오염 억제가 가능한지 본다.
    82. atom text, selected occurrence, scoring, acceptance/search gate 는 그대로 두어 변화 원인을 route penalty 로 분리한다.

실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV95Test.py

    $env:DARTLAB_HORIZON_V95_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V95_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V95_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V95_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV95Test.py

    $env:DARTLAB_HORIZON_V95_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V95_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V95_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V95_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV95Test.py

판정 기준:
    V88 과 같은 품질 기준을 유지해야 한다. 동시에 기존 suffix cohort contrast 통계만 재사용해
    `복구충당금 -> 대손충당금` 오염 점수를 V88 0.177 보다 낮추고, V93 처럼 buildSignatures 를
    22초대로 늘리지 않아야 한다.

결과:
    1,200 units + direct side payload 600개:
        suffixCommonProbe 는 `복구충당금 -> 대손충당금` cm=0.019/pen=0.000 으로 사실상 신호가 없었다.
        buildSketches=6.0s, buildSignatures=12.2s, buildFocusedRelationIndexes=2.1s,
        buildSideBoundPayloadIndex=1.9s, buildUnitIndex=1.2s, modelSeconds=33.5, totalSeconds=35.6.
        `복구충당금 -> 대손충당금` 은 score 1.065 로 V88 1.069 와 거의 같았다.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5.

    4,000 units + direct side payload 600개:
        suffixCommonProbe 는 `복구충당금 -> 대손충당금` cm=0.336/pen=0.024 로 약하게만 걸렸다.
        buildSketches=14.1s, buildSignatures=22.0s, buildFocusedRelationIndexes=6.8s,
        buildSideBoundPayloadIndex=2.1s, buildUnitIndex=4.1s, modelSeconds=71.5, totalSeconds=78.5.
        `복구충당금 -> 대손충당금` 은 score 0.152 로 V88 0.177 보다는 낮았지만 V93 0.105 보다 약했다.
        `대출채권 -> 대손충당금` -0.134, `대출채권 -> 매출채권` -0.124 유지.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5.

판정:
    실패/진단 성공. 기존 contrast commonness 는 build-time 추가 인덱스 없이 재사용 가능하지만,
    nearest-order commonness 가 잡은 `복구충당금` 오염을 충분히 설명하지 못한다. V93 의 핵심은
    일반 meaning atom 공통성이 아니라 "stem 앞뒤 nearest content order 경험" 이다. 다음은 이 신호 자체를
    build-time 전역 profile 이 아니라 route-time lazy profile 로 옮겨야 한다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V95_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V95_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V95_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V95_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V95_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V95_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V95_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V95_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V95_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V95_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V95_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V95_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V95_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V95_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V95_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V95_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V95_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V95_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V95_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V95_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V95_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V95_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
LANE_MISMATCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V95_LANE_MISMATCH_PENALTY", "0.18"))
LANE_ARTIFACT_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V95_LANE_ARTIFACT_PENALTY", "0.10"))
SUFFIX_COMMON_PENALTY_MIN = float(os.environ.get("DARTLAB_HORIZON_V95_SUFFIX_COMMON_PENALTY_MIN", "0.20"))
SUFFIX_COMMON_PENALTY_SCALE = float(os.environ.get("DARTLAB_HORIZON_V95_SUFFIX_COMMON_PENALTY_SCALE", "0.18"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V95_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V95_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V95_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V95_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V95_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V95_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V95_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V95_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V95_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V95_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V95_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V95_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V95_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V95_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V95_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V95_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V95_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V95_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V95_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V95_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V95_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V95_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V95_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V95_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V95_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V95_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V95_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V95_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V95_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V95_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V95_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V95_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V95_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V95_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V95_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
RELAY_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V95_RELAY_ROW_LIMIT", "160"))
RELAY_SPECIFIC_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V95_RELAY_SPECIFIC_ROW_LIMIT", "320"))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V95_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V95_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V95_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V95_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V95_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V95_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V95_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V95_SKETCH_OCC_BUCKETS", "16"))
...
def suffixCommonMass(surface: str, signature: Counter[str], model: Model) -> float:
    total = 0.0
    common = 0.0
    for atom, weight in signature.items():
        if not meaningAtom(atom):
            continue
        mass = abs(float(weight))
        if mass <= 0:
            continue
        total += mass
        common += mass * cohortCommonRatio(surface, atom, model)
    if total <= 0:
        return 0.0
    return common / total


def suffixCommonGatePenalty(
    surface: str,
    target: str,
    sameSuffix: bool,
    resonance: float,
    ...
    if not sameSuffix or resonance >= RESONANCE_ACCEPT_MIN:
        return 0.0
    signal = max(
        suffixCommonMass(surface, querySignature, model),
        suffixCommonMass(target, targetSignature, model),
    )
    if signal <= SUFFIX_COMMON_PENALTY_MIN:
        return 0.0
    return (signal - SUFFIX_COMMON_PENALTY_MIN) * SUFFIX_COMMON_PENALTY_SCALE
...
def route(surface: str, model: Model):
    ...
    for target in TARGETS:
        ...
        suffixCommonPenalty = suffixCommonGatePenalty(surface, target, sameSuffix, resonance, query, targetSig, model)
...
def main() -> None:
    ...
    print(
        ...
        + " | ".join(
            (
                f"{left}->{right}:"
                f"cm{max(suffixCommonMass(left, inferSignature(left, model), model), suffixCommonMass(right, inferSignature(right, model), model)):.3f}/"
                f"pen{suffixCommonGatePenalty(left, right, longestCommonSuffixSize(left, right) >= COHORT_SUFFIX_MIN, coordResonance(left, right, model), inferSignature(left, model), inferSignature(right, model), model):.3f}"
            )
            for left, right in (
                ("복구충당금", "대손충당금"),
                ("손실충당금", "대손충당금"),
