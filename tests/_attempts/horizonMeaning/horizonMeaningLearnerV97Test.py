"""Horizon Meaning Learner V97 - target-suffix lazy nearest-order gate.

V97 실제 기록
--------------
아이디어:
    V97 은 V96 의 lazy nearest-order gate 를 더 줄인다. V96 은 route-time 에만 nearest-order cohort 를 계산했지만
    buildSignatures 에서 모든 suffix surface 의 sampled occurrence 위치를 저장해 4,000 표본 buildSignatures 가
    23.2초까지 늘었다. route 는 TARGETS 와의 same-suffix 후보에만 이 gate 를 쓰므로, TARGETS 의 suffix cohort key 와
    겹치는 surface 의 occurrence 만 lazy index 에 보존한다. 목표는 V96 의 강한 오염 억제를 유지하면서 V88/V93 보다
    작은 build overhead 로 되돌리는 것이다.
    ...
    나열하는 핵심 표현이라 내용은 유지해야 한다. V97 는 line atom 문자열도 canonical cache 로 재사용해,
...
시도 방법:
    ...
    80. V97 은 V96 의 lazy nearest-order gate 를 TARGETS 의 suffix cohort key 와 겹치는 surface 로 제한한다.
    81. buildSignatures 는 route 에서 실제로 같은 suffix 후보가 될 수 있는 sampled occurrence 위치만 lazy index 에 보존한다.
    82. atom text, selected occurrence, scoring, acceptance/search gate 는 그대로 두어 변화 원인을 target-suffix index pruning 으로 분리한다.

실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV97Test.py

    $env:DARTLAB_HORIZON_V97_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V97_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V97_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V97_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV97Test.py

    $env:DARTLAB_HORIZON_V97_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V97_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V97_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V97_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV97Test.py

판정 기준:
    V88/V93 과 같은 품질 기준을 유지해야 한다. V96 의 nearest-order 오염 억제를 유지하면서,
    lazy occurrence index 를 TARGETS 의 suffix cohort key 와 겹치는 surface 로 제한해 buildSignatures 와
    totalSeconds 를 V88 기준선 수준으로 되돌려야 한다.

결과:
    1,200 units + direct side payload 600개:
        nearestOrderLazySeed 는 V96 의 surfaces=9,528/positions=77,673 에서 surfaces=226/positions=3,611 로 줄었다.
        orderProbe 는 `복구충당금 -> 대손충당금` sim=0.000/cm=1.000/pen=0.152,
        `손실충당금 -> 대손충당금` cm=1.000/pen=0.000 이었다.
        buildSketches=5.9s, buildSignatures=12.3s, buildFocusedRelationIndexes=2.1s,
        buildSideBoundPayloadIndex=1.8s, buildUnitIndex=1.2s, modelSeconds=33.0, totalSeconds=35.2.
        `복구충당금 -> 대손충당금` 은 score 0.917 로 V88 1.069 보다 낮아졌다.
        `대출채권 -> 매출채권` 은 -0.151 로 더 낮아졌고 positiveHits=4/4, badAccepted=0/7,
        searchTop1=5/5, reliableSearch=5/5 를 유지했다.

    4,000 units + direct side payload 600개:
        nearestOrderLazySeed 는 V96 의 surfaces=14,475/positions=171,901 에서 surfaces=276/positions=5,844 로 줄었다.
        orderProbe 는 `복구충당금 -> 대손충당금` sim=0.000/cm=1.000/pen=0.152,
        `손실충당금 -> 대손충당금` cm=1.000/pen=0.000 이었다.
        buildSketches=13.8s, buildSignatures=21.9s, buildFocusedRelationIndexes=6.3s,
        buildSideBoundPayloadIndex=2.0s, buildUnitIndex=4.0s, modelSeconds=69.8, totalSeconds=76.4.
        ...
        lazy stats 는 profileBuild=230, profilePositions=4,968, cohortBuild=16 이었다.

판정:
    성공/구조 개선. V97 은 nearest-order experience graph 를 전역 signature 로 넣지 않고, route target suffix 와
    실제로 만나는 surface 의 sampled occurrence 위치만 보존한 뒤 route-time 에 cohort commonness 를 계산한다.
    4,000 기준 V88 과 같은 totalSeconds=76.4/modelSeconds=69.8/buildSignatures=21.9 를 유지하면서
    `복구충당금 -> 대손충당금` 오염 점수를 0.177 -> 0.025 로 낮췄다. V93/V94 보다 오염 억제는 강하고
    build 비용은 낮다. 다음 단계는 이 lazy nearest-order gate 를 route 대상 고정 구조에서 더 일반적인 query-time
    target set 으로 확장할 수 있는지, 그리고 cm=1.000 포화 신호를 더 세밀하게 calibrate 할 수 있는지 검증하는 것이다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V97_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V97_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V97_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V97_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V97_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V97_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V97_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V97_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V97_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V97_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V97_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V97_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V97_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V97_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V97_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V97_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V97_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V97_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V97_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V97_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V97_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V97_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
LANE_MISMATCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V97_LANE_MISMATCH_PENALTY", "0.18"))
LANE_ARTIFACT_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V97_LANE_ARTIFACT_PENALTY", "0.10"))
NEAREST_ORDER_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V97_NEAREST_ORDER_SIGNATURE_LIMIT", "24"))
NEAREST_ORDER_PENALTY_MIN = float(os.environ.get("DARTLAB_HORIZON_V97_NEAREST_ORDER_PENALTY_MIN", "0.05"))
NEAREST_ORDER_PENALTY_SCALE = float(os.environ.get("DARTLAB_HORIZON_V97_NEAREST_ORDER_PENALTY_SCALE", "0.16"))
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V97_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V97_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V97_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V97_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V97_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V97_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V97_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V97_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V97_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V97_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V97_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V97_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V97_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V97_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V97_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V97_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V97_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V97_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V97_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V97_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V97_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V97_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V97_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V97_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V97_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V97_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V97_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V97_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V97_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V97_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V97_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V97_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V97_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V97_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V97_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
RELAY_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V97_RELAY_ROW_LIMIT", "160"))
RELAY_SPECIFIC_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V97_RELAY_SPECIFIC_ROW_LIMIT", "320"))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V97_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V97_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V97_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V97_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V97_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V97_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V97_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V97_SKETCH_OCC_BUCKETS", "16"))
...
@lru_cache(maxsize=1)
def routeTargetSuffixCohortKeys() -> frozenset[str]:
    keys: set[str] = set()
    for target in TARGETS:
        keys.update(suffixCohortKeys(target))
    return frozenset(keys)


def routeNearestOrderIndexedSurface(surface: str) -> bool:
    return bool(routeTargetSuffixCohortKeys().intersection(suffixCohortKeys(surface)))
...
def buildSignatures(
    ...
    sampled = time.perf_counter()
    occurrenceRows: dict[str, list[tuple[int, int]]] = defaultdict(list)
    for cache, occ in sampledOccs:
        lineView = lineTokenViews[cache.unit.unitId]
        raw[occ.surface].update(cachedHorizonAtoms(cache, occ.position, horizonAtomCache, horizonTokenViews, stats))
        raw[occ.surface].update(lineAtomsFromView(occ.position, lineView))
        if routeNearestOrderIndexedSurface(occ.surface):
            occurrenceRows[occ.surface].append((cache.unit.unitId, occ.position))
    for surface, counter in raw.items():
        for atom in coordAtoms(surface):
            counter[atom] += 1
    rawBuilt = time.perf_counter()
    ...
    occurrenceIndex = {surface: tuple(rows) for surface, rows in occurrenceRows.items()}
    print(
        f"[nearestOrderLazySeed] surfaces={len(occurrenceIndex)} "
        f"positions={sum(len(rows) for rows in occurrenceIndex.values())}"
    )
    return signatures, postings, lineTokenViews, occurrenceIndex
...
def nearestOrderCohortSurfaceList(key: str, model: Model) -> tuple[str, ...]:
    ...
    if cached is not None:
        return cached
    surfaces = tuple(surface for surface in model.signatureOccurrenceIndex if key in suffixCohortKeys(surface))
...
def buildModel() -> Model:
    ...
    stage("buildSketches")
    signatures, signatureCoordPostings, lineTokenViews, signatureOccurrenceIndex = buildSignatures(
        caches,
        sketches,
        horizonAtomCache,
        horizonTokenViews,
