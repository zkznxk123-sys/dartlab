"""Horizon Meaning Learner V98 - bounded cohort nearest-order buckets.

V98 실제 기록
--------------
아이디어:
    V98 은 V97 의 `TARGETS` suffix prefilter 를 제거한다. V96 처럼 모든 suffix surface occurrence 를 surface 단위로
    저장하면 4,000 표본에서 14,475 surfaces/171,901 positions 로 커졌다. V97 은 이것을 TARGETS suffix 와 겹치는
    276 surfaces/5,844 positions 로 줄여 성공했지만, 검색 엔진으로 가려면 target set 이 고정되어 있으면 안 된다.
    V98 은 buildSignatures 에서 모든 suffix cohort key 에 대해 bounded occurrence bucket 을 만든다. key 마다
    surface 수와 position 수를 제한하고, route-time 에는 target suffix key bucket 만 읽는다. 목표는 target 고정을
    줄이면서 V97 의 오염 억제와 V88 수준의 CPU 비용을 얼마나 유지할 수 있는지 보는 것이다.
    ...
    나열하는 핵심 표현이라 내용은 유지해야 한다. V98 는 line atom 문자열도 canonical cache 로 재사용해,
...
시도 방법:
    ...
    80. V98 은 V97 의 TARGETS suffix prefilter 를 제거하고 모든 suffix cohort key 에 bounded occurrence bucket 을 만든다.
    81. buildSignatures 는 sampled occurrence 를 key/surface/position limit 으로 cohort bucket 에 저장해 target 고정 없이 lazy gate 를 열어본다.
    82. atom text, selected occurrence, scoring, acceptance/search gate 는 그대로 두어 변화 원인을 bounded cohort bucket 으로 분리한다.

실행:
    uv run python -X utf8 -m py_compile tests/_attempts/horizonMeaning/horizonMeaningLearnerV98Test.py

    $env:DARTLAB_HORIZON_V98_MAX_FILES_PER_SOURCE='8'
    $env:DARTLAB_HORIZON_V98_MAX_RECORDS_PER_SOURCE='180'
    $env:DARTLAB_HORIZON_V98_MAX_UNITS='1200'
    $env:DARTLAB_HORIZON_V98_MAX_WINDOWS_PER_RECORD='2'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV98Test.py

    $env:DARTLAB_HORIZON_V98_MAX_FILES_PER_SOURCE='20'
    $env:DARTLAB_HORIZON_V98_MAX_RECORDS_PER_SOURCE='600'
    $env:DARTLAB_HORIZON_V98_MAX_UNITS='4000'
    $env:DARTLAB_HORIZON_V98_MAX_WINDOWS_PER_RECORD='3'
    uv run python -X utf8 tests/_attempts/horizonMeaning/horizonMeaningLearnerV98Test.py

판정 기준:
    V88/V93 과 같은 품질 기준을 유지해야 한다. V97 의 nearest-order 오염 억제를 유지하면서,
    TARGETS suffix prefilter 없이 cohort bucket bounds 만으로 buildSignatures 와 totalSeconds 를 V88/V97 기준선에
    가깝게 유지해야 한다.

결과:
    1,200 units + direct side payload 600개:
        nearestOrderLazySeed 는 mode=boundedCohort, keys=10,121, surfaces=9,528, positions=77,673,
        cohortPositions=175,872 이었다. 설정한 limit 은 실제로 거의 작동하지 않았다.
        buildSignatures=12.5s, modelSeconds=33.2, totalSeconds=35.5.
        `복구충당금 -> 대손충당금` 은 score 0.918 로 낮아졌고 positiveHits=4/4, badAccepted=0/7,
        searchTop1=5/5, reliableSearch=5/5 를 유지했다.

    4,000 units + direct side payload 600개:
        nearestOrderLazySeed 는 mode=boundedCohort, keys=14,607, surfaces=14,475, positions=171,901,
        cohortPositions=388,957 이었다. V96 과 거의 같은 footprint 라 build-time index 축소에는 실패했다.
        buildSignatures=23.0s, modelSeconds=71.0, totalSeconds=78.0.
        `복구충당금 -> 대손충당금` 은 score 0.025 로 V97 과 같은 오염 억제를 유지했다.
        positiveHits=4/4, badAccepted=0/7, searchTop1=5/5, reliableSearch=5/5.

판정:
    실패/진단 성공. TARGETS suffix prefilter 를 제거해도 nearest-order 오염 억제 신호 자체는 유지되지만,
    모든 suffix key 에 bounded bucket 을 물리화하면 bound 가 충분히 줄지 않아 V96 같은 큰 footprint 로 돌아간다.
    다음은 build-time cohort bucket 을 만들지 않고, route-time 에 필요한 cohort 만 sampled row 에서 복원하는 방향이다.
"""
...
MAX_FILES_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V98_MAX_FILES_PER_SOURCE", "30"))
MAX_RECORDS_PER_SOURCE = int(os.environ.get("DARTLAB_HORIZON_V98_MAX_RECORDS_PER_SOURCE", "700"))
MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V98_MAX_UNITS", "8000"))
MAX_WINDOWS_PER_RECORD = int(os.environ.get("DARTLAB_HORIZON_V98_MAX_WINDOWS_PER_RECORD", "3"))
SIDE_MAX_FILES_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V98_SIDE_MAX_FILES_PER_SOURCE", str(max(20, MAX_FILES_PER_SOURCE)))
)
SIDE_MAX_RECORDS_PER_SOURCE = int(
    os.environ.get("DARTLAB_HORIZON_V98_SIDE_MAX_RECORDS_PER_SOURCE", str(max(600, MAX_RECORDS_PER_SOURCE)))
)
SIDE_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_V98_SIDE_MAX_UNITS", "600"))
WINDOW_CHARS = int(os.environ.get("DARTLAB_HORIZON_V98_WINDOW_CHARS", "720"))
RADIUS = int(os.environ.get("DARTLAB_HORIZON_V98_RADIUS", "6"))
SKETCH_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V98_SKETCH_LIMIT", "32"))
SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V98_SIGNATURE_LIMIT", "96"))
POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V98_POSTING_LIMIT", "1200"))
SEARCH_RELATION_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V98_SEARCH_RELATION_POSTING_LIMIT", "2400"))
SEARCH_CANDIDATE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V98_SEARCH_CANDIDATE_LIMIT", "420"))
ROUTE_MIN_SCORE = float(os.environ.get("DARTLAB_HORIZON_V98_ROUTE_MIN_SCORE", "0.075"))
ROUTE_MIN_EXPERIENCE = float(os.environ.get("DARTLAB_HORIZON_V98_ROUTE_MIN_EXPERIENCE", "0.018"))
COHORT_SUFFIX_MIN = int(os.environ.get("DARTLAB_HORIZON_V98_COHORT_SUFFIX_MIN", "2"))
COHORT_SUFFIX_MAX = int(os.environ.get("DARTLAB_HORIZON_V98_COHORT_SUFFIX_MAX", "4"))
CONTRAST_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V98_CONTRAST_COMMON_RATIO", "0.34"))
CONTRAST_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V98_CONTRAST_ACCEPT_MIN", "0.010"))
RESONANCE_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V98_RESONANCE_ACCEPT_MIN", "0.030"))
COMPOUND_ASSOC_ACCEPT_MIN = float(os.environ.get("DARTLAB_HORIZON_V98_COMPOUND_ASSOC_ACCEPT_MIN", "0.045"))
LANE_MISMATCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V98_LANE_MISMATCH_PENALTY", "0.18"))
LANE_ARTIFACT_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V98_LANE_ARTIFACT_PENALTY", "0.10"))
NEAREST_ORDER_SIGNATURE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V98_NEAREST_ORDER_SIGNATURE_LIMIT", "24"))
NEAREST_ORDER_PENALTY_MIN = float(os.environ.get("DARTLAB_HORIZON_V98_NEAREST_ORDER_PENALTY_MIN", "0.05"))
NEAREST_ORDER_PENALTY_SCALE = float(os.environ.get("DARTLAB_HORIZON_V98_NEAREST_ORDER_PENALTY_SCALE", "0.16"))
NEAREST_ORDER_COHORT_SURFACE_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V98_NEAREST_ORDER_COHORT_SURFACE_LIMIT", "384")
)
NEAREST_ORDER_COHORT_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V98_NEAREST_ORDER_COHORT_POSITION_LIMIT", "8192")
)
NEAREST_ORDER_SURFACE_POSITION_LIMIT = int(
    os.environ.get("DARTLAB_HORIZON_V98_NEAREST_ORDER_SURFACE_POSITION_LIMIT", "48")
)
ROUTE_ACCEPT_MARGIN_RATIO = float(os.environ.get("DARTLAB_HORIZON_V98_ROUTE_ACCEPT_MARGIN_RATIO", "0.42"))
ROUTE_ACCEPT_MARGIN_GAP = float(os.environ.get("DARTLAB_HORIZON_V98_ROUTE_ACCEPT_MARGIN_GAP", "0.060"))
SEARCH_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V98_SEARCH_EVIDENCE_MIN", "0.34"))
SPAN_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V98_SPAN_MAX_DISTANCE", "160"))
FRAME_MAX_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V98_FRAME_MAX_DISTANCE", "180"))
FOCUSED_FRAME_DISTANCE = int(os.environ.get("DARTLAB_HORIZON_V98_FOCUSED_FRAME_DISTANCE", str(FRAME_MAX_DISTANCE * 2)))
TABLE_ROW_LEAK_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V98_TABLE_ROW_LEAK_EVIDENCE_CAP", "0.18"))
TABLE_ROW_LEAK_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V98_TABLE_ROW_LEAK_SEARCH_PENALTY", "8.0"))
ROLE_BOUND_EVIDENCE_CAP = float(os.environ.get("DARTLAB_HORIZON_V98_ROLE_BOUND_EVIDENCE_CAP", "0.48"))
ROLE_BOUND_SEARCH_PENALTY = float(os.environ.get("DARTLAB_HORIZON_V98_ROLE_BOUND_SEARCH_PENALTY", "5.0"))
RELIABLE_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V98_RELIABLE_BOUND_MIN", "0.55"))
WEAK_BOUND_MIN = float(os.environ.get("DARTLAB_HORIZON_V98_WEAK_BOUND_MIN", "0.34"))
RELIABLE_EVIDENCE_MIN = float(os.environ.get("DARTLAB_HORIZON_V98_RELIABLE_EVIDENCE_MIN", "0.70"))
SIDE_FALLBACK_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V98_SIDE_FALLBACK_LIMIT", "220"))
RAW_BRIDGE_MIN_SIM = float(os.environ.get("DARTLAB_HORIZON_V98_RAW_BRIDGE_MIN_SIM", "0.24"))
RAW_BRIDGE_MIN_SIZE = int(os.environ.get("DARTLAB_HORIZON_V98_RAW_BRIDGE_MIN_SIZE", "4"))
RAW_BRIDGE_MAX_SIZE = int(os.environ.get("DARTLAB_HORIZON_V98_RAW_BRIDGE_MAX_SIZE", "8"))
RAW_BRIDGE_MAX_TOKEN = int(os.environ.get("DARTLAB_HORIZON_V98_RAW_BRIDGE_MAX_TOKEN", "18"))
CORPUS_BRIDGE_SEED_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V98_CORPUS_BRIDGE_SEED_LIMIT", "2400"))
CORPUS_BRIDGE_SEED_MIN_DF = int(os.environ.get("DARTLAB_HORIZON_V98_CORPUS_BRIDGE_SEED_MIN_DF", "1"))
CORPUS_BRIDGE_GRAM_POSTING_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V98_CORPUS_BRIDGE_GRAM_POSTING_LIMIT", "360"))
CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM = float(
    os.environ.get("DARTLAB_HORIZON_V98_CORPUS_BRIDGE_NON_CONTAINMENT_MIN_SIM", "0.46")
)
CORPUS_BRIDGE_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V98_CORPUS_BRIDGE_RELATION_RADIUS", "9"))
CORPUS_BRIDGE_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V98_CORPUS_BRIDGE_VALUE_RADIUS", "7"))
CORPUS_BRIDGE_MIN_EVIDENCE = float(os.environ.get("DARTLAB_HORIZON_V98_CORPUS_BRIDGE_MIN_EVIDENCE", "0.55"))
CORPUS_BRIDGE_SUBSURFACE_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V98_CORPUS_BRIDGE_SUBSURFACE_LIMIT", "4"))
COHORT_CONTRAST_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V98_COHORT_CONTRAST_ATOM_LIMIT", "48"))
RELAY_NEIGHBOR_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V98_RELAY_NEIGHBOR_LIMIT", "6"))
RELAY_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V98_RELAY_ATOM_LIMIT", "16"))
RAW_PRUNE_XP_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V98_RAW_PRUNE_XP_LIMIT", "96"))
RAW_PRUNE_HX_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V98_RAW_PRUNE_HX_LIMIT", "96"))
RAW_PRUNE_EL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V98_RAW_PRUNE_EL_LIMIT", "48"))
RAW_PRUNE_OTHER_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V98_RAW_PRUNE_OTHER_LIMIT", "32"))
RELAY_COMMON_ATOM_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V98_RELAY_COMMON_ATOM_LIMIT", "40"))
RELAY_COMMON_RATIO = float(os.environ.get("DARTLAB_HORIZON_V98_RELAY_COMMON_RATIO", str(CONTRAST_COMMON_RATIO)))
RELAY_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V98_RELAY_ROW_LIMIT", "160"))
RELAY_SPECIFIC_ROW_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V98_RELAY_SPECIFIC_ROW_LIMIT", "320"))
SIGNATURE_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V98_SIGNATURE_OCC_FULL_LIMIT", "8"))
SIGNATURE_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V98_SIGNATURE_OCC_BUDGET", "48"))
SIGNATURE_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V98_SIGNATURE_OCC_BUCKETS", "12"))
SIGNATURE_OCC_RELATION_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V98_SIGNATURE_OCC_RELATION_RADIUS", "8"))
SIGNATURE_OCC_VALUE_RADIUS = int(os.environ.get("DARTLAB_HORIZON_V98_SIGNATURE_OCC_VALUE_RADIUS", "6"))
SKETCH_OCC_FULL_LIMIT = int(os.environ.get("DARTLAB_HORIZON_V98_SKETCH_OCC_FULL_LIMIT", "12"))
SKETCH_OCC_BUDGET = int(os.environ.get("DARTLAB_HORIZON_V98_SKETCH_OCC_BUDGET", "96"))
SKETCH_OCC_BUCKETS = int(os.environ.get("DARTLAB_HORIZON_V98_SKETCH_OCC_BUCKETS", "16"))
...
class Model:
    ...
    nearestOrderCohortOccurrenceIndex: dict[str, dict[str, tuple[tuple[int, int], ...]]]
...
def buildSignatures(
    ...
    sampled = time.perf_counter()
    cohortRows: dict[str, dict[str, list[tuple[int, int]]]] = defaultdict(dict)
    cohortPositionCounts: Counter[str] = Counter()
    cohortStats: Counter[str] = Counter()
    for cache, occ in sampledOccs:
        lineView = lineTokenViews[cache.unit.unitId]
        raw[occ.surface].update(cachedHorizonAtoms(cache, occ.position, horizonAtomCache, horizonTokenViews, stats))
        raw[occ.surface].update(lineAtomsFromView(occ.position, lineView))
        for key in suffixCohortKeys(occ.surface):
            cohortStats["suffixKeyChecks"] += 1
            bucket = cohortRows[key]
            surfaceRows = bucket.get(occ.surface)
            if surfaceRows is None:
                if len(bucket) >= NEAREST_ORDER_COHORT_SURFACE_LIMIT:
                    cohortStats["surfaceLimitSkips"] += 1
                    continue
                surfaceRows = []
                bucket[occ.surface] = surfaceRows
            if len(surfaceRows) >= NEAREST_ORDER_SURFACE_POSITION_LIMIT:
                cohortStats["surfacePositionLimitSkips"] += 1
                continue
            if cohortPositionCounts[key] >= NEAREST_ORDER_COHORT_POSITION_LIMIT:
                cohortStats["cohortPositionLimitSkips"] += 1
                continue
            surfaceRows.append((cache.unit.unitId, occ.position))
            cohortPositionCounts[key] += 1
            cohortStats["keptPositions"] += 1
    for surface, counter in raw.items():
        for atom in coordAtoms(surface):
            counter[atom] += 1
    rawBuilt = time.perf_counter()
    ...
    cohortOccurrenceIndex = {
        key: {surface: tuple(rows) for surface, rows in bucket.items()} for key, bucket in cohortRows.items()
    }
    occurrenceSets: dict[str, set[tuple[int, int]]] = defaultdict(set)
    for bucket in cohortOccurrenceIndex.values():
        for surface, rows in bucket.items():
            occurrenceSets[surface].update(rows)
    occurrenceIndex = {surface: tuple(sorted(rows)) for surface, rows in occurrenceSets.items()}
    print(
        f"[nearestOrderLazySeed] mode=boundedCohort keys={len(cohortOccurrenceIndex)} "
        f"surfaces={len(occurrenceIndex)} positions={sum(len(rows) for rows in occurrenceIndex.values())} "
        f"cohortPositions={sum(cohortPositionCounts.values())} stats={dict(cohortStats)} "
        f"limits=surface{NEAREST_ORDER_COHORT_SURFACE_LIMIT}/pos{NEAREST_ORDER_COHORT_POSITION_LIMIT}/surfacePos{NEAREST_ORDER_SURFACE_POSITION_LIMIT}"
    )
    return signatures, postings, lineTokenViews, occurrenceIndex, cohortOccurrenceIndex
...
def nearestOrderCohortSurfaceList(key: str, model: Model) -> tuple[str, ...]:
    cached = model.nearestOrderCohortSurfaces.get(key)
    if cached is not None:
        return cached
    surfaces = tuple(model.nearestOrderCohortOccurrenceIndex.get(key, ()))
    model.nearestOrderCohortSurfaces[key] = surfaces
...
def nearestOrderCommonMass(surface: str, model: Model) -> float:
    ...
    if total <= 0:
        return 0.0
    return sum(abs(weight) * nearestOrderCommonRatio(surface, atom, model) for atom, weight in profile.items()) / total
...
def nearestOrderGatePenalty(surface: str, target: str, sameSuffix: bool, resonance: float, model: Model) -> float:
    ...
    signal = max(
        ...
        nearestOrderCommonMass(surface, model),
        nearestOrderCommonMass(target, model),
    )
...
def buildModel() -> Model:
    ...
    stage("buildSketches")
    signatures, signatureCoordPostings, lineTokenViews, signatureOccurrenceIndex, nearestOrderCohortOccurrenceIndex = (
        buildSignatures(
            caches,
            sketches,
            horizonAtomCache,
    ...
    model = Model(
        ...
        nearestOrderCohortOccurrenceIndex,
        {},
        {},
        {},
        Counter(),
