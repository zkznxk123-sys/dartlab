"""Horizon Posting Meaning Search v11 — posting association graph.
...
---------
기본 샘플 실행:
    uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonPostingMeaningSearchV11Test.py
...
pairPostings 1,474, assocFocus 60, assocEdges 1,064, rough memory 56.0MB.
exactQuality 79, fireQuality 80, assocQuality 79. evidence rubric 은 exactRubric 78,
fireRubric 78, assocRubric 78. fire 는 heuristic 1점 개선이지만 rubric 개선은 0/same 8/worsened 0.
association graph 를 독립 후보 확장으로 쓰는 중간 시도는 assocQuality 78, assocRubric 77 로
매출채권 회수 지연 probe 를 악화시켰다. 최종 코드는 exact 후보 내부 rerank/explanation 으로
제한했고 assocImproved 0/same 8/worsened 0, assocRubricImproved 0/same 8/worsened 0.
속도는 exact 9.82ms, fire 44.38ms, assoc 11.48ms. 결론: posting association 은 빠르고
안전한 보조 설명 신호지만, 현재 방식으로는 strict rubric 품질을 올리지 못한다.
"""
...
def summarizeMode(
    ...
    *,
    ...
    t0 = time.perf_counter()
    rows = search(model, spec.query, useFire=useFire, useAssociation=useAssociation)
...
def evaluateExperiment(model: Model) -> None:
    print("\n" + "=" * 88)
    print("[evaluation] exact-only vs exact+fire vs exact+association")
    print("=" * 88)
    ...
    fireWorsened = 0
    fireSame = 0
    fireRubricImproved = 0
    fireRubricWorsened = 0
    fireRubricSame = 0
    assocImproved = 0
    assocWorsened = 0
    assocSame = 0
    assocRubricImproved = 0
    assocRubricWorsened = 0
    assocRubricSame = 0
    exactLatency: list[float] = []
    fireLatency: list[float] = []
    assocLatency: list[float] = []
    fireDuplicateTotal = 0
    fireTableTotal = 0
    assocDuplicateTotal = 0
    assocTableTotal = 0
    ...
    for spec in PROBE_QUERIES:
        ...
        assocScore = int(assoc["qualitySum"])
        exactRubricScore = int(exact["rubricQualitySum"])
        fireRubricScore = int(fire["rubricQualitySum"])
        assocRubricScore = int(assoc["rubricQualitySum"])
        exactTotal += exactScore
        fireTotal += fireScore
        assocTotal += assocScore
        exactRubricTotal += exactRubricScore
        fireRubricTotal += fireRubricScore
        assocRubricTotal += assocRubricScore
        exactLatency.append(float(exact["latencyMs"]))
        fireLatency.append(float(fire["latencyMs"]))
        assocLatency.append(float(assoc["latencyMs"]))
        fireDuplicateTotal += int(fire["duplicates"])
        fireTableTotal += int(fire["tables"])
        assocDuplicateTotal += int(assoc["duplicates"])
        assocTableTotal += int(assoc["tables"])
        if fireScore > exactScore:
            fireVerdict = "IMPROVED"
            fireImproved += 1
        elif fireScore < exactScore:
            fireVerdict = "WORSENED"
            fireWorsened += 1
        else:
            fireVerdict = "SAME"
            fireSame += 1
        if fireRubricScore > exactRubricScore:
            fireRubricVerdict = "IMPROVED"
            fireRubricImproved += 1
        elif fireRubricScore < exactRubricScore:
            fireRubricVerdict = "WORSENED"
            fireRubricWorsened += 1
        else:
            fireRubricVerdict = "SAME"
            fireRubricSame += 1
        if assocScore > exactScore:
            assocVerdict = "IMPROVED"
            assocImproved += 1
        elif assocScore < exactScore:
            assocVerdict = "WORSENED"
            assocWorsened += 1
        else:
            assocVerdict = "SAME"
            assocSame += 1
        if assocRubricScore > exactRubricScore:
            assocRubricVerdict = "IMPROVED"
            assocRubricImproved += 1
        elif assocRubricScore < exactRubricScore:
            assocRubricVerdict = "WORSENED"
            assocRubricWorsened += 1
        else:
            assocRubricVerdict = "SAME"
            assocRubricSame += 1

        print("\n" + "-" * 88)
        print(
            f"[query] {spec.query} fireVerdict={fireVerdict} fireRubricVerdict={fireRubricVerdict} "
            f"assocVerdict={assocVerdict} assocRubricVerdict={assocRubricVerdict} "
            f"exactQuality={exactScore} fireQuality={fireScore} assocQuality={assocScore} "
            f"exactRubric={exactRubricScore} fireRubric={fireRubricScore} assocRubric={assocRubricScore} "
            f"exactMs={float(exact['latencyMs']):.2f} fireMs={float(fire['latencyMs']):.2f} assocMs={float(assoc['latencyMs']):.2f} "
            f"fireDup={fire['duplicates']} fireTable={fire['tables']} "
            f"assocDup={assoc['duplicates']} assocTable={assoc['tables']}"
        )
        ...
        printRows(model, assoc["rows"])  # type: ignore[arg-type]

    print("\n" + "=" * 88)
    print(
        f"[summary] exactQuality={exactTotal} fireQuality={fireTotal} assocQuality={assocTotal} "
        f"fireImproved={fireImproved} fireSame={fireSame} fireWorsened={fireWorsened} "
        f"assocImproved={assocImproved} assocSame={assocSame} assocWorsened={assocWorsened} "
        f"exactRubric={exactRubricTotal} fireRubric={fireRubricTotal} assocRubric={assocRubricTotal} "
        f"fireRubricImproved={fireRubricImproved} fireRubricSame={fireRubricSame} fireRubricWorsened={fireRubricWorsened} "
        f"assocRubricImproved={assocRubricImproved} assocRubricSame={assocRubricSame} assocRubricWorsened={assocRubricWorsened} "
        f"exactAvgMs={sum(exactLatency) / len(exactLatency):.2f} "
        f"fireAvgMs={sum(fireLatency) / len(fireLatency):.2f} "
        f"assocAvgMs={sum(assocLatency) / len(assocLatency):.2f} "
        f"fireDuplicateTop5={fireDuplicateTotal} fireTableTop5={fireTableTotal} "
        f"assocDuplicateTop5={assocDuplicateTotal} assocTableTop5={assocTableTotal}"
    )


def main() -> None:
    print("=" * 88)
    print("Horizon Posting Meaning Search v11")
    print("=" * 88)
    print(
