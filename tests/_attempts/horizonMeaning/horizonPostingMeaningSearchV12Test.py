"""Horizon Posting Meaning Search v12 — slot-aware posting rerank.
...
---------
기본 샘플 실행:
    uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonPostingMeaningSearchV12Test.py
...
pairPostings 1,474, assocFocus 60, assocEdges 1,064, slotHits 55,257, rough memory 56.3MB.
exactQuality 79, fireQuality 80, assocQuality 79, slotQuality 79. evidence rubric 은
exactRubric 78, fireRubric 78, assocRubric 78, slotRubric 79. slotRubricImproved 1/same 7/worsened 0.
첫 구현은 검색 시점 문자열 검사로 slotAvgMs 34.97ms 였지만, slot hit 를 build 단계에
precompute 하니 slotAvgMs 8.38ms 로 내려갔다. buildSeconds 는 33.7s 로 v11 보다 증가.
결론: fire/association 보다 semantic slot coverage 가 strict 근거 품질을 실제로 올리는 첫 신호다.
다만 현재는 probe rubric 을 slot proxy 로 쓰는 upper-bound 이므로, 다음은 query 에서 slot 을 자동
추출하거나 query type 별 slot lexicon 으로 일반화해야 한다.
"""
...
def search(
    ...
    *,
    ...
    for unitId in list(scores):
        ...
        if useSlot:
            scores[unitId] += slotCoverageBonus(model, query, unitId, spec, coverage, pairHits)
...
def summarizeMode(
    ...
    *,
    ...
    t0 = time.perf_counter()
    rows = search(model, spec.query, useFire=useFire, useAssociation=useAssociation, useSlot=useSlot)
...
def evaluateExperiment(model: Model) -> None:
    print("\n" + "=" * 88)
    print("[evaluation] exact-only vs exact+fire vs exact+association vs exact+slot")
    print("=" * 88)
    ...
    totals = {name: 0 for name in ("exact", "fire", "assoc", "slot")}
    rubricTotals = {name: 0 for name in ("exact", "fire", "assoc", "slot")}
    latencies: dict[str, list[float]] = {name: [] for name in ("exact", "fire", "assoc", "slot")}
    duplicates = {name: 0 for name in ("fire", "assoc", "slot")}
    tables = {name: 0 for name in ("fire", "assoc", "slot")}
    verdicts = {name: {"improved": 0, "same": 0, "worsened": 0} for name in ("fire", "assoc", "slot")}
    rubricVerdicts = {name: {"improved": 0, "same": 0, "worsened": 0} for name in ("fire", "assoc", "slot")}
    ...
    for spec in PROBE_QUERIES:
        modes = {
            ...
            "slot": summarizeMode(model, spec, useFire=False, useSlot=True),
        }
        ...
        slotVerdict = classify(quality["slot"], quality["exact"], verdicts["slot"])
        slotRubricVerdict = classify(rubric["slot"], rubric["exact"], rubricVerdicts["slot"])
        ...
        print(
            ...
            f"slotDup={modes['slot']['duplicates']} slotTable={modes['slot']['tables']}"
        )
        ...
        print("  [exact+slot top5]")
        printRows(model, modes["slot"]["rows"])  # type: ignore[arg-type]

    print("\n" + "=" * 88)
    print(
        f"[summary] exactQuality={totals['exact']} fireQuality={totals['fire']} "
        f"assocQuality={totals['assoc']} slotQuality={totals['slot']} "
        ...
        f"slotImproved={verdicts['slot']['improved']} slotSame={verdicts['slot']['same']} slotWorsened={verdicts['slot']['worsened']} "
        f"exactRubric={rubricTotals['exact']} fireRubric={rubricTotals['fire']} "
        f"assocRubric={rubricTotals['assoc']} slotRubric={rubricTotals['slot']} "
        ...
        f"slotRubricImproved={rubricVerdicts['slot']['improved']} slotRubricSame={rubricVerdicts['slot']['same']} slotRubricWorsened={rubricVerdicts['slot']['worsened']} "
        ...
        f"slotAvgMs={sum(latencies['slot']) / len(latencies['slot']):.2f} "
        ...
        f"slotDuplicateTop5={duplicates['slot']} slotTableTop5={tables['slot']}"
    )


def main() -> None:
    print("=" * 88)
    print("Horizon Posting Meaning Search v12")
    print("=" * 88)
    print(
        ...
        f"slotWeight={SLOT_RERANK_WEIGHT:g}"
    )
