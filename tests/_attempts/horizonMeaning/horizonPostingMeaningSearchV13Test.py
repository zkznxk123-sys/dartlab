"""Horizon Posting Meaning Search v13 — query-type slot lexicon.
...
---------
기본 샘플 실행:
    uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonPostingMeaningSearchV13Test.py
...
pairPostings 1,474, assocFocus 60, assocEdges 1,064, proxy slotHits 55,257,
lexSlotHits 79,633, rough memory 56.7MB. exactQuality 79, fireQuality 80,
assocQuality 79, proxySlotQuality 79, lexSlotQuality 79. evidence rubric 은 exactRubric 78,
fireRubric 78, assocRubric 78, proxySlotRubric 79, lexSlotRubric 79.
lexSlotRubricImproved 1/same 7/worsened 0, lexSlotAvgMs 8.04ms, duplicate top5 0.
결론: 평가 rubric 을 직접 쓰는 proxySlot 이 아니라 query-type lexicon 으로 추론한 lexSlot 도
strict 근거 품질 개선을 재현했다. 이는 semantic slot coverage 가 단순 upper-bound 가 아니라
일반화 가능한 검색 신호일 가능성을 보인 첫 결과다. 다음은 hand lexicon 의존을 줄이고 corpus posting
통계에서 slot lexicon 을 자동 확장하는 방향이다.
"""
...
def evaluateExperiment(model: Model) -> None:
    print("\n" + "=" * 88)
    print("[evaluation] exact-only vs exact+fire vs exact+association vs proxySlot vs lexSlot")
    print("=" * 88)
    ...
    modeNames = ("exact", "fire", "assoc", "proxySlot", "lexSlot")
    compareNames = ("fire", "assoc", "proxySlot", "lexSlot")
    ...
    for spec in PROBE_QUERIES:
        ...
        print(
            ...
            f"assocQuality={quality['assoc']} proxySlotQuality={quality['proxySlot']} lexSlotQuality={quality['lexSlot']} "
            f"exactRubric={rubric['exact']} fireRubric={rubric['fire']} "
            f"assocRubric={rubric['assoc']} proxySlotRubric={rubric['proxySlot']} lexSlotRubric={rubric['lexSlot']} "
    ...
    print(
        ...
        f"assocQuality={totals['assoc']} proxySlotQuality={totals['proxySlot']} lexSlotQuality={totals['lexSlot']} "
        ...
        f"assocRubric={rubricTotals['assoc']} proxySlotRubric={rubricTotals['proxySlot']} lexSlotRubric={rubricTotals['lexSlot']} "
...
def main() -> None:
    print("=" * 88)
    print("Horizon Posting Meaning Search v13")
    print("=" * 88)
    print(
