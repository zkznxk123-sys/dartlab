"""Horizon Posting Meaning Search v14 — corpus-expanded slot lexicon.
...
---------
기본 샘플 실행:
    uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonPostingMeaningSearchV14Test.py
...
결과 기록
---------
기본 샘플 결과: 현재 data/dart/allFilings 는 content_html 스키마 1파일이라 v14 에서
section_content/content_html 양쪽을 읽도록 보강했다. 기본 40K 설정에서 실제 수집은
...
autoSlotQuality 71. evidence rubric 은 exactRubric 55, fireRubric 55, assocRubric 58,
proxySlotRubric 78, lexSlotRubric 71, autoSlotRubric 70. autoSlotRubricImproved 5/same 3/worsened 0,
autoSlotAvgMs 4.39ms, duplicate top5 0. 결론: corpus-expanded autoSlot 은 strict 근거 품질을
55→70 으로 크게 올리고 악화는 없지만, hand lexSlot 71 보다는 낮다. 자동 확장은 가능성이 있으나
확장 term 만으로 slot 을 인정하면 넓어지므로 seed hit 이 있는 unit 에서만 확장을 보너스로 인정하는
보수적 gating 이 필요하다. 다음은 slot 별 expansion threshold 를 다르게 잡거나 posting 통계로
false-positive term 을 제거하는 방향이다.
"""
...
def evaluateExperiment(model: Model) -> None:
    print("\n" + "=" * 88)
    print("[evaluation] exact-only vs exact+fire vs exact+association vs proxySlot vs lexSlot vs autoSlot")
    print("=" * 88)
    ...
    modeNames = ("exact", "fire", "assoc", "proxySlot", "lexSlot", "autoSlot")
    compareNames = ("fire", "assoc", "proxySlot", "lexSlot", "autoSlot")
...
def main() -> None:
    print("=" * 88)
    print("Horizon Posting Meaning Search v14")
    print("=" * 88)
    print(
        ...
        f"autoSlotFloor={AUTO_SLOT_SCORE_FLOOR:g}"
    )
