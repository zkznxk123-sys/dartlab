"""Horizon Posting Meaning Search v15 — pruned auto slot delta.
...
---------
기본 샘플 실행:
    uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonPostingMeaningSearchV15Test.py
...
결과 기록
---------
기본 샘플 결과: 현재 data/dart/allFilings 는 content_html 스키마 1파일이라 실제 수집은
21,055 units(allFilings 1,055 + docs 20,000), stems 35,721, postings 557,450,
focused pairs 84, pairPostings 689, assocFocus 91, assocEdges 1,590,
proxy slotHits 22,637, lexSlotHits 30,968, autoSlotHits 35,690, rough memory 26.5MB.
exactQuality 64, fireQuality 64, assocQuality 59, proxySlotQuality 78, lexSlotQuality 72,
autoSlotQuality 71, prunedAutoSlotQuality 72. evidence rubric 은 exactRubric 55,
fireRubric 55, assocRubric 58, proxySlotRubric 78, lexSlotRubric 71, autoSlotRubric 70,
prunedAutoSlotRubric 71. prunedAutoSlotRubricImproved 6/same 2/worsened 0,
prunedAutoSlotAvgMs 4.41ms, duplicate top5 0. 결론: autoSlot 을 lexSlot 위의 작은 delta 로
제한하면 v14 autoSlot 의 false-positive 손실을 회복해 hand lexSlot 과 같은 strict rubric 71 에 도달한다.
자동 확장 자체가 lexicon 을 넘어서지는 못했지만, posting association expansion 을 안전한 보조 신호로
쓰는 조건은 확인했다. 다음은 auto expansion term 을 slot 별로 학습/선별해 hand lexicon 의존도를 낮추는 방향이다.
"""
...
def iterAllFilingRows() -> tuple[str, Path, list[str]]:
    ...
    for path in sorted(ALLFILINGS_DIR.glob("*.parquet")):
        ...
        if "section_content" not in schema and "content_html" not in schema:
            print(f"[skip] allFilings {path.name}: no section_content/content_html column")
            continue
...
def collectUnits() -> list[Unit]:
    ...
    for rowSource, path, columns in iterAllFilingRows():
        ...
        for row in readRows(rowSource, path, columns):
            ...
            content = safeValue(row.get("section_content") or row.get("content_html"))
...
def evaluateExperiment(model: Model) -> None:
    print("\n" + "=" * 88)
    print(
        "[evaluation] exact-only vs exact+fire vs exact+association vs proxySlot vs lexSlot vs autoSlot vs prunedAutoSlot"
    )
    print("=" * 88)
    ...
    modeNames = ("exact", "fire", "assoc", "proxySlot", "lexSlot", "autoSlot", "prunedAutoSlot")
    compareNames = ("fire", "assoc", "proxySlot", "lexSlot", "autoSlot", "prunedAutoSlot")
    ...
    for spec in PROBE_QUERIES:
        ...
        print(
            ...
            f"prunedAutoSlotQuality={quality['prunedAutoSlot']} "
            ...
            f"prunedAutoSlotRubric={rubric['prunedAutoSlot']} "
    ...
    print(
        ...
        f"prunedAutoSlotQuality={totals['prunedAutoSlot']} "
        ...
        f"prunedAutoSlotRubric={rubricTotals['prunedAutoSlot']} "
...
def main() -> None:
    print("=" * 88)
    print("Horizon Posting Meaning Search v15")
    print("=" * 88)
    print(
        ...
        f"prunedMinSeed={PRUNED_AUTO_MIN_SEED_HITS}"
    )
