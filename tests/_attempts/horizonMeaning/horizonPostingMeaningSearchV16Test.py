"""Horizon Posting Meaning Search v16 — support-pruned auto slot.
...
---------
기본 샘플 실행:
    uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonPostingMeaningSearchV16Test.py
...
결과 기록
---------
기본 샘플 결과: 현재 data/dart/allFilings 는 content_xml 스키마라 v16 에서
section_content/content_html/content_xml 을 모두 읽도록 보강했다. 기본 40K 설정에서 실제 수집은
21,174 units(allFilings 1,174 + docs 20,000), stems 36,085, postings 560,373,
focused pairs 84, pairPostings 690, assocFocus 91, assocEdges 1,590,
proxy slotHits 22,869, lexSlotHits 31,246, autoSlotHits 35,939,
supportAutoSlotHits 33,849, rough memory 26.9MB.
exactQuality 64, fireQuality 64, assocQuality 63, proxySlotQuality 78, lexSlotQuality 72,
autoSlotQuality 71, prunedAutoSlotQuality 72, supportAutoSlotQuality 72. evidence rubric 은
exactRubric 56, fireRubric 56, assocRubric 59, proxySlotRubric 78, lexSlotRubric 71,
autoSlotRubric 70, prunedAutoSlotRubric 71, supportAutoSlotRubric 71.
supportAutoSlotRubricImproved 6/same 2/worsened 0, supportAutoSlotAvgMs 3.74ms, duplicate top5 0.
결론: seed slot unit 과의 co-occurrence support 로 확장어를 prune 하면 autoSlot 의 손실
70→71 을 회복하고 hand lexSlot/prunedAutoSlot 과 같은 strict rubric 에 도달한다. support pruning 은
autoSlotHits 를 35,939→33,849 로 줄이면서 품질을 유지했으므로, 확장어 선별 방향은 유효하다.
다음은 slot 별 support threshold 를 다르게 잡아 hand lexicon 을 줄이면서도 proxySlot 과의 격차를 좁히는 방향이다.
"""
...
def evaluateExperiment(model: Model) -> None:
    print("\n" + "=" * 88)
    print(
        "[evaluation] exact-only vs exact+fire vs exact+association vs proxySlot vs lexSlot vs autoSlot vs prunedAutoSlot vs supportAutoSlot"
    )
    print("=" * 88)
    ...
    modeNames = ("exact", "fire", "assoc", "proxySlot", "lexSlot", "autoSlot", "prunedAutoSlot", "supportAutoSlot")
    compareNames = ("fire", "assoc", "proxySlot", "lexSlot", "autoSlot", "prunedAutoSlot", "supportAutoSlot")
...
def main() -> None:
    print("=" * 88)
    print("Horizon Posting Meaning Search v16")
    print("=" * 88)
    print(
        ...
        f"supportPrecision={SUPPORT_AUTO_MIN_PRECISION:g}"
    )
