"""Horizon Posting Meaning Search v17 — trigger-only support slot.
...
---------
기본 샘플 실행:
    uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonPostingMeaningSearchV17Test.py
...
- build: stems 36,085, postings 560,373, assocEdges 1,590,
  ...
  triggerSeedSlotHits 11,447, triggerSupportSlotHits 15,884,
  roughMemoryMb 27.1, buildSeconds 22.7.
- strict rubric: exact 56, fire 56, assoc 59, proxySlot 78, lexSlot 71,
  autoSlot 70, prunedAutoSlot 71, supportAutoSlot 71, triggerSupportSlot 57.
- triggerSupportSlot 은 improved/same/worsened = 1/7/0, avg 4.27ms 로 악화는 막았지만
  lexSlot/supportAutoSlot 의 71점에 크게 못 미쳤다.

결론:
query 에 직접 나타난 trigger 단어만 seed 로 쓰면 hand evidence lexicon 의 의존성을 줄일 수는
있지만, 의미 slot 을 충분히 복원하지 못한다. trigger-only support 는 안전한 약한 delta 로
두면 exact 대비 +1 rubric 의 미세 개선만 만든다. 다음 단계는 trigger 를 버리는 것이 아니라
corpus association 으로 slot 후보를 만든 뒤, slot 별 precision/support 를 더 강하게 학습해
hand lexSlot 에 가까운 구조를 자동화하는 것이다.
"""
...
def evaluateExperiment(model: Model) -> None:
    print("\n" + "=" * 88)
    print(
        "[evaluation] exact-only vs exact+fire vs exact+association vs proxySlot vs lexSlot vs autoSlot vs prunedAutoSlot vs supportAutoSlot vs triggerSupportSlot"
    )
    print("=" * 88)
    ...
    for spec in PROBE_QUERIES:
        ...
        print(
            ...
            f"triggerSupportSlotQuality={quality['triggerSupportSlot']} "
            ...
            f"triggerSupportSlotRubric={rubric['triggerSupportSlot']} "
    ...
    print(
        ...
        f"triggerSupportSlotQuality={totals['triggerSupportSlot']} "
        ...
        f"triggerSupportSlotRubric={rubricTotals['triggerSupportSlot']} "
...
def main() -> None:
    print("=" * 88)
    print("Horizon Posting Meaning Search v17")
    print("=" * 88)
    print(
