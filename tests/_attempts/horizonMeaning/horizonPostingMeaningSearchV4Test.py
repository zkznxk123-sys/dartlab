"""Horizon Posting Meaning Search v4 — 관계형 scoring + table penalty + gated fire.
...
5. v3 에서 남은 문제인 약한 exact 후보 fire boost 를 relation pair/proximity/table penalty 로 제어한다.

실행 코드
---------
기본 샘플 실행:
    uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonPostingMeaningSearchV4Test.py
...
---------
2026-05-28 기본 샘플 실행:
- command: uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonPostingMeaningSearchV4Test.py
...
- speed: build 31.3s / total 46.9s / exact-only avg 46.08ms / exact+fire avg 89.95ms
- quality heuristic: exactQuality 79, fireQuality 79, improved 0, same 8, worsened 0
- 해석:
  - relation scoring + table penalty 로 v3 의 "매출채권 회수 지연" 악화는 제거됐다.
  - exact baseline 품질도 relation pair heuristic 덕분에 77 → 79 로 올랐다.
  - fire 는 더 이상 악화하지 않지만, 아직 top-5 품질을 개선하지도 못한다.
  - relation/table 계산 때문에 exact-only latency 가 8.58ms → 46.08ms 로 상승했다.
- 다음 개선:
  - relation scoring 은 효과가 있으므로 유지하되, candidate 상위 N 에만 적용해 latency 를 낮춘다.
  - fire 는 품질 개선이 없으므로 점수 가산보다 explanation/rationale 또는 tie-breaker 로 제한한다.
  - table penalty 는 row 구조를 보존한 meaning unit 에서는 penalty 가 아니라 별도 type scoring 으로 분리해야 한다.
"""
...
def search(
    ...
    for unitId in list(scores):
        ...
        if (
            useFire
            and expanded
            and unitId in fireCandidateUnitIds
            and coverage >= 0.67
            and (adjacentPairs or nearPairs)
        ):
        ...
        relationFactor = 1.0 + 0.35 * adjacentPairs + 0.15 * nearPairs
        scores[unitId] += min(MAX_FIRE_BOOST, SEMANTIC_WEIGHT * semanticBoost * coverage * relationFactor)
        ...
        if queryText and queryText in titleSurface:
            scores[unitId] += 8.0
        if isTableLike(unit) and coverage < 1.0 and adjacentPairs == 0:
...
def main() -> None:
    print("=" * 88)
    print("Horizon Posting Meaning Search v4")
    print("=" * 88)
    print(
        ...
        f"maxFireBoost={MAX_FIRE_BOOST:g} tablePenalty={TABLE_PENALTY:g} relSpan={RELATION_SPAN_LIMIT}"
    )
