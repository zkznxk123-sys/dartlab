"""Horizon Posting Meaning Search v5 — fast relation rerank + fire tie-breaker/explanation.
...
5. v4 에서 확인한 relation scoring 을 상위 후보에만 적용해 속도를 낮춘다.
6. fire pattern 은 점수 주역이 아니라 tie-breaker 와 explanation stem 으로만 사용한다.

실행 코드
---------
기본 샘플 실행:
    uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonPostingMeaningSearchV5Test.py
...
---------
2026-05-28 기본 샘플 실행:
- command: uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonPostingMeaningSearchV5Test.py
...
- noise: noisy stems 1,185 / 71,755 = 1.7%
- top5 artifact: exact+fire duplicate count 0, table-like unit count 12
- speed: build 22.1s / total 32.1s / exact-only avg 16.52ms / exact+fire avg 60.52ms
- quality heuristic: exactQuality 79, fireQuality 79, improved 0, same 8, worsened 0
- 해석:
  - relation scoring 을 상위 220 후보로 제한하니 v4 exact 46.08ms → v5 exact 16.52ms 로 감소했다.
  - 품질은 v4 와 같은 79/79 를 유지했고, fire 로 악화되는 질의는 없다.
  - fire 는 여전히 top-5 품질 개선 신호가 없지만, semantic explanation stem 을 붙이는 용도로는 안전해졌다.
  - exact-only 만으로 40K units 에서 8개 probe 모두 근거 unit 을 안정적으로 반환한다.
- 다음 개선:
  - fire path 는 검색 ranking 이 아니라 explanation/tie-breaker 로 분리 유지한다.
  - relation scoring 을 posting sequence 에 precomputed pair channel 로 옮겨 10ms 이하를 목표로 한다.
  - table-like unit 은 penalty 가 아니라 type-aware result group 으로 분리해 표 행 meaning unit 을 살린다.
"""
...
PROBE_QUERIES = [
    ...
    QuerySpec(
        "전환사채 발행", ("전환사채", "사채"), ("발행", "청약", "신주인수권"), (("전환사채", "발행"), ("사채", "발행"))
    ),
    QuerySpec("배당 지급", ("배당", "지급"), ("현금", "주주총회", "결의"), (("배당", "지급"), ("현금", "배당"))),
]
...
def search(
    ...
    relationCandidateUnitIds = {unitId for unitId, _ in scores.most_common(max(RELATION_RERANK_CANDIDATES, topK * 6))}
    ...
    for unitId in list(scores):
        ...
        adjacentPairs = 0
        nearPairs = 0
        if unitId in relationCandidateUnitIds:
            adjacentPairs, nearPairs = relationStats(sequence, queryIds)
        scores[unitId] += 30.0 * coverage
        if unitId in relationCandidateUnitIds:
            scores[unitId] += proximityBonus(sequence, queryIds)
            scores[unitId] += min(RELATION_PAIR_BONUS, 3.0 * adjacentPairs + 1.5 * nearPairs)
        ...
        if queryText and queryText in titleSurface:
            scores[unitId] += 8.0
        if unitId in relationCandidateUnitIds and isTableLike(unit) and coverage < 1.0 and adjacentPairs == 0:
            scores[unitId] -= TABLE_PENALTY
        if useFire and expanded and unitId in fireCandidateUnitIds and coverage >= 0.67:
...
def main() -> None:
    print("=" * 88)
    print("Horizon Posting Meaning Search v5")
    print("=" * 88)
    print(
        ...
        f"relSpan={RELATION_SPAN_LIMIT} relCandidates={RELATION_RERANK_CANDIDATES:,}"
    )
