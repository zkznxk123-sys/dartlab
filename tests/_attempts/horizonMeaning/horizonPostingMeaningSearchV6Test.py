"""Horizon Posting Meaning Search v6 — precomputed pair relation channel.
...
5. v5 의 relation rerank 를 검색 시점 sequence scan 에서 빌드 시점 pair posting 으로 옮긴다.
6. fire pattern 은 계속 ranking 주역이 아니라 tie-breaker 와 explanation stem 으로만 사용한다.

실행 코드
---------
기본 샘플 실행:
    uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonPostingMeaningSearchV6Test.py
...
---------
2026-05-28 기본 샘플 실행:
- command: uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonPostingMeaningSearchV6Test.py
...
- pair channel: pairWindow 10 / pairMaxUnits 600 / pairs 2,433,644 / pairPostings 6,978,268
- rough memory: 82.2MB / noisy stems 1,185 / 71,755 = 1.7%
- top5 artifact: exact+fire duplicate count 0, table-like unit count 14
- speed: build 33.7s / total 43.6s / exact-only avg 5.19ms / exact+fire avg 49.94ms
- quality heuristic: exactQuality 77, fireQuality 79, improved 1, same 7, worsened 0
- 해석:
  - precomputed pair relation channel 로 exact search 는 v5 16.52ms → v6 5.19ms 로 목표한 10ms 이하에 진입했다.
  - pair cap/window 때문에 exactQuality 는 v5 79 → v6 77 로 내려갔다.
  - fire tie-breaker/explanation 이 "배당 지급" 에서 품질을 회복해 fireQuality 는 79 로 유지됐다.
  - pair channel 은 빠르지만 build time 과 memory 가 증가한다. v5 55.6MB → v6 82.2MB.
- 다음 개선:
  - pair channel 을 모든 pair 로 만들지 말고 query/도메인 관계 stem 중심으로 압축한다.
  - pair posting 은 list 대신 sorted array/CSR 로 저장해 memory 와 build time 을 줄인다.
  - exact+relation 5ms 수준은 긍정적이므로 fire 는 계속 tie-breaker/explanation 으로만 유지한다.
"""
...
PAIR_INDEX_WINDOW = int(os.environ.get("DARTLAB_HORIZON_PAIR_WINDOW", "10"))
PAIR_INDEX_MAX_UNITS = int(os.environ.get("DARTLAB_HORIZON_PAIR_MAX_UNITS", "600"))
...
def buildModel(units: list[Unit]) -> Model:
    ...
    print(
        ...
        f"pairs={len(pairUnitsBuild):,} pairPostings={sum(len(v) for v in pairUnitsBuild.values()):,} "
        f"{elapsed:.1f}s"
    )
    return Model(
        units=units,
...
def main() -> None:
    print("=" * 88)
    print("Horizon Posting Meaning Search v6")
    print("=" * 88)
    print(
