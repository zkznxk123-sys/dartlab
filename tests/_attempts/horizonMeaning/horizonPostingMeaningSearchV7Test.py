"""Horizon Posting Meaning Search v7 — relation-focused compressed pair channel.
...
5. v6 의 전체 pair posting 을 관계 stem 중심 pair posting 으로 압축한다.
6. fire pattern 은 계속 ranking 주역이 아니라 tie-breaker 와 explanation stem 으로만 사용한다.

실행 코드
---------
기본 샘플 실행:
    uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonPostingMeaningSearchV7Test.py
...
---------
2026-05-28 기본 샘플 실행:
- command: uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonPostingMeaningSearchV7Test.py
...
- speed: build 25.2s / total 35.0s / exact-only avg 6.51ms / exact+fire avg 48.01ms
- quality heuristic: exactQuality 77, fireQuality 79, improved 1, same 7, worsened 0
- 해석:
  - relation-focused pair channel 은 v6 의 pairPostings 6,978,268 → 1,474 로 압축했고 memory 를 82.2MB → 55.6MB 로 되돌렸다.
  - exact-only 는 6.51ms 로 10ms 이하를 유지했지만 exactQuality 는 v6 와 같은 77 이다.
  - fire tie-breaker/explanation 은 "배당 지급" 에서 exact 품질 손실을 회복해 fireQuality 79 를 유지했다.
  - relation pair 보너스를 더 키운 재측정은 fireQuality 79 → 77 로 악화되어, 보수 가중치가 더 안정적이다.
- 다음 개선:
  - exact 품질 79 회복은 pair 보너스 강화가 아니라 query-type aware scoring 으로 풀어야 한다.
  - focused pair channel 은 유지하고, "배당 지급" 같은 event query 는 report/section type boost 를 별도로 둔다.
  - fire 는 여전히 ranking 주역이 아니라 손실 회복용 tie-breaker/explanation 에서 가장 안전하다.
"""
...
def main() -> None:
    print("=" * 88)
    print("Horizon Posting Meaning Search v7")
    print("=" * 88)
    print(
