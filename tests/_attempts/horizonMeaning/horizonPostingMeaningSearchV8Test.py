"""Horizon Posting Meaning Search v8 — query-type aware report/section scoring.
...
6. event/risk/finance 같은 query type 별 report/section hint 를 점수에 반영한다.
7. fire pattern 은 계속 ranking 주역이 아니라 tie-breaker 와 explanation stem 으로만 사용한다.

실행 코드
---------
기본 샘플 실행:
    uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonPostingMeaningSearchV8Test.py
...
---------
2026-05-28 기본 샘플 실행:
- command: uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonPostingMeaningSearchV8Test.py
- input: allFilings 20,000 units + docs 20,000 units = 40,000 meaning units
- collect: duplicate unit skips 78,782
- build: stems 71,755 / postings 1,082,237 / horizon buckets 2,048
- focused pair channel: focusStems 27 / pairWindow 14 / pairMaxUnits 20,000 / pairs 152 / pairPostings 1,474
- rough memory: 55.6MB / noisy stems 1,185 / 71,755 = 1.7%
- top5 artifact: exact+fire duplicate count 0, table-like unit count 13
- speed: build 25.9s / total 36.6s / exact-only avg 10.55ms / exact+fire avg 61.11ms
- quality heuristic: exactQuality 79, fireQuality 80, improved 1, same 7, worsened 0
- 해석:
  - query-type aware report/section scoring 으로 v7 exactQuality 77 → v8 exactQuality 79 를 회복했다.
  - fire tie-breaker/explanation 은 "배당 지급" 에서 추가 개선을 만들어 fireQuality 80 으로 처음 80대에 진입했다.
  - focused pair channel 의 메모리 장점은 유지됐다. pairPostings 1,474, rough memory 55.6MB.
  - exact latency 는 10.55ms 로 10ms 근처다. 품질 회복 비용으로 v7 6.51ms 보다 느려졌다.
- 다음 개선:
  - query-type boost 를 더 싼 integer flag/channel 로 precompute 해 exact 10ms 이하를 안정화한다.
  - fire path 는 61.11ms 로 여전히 느리므로, explanation stem 을 top exact 후보에 대해서만 계산하도록 줄인다.
  - 현재까지 결론은 embedding 없이도 exact+relation+query-type 으로 근거 있는 semantic retrieval 이 가능하다는 쪽으로 강해졌다.
"""
...
def buildModel(units: list[Unit]) -> Model:
    ...
    unitTypeSurface = [compactText(f"{unit.reportName} {unit.sectionTitle}") for unit in units]
    ...
    return Model(
        ...
        unitTypeSurface=unitTypeSurface,
...
def queryTypeBoost(typeSurface: str, spec: QuerySpec | None, coverage: float) -> float:
    if spec is None or not spec.typeHints:
        return 0.0
    hits = 0
    ...
    if hits <= 0:
        return 0.0
    return min(10.0, hits * 2.5 * max(coverage, 0.5))
...
def search(
    ...
    for unitId in list(scores):
        ...
        if queryText and queryText in titleSurface:
            scores[unitId] += 8.0
        scores[unitId] += queryTypeBoost(model.unitTypeSurface[unitId], spec, coverage)
...
def estimateMemoryMb(model: Model) -> float:
    ...
    roughBytes = postingCells * 4 + unitChars * 2 + fireCells * 12 + pairCells * 4 + stemChars * 2
...
def main() -> None:
    print("=" * 88)
    print("Horizon Posting Meaning Search v8")
    print("=" * 88)
    print(
