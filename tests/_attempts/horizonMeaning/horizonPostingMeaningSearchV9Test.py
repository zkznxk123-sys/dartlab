"""Horizon Posting Meaning Search v9 — precomputed type channel + top-fire explanation.
...
---------
기본 샘플 실행:
    uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonPostingMeaningSearchV9Test.py
...
---------
2026-05-28 기본 샘플 실행:
- command: uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonPostingMeaningSearchV9Test.py
...
- type channel: sparse precomputed query-type boost / fireCandidates 100
- rough memory: 56.0MB / noisy stems 1,185 / 71,755 = 1.7%
- top5 artifact: exact+fire duplicate count 0, table-like unit count 13
- speed: build 27.0s / total 37.1s / exact-only avg 6.56ms / exact+fire avg 35.37ms
- quality heuristic: exactQuality 79, fireQuality 80, improved 1, same 7, worsened 0
- 해석:
  - query-type boost precompute 로 v8 exact 10.55ms → v9 exact 6.56ms 로 개선하면서 exactQuality 79 를 유지했다.
  - fireCandidates 60 은 fireQuality 79 로 낮아졌고, 100 이 fireQuality 80 과 속도 개선의 균형점이었다.
  - fire path 는 v8 61.11ms → v9 35.37ms 로 낮아졌지만 아직 검색 주경로로 쓰기에는 느리다.
  - 현재 구조는 embedding 없이 exact+relation+query-type 으로 근거 semantic retrieval 을 10ms 이하로 수행하고, fire 는 35ms대 tie-breaker/explanation 으로 의미 보강한다.
- 다음 개선:
  - fire explanation 을 top-5 또는 top-10 exact 후보에만 계산해 20ms 이하를 목표로 한다.
  - type channel 과 focused pair channel 을 CSR/array 형태로 바꾸면 production 후보 구조가 보인다.
  - 평가 heuristic 을 수동 라벨/fixture 로 분리해 품질 80 이 실제 판단과 일치하는지 검증한다.
"""
...
def main() -> None:
    print("=" * 88)
    print("Horizon Posting Meaning Search v9")
    print("=" * 88)
    print(
