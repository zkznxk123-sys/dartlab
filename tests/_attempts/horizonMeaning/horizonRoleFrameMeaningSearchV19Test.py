"""Horizon Role Frame Meaning Search v19 — contrastive role frame.
...
---------
기본 샘플 실행:
    uv run --no-sync python -X utf8 tests/_attempts/horizonMeaning/horizonRoleFrameMeaningSearchV19Test.py
...
- build: stems 42,745, postings 647,818, assocEdges 1,677,
  cohortSlotHits 471, roleFrameSlotHits 14,396, coreAnchorHits 4,754,
  roughMemoryMb 31.7, buildSeconds 29.5.
- strict rubric: exact 57, fire 57, assoc 64, proxySlot 78, lexSlot 72,
  ...
  cohortSlot 62, roleFrameSlot 71.
- roleFrameSlot 은 rubric improved/same/worsened = 6/2/0, avg 4.95ms,
  duplicateTop5 2, tableTop5 2. exact 대비 +14, cohortSlot 대비 +9 rubric 이다.

결론:
v19 의 핵심 신호는 "role frame 은 단순 cohort 확장보다 강하다" 이다. exact/cohort 만으로는
놓치던 유상증자 목적, 매출채권 회수 지연, 배당 지급까지 개선했고, HBM 오작동은 core anchor
게이트로 막았다. hand lexSlot/supportAutoSlot 72 에 1점 차이까지 접근했으므로,
embedding 없이도 역인덱스 + posting association + contrastive role frame 만으로 근거 있는
semantic retrieval 이 가능하다는 강한 중간 증거다.

남은 한계:
role marker 는 아직 작은 수동 도메인 문법이고, roleFrameSlotDuplicateTop5 2 는 배당 probe 에서
같은 유형 문장이 반복된 결과다. 다음 단계는 role marker 를 hand list 로 두지 않고, query core
anchor 주변의 fire/association pattern 으로 자동 role marker 를 학습하고, 같은 corp/report 반복
diversity penalty 를 추가하는 것이다.
"""
...
def evaluateExperiment(model: Model) -> None:
    print("\n" + "=" * 88)
    print(
        "[evaluation] exact-only vs exact+fire vs exact+association vs proxySlot vs lexSlot vs autoSlot vs prunedAutoSlot vs supportAutoSlot vs triggerSupportSlot vs cohortSlot vs roleFrameSlot"
    )
    print("=" * 88)
...
def main() -> None:
    print("=" * 88)
    print("Horizon Role Frame Meaning Search v19")
    print("=" * 88)
    print(
