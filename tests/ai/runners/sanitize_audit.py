...
hash 모드 (기본) — question 을 question_hash 로 대체
    uv run python tests/ai/runners/sanitize_audit.py --in data/audit/ai-ask/ --out /tmp/sanitized/ --mode hash

drop 모드 — question 필드 완전 삭제
    uv run python tests/ai/runners/sanitize_audit.py --in data/audit/ai-ask/ --out /tmp/sanitized/ --mode drop

mask 모드 — 종목명·이메일·URL 만 마스킹 (공개 블로그용)
    uv run python tests/ai/runners/sanitize_audit.py --in data/audit/ai-ask/ --out /tmp/sanitized/ --mode mask

check 모드 — 민감 토큰 검출 리포트만 (파일 미작성)
    uv run python tests/ai/runners/sanitize_audit.py --check data/audit/ai-ask/
