...
실행 모드:
    파일 모드 — 명시 파일 lint::
        ...
        python -X utf8 tests/audit/lint_camelcase_ast.py path/to/file.py [...] [--strict]
        python -X utf8 tests/audit/lint_camelcase_ast.py --changed         # git diff (staged + unstaged)
        python -X utf8 tests/audit/lint_camelcase_ast.py --all             # src/dartlab/ 전수
        python -X utf8 tests/audit/lint_camelcase_ast.py --all --no-shim   # P6 codemod 후 absolute

    Hook 모드 — PostToolUse 호환 stdin JSON::

        python -X utf8 tests/audit/lint_camelcase_ast.py --hook
