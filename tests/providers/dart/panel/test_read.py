"""panel _read 엔진 mirror — scope/anchor/long/wide 순수 단위 (데이터 0).

``providers/dart/panel/_read.py`` 의 1:1 mirror. scopeExpr(연결/별도)·anchorLatest(era drift
...
def test_scope_expr_consolidated_standalone() -> None:
    ...
    from dartlab.providers.dart.panel._read import scopeExpr
...
def test_anchor_latest_propagates_latest_label() -> None:
    ...
    from dartlab.providers.dart.panel._read import anchorLatest
...
def test_panel_dir_path_market_namespace() -> None:
    ...
    from dartlab.providers.dart.panel._read import _panelDir
...
def test_read_long_and_wide_none_when_absent() -> None:
    ...
    from dartlab.providers.dart.panel._read import readLong, readWide
