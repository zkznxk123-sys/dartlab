...
========================================
외부 `uv run python -X utf8 main.py` 에서 Company("005930").topics 한 줄이
...
원인:
    ...
    공개 facade None 리턴 → 후속 .columns 접근 크래시.

    이 테스트는 "topics 가 None 이 아니어야 한다" 는 불변을 PyPI 릴리즈 직전에
...
@pytest.fixture
def freshCompanyFacadeCache():
    """Company facade 캐시를 테스트 전후로 모두 비운다.
...
class TestFreshInstallSmoke:
    ...
    def test_topicsNotNone_coldCache(self, freshCompanyFacadeCache):
        """c.topics 가 cold cache 에서도 DataFrame — 과거 크래시 직접 재현 방어."""
        ...
        topics = c.topics
        assert topics is not None, "c.topics 가 None — fresh install 크래시 회귀"
        assert isinstance(topics, pl.DataFrame)
        assert topics.height > 0, "topics DataFrame 이 비어있음 — parquet 파싱 전멸"
        # chapter 컬럼이 있어야 section 구조가 정상
        assert "chapter" in topics.columns
        assert "topic" in topics.columns

    def test_showIS_coldCache(self, freshCompanyFacadeCache):
    ...
    def test_selectIS_renderHtml_hasPeriodColumns(self, freshCompanyFacadeCache):
