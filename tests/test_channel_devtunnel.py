...
class TestImports:
    def test_module_exports(self):
        ...
        from dartlab.channel import setupDevtunnel as FnFromInit

        assert ExFromInit is DevTunnelSetupError
        assert FnFromInit is setupDevtunnel

    def test_callables(self):
        assert callable(findDevtunnelBinary)
        assert callable(setupDevtunnel)
...
class TestFindBinary:
    def test_returns_none_when_absent(self, tmp_path, monkeypatch):
        ...
        assert findDevtunnelBinary() is None
...
class TestChannelCLI:
    def test_channel_command_registered(self):
        ...
        from dartlab.cli.commands.channel import configureParser, run

        assert callable(run)
        assert callable(configureParser)
