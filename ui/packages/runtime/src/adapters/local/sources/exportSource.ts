...
// silent fallback 금지(fetchJson.ts 규약) — 실패는 throw/정직 표기, 다른 소스 우회 0.
...
/**
 * local ExportPort 생성 — 엔진 /api/export/* 프록시.
 *
 * @param apiBase API 베이스(기본 '' = same-origin; dev 는 vite proxy).
 * @returns ExportPort.
 *
 * @example
 * const port = localExportPort('');
 */
export function localExportPort(apiBase: string): ExportPort {
	return {
		...
		async listTemplates(): Promise<ExcelTemplate[]> {
			const res = await getJson<{ templates: ExcelTemplate[] }>(apiBase, '/api/export/templates');
			return res?.templates ?? [];
		},

		async saveTemplate(template: ExcelTemplate): Promise<string> {
			const r = await fetch(`${apiBase}/api/export/templates`, {
				method: 'POST',
		...
		async deleteTemplate(id: string): Promise<boolean> {
			const r = await fetch(`${apiBase}/api/export/templates/${encodeURIComponent(id)}`, {
				method: 'DELETE'
			});
		...
		async generate(input: ExportInput): Promise<ExportArtifact> {
			...
			const r = await fetch(`${apiBase}/api/export/excel`, {
				method: 'POST',
				headers: { 'content-type': 'application/json' },
