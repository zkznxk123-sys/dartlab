	...
	{#if isMobile && sidebarOpen}
		...
		<div class="dl-mobile-sidebar">
			<Sidebar
				...
				onDuplicate={(id) => { const newId = store.duplicateConversation?.(id); if (newId) store.setActive(newId); sidebarOpen = false; }}
				onOpenSettings={() => { showSettings = true; sidebarOpen = false; }}
				{version}
			/>
		</div>
	{/if}
