...
<div class="relative flex flex-col h-full min-h-0">
	...
	<div class="flex-shrink-0 px-3 sm:px-5 pb-4 pt-2">
		<div class="w-full sm:max-w-[720px] mx-auto">
			...
			{/if}
			<!-- 모델 스위처 chip (composer 위) — PR-7 cosmetic 1 차. 클릭 시 설정 패널 오픈 -->
			{#if (providerLabel || modelLabel) && onOpenSettings}
				<button
					type="button"
					class="mb-2 inline-flex items-center gap-1.5 self-start rounded-full border border-dl-border/40 bg-dl-bg-card/45 px-2.5 py-1 text-[10px] font-medium text-dl-text-dim transition hover:border-dl-primary/30 hover:text-dl-text"
					onclick={onOpenSettings}
					title="모델 변경"
				>
					<span class="h-1.5 w-1.5 rounded-full bg-dl-accent"></span>
